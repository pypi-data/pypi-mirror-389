...
class ArtifactsRequestBuilder(BaseRequestBuilder):
    ...
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/search/artifacts{?artifactId*,artifactType*,canonical*,contentId*,description*,globalId*,groupId*,labels*,limit*,name*,offset*,order*,orderby*}", path_parameters)
    ...
    async def post(self,body: bytes, content_type: str, request_configuration: Optional[RequestConfiguration[ArtifactsRequestBuilderPostQueryParameters]] = None) -> Optional[ArtifactSearchResults]:
        """
    ...
    def to_post_request_information(self,body: bytes, content_type: str, request_configuration: Optional[RequestConfiguration[ArtifactsRequestBuilderPostQueryParameters]] = None) -> RequestInformation:
        """
        Returns a paginated list of all artifacts with at least one version that matches theposted content.This operation can fail for the following reasons:* Provided content (request body) was empty (HTTP error `400`)* A server error occurred (HTTP error `500`)
        param body: Binary request body
        param content_type: The request body content type.
