...
class ExtractionTask:
    ...
    Attributes:
        ...
        node_type: 节点类型 ('extractor' | 'function' | 'data_source')
        func: 函数对象（仅当 node_type='function' 时）
        extractor_name: 提取器注册名（仅当 node_type='extractor' 时）
    """
    exp_id: int
    feature_name: str
    ...
    input_sources: List[str] = field(default_factory=list)  # 输入数据源（如 ['transfer']）
    inputs: List[str] = field(default_factory=list)  # 输入特征列表（如 ['gm_max', 'Von']）
    node_type: str = 'extractor'  # 'extractor' | 'function' | 'data_source'
    func: Optional[Any] = None  # 函数对象（node_type='function' 时）
    extractor_name: Optional[str] = None  # 提取器名（node_type='extractor' 时）
