...
class ConfigParser:
    ...
    def parse(self, experiment=None, unified_experiment=None) -> FeatureSet:
        ...
        Returns:
            FeatureSet 对象
        """
        # 自动导入指定的模块（用于注册自定义提取器）
        if self.config.import_modules:
            self._import_modules(self.config.import_modules)
    ...
    def _import_modules(self, module_list):
        """导入指定的模块列表
        ...
        for module_name in module_list:
            try:
                ...
                logger.warning(
                    f"⚠️ 无法导入模块 '{module_name}'：模块不存在。"
                    f"请确保模块在 Python 路径中或已安装。"
                )
            except Exception as e:
                logger.error(
                    f"❌ 导入模块 '{module_name}' 时发生错误: {e}"
                )
