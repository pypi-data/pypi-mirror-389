...
**方式 1：配置文件自动导入（推荐）✨ v2.0.1+**
```yaml
# my_features.yaml
import_modules:
  - autotau_extractors  # 自动导入此模块
  - my_custom_extractors

features:
  - name: tau_cycles
    extractor: transient.tau_on_off  # ✅ 已注册
    ...
```
- 优点：零代码、配置即文档、适合团队共享
- 使用：`FeatureSet.from_config('my_features.yaml', unified_experiment=exp)`

**方式 2：代码中手动导入（灵活）**
```python
import autotau_extractors  # ← 导入触发 @register 装饰器
from infra.features_v2 import FeatureSet

features = FeatureSet(...)
features.add('tau_cycles', extractor='transient.tau_on_off', ...)  # ✅ 已注册
```
- 优点：动态控制、适合交互式探索
- 使用场景：Jupyter notebook、临时测试

**方式 3：Lambda/Func（无需注册）**
```python
features.add(
    'my_custom_feature',
...
@register('my_category.my_feature')  # ← 装饰器在模块导入时执行
class MyExtractor(BaseExtractor):
    def extract(self, data, params):
        return np.array(...)  # 返回 shape 符合 output_shape

    @property
    def output_shape(self):
        return ('n_steps',)  # 或 ('n_steps', n_features)
```
- `@register` 只在模块**被导入时**执行（Python 装饰器机制）
- 因此必须确保模块被导入（通过上述三种方式之一）
