...
class ServiceTaskGilSpec:
    ...
    def GetGilSpecTable(fiboStart_ : int =None, tableSize_ : int =None) -> Dict:
        _bLnx = platform.system() == 'Linux'

        if fiboStart_ is None:
            fiboStart_ = 18 if _bLnx else 17
        if tableSize_ is None:
            tableSize_ = 9 if _bLnx else 10

        res = dict()
        for _nn in range(fiboStart_, fiboStart_+tableSize_):
            _cpu   = UserAppUtil.FibonacciCpuTimeUS(_nn)
            _freq  = int(11 * _cpu) // 1000
            _dfreq = _freq - max(1, int(_freq * 0.2))
            _spec  = ServiceTaskGilSpec(fiboInput_=_nn, runPhaseFreqMS_=_freq, deficientRunPhaseFreqMS_=_dfreq)
