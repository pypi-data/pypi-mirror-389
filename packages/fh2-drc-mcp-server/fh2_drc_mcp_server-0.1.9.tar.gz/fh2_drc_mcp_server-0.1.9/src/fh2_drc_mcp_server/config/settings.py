...
# Mock模式下大幅缩短等待时间（2秒），因为不需要真实的设备启动
if MOCK_MODE:
    TAKEOFF_WAIT_TIME = int(os.getenv("DRC_TAKEOFF_WAIT_TIME", "2"))
else:
...
def validate_config() -> None:
    ...
    if MOCK_MODE:
        ...
        print(f"   ⚡ 性能优化: 起飞等待 {TAKEOFF_WAIT_TIME}秒 (生产模式: 30秒)")
        print(f"   ⚡ 性能优化: 飞行到达检测立即返回 (生产模式: 轮询等待)")
        print(f"   📊 预期执行时间: fly_to_point_smart ≈ 2-3秒 (生产模式: 30-180秒)")
