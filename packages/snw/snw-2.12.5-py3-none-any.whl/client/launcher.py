...
class VersionAction(argparse.Action):
    ...
    def __call__(self, current_parser, namespace, values, option_string=None):
        ...
        r = requests.get(
            os.path.join(os.getenv('LAUNCHER_URL'), "version"),
            timeout=REQ_TIMEOUT
        )
...
exec_arguments = [
    ...
    ['--custom_options', {"default": "", "type": str, "help": "Additional docker options"}],
    ['-n', '--name',
...
parser_launch.add_argument('-t', '--docker_tag',
                           help='Docker image tag (is infered from docker_image if missing)')
parser_launch.add_argument('--custom_options', default= "", type= str, help= "Additional docker options")
...
def process_request(s_list, cmd, subcmd, is_json, args, auth=None):
    ...
    if cmd == "service" and subcmd == "list":
        params = {'all': args.all}
        r = requests.get(
            os.path.join(args.url, "service/list"),
            auth=auth,
            params=params,
            timeout=REQ_TIMEOUT
        )
        ...
        r = requests.get(
            os.path.join(args.url, "task/list", args.prefix + '*'),
            auth=auth,
            params={
                "with_parent": args.parent, "service": args.service, "status": args.status
            },
            timeout=REQ_TIMEOUT
        )
        if r.status_code != 200:
        ...
        r = requests.get(
            os.path.join(args.url, "service/describe", args.service),
            auth=auth,
            timeout=REQ_TIMEOUT
        )
        if r.status_code != 200:
        ...
        r = requests.get(
            os.path.join(args.url, "service/check", args.service),
            json=options,
            auth=auth,
            timeout=REQ_TIMEOUT
        )
        ...
        r = requests.get(
            os.path.join(args.url, "task/status", args.task_id),
            auth=auth,
            timeout=REQ_TIMEOUT
        )
        if r.status_code != 200:
        ...
        if not is_json:
            ...
            if confirm():
                for k in result:
                    r = requests.delete(
                        os.path.join(args.url, "task", k["task_id"]),
                        auth=auth,
                        timeout=REQ_TIMEOUT
                    )
                    if r.status_code != 200:
        ...
        r = requests.get(
            os.path.join(args.url, "task/terminate", args.task_id),
            auth=auth,
            timeout=REQ_TIMEOUT
        )
        if r.status_code != 200:
        ...
        r = requests.get(
            os.path.join(args.url, "task/file", args.task_id, args.filename),
            auth=auth,
            timeout=REQ_TIMEOUT
        )
        if r.status_code != 200:
        ...
        r = requests.get(
            os.path.join(args.url, "task/log", args.task_id),
            auth=auth,
            timeout=REQ_TIMEOUT
        )
        if r.status_code != 200:
