...
class RunXGBoostClassifierAPI(BaseEstimatorAPI):
    ...
    def predict(self, X_test, y_test):
        ...
        metric = self.metric
        if metric is None:
            metric = self.params.get("metric") if isinstance(self.params, dict) else getattr(self.params, "metric", None)
        ...
        if metric == "auroc":
            ...
            error_message = "Metric {} is not supported in {} yet".format(metric, self.clf_name)
