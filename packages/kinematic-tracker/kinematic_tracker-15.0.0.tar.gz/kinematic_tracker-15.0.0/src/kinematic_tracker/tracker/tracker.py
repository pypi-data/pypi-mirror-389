...
class NdKkfTracker:
    ...
    def set_association_metric(self, metric_name: str, mah_factor: float = 1.0) -> None:
    ...
    def set_ind_pos_size(self, ind_pos_size: Sequence[int]) -> None:
        ...
        """
        self.tracks.clear()
        self.association.set_ind_pos_size(ind_pos_size)
        self.metric_driver = self.association.get_metric_driver()
    ...
    def update_for_loose_reports(
        self,
        reports: np.ndarray[int, np.dtype[int]],
        det_rz: Sequence[np.ndarray],
        ids_r: Sequence[int],
    ) -> None:
    ...
    def advance(
        self, stamp_ns: int, det_rz: list[np.ndarray] | np.ndarray, ids_r: Sequence[int]
    ) -> list[NdKkfTrack]:
        ...
        Returns:
            list[NdKkfTrack]: Updated list of tracks.
        """
        ...
        self.correct_associated(reports, targets, score_rt, det_rz, ids_r)
        num_tracks_before = len(self.tracks)
        self.update_for_loose_reports(reports, det_rz, ids_r)
