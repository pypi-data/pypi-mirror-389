...
export class BackendCacheService {
  ...
  private static async makeRequest(
    ...
    const url = URLExt.join(settings.baseUrl, 'signalpilot-ai', 'cache', endpoint);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`Backend cache request failed: ${response.status} ${errorText}`);
    ...
    try {
      if (BackendCacheService.isChatHistoryKey(key)) {
        ...
        if (key === 'chatHistories') {
          const response = await BackendCacheService.makeRequest('chat-histories');
          return (response.chat_histories || defaultValue) as T;
        } else {
          // Individual chat history
          const response = await BackendCacheService.makeRequest(`chat-histories/${encodeURIComponent(key)}`);
          return (response.history || defaultValue) as T;
        }
      } else {
        // Handle app values
        const response = await BackendCacheService.makeRequest(`app-values/${encodeURIComponent(key)}`);
        return (response.value !== undefined ? response.value : defaultValue) as T;
      }
    } catch (error) {
      console.warn(
    ...
    try {
      if (BackendCacheService.isChatHistoryKey(key)) {
        ...
        if (key === 'chatHistories') {
          ...
          // Individual chat history
          await BackendCacheService.makeRequest(`chat-histories/${encodeURIComponent(key)}`, 'POST', {
            history: value
          });
        }
      } else {
        // Handle app values
        await BackendCacheService.makeRequest(`app-values/${encodeURIComponent(key)}`, 'POST', {
          value: value
        });
      }
    } catch (error) {
    ...
    try {
      if (BackendCacheService.isChatHistoryKey(key)) {
        ...
        if (key === 'chatHistories') {
          ...
          // Individual chat history
          await BackendCacheService.makeRequest(`chat-histories/${encodeURIComponent(key)}`, 'DELETE');
        }
      } else {
        // Handle app values
        await BackendCacheService.makeRequest(`app-values/${encodeURIComponent(key)}`, 'DELETE');
      }
    } catch (error) {
      console.error(
  ...
  public static async isAvailable(): Promise<boolean> {
    try {
      ...
      console.warn('[BackendCacheService] Service availability check failed:', error);
      return false;
    }
  }
