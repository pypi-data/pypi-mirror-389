...
class CodeToolsClient:
    ...
    def __init__(self, server_url: str = "http://localhost:18080", auto_start: bool = True,
        ...
        # Image freshness controls
        self.refresh_image = refresh_image
        self.pull_latest_base = pull_latest_base
...
# Always upgrade to the latest published aicodetools
        dockerfile_content = f"""FROM {self.docker_image}
RUN pip install --no-cache-dir --upgrade --break-system-packages aicodetools
...
"""
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Optionally force pulling latest base and avoid cache
                if self.pull_latest_base:
                    build_cmd.insert(2, '--pull')
    ...
    def _start_docker_server(self) -> bool:
        ...
        try:
            ...
            if self._prebuilt_image_exists() and not self.refresh_image:
                ...
                if not check_result.stdout.strip() or self.pull_latest_base:
                    # Base image doesn't exist or we want the latest - pull it
                    logging.info(f"Pulling base image: {self.docker_image}")
                    pull_result = subprocess.run(['docker', 'pull', self.docker_image],
                                               capture_output=True, text=True, timeout=180)
                    if pull_result.returncode != 0:
                        logging.error(f"Image '{self.docker_image}' not found/pull failed. Ensure it exists or use a standard Python image like 'python:3.11-slim'.")
                        return False
