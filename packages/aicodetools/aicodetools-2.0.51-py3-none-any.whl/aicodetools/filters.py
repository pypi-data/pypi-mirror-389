...
def _get_token_encoder(encoding_name: str = _ENCODING_NAME):
    ...
    if not HAS_TIKTOKEN:
        if _ENCODING is None or not isinstance(_ENCODING, SimpleWordEncoder):
            ...
            logging.info("Guardrail encoder: using fallback 'simple_word_encoder' (tiktoken unavailable)")
        return _ENCODING
    ...
    if (not _ENCODING_TRIED) or (_ENCODING_NAME != encoding_name):
        try:
            ...
            logging.info(f"Guardrail encoder: initialized tiktoken '{encoding_name}'")
        except Exception:
            # Fallback to words-as-tokens encoder on failure
            _ENCODING = SimpleWordEncoder()
            _ENCODING_NAME = "simple_word_encoder"
            logging.warning(f"Guardrail encoder: failed to init tiktoken '{encoding_name}', using fallback 'simple_word_encoder'")
        _ENCODING_TRIED = True
    return _ENCODING
