...
class RTMDet(nn.Module):
    ...
    def make_grid_priors(
        img_size: int,
        strides: List[int],
        offset: float = 0.0,  # 0.0 -> top-left corner; 0.5 -> cell center
        device: Optional[torch.device] = None,
    ) -> List[Tensor]:
        """
        Creates grid reference points (priors) for each FPN level.
        Assumes a square image (img_size x img_size)

        Each prior contains [x, y, stride]
        """
        # Create grid coordinates
        mlvl_priors = []

        for stride in strides:
            num_cells = img_size // stride

            grid = torch.arange(num_cells, device=device) * stride + offset * stride
            ...
            #   [ [0,64], [32,64], [64,64] ]]
            # ]
            block = torch.stack([gx, gy], dim=-1)  # [H, W, 2]

            # Add stride value for each point
            stride_tensor = torch.full((num_cells, num_cells, 1), stride, device=device)
            priors = torch.cat([block, stride_tensor], dim=-1)  # [H, W, 3]
    ...
    def decode(
        ...
        priors_per_level = self.make_grid_priors(
            self.img_size,
            self.strides,
            offset=0.0,
            device=device,
        ...
        for i, (cls, box) in enumerate(zip(cls_outputs, box_outputs)):
            ...
            priors = priors_per_level[i]  # [H, W, 3]
            grid_xy = priors[..., :2]  # [H, W, 2]

            # Adjust predicted offsets relative to grid position
            box[..., :2] = grid_xy - box[..., :2]  # top-left
            box[..., 2:4] = grid_xy + box[..., 2:4]  # bottom-right

            # Flatten spatial dimensions -> [B, H*W, 6]
        ...
        boxes[..., 0::2] = boxes[..., 0::2].clamp_(0, self.img_size - 1)
        boxes[..., 1::2] = boxes[..., 1::2].clamp_(0, self.img_size - 1)
