...
class Client(object):
    ...
    # Backwards-compat sync wrappers (use only if necessary)
    def send_code_sync(self, *args, **kwargs):
        return asyncio.get_event_loop().run_until_complete(self.send_code(*args, **kwargs))

    def sign_in_sync(self, *args, **kwargs):
        return asyncio.get_event_loop().run_until_complete(self.sign_in(*args, **kwargs))