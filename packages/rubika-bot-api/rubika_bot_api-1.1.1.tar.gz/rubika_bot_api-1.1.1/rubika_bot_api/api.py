...
class Robot:
    ...
    async def upload_file(self, upload_url: str, file_name: str, file_path: Union[str, Path]) -> str:
        ...
        if isinstance(file_path, str) and file_path.startswith("http"):
            # Async download using aiohttp to avoid blocking the event loop
            import tempfile
            is_temp = True
            ...
            async with aiohttp.ClientSession() as dl_sess:
                async with dl_sess.get(file_path) as resp:
                    if resp.status != 200:
                        ...
                        raise APIRequestError(f"Failed to download file from URL ({resp.status}): {text}")
                    # stream to temp file asynchronously
                    import aiofiles
