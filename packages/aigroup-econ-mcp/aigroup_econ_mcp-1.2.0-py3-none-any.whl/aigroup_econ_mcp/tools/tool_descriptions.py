...
输入方式优先级:
1. file_path: 文件路径 (推荐使用，支持CSV/JSON/TXT格式)
...
- 数据质量: 缺失值统计、异常值检测""",
    field_descriptions={
        "file_path": "CSV/JSON/TXT文件路径。支持相对路径和绝对路径，文件应包含数值型数据列",
        "file_content": "CSV/JSON/TXT文件内容字符串。适合小文件直接传输，避免路径依赖",
        "file_format": "文件格式: csv/json/txt/auto。auto模式自动检测文件格式",
...
- 置信区间和预测区间""",
    field_descriptions={
        "file_path": "CSV/JSON/TXT文件路径。CSV格式: 最后一列为因变量，其余列为自变量",
        "file_content": "CSV/JSON/TXT文件内容字符串。JSON格式: {'y': [因变量], 'x1': [自变量1], ...}",
        "file_format": "文件格式: csv/json/txt/auto。推荐使用CSV格式",
...
- 多变量预测建模""",
    field_descriptions={
        "file_path": "多变量时间序列文件路径。支持CSV/JSON/TXT格式",
