...
class OutputHumioHecExtraHTTPHeaderTypedDict(TypedDict):
    value: str
    name: NotRequired[str]


class OutputHumioHecExtraHTTPHeader(BaseModel):
    value: str
...
class OutputHumioHecResponseRetrySettingTypedDict(TypedDict):
    http_status: float
...
class OutputHumioHecResponseRetrySetting(BaseModel):
...
class OutputHumioHecTimeoutRetrySettingsTypedDict(TypedDict):
...
class OutputHumioHecTimeoutRetrySettings(BaseModel):
...
class OutputHumioHecTypedDict(TypedDict):
    ...
    extra_http_headers: NotRequired[List[OutputHumioHecExtraHTTPHeaderTypedDict]]
    r"""Headers to add to all events"""
    use_round_robin_dns: NotRequired[bool]
    r"""Enable round-robin DNS lookup. When a DNS server returns multiple addresses, @{product} will cycle through them in the order returned. For optimal performance, consider enabling this setting for non-load balanced destinations."""
    failed_request_logging_mode: NotRequired[OutputHumioHecFailedRequestLoggingMode]
    r"""Data to log when a request fails. All headers are redacted by default, unless listed as safe headers below."""
    safe_headers: NotRequired[List[str]]
    r"""List of headers that are safe to log in plain text"""
    format_: NotRequired[OutputHumioHecRequestFormat]
    r"""When set to JSON, the event is automatically formatted with required fields before sending. When set to Raw, only the event's `_raw` value is sent."""
    auth_type: NotRequired[OutputHumioHecAuthenticationMethod]
    r"""Select Manual to enter an auth token directly, or select Secret to use a text secret to authenticate"""
    response_retry_settings: NotRequired[
        List[OutputHumioHecResponseRetrySettingTypedDict]
    ]
    r"""Automatically retry after unsuccessful response status codes, such as 429 (Too Many Requests) or 503 (Service Unavailable)"""
    timeout_retry_settings: NotRequired[OutputHumioHecTimeoutRetrySettingsTypedDict]
    response_honor_retry_after_header: NotRequired[bool]
    r"""Honor any Retry-After header that specifies a delay (in seconds) no longer than 180 seconds after the retry request. @{product} limits the delay to 180 seconds, even if the Retry-After header specifies a longer delay. When enabled, takes precedence over user-configured retry options. When disabled, all Retry-After headers are ignored."""
    on_backpressure: NotRequired[OutputHumioHecBackpressureBehavior]
    r"""How to handle events when all receivers are exerting backpressure"""
    ...
    pq_compress: NotRequired[OutputHumioHecCompression]
    r"""Codec to use to compress the persisted data"""
...
class OutputHumioHec(BaseModel):
    ...
    extra_http_headers: Annotated[
        Optional[List[OutputHumioHecExtraHTTPHeader]],
    ...
    response_retry_settings: Annotated[
        Optional[List[OutputHumioHecResponseRetrySetting]],
        pydantic.Field(alias="responseRetrySettings"),
    ] = None
    r"""Automatically retry after unsuccessful response status codes, such as 429 (Too Many Requests) or 503 (Service Unavailable)"""

    timeout_retry_settings: Annotated[
        Optional[OutputHumioHecTimeoutRetrySettings],
