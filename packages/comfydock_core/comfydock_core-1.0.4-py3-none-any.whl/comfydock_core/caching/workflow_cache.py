...
def _get_version() -> str:
    """Get comfydock_core version."""
    try:
        return version('comfydock-core')
    except Exception:
...
class WorkflowCacheRepository:
    ...
    def get(
        ...
        query = """
            SELECT workflow_hash, dependencies_json, resolution_json,
                   resolution_context_hash, pyproject_mtime, comfydock_version
            FROM workflow_cache
        ...
        # Check version match
        if cached_row['comfydock_version'] != _get_version():
            elapsed = (time.perf_counter() - start_time) * 1000
    ...
    def set(
        ...
        comfydock_version = _get_version()
        ...
        query = """
            ...
            (environment_name, workflow_name, workflow_hash, workflow_mtime,
             ...
             comfydock_version, dependencies_json, resolution_json, cached_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
    ...
    def _compute_resolution_context_hash(
        ...
        context["comfydock_version"] = _get_version()
