...
class EasyCodeReaderServer:
        ...
        async def handle_list_tools() -> List[Tool]:
            ...
            return [
                ...
                Tool(
                    ...
                    description="递归列出指定 Java 项目的目录结构，只包含源代码文件（.java）、配置文件和文件夹，过滤编译产物",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "project_name": {
        ...
        async def handle_call_tool(name: str, arguments: Any) -> List[TextContent]:
            ...
            try:
                if name == "read_jar_source":
                    ...
                    return await self._list_project_tree(**arguments)
                else:
    ...
    async def _list_project_tree(self, project_name: str, project_dir: Optional[str] = None) -> List[TextContent]:
        """
        递归列出指定 Java 项目的目录结构
        只包含源代码文件（.java）、配置文件和文件夹，过滤编译产物和不必要的文件
        ...
        def build_tree(path: Path, relative_path: str = "") -> dict:
            ...
            参数:
                path: 当前路径
                relative_path: 相对于项目根目录的路径（用于在结果中显示）
            """
            ...
            try:
                ...
                for item in items:
                    ...
                    if item.is_dir():
                        ...
                        subtree = build_tree(item, child_relative)
                        
                        # 只有当子目录包含有用内容时才添加
                        if subtree["children"]:
                            node["children"][item.name] = subtree
                    else:
                        # 只包含指定的文件类型
        ...
        tree = {project_path.name: build_tree(project_path)}
        ...
        total_dirs, total_files = count_items(tree[project_path.name])
        ...
        result = {
            ...
            "summary": {
                ...
                "note": "仅包含源代码文件（.java）、配置文件和文件夹，已过滤编译产物"
            },
            "tree": tree
        }
