...
class EasyCodeReaderServer:
        ...
        async def handle_list_tools() -> List[Tool]:
            ...
            return [
                ...
                Tool(
                    ...
                    description="递归列出指定项目下所有文件和文件夹，返回带层级关系的目录树结构（含父子关联）",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "project_name": {
    ...
    async def _read_project_code(self, project_name: str, class_name: str, 
        ...
        search_patterns = [
            ...
            f"{class_path}.java",
            f"src/main/kotlin/{class_path}.kt",
            f"src/{class_path}.kt",
            f"{class_path}.kt",
        ]
    ...
    async def _list_project_tree(self, project_name: str, project_dir: Optional[str] = None) -> List[TextContent]:
        """
        递归列出指定项目下所有文件和文件夹，返回带层级关系的目录树结构（含父子关联）
                ...
                for item in sorted(path.iterdir(), key=lambda p: p.name):
                    if item.is_dir():
                        ...
                        try:
                            ...
                            node["children"][item.name] = {"type": "file", "path": str(item)}
            except Exception as e:
                logger.warning(f"遍历目录时出错: {e}")
            return node
