...
class LMA:
    @nvtx.annotate(
        "save_parameters", domain="cuPyLMA", category="torch", color="orange"
    )
    @torch.no_grad()
    ...
    @nvtx.annotate(
        "restore_parameters", domain="cuPyLMA", category="torch", color="orange"
    )
    @torch.no_grad()
    ...
    @nvtx.annotate(
        "replicate_model", domain="cuPyLMA", category="communication", color="red"
    )
    def _replicate_model(self, source_model, devices):
        ...
        for device, model in device_model_map.items():
            ...
            buffer["flat_params"] = torch.cat(
                [p.detach().flatten() for p in buffer["params"].values()]
            )
    ...
    @nvtx.annotate(
        "preprocess", domain="cuPyLMA", category="communication", color="red"
    )
    @torch.no_grad()
    def _preprocess(
        self, h_batch: torch.Tensor, slice_size: int
    ) -> Tuple[SlicedTensor, int]:
    ...
    def _forward(self, sliced_inputs: SlicedTensor) -> SlicedTensor:
        ...
        for tensor in sliced_inputs:
            ...
            outputs_list.append(
                torch.func.functional_call(model, buffer["params_and_buffers"], tensor)
            )
    ...
    def _compute_residuals(self, flat_params, input_tensor, target_tensor, residual_fn):
        ...
        output_tensor = torch.func.functional_call(
            model, params_and_buffers, input_tensor
        )
        ...
        if not configuration.FORCED_JACOBIAN_MODE:
            ...
            if mode == "forward":
                ...
                J = torch.autograd.functional.jacobian(
                    f, model_params, create_graph=True, vectorize=True
                )
                return J
            else:
    ...
    def _async_copy_to_host(
        self, d_tensor: torch.Tensor, stream: torch.cuda.Stream = None
    ...
    def _build_equation_no_overlap(
        self, sliced_inputs, sliced_targets, sliced_residuals, model_size, batch_size
    ):
        ...
        if overlap_d2h_h2d and not overlap_h2d:
            raise ValueError(
                "Enabling bi-direction overlap require host-to-device overlap"
            )
        ...
        if overlap_d2h_h2d:
            stream_device_map = {
                device: torch.cuda.Stream(device) for device in self.devices
            }
        else:
            stream_device_map = {
                device: torch.cuda.default_stream(device) for device in self.devices
            }

        mini_slice_start = 0
        for input_tensor, target_tensor, residual_tensor in zip(
            sliced_inputs, sliced_targets, sliced_residuals
        ):
            ...
            # Transfer residual
            with nvtx.annotate(
                f"residual_d2h[{mini_slice_start}:{mini_slice_end}]",
                domain="cuPyLMA",
                ...
                if overlap_h2d:
                    ...
                    h_target_tensor, copy_event = self._async_copy_to_host(
                        residual_tensor, d2h_stream
                    )
                    host_buffer.put(
                        (
                            r,
                    ...
                    r[mini_slice_start:mini_slice_end] = self._sync_copy_to_host(
                        residual_tensor
                    )

            # Compute slice
            with nvtx.annotate(
                f"jacobian[{mini_slice_start}:{mini_slice_end}]",
                domain="cuPyLMA",
                category="torch",
                color="orange",
            ):
                d_mini_slice = self._jacobian(
                    input_tensor, target_tensor, self.residual_fn
                )

            # Transfer slice
            with nvtx.annotate(
                f"jacobian_d2h[{mini_slice_start}:{mini_slice_end}]",
                domain="cuPyLMA",
                category="communication",
                color="red",
            ):
                if overlap_h2d:
                    ...
                    )  # Wait for completing Jacobian computation
                    h_mini_slice, copy_event = self._async_copy_to_host(
                        d_mini_slice.detach(), d2h_stream
                    )
                    host_buffer.put(
                        (J, h_mini_slice, mini_slice_start, mini_slice_end, copy_event)
                    )
                else:
                    J[mini_slice_start:mini_slice_end] = self._sync_copy_to_host(
                        d_mini_slice.detach()
                    )
        ...
        if overlap_h2d:
            while not host_buffer.empty():
                ...
                if e.query():
                    with nvtx.annotate(
                        f"jacobian_residual_h2d[{mini_slice_start}:{mini_slice_end}]",
                        domain="cuPyLMA",
                        category="communication",
                        color="red",
                    ):
        ...
        # Determine equation
        with nvtx.annotate(
            "build_equation", domain="cuPyLMA", category="cupynumeric", color="green"
        ):
            if batch_size > model_size:
                JJ = J.T @ J
                rhs = J.T @ r
        ...
        # Normalization
        with nvtx.annotate(
            "normalization", domain="cuPyLMA", category="cupynumeric", color="green"
        ):
    ...
    def __init__(
        ...
        self.device_model_map, self.device_buffer_map = self._replicate_model(
            model, devices
        )
        self.devices = devices
        self.num_devices = len(self.devices)
        self.model_size = next(iter(self.device_buffer_map.values()))[
            "flat_params"
        ].shape[0]
        self.model_dtype = next(iter(self.device_buffer_map.values()))[
            "flat_params"
        ].dtype
    ...
    def step(
        self, h_inputs: torch.Tensor, h_targets: torch.Tensor, slice_size: int = None
    ) -> bool:
        ...
        # Compute residuals
        with nvtx.annotate(
            "compute_residual", domain="cuPyLMA", category="torch", color="orange"
        ):
            residual_list = []
            for output_tensor, target_tensor in zip(sliced_outputs, sliced_targets):
        ...
        J, JJ, rhs = self._build_equation_no_overlap(
            sliced_inputs, sliced_targets, sliced_residuals, model_size, batch_size
        )
        ...
        for i in range(configuration.UPDATE_ATTEMPTS):
            ...
            with nvtx.annotate(
                f"damped_hessian",
                domain="cuPyLMA",
                category="cupynumeric",
                color="green",
            ):
                JJ_damped = JJ + self.damping_factor * np.eye(
                    JJ.shape[0], dtype=self.dtype
                )
            ...
            try:
                with nvtx.annotate(
                    f"solve", domain="cuPyLMA", category="cupynumeric", color="green"
                ):
                    eigens = np.linalg.eigvals(JJ_damped)
                    if not np.any(np.isclose(eigens, 0.0)):
            ...
            if solved:
                ...
                if new_loss_val < loss_val:
                    ...
                    self.damping_factor = max(
                        self.damping_factor * self.damping_down,
                        configuration.DAMPING_MIN,
                    )
                    self._save_parameters()
                    break
                else:
