...
def normalize_schema(aws_data: dict[str, Any], punctuate: bool = False) -> dict[str, Any]:
    """
    ...
    for item in aws_obj.results.items:
        if item.type == 'punctuation' and punctuate == True:    # Skips over punctuation items, which are not present in Deepgram outputs by default
            continue
