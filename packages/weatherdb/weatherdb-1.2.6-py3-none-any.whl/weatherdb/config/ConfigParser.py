...
class ConfigParser(configparser.ConfigParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._keyring_works = self._check_keyring_backend()
    ...
    def _check_keyring_backend(self):
        ...
        if keyring.get_keyring().priority <= 0:
            logger.warning(textwrap.dedent("""
                WARNING: No keyring backend available!
                It is not possible to store/retrieve database passwords securely.
                Please make sure your system has a working keyring backend.
                For more information, please visit https://pypi.org/project/keyring/"""))
            return False
        try:
            keyring.get_keyring().get_password("weatherDB_test", "test_user")
    ...
    def set_db_credentials(
        ...
        if not self._keyring_works:
            if self._check_keyring_backend():
                ...
                if db_key == "environment_variables":
                    ...
                    logger.warning("Keyring backend not working, cannot store password securely.")
    ...
    def get_db_credentials(self, db_key=None):
        ...
        db_pass = None
        if db_user is not None and self._keyring_works:
            db_pass = keyring.get_password(keyring_key, db_user)
