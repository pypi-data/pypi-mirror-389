...
class Node(BaseModel):
    context: Optional[str] = Field(None, validation_alias='@context', serialization_alias='@context', description='The context JSON-LD file')
    id: str = Field(validation_alias=AliasChoices("@id", "id"), serialization_alias='@id', description='Unique id assigned by HESTIA')
    flow_metadata: SkipJsonSchema[Optional[dict]] = Field(default={}, exclude=True, description="dev storage", repr=False)

    @property
    def get_conversion_factor(self) -> Optional[Decimal]:
        if (
            self.flow_metadata and
            isinstance(self.flow_metadata, dict) and
            self.flow_metadata.get('conversion_factor') and
            isinstance(self.flow_metadata.get('conversion_factor'), Decimal)
        ):
...
class BlankNode(BaseModel):
    flow_metadata: SkipJsonSchema[Optional[dict]] = Field(default_factory=dict, exclude=True, description="dev storage", repr=False)
...
class NodeRef(BaseModel):
    id: str = Field(validation_alias=AliasChoices("@id", "id"), serialization_alias='@id', description='Unique id assigned by HESTIA')
    type: SchemaType = Field(validation_alias=AliasChoices('@type', 'type'), serialization_alias='@type', description='Type of the Node')
...
from pydantic import BaseModel, Extra, Field, condecimal, constr
...
class Bibliography(BlankNode):
    ...
    year: condecimal(ge=0.0) = Field(..., description='The year the document or database was published, or the year this version of the database was released.', examples=[2018])
    volume: Optional[condecimal(ge=0.0)] = Field(None, description='The volume holding the document.', examples=[])
...
from pydantic import BaseModel, Extra, Field, PositiveFloat, condecimal, constr
...
class Site(Node):
    ...
    numberOfSites: Optional[condecimal(ge=1.0)] = Field(None, description='If data on multiple Sites are aggregated and represented as a single Site, the number of Sites.', examples=[])
    ...
    latitude: Optional[condecimal(ge=-90.0, le=90.0)] = Field(None, description='The latitude of the centroid of the Site (-90 to 90, WGS84 datum). Latitude should be provided with at least five decimal place precision (which roughly corresponds to 1 meter precision at the equator).', examples=[])
    longitude: Optional[condecimal(ge=-180.0, le=180.0)] = Field(None, description='The longitude of the centroid of the Site (-180 to 180, WGS84 datum). Longitude should be provided with at least five decimal place precision.', examples=[])
    ...
    glnNumber: Optional[constr(pattern='^\\d{13}$')] = Field(None, description='The Global Location Number.', examples=[])
    ...
    boundaryArea: Optional[condecimal(ge=0.0)] = Field(None, description='The area in km2 of the boundary, used for validation and gap filling. This field is automatically calculated when boundary is provided.', examples=[])
...
from pydantic import BaseModel, Extra, Field, PositiveFloat, condecimal, conint, constr
...
class Cycle(Node):
    ...
    cycleDuration: Optional[condecimal(ge=0.0, le=3650.0)] = Field(None, description='The duration of the Cycle in days, equal to endDate - startDate. For crop production Cycles, this should generally be 365 days or less, but to account for short fallows before the main production Cycle it can be longer, but must be 730 days or less.', examples=[213, 365])
    ...
    siteDuration: Optional[condecimal(ge=0.0)] = Field(None, description='The duration the Site is used for this Cycle in days. For temporary crops, the use period is defined as including the short fallow period and is therefore the period from harvest of the previous crop to harvest of the current crop (siteDuration = cycleDuration when startDateDefinition = harvest of previous crop). For permanent crops, the use period is equal to the cycleDuration. For animal production Cycles, this should be the time the animals are physically present on the Site and siteUnusedDuration should be used to record periods where the Site is un-grazed, fallow, or unused.', examples=[213, 300])
    otherSitesDuration: Optional[list[Optional[condecimal(ge=0.0)]]] = Field(None, description='If a Cycle occurred on multiple Sites, a corresponding array to otherSites with the number of days on each Site.', examples=[[65]])
    siteUnusedDuration: Optional[condecimal(ge=0.0)] = Field(None, description='During a Cycle, or over a number of Cycles, Sites are often unused for production for a period of time (e.g., the long fallow duration in crop production systems or periods when animals have been rotated to a different field). The unused duration is included in land occupation calculations. For cropping Cycles, siteUnusedDuration is calculated as the long fallow duration divided by the rotation duration multiplied by the siteDuration. It is therefore a time weighted allocation of the long fallow period to each Cycle in the crop rotation. For animal production Cycles, it is the period when animals are not present on the Site (e.g., if animals spend half the year in housing and half the year on pasture) it would be calculated 365/2 days. For animal production Cycles, if there are no other uses of a Site, it can simply be calculated as cycleDuration - siteDuration for each Site. For aquaculture production Cycles, it is the time between previous harvest and the current stocking (see Fallow period (aquaculture)).', examples=[65])
    otherSitesUnusedDuration: Optional[list[Optional[condecimal(ge=0.0)]]] = Field(None, description='If a Cycle occurred on multiple Sites, a corresponding array to otherSites with the number of days the Site was unused for.', examples=[[300]])
    siteArea: Optional[condecimal(ge=0.0)] = Field(None, description='The area of the site in hectares per functionalUnit. If the functionalUnit is 1 ha, this will default to 1. If the functionalUnit is relative this is the area of site required to produce the products from this Cycle. For cropland, this must be area under cultivation, also called sown area. Harvested area, which can be less than sown area, can be defined using the field harvestedArea.', examples=[1, 33.12])
    otherSitesArea: Optional[list[Optional[condecimal(ge=0.0)]]] = Field(None, description='If a Cycle occurred on multiple Sites, a corresponding array to otherSites with the area of each of each of the other Sites, in hectares, required to produce the products from this Cycle.', examples=[[0.28]])
    ...
    numberOfReplications: Optional[conint(ge=1)] = Field(None, description='In experimental studies, the number of replicates for the treatment.', examples=[])
    sampleWeight: Optional[conint(ge=0, le=100)] = Field(None, description='In surveys, the sample weight for this Cycle relative to all other Cycles included in the Source.', examples=[])
    ...
    aggregatedQualityScore: Optional[conint(ge=0)] = Field(None, description='A data quality score for aggregated data. The points depend on the primary Product of the aggregation. One point is awarded for each of the categories satisfied, according to the table below. For global aggregations, an additional point is awarded for both farm-stage and processing aggregations if the aggregation includes countries representing over 75% of production of the farm-gate product. The global aggregation is then scored out of 4 or 5 for processing and crop respectively.\n        Primary Product\n        Emissions\n        Number of Cycles\n        Yield\n        Completeness\n\n        Crop\n        No emissions included in the system boundary are missing\n        Over 50 Cycles\n        Within +/- 20% of FAOSTAT yield for the primary Product\n        True for all priority areas (animalFeed, cropResidue, electricityFuel, excreta, fertiliser, freshForage, ingredient, liveAnimalInput, otherChemical, pesticdieVetinaryDrug, product, seed, water)\n\n        Processed food\n        No emissions included in the system boundary are missing\n        Over 10 Cycles\n        -\n        True for all priority areas True for all priority areas (electricityFuel, ingredient, product, water)\n', examples=[1])
    aggregatedQualityScoreMax: Optional[conint(ge=3)] = Field(None, description='The maximum value for the aggregated quality score.', examples=[4])
...
class Practice(BlankNode):
    ...
    sd: Optional[list[Optional[condecimal(ge=0.0)]]] = Field(None, description='The standard deviation of value.', examples=[])
    ...
    observations: Optional[list[Optional[condecimal(ge=1.0)]]] = Field(None, description='The number of observations the descriptive statistics are calculated over.', examples=[])
    ...
    areaPercent: Optional[condecimal(ge=0.0)] = Field(None, description='The area of the Site that Practice occurred on, specified as a percentage of Site area. If the units of the term are already in % area, do not use this field and use value instead to record these data.', examples=[])
    price: Optional[condecimal(ge=0.0)] = Field(None, description='The price paid for this Practice. The price should be expressed per the units defined in the term. The currency must be specified. The price of the inputs associated with this practice should be included in the inputs rather than here.', examples=[])
    priceSd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of price.', examples=[])
    priceMin: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of price.', examples=[])
    priceMax: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of price.', examples=[])
    ...
    cost: Optional[condecimal(ge=0.0)] = Field(None, description='The total cost of this Practice (price x quantity), expressed as a positive value. The currency must be specified. The cost of the inputs associated with this practice should be included in the inputs rather than here.', examples=[])
    costSd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of cost.', examples=[])
    costMin: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of cost.', examples=[])
    costMax: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of cost.', examples=[])
    ...
    currency: Optional[constr(pattern='^[A-Z]{3}$')] = Field(None, description='The three letter currency code in ISO 4217 format.', examples=[])
    ...
    primaryPercent: Optional[condecimal(ge=0.0, le=100.0)] = Field(None, description='For primary processing operations in food processing Cycles, the percent of primary Product produced with the operation. For example, if the Cycle represents Oil palm, oil (crude) produced only with the operation Pressing, with screw press, the primaryPercent would be 100.', examples=[])
...
class Product(BlankNode):
    ...
    value: Optional[list[Optional[condecimal(ge=0.0)]]] = Field(None, description='The quantity of the Product. If an average, it should always be the mean. Can be a single number (array of length one) or an array of numbers with associated dates (e.g., for multiple harvests in one Cycle]). The units are always specified in the [Term. For crops, value should always be per harvest or per year, following FAOSTAT conventions.', examples=[[7282]])
    distribution: Optional[Union[list[list[DecimalValue]], list[DecimalValue]]] = Field(None, description='An array of up to 1000 random samples from the posterior distribution of valuedates field.', examples=[], max_items=1000)
    sd: Optional[list[Optional[condecimal(ge=0.0)]]] = Field(None, description='The standard deviation of value.', examples=[])
    min: Optional[list[Optional[DecimalValue]]] = Field(None, description='The minimum of value.', examples=[])
    max: Optional[list[Optional[DecimalValue]]] = Field(None, description='The maximum of value.', examples=[])
    ...
    price: Optional[condecimal(ge=0.0)] = Field(None, description='The sale price of this Product. The price should be expressed per the units defined in the term, for example per "kg liveweight". The currency must be specified.', examples=[])
    priceSd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of price.', examples=[])
    priceMin: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of price.', examples=[])
    priceMax: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of price.', examples=[])
    priceStatsDefinition: Optional[PriceStatsDefinition] = Field(None, description='What the descriptive statistics for price are calculated across.', examples=[])
    revenue: Optional[condecimal(ge=0.0)] = Field(None, description='The total revenue (price x quantity) of this Product. The currency must be specified.', examples=[])
    revenueSd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of revenue.', examples=[])
    revenueMin: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of revenue.', examples=[])
    revenueMax: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of revenue.', examples=[])
    ...
    currency: Optional[constr(pattern='^[A-Z]{3}$')] = Field(None, description='The three letter currency code in ISO 4217 format.', examples=[])
    economicValueShare: Optional[condecimal(ge=0.0, le=100.0)] = Field(None, description='The economic value (typically revenue) of this Product, divided by the total economic value of all Products, expressed as a percentage.', examples=[94.8])
...
from pydantic import BaseModel, Extra, Field, condecimal, conint, constr
...
class Term(Node):
    ...
    casNumber: Optional[constr(pattern='^[\\d\\-]+$')] = Field(None, description='The unique numerical identifier assigned by the Chemical Abstracts Service (CAS) to every chemical substance described in the open scientific literature.', examples=[])
    ...
    iccCode: Optional[conint(ge=0)] = Field(None, description='The Indicative Crop Classification code.', examples=[11])
    ...
    gadmLevel: Optional[conint(ge=0, le=5)] = Field(None, description='The level of the administrative region in the GADM database.', examples=[])
    ...
    gtin: Optional[constr(pattern='^[\\d]{8,14}$')] = Field(None, description='The Global Trade Item Number (GTIN) is an identifier for trade items, developed by GS1.', examples=[])
    ...
    latitude: Optional[condecimal(ge=-90.0, le=90.0)] = Field(None, description='The latitude (-90 to 90, WGS84 datum). If a polygon, the centroid.', examples=[46.55891593])
    longitude: Optional[condecimal(ge=-180.0, le=180.0)] = Field(None, description='The longitude (-180 to 180, WGS84 datum). If a polygon, the centroid.', examples=[2.553552532])
    area: Optional[condecimal(ge=0.0)] = Field(None, description='The area of the region in km2.', examples=[])
...
from pydantic import BaseModel, Extra, Field, condecimal, conint
...
class Indicator(BlankNode):
    ...
    sd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of value.', examples=[])
    ...
    statsDefinition: Optional[StatsDefinition] = Field(None, description='What the descriptive statistics (sd, min, max, and value) are calculated across, or whether they are simulated or the output of a model. spatial refers to descriptive statistics calculated across spatial units (e.g., pixels) within a region or country. time refers to descriptive statistics calculated across units of time (e.g., hours).', examples=[])
    observations: Optional[conint(ge=1)] = Field(None, description='The number of observations the descriptive statistics are calculated over.', examples=[])
...
from pydantic import BaseModel, Extra, Field, PositiveFloat, condecimal
...
class Measurement(BlankNode):
    ...
    depthUpper: Optional[condecimal(ge=0.0, le=9999.0)] = Field(None, description='For soil Measurements, the upper (shallower) depth of the Measurement interval in centimeters, using positive numbers.', examples=[])
    depthLower: Optional[condecimal(ge=0.0, le=9999.0)] = Field(None, description='For soil Measurements, the lower (deeper) depth of the Measurement interval in centimeters.', examples=[])
    latitude: Optional[condecimal(ge=-90.0, le=90.0)] = Field(None, description='The latitude of the Measurement if different from the centroid of the Site (-90 to 90, WGS84 datum).', examples=[])
    longitude: Optional[condecimal(ge=-180.0, le=180.0)] = Field(None, description='The longitude of the Measurement if different from the centroid of the Site (-90 to 90, WGS84 datum).', examples=[])
...
from pydantic import BaseModel, Extra, Field, condecimal
...
class Transport(BlankNode):
    ...
    value: Optional[condecimal(ge=0.0)] = Field(None, description='The distance transported times the number of tonnes transported. E.g., if 40 kg of Urea (kg N) are used during the Cycle, the nitrogen content of Urea is 45.5%, and the urea was transported 91 km, this field is calculated as 40 / 1000 / 45.5% * 91 = 8 tonne kilometers.', examples=[8])
    distribution: Optional[list[DecimalValue]] = Field(None, description='An array of up to 1000 random samples from the posterior distribution of value</code. This should describe the entire distribution of the dataset and not the distribution of the mean.', examples=[], max_items=1000)
    sd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of value.', examples=[])
    min: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of value.', examples=[])
    max: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of value.', examples=[])
    ...
    observations: Optional[list[Optional[condecimal(ge=1.0)]]] = Field(None, description='The number of observations the descriptive statistics are calculated over.', examples=[])
    distance: Optional[condecimal(ge=0.0)] = Field(None, description='The distance transported in kilometers.', examples=[])
    distanceSd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of distance.', examples=[])
    distanceMin: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of distance.', examples=[])
    distanceMax: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of distance.', examples=[])
    ...
    distanceObservations: Optional[list[Optional[condecimal(ge=1.0)]]] = Field(None, description='The number of observations the descriptive statistics for distance are calculated over.', examples=[])
...
class Completeness(BlankNode):
    ...
    animalFeed: bool = Field(..., description='\n        The types and quantities of all animal feed\n         used during the Cycle, including hay and silage, are specified.\n         Note that fresh forage has its own completeness field.\n       Set to true\n\n       No animal feed was used during the Cycle.\n       Set to true\n\n       Animal feed was used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    animalPopulation: bool = Field(..., description='\n       The types and quantities of all\n         live animals or\n         live aquatic species\n         that were present during the Cycle are specified in the animal node.\n       Set to true\n\n       No live animals were present during Cycle.\n       Set to true\n\n       Live animals were present during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    cropResidue: bool = Field(..., description='\n      The quantity of above and below ground\n       crop residue created\n       and its management are recorded.\n      Set to true\n\n      No crop residue was created or managed during the Cycle.\n      Set to true\n\n      Crop residue was created during the Cycle,\n        but the quantities and management are not fully recorded.\n      Set to false\n', examples=[])
    electricityFuel: bool = Field(..., description='\n       The types and quantities of all electricity and\n         fuel used during the Cycle,\n         excluding during the transport phase, are recorded.\n       Set to true\n\n       Electricity and fuel were not used during the Cycle.\n       Set to true\n\n       Electricity and/or fuel were used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[])
    excreta: bool = Field(..., description='\n      The types and quantities of excreta created\n        and its management are specified.\n\n      Set to true\n\n      No excreta was created or managed during the Cycle.\n      Set to true\n\n      Excreta was created during the Cycle,\n        but the quantities and management are not fully recorded.\n      Set to false\n', examples=[True])
    fertiliser: bool = Field(..., description='\n       The types and quantities of all\n         organic fertiliser and\n         inorganic fertiliser,\n         or the quantity of each fertiliser brand name,\n         used during the Cycle are recorded.\n       Set to true\n\n       No fertilisers were used during the Cycle.\n       Set to true\n\n       Fertilisers were used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    freshForage: bool = Field(..., description='\n        The types and quantities of all fresh forage\n         fed to, or grazed by, animals during the Cycle are recorded.\n       Set to true\n\n       No fresh forage was consumed during the Cycle.\n       Set to true\n\n       Fresh forage was consumed during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    ingredient: bool = Field(..., description='\n      For feed or food processing Cycles, the type and quantities of all\n         feed or food ingredients used, such as crop products,\n         animal products,\n         processed foods,\n         and/or feed or food additives are recorded.\n\n      Set to true\n\n      No feed or food ingredients were used during the Cycle.\n      Set to true\n\n      Food or feed ingredients were used during the Cycle,\n         but the types and quantities are not fully recorded.\n      Set to false\n', examples=[True])
    liveAnimalInput: bool = Field(..., description='\n       The types and quantities of all\n         live animals or\n         live aquatic species\n         which were Inputs into the Cycle are specified. For example,\n         piglets might be an Input into a pig\n         fattening Cycle.\n       Set to true\n\n       No live animals were Inputs into the Cycle.\n       Set to true\n\n       Live animals were Inputs into the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    material: bool = Field(..., description='\n       The types and quantities of all material\n         and substrate Inputs, which includes capital\n         equipment depreciated over the Cycle, are recorded.\n       Set to true\n\n       No material inputs were used during the Cycle.\n       Set to true\n\n       Material inputs were used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[])
    operation: bool = Field(..., description='\n       The types of all mechanical operation\n         performed during the Cycle and either their duration or\n         the percentage of area they covered are recorded.\n       Set to true\n\n       No mechanical operations were performed during the Cycle.\n       Set to true\n\n       Mechanical operations were performed during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[])
    otherChemical: bool = Field(..., description='\n       The types and quantities of all other chemicals (including\n         processing aids,\n         other inorganic chemicals, and\n         other organic chemicals) used during\n         the Cycle are recorded.\n       Set to true\n\n       No processing aids or other chemicals were used during the Cycle.\n       Set to true\n\n       Processing aids and/or other chemicals were used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    pesticideVeterinaryDrug: bool = Field(..., description='\n       The types and quantities of all pesticides (either as\n         active ingredients\n         or brand names) and\n         veterinary drugs used during\n         the Cycle are recorded.\n       Set to true\n\n       No pesticides or veterinary drugs were used during the Cycle.\n       Set to true\n\n       Pesticides and/or veterinary drugs were used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    product: bool = Field(..., description='\n      The types and quantities of all\n        crop,\n        live animal,\n        live aquatic species,\n        animal product, and\n        processed food\n        produced during the Cycle are recorded. In the case where\n        Products were intended to be produced but no production occurred (e.g., if crops\n        fail due to disease) the types of products should still be recorded and the\n        quantity set to zero.\n\n      Set to true\n\n      No Products were produced (or intended to be produced) during the Cycle.\n      Set to true\n\n      Products were produced during the Cycle,\n         but the types and quantities are not fully recorded.\n      Set to false\n', examples=[True])
    seed: bool = Field(..., description='\n      The types and quantities of all seed Inputs,\n       such as seed,saplings, or semen are recorded.\n      Set to true\n\n      No seed Inputs were used during the Cycle.\n      Set to true\n\n      Seed Inputs were used during the Cycle,\n         but the types and quantities are not fully recorded.\n      Set to false\n', examples=[])
    soilAmendment: bool = Field(..., description='\n       The types and quantities of all\n         soil amendments and\n         biochar used\n         during the Cycle are recorded.\n       Set to true\n\n       No soil amendments were used during the Cycle.\n       Set to true\n\n       Soil amendments were used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    transport: bool = Field(..., description='\n       The transport modes and distances\n         for each Input to the Site are recorded.\n         If Products were also Transported during this Cycle, the distances\n         and modes are specified.\n       Set to true\n\n       No Inputs were transported to the Site and\n         no Products were transported during the Cycle.\n       Set to true\n\n       Inputs and/or Products were transported, but data on the\n       modes and distances are not fully recorded.\n       Set to false\n', examples=[])
    waste: bool = Field(..., description='\n       The types and quantities all waste streams,\n         their and management, and their transport to where\n         they are managed are specified (note that crop residue\n         and excreta waste streams and management have their own\n         completeness fields).\n         Examples of waste streams include dead animals or plastic films for greenhouses.\n         Examples of management include disposal into a water body or bio-digestion.\n         Examples of transport include taking waste to a disposal center.\n\n       Set to true\n\n       No waste streams were created or managed during the Cycle.\n       Set to true\n\n       Waste was created or managed during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[])
    water: bool = Field(..., description='\n       The types and quantities of all water used\n         during the Cycle are recorded.\n       Set to true\n\n       No water was used during the Cycle or only very small quantities\n         were used but not recorded (e.g., water mixed with fertilisers or pesticides\n         for spraying).\n       Set to true\n\n       Water was used during the Cycle,\n         but the types and quantities are not fully recorded.\n       Set to false\n', examples=[True])
    schemaVersion: Optional[str] = Field(None, description='The version of the schema when these data were created.', examples=[])
    added: Optional[list[str]] = Field(None, description='A list of fields that have been added to the original dataset.', examples=[])
    addedVersion: Optional[list[str]] = Field(None, description='A list of versions of the model used to add these fields.', examples=[])
    updated: Optional[list[str]] = Field(None, description='A list of fields that have been updated on the original dataset.', examples=[])
...
class Animal(BlankNode):
    ...
    max: Optional[DecimalValue] = Field(None, description='The maximum of value.', examples=[])
    statsDefinition: Optional[StatsDefinition] = Field(None, description='What the descriptive statistics (sd, min, max, and value) are calculated across, or whether they are simulated or the output of a model. spatial refers to descriptive statistics calculated across spatial units (e.g., pixels) within a region or country. time refers to descriptive statistics calculated across units of time (e.g., hours).', examples=[])
    observations: Optional[condecimal(ge=1.0)] = Field(None, description='The number of observations the descriptive statistics are calculated over.', examples=[])
    price: Optional[condecimal(ge=0.0)] = Field(None, description='The price of the Animal. The price should be expressed per animal. The currency must be specified.', examples=[])
    currency: Optional[constr(pattern='^[A-Z]{3}$')] = Field(None, description='The three letter currency code in ISO 4217 format.', examples=[])
...
class Input(BlankNode):
    ...
    distribution: Optional[Union[list[list[DecimalValue]], list[DecimalValue]]] = Field(None, description='An array of up to 1000 random samples from the posterior distribution of valuedates field.', examples=[], max_items=1000)
    sd: Optional[list[Optional[condecimal(ge=0.0)]]] = Field(None, description='The standard deviation of value.', examples=[[180]])
    min: Optional[list[Optional[DecimalValue]]] = Field(None, description='The minimum of value.', examples=[])
    max: Optional[list[Optional[DecimalValue]]] = Field(None, description='The maximum of value.', examples=[])
    statsDefinition: Optional[StatsDefinition] = Field(None, description='What the descriptive statistics (sd, min, max, and value) are calculated across, or whether they are simulated or the output of a model. spatial refers to descriptive statistics calculated across spatial units (e.g., pixels) within a region or country. time refers to descriptive statistics calculated across units of time (e.g., hours).', examples=['replications'])
    observations: Optional[list[Optional[condecimal(ge=1.0)]]] = Field(None, description='The number of observations the descriptive statistics are calculated over.', examples=[[10]])
    ...
    price: Optional[condecimal(ge=0.0)] = Field(None, description='The price paid for this Input. The currency must be specified. The price should be expressed per the units defined in the term, for example per "kg active ingredient". In situations where a term describes one physical item (e.g., "NP Blend" fertiliser) but the glossary uses terms with unit that split the item (e.g., "NP Blend (kg N)" and "NP Blend (kg P2O5)") the price must be divided by the relative mass of each term.', examples=[])
    priceSd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of price.', examples=[])
    priceMin: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of price.', examples=[])
    priceMax: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of price.', examples=[])
    priceStatsDefinition: Optional[PriceStatsDefinition] = Field(None, description='What the descriptive statistics for price are calculated across.', examples=[])
    cost: Optional[condecimal(ge=0.0)] = Field(None, description='The total cost of this Input (price x quantity), expressed as a positive value. The currency must be specified.', examples=[])
    costSd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of cost.', examples=[])
    costMin: Optional[condecimal(ge=0.0)] = Field(None, description='The minimum of cost.', examples=[])
    costMax: Optional[condecimal(ge=0.0)] = Field(None, description='The maximum of cost.', examples=[])
    costStatsDefinition: Optional[CostStatsDefinition] = Field(None, description='What the descriptive statistics for cost are calculated across.', examples=[])
...
class Organisation(Node):
    ...
    boundaryArea: Optional[condecimal(ge=0.0)] = Field(None, description='The area in km2 of the boundary. This field is automatically calculated when boundary is provided.', examples=[])
    area: Optional[PositiveFloat] = Field(None, description='The area of the Organisation in hectares.', examples=[])
    latitude: Optional[condecimal(ge=-90.0, le=90.0)] = Field(None, description='The latitude of the Organisation (-90 to 90).', examples=[49.16472])
    longitude: Optional[condecimal(ge=-180.0, le=180.0)] = Field(None, description='The longitude of the Organisation (-180 to 180).', examples=[6.901544])
...
class ImpactAssessment(Node):
    ...
    aggregatedQualityScore: Optional[conint(ge=0)] = Field(None, description='A data quality score for aggregated data, set equal to the quality score of the linked Cycle.', examples=[])
    aggregatedQualityScoreMax: Optional[conint(ge=3)] = Field(None, description='The maximum value for the aggregated quality score, set equal tothe max quality score of the linked Cycle.', examples=[])
...
class Property(BlankNode):
    ...
    share: Optional[condecimal(ge=0.0, le=100.0)] = Field(100, description='The percentage of the Product or Input value the Property refers to (e.g., if 70% of the steers sold are 450 days old and the remaining 30% are 900 days old, the term Beef cattle, steer should be recorded once, with two Age properties associated with it: the first one with value 450 and share 70, the second one with value 900 and share 30).', examples=[])
    sd: Optional[condecimal(ge=0.0)] = Field(None, description='The standard deviation of value.', examples=[])
    min: Optional[DecimalValue] = Field(None, description='The minimum of value.', examples=[])
    max: Optional[DecimalValue] = Field(None, description='The maximum of value.', examples=[])
    statsDefinition: Optional[StatsDefinition] = Field(None, description='What the descriptive statistics (sd, min, max, and value) are calculated across, or whether they are simulated or the output of a model. spatial refers to descriptive statistics calculated across spatial units (e.g., pixels) within a region or country. time refers to descriptive statistics calculated across units of time (e.g., hours).', examples=[])
...
class Emission(BlankNode):
    ...
    depth: Optional[condecimal(ge=0.0, le=9999.0)] = Field(None, description='For soil Emissions, the depth at which the Emissions was recorded in centimeters.', examples=[])
