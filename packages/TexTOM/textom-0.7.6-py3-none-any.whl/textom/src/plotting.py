...
def plot_powder_pattern(cif_path, cutoff_structure_factor=1e-4, max_hkl=4, q_min=0, q_max=60, 
                        wavelength=None, q_dat=None, chi_dat=None, I_dat=None):
    q, hkl, S, M = cry.structure_factor_from_cif(cif_path, cutoff_structure_factor, max_hkl,  
    ...
    for (xi, yi, l, m) in zip(q, powder_intensities, hkl, M):
        ...
        print(f'[{l[0]:<2} {l[1]:<2} {l[2]:<2}]\t{xi:5.3}\t{100*yi/powder_intensities.max():.1f}\t\t{m}')
    # if np.any(q_dat):
    #     
    #     q_dat = q_dat[msk_q]
