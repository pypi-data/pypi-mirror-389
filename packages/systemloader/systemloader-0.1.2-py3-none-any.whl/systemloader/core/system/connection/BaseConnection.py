    ...
    for priority in set(value.get('priority', 0) for value in connections.values()):
        ...
        for connection_name, value in connections.items():
            ...
            if value.get('priority', 0) == priority:
                ...
                app_module_name = f"app.connection.{value['class']}"
                try:
                    module = import_module(app_module_name)
                except ModuleNotFoundError as err:
                    if err.name != app_module_name:
            ...
            for index, result in enumerate(await asyncio.gather(*tasks, return_exceptions=True)):
                if result is True:
                    ...
                    if isinstance(result, Exception):
                        logger.error(f"connection.{obj_list[index].connection_name}:")
                        # Выводим полный трейсбэк
                        for line in traceback.format_exception(type(result), result, result.__traceback__):
                            logger.error(line.strip())
                    else:
                        logger.error(f"connection.{obj_list[index].connection_name}: {result}")
                    return False
