...
class Events(FitnessFunc):
    ...
    def fitness(self, N_k, T_k):
        ...
        # If N_k is zero, the equation N_k * (np.log(N_k / T_k)) returns nan 
        # (since 0 * -inf is undefined), however the limit of x * log(x) 
        # when x -> 0 is 0, so we use this value for the fitness if N_k is zero.
        f = np.zeros_like(N_k) #- 20
        f[N_k > 0] = N_k[N_k > 0] * (np.log(N_k[N_k > 0] / T_k[N_k > 0]))
        return f 
