...
def best_pai_score_improved_this_epoch(tracker, first_call=True):
    ...
    for layer in tracker.neuron_module_vector:
        if GPA.pc.get_dendrite_learn_mode() and (
            ...
            print(
                "You set GPA.pc.get_initial_correlation_batches() to be greater than an entire epoch %d < %d.  This can result in weights not being updated.  You should set that GPA.pc.set_initial_correlation_batches(x) to be lower than the batches in one epoch. Start over or Load from 'latest' for %s. It was caught on layer%s"
                % (
                    layer.dendrite_module.dendrite_values[0].initialized,
                    GPA.pc.get_initial_correlation_batches(),
                    tracker.save_name,
