...
import requests
from mcp.server.fastmcp import FastMCP, ParamInfo
...
mcp = FastMCP(
      ...
      description="调用小牛翻译 API，提供多语种文本翻译及语种目录。"
  )
...
@mcp.tool(description="使用小牛翻译接口进行文本翻译。")
def translate_text(
    text: Annotated[str, ParamInfo(description="待翻译的原文文本。")],
    source: Annotated[str, ParamInfo(description="源语言代码或常见别名，例如 zh、中文、chinese。")],
    target: Annotated[str, ParamInfo(description="目标语言代码或常见别名，例如 en、英文、english。")],
) -> Dict[str, Any]:
    """使用小牛翻译接口进行文本翻译。

    Args:
        text: 待翻译的原文文本，可以是任意长度的字符串。
        source: 源语言代码或常见别名（例如 "zh"、"中文"、"chinese"）。
        target: 目标语言代码或常见别名（例如 "en"、"英文"、"english"）。

    Returns:
        包含标准化语种代码、原文、译文以及小牛接口返回的原始数据。
    """
