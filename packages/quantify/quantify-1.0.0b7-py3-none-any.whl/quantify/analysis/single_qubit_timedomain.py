...
from quantify.analysis.calibration import (
    has_calibration_points,
    rotate_to_calibrated_axis,
)
from quantify.visualization.mpl_plotting import (
    plot_fit,
    plot_textbox,
...
class SingleQubitTimedomainAnalysis(ba.BaseAnalysis):
    ...
    def run(
        self, calibration_points: bool | Literal["auto"] = "auto"
    ) -> ba.BaseAnalysis:
        r"""
        Parameters
        ----------
    ...
    def _rotate_to_calibrated_axis(
        self, ref_idx_0: int = -2, ref_idx_1: int = -1
    ) -> None:
    ...
    def _choose_data_for_fit(self):  # noqa: ANN202
        if self.calibration_points:
...
class _DecayFigMixin:
    ...
    def _create_decay_figure(self, fig_id: str):  # noqa: ANN202
        ...
        plot_textbox(ax, self.quantities_of_interest["fit_msg"])  # type: ignore

        if self.calibration_points:  # type: ignore
            ax.plot(
                self.dataset_processed.x0,  # type: ignore
                self.dataset_processed.pop_exc,  # type: ignore
                marker=".",
                ls="",
            )
            set_ylabel(
                self.dataset_processed.pop_exc.long_name,  # type: ignore
                self.dataset_processed.pop_exc.units,  # type: ignore
                ax,
            )
        else:
            ax.plot(
                self.dataset_processed.x0,  # type: ignore
                abs(self.dataset_processed.S21),  # type: ignore
                marker=".",
                ls="",
            )
            set_ylabel(
                r"Magnitude |$S_{21}|$",
                self.dataset_processed.S21.units,  # type: ignore
                ax,
            )

        set_xlabel(
            self.dataset_processed.x0.long_name,  # type: ignore
            self.dataset_processed.x0.units,  # type: ignore
            ax,
        )

        set_suptitle_from_dataset(fig, self.dataset)  # type: ignore
...
class T1Analysis(SingleQubitTimedomainAnalysis, _DecayFigMixin):
    ...
    def run_fitting(self) -> None:
        """Fit the data to :class:`~quantify.analysis.fitting_models.ExpDecayModel`."""
...
class RamseyAnalysis(SingleQubitTimedomainAnalysis, _DecayFigMixin):
    ...
    def run(  # noqa: ANN201 # type: ignore
        self,
        artificial_detuning: float = 0,
        qubit_frequency: float | None = None,
        calibration_points: bool | Literal["auto"] = "auto",
    ):
        r"""
        Parameters
        ----------
    ...
    def run_fitting(self) -> None:
        """
        Fits a :class:`~quantify.analysis.fitting_models.DecayOscillationModel`
        to the data.
        """
    ...
    def analyze_fit_results(self) -> None:
        ...
        self.quantities_of_interest["detuning"] = (
            self.quantities_of_interest["fitted_detuning"] - self.artificial_detuning  # type: ignore
        )
...
class AllXYAnalysis(SingleQubitTimedomainAnalysis):
    ...
    def run(self):  # noqa: ANN201 # type: ignore
        """
    ...
    # pylint: disable=arguments-differ
    def _rotate_to_calibrated_axis(self) -> None:  # type: ignore
        if not isinstance(self.dataset, xr.Dataset):
            raise TypeError(
                f"self.dataset must be of type xr.Dataset but is {type(self.dataset)}"
            )
        if len(self.dataset.x0) == 21:
            ...
            raise ValueError("ref_idx_1 must be defined")
...
class RabiAnalysis(SingleQubitTimedomainAnalysis):
    ...
    def run(self, calibration_points: bool = True):  # noqa: ANN201 # type: ignore
        """
        Parameters
        ----------
        calibration_points
