...
from quantify.helpers.waveforms import get_waveform
from quantify.operations.control_flow_library import ControlFlowOperation
from quantify.operations.pulse_library import NumericalPulse
...
def determine_relative_latency_corrections(
    ...
    """
    Generates the latency configuration dict for all port-clock combinations
    that are present in the schedule (or in the hardware config, if an old-style
    zhinst config is passed). This is done by first setting unspecified latency
    corrections to zero, and then subtracting the minimum latency from all latency
    corrections.
    """

    def _extract_port_clocks(hardware_cfg: dict[str, Any]) -> Generator:
        """
        Extract all port-clock combinations present in a hardware configuration.
    ...
    if isinstance(hardware_cfg, HardwareCompilationConfig):
        ...
        port_clocks = [
            ...
            for port_clock in schedule.get_used_port_clocks()
        ]
...
def apply_software_distortion_corrections(
    ...
    **only** replacing the dict in ``"pulse_info"`` associated to that specific
    ...
    -------
    :
        The new operation with distortion-corrected operations, if replacement is
        needed. Returns ``None`` if no replacement in the schedule or control flow
        is required.
