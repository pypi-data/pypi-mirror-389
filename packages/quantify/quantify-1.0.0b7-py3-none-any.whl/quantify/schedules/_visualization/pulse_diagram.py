...
def get_sampled_pulses_from_voltage_offsets(
    ...
    for port in offset_infos:  # noqa: PLC0206
        for clock, info_list in offset_infos[port].items():
            ...
            signal_as_array = np.array(signal)
    ...
    for i, (port, (pulses, acqs)) in enumerate(sampled_pulses_and_acqs.items()):
        ...
        for pulse in pulses:
            ...
            fig.add_trace(
                go.Scatter(
                    x=pulse.time,
                    y=pulse.signal.real,  # type: ignore
                    mode="lines",
                    name=pulse.label,
            ...
            if np.iscomplexobj(pulse.signal):
                fig.add_trace(
                    go.Scatter(
                        x=pulse.time,
                        y=pulse.signal.imag,  # type: ignore
                        mode="lines",
...
def plot_single_subplot_mpl(
    ...
    for i, (port, data) in enumerate(sampled_schedule.items()):
        for pulse in data:
            ax.plot(pulse.time, pulse.signal.real, color=f"C{i}", label=f"port {port}")  # type: ignore
            ax.fill_between(pulse.time, pulse.signal.real, color=f"C{i}", alpha=0.2)  # type: ignore

            if np.iscomplexobj(pulse.signal):
                ax.plot(
                    pulse.time,
                    pulse.signal.imag,  # type: ignore
                    color=f"C{i}",
                    linestyle="--",
                    label=f"port {port} (imag)",
                )
                ax.fill_between(pulse.time, pulse.signal.imag, color=f"C{i}", alpha=0.4)  # type: ignore
