...
from quantify.utilities.general import delete_keys_from_dict, get_subclasses, load_json
...
def _instrument_submodules_settable(
    ...
    # if not isinstance(settable, (ParameterBase, InstrumentBase, DummyBatchedSettable)):  # noqa: E501
    #     raise TypeError(
    #         f"settable must of type ParameterBase or InstrumentBase but is "
    #         f"{type(settable)}"
    #     )
    try:
        root = settable.root_instrument  # type: ignore
...
def _generate_long_name(settable: Settable) -> str:
    ...
    for x in path:
        if hasattr(x, "label"):
            sublabels.append(x.label)  # type: ignore
        elif hasattr(x, "short_name"):
            sublabels.append(x.short_name)  # type: ignore
        else:
            sublabels.append(x.name)  # type: ignore
...
def _generate_name(settable: Settable) -> str:
    ...
    for x in path:
        ...
        if hasattr(x, "short_name"):
            subnames.append(x.short_name)  # type: ignore
        else:
            x_name = x.name  # type: ignore
