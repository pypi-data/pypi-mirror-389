...
import xarray as xr
from qcodes import Instrument, InstrumentChannel, ManualParameter
...
def test_write_quantify_dataset_np_bool():
    ...
    assert bool(load_dataset.attrs["test_attr"]) is True
    assert bool(load_dataset.x0.attrs["test_attr"]) is True
    assert bool(load_dataset.y0.attrs["test_attr"]) is False
...
def test_snapshot_dead_instruments():
    ...
    instrument_a = Instrument("a")  # noqa: F841
    instrument_b = Instrument("b")  # noqa: F841
    instrument_a = 123  # noqa: F841
    gc.collect()

    snap = dh.snapshot()  # noqa: F841
