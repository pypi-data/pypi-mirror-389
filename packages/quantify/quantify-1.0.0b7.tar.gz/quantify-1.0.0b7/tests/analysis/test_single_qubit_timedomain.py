...
from quantify.analysis.single_qubit_timedomain import (
    AllXYAnalysis,
    EchoAnalysis,
    RabiAnalysis,
    RamseyAnalysis,
...
def test_echo_analysis_no_cal(tmp_test_data_dir):
    ...
    assert set(analysis_obj.figs_mpl.keys()) == {  # type: ignore
        "Echo_decay",
    }
    ...
    for fit_name, fit_result in analysis_obj.fit_results.items():
        loaded_fit_result = EchoAnalysis.load_fit_result(
            tuid=analysis_obj.tuid,  # type: ignore
            fit_name=fit_name,
        )
...
def test_ramsey_no_cal_generated(tmp_test_data_dir):
    ...
    assert set(analysis.figs_mpl.keys()) == {  # type: ignore
        "Ramsey_decay",
    }
...
def test_allxy_load_fit_results_missing(tmp_test_data_dir):
    ...
    with pytest.raises(
        ...
        AllXYAnalysis.load_fit_result(
            tuid="20210419-173649-456-23c5f3",  # type: ignore
            fit_name="fit_name",
        )
