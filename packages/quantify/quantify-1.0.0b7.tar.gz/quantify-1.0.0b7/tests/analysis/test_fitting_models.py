...
def test_fft_freq_phase_guess(tmp_test_data_dir):
    ...
    # Go through all the test datasets
    for idx, _ in enumerate(tuid_list):
        dataset = dh.load_dataset(tuid=tuid_list[idx])  # type: ignore
        time = np.array(dataset["x0"])
        magnitude = dataset["y0"]
        (freq_guess, _) = fm.fft_freq_phase_guess(magnitude, time)  # type: ignore
...
def test_cosine_model():
    ...
    assert np.mod(fit.best_values["phase"], 2 * np.pi) == pytest.approx(
        test_phs, rel=fit_tolerance
    )
