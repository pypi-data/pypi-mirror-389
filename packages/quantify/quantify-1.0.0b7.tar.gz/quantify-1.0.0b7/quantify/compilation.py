...
from quantify.enums import SchedulingStrategy
from quantify.json_utils import load_json_schema, validate_json
...
def _determine_absolute_timing(
    ...
    config
        ...
        :class:`~quantify.backends.graph_compilation.QuantifyCompiler`.
    time_unit
...
def _determine_absolute_timing_schedule(
    ...
    for op_key in schedule.operations:
        ...
        # an Operation after the `or`.
            ...
            # TODO move InlineQ1ASM to quantify .
            ...
            raise RuntimeError(
                f"Operation {schedule.operations[op_key].name} is not a valid pulse or "
                f"acquisition. Please check whether the device compilation has been "
                f"performed. Operation data: {schedule.operations[op_key]!r}"
            )
    ...
    for i, schedulable_name in enumerate(schedulables_sorted_by_reference):
        ...
        timing_constraints: list[TimingConstraint] = schedulable.data[
            "timing_constraints"
        ]
        operation: Operation | Schedule = schedule.operations[
        ...
        if i == 0:
            ...
            for timing_constraint in timing_constraints:
                abs_time = _get_start_time(
                    schedule, timing_constraint, operation, time_unit
                )
...
def _determine_scheduling_strategy(
    config: CompilationConfig | None = None,
) -> SchedulingStrategy:
...
def _validate_schedulable_references(
    schedule: Schedule, references_graph: nx.DiGraph
) -> None:
    ...
    if not nx.is_directed_acyclic_graph(references_graph):
        raise TypeError(
            "`schedulable_references` is not a Directed Acyclic Graph."
...
def _make_timing_constraints_explicit_for_schedulable(
    ...
    schedulable: Schedulable = schedule.schedulables[schedulable_name]
    given_timing_constraints: list[TimingConstraint] = schedulable.data[
        "timing_constraints"
    ]
    ...
    if strategy == SchedulingStrategy.ALAP and len(given_timing_constraints) != 1:
        raise NotImplementedError(
            "Only exactly one timing constraint per Schedulable supported."
        )
...
def _determine_default_ref_pt(strategy: SchedulingStrategy) -> Literal["start", "end"]:
    ...
    raise ValueError(
        f"Cannot determine default `ref_pt`. Unknown scheduling strategy: {strategy}"
    )


def _determine_default_ref_pt_new(
    strategy: SchedulingStrategy,
) -> Literal["start", "end"]:
    if strategy == SchedulingStrategy.ASAP:
        return "start"
        ...
        raise ValueError(
            "Cannot determine default `ref_pt_new`."
...
def _determine_default_ref_schedulables_by_schedulable(
    ...
    if strategy == SchedulingStrategy.ASAP:
        default_schedulable_names: list[str | None] = [None] + list(
            schedule.schedulables
        )[:-1]
    elif strategy == SchedulingStrategy.ALAP:
        default_schedulable_names: list[str | None] = list(schedule.schedulables)[
            1:
        ] + [None]
    else:
...
def _normalize_absolute_timing(
    ...
    # See test_negative_absolute_timing_is_normalized_with_subschedule
    # in test_compilation.py and https://gitlab.com/quantify-os/quantify-scheduler/-/issues/489
    min_time = min(
        schedulable["abs_time"] for schedulable in schedule.schedulables.values()
    )
    if min_time < 0:
...
def validate_config(config: dict, scheme_fn: str) -> bool:
    ...
    scheme_fn
        The name of a json schema in the quantify.schemas folder.
