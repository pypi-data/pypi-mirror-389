...
from quantify import math as math_helpers
...
def exec_waveform_function(wf_func: str, t: np.ndarray, pulse_info: dict) -> np.ndarray:
    ...
    If the wf_func is defined outside quantify then the
...
def apply_mixer_skewness_corrections(
    ...
    def skew_real(_waveform: np.ndarray, alpha: float, phi: float) -> np.ndarray:
        original_amp = np.max(np.abs(np.real(_waveform)))
        intermediate_wf = np.real(_waveform) + np.imag(_waveform) * np.tan(phi)
    ...
    def skew_imag(_waveform: np.ndarray, alpha: float, phi: float) -> np.ndarray:
        original_amp = np.max(np.abs(np.imag(_waveform)))
        intermediate_wf = np.imag(_waveform) / np.cos(phi)
        new_amp = np.max(np.abs(intermediate_wf))
        intermediate_wf = (
            intermediate_wf / new_amp
            if new_amp != 0
...
def normalize_waveform_data(data: np.ndarray) -> tuple[np.ndarray, float, float]:
    ...
    """
    amp_real_index = np.argmax(np.abs(np.real(data)))
    amp_imag_index = np.argmax(np.abs(np.imag(data)))

    amp_real = np.real(data)[amp_real_index]
    amp_imag = np.imag(data)[amp_imag_index]

    norm_data_re = (
        np.real(data) / amp_real
        if not math.isclose(amp_real, 0.0)
        else np.zeros(np.real(data).shape)
    )
    if math.isclose(amp_imag, 0.0):
        rescaled_data = norm_data_re
    else:
        norm_data_im = np.imag(data) / amp_imag
