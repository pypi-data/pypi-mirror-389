...
from quantify.helpers.waveforms import area_pulse
...
def _determine_sum_and_end_of_all_pulses(
    ...
    """
    Calculates the sum (or integral) of amplitudes of all pulses in the operation,
    and the end time of the last pulse in the operation.
    The function assumes no operation needs to be pulse compensated inside,
    and that absolute timings have already been calculated in the schedule.
    ...
    is_conditional
        Boolean variable to identify if a conditional operation
        is encountered during recursion.
    ...
    if isinstance(operation, ScheduleBase):
        ...
        if isinstance(operation, LoopOperation):
            ...
            raise ValueError(
                "Error calculating compensation pulse amplitude "
                f"for '{operation}'. This control flow operation type is not allowed "
        ...
        raise ValueError(
            "Error calculating compensation pulse amplitude "
            f"for '{operation}'. Voltage offset operation type is not allowed "
            "in a pulse compensation structure. "
        )
        ...
        for pulse_info in operation["pulse_info"]:
            ...
            if port in ports and pulse_info["clock"] == BasebandClockResource.IDENTITY:
                if is_conditional:
                    raise ValueError(
                        "Error calculating compensation pulse amplitude "
                        f"for '{operation}'. Operation is not allowed "
                        "within a conditional operation. "
                    )
...
def _determine_compensation_pulse(
    ...
    if a compensation square pulse is inserted in the schedule
    at `start` with duration `duration`, and amplitude `amp`, then
    ...
    * the compensation pulse starts just after the previous pulse.
    The function assumes there is no operation which needs
    to be pulse compensated inside. The clock is assumed to be the baseband clock.
...
def process_compensation_pulses(
    ...
    """
    Replaces ``PulseCompensation`` with a subschedule with
    an additional compensation pulse.
    ...
    if isinstance(operation, ScheduleBase):
        for inner_op_key in operation.operations:
            ...
            # will also be modified. We assume those are
