...
r"""
Module containing :class:`quantify.measurement.types.Gettable`\s for use with
quantify.
...
from quantify.helpers.diagnostics_report import _generate_diagnostics_report
...
class ScheduleGettable:
    ...
    the :class:`quantify.schedules.schedule.Schedule` using the
    ...
    schedule_function
        ...
        :class:`quantify.schedules.schedule.Schedule`. The
    ...
    def compile(self) -> CompiledSchedule:
        ...
        compiled_result = self._backend.compile(
            schedule=schedule, config=compilation_config
        )
        if isinstance(compiled_result, CompiledSchedule):
            self._compiled_schedule = compiled_result
            return self._compiled_schedule
        else:
            raise TypeError(f"Expected CompiledSchedule, got {type(compiled_result)}")
    ...
    def compiled_schedule(self) -> CompiledSchedule | None:
        """
        Get the latest compiled schedule, or None if nothing has been compiled
        yet.
        """
        return self._compiled_schedule
    ...
    def initialize_and_get_with_report(self) -> str:
        ...
        the instruments, and logs from the actual hardware to the zipfile.
