...
class _FiguresMplCache:
    ...
    figs: dict[str, matplotlib.figure.Figure]  # type: ignore
    axes: dict[str, matplotlib.axes.Axes]  # type: ignore
    initialized: bool
...
class AnalysisMeta(ABCMeta):
    ...
    def __new__(  # noqa: D102
        cls, name: str, bases: tuple[type, ...], namespace: dict, /, **kwargs
    ) -> AnalysisMeta:
        if "create_figures" in namespace:
            ...
            def create_figures_patched(self, *args, **kwargs) -> None:  # noqa: ANN001
...
class BaseAnalysis(metaclass=AnalysisMeta):
    ...
    @staticmethod
    def _get_analysis_dir(tuid: TUID, name: str, create_missing: bool = True) -> str:
        """
    ...
    def analysis_dir(self) -> str:
        ...
        return self._get_analysis_dir(tuid=self.tuid, name=self.name)  # type: ignore

    @staticmethod
    def _get_results_dir(analysis_dir: str, create_missing: bool = True) -> str:
        """
    ...
    def _analyses_figures_cache(self):  # noqa: ANN202
    ...
    def execute_analysis_steps(self) -> None:
        """
    ...
    def extract_data(self) -> None:
        ...
        if self.dataset is None:
            ...
            self.dataset = load_dataset(tuid=self.tuid)  # type: ignore
    ...
    def _add_fit_res_to_qoi(self) -> None:
    ...
    def adjust_figures(self) -> None:
        ...
        """
        for fig in self.figs_mpl.values():  # type: ignore
    ...
    def save_quantities_of_interest(self) -> None:
        """
    ...
    def save_figures_mpl(self, close_figs: bool = True) -> None:
        """
    ...
    def display_figs_mpl(self) -> None:
    ...
    def adjust_cmap(
        self, cmap: Colormap | str | None, ax_ids: list[str] | None = None
    ) -> None:
        """
            ...
            if ax_id in ax_ids:
                ...
                for image_or_collection in (
                    ...
                    *(c for c in ax.collections if isinstance(c, QuadMesh)),
                    ...
                    image_or_collection.set_cmap(cmap)  # type: ignore
...
class BasicAnalysis(BaseAnalysis):
    ...
    def create_figures(self) -> None:
        ...
        if not isinstance(dataset, xr.Dataset):
            raise TypeError(
                f"dataset must be of type xr.Dataset but is {type(dataset)}"
            )
            ...
            for yi, yvals in dataset.data_vars.items():
                ...
                if yi.startswith("y"):
                    ...
                    if not isinstance(self.dataset, xr.Dataset):
                        raise TypeError(
                            f"self.dataset must be of type xr.Dataset but is "
                            f"{type(self.dataset)}"
                        )
...
class Basic2DAnalysis(BaseAnalysis):
    ...
    def create_figures(self) -> None:  # noqa: D102
        gridded_dataset = to_gridded_dataset(self.dataset)  # type: ignore
        ...
        for yi, yvals in gridded_dataset.data_vars.items():
            ...
            quadmesh = yvals.transpose().plot(ax=ax, cmap=plt.get_cmap())  # type: ignore
            ...
            qpl.set_cyclic_colormap(
                quadmesh, shifted=bool(yvals.min() < 0), unit=yvals.units
            )

            qpl.set_suptitle_from_dataset(fig, self.dataset, f"x0x1-{yi}")  # type: ignore
...
def flatten_lmfit_modelresult(model):  # noqa: ANN001, ANN201
    """
    ...
    dic = {}
    dic["success"] = model.success  # type: ignore
    dic["message"] = model.message  # type: ignore
    dic["params"] = {}
    for param_name in model.params:  # type: ignore
        dic["params"][param_name] = {}
        param = model.params[param_name]  # type: ignore
...
def lmfit_par_to_ufloat(param: lmfit.parameter.Parameter):  # noqa: ANN201
    """
...
def check_lmfit(fit_res: lmfit.model.ModelResult) -> str | None:
    """
...
def wrap_text(
    ...
    :func:`~quantify.visualization.mpl_plotting.plot_textbox` in order to avoid
    too wide figure when, e.g.,
    :func:`~quantify.analysis.base_analysis.check_lmfit` fails and
    a warning message is generated.

    For usage see, for example, source code of
    :meth:`~quantify.analysis.single_qubit_timedomain.T1Analysis.create_figures`.
...
def analysis_steps_to_str(
    ...
    """
    col0 = tuple(element.name for element in analysis_steps)  # type: ignore
    col1 = tuple(element.value for element in analysis_steps)  # type: ignore
