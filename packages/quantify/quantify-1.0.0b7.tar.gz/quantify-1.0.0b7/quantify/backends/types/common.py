...
from quantify.structure.types import (
    Graph,  # noqa: TC001, TC004 - required at runtime for Pydantic forward refs
    NDArray,  # noqa: TC001, TC004 - required at runtime for Pydantic forward refs
)
...
if TYPE_CHECKING:
    ...
    from quantify.enums import TriggerCondition
...
class SoftwareDistortionCorrection(DataStructure):
    ...
    def _only_two_clipping_values(
        cls, clipping_values: list | None
    ) -> list | None | ValueError:
...
class DistortionCorrection(DataStructure):
    ...
    def __init__(self, **data) -> None:
        """Initialize the DistortionCorrection object."""
        super().__init__(**data)
        warnings.warn(
...
class LocalOscillatorDescription(DataStructure):
    ...
    @classmethod
    def _only_default_generic_icc_name(cls, generic_icc_name: str | None) -> str | None:
        if (
...
class Connectivity(DataStructure):
    ...
    set of two strings that each correspond to an input/output on an instrument or a
    port on the quantum device.
    ...
    @classmethod
    def _unroll_lists_of_ports_in_edges_input(
        cls, graph: list[tuple[Any, Any]]
    ) -> list[tuple[Any, Any]]:
...
class HardwareCompilationConfig(DataStructure):
    ...
    Specific hardware backends should customize fields within this structure by
    inheriting from this class and specifying their own `"config_type"`.
    """

    config_type: type[HardwareCompilationConfig] = Field(  # type: ignore
        default="quantify.backends.types.common.HardwareCompilationConfig",
    ...
    """
    The :class:`~quantify.backends.types.common.HardwareOptions` used in the
    ...
    def _serialize_backend_func(self, v: type[HardwareCompilationConfig]) -> str:
    ...
    def _check_connectivity_graph_nodes_format(self) -> Self:
        if isinstance(self.connectivity, Connectivity):
            for node in self.connectivity.graph:
                if "." in node:
                    ...
                    self.connectivity.graph.nodes[node]["instrument_name"] = (
                        instrument_name
                    )
                elif ":" in node:
                    self.connectivity.graph.nodes[node]["instrument_name"] = (
