...
from quantify.schedules.schedule import Schedulable, Schedule, ScheduleBase
...
def compile_circuit_to_device_with_config_validation(
    ...
    ``config`` as they are expected to arrive at the device. Latency or distortion
    ...
    config
        ...
        :class:`~quantify.backends.graph_compilation.QuantifyCompiler`, of
    ...
    :
        ...
        or the unmodified schedule if circuit to device compilation is not
        necessary.
...
def _compile_circuit_to_device(  # noqa: PLR0911
    ...
    if isinstance(operation, ScheduleBase):
    ...
    elif isinstance(operation, InlineQ1ASM):  # TODO move InlineQ1ASM to quantify
        ...
        # It is a two-device-element operation if the operation is not present
        # in the configuration for the device element.
        elif (
        ...
        # we only support single and two-device-element operations.
        # certain single-device-element operations (e.g., reset, measure)
        # may act on multiple device elements at once; this is handled
        # by the logic in this for-loop.
        else:
...
def _set_pulse_and_acquisition_clock(
    ...
    """
    if isinstance(operation, ScheduleBase):
        # check required clocks are present; warn if inconsistent
        verified_clocks = []
        for inner_op_key in operation.operations:
...
def _compile_circuit_to_device_pulse_compensation(
    operation: PulseCompensation,
    device_cfg: DeviceCompilationConfig,
    device_overrides: dict,
) -> PulseCompensation:
    """Compiles circuit-level pulse compensation operation to device-level."""
    if (
        device_elements := operation.data.get("pulse_compensation_info", {}).get(
            "device_elements"
        )
    ) is not None:
        ...
        for device_element in device_elements:
            if (
                pulse_compensation_element := device_cfg.elements.get(
                    device_element, {}
                ).get("pulse_compensation")
            ) is not None:
                if pulse_compensation_element.factory_func is not None:
                    raise ValueError(
                        "'factory_func' in the device configuration for pulse "
                        f"compensation for device element '{device_element}' is "
                        "not 'None'. Only 'None' is allowed for 'factory_func' "
                ...
                if (sampling_rate != current_sampling_rate) and (
                    ...
                    raise ValueError(
                        ...
                        "for pulse compensation. 'sampling_rate' for device element "
                        f"'{device_element}' is '{current_sampling_rate}', for others "
                        f"it is '{sampling_rate}'."
                    )
        ...
        return PulseCompensation(
            body=_compile_circuit_to_device(
                operation.body, device_cfg, device_overrides
            ),
...
def _get_device_repr_from_cfg(
    ...
    # Filter out unexpected kwargs to be resilient to optional gate_info entries
    try:
        sig = inspect.signature(factory_func)
        accepted = set(sig.parameters.keys())
        factory_kwargs = {k: v for k, v in factory_kwargs.items() if k in accepted}
    except (ValueError, TypeError):
        # If introspection fails, proceed without filtering.
        # This can happen for e.g. built-in functions.
        logging.info(
            "Could not determine the signature of '%s'. "
            "Proceeding without filtering keyword arguments. "
...
def _get_device_repr_from_cfg_multiplexed(
    ...
    if operation_cfg.gate_info_factory_kwargs is not None:
        for key in operation_cfg.gate_info_factory_kwargs:
            ...
            # Hack alert: not all parameters in a multiplexed operation are necessarily
            # passed for each element separately. We assume that if they are (e.g.,
            # acquisition index and channel for measurement), they are passed as a list
            # or tuple. If they are not (e.g., it is hard to imagine different
            # acquisition protocols for each device element during multiplexed readout),
            # they are assumed to NOT be a list or tuple. If this spoils the correct
            # behaviour of your program in the future: sorry :(
