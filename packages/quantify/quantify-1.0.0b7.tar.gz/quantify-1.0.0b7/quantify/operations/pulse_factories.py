...
def rxy_drag_pulse(
    ...
    reference_magnitude :
        :class:`~quantify.operations.pulse_library.ReferenceMagnitude`
        Optional scaling value and unit for the unitless amplitude.
        Uses settings in hardware config if not provided.
...
def rxy_gauss_pulse(
    ...
    that achieves the right rotation angle ``theta`` based on a
    calibrated pi-pulse amplitude.
...
def phase_shift(
    ...
    """
    Generate a :class:`~.operations.pulse_library.ShiftClockPhase`
    that shifts the phase of the ``clock`` by an angle `theta`.
...
def rxy_pulse(
    ...
    """
    Generate a Hermite or Gaussian drive pulse for a specified rotation
    on the Bloch sphere.

    The pulse achieves the desired rotation angle ``theta`` using a calibrated pi-pulse
    amplitude ``amp180``. The shape of the pulse can be either a skewed Hermite pulse
    or a Gaussian pulse, depending on the specified `pulse_shape`.
    ...
    reference_magnitude : pulse_library.ReferenceMagnitude | None, optional
        Reference magnitude for hardware configuration.
    ...
    if pulse_shape == "SkewedHermitePulse":
        ...
        raise ValueError(
            f"Unsupported pulse shape: {pulse_shape}. "
            "Use 'SkewedHermitePulse' or 'GaussPulse'."
        )
...
def nv_spec_pulse_mw(
    ...
    Returns
    -------
    pulse_library.SquarePulse | pulse_library.SkewedHermitePulse |
    pulse_library.GaussPulse
        The generated pulse operation based on the specified shape and parameters.
...
def non_implemented_pulse(**kwargs) -> Schedule:
    ...
    raise NotImplementedError(
        "The gate or pulse you are trying to use is not implemented yet."
    )
