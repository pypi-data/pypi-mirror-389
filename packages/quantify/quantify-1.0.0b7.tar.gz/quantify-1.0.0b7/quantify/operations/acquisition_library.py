...
"""Standard acquisition protocols for use with the quantify."""
...
from quantify.resources import DigitalClockResource
...
class Acquisition(Operation):
    ...
    :func:`~.quantify.schedules._visualization.pulse_diagram.plot_acquisition_operations`
...
class Trace(Acquisition):
    ...
    bin_mode
        ...
        acquisition in a Schedule is stored, e.g. for a Trace acquisition.
    t0
    ...
    def __init__(
        ...
        """Initialize a Trace acquisition."""
    ...
    def __str__(self) -> str:
        """Return a string representation of the acquisition."""
...
class WeightedIntegratedSeparated(Acquisition):
    ...
    def __init__(
        ...
        """Initialize a WeightedIntegratedSeparated acquisition."""
        _warn_deprecated_acq_index(acq_index)
        if phase != 0:
...
class SSBIntegrationComplex(Acquisition):
    ...
    def __init__(
        ...
        """Initialize a SSBIntegrationComplex acquisition."""
        _warn_deprecated_acq_index(acq_index)
        waveform_i = {
            "port": port,
            "clock": clock,
...
class ThresholdedAcquisition(Acquisition):
    ...
    .. admonition:: Examples
        ...
        .. jupyter-execute::
            ...
            from quantify.operations.acquisition_library import ThresholdedAcquisition
            ...
            schedule.add(
                ThresholdedAcquisition(port="q0:res", clock="q0.ro", duration=1e-6)
            )
    ...
    def __init__(
        ...
        """Initialize a ThresholdedAcquisition."""
        _warn_deprecated_acq_index(acq_index)
        waveform_i = {
            "port": port,
            "clock": clock,
...
class NumericalSeparatedWeightedIntegration(WeightedIntegratedSeparated):
    r"""
    Subclass of :class:`~WeightedIntegratedSeparated`
    with parameterized waveforms as weights.
    ...
    def __init__(
        ...
        """Initialize a NumericalSeparatedWeightedIntegration."""
        ...
        self.data["acquisition_info"][0]["protocol"] = (
...
class NumericalWeightedIntegration(NumericalSeparatedWeightedIntegration):
    """
    Subclass of :class:`~NumericalSeparatedWeightedIntegration`
    ...
    def __init__(
        ...
        """Initialize a NumericalWeightedIntegration."""
        super().__init__(
            port=port,
            clock=clock,
            weights_a=weights_a,
...
class WeightedThresholdedAcquisition(NumericalWeightedIntegration):
    ...
    def __init__(
        ...
        """Initialize a WeightedThresholdedAcquisition."""
        if acq_rotation is not None:
            warnings.warn(
                "Using the `acq_rotation` argument in `WeightedThresholdedAcquisition` "
                "is deprecated and will be removed in quantify."
                "The integration protocol now inherently provides the necessary "
        ...
        self.data["acquisition_info"][0]["feedback_trigger_label"] = (
...
class TriggerCount(Acquisition):
    ...
    fine_start_delay
        ...
        hardware instruction timing grid.By default 0.
    fine_end_delay
    ...
    def __init__(
        ...
        """Initialize a TriggerCount."""
        _warn_deprecated_acq_index(acq_index)
        if bin_mode == BinMode.AVERAGE:
            warnings.warn(
                (
                    f"{bin_mode} is deprecated for the TriggerCount "
                    "acquisition protocol"
                    " and will be removed in quantify. "
...
class TimetagTrace(Acquisition):
    ...
    def __init__(
        ...
        if time_ref == TimeRef.PORT and time_ref_port is None:
            raise ValueError(
                f"If {time_ref} is used, `time_ref_port` must contain a valid port."
            )
...
class Timetag(Acquisition):
    ...
    time_ref
        ...
        * ``timestamp``: record relative to the timestamp marked using the
          :class:`~quantify.operations.pulse_library.Timestamp` operation.
        * ``port``: record relative to the timetag measured on another port. If this
          option is used, the ``time_ref_port`` argument must be specified as well. The
          acquisition operation that is measuring the timetag on the *other* port must
          end before or at the same time as *this* acquisition operation.
    ...
    fine_end_delay
        ...
        of time, independent of the hardware instruction timing grid. By default 0.
...
class ThresholdedTriggerCount(Acquisition):
    """
    Thresholded trigger counting acquisition protocol returning the
    comparison result with a threshold.

    If the number of triggers counted is less than the threshold, a 0 is
    returned, otherwise a 1.

    The analog threshold for registering a single count is set in the
    hardware configuration.

    .. important::

        The exact duration of this operation, and the possible bin modes may
        depend on the control hardware. Please consult your hardware vendor's
        :ref:`Reference guide` for more information.
    ...
    feedback_trigger_label
        The label corresponding to the feedback trigger, which is mapped by
        the compiler to a feedback trigger address on hardware, by default
        None.
    ...
    acq_index
        ...
        The data register in which the acquisition is stored, by default ``None``.
        Describes the "when"
        information of the measurement, used to label or tag individual
        measurements in a large circuit. Typically corresponds to the
        setpoints of a schedule (e.g., tau in a T1
        experiment).
    bin_mode
        Describes what is done when data is written to a register that already
        contains a value. Options are "append" which appends the result to the list
        or "average" which stores the count value of the new result and the old
        register value, by default BinMode.APPEND.
    t0
        The acquisition start time in seconds, by default 0.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a ThresholdedTriggerCount."""
        _warn_deprecated_acq_index(acq_index)
        if bin_mode == BinMode.AVERAGE and acq_index != 0:
            ...
            raise NotImplementedError(
                "Using nonzero acq_index is not yet implemented for "
                "AVERAGE bin mode for "
    ...
    def __str__(self) -> str:
        ...
        # The __str__ method is used for serialization.
        # The keys must match the input parameters.
        acq_info["threshold"] = acq_info["thresholded_trigger_count"]["threshold"]
        acq_info["feedback_trigger_condition"] = acq_info["thresholded_trigger_count"][
            "condition"
        ]
        return self._get_signature(acq_info)
...
class DualThresholdedTriggerCount(Acquisition):
    ...
    Four outcomes are possible for this measurement, and each of those four
    can be assigned a label to use in a
    :class:`~quantify.operations.control_flow_library.ConditionalOperation`:

    - "low" if ``counts < threshold_low``,
    - "mid" if ``threshold_low <= counts < threshold_high``,
    - "high" if ``counts >= threshold_high``,
    - "invalid" if the counts are invalid (can occur in very rare cases,
      e.g. when the counter overflows).
    ...
    label_mid
        The label that can be used to link a result of
        `threshold_low <= counts < threshold_high` to a
        ConditionalOperation, by default None.
    label_high
        The label that can be used to link a result of `counts >= threshold_high` to a
        ConditionalOperation, by default None.
    label_invalid
        The label that can be used to link an invalid counts result (e.g.
        a counter overflow) to a ConditionalOperation, by default None.
    feedback_trigger_condition
        The comparison condition (greater-equal, less-than) for the
        ThresholdedTriggerCount acquisition.
    acq_channel
    ...
    bin_mode
        Describes what is done when data is written to a register that already
        contains a value. Options are "append" which appends the result to the
        list or "average" which stores the count value of the new result and
        the old register value, by default BinMode.APPEND.
    t0
        The acquisition start time in seconds, by default 0.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a DualThresholdedTriggerCount."""
        _warn_deprecated_acq_index(acq_index)
        if bin_mode == BinMode.AVERAGE and acq_index != 0:
            # In average mode the count distribution is measured,
            # and currently we do not support multiple indices for this,
    ...
    def __str__(self) -> str:
        """Return a string representation of the acquisition."""
        acq_info = copy(self.data["acquisition_info"][0])
        acq_info["threshold_low"] = acq_info["thresholded_trigger_count"][
            "threshold_low"
        ]
        acq_info["threshold_high"] = acq_info["thresholded_trigger_count"][
            "threshold_high"
        ]
...
def _warn_deprecated_acq_index(acq_index: int | None) -> None:
    if acq_index is not None:
        warnings.warn(
            ...
            "Use the `coords` argument to define arbitrary user data for the "
            "acquisition. ",
            FutureWarning,
        )
