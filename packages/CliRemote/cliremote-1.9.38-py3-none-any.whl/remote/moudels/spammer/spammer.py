...
- WARNING: This version sends real messages. Test only in your safe test group.
"""
...
def _ensure_account_token_state(phone: str, rate_per_minute: float):
    """Initialize/refill token state for account. rate_per_minute: allowed messages per minute."""
    now = time.time()
    ...
    elapsed = now - _account_token_ts.get(phone, now)
    _account_tokens[phone] = min(_account_tokens.get(phone, 0.0) + elapsed * refill_per_sec, max(1.0, rate_per_minute))
    _account_token_ts[phone] = now
...
async def safe_send_real(acc_phone: str, spam_config: Dict[str, Any], text: str, remove_client_from_pool: Callable[[str], None]) -> bool:
    ...
    # ensure per-client lock exists (prefer client_manager.client_locks)
    try:
        ...
        try:
            ...
            while attempt < max_attempts:
                ...
                try:
                    ...
                    logger.warning(f"{acc_phone}: FloodWait {wait_for}s (attempt {attempt}). Sleeping then retrying.")
