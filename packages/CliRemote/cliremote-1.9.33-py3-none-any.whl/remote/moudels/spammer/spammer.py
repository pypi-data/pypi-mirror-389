...
async def _perform_action_use_final_text(acc_phone: str, spam_config: Dict[str, Any]) -> bool:
    """
    اجرای عمل ارسال پیام (یا شبیه‌سازی آن) با متن تولیدشده از build_final_text.
    در این نسخه:
      - ابتدا تلاش می‌کند از build_final_text(spam_config) استفاده کند.
      - اگر signature آن متفاوت بود، به build_final_text() fallback می‌کند.
      - اگر خروجی خالی بود، مقدار پیش‌فرض "[EMPTY TEXT FROM finaly_text]" جایگزین می‌شود.
    """
    try:
        # تلاش برای تولید متن نهایی با پارامتر
        try:
            text = build_final_text()
        except TypeError:
            # نسخه‌هایی از تابع که ورودی نمی‌خواهند
            text = build_final_text(None)
        except Exception as e:
            logger.warning(f"{acc_phone}: build_final_text failed: {e}")
        ...
        # لاگ یا ارسال
        logger.info(f"[SIM-SEND] {acc_phone} -> {target} | text: {text[:400]!r}")

        # در حالت واقعی به جای logger خط زیر را فعال می‌کنی:
        # await cli.send_message(target, text, parse_mode="HTML")

        logger.info(f"{acc_phone}: simulated send printed.")
        return True

    except Exception as e:
        logger.error(f"{acc_phone}: exception in perform_action: {e}", exc_info=True)
        return False
