...
class Territorial(TortillaExtension):
    ...
    max_retries: int = Field(
        2, ge=0, description="Maximum retry attempts for failed Earth Engine requests."
    )
    ...
    def _reduce_chunk(
        ...
        Args:
            ...
            ee: Earth Engine module (passed in to avoid repeated imports)
        ...
        for i in range(len(all_results[0])):
            ...
            # TODO: THIS IS HORRIBLE - FIX EARTH ENGINE MODE REDUCER OUTPUT
            # When using mode() reducer with multiple products,
            # Earth Engine may return "mode", "mode_1", "mode_2" instead of band names
            for reducer, products in reducer_groups.items():
                if str(reducer) == "Reducer.mode()":
                    # Map mode columns to actual variable names
    ...
    def _reduce_chunk_with_retry(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                return self._reduce_chunk(chunk, reducer_groups, ee)
            except Exception:
    ...
    def _process_admin_variables(
        ...
        for admin_var in admin_vars:
            ...
            # Create a function that captures admin_map properly to avoid B023
            def create_lookup_function(lookup_dict):
                def lookup_admin_name(x):
                    if x is not None:
            ...
            # Add name column using the lookup function
            df_data = df_data.with_columns(
                [
                    pl.col(admin_var)
                    .cast(pl.Int64, strict=False)
                    .map_elements(lookup_fn, return_dtype=pl.Utf8)
    ...
    def _compute(self, tortilla: "Tortilla") -> pl.DataFrame:
        ...
        with ThreadPoolExecutor(max_workers=self.max_concurrency) as executor:
            ...
            futures = {
                executor.submit(
                    self._reduce_chunk_with_retry, chunk, reducer_groups, ee
                ): chunk
        ...
        admin_vars = [
            p["name"] for p in active_products if p["name"].startswith("admin")
        ]
        if admin_vars:
