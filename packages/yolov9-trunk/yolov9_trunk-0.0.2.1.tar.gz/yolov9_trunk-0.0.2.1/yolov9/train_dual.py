...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    train_loader, dataset = create_dataloader(
        ...
        split_json=opt.split_json if hasattr(opt, 'split_json') else None,
        split_key=opt.split_key_train if hasattr(opt, 'split_key_train') else 'train',
        data_root=opt.data_root if hasattr(opt, 'data_root') else None,
        label_remapping_file=opt.label_remapping_file if hasattr(opt, 'label_remapping_file') else None)
    ...
    if RANK in {-1, 0}:
        val_loader = create_dataloader(
            ...
            prefix=colorstr('val: '),
            split_json=opt.split_json if hasattr(opt, 'split_json') else None,
            split_key=opt.split_key_val if hasattr(opt, 'split_key_val') else 'test',
            data_root=opt.data_root if hasattr(opt, 'data_root') else None,
            label_remapping_file=opt.label_remapping_file if hasattr(opt, 'label_remapping_file') else None)[0]
    ...
    nw = max(
        round(hyp['warmup_epochs'] * nb), 100)  # number of warmup iterations, max(3 epochs, 100 iterations)
    # nw = min(nw, (epochs - start_epoch) / 2 * nb)  # limit warmup to < 1/2 of training
    last_opt_step = -1
    maps = np.zeros(nc)  # mAP per class
    results = (0, 0, 0, 0, 0, 0, 0)  # P, R, mAP@.5, mAP@.5-.95, val_loss(box, obj, cls)
    scheduler.last_epoch = start_epoch - 1  # do not move
    # Use appropriate device for GradScaler (cuda, mps, or cpu)
    device_type = 'cuda' if device.type == 'cuda' else 'cpu'  # MPS doesn't support GradScaler yet
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        '--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-high.yaml', help='hyperparameters path')
    ...
    # JSON Split arguments
    parser.add_argument('--split-json', type=str, default=None, help='Path to JSON split file (optional)')
    parser.add_argument('--split-key-train', type=str, default='train', help='JSON key for training split')
    parser.add_argument('--split-key-val', type=str, default='test', help='JSON key for validation split')
    parser.add_argument('--data-root', type=str, default=None, help='Root directory for dataset (required with --split-json)')
    ...
    # MLFlow arguments
    parser.add_argument(
        '--mlflow-tracking-uri', type=str, default=None, 
        help='MLFlow tracking server URI (e.g., http://localhost:5000 or https://your-mlflow-server.com)')
    parser.add_argument(
        '--mlflow-experiment-name', type=str, default=None,
        help='MLFlow experiment name (default: yolov9-training)')

    return parser.parse_known_args()[0] if known else parser.parse_args()


def main(opt, callbacks=Callbacks()):
    # Set MLFlow environment variables from arguments if provided
    if hasattr(opt, 'mlflow_tracking_uri') and opt.mlflow_tracking_uri:
        os.environ['MLFLOW_TRACKING_URI'] = opt.mlflow_tracking_uri
        if RANK in {-1, 0}:
            LOGGER.info(f'MLFlow tracking URI set to: {opt.mlflow_tracking_uri}')
    
    if hasattr(opt, 'mlflow_experiment_name') and opt.mlflow_experiment_name:
        os.environ['MLFLOW_EXPERIMENT_NAME'] = opt.mlflow_experiment_name
        if RANK in {-1, 0}:
            LOGGER.info(f'MLFlow experiment name set to: {opt.mlflow_experiment_name}')
    ...
    if opt.resume and not opt.evolve:
        ...
        # Validate JSON split parameters
        if opt.split_json is not None:
            if opt.data_root is None:
                raise ValueError('--data-root is required when using --split-json')
            opt.split_json = check_file(opt.split_json)
            LOGGER.info(f'Using JSON split file: {opt.split_json}')
            LOGGER.info(f'  Train split key: {opt.split_key_train}')
            LOGGER.info(f'  Val split key: {opt.split_key_val}')
            LOGGER.info(f'  Data root: {opt.data_root}')
    ...
    # Apple Silicon (MPS) info
    if device.type == 'mps':
        LOGGER.info('Training on Apple Silicon (MPS). Note: AMP is disabled, DDP is not supported.')
