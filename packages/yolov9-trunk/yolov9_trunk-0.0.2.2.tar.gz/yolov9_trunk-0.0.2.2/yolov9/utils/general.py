    ...
    for r in requirements:
        try:
            ...
            if req.specifier and installed_version not in req.specifier:
                raise ValueError(f"Version mismatch for {req.name}")
        except (metadata.PackageNotFoundError, ValueError):  # exception if requirements not met
            s += f'"{r}" '
            n += 1
...
def check_dataset(data, autodownload=True):
    ...
    # Checks
    # Require 'train', 'names', and either 'val' or 'test'
    assert 'train' in data, emojis(f"data.yaml 'train:' field missing ❌")
    assert 'names' in data, emojis(f"data.yaml 'names:' field missing ❌")
    assert 'val' in data or 'test' in data, emojis(f"data.yaml must have either 'val:' or 'test:' field ❌")
    ...
    for k in 'train', 'val', 'test':
        if data.get(k):  # prepend path
            # Skip path resolution for dummy placeholder values (used with JSON splits)
            is_dummy = data[k] in ('train', 'val', 'test')
            if not is_dummy:
    ...
    if val:
        # Skip validation if val is a dummy placeholder value (used with JSON splits)
        # When using --split-json, these dummy values are overridden by the dataloader
        is_dummy_val = val in ('train', 'val', 'test')
        if not is_dummy_val:
