...
class VarifocalLoss(nn.Module):
    ...
    def forward(self, pred_score, gt_score, label, alpha=0.75, gamma=2.0):
        ...
        # Infer device type from input tensor
        device_type = 'cuda' if pred_score.device.type == 'cuda' else 'cpu'
...
class ComputeLoss:
    ...
    def __call__(self, p, targets, masks, img=None, epoch=0):
        ...
        pred_distri2, pred_scores2 = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats2],
                                               2).split((self.reg_max * 4, self.nc), 1)
            ...
            for i in range(batch_size):
                if fg_mask[i].sum():
                    ...
                    loss[1] += self.single_mask_loss(
                        gt_mask, pred_masks[i][fg_mask[i]], proto[i], mxyxy, marea)  # seg loss
        ...
        if fg_mask2.sum():
            loss0_, loss3_, _ = self.bbox_loss2(
                pred_distri2, pred_bboxes2, anchor_points, target_bboxes2 / stride_tensor, target_scores2,
                target_scores_sum2, fg_mask2)
            ...
            for i in range(batch_size):
                if fg_mask2[i].sum():
                    ...
                    mxyxy = xyxyn * torch.tensor([mask_w, mask_h, mask_w, mask_h], device=self.device)
                    loss[1] += self.single_mask_loss(
                        gt_mask, pred_masks2[i][fg_mask2[i]], proto2[i], mxyxy, marea)  # seg loss
    ...
    def __call__(self, p, targets, masks, img=None, epoch=0):
        ...
        if fg_mask.sum():
            loss0_, loss3_, _ = self.bbox_loss(
                pred_distri2, pred_bboxes2, anchor_points, target_bboxes / stride_tensor, target_scores,
                target_scores_sum, fg_mask)
            ...
            for i in range(batch_size):
                if fg_mask[i].sum():
                    ...
                    mxyxy = xyxyn * torch.tensor([mask_w, mask_h, mask_w, mask_h], device=self.device)
                    loss[1] += self.single_mask_loss(
                        gt_mask, pred_masks2[i][fg_mask[i]], proto2[i], mxyxy, marea)  # seg loss
        ...
        if fg_mask.sum():
            ...
            for i in range(batch_size):
                if fg_mask[i].sum():
                    ...
                    marea = xyxy2xywh(xyxyn)[:, 2:].prod(1)
                    mxyxy = xyxyn * torch.tensor([mask_w, mask_h, mask_w, mask_h], device=self.device)
                    loss[1] += 0. * self.single_mask_loss(
                        gt_mask, pred_masks2[i][fg_mask[i]], proto2[i], mxyxy, marea)  # seg loss
