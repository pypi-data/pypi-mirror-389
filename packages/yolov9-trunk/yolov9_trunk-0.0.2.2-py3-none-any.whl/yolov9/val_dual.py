...
def run(
    ...
    if training:  # called by train.py
        ...
        half &= device.type == 'cuda'  # half precision only supported on CUDA (not CPU or MPS)
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        if compute_loss:
            preds = preds[1]
            # train_out is already [d1, d2] from model output (y, [d1, d2])
            # Don't index it again - just pass it directly
        ...
        if plots and batch_i < 3:
            ...
            plot_images(
                im, output_to_target(preds), paths, save_dir / f'val_batch{batch_i}_pred.jpg', names)  # pred
    ...
    if save_json and len(jdict):
        w = Path(
            weights[0] if isinstance(weights, list) else weights
        ).stem if weights is not None else ''  # weights
        anno_json = str(
            Path(data.get('path', '../coco')) / 'annotations/instances_val2017.json')  # annotations json
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        opt.half = torch.cuda.is_available() and opt.device not in ('cpu', 'mps')  # FP16 for fastest results (CUDA only)
