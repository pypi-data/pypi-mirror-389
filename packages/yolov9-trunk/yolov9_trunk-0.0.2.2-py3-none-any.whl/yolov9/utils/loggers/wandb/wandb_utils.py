...
class WandbLogger():
    ...
    def __init__(self, opt, run_id=None, job_type='Training'):
       ...
       """
        ...
        # Configure wandb for local server if WANDB_BASE_URL is set
        wandb_base_url = os.getenv('WANDB_BASE_URL')
        if wandb_base_url:
            LOGGER.info(f'Using local W&B server at {wandb_base_url}')
    ...
    def check_and_upload_dataset(self, opt):
        ...
        config_path = self.log_dataset_artifact(
            opt.data, opt.single_cls, 'YOLOv9' if opt.project == 'runs/train' else Path(opt.project).stem)
