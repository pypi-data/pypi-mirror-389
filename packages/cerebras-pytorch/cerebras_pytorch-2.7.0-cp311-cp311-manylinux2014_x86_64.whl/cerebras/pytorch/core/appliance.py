...
from cerebras.appliance.appliance_manager import ApplianceManager
...
class ApplianceMode(ApplianceManager):
    ...
    def send_weights(
        ...
        with ExitStack() as exit_stack:
            ...
            if exceptions:
                ...
                self.logger.error("".join(format_exception(exc)))
                raise exc
    ...
    def host_inference(
        ...
        def get_task_map(compile_response, task_type):
            for task in compile_response.cluster_details.tasks:
                if task.task_type == task_type:
                    return task.task_map
            raise RuntimeError(
                f"Unable to find task map for task type {task_type}"
            )
        ...
        def update_cluster_details(
            ...
            for task in self.compile_resp.cluster_details.tasks:
                if task.task_type in add_tasks:
                    draft_task_map = get_task_map(
                        compile_response, task.task_type
                    )
                    for i in range(num_models):
                        ...
                        for dt in draft_task_map:
                            ...
                            new.task_id.wse_id += num_csx_offset
                            for i, wse_id in enumerate(new.task_id.wse_ids):
                                new.task_id.wse_ids[i] = wse_id + num_csx_offset

                            if ini:
                                add_task_ini(
                                    task.task_type, new.task_id.task_id, ini
                                )

        num_csx_for_target = len(get_task_map(self.compile_resp, TaskType.WSE))
        ...
        if draft_model_params:
            ...
            num_csx_for_draft_inference = len(
                get_task_map(draft_model_params.compile_response, TaskType.WSE)
            )
        ...
        if image_encoder_model_params:
            ...
            # 2. Original INIs passed to the host_inference.
            ...
            num_csx_for_image_encoder = len(
                get_task_map(
                    image_encoder_model_params.compile_response, TaskType.WSE
                )
            )
