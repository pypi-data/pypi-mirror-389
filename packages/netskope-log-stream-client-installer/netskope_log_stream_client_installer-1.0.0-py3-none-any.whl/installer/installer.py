...
def run_container(client_key: str, directory: str, option: str) -> bool:
    ...
    try:
        ...
        if option == "install":
            ...
            print(f"INFO: Environment file created succcessfully")
        ...
        print(f"Contaier files and Log files will be stored in: {directory}")
...
def run_watcher_container(directory: str, day_of_week: str, time_of_day: str):
    try:
...
def run_silent_install():
    try:
        ...
        try:
            if run_container(client_key, default_directory, "install"):
                run_watcher_container(default_directory,default_day,default_time)
...
def reinstall(option: str):
    ...
    try:
        ...
        try:
            ...
            if run_container(CLIENT_KEY, directory_value, option):
                success, is_grpc_error = wait_for_config_download(log_path)
                if success:
                    logger.info("Configuration successfully completed.")
...
def uninstall():
    ...
    try:
        ...
        container_logs = os.path.join(directory, "container_logs")

        if os.path.exists(container_files) and os.path.exists(container_logs):
            shutil.rmtree(container_files)
            shutil.rmtree(container_logs)
            print(f"INFO: Deleted Directory: {container_files}")
            print(f"INFO: Deleted Directory: {container_logs}")
            logger.info(f"Deleted Directory: {container_files}")
            logger.info(f"Deleted Directory: {container_logs}")
        else:
...
def display_help():
    ...
    - Install with user prompts:
        ns-installer Install
    - Reinstall with saved configuration:
        ns-installer Reinstall
    - Silent install with default values:
        ns-installer --silent
    - Silent install with auto-upgrade:
        ns-installer --silent --auto-upgrade
    - Install without version check:
        ns-installer Install --no-version-check
    - Uninstall everything:
        ns-installer Uninstall
...
def main():
    ...
    try:
        print("NETSKOPE LOG STREAM CLIENT INSTALLER")
        ...
        if option == "install":
            ...
            if check_os() and check_architecture() and check_docker():
                ...
                if validate_inputs(directory, client_key):
                    ...
                    try:
                        if run_container(client_key, directory, option):
                            run_watcher_container(directory,default_day, default_time)
