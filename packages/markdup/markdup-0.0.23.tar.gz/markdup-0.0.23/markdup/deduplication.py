...
def process_window(window_data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        core_reads_iter = _get_core_reads(
            extended_reads_iter, window_start, window_end, WORKER_SETTINGS.max_fragment_size
        )
        ...
        if not fragments:
            return {
                ...
                **_calculate_stats(0, 0, WORKER_SETTINGS.keep_duplicates),
            }
        ...
        stats = _calculate_stats(
            read_count, deduplicated_count, WORKER_SETTINGS.keep_duplicates
        )
        ...
        try:
            ...
            if self.report_file:
                with open(self.report_file, "w") as f:
                    ...
                    f.write("\n")
                    f.write(f"Total reads processed: {self.stats['total_reads_processed']:,}\n")
                    if self.keep_duplicates:
                        f.write(f"Duplicates detected: {self.stats['total_duplicates_detected']:,}\n")
                    else:
                        f.write(f"Duplicates removed: {self.stats['total_duplicates_removed']:,}\n")
                    f.write(f"Chromosomes processed: {self.stats['total_chromosomes_processed']}\n")
                    f.write(f"Deduplication rate: {self.stats['deduplication_rate']:.2f}%\n")
                    f.write("\n")
                    f.write("Read Types:\n")
                    f.write(f"  Properly paired: {self.stats['properly_paired']:,} × 2\n" if self.stats["properly_paired"] > 0 else "  Properly paired: 0\n")
                    f.write(f"  Improperly paired: {self.stats['paired_no_mate']:,}\n")
                    f.write(f"  Single-end: {self.stats['single_end']:,}\n")
