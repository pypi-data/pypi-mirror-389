...
class UnifiedProcessor:
        ...
        with Progress(
            ...
            with ProcessPoolExecutor(
                ...
                if total_windows_skipped > 0:
                    print(
                        f"ℹ️ Skipped {total_windows_skipped} empty windows during processing"
                    )
    ...
    def _final_sort_and_cleanup(self):
        ...
        if not temp_files:
            print("ℹ️  No temp files to process - creating empty output")
        ...
        with Progress(
            ...
            try:
                ...
                if not valid_temp_files:
                    print(
                        "ℹ️  No valid temp files found after waiting - creating empty output"
                    )
                    # Create an empty BAM file with header only
                    with pysam.AlignmentFile(self.bam_file, "rb") as _hfsrc:
                        header = _hfsrc.header
