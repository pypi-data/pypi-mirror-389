...
from parabellum.geo import geometry_fn
...
class Env:
    def __init__(self, cfg: DictConfig):
        ...
        self.map = jnp.array(geometry_fn(cfg.place, cfg.size // 2), dtype=jnp.int32)
        ...
        self.groups = jnp.arange(self.types.size) % cfg.groups
...
def spawn(env, rng) -> Array:
    loc = jnp.zeros_like(env.map).at[: env.cfg.size // 4, : env.cfg.size // 4].set(1)
    aux = lambda x, loc: random.choice(rng, jnp.arange(env.cfg.size**2), (x,), replace=True, p=((1 - env.map) & loc).flatten())  # noqa
