...
class ModSpectrumConfiguration(object):
    ...
    def configure_averaging(self, selector_string, averaging_enabled, averaging_count):
        r"""Configures averaging for the ModSpectrum measurement.
        ...
        Args:
            ...
            averaging_enabled (enums.ModSpectrumAveragingEnabled, int):
                This parameter specifies whether to enable averaging for the ModSpectrum measurement. The default value is **False**.
                ...
                | False (0)    | The measurement is performed on a single acquisition.                                                                    |
                +--------------+--------------------------------------------------------------------------------------------------------------------------+
                | True (1)     | The measurement uses the Averaging Count parameter as the number of acquisitions over which the ModSpectrum measurement  |
                |              | is averaged.                                                                                                             |
                +--------------+--------------------------------------------------------------------------------------------------------------------------+
        ...
        try:
            ...
            averaging_enabled = (
                ...
                if type(averaging_enabled) is enums.ModSpectrumAveragingEnabled
                else averaging_enabled
            )
            updated_selector_string = _helper.validate_and_update_selector_string(
                selector_string, self._signal_obj
            )
            error_code = self._interpreter.modspectrum_configure_averaging(
    ...
    def configure_burst_synchronization_type(self, selector_string, burst_synchronization_type):
        r"""Configures the burst synchronization type for ModSpectrum measurement.
        ...
        Args:
            ...
            burst_synchronization_type (enums.ModSpectrumBurstSynchronizationType, int):
                This parameter specifies the type of synchronization used for detecting the start of packet in the ModSpectrum
        ...
        try:
            ...
            burst_synchronization_type = (
                ...
                if type(burst_synchronization_type) is enums.ModSpectrumBurstSynchronizationType
                else burst_synchronization_type
            )
            updated_selector_string = _helper.validate_and_update_selector_string(
                selector_string, self._signal_obj
            )
            error_code = self._interpreter.modspectrum_configure_burst_synchronization_type(
