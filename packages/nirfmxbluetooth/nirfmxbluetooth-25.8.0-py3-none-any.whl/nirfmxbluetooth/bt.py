...
class _BTSignalConfiguration:
    """Contains static methods to create and delete BT signal."""

    @staticmethod
    def get_bt_signal_configuration(instr_session, signal_name="", cloning=False):
        ...
        return _BTSignalConfiguration.init(instr_session, updated_signal_name, cloning)  # type: ignore

    @staticmethod
    def init(instr_session, signal_name, cloning):
        with instr_session._signal_lock:
            if signal_name.lower() == BT._default_signal_name_user_visible.lower():
                signal_name = BT._default_signal_name

            existing_signal = instr_session._signal_manager.find_signal_configuration(
                BT._signal_configuration_type, signal_name
            )
            if existing_signal is None:
                signal_configuration = BT(instr_session, signal_name, cloning)  # type: ignore
                ...
                if signal_configuration._interpreter.check_if_current_signal_exists() is False:
                    if not signal_configuration.signal_configuration_name.lower():
                        instr_session._interpreter.create_default_signal_configuration(
                            BT._default_signal_name_user_visible,
...
class _BTBase(SignalConfiguration):
    """Defines a base class for BT."""
    ...
    def __init__(self, session, signal_name="", cloning=False):
        ...
        self._interpreter.set_session_handle(self._rfmxinstrsession_interpreter._vi)
    ...
    def _delete_signal_configuration(self, ignore_driver_error):
        ...
        try:
            if not self.is_disposed:
                ...
                _BTSignalConfiguration.remove_signal_configuration(self._rfmxinstrsession, self)  # type: ignore
                self.is_disposed = True
        finally:
    ...
    def get_vhdt_mode_enabled(self, selector_string):
        ...
        Returns:
            ...
            attr_val (enums.VdhtModeEnabled):
                +--------------+-------------+
                | Name (Value) | Description |
                +==============+=============+
                | False (0)    |             |
        ...
        try:
            ...
            attr_val = enums.VdhtModeEnabled(attr_val)
        except (KeyError, ValueError):
            raise errors.DriverTooNewError()  # type: ignore
        finally:
            self._session_function_lock.exit_read_lock()
    ...
    def set_vhdt_mode_enabled(self, selector_string, value):
        ...
        Args:
            ...
            value (enums.VdhtModeEnabled, int):
                +--------------+-------------+
                | Name (Value) | Description |
                +==============+=============+
                | False (0)    |             |
        ...
        try:
            ...
            value = value.value if type(value) is enums.VdhtModeEnabled else value
            error_code = self._interpreter.set_attribute_i32(  # type: ignore
    ...
    def commit(self, selector_string):
        r"""Commits settings to the hardware. Calling this method is optional. RFmxBT commits settings to the hardware when you
...
class BT(_BTBase):
    ...
    def __init__(self, session, signal_name="", cloning=False):
        """Initializes a BT signal configuration."""
