...
class MLServeClient:
    ...
    #     This method calls the `/api/v1/metrics/rl/{model}/{version}` endpoint,
    #     which aggregates epsilon trends, rewards, and policy selection statistics
    #     for policy-based RL models (e.g., epsilon-greedy, multi-policy managers).
    ...
    #         Time window (in hours) for metric aggregation (e.g., 168 = past 7 days)
    ...
    #         A dictionary with summary metrics and time-bucketed series.
    ...
    #     epsilon_series = pd.DataFrame(resp.get("series", {}).get("epsilon", []))
    #     reward_series = pd.DataFrame(resp.get("series", {}).get("reward", []))
    #     policy_series = pd.DataFrame(resp.get("series", {}).get("policy_selection", []))
