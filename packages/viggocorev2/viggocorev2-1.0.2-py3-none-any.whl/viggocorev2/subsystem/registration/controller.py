...
class Controller(controller.Controller):
    ...
    def _get_application_by_name(self, name):
        ...
        if not applications:
            raise exception.BadRequest(
                f'Não foi encontrada nenhuma aplicação com o nome "{name}".')
    ...
    def _get_register_data(self):
        ...
        if not (username and email and password and domain_name and
                application_name):
            raise exception.BadRequest(
                'Um dos campos a seguir não foi informado: "username", '
                '"email", "password", "domain_name", "application_name".')
    ...
    def _get_activate_data(self):
        ...
        if not (user_id and domain_id):
            raise exception.BadRequest(
                'Os campos "user_id" e "domain_id" são obrigatórios.')
    ...
    def _get_role_admin(self):
        ...
        if not roles:
            raise exception.BadRequest(
                'Não foi encontrado nenhum papel com o nome "Admin".')
        return roles[0]
    ...
    def activate(self):
        try:
            ...
            if not (domain and user):
                raise exception.BadRequest(
                    'O domínio ou o usuário não foi encontrado '
                    'para serem ativados.')
