...
class Create(operation.Create):
    ...
    def pre(self, session, **kwargs):
        allowed_file = (
            self.is_internal or self.manager.allowed_file(self.file.mimetype))
        if (self.file and allowed_file):
            ...
            e = BadRequest()
            e.message = ('Arquivo é de um tipo não permitido ou não '
                         'foi passado na requisição.')
            raise e
...
class Manager(manager.Manager):
    ...
    @classmethod
    def allowed_file(cls, mimetype, delimiter='/'):
        return mimetype.split(delimiter)[-1] in cls.ALLOWED_EXTENSIONS
