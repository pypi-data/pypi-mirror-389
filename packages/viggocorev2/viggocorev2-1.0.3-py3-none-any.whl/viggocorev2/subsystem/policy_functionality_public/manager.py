...
from viggocorev2.subsystem.policy_functionality_public.resource \
    import PolicyFunctionalityPublic
    ...
    def do(self, session, **kwargs):
        query = session.query(CapabilityFunctionality, Functionality). \
            ...
            join(PolicyFunctionalityPublic,
                 and_(PolicyFunctionalityPublic.capability_functionality_id == CapabilityFunctionality.id,  # noqa
                      PolicyFunctionalityPublic.role_id == self.role_id),
                 isouter=True). \
            filter(
                CapabilityFunctionality.application_id == self.application_id)
        query = query.filter(PolicyFunctionalityPublic.role_id.is_(None))
        ...
        dict_compare = {'functionality.': CapabilityFunctionality,
                        'policy_functionality_public.': PolicyFunctionalityPublic}
...
class GetSelectedFunctionalitys(operation.List):
    ...
    def do(self, session, **kwargs):
        query = session.query(CapabilityFunctionality, PolicyFunctionalityPublic.id,
                              Functionality). \
            join(Functionality,
                 CapabilityFunctionality.functionality_id == Functionality.id
                 ). \
