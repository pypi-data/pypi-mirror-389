...
# Global cache manager instance with CLI-safe configuration
from ..cache.types import CacheConfiguration

# Use memory-only cache to prevent SQLite hanging issues in CLI
_cli_safe_config = CacheConfiguration(
    enable_disk_cache=False,  # Disable disk cache to prevent hanging
    max_entries=200,
    max_size_bytes=20 * 1024 * 1024,  # 20MB max
)
cache_manager = CacheManager(config=_cli_safe_config)
