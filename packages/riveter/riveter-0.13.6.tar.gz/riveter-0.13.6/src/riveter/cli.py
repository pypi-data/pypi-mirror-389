    ...
    try:
        ...
        try:
            ...
            if rule_packs:
                ...
                validation_results = validate_resources(resources["resources"], all_rules)

                # Report results
                exit_code = report_results(
                    validation_results, output_format=output_format or "table"
                )

                raise SystemExit(exit_code)
            else:
                console.print(
                    "[red]Error:[/red] Must specify at least one rule pack with -p/--rule-pack"
                )
                raise SystemExit(1)
