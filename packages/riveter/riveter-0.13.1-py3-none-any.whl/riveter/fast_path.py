...
class FastPathRouter:
    ...
    def is_lightweight_command(self, args: List[str]) -> bool:
        ...
        if args[0] in ["list-rule-packs", "list-installed-packs"]:
            ...
            complex_options = ["--format", "--debug", "--log-level"]
            if not any(opt in args for opt in complex_options):
                return True

        return False

    def route_command(self, args: List[str]) -> Optional[int]:
    ...
    def _safe_execute(self, handler_func: Any, *args: Any) -> Optional[int]:
        ...
        try:
            ...
            self._log_fallback_reason("import_error", f"Failed to import required module: {str(e)}")
            return None
    ...
    def _handle_help(self, args: List[str]) -> Optional[int]:
...
"""
    ...
    def _handle_list_rule_packs(self) -> Optional[int]:
        """Handle list-rule-packs command with fast path and error preservation.
    ...
    def _handle_list_installed_packs(self) -> Optional[int]:
