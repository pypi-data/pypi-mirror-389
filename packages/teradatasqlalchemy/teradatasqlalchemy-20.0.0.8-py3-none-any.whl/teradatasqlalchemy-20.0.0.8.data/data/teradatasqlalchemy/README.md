...
- Added support for below Teradata's datatypes:
  - VECTOR32
- Optimized telemetry utility to gracefully handle character limits of queryband.

### teradatasqlalchemy 20.0.0.7
- `get_indexes()` no longer raises exception if query is mixed with ANSI temporal syntax and Teradata temporal syntax.

### teradatasqlalchemy 20.0.0.6
- Implemented `partition_by()` method to add partitioning clause while generating SQL query.

### teradatasqlalchemy 20.0.0.5
- Added support for below Teradata's datatypes:
  - VECTOR

### teradatasqlalchemy 20.0.0.4
- Updated `visit_column()` method to accept new keyword argument `table_only` to process only table name without schema name while generating string representation of SQL expression.
...
- Implemented `visit_column()` method to add `table name` or `alias name` while generating string representation of SQL expression.
