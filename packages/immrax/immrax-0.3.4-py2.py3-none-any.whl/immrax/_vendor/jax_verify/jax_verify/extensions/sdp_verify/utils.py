...
def fwd(inputs, layer_params):
    ...
    if isinstance(layer_params, dict):
        ...
        if "transpose" in layer_params and layer_params["transpose"]:
            # ConvTranspose
            return (
                default_conv_transpose(
                    inputs,
                    W,
                    (layer_params["stride"], layer_params["stride"]),
                    layer_params["padding"],
                )
                + b
            )
        else:
            ...
            dn = lax.conv_dimension_numbers(
                inputs.shape, W.shape, ("NHWC", "HWIO", "NHWC")
            )
            return (
        ...
        inputs = (
            inputs.reshape(inputs.shape[0], -1) if len(inputs.shape) == 4 else inputs
        )
        (W, b) = layer_params
        return jnp.dot(inputs, W) + b
...
def get_layer_params(fun_to_extract, example_input):
    ...
    jax_parameters = {
        var: param_vals
        for var, param_vals in zip(parsed.jaxpr.constvars, parsed.literals)
    }
    ...
    for eqn in parsed.jaxpr.eqns:
        if eqn.primitive == lax.reshape_p:
            if eqn.invars[0] in jax_parameters:
                ...
                # reshaping itself
                continue
        elif eqn.primitive in (lax.dot_general_p, lax.conv_general_dilated_p):
            if next_is_relu:
                raise ValueError(
                    "Unsupported architecture. Only supported networks are"
            ...
            if centering is not None:
                ...
                equivalent_bias = eqn.primitive.bind(
                    inp_bias, weight_params, **eqn.params
                )
            ...
            if layers:
                ...
                # the network.
                raise ValueError(
                    "Unsupported operation. sub is only supported as a"
            ...
            if layers:
                ...
                # the network.
                raise ValueError(
                    "Unsupported operation. div is only supported as a"
            ...
            if (
                (not next_is_relu)
                or (not isinstance(eqn.invars[1], jax.extend.core.Literal))
            ...
            raise ValueError(
                f"Unsupported primitive {eqn.primitive}. The only "
...
def boundprop(params, bounds_in):
    ...
    for layer_params in params:
        ...
        center = (lb_old + ub_old) / 2.0
        radius = (ub_old - lb_old) / 2.0
        ...
        layer_bounds.append(
            IntBound(
                lb_pre=lb, ub_pre=ub, lb=jnp.maximum(lb, 0.0), ub=jnp.maximum(ub, 0.0)
            )
        )
...
def init_bound(x, epsilon, input_bounds=(0.0, 1.0), add_batch_dim=True):
...
def ibp_bound_nonelided(verif_instance):
    ...
    obj_val = np.sum(np.maximum(obj_orig, 0.0) * ub + np.minimum(obj_orig, 0.0) * lb)
...
def make_relu_robust_verif_instance_elided(params, bounds=None, input_bounds=None):
    """Make VerifInstance from network weights and input.
...
def fgsm_single(
    ...
    return pgd(adv_loss_x, x, epsilon, num_steps, step_size, input_bounds=input_bounds)
...
def pgd_default(
    model_fn, x, label, epsilon, num_steps, step_size, input_bounds=(0.0, 1.0)
):
...
def pgd(adv_loss, x_init, epsilon, num_steps, step_size, input_bounds=(0.0, 1.0)):
