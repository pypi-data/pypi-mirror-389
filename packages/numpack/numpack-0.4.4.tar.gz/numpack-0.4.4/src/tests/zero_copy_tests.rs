...
mod zero_copy_tests {
    ...
    fn test_continuous_memory_optimizer() {
        ...
        assert_eq!(optimizer.calculate_optimal_block_size(512 * 1024), 16 * 1024); // 512KB -> 16KB块
        assert_eq!(optimizer.calculate_optimal_block_size(2 * 1024 * 1024), 64 * 1024); // 2MB -> 64KB块
    }
    ...
    fn test_zero_copy_performance_vs_copy() {
        ...
        println!("Performance comparison for {} indices:", large_continuous_indices.len());
    ...
    fn test_zero_copy_bounds_safety() {
        ...
        // 测试正常边界
        let valid_view = handler.create_safe_zero_copy_view(
            &test_data, 10, 50, 4, lifetime_guard.clone()
        );
        assert!(valid_view.is_some());
        
        // 测试越界访问
        let invalid_view = handler.create_safe_zero_copy_view(
            &test_data, 200, 100, 4, lifetime_guard.clone()
        );
        assert!(invalid_view.is_none()); // 应该返回None而不是崩溃
        
        // 测试零大小
        let zero_size_view = handler.create_safe_zero_copy_view(
            &test_data, 10, 0, 4, lifetime_guard
        );
    ...
    fn test_zero_copy_with_various_data_sizes() {
        ...
        for (rows, cols) in [(10, 10), (100, 50), (1000, 20), (50, 1000)] {
            ...
            for (i, row_data) in result.iter().enumerate() {
                if i < rows {
                    assert!(!row_data.is_empty(), "Row data should not be empty for valid index");
                }
            }
        }
    }
