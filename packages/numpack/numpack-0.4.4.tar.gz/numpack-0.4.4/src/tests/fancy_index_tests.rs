...
fn create_test_data_file(rows: usize, cols: usize, item_size: usize) -> (PathBuf, Vec<u8>) {
    ...
    let temp_path = temp_dir.join(format!("test_fancy_index_{}.dat", 
        std::process::id()));
...
/// 创建测试用的访问模式分析
fn create_test_pattern(pattern_type: AccessPatternType, size_category: SizeCategory) -> AccessPatternAnalysis {
