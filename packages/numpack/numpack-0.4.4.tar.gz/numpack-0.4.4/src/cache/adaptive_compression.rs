    ...
    fn calculate_delta_variance(data: &[u8], dtype: DataType) -> f64 {
        ...
        match dtype {
            DataType::Int32 => {
                ...
                let deltas: Vec<i32> = values.windows(2)
                    .map(|w| w[1] - w[0])
                    .collect();
...
impl CompressionSelector {
    /// 根据数据特征选择最优压缩算法
    pub fn select_algorithm(characteristics: &DataCharacteristics, dtype: DataType) -> CompressionAlgorithm {
        // 规则1: 高重复率 -> RLE
    ...
    /// 解压缩
    pub fn decompress(compressed: &[u8], algorithm: CompressionAlgorithm, _dtype: DataType) -> io::Result<Vec<u8>> {
        match algorithm {
...
mod tests {
    ...
    fn test_adaptive_compression() {
        ...
        let (compressed, algorithm) = CompressionSelector::compress(&data, DataType::Uint8).unwrap();
        
        // 应该使用RLE
        assert!(compressed.len() < data.len());
        println!("RLE compression: {} -> {} bytes ({:?})", data.len(), compressed.len(), algorithm);
    }
    ...
    fn test_delta_encoding() {
        ...
        let (compressed, algorithm) = CompressionSelector::compress(bytes, DataType::Int32).unwrap();
        println!("Delta encoding: {} -> {} bytes ({:?})", bytes.len(), compressed.len(), algorithm);
    }
}
