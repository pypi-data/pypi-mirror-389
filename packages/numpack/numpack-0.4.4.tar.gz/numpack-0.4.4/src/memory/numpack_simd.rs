...
impl CpuCapabilities {
    ...
    fn detect_sse2() -> bool { false }
    #[cfg(not(any(target_arch = "x86", target_arch = "x86_64")))]
    fn detect_avx2() -> bool { false }
    #[cfg(not(any(target_arch = "x86", target_arch = "x86_64")))]
    fn detect_avx512f() -> bool { false }
    #[cfg(not(any(target_arch = "x86", target_arch = "x86_64")))]
    fn detect_avx512vl() -> bool { false }
    ...
    fn detect_preferred_vector_width() -> usize {
        ...
        #[cfg(target_arch = "aarch64")]
        {
            if std::arch::is_aarch64_feature_detected!("neon") { 16 }
            else { 8 }
        }
...
impl NumPackSIMD {
    ...
    pub fn get_optimal_strategy(&self, dtype: DataType, data_size: usize) -> SIMDStrategy {
        ...
        match dtype {
            ...
            DataType::Uint8 | DataType::Int8 => self.choose_byte_strategy(data_size, elements_per_vector),
            DataType::Uint16 | DataType::Int16 | DataType::Float16 => self.choose_word_strategy(data_size, elements_per_vector),
            DataType::Uint32 | DataType::Int32 | DataType::Float32 => self.choose_dword_strategy(data_size, elements_per_vector),
            DataType::Uint64 | DataType::Int64 | DataType::Float64 => self.choose_qword_strategy(data_size, elements_per_vector),
            DataType::Complex64 | DataType::Complex128 => self.choose_complex_strategy(dtype, data_size),
        }
    }
    ...
    pub fn copy_rows(&self, src: &[u8], dst: &mut [u8], indices: &[usize], row_size: usize, dtype: DataType) -> Result<(), SIMDError> {
        let strategy = self.get_optimal_strategy(dtype, indices.len() * row_size);
        
        match strategy {
            SIMDStrategy::AVX512Byte | SIMDStrategy::AVX512Word | SIMDStrategy::AVX512DWord | SIMDStrategy::AVX512QWord => {
                self.avx512_copy_rows(src, dst, indices, row_size)
            },
            SIMDStrategy::AVX2Byte | SIMDStrategy::AVX2Word | SIMDStrategy::AVX2DWord | SIMDStrategy::AVX2QWord => {
                self.avx2_copy_rows(src, dst, indices, row_size)
            },
            SIMDStrategy::SSE2Byte | SIMDStrategy::SSE2Word | SIMDStrategy::SSE2DWord | SIMDStrategy::SSE2QWord => {
                self.sse2_copy_rows(src, dst, indices, row_size)
            },
            SIMDStrategy::NEONByte | SIMDStrategy::NEONWord | SIMDStrategy::NEONDWord | SIMDStrategy::NEONQWord => {
                self.neon_copy_rows(src, dst, indices, row_size)
            },
            _ => {
    ...
    pub fn batch_convert(&self, src: &[u8], dst: &mut [u8], src_dtype: DataType, dst_dtype: DataType) -> Result<(), SIMDError> {
        ...
        match strategy {
            ConversionStrategy::SIMDIntToFloat => self.simd_int_to_float_convert(src, dst, src_dtype, dst_dtype),
            ConversionStrategy::SIMDFloatToInt => self.simd_float_to_int_convert(src, dst, src_dtype, dst_dtype),
            ConversionStrategy::SIMDWidening => self.simd_widening_convert(src, dst, src_dtype, dst_dtype),
            ConversionStrategy::SIMDNarrowing => self.simd_narrowing_convert(src, dst, src_dtype, dst_dtype),
            ConversionStrategy::Scalar => self.scalar_convert(src, dst, src_dtype, dst_dtype),
        }
    }

    fn get_conversion_strategy(&self, src_dtype: DataType, dst_dtype: DataType, size: usize) -> ConversionStrategy {
        // 如果数据量太小，使用标量版本
        if size < 64 {
