...
impl DeletionBitmap {
    ...
    fn load(path: &Path, expected_rows: usize) -> NpkResult<Self> {
        ...
        if buffer.len() < 16 {
            return Err(NpkError::InvalidMetadata(
                format!("Bitmap file too small: {}", buffer.len())
            ));
        }
        ...
        let (total_rows, deleted_count, words_start) = if buffer.len() == expected_file_size {
            ...
            if buffer.len() % 8 != 0 {
                return Err(NpkError::InvalidMetadata(
                    format!("Invalid bitmap file size: {}", buffer.len())
                ));
            }
    ...
    pub fn mark_deleted(&mut self, row: usize) -> NpkResult<()> {
        if row >= self.total_rows {
            return Err(NpkError::IndexOutOfBounds(row as i64, self.total_rows as u64));
        }
