...
def ssim_similarity(image_1, image_2, window_size=None):
    ...
    height_2, width_2 = image_2.shape[0], image_2.shape[1]
    effective_window_size = window_size if window_size is not None else 7

    assert min(height, width, height_2, width_2) > effective_window_size, \
        "The smallest image side must be larger than the SSIM window size"

    if window_size is not None:
        assert window_size % 2 == 1, "The SSIM window size must be an odd number"
    else:
        window_size = effective_window_size

    if window_size >= min(height, width):
        raise ValueError("The SSIM window size must be smaller than both image dimensions")
    image_1 = image_1.astype("float")
    image_2 = image_2.astype("float")

    if image_1.max() > 1 or image_2.max() > 1:
        data_range = 255
    else:
        data_range = 1

    if len(image_1.shape) == 2:
        score = structural_similarity(image_1, image_2, win_size=window_size, multichannel=False, data_range=data_range)
    elif len(image_1.shape) > 2:
        score = structural_similarity(image_1, image_2, win_size=window_size, multichannel=True, channel_axis=-1, data_range=data_range)
