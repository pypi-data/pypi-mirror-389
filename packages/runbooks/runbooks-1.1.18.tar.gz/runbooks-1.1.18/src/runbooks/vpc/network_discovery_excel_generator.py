...
from runbooks.common.rich_utils import print_header, print_success, print_error, print_info, Console, create_progress_bar
...
class NetworkDiscoveryExcelGenerator:
    ...
    def _generate_sheet_6_cost_optimization(self, wb: Workbook, baseline_data: Dict[str, Any]):
        ...
        vpce_monthly = len(interface_endpoints) * self.pricing_config.get_vpc_endpoint_interface_monthly_cost(self.region)

        data = [
            ["Optimization Opportunity", "Current Monthly", "Potential Savings", "Annual Savings"],
            ["NAT Gateway Consolidation", f"${nat_monthly:,.2f}", f"${nat_monthly * 0.30:,.2f}", f"${nat_monthly * 0.30 * 12:,.0f}"],
            ["VPC Endpoint Optimization", f"${vpce_monthly:,.2f}", f"${vpce_monthly * 0.20:,.2f}", f"${vpce_monthly * 0.20 * 12:,.0f}"],
            ["Data Transfer Optimization", "$0.00", "$0.00", "$0"],
            ["", "", "", ""],
            ["TOTAL OPTIMIZATION", f"${nat_monthly + vpce_monthly:,.2f}",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate AWS Network Discovery Excel Report",
