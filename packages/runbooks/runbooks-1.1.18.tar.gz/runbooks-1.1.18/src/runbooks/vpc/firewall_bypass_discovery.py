...
class FirewallBypassDiscovery:
    ...
    def _discover_vpcs_multi_account(
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Discovering VPCs...",
                total=len(accounts) * len(self.regions)
            )
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                future_to_account = {
                    executor.submit(
                        self._discover_vpcs_in_account,
                        account,
                        region
                    ): (account, region)
                ...
                for future in as_completed(future_to_account):
                    ...
                    try:
                        ...
                        console.print(
                            f"[dim red]Failed to discover VPCs in {account['id']} ({region}): {e}[/]"
                        )
    ...
    def _discover_vpcs_in_account(
        self, account: Dict, region: str
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            session = boto3.Session(
                profile_name=account.get('profile', self.operational_profile),
                region_name=region
            )
            ...
            if error_code == 'UnauthorizedOperation':
                ...
                console.print(
                    f"[dim red]EC2 API error in {account['id']} ({region}): {error_code}[/]"
                )
        except Exception as e:
            console.print(
                f"[dim red]Unexpected error in {account['id']} ({region}): {e}[/]"
            )

        return vpcs

    def _analyze_tgw_attachments(
        self, vpcs: List[Dict]
    ) -> Dict[str, Dict]:
        """
        ...
        for region, regional_vpcs in vpcs_by_region.items():
            try:
                ...
                console.print(
                    f"[dim red]Failed to query TGW attachments in {region}: {e.response['Error']['Code']}[/]"
                )

        return tgw_attachments

    def _analyze_routing_patterns(
        self, vpcs: List[Dict], tgw_attachments: Dict[str, Dict]
    ) -> None:
        """
        ...
        for vpc in vpcs:
            ...
            try:
                ...
                routing_pattern, default_target, target_type = self._classify_routing_pattern(
                    route_tables
                )
                ...
                console.print(
                    f"[dim red]Failed to analyze routes for {vpc_id}: {e.response['Error']['Code']}[/]"
                )
                ...
                if dest == '0.0.0.0/0':
                    ...
                    if 'GatewayId' in route:
                        ...
                        if gateway_id.startswith('igw-'):
                            return (
                                RoutingPattern.DIRECT_IGW,
                                gateway_id,
                                'igw'
                            )
                        elif gateway_id.startswith('vgw-'):
                            return (
                                RoutingPattern.VPN_GATEWAY,
                                gateway_id,
                                'vpn-gateway'
                            )
    ...
    def _display_summary(self) -> None:
        ...
        none_count = sum(
            1 for r in self.inspection_records
            if r.inspection_status == InspectionStatus.NONE
        )
        egress_only_count = sum(
            1 for r in self.inspection_records
            if r.inspection_status == InspectionStatus.EGRESS_ONLY
        )
        full_count = sum(
            1 for r in self.inspection_records
            if r.inspection_status == InspectionStatus.INGRESS_EGRESS
        )
        unknown_count = sum(
            1 for r in self.inspection_records
            if r.inspection_status == InspectionStatus.UNKNOWN
        )
    ...
    def export_to_csv(self, output_path: Path) -> None:
        ...
        with open(output_path, 'w', newline='') as csvfile:
            fieldnames = [
                'account_id', 'account_name', 'region', 'vpc_id', 'vpc_name',
                'cidr_block', 'routing_pattern', 'default_route_target',
                'default_route_target_type', 'tgw_attached', 'tgw_id',
                'inspection_status', 'discovery_timestamp'
            ]
