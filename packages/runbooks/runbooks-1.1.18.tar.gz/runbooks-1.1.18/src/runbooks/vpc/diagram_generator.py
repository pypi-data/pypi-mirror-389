...
class NetworkDiagramGenerator:
    ...
    def __init__(
        ...
        if not DIAGRAMS_AVAILABLE:
            raise ImportError(
                "diagrams library not available. Install with: uv pip install diagrams"
            )
    ...
    def generate_transit_gateway_diagram(
        ...
        output_path = (
            output_filename
            or str(self.output_dir / "transit_gateway_topology.png")
        )

        print_info(
            f"Generating Transit Gateway topology diagram for {target_region}..."
        )
        ...
        if not tgws:
            print_warning(
                f"No Transit Gateways found in {target_region}. Creating reference diagram."
            )
        ...
        with Diagram(
            ...
            # Connections
            org_node >> Edge(
                label="Organization\nResource Share", style="dashed"
            ) >> tgw_node

            tgw_node >> Edge(
                label="Inspection", color="red", style="bold"
            ) >> nfw
            tgw_node >> Edge(
                label="Internet Egress", color="green"
            ) >> nat

            for vpc_node, rt in vpc_nodes:
                if rt:
                    vpc_node >> rt >> Edge(
                        label="TGW Attachment", color="blue"
                    ) >> tgw_node
                else:
                    vpc_node >> Edge(
                        label="TGW Attachments", color="blue"
                    ) >> tgw_node

        print_success(f"Transit Gateway diagram generated: {output_path}")
        return output_path

    def generate_ipam_current_state(
        ...
        """
        output_path = (
            output_filename or str(self.output_dir / "ipam_current_state.png")
        )
        ...
        with Diagram(
            ...
            with Cluster(f"AWS Organization ({org['master_account_id']})"):
                ...
                if tgws:
                    with Cluster(
                        f"Network Security Account ({self.account_id})\n🌐 Transit Gateway Hub"
                    ):
                        tgw = TransitGateway(
                            f"Transit Gateway\nCentral Hub\n{tgws[0]['id']}"
                        )
    ...
    def generate_ipam_target_state(
        self, output_filename: Optional[str] = None
    ) -> str:
        """
        ...
        """
        output_path = (
            output_filename or str(self.output_dir / "ipam_target_state.png")
        )
    ...
    def generate_direct_connect_topology(
        ...
        target_region = region or self.regions[0]
        output_path = (
            output_filename or str(self.output_dir / "direct_connect_topology.png")
        )
        ...
        with Diagram(
            ...
            onprem_router >> Edge(
                label="Dedicated Link", color="red", style="bold", penwidth="3.0"
            ) >> dx_node
            dx_node >> Edge(
                label="Virtual\nInterface", color="red", style="bold"
            ) >> dx_gateway
            dx_gateway >> Edge(
                label="DX Gateway\nAttachment",
                color="red",
            ...
            corporate >> Edge(
                label="Public\nInternet", color="gray", style="dashed"
            ) >> igw
            igw >> Edge(
                label="Public\nTraffic", color="gray", style="dashed"
            ) >> vpc_high
    ...
    def generate_network_firewall_inspection(
        ...
        output_path = (
            output_filename
            or str(self.output_dir / "network_firewall_inspection.png")
        )
        ...
        with Diagram(
            ...
            nfw >> Edge(
                label="Inspected\nTraffic", color="green", style="bold"
            ) >> inspection_rt
            ...
            rule_domain >> Edge(
                label="BLOCK\nMalicious", color="red", style="dashed"
            ) >> Shield("Dropped\nPackets")
    ...
    def generate_all_diagrams(self) -> Dict[str, str]:
        ...
        with create_progress_bar() as progress:
            ...
            task = progress.add_task(
                "[cyan]Generating diagrams...", total=total_diagrams
            )
