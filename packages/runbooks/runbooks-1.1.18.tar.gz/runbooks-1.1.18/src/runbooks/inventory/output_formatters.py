...
class ResourceExplorerFormatter:
    ...
    def export_csv(resources: List[Dict], output_file: str, include_header: bool = True) -> None:
        ...
        try:
            # Define CSV headers
            headers = [
                'resource_arn', 'account_id', 'region', 'resource_type',
                'resource_id', 'tags', 'monthly_cost', 'annual_cost'
            ]
        ...
        try:
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                for resource in resources[:100]:
                    ...
                    f.write(f"| {arn} | {resource.get('account_id', 'N/A')} | "
    ...
    def export_csv_console(resources: List[Dict], output_file: str, include_header: bool = True) -> None:
        ...
        try:
            ...
            print_success(f"Exported {len(resources)} resources to {output_file} (CSV Console format, {len(headers)} columns)")
    ...
    def to_csv_string(resources: List[Dict]) -> str:
        ...
        fieldnames = ['resource_arn', 'account_id', 'region', 'resource_type',
                     'resource_id', 'tags', 'monthly_cost', 'annual_cost']
    ...
    **kwargs
    ...
    if data_type == "organizations":
        if format_type == "json":
            ...
            raise ValueError(f"Unsupported format for organizations: {format_type}. Supported formats: {supported_formats}")
        ...
        if format_type == "json":
            ...
            raise ValueError(f"Unsupported format for resource_explorer: {format_type}. Supported formats: {supported_formats}")
