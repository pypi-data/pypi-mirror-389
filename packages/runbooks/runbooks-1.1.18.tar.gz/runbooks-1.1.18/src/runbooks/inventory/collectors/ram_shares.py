...
Author: CloudOps Runbooks Team (adapted from cloud-foundations-on-aws upstream)
Version: 1.0.0
"""
...
class RAMSharesCollector(CloudFoundationsBase):
    ...
    def __init__(
        ...
        try:
            ...
            self.ram_client = create_timeout_protected_client(
                session,
                'ram',
                region_name=region
            )
    ...
    def discover_ram_shares(
        ...
        try:
            ...
            if not all_shares:
                print_warning(
                    f"No RAM shares found - "
                    f"Status: {status_filter or 'ALL'} | Type: {type_filter or 'ALL'}"
                )
                return pd.DataFrame()
            ...
            if error_code in ['AccessDenied', 'UnauthorizedOperation']:
                ...
                print_error(
                    "❌ AWS API error during RAM shares discovery",
            ...
            print_error(
                ...
                f"Configure credentials: aws configure --profile {self.profile}"
            )
            raise

        except Exception as e:
            print_error(
                "❌ Unexpected error during RAM shares discovery",
                f"Error Type: {type(e).__name__}"
            )
            print_info(f"Message: {str(e)}")
            raise

    def _get_resource_shares(
        self,
        resource_owner: str,
        status_filter: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        return self.create_result(
            success=True,
            message="RAMSharesCollector uses discover_ram_shares() method directly",
            data={}
        ).model_dump()
...
def collect_ram_shares(
    ...
    df = collector.discover_ram_shares(
