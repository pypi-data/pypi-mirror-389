...
def get_profile_for_operation(
    ...
    if user_specified_profile and user_specified_profile != "default":
        ...
        if user_specified_profile in available_profiles:
            # SESSION-AWARE LOGGING: Only log when cache miss occurs AND not silent
            if profile_cache_key not in _profile_cache and not silent:
...
def create_session_with_auto_refresh(
    profile_name: str,
    max_retries: int = 2,
    timeout: int = 60
) -> boto3.Session:
