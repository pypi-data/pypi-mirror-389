...
class RichOperateGroup(click.Group):
    ...
    def format_help(self, ctx, formatter):
        ...
        compute_table.add_row("ec2 start", "Start EC2 instances")
        compute_table.add_row("ec2 stop", "Stop EC2 instances")
        ...
        storage_table.add_row("s3 create-bucket", "Create S3 buckets with configs")
        ...
        network_table.add_row("vpc create-vpc", "Create VPCs with configs")
        ...
        iac_table.add_row("cloudformation deploy", "Deploy CloudFormation stacks")
        iac_branch.add(iac_table)

        console.print(tree)
        console.print("\n💡 Usage: runbooks operate [RESOURCE] [OPERATION] [OPTIONS]")
        console.print("📖 KISS Architecture: Simple resource operations")
...
def create_operate_group():
    ...
    @click.option("--format", type=click.Choice(["compact", "table", "json"]), default="compact", help="Output format")
    @click.pass_context
    def create_bucket(ctx, profile, region, dry_run, bucket_name, encryption, versioning, public_access_block, all, verbose, format):
