...
Enterprise-grade output management for CloudOps Runbooks with 3-line compact defaults.
...
class OutputController:
    ...
    def __init__(
        ...
        logger.debug(
            f"OutputController initialized: verbose={verbose}, format={format}"
        )

    def truncate_profile(self, profile: str, max_length: int = 30) -> str:
        """
    ...
    def _print_table_summary(
        ...
        if added_columns:
            table.add_row(
                "Added Columns",
                f"{len(added_columns)}: {', '.join(added_columns)}"
            )
