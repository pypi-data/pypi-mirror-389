...
class WorkSpacesCostAnalyzer:
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        3. Service-specific default (ams-admin-ReadOnlyAccess-909135376185)
        ...
        try:
            ...
            with create_progress_bar() as progress:
                task = progress.add_task(
                    "[cyan]Checking volume encryption and connection status...",
                    total=len(df)
                )
    ...
    def get_cloudwatch_user_connected(
        self,
        df,
        ...
        try:
            ...
            with create_progress_bar() as progress:
                task = progress.add_task(
                    "[cyan]Checking UserConnected metrics...",
                    total=len(df)
                )
    ...
    def calculate_dynamic_breakeven(
        self,
        df
    ) -> pd.DataFrame:
        """
    ...
    def calculate_decommission_scores(
        self,
        df
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            with create_progress_bar() as progress:
                task = progress.add_task(
                    "[cyan]Scoring WorkSpaces for decommission...",
                    total=len(df)
                )
            ...
            print_info(f"   MUST: {tier_counts.get('MUST', 0)} | SHOULD: {tier_counts.get('SHOULD', 0)} | COULD: {tier_counts.get('COULD', 0)} | KEEP: {tier_counts.get('KEEP', 0)}")
    ...
    def enrich_dataframe_with_organizations(
        self, workspaces_df, management_profile: str
    ):
        """
        ...
        try:
            ...
            orgs_columns = ["account_name", "account_email", "wbs_code", "cost_group", "technical_lead", "account_owner"]
            ...
            with create_progress_bar() as progress:
                task_id = progress.add_task("[cyan]Enriching WorkSpaces with Organizations...", total=len(workspaces_df))
            ...
            print_success(f"Organizations enrichment complete: {enriched_count}/{len(workspaces_df)} WorkSpaces enriched")
            return workspaces_df
    ...
    def export_markdown(
        ...
        summary_metrics = {
            ...
            "HIGH Tier (W1-W7: 95-99)": len(df[(df['total_score'] >= 95) & (df['total_score'] < 100)]) if 'total_score' in df.columns else 0,
            "Unencrypted WorkSpaces (W7)": len(df[df.get('w7_score', pd.Series([0] * len(df))) == 5]) if 'w7_score' in df.columns else 0,
...
def analyze_workspaces_costs(
    ...
    try:
        ...
        analyzer = WorkSpacesCostAnalyzer(
            profile=operational_profile or management_profile
        )
        ...
        if enable_cost and include_12month_cost:
            ...
            cost_df = cost_enricher.get_12_month_cost_breakdown(
                ...
                service_filter='Amazon WorkSpaces'
            )
            ...
            if not cost_df.empty:
                ...
                cost_summary = cost_df.groupby('account_id').agg({
        ...
        if enable_activity:
            ...
            enriched_df = ct_enricher.enrich_with_activity(
                enriched_df,
                resource_id_column='WorkSpace ID',
                management_profile=management_profile,
                lookback_days=90
            )
