...
class OptimizationMetricsTracker:
    ...
    def get_current_state(self) -> Dict[str, Any]:
        ...
        for region in self.regions:
            try:
                ...
                # Count NAT Gateways
                nat_gateways = ec2.describe_nat_gateways(Filters=[{"Name": "state", "Values": ["available"]}])["NatGateways"]
        ...
        print_success(f"Current state collected: {current_state['nat_gateways']} NAT Gateways, {current_state['transit_gateways']} Transit Gateways")
    ...
    def calculate_progress(self, current_state: Optional[Dict[str, Any]] = None) -> Dict[str, float]:
        ...
        progress = {
            "nat_reduction": self._calculate_reduction_pct(self.baseline["nat_gateways"], current_state["nat_gateways"]),
            "tgw_reduction": self._calculate_reduction_pct(self.baseline["transit_gateways"], current_state["transit_gateways"]),
            "endpoint_reduction": self._calculate_reduction_pct(self.baseline["vpc_endpoints"], current_state["vpc_endpoints"]),
            "vpn_reduction": self._calculate_reduction_pct(self.baseline["vpn_connections"], current_state["vpn_connections"]),
            "cost_reduction": self._calculate_reduction_pct(self.baseline["monthly_cost"], current_state["monthly_cost"]),
        }

        return progress

    def generate_executive_report(self, current_state: Optional[Dict[str, Any]] = None, output_file: Optional[str] = None) -> str:
        """
    ...
    def _display_executive_dashboard(
        ...
        progress_table.add_row(
            "NAT Gateways", str(self.baseline["nat_gateways"]), str(current_state["nat_gateways"]), f"{progress['nat_reduction']:.1f}%"
        )
        progress_table.add_row(
            "Transit Gateways",
        ...
        progress_table.add_row(
            "VPC Endpoints", str(self.baseline["vpc_endpoints"]), str(current_state["vpc_endpoints"]), f"{progress['endpoint_reduction']:.1f}%"
        )
        progress_table.add_row(
            "VPN Connections",
