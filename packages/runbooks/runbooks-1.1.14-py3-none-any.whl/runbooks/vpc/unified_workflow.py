...
class UnifiedNetworkWorkflow:
    ...
    async def execute_full_workflow(self) -> Dict[str, Any]:
        ...
        with create_progress_bar() as progress:
            workflow_task = progress.add_task(
                "[cyan]Executing workflow...", total=6
            )
    ...
    async def _step_1_discovery(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print_success(
                f"Discovery complete: {nat_count} NAT GWs, {tgw_count} TGWs, "
                f"{vpce_count} VPCEs, {vpn_count} VPNs"
            )
    ...
    async def _step_2_excel_generation(self) -> str:
        ...
        try:
            ...
            excel_file = excel_gen.generate_consolidated_excel(
                output_dir=str(self.network_discovery_dir)
            )
    ...
    async def _step_3_finops_enrichment(self, excel_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save FinOps report
            finops_report_file = (
                self.finops_dir / f"cost-analysis-{self.timestamp}.json"
            )
            with open(finops_report_file, "w") as f:
                json.dump(finops_data, f, indent=2, default=str)

            print_success(
                f"FinOps enrichment complete: ${total_cost:.2f} network costs"
            )
            ...
            if error_code == "AccessDeniedException":
                print_warning(
                    f"Cost Explorer access denied for profile {self.billing_profile}"
                )
                print_info(
                    "Skipping FinOps enrichment (requires ce:GetCostAndUsage permission)"
                )
            else:
    ...
    async def _step_5_notebook_execution(self, excel_file: str) -> str:
        ...
        notebook_input = Path(
            "notebooks/automation/aws/network/vpc_optimization_dashboard.ipynb"
        )
    ...
    async def _step_6_workflow_summary(
        ...
        summary = {
            ...
            "outputs": {
                ...
                "diagrams_count": len(
                    workflow_results["steps"].get("diagrams", [])
                ),
                "notebook": workflow_results["steps"].get("notebook"),
                "finops_status": workflow_results["steps"]
                .get("finops", {})
                .get("status"),
            },
        }
        ...
        summary_file = (
            self.artifacts_dir / f"workflow-summary-{self.timestamp}.json"
        )
    ...
    def _print_workflow_completion(self, workflow_results: Dict[str, Any]):
        ...
        self.console.print("\n" + "=" * 80)
        self.console.print(
            "[bold green]✅ UNIFIED WORKFLOW COMPLETE[/bold green]"
        )
        ...
        # FinOps
        finops_status = (
            workflow_results["steps"].get("finops", {}).get("status")
        )
