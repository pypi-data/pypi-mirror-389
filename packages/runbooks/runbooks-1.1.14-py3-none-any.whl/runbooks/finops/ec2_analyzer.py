...
class EC2CostAnalyzer:
    ...
    def __init__(self, config: EC2AnalysisConfig):
        ...
        logger.debug(f"EC2 analyzer initialized with profiles: "
    ...
    def analyze(self, input_file: str, input_sheet: str = 'ec2', enable_decommission_scoring: bool = True) -> pd.DataFrame:
        """
    ...
    def _enrich_organizations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for col in ['account_name', 'account_email', 'wbs_code',
    ...
    def discover_instances_via_resource_explorer(
        self,
        aggregator_profile: str = 'ams-centralised-ops-ReadOnlyAccess-335083429030',
        aggregator_region: str = 'ap-southeast-2'
    ) -> pd.DataFrame:
        """
    ...
    def _enrich_ec2_context(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            print_info(f"SSO accessible instances: {sso_accessible_count}/{len(df)} ({sso_accessible_count/len(df)*100:.1f}%)")
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Enriching with 28-column EC2 metadata (HYBRID: SSO + AssumeRole)...",
                            ...
                            for reservation in response.get('Reservations', []):
                                for instance in reservation.get('Instances', []):
                                    ...
                                    if mask.any():
                                        ...
                                        enriched_count += 1
                                        logger.debug(f"Successfully enriched instance {instance_id} (count: {enriched_count})")
                                    else:
                                ...
                                for instance_id in batch:
                                    ...
                                    if mask.any():
                                        ...
                                        if 'monthly_cost' in df.columns and 'annual_cost_12mo' in df.columns:
                                            monthly = df.at[idx, 'monthly_cost'] if pd.notna(df.at[idx, 'monthly_cost']) else 0
                                            annual = df.at[idx, 'annual_cost_12mo'] if pd.notna(df.at[idx, 'annual_cost_12mo']) else 0
        ...
        metrics_table.add_row("Access Denied (Cross-Account)", f"{access_denied_count}/{total_instances}",
                             f"{(access_denied_count/total_instances*100):.1f}%" if total_instances > 0 else "0.0%")
        metrics_table.add_row("Terminated/Missing", f"{terminated_count}/{total_instances}",
    ...
    def _extract_instance_metadata(self, df: pd.DataFrame, idx: int, instance: Dict) -> None:
        ...
        security_groups = [sg.get('GroupName', sg.get('GroupId', 'N/A'))
        ...
        block_device_list = [f"{bd.get('DeviceName', 'N/A')}:{bd.get('Ebs', {}).get('VolumeId', 'N/A')}"
    ...
    def _enrich_volume_encryption(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if compliance_rate < 100:
            print_warning(f"⚠️  PCI-DSS/HIPAA compliance risk: {total_with_volumes - compliant_count} instances not fully encrypted")
    ...
    def _enrich_cost_rightsizing(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if terminate_count > 0:
                print_info(f"   Terminate recommendations: {terminate_count} instances, ${total_savings:.2f}/month savings")
    ...
    def export_markdown(self, df: pd.DataFrame, output_file: str) -> None:
        ...
        summary_metrics = {
            ...
            "MUST Tier (E1-E7: 80-100)": len(df[df['decommission_score'] >= 80]) if 'decommission_score' in df.columns else 0,
            "SHOULD Tier (E1-E7: 60-79)": len(df[(df['decommission_score'] >= 60) & (df['decommission_score'] < 80)]) if 'decommission_score' in df.columns else 0,
            "Estimated Monthly Savings": f"${df['monthly_cost'].sum():.2f}" if 'monthly_cost' in df.columns else "$0.00"
        }
    ...
    try:
        ...
        if decommission_mode:
            decommission_cols = [
                ...
                # Instance context (3)
                'instance_type', 'instance_state', 'launch_date',
                # Cost metrics (3)
                'monthly_cost', 'annual_cost_12mo', 'cost_trend',
                # Decommission scoring (4)
                'decommission_score', 'decommission_tier', 'decommission_reason', 'decommission_confidence',
                # E1-E7 signals (7)
                'e1_signal', 'e2_signal', 'e3_signal', 'e4_signal', 'e5_signal', 'e6_signal', 'e7_signal',
                # Signal evidence - Compute Optimizer (3)
                'compute_optimizer_finding', 'co_recommendation', 'co_lookback_days',
                # Signal evidence - CloudWatch (3)
                'p95_cpu', 'p95_network', 'p95_disk_io',
                # Signal evidence - CloudTrail (4)
                'days_since_activity', 'activity_count', 'last_activity_date', 'is_idle',
                # Signal evidence - SSM (2)
                'ssm_ping_status', 'ssm_last_ping_days',
                # Signal evidence - Attachments (1)
                'attached_to_service',
                # Organizations metadata (3)
                'wbs_code', 'cost_group', 'technical_owner',
                # Operational metadata (3)
                'vpc_id', 'subnet_id', 'availability_zone',
                # Tags (3)
                'Tags', 'Tag:costgroup', 'Tag:owner'
            ]
            # Filter to only columns that exist in DataFrame
            df_export = df[[col for col in decommission_cols if col in df.columns]]
            print_info(f"🎯 Decommission mode: Filtered to {len(df_export.columns)} focused columns (from {len(df.columns)} total)")
        else:
            df_export = df
