...
def create_finops_group():
    ...
    @common_aws_options
    @click.pass_context
    def finops(ctx, profile, region, dry_run, format, output_dir, export,
...
[dim]Analysis Period: {timeframe.title()} view | Account: {account_id}[/dim]""",
    ...
    def analyze_ec2(
        ...
        """
        Analyze EC2 costs with comprehensive Organizations and Cost Explorer enrichment.

        This command provides enterprise-grade EC2 cost analysis with:
        • 28-column EC2 context enrichment (instance type, state, networking, storage)
        • Organizations metadata (6 columns: account name, email, WBS code, cost group, owners)
        • Cost Explorer 12-month historical costs with trend analysis
        • Optional CloudTrail activity tracking for idle detection
        • Multi-sheet Excel export with cost visualizations

        Enterprise Features:
        • Multi-account support via Organizations API
        • 12-month cost trends with increase/decrease/stable indicators
        • Rich CLI cost hierarchy trees (Account → Instance → Cost)
        • Validation metrics and quality gate reports

        Examples:
            # Basic EC2 cost analysis with Organizations and Cost Explorer
            runbooks finops analyze-ec2 \\
                --input data/ec2.xlsx \\
                --output data/ec2-enriched.xlsx \\
                --management-profile mgmt-ro \\
                --billing-profile billing-ro

            # Comprehensive analysis with activity tracking
            runbooks finops analyze-ec2 \\
                -i data/ec2.xlsx \\
                -o data/ec2-full-analysis.xlsx \\
                -m mgmt-ro \\
                -b billing-ro \\
                --enable-activity

            # Minimal analysis (no cost data)
            runbooks finops analyze-ec2 \\
                -i data/ec2.xlsx \\
                -o data/ec2-org-only.xlsx \\
                -m mgmt-ro \\
                -b billing-ro \\
                --no-cost \\
                --no-12month-cost
        ...
        Input File Format:
            ...
            - Identifier: EC2 instance ID (i-xxxxx)
            - AWS Account: 12-digit AWS account ID
            - Region: AWS region (ap-southeast-2, etc.)

        Output File Format:
            Multi-sheet Excel workbook with:
            - Sheet 1: Enriched Data (28+ columns)
            - Sheet 2: Cost Summary (by account)
            - Sheet 3: Organizations Hierarchy
            - Sheet 4: Validation Metrics

        Performance Notes:
            • EC2 API enrichment: ~2-5 seconds per 100 instances
            • Cost Explorer API: ~3-5 seconds per account
            • CloudTrail activity (optional): 60-90 seconds total

        Pattern: Migrated from notebooks/compute/ec2.ipynb with production-grade error handling
        """
        try:
            ...
            print_success(f"   📊 Analyzed {len(result_df)} EC2 instances")
    ...
    def lambda_analysis(
        ...
        """
        Lambda cost optimization and serverless analysis.

        This command provides comprehensive Lambda function cost analysis with:
        • L1-L4 optimization signal framework
        • Organizations metadata (6 columns: account name, email, WBS code, cost group, owners)
        • Cost Explorer 12-month historical costs with trend analysis
        • CloudWatch metrics (invocations, errors, duration, throttles)
        • Idle function detection (zero invocations in 14d)
        • Memory rightsizing opportunities (oversized allocation detection)
        • Cold start analysis (p95/p50 duration ratios)

        Optimization Signals (L1-L4):
        • L1: High invocation cost (top 10% cost functions) → +10 points
        • L2: Idle functions (zero invocations in 14d) → +15 points
        • L3: Oversized memory (duration << timeout) → +8 points
        • L4: Cold start issues (high p95/p50 ratio) → +5 points

        Optimization Tiers:
        • HIGH (≥20 points): Immediate optimization candidates
        • MEDIUM (10-19 points): Review recommended
        • LOW (<10 points): Monitor and maintain

        Examples:
            # Basic Lambda analysis
            runbooks finops lambda-analysis --profile default

            # Multi-region analysis with specific output
            runbooks finops lambda-analysis \\
                --profile ops-profile \\
                --regions ap-southeast-2 ap-southeast-6 \\
                --output lambda-report.xlsx

            # Minimal analysis (no Organizations or Cost data)
            runbooks finops lambda-analysis \\
                --profile default \\
                --no-organizations \\
                --no-cost
        ...
        Output File Format:
            Multi-sheet Excel workbook with:
            - Sheet 1: Enriched Data (30+ columns + L1-L4 signals)
            - Sheet 2: Optimization Summary (by tier and account)
            - Sheet 3: Organizations Hierarchy
            - Sheet 4: Validation Metrics

        Performance Notes:
            • Function discovery: ~5-10 seconds per region
            • CloudWatch metrics: ~1-2 seconds per 50 functions
            • Cost Explorer API: ~3-5 seconds per account
            • Organizations API: ~2-3 seconds (one-time per execution)

        Pattern: New implementation following ec2_analyzer.py patterns for serverless optimization
        """
        try:
            ...
            print_success(f"   📊 Analyzed {len(result_df)} Lambda functions")
            print_success(f"   📁 Output: {output_file}")

            # Display optimization summary
            if len(result_df) > 0 and 'optimization_tier' in result_df.columns:
    ...
    def analyze_workspaces(
        ...
        """
        Analyze WorkSpaces costs with comprehensive Organizations and Cost Explorer enrichment.

        This command provides enterprise-grade WorkSpaces cost analysis with:
        • Organizations metadata (6 columns: account name, email, WBS code, cost group, owners)
        • Cost Explorer 12-month historical costs with trend analysis
        • Volume encryption analysis (PCI-DSS/HIPAA compliance)
        • CloudWatch UserConnected metrics (30-day usage patterns)
        • W1-W6 decommission signal integration
        • Optional CloudTrail activity tracking for idle detection
        • Multi-sheet Excel export with cost visualizations
        ...
        • Multi-account support via Organizations API
        • 12-month cost trends with increase/decrease/stable indicators
        • Rich CLI cost hierarchy trees (Account → WorkSpace → Cost)
        • Validation metrics and quality gate reports

        Examples:
            # Basic WorkSpaces cost analysis with Organizations and Cost Explorer
            runbooks finops analyze-workspaces \\
                --input data/workspaces.xlsx \\
                --output data/workspaces-enriched.xlsx \\
                --management-profile mgmt-ro \\
                --billing-profile billing-ro
            ...
            runbooks finops analyze-workspaces \\
                ...
                -o data/workspaces-full-analysis.xlsx \\
                -m mgmt-ro \\
                -b billing-ro \\
                --enable-activity
            ...
            runbooks finops analyze-workspaces \\
                -i data/workspaces.xlsx \\
                -o data/workspaces-org-only.xlsx \\
                -m mgmt-ro \\
                -b billing-ro \\
                --no-cost \\
                --no-12month-cost
        ...
        Output File Format:
            Multi-sheet Excel workbook with:
            - Sheet 1: Enriched Data (30+ columns)
            - Sheet 2: Cost Summary (by account)
            - Sheet 3: Organizations Hierarchy
            - Sheet 4: Validation Metrics

        Performance Notes:
            • WorkSpaces API enrichment: ~2-5 seconds per 100 WorkSpaces
            • Cost Explorer API: ~3-5 seconds per account
            • CloudTrail activity (optional): 60-90 seconds total

        Pattern: Matches EC2 analyzer architecture exactly for enterprise DRY compliance
        """
        try:
            ...
            print_success(f"\n✅ WorkSpaces analysis complete!")
            print_success(f"   📊 Analyzed {len(result_df)} WorkSpaces")
            print_success(f"   📁 Output: {output_file}")
