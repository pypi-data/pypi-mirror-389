...
Author: CloudOps Runbooks Team (sre-automation-specialist)
Version: 1.1.x
...
class MCPFirewallValidator:
    ...
    async def cross_validate_results(
        ...
        mcp_source: str = "awslabs.core-mcp"
    ...
    async def _query_mcp_vpc_topology(self, mcp_source: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for region in ec2_regions:
                try:
                    ec2_client = create_timeout_protected_client(
                        self.ops_session,
                        'ec2',
                        region_name=region
                    )
                    ...
                    for vpc in vpcs_response.get('Vpcs', []):
                        ...
                        inspection_status = await self._get_inspection_status_mcp(
                            ec2_client,
                            vpc_id,
                            region
                        )
    ...
    async def _get_inspection_status_mcp(
        self,
        ec2_client,
        vpc_id: str,
        region: str
        ...
        try:
            ...
            for attachment in attachments:
                ...
                if route_table_id:
                    ...
                    is_inspection_rt = await self._is_inspection_route_table(
                        ec2_client,
                        route_table_id
                    )
    ...
    async def _is_inspection_route_table(
        self,
        ec2_client,
        route_table_id: str
    ) -> bool:
        ...
        try:
            ...
            rt_response = ec2_client.describe_transit_gateway_route_tables(
...
"""
        ...
        panel = Panel(
            summary_text.strip(),
            title="🔍 MCP Firewall Bypass Cross-Validation",
            style=f"bold {accuracy_color}"
        )
        ...
        if result.validation_passed:
            print_success(
                f"✅ MCP validation PASSED: {result.accuracy_percentage}% accuracy achieved"
            )
        else:
            print_error(
                f"❌ MCP validation FAILED: {result.accuracy_percentage}% < {self.accuracy_target}%"
            )
...
if __name__ == "__main__":
    ...
    asyncio.run(validate_firewall_bypass_discovery(
        primary_results=example_primary_results,
        management_profile='ams-admin-ReadOnlyAccess-909135376185',
        operational_profile='ams-centralised-ops-ReadOnlyAccess-335083429030',
