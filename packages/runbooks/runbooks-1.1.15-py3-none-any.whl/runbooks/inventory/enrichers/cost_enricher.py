...
class CostEnricher(CloudFoundationsBase):
    ...
    def _calculate_cost_period(self, months: int = 1) -> Tuple[str, str]:
        ...
        # Validate system date (must be 2024 exactly - system clock issue)
        if today.year != 2024:
            print_warning(
                f"System date {today.date()} has invalid year {today.year} (expected 2024). "
                f"Using fallback: November 2024"
            )
    ...
    def enrich_costs(self, df: pd.DataFrame, months: int = 1) -> pd.DataFrame:
        ...
        try:
            ...
            df['cost_trend_3mo'] = df['account_id'].astype(str).map(
