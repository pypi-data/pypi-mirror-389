...
Usage:
    ...
    enricher = ActivityEnricher(operational_profile='ams-centralised-ops-ReadOnlyAccess-335083429030')
...
class ActivityEnricher:
    ...
    def enrich_activity(self, df: pd.DataFrame, resource_type: str) -> pd.DataFrame:
        ...
        """
        print_info(f"🔄 Starting multi-API activity enrichment for {len(df)} {resource_type} resources...")
    ...
    def _enrich_cloudtrail(self, df: pd.DataFrame, resource_type: str) -> pd.DataFrame:
        ...
        """
        print_info("   📋 CloudTrail: Querying 90-day activity history...")
        ...
        with create_progress_bar() as progress:
            task = progress.add_task("[cyan]CloudTrail activity...", total=len(df))
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    ...
                    if events:
                        ...
                        # No events found - potential abandonment signal
        ...
        with create_progress_bar() as progress:
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    ...
                    # Graceful degradation on errors
                    logger.warning(f"CloudWatch metrics failed for {resource_id}: {e}")
                    pass
    ...
    def _enrich_ssm(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            managed_count = len([s for s in ssm_status.values() if s['ping_status'] not in ['Not SSM managed', 'SSM access denied']])
