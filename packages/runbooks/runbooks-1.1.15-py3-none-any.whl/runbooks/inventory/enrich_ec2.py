...
class EC2Enricher:
    ...
    def _enrich_cost(self, ec2_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ce_client = boto3.Session(
                profile_name=self.billing_profile
            ).client('ce', region_name='ap-southeast-2')
    ...
    def _enrich_activity(self, ec2_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ct_client = boto3.Session(
                profile_name=self.management_profile
            ).client('cloudtrail', region_name=self.region)
        except Exception as e:
        ...
        with create_progress_bar() as progress:
            task = progress.add_task(
                "[cyan]Enriching with CloudTrail...",
                total=len(ec2_df)
            )
...
@click.option(
    '--profile', '-p',
    default='default',
    help='AWS management profile (Organizations + CloudTrail access)'
)
@click.option(
    '--billing-profile', '-b',
    default=None,
