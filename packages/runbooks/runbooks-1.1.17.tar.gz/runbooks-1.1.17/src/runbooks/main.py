"""
CloudOps Runbooks - Enterprise CLI Interface (SLIM VERSION)
...
def display_banner():
    ...
    banner = f"""
[bold blue]CloudOps Runbooks[/bold blue] [dim]v{__version__}[/dim]
...
def main(ctx, debug, log_level, json_output, profile, region, dry_run, config):
    """
    CloudOps Runbooks - Enterprise AWS Automation CLI
    ...
    if ctx.invoked_subcommand is None:
        if _HAS_RICH:
            ...
            console.print("\n[bold blue]CloudOps Runbooks[/bold blue] [dim]Enterprise AWS Automation Platform[/dim]\n")

            # Cost Optimization category
            console.print(create_categorized_help_panel(
                category="Cost Optimization",
                commands=["finops compute-costs", "finops savings-plans", "finops analyze-ec2", "finops analyze-workspaces"],
            ...
            console.print(create_categorized_help_panel(
                ...
                commands=["inventory resource-explorer", "inventory enrich-accounts", "inventory enrich-costs", "inventory enrich-activity", "inventory score-decommission"],
...
def list_commands(ctx, output_format):
    ...
    if output_format == "json":
        ...
        # Rich table format
        table = Table(title="CloudOps Runbooks Commands", show_header=True)
