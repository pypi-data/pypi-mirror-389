...
def validate_profile(profile_name: str, region: str = "ap-southeast-2") -> Dict:
    ...
    Example:
        >>> result = validate_profile("ams-billing-ViewBillingAccess-335083429030")
        >>> # ✅ Profile validated: account 335083429030
    """
    try:
...
def validate_profiles_batch(
    profiles: List[str], region: str = "ap-southeast-2", fail_fast: bool = True
) -> Dict:
    """
    ...
    for profile_name in profiles:
        ...
        if result["valid"]:
            ...
            if fail_fast:
                raise ValueError(
                    f"Profile validation failed: {profile_name} - {result['error']}"
                )
    ...
    if all_valid:
        print_success(
            f"✅ All {valid_count} profiles validated successfully"
        )
    else:
        print_warning(
            f"⚠️  Profile validation: {valid_count} valid, {invalid_count} invalid"
        )
...
def validate_csv_profiles(csv_path: str, aws_config_path: Optional[str] = None) -> Dict:
    ...
    if "AWS-Profile" not in df.columns:
        if aws_config_path:
            ...
            raise ValueError(
                f"CSV missing AWS-Profile column. "
                "Provide aws_config_path to auto-enrich or add column manually."
            )
    ...
    print_info(
        f"🔍 Validating {len(unique_profiles)} unique profiles from {csv_path}"
    )

    # Validate all profiles (fail-fast on errors)
    validation_results = validate_profiles_batch(
        unique_profiles, fail_fast=True
    )
