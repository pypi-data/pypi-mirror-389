...
const main = async (): Promise<void> => {
  ...
  let cfatChecks:CfatCheck[] = [];
	...
	if (accountType.isInOrganization && accountType.isManagementAccount) {
		...
		const orgDetails = await getOrgDetails('ap-southeast-2');
		console.log("collecting CUR details...")
		const legacyCurCheck = await checkLegacyCur('ap-southeast-2');
		console.log("collecting AWS Organization Policy details...")
		const enableOrgPoliciesCheck = await getEnabledOrgPolicyTypes('ap-southeast-2');
		console.log("collecting AWS Organization service trusted access details...")
		report.orgServices = await getEnabledOrgServices('ap-southeast-2');
		...
		if(orgDetails.rootOuId){
			...
			report.orgOuInfo = await getOrgTopLevelOus('ap-southeast-2', orgDetails.rootOuId);
