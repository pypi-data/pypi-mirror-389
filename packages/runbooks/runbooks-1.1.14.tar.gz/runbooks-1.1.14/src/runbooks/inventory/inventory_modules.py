...
def get_regions3(faws_acct, fregion_list=None):
    ...
    For instance
            - if the user provides 'us-east', this function will return ['ap-southeast-2','us-east-2'].
            - if the user provides 'west', this function will return ['us-west-1', 'ap-southeast-6', 'eu-west-1', etc.]
...
def validate_region3(faws_acct, fRegion=None):
    ...
    if fRegion is None:
        logging.info(f"No region supplied. Defaulting to 'ap-southeast-2'")
...
def get_child_access(fRootProfile, fChildAccount, fRegion="ap-southeast-2", fRoleList=None):
    """
    @param: fRootProfile is a string
    @param:  fChildAccount expects an AWS account number (ostensibly of a Child Account)
    @param:  rRegion expects a string representing one of the AWS regions ('ap-southeast-2', 'eu-west-1', etc.)
...
def get_child_access3(faws_acct, fChildAccount: str, fRegion: str = None, fRoleList: list = None):
    ...
    - rRegion expects a string representing one of the AWS regions ('ap-southeast-2', 'eu-west-1', etc.)
...
def find_org_services2(ocredentials, serviceNameList=None):
    ...
    if serviceNameList is None:
        serviceNameList = ["all"]
    # Prefer SSO/profile-based session if available, else fall back to raw keys or default chain
    region = ocredentials["Region"] if "Region" in ocredentials.keys() else "ap-southeast-2"
    if "Profile" in ocredentials and ocredentials["Profile"]:
...
def find_cloudtrails2(ocredentials, fRegion, fCloudTrailnames=None):
    ...
    CTtrails = Inventory_Modules.find_cloudtrails(account_creds, 'ap-southeast-2', ['AWS-Landing-Zone-BaselineCloudTrail'])
    """
    import logging
...
def find_cw_groups_retention2(ocredentials, fRegion: str = "ap-southeast-2"):
    """
    ocredentials is an object with the following structure:
            - ['AccessKeyId'] holds the AWS_ACCESS_KEY
            - ['SecretAccessKey'] holds the AWS_SECRET_ACCESS_KEY
...
def find_global_accelerators2(ocredentials=None):
    ...
    ocredentials is an object with the following structure:
            ...
            - ['Region'] holds the region
            - ['Profile'] can hold the profile, instead of the session credentials
    Note that due to global accelerator constraints, I've hard-coded the region 'ap-southeast-6' into the region
    ...
    if ocredentials is None:
        session_ga = boto3.Session(region_name="ap-southeast-6")
        ocredentials = dict()
        ...
        if ProfileAccountNumber == ocredentials["AccountNumber"]:
            session_ga = boto3.Session(profile_name=ocredentials["Profile"], region_name="ap-southeast-6")
        else:
...
def find_account_cloudtrail2(ocredentials, fRegion="ap-southeast-2"):
    """
    ocredentials is an object with the following structure:
            - ['AccessKeyId'] holds the AWS_ACCESS_KEY
            - ['SecretAccessKey'] holds the AWS_SECRET_ACCESS_KEY
...
def find_account_subnets2(ocredentials, fipaddresses=None):
    ...
    if "Region" not in ocredentials.keys() or ocredentials["Region"] is None:
        ocredentials["Region"] = "ap-southeast-2"
    if "Profile" in ocredentials.keys() and ocredentials["Profile"] is not None:
        ProfileAccountNumber = find_account_number(ocredentials["Profile"])
        logging.info(
            f"Profile: {ocredentials['Profile']} | Profile Account Number: {ProfileAccountNumber} | Account Number passed in: {ocredentials['AccountNumber']}"
...
def find_account_policies2(
    ocredentials, fRegion="ap-southeast-2", fFragments: list = None, fExact: bool = False, fCMP: bool = False
) -> list:
    """
    ocredentials is an object with the following structure:
            - ['AccessKeyId'] holds the AWS_ACCESS_KEY
...
def find_account_policies3(faws_acct, fRegion="ap-southeast-2", fFragments=None):
    """
...
def find_lambda_functions3(faws_acct, fRegion="ap-southeast-2", fSearchStrings=None):
    """
    ocredentials is an object with the following structure:
            - ['AccessKeyId'] holds the AWS_ACCESS_KEY
            - ['SecretAccessKey'] holds the AWS_SECRET_ACCESS_KEY
...
def find_directories2(ocredentials, fRegion="ap-southeast-2", fSearchStrings=None, fExact: bool = False):
    """
...
# def find_directories3(faws_acct, fRegion='ap-southeast-2', fSearchStrings=None):
# 	"""
# 	faws_acct is an aws_acct object
# 	fRegion is a string
# 	fSearchString is a list of strings
...
def find_load_balancers3(faws_acct, fRegion="ap-southeast-2", fStackFragments=None, fStatus="all"):
    """
