#!/usr/bin/env python3
"""
Dual Diagram Generation Service - CloudOps Runbooks VPC Module
...
class DualDiagramService:
    ...
    def generate_via_runbooks(
        self, diagram_types: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        print_info(
            f"[Method 1] Generating diagrams via Runbooks API (profile: {self.profile}, region: {self.region})"
        )
            ...
            for diagram_type in target_types:
                try:
                    ...
                    if method:
                        ...
                        output_filename = str(
                            self.output_dir_runbooks / f"{diagram_type}.png"
                        )
                        ...
                        results["diagrams"][diagram_type] = {
                            ...
                            "size_bytes": Path(diagram_path).stat().st_size
                            if Path(diagram_path).exists()
                            else 0,
                        }

                        print_success(
                            f"✅ Generated {diagram_type} via Runbooks API: {diagram_path}"
                        )
                    else:
                        ...
                        print_warning(
                            f"⚠️  {diagram_type} not yet implemented in Runbooks API"
                        )
        ...
        success_count = sum(
            1
            for d in results["diagrams"].values()
        ...
        results["summary"] = {
            ...
            "failed": error_count,
            "success_rate": (
                success_count / len(target_types) * 100 if target_types else 0
            ),
        }
    ...
    async def generate_via_mcp(
        ...
        results = {
            ...
            "implementation_notes": {
                ...
                "reference_config": {
                    ...
                    "env": {
                        "AWS_PROFILE": self.config.aws_centralised_ops_profile
                        or self.profile,
        ...
        print_warning(
            "⚠️  MCP diagram generation not yet implemented. Future integration will:"
        )
    ...
    def compare_outputs(
        ...
        runbooks_success = sum(
            1
            for d in comparison["diagrams"].values()
            if d["runbooks_status"] == "success"
        )
        mcp_success = sum(
            1 for d in comparison["diagrams"].values() if d["mcp_status"] == "success"
        )
        both_success = sum(
            1
            for d in comparison["diagrams"].values()
        ...
        comparison["summary"] = {
            ...
            "runbooks_success_rate": (
                runbooks_success / total_diagrams * 100 if total_diagrams else 0
            ),
            "mcp_success_rate": (
                mcp_success / total_diagrams * 100 if total_diagrams else 0
            ),
            "dual_method_coverage": (
                both_success / total_diagrams * 100 if total_diagrams else 0
            ),
        ...
        comparison_file = (
            self.output_dir_comparison / "dual-diagram-validation.json"
        )
    ...
    def _assess_consistency(
        self, runbooks_info: Dict[str, Any], mcp_info: Dict[str, Any]
    ) -> str:
        """
        ...
        if runbooks_status == "success" and mcp_status == "success":
            ...
            runbooks_status == "not_implemented" and mcp_status == "not_implemented"
        ):
    ...
    def display_comparison_table(self, comparison: Dict[str, Any]) -> None:
        ...
        summary_table.add_row(
            "Total Diagram Types", str(summary["total_diagram_types"])
        )
        summary_table.add_row(
    ...
    async def generate_all_dual_methods(
        self, diagram_types: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
...
if __name__ == "__main__":
    ...
    print(
        f"  Method 1 (Runbooks): {results['method_1_runbooks']['summary']['successful']} diagrams"
    )
    print(
        f"  Method 2 (MCP): {results['method_2_mcp']['status']} (implementation pending)"
    )
