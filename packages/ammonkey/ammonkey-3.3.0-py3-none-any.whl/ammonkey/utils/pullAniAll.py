'''
An iterating finisher for anipose triangulate + collect final CSVs
Comes:
    AFTER DLC processed data.   [pullDLCAll.py]
...
def getAllDates(raw_dir:Path) -> list[Path]:
    '''go thru all dates'''
    days = []
    ...
    # for praw, pdat in zip(raws, datas):
        with silence(True):
            ...
            if len(csv_paths) == 0:
                ...
                continue
            ets = mky.getTasksInDAET(PPATH_RAW=str(praw), task=t) # date-animal-Experiment-Task 'S
            if ets: # has target task
                stats = {et:getCSVStatus(csv_paths, et) for et in ets}
                recordStatsSummary(summary_csv, praw, stats)
                # print(stats)
                r'''if any([stat != 3 for stat in stats.values()]):
                    print(praw.name)
                    ...
                    if not os.path.exists(mky.pm.ani_base_path):
                        continue
