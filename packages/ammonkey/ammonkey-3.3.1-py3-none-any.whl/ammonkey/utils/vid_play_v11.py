...
class MarkersWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        duration_ratio = (
            (slider_width - TIMELINE_OFFSET[1]) / total_duration
            if total_duration
            else 0
        )
...
class VideoPlayer(QMainWindow):
    ...
    def loadEvents(self):
        file_name, _ = QFileDialog.getOpenFileName(
            self,
            "Select event file",
            self.settings.value("Path/last_evt_path", self.fname, type=str),
            "Text file (*.txt)",
        )
        if file_name:
    ...
    def update_position(self, position=None):
        ...
        current_time = (
            QTime(0, 0, 0).addMSecs(self.media_player.position()).toString("HH:mm:ss")
        )
        duration = (
            QTime(0, 0, 0).addMSecs(self.media_player.duration()).toString("HH:mm:ss")
        )
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if (
            ...
            self.delicate and self.pending_num and (event.text().isalpha())
        ):  #  or event.text().isnumeric()
        ...
        # Ctrl+←: jump to the previous marker
            ...
            all_frames = sorted(
                f for frames in self.event_markers.values() for f in frames
            )
            if not all_frames:
                return
    ...
    def mark_event(self, event_type):
        try:
            if self.pending_frame is None:
                frame = round(
                    (self.media_player.position() + MAGIC) / (1000 / FPS_ORIG)
                )
            else:
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, "fname"):
            p = (
                ...
                if "." in os.path.basename(self.fname)
                else self.fname
            )
    ...
    def saveEventToFile(self):
        ...
        if (
            any(self.event_markers.values())
            and len(self.redo_stack) + len(self.undo_stack) > 0
            ...
            try:
                ...
                m = re.search(
                    r"2025\d{4}-(Pici|Fusillo)-(TS|BBT|Brinkman|Pull).*?-\d{1,2}",
                    self.fname,
                    re.IGNORECASE,
                )
                if m:
                ...
                base_path = self.settings.value(
                    "Path/save_path",
                    os.path.join(
                        os.path.dirname(os.path.abspath(__file__)), "Marked Events"
                    ),
                    type=str,
                )
                ...
                while os.path.exists(
                    os.path.join(base_path, f"event-{fnm}{suffix}.txt")
                ):
                    suffix += " (new)"
                file_path = os.path.join(base_path, f"event-{fnm}{suffix}.txt")
                ...
                raise RuntimeError(
                    f"Error when saving events; plz copy it yourself!!\n{e}"
                )
    ...
    def saveEventAs(self):
        if any(self.event_markers.values()):
            ...
            try:
                base_path = self.settings.value(
                    "Path/save_path",
                    os.path.join(os.path.dirname(self.fname), "Marked Events"),
                    type=str,
                )
                ...
                print(self.event_markers)
                raise RuntimeError(
                    f"Error when saving events as new file; plz copy it yourself!!\n{e}"
                )
