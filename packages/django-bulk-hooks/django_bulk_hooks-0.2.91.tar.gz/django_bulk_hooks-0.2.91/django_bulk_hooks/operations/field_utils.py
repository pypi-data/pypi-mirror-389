...
def _extract_fk_value(obj, field_name, field, model_cls):
    ...
    if value is None:
        logger.debug("FIELD_VALUE_MTI_FALLBACK: Direct access to '%s' returned None, trying MTI fallback",
                     attname)
        ...
        elif obj.__class__ != model_cls and not field_was_explicitly_set:
            # Try database refresh for existing records (original MTI fallback)
