...
def get_field_value_for_db(obj, field_name, model_cls=None):
    ...
    logger.debug(f"FIELD_VALUE_EXTRACTION: Getting field '{field_name}' from {obj.__class__.__name__} instance (pk={getattr(obj, 'pk', 'None')})")

    try:
        field = model_cls._meta.get_field(field_name)
        logger.debug(f"FIELD_VALUE_FIELD_FOUND: Field '{field_name}' is {type(field).__name__} on {field.model.__name__}")
    except Exception as e:  # noqa: BLE001
        # Field doesn't exist - just get attribute
        logger.debug(f"FIELD_VALUE_FIELD_NOT_FOUND: Field '{field_name}' not found via _meta.get_field, falling back to getattr")
        value = getattr(obj, field_name, None)
        logger.debug(f"FIELD_VALUE_FALLBACK: getattr result for '{field_name}': {value} (type: {type(value)})")
        return value
    ...
    if isinstance(field, ForeignKey):
        ...
        logger.debug(f"FIELD_VALUE_FK_EXTRACTION: FK field '{field_name}' using attname '{attname}', value: {value} (type: {type(value)})")
        return value

    # Regular field - get normally
    value = getattr(obj, field_name, None)
    logger.debug(f"FIELD_VALUE_REGULAR_EXTRACTION: Regular field '{field_name}', value: {value} (type: {type(value)})")
    return value
