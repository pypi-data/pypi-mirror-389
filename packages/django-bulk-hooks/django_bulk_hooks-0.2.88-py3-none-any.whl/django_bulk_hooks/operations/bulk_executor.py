    ...
    def _execute_mti_update_plan(self, plan):
        ...
        with transaction.atomic(using=self.queryset.db, savepoint=False):
            ...
            for field_group in plan.field_groups:
                ...
                case_statements = {}
                logger.debug(f"MTI_UPDATE_BUILD_CASE: Building CASE statements for {field_group.model_class.__name__} with {len(field_group.fields)} fields: {field_group.fields}")

                for field_name in field_group.fields:
                    logger.debug(f"MTI_UPDATE_FIELD_CASE: Building CASE for field '{field_name}' on {field_group.model_class.__name__}")
                    field = field_group.model_class._meta.get_field(field_name)
                    when_statements = []
                    logger.debug(f"MTI_UPDATE_FIELD_TYPE: Field '{field_name}' is {type(field).__name__}")


                    # Debug for relation fields
                    if field.related_model is not None:
                        logger.debug(f"MTI_UPDATE_CASE_RELATION: Field '{field_name}' is relation to {field.related_model.__name__}")

                    # Determine the correct output field for type casting
                    # Use the field itself for proper database type casting
                    case_output_field = field
                    logger.debug(f"MTI_UPDATE_CASE_OUTPUT_FIELD: Using {case_output_field} for type casting")

                    # Log all field values for all objects before building CASE statements
                    logger.debug(f"MTI_UPDATE_FIELD_VALUES_START: Showing all values for field '{field_name}' on {field_group.model_class.__name__}")
                    for i, (pk, obj) in enumerate(zip(root_pks, plan.objects)):
                        obj_pk = getattr(obj, "pk", None) or getattr(obj, "id", None)
                        if obj_pk is None:
                            logger.debug(f"MTI_UPDATE_FIELD_VALUES_OBJ_{i}: obj with no PK, skipping")
                            continue

                        # Get the field value using centralized field extraction
                        raw_value = get_field_value_for_db(obj, field_name, field_group.model_class)
                        converted_value = field.to_python(raw_value)
                        logger.debug(f"MTI_UPDATE_FIELD_VALUES_OBJ_{i}: obj_pk={obj_pk}, field='{field_name}', "
                                f"raw_value={raw_value} ({type(raw_value).__name__}), "
                                f"converted_value={converted_value} ({type(converted_value).__name__})")
                    logger.debug(f"MTI_UPDATE_FIELD_VALUES_END: Finished showing values for field '{field_name}'")

                    for pk, obj in zip(root_pks, plan.objects):
                        obj_pk = getattr(obj, "pk", None) or getattr(obj, "id", None)
                        if obj_pk is None:
                            logger.debug(f"MTI_UPDATE_SKIP_OBJ: Skipping obj with no PK")
                            continue

                        logger.debug(f"MTI_UPDATE_OBJ_PROCESSING: Processing obj_pk={obj_pk} for field '{field_name}'")

                        # Get the field value using centralized field extraction
                        value = get_field_value_for_db(obj, field_name, field_group.model_class)
                        logger.debug(f"MTI_UPDATE_VALUE_EXTRACTED: obj_pk={obj_pk}, field_name={field_name}, raw_value={value}, raw_type={type(value)}")

                        # Ensure proper type conversion for the database
                        # Django fields can have string values from forms/JSON, but we need proper Python types for CASE statements
                        value = field.to_python(value)
                        logger.debug(f"MTI_UPDATE_VALUE_CONVERTED: obj_pk={obj_pk}, field_name={field_name}, "
                                    f"converted_value={value}, converted_type={type(value)}")

                        # Use Cast for all values to ensure proper type coercion in CASE statements
                        # PostgreSQL CASE statements need explicit casting to match column types
                        when_statement = When(
                            **{field_group.filter_field: pk},
                            then=Cast(Value(value), output_field=case_output_field),
                        )
                        when_statements.append(when_statement)
                        logger.debug(f"MTI_UPDATE_WHEN_CREATED: obj_pk={obj_pk}, when_condition={{'{field_group.filter_field}': {pk}}}, then_value={value}")

                    if when_statements:
                        case_statements[field_name] = Case(*when_statements, output_field=case_output_field)
                        logger.debug(f"MTI_UPDATE_CASE_CREATED: Field '{field_name}' has {len(when_statements)} WHEN statements")
                    else:
                        logger.debug(f"MTI_UPDATE_CASE_EMPTY: Field '{field_name}' has no WHEN statements")

                # Execute bulk update
                if case_statements:
                    logger.debug(f"MTI_UPDATE_EXECUTE: Executing update for {field_group.model_class.__name__} with {len(case_statements)} fields: {list(case_statements.keys())}")
                    logger.debug(f"MTI_UPDATE_FILTER: Filtering on {field_group.filter_field}__in with {len(root_pks)} PKs: {root_pks}")

                    # Log detailed field update information
                    logger.debug(f"MTI_UPDATE_MODEL_SUMMARY: Updating {field_group.model_class.__name__} "
                                f"with {len(case_statements)} fields for {len(root_pks)} records")
                    for field_name, case_stmt in case_statements.items():
                        logger.debug(f"MTI_UPDATE_FIELD_SUMMARY: Field '{field_name}' on "
                                    f"{field_group.model_class.__name__} has CASE statement with "
                                    f"{len(case_stmt.source_expressions)} WHEN clauses")

                    try:
                        # Log the query before execution
                        query_qs = base_qs.filter(**{f"{field_group.filter_field}__in": root_pks})
                        logger.debug(f"MTI_UPDATE_QUERY: Base filter query SQL: {query_qs.query}")
                        logger.debug(f"MTI_UPDATE_CASE_STATEMENTS: Case statements: {case_statements}")
                        logger.debug(f"MTI_UPDATE_UPDATE_FIELDS: Update fields dict: {case_statements}")

                        # Log current database state before update
                        logger.debug(f"MTI_UPDATE_PRE_UPDATE_STATE: Checking current values in {field_group.model_class.__name__}")
                        current_values = list(query_qs.values_list('pk', *case_statements.keys())[:5])  # Limit to first 5 for logging
                        for i, values in enumerate(current_values):
                            pk_val = values[0]
                            field_values = values[1:]
                            logger.debug(f"MTI_UPDATE_PRE_UPDATE_ROW_{i}: pk={pk_val}, "
                                    f"current_values={dict(zip(case_statements.keys(), field_values))}")

                        updated_count = query_qs.update(**case_statements)
                        logger.debug(f"MTI_UPDATE_RESULT: Updated {updated_count} records in {field_group.model_class.__name__}")

                        # Log state after update
                        logger.debug(f"MTI_UPDATE_POST_UPDATE_STATE: Checking updated values in {field_group.model_class.__name__}")
                        updated_values = list(base_qs.filter(**{f"{field_group.filter_field}__in": root_pks}).values_list('pk', *case_statements.keys())[:5])  # Limit to first 5 for logging
                        for i, values in enumerate(updated_values):
                            pk_val = values[0]
                            field_values = values[1:]
                            logger.debug(f"MTI_UPDATE_POST_UPDATE_ROW_{i}: pk={pk_val}, "
                                    f"updated_values={dict(zip(case_statements.keys(), field_values))}")

                        total_updated += updated_count
                    except Exception as e:
                        logger.error(f"MTI bulk update failed for {field_group.model_class.__name__}: {e}")
                        logger.error(f"MTI_UPDATE_FAILED_QUERY: Failed query: {base_qs.filter(**{f'{field_group.filter_field}__in': root_pks}).query}")
                        logger.error(f"MTI_UPDATE_FAILED_CASE: Failed case statements: {case_statements}")
                        # Re-raise to preserve original behavior
                        raise
                else:
                    logger.debug(f"MTI_UPDATE_SKIP: No case statements for {field_group.model_class.__name__}, skipping update")
