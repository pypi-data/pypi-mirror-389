    ...
    while index < len(spinham._1):
        ...
        if spinham._1[index][0] == alpha:
            ...
            if when_present.lower() == "replace":
                spinham._1[index][1] = parameter
            # Or add
            elif when_present.lower() == "add":
                spinham._1[index][1] = spinham._1[index][1] + parameter
            # Or replace with mean value
            elif when_present.lower() == "mean":
                spinham._1[index][1] = (spinham._1[index][1] + parameter) / 2.0
            # Or do nothing
            ...
            # Or raise an error
                ...
                raise ValueError(
                    f"(One spin & one site) parameter is already set for atom {alpha} ('{spinham.atoms.names[alpha]}'."
                )
            else:
                raise ValueError(
