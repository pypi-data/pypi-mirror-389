...
class RunnerSession:
    ...
    async def execute(
        ...
        # Validate build or TestFlight parameters
        if app_build_path:
            appium_caps["appium:app"] = app_build_path
        elif app_store_id:
            logger.info(
                "Using TestFlight for app installation", app_store_id=app_store_id
            )
        else:
            raise ValueError(
                "Either app_build_path or app_store_id must be provided to start session"
            )
        ...
        for test_info in tests:
            ...
            try:
                async with AppiumClient(
                    ...
                    # Install app via TestFlight if using app_store_id
                    if app_store_id:
                        logger.info(
                        ...
                        try:
                            success = await appium_client.install_app_testflight(
                                app_bundle_id=app_bundle_id, app_store_id=app_store_id
                            )
                            if not success:
                                raise AppiumError(
                                    appium_url=appium_url,
                                    original_error="TestFlight installation timeout or failed",
                                )
                            logger.info("App installed via TestFlight")
                        except Exception as e:
                            logger.error(
                                "TestFlight installation failed",
                                error=str(e),
                                exc_info=True,
                            )
                            raise
