...
function ecc206_tflxauth_json(jsonOBJ) {
...
function ecc206_tflxauth_proto_prov(jsonOBJ) {
  console.log([String(jsonOBJ)]);
...
$("input[type=radio][name=devIface]").change(function () {
  ...
  if (this.value == "i2c") {
    ...
    fetch(
      "http://localhost:5001/boards/is_kitparser_updated/?board_name=DM320118&target_version=3.0.7"
    )
      .then(function (res) {
        return res.json();
      })
...
counterInput.addEventListener("change", (e) => {
  ...
  counter_val = document.getElementById("counterVal").value
    ...
    : 10000;
  document.getElementById("limitedUsePrivate").title =
    "Connect ECC Private Key to Monotonic Counter for " +
    counter_val +
    " counts";
  document.getElementById("limitedUseSecret").title =
