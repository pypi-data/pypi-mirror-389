...
function gererateXML(xml_type) {
  ...
  let jsObj = { base_xml: "ECC206_TFLXAUTH" };
  Object.assign(jsObj, { ["xml_type"]: xml_type });
  Object.assign(jsObj, { ["interface"]: "swi" });
  ...
  partNumberString = document
    ...
    .value.toUpperCase();
  if (partNumberString == "") partNumberString = "ECC206-MAHAA-T";
  ...
  if (
    ...
    if (xml_type === "proto_xml" || xml_type === "prod_xml") {
      ecc206_tflxauth_json(JSON.stringify(jsObj));
    } else {
      ecc206_tflxauth_proto_prov(JSON.stringify(jsObj));
    }
  }
}
