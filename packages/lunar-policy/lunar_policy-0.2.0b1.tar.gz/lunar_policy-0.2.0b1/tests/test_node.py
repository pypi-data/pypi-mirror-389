...
class TestDataLoading:
    def test_from_merged_json(self):
        merged_json = {'string': 'hello world'}
        data = Node.from_merged_json(json.dumps(merged_json))
        assert data.get_value('.string') == 'hello world'
        assert data.get_all_values('.string') == ['hello world']

    def test_from_merged_json_invalid_json(self):
        merged_json = '}'
        with pytest.raises(json.JSONDecodeError):
            Node.from_merged_json(merged_json)
    ...
    def test_from_bundle_json(self):
        ...
        data = Node.from_bundle_json(json.dumps(test_data))
        assert data.get_value('.string') == 'hello world'
        assert data.get_all_values('.string') == ['hello world']

    def test_from_bundle_json_invalid_json(self):
        test_data = '}'
        with pytest.raises(json.JSONDecodeError):
            Node.from_bundle_json(test_data)

    def test_from_bundle_missing_bundle_info(self):
        test_data = {'metadata_instances': [], 'merged_blob': {}}
        with pytest.raises(ValueError):
            Node.from_bundle_json(json.dumps(test_data))

    def test_from_bundle_missing_metadata_instances(self):
        test_data = {'bundle_info': {}, 'merged_blob': {}}
        with pytest.raises(ValueError):
            Node.from_bundle_json(json.dumps(test_data))

    def test_from_bundle_missing_merged_blob(self):
    ...
    def test_from_file(self):
        ...
        data = Node.from_file(FSPath(tmp_path))
        assert data.get_value('.string') == 'hello world'
        assert data.get_all_values('.string') == ['hello world']

    def test_from_file_invalid_path(self):
        with pytest.raises(FileNotFoundError):
            Node.from_file(FSPath('/my/path/does_not_exist.json'))

    def test_from_file_relative_path(self):
        with pytest.raises(ValueError):
            Node.from_file(FSPath('relative/path.json'))
    ...
    def test_from_file_invalid_json(self):
        ...
        with pytest.raises(json.JSONDecodeError):
            Node.from_file(FSPath(tmp_path))
...
class TestGetMerged:
    ...
    def snippet_data(self):
        ...
        return Node.from_bundle_json(json.dumps(test_data))
