...
from dharitri_py_sdk.core.tokens import Token
...
def network_config_from_response(raw_response: dict[str, Any]) -> NetworkConfig:
    chain_id = raw_response.get("drt_chain_id", "")
    gas_per_data_byte = raw_response.get("drt_gas_per_data_byte", 0)
    gas_price_modifier = float(raw_response.get("drt_gas_price_modifier", 0))
    min_gas_limit = raw_response.get("drt_min_gas_limit", 0)
    min_gas_price = raw_response.get("drt_min_gas_price", 0)
    extra_gas_limit_guarded_tx = raw_response.get("drt_extra_gas_limit_guarded_tx", 0)
    num_shards = raw_response.get("drt_num_shards_without_meta", 0)
    round_duration = raw_response.get("drt_round_duration", 0)
    rounds_per_epoch = raw_response.get("drt_rounds_per_epoch", 0)
    genesis_timestamp = raw_response.get("drt_start_time", 0)
...
def network_status_from_response(raw_response: dict[str, Any]) -> NetworkStatus:
    block_timestamp = raw_response.get("drt_block_timestamp", 0)
    block_nonce = raw_response.get("drt_nonce", 0)
    highest_final_nonce = raw_response.get("drt_highest_final_nonce", 0)
    current_round = raw_response.get("drt_current_round", 0)
    currernt_epoch = raw_response.get("drt_epoch_number", 0)
