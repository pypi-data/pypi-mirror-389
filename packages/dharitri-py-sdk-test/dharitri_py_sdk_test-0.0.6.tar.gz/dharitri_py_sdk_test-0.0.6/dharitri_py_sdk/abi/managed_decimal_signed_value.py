...
from dharitri_py_sdk.abi.shared import decode_length, read_bytes_exactly
...
class ManagedDecimalSignedValue:
    ...
    def decode_top_level(self, data: bytes):
        ...
        if self.is_variable:
            # read BigInt value length in bytes
            value_length = self._unsigned_from_bytes(data[:U32_SIZE_IN_BYTES])

            # remove BigInt length; data is only BigInt value and scale
            data = data[U32_SIZE_IN_BYTES:]

            # read BigInt value
            value.decode_top_level(data[:value_length])

            # remove BigInt value; data contains only scale
