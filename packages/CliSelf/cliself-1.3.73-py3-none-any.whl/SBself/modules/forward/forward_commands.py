...
async def _paged_history(app: Client, src: Union[str, int], page_size: int = _SAFE_PAGE_SIZE_DEFAULT):
    """
    تاریخچه را صفحه‌به‌صفحه می‌دهد. در Pyrogram v2، get_chat_history
    async generator برمی‌گرداند، پس باید با async for بخوانیم.
    خروجی هر صفحه را قدیم→جدید برمی‌گردانیم.
    """
    offset_id = 0
    while True:
        batch: List[Message] = []
        # ⚠️ اینجا دیگه await نداریم؛ با async for می‌خوانیم
        async for msg in app.get_chat_history(chat_id=src, offset_id=offset_id, limit=page_size):
            batch.append(msg)

        if not batch:
            break

        # جدید→قدیم به قدیم→جدید
        batch.reverse()
        yield batch

        # برای صفحه‌ی بعدی، offset_id را به قدیمی‌ترین پیام همین صفحه می‌بریم
