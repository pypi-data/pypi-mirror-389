...
async def _paged_history(app: Client, src: Union[str, int], page_size: int = _SAFE_PAGE_SIZE_DEFAULT):
    """
    تاریخچه‌ی پیام‌ها را صفحه‌به‌صفحه برمی‌گرداند (Pyrogram: جدید→قدیم).
    اما هر صفحه را reverse می‌کنیم تا ترتیب نهایی قدیم→جدید باشد.
    هدف: مصرف کم RAM و CPU با پردازش تدریجی.
    """
    last_id = 0
    while True:
        ...
        if not batch:
            break
        batch.reverse()      # جدید→قدیم به قدیم→جدید
        yield batch
        last_id = batch[0].id  # قدیمی‌ترین پیام این صفحه
