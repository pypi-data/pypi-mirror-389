...
class DataRegistry:
        ...
        try:
            ...
            try:
                ...
                if hasattr(importlib_resources, 'files'):
                    ...
                    try:
                        ref_files = list(importlib_resources.files('bunduk_mcp').joinpath('references').glob('*.csv'))
                        package_files.extend(ref_files)
                    except Exception:
                        pass  # References directory might not exist

                    # Look in data/active directory
                    try:
                        active_files = list(importlib_resources.files('bunduk_mcp').joinpath('data/active').glob('*.csv'))
                        package_files.extend(active_files)
                    except Exception:
                        pass  # data/active directory might not exist
            ...
            # Map package files to their paths - use full relative path as key
            for file_path in package_files:
                if isinstance(file_path, str):
                    file_path = Path(file_path)

                # Create a key that matches what the loading functions expect
                if 'references' in str(file_path):
                    key_name = file_path.name  # For references, just use filename
                elif 'data/active' in str(file_path):
                    key_name = file_path.name  # For active data, just use filename
                else:
            ...
            logger.debug(f"Could not load package data: {e}")
    ...
    def _resolve_file_path(self, file_path: Path) -> Optional[Path]:
        ...
        # Try to find in package data by exact name match
        package_file = self._package_data_paths.get(file_path.name)
        if package_file and package_file.exists():
            return package_file

        # Try to find by full relative path
        relative_path = file_path.relative_to(Path.cwd()) if file_path.is_absolute() else file_path
        for pkg_path in self._package_data_paths.values():
            # Check if the package file ends with the relative path we need
            try:
                if str(pkg_path).endswith(str(relative_path)) or pkg_path.name == file_path.name:
                    if pkg_path.exists():
                        return pkg_path
            except ValueError:
                # relative_to failed, just continue
                pass
