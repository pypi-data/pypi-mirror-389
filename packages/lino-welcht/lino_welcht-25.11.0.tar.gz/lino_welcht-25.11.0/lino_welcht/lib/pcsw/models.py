...
dd.inject_field('pcsw.Exclusion', 'unemployment_right',
                dd.ForeignKey('pcsw.UnemploymentRight', blank=True, null=True))

dd.inject_field('pcsw.Exclusion', 'followed_by_forem',
                FollowedFORem.field(blank=True))
dd.inject_field('pcsw.Exclusion', 'advisor',
                dd.CharField(_("Advisor"), max_length=50, blank=True))


@dd.chooser()
def unemployment_right_choices(cls):
    return UnemploymentRight.objects.filter(active=True)


dd.inject_action('pcsw.Exclusion', group_choices=unemployment_right_choices)
...
dd.inject_action('pcsw.Client', group_choices=group_choices)
...
class ClientDetail(ClientDetail):
    ...
    art61.ContractsByClient art61.ActivationsByClient
    """,
