...
class AgnoService:
    ...
    async def _build_toolset_tools(self, toolset_defs: List[Dict[str, Any]]) -> List[Any]:
        """
        Build OS-level toolset tools from toolset definitions.

        Args:
            toolset_defs: List of resolved toolset definitions with configurations

        Returns:
            List of instantiated toolset tool instances
        """
        ...
        # Tool registry mapping toolset types to agno tool classes
        TOOLSET_REGISTRY = {
        ...
        if not toolset_defs:
            logger.info("No toolset definitions provided, agent will run without OS-level tools")
            return tools

        logger.info(
            f"Building toolset tools from {len(toolset_defs)} toolset definitions",
            extra={
                "toolset_count": len(toolset_defs),
                "toolset_names": [t.get("name") for t in toolset_defs],
                "toolset_types": [t.get("type") for t in toolset_defs],
                "toolset_sources": [t.get("source") for t in toolset_defs],
            }
        )

        for toolset_def in toolset_defs:
            if not toolset_def.get("enabled", True):
                logger.debug(
                    f"Skipping disabled toolset",
                    extra={"toolset_name": toolset_def.get("name")}
                )
                continue
            ...
            if not tool_class:
                logger.warning(
                    f"Unknown toolset type: {toolset_type}",
                    extra={
            ...
            # Get configuration from toolset definition
            ...
            try:
                ...
                logger.info(
                    f"Toolset instantiated: {toolset_def.get('name')}",
                    extra={
                        "toolset_name": toolset_def.get("name"),
                        "toolset_type": toolset_type,
                        "source": toolset_def.get("source"),
                ...
                import traceback
                logger.error(
                    f"Failed to instantiate toolset '{toolset_def.get('name')}': {str(e)}",
                    extra={
                        "toolset_name": toolset_def.get("name"),
                        "toolset_type": toolset_type,
                        "error": str(e),
        ...
        logger.info(
            f"Built {len(tools)} toolset tools",
            extra={
    ...
    async def execute_agent_async(
        ...
        **kwargs: Any,
        ...
        """
        Execute an agent using Agno Teams with MCP and OS-level toolset support and session management.
        ...
        Args:
            ...
            toolsets: List of resolved toolset definitions (OS-level tools)
        ...
        try:
            ...
            # Build OS-level toolset tools
            toolset_tools = await self._build_toolset_tools(toolsets or [])
            ...
            # Combine MCP tools and OS-level toolset tools for the main team
            all_tools = mcp_tools + toolset_tools
            logger.info(
                f"Total tools available: {len(all_tools)} (MCP: {len(mcp_tools)}, Toolsets: {len(toolset_tools)})",
                extra={
                    "mcp_tool_count": len(mcp_tools),
                    "toolset_tool_count": len(toolset_tools),
                    "total_tools": len(all_tools)
                }
            )

            # Create the team with database for session management
            # The team itself gets all tools (both MCP and OS-level toolsets)
            logger.info(
                ...
                extra={
                    ...
                    "toolset_tools": len(toolset_tools),
            ...
            if stream:
                ...
                response_stream: Iterator[TeamRunOutputEvent] = await team.arun(
                    input_text,
                    stream=True,
