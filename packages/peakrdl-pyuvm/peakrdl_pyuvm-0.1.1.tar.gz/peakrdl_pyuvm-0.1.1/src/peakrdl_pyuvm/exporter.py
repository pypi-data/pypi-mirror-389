...
class PyUVMExporter:
    ...
    def __init__(
        ...
        if user_template_dir:
            loader = ChoiceLoader([
                ...
                PrefixLoader({
                    'user': FileSystemLoader(user_template_dir),
                    'base': FileSystemLoader(os.path.join(os.path.dirname(__file__), "templates"))
                }, delimiter=":")
            ])
        else:
            loader = ChoiceLoader([
        ...
        self.jj_env = Environment(
            loader=loader,
            undefined=StrictUndefined
        )
        self.user_template_context = user_template_context
    ...
    def export(self, node: Node, path: str, **kwargs) -> None:
        """
        Perform the export!
        ...
        template = self.jj_env.get_template("init.py.j2")
    ...
    def _get_endianness(self, node: Node) -> str:
        ...
        if amap.get_property("bigendian"):
            return "uvm_endianness_e.UVM_BIG_ENDIAN"
        elif amap.get_property("littleendian"):
            return "uvm_endianness_e.UVM_LITTLE_ENDIAN"
        else:
            return "uvm_endianness_e.UVM_NO_ENDIAN"
