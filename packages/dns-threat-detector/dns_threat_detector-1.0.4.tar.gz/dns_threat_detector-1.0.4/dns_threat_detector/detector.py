...
class DNS_ThreatDetector:
    ...
    def _extract_etld_plus_one(self, domain: str) -> str:
        ...
        - paypal.verify.info → verify.info (NOT paypal!)

        Uses a simplified approach for common TLDs and multi-level TLDs.
        For production, consider using 'tldextract' library for full PSL support.

        Args:
            domain: Full domain name (e.g., "login.paypal.com")

        Returns:
            The eTLD+1 (e.g., "paypal.com")
        """
        # Remove any trailing/leading whitespace and convert to lowercase
        ...
        # Common multi-level TLDs (country code TLDs with SLDs)
        multi_level_tlds = {
            "co.uk",
            "co.jp",
            "co.nz",
        ...
        # For standard TLDs, return domain.tld (last 2 parts)
        return ".".join(parts[-2:])
    ...
    def _check_typosquatting(self, domain: str) -> Optional[Dict]:
        ...
        # Only flag as typosquatting if edit distance is small (1-3 characters)
        if 1 <= min_distance <= 3:
            return {
                "is_typosquatting": True,
                "is_brand": False,
    ...
    def predict(self, domain: str) -> Dict:
        ...
        4. Check suspicious patterns (brand+service tokens) -> SUSPICIOUS
        5. Use ML ensemble -> MALICIOUS/BENIGN
        ...
        # Check for suspicious patterns before ML ensemble
        ...
        # Use ML ensemble for final classification
