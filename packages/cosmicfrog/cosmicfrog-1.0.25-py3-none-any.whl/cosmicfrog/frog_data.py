...
class FrogModel:
    ...
    def copy_tables_or_views_to_model(self, names: List[str], destination_model_name: str = None, overwrite: bool = False) -> List[Dict[str, Any]]:
        self.log.info(f"Copying {len(names)} objects to model '{destination_model_name}'")
        ...
        if not destination_model_name or not isinstance(destination_model_name, str):
            ...
            return [{
                "success": False, 
                "message": "Destination model name is required"
            }]
        ...
        self.log.info(f"Copying {len(sanitized_names)} sanitized objects to model '{destination_model_name}'")
        ...
        try:
            ...
            for name in sanitized_names:
                if name in inspector.get_table_names():
                    ...
                    if not view_def:
                        results.append( { "success": False, "message": f"View '{name}' definition not found."} )
                        continue
                    ...
                    results.append( { "success": True, "message": f"View '{name}' created."} )
                else:
    ...
    def upsert_ext(
        ...
        anura_abbreviated_names = AnuraSchema.get_anura_abbreviated_table_names(self.anura_version, True)
    ...
    def share(
        ...
        try:
            ...
            self.log.info(f"Model {self.model_name} shared successfully with {target_user}")
