...
class LabelSDK:
    ...
    def batch_evaluate_async(
        ...
        if is_pre_calculated:
            ...
            # Aplicar calibración a similitudes pre-calculadas si es Enterprise
            if self.tier == "enterprise" and self.calibration_profile:
                similarities_batch = [
                    [self.calibration_profile.calibrate(sim, fields[i]) 
                    for i, sim in enumerate(sims)]
                    for sims in similarities_batch
        ]
            logger.info(f"✅ Similitudes extraídas y calibradas en {time.time()-t0:.2f}s")
