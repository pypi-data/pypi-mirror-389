...
def compute_cam_with_sliding_window(
    ...
    if verbose:
        print(f"DEBUG CAM_CORE: data.shape = {data.shape}")
        print(f"DEBUG CAM_CORE: cam_data.shape = {cam_data.shape}")
        print(f"DEBUG CAM_CORE: slicer_revert_padding = {slicer_revert_padding}")
    ...
    if cam_type == "2d":
        ...
        image_size = data.shape[1:]  # POTENTIAL BUG: Should this be cam_data.shape[1:]?
        if verbose:
    ...
    for params in list_of_parameters:
        ...
        with cam_class(model, target_layers=target_layers) as cam:
            ...
            for sl in tqdm(slicers, desc="Processing patches", disable=not verbose):
                ...
                if verbose and len(slicers) < 100:  # Only print for first few patches
                    print(f"DEBUG PATCH: workon.shape = {workon.shape}")
                    print(f"DEBUG PATCH: cam_output.shape = {cam_output.shape}")
                    print(f"DEBUG PATCH: grayscale_cam.shape = {grayscale_cam.shape}")
                    print(f"DEBUG PATCH: sl = {sl}")
                    print(
                        f"DEBUG PATCH: predicted_cam_file[sl].shape would be = {predicted_cam_file[sl].shape}"
                    )
