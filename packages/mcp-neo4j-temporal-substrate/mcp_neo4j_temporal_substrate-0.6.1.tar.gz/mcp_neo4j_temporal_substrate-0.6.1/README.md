...
This server is designed for **temporal knowledge graphs** at scale:
...
For toy chatbot memory (dozens of isolated facts), consider the original `mcp-neo4j-memory` package. This fork (`mcp-neo4j-temporal-substrate`) optimizes for large-scale temporal knowledge graphs where unbounded queries are infeasible.
...
**Note:** Both tools implement bounded relationship traversal. Relationships are constrained to only the entities returned by the query, preventing exponential expansion in dense temporal knowledge graphs.
...
```bash
pip install mcp-neo4j-temporal-substrate
```
...
"mcpServers": {
  "neo4j": {
    "command": "uvx",
    "args": [
      "mcp-neo4j-temporal-substrate@0.6.0",
      "--db-url",
...
"mcpServers": {
  "neo4j": {
    "command": "uvx",
    "args": [ "mcp-neo4j-temporal-substrate@0.6.0" ],
    "env": {
...
```json
"args": [ "mcp-neo4j-temporal-substrate@0.6.0", "--namespace", "myapp", "--db-url", "..." ]
```
Tools become: `myapp-search_memories`, `myapp-create_entities`, etc.
