...
from explainable_ai.common import save_image, plot_stack
from explainable_ai.fusion_grad.fusion_grad import FusionGrad
from explainable_ai.guided_backpropagation import GuidedBackpropagation
from explainable_ai.metrics.evaluate_explanation import evaluate
from explainable_ai.smooth_grad.smooth_grad import SmoothGrad
...
def generate_explainability_heatmaps(
    ...
    for i in range(C):
        ...
        for method_name, heatmaps in explainability_methods.items():
            ...
            # Evaluate (aggregate per-channel like v1)
            scores = evaluate(
                xai_name=method_name,
                model=model,
                x_batch=np.expand_dims(img, axis=0),  # B=1
                y_batch=overlay_index,
                a_batch = np.expand_dims(heatmap, axis=0),
                explain_func_kwargs={"channel": i},
            )
            eval_results_for_channel[method_name] = scores
