...
from aidge_core import Log, Node, LnOp
from aidge_onnx.node_import import auto_register_import


@auto_register_import("log")
def import_log(
    ...
    if len(onnx_attrs) > 0:
        Log.warn(
            f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Log' with opset {opset}.\nThis node will be filled by a GenericOperator."
        )
        return None

    my_node: Node = Node(LnOp(), name=node_name)
    Log.info(
