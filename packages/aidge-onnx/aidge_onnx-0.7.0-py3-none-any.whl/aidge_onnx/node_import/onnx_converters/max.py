...
def import_max(
    ...
    if len(onnx_attrs) > 0:
        Log.warn(
            f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Max' with opset {opset}."
        )
        return None

    my_node = aidge_core.Max(nb_inputs = len(input_nodes), name=node_name)
