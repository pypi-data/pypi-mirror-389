...
from aidge_core import Log, Node, ReciprocalOp
from aidge_onnx.node_import import auto_register_import


@auto_register_import("reciprocal")
def import_reciprocal(
    ...
    if len(onnx_attrs) > 0:
        Log.warn(
            f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Relu' with opset {opset}.\nThis node will be filled by a GenericOperator."
        )
        return None

    my_node: Node = Node(ReciprocalOp(), name=node_name)
    Log.info(
