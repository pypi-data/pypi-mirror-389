...
def import_tile(
    ...
    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Tile' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None

    tile_node = aidge_core.Tile(name=node_name)
