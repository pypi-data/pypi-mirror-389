...
from aidge_core import Log, Node, ExpOp
from aidge_onnx.node_import import auto_register_import


@auto_register_import("exp")
def import_exp(
    ...
    if len(onnx_attrs) > 0:
        Log.warn(
            f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'Exp' with opset {opset}.\nThis node will be filled by a GenericOperator."
        )
        return None

    my_node: Node = Node(ExpOp(), name=node_name)
    Log.info(
