...
from aidge_core import Node, ScatterOp
...
def import_scatter_elements(onnx_node: NodeProto, input_nodes: List[Tuple[Node, int]], opset: int) -> Node:
    """
    ...
    if len(onnx_attrs) > 0:
        Log.warn(f"Warning: unsupported attribute(s): {onnx_attrs.keys()} for operator 'ScatterElements' with opset {opset}.\nThis node will be filled by a GenericOperator.")
        return None
