...
class TiffReader: 
    ...
    def __loadThumbnail__(self):
        ...
        if (isinstance(strip.offset['stripOffset'][0], int)):
            resultOffset = [ strip.offset['stripOffset'][0] ]
            typeOffset = strip.offset['stripOffset'][1]
            strip.offset['stripOffset'] = ( resultOffset, typeOffset )
            resultCount = [ strip.byteCounts['stripByteCounts'][0] ]
            typeCount = strip.byteCounts['stripByteCounts'][1]
            strip.byteCounts['stripByteCounts'] = ( resultCount, typeCount )
        elif not isinstance(strip.offset['stripOffset'][0], list):
            return True
