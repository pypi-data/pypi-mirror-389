...
__LOGO=''
__HELP_TEXT=''
_CONFIG_DATA={_B3:'_sbdl\\s?\\(',_B4:'\\)$',_B5:'_sbdl_block_begin',_B6:'_sbdl_block_end',_B7:'^\\.\\.\\.$',_B8:_B,_B9:_A,_BA:_C,_p:_A,_BB:{},_W:_C,_BC:_A,_BD:'.sbdl',_BE:_O,_BF:[],_k:[],_BG:5,_BH:_A,_BI:'sbdl-package.tar.gz',_AK:_C,_A1:_C,_A2:_C,_BJ:_A,_BK:_A,_AL:_C,_BL:'.sbdlc',_BM:_A,_l:_A,_A3:_C,_AM:_B,_AN:_C,_BN:'plantuml',_BO:65536,_BP:'png',_BQ:_A,_AO:_C,_AP:_C,_A4:_A,_BR:_A,_AQ:_A,_AR:_A,_T:{},_BS:_A,_BT:_A,_BU:_A,_BV:38,_BW:22,_BX:_C,_BY:_A,_AS:20000,_BZ:_CB,_Ba:200,_Bb:'',_Bc:_C,_Bd:2,_Be:_C,_X:"!pragma layout newlayouter\nskinparam dpi 200\nskinparam backgroundColor transparent\n'skinparam monochrome reverse\n\n",_Bf:'\n<style>\n  node {\n      HorizontalAlignment center\n      MaximumWidth 150\n  }\n  wbsDiagram {\n    Linecolor black\n    arrow {\n      LineColor black\n    }\n    .toplevel {\n    }\n    .source {\n      LineStyle 2\n      RoundCorner 10\n    }\n    .aspect {\n      LineStyle 8.0;3.0\n      LineColor lightgray\n      BackgroundColor lightgray\n      LineThickness 1.0\n      RoundCorner 0\n      Shadowing 0.0\n    }\n  }\n</style>',_Bg:_B,_AT:'hide circle',_Bh:'frame',_Bi:'class',_Bj:_C,_AU:_CC,_Bk:_C,_Bl:'participant','process_diagram_style':_CC,_Bm:_C,_q:_C,_c:15,_Bn:40,_Bo:'',_Bp:_C,_Bq:'',_AV:_C,_Br:0,_Bs:'[...]',_Bt:_C,_Bu:'less -R'if os.name=='posix'else'',_Bv:'SBDL_NO_PRETTY',_Bw:_C,_Bx:_A,_By:_C,_Bz:'1879da28-0fc2-4f1d-8ff1-407f4c070c44',_B_:'{base}/{endp}'.format(base=__URL,endp='rest.api'),_AW:'; ',_C0:'',_C1:_A,_C2:_A,_C3:_C,_C4:'https://fmea.dev',_C5:_A,_C6:_C,_C7:_U,_C8:_B,_C9:8,_CA:'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'}
...
class SBDL_Parser:
	...
	def sanitize(self,unclean_string):
		...
		return result.encode(_CI).decode(_Ad).replace('\\\\',_Y)
	@classmethod
	def sanitize_escaped(self,unclean_string,reverse=_C):
		...
		return result
	@classmethod
	def remove_escape_chars(self,escaped_string):return escaped_string.encode(_r).decode(_CI)
	@classmethod
...
class SBDL_Semantics:
	...
	def get_language_information_struct(self):
		...
		lang_info_struct[_v]=self.property_descriptions;lang_info_struct[_Ai]=dict.fromkeys([x for x in vars(SBDL_Parser.Macros).values()if isinstance(x,str)and x!=_j and x!='__main__'],'No Description')
...
def plantuml_identifier(in_string):
	result=in_string.strip(_I).replace('/',_U).replace(_N,_U).replace(_V,_U).replace(_m,_U)
	if result=='':result=_U
	return result
def write_requirement_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_requirement_diagram_wrap(content):
		...
		plant_uml_prepost='@startwbs\n{style}\n\n{header}\n{content}\n{legend}@endwbs\n';return plant_uml_prepost.format(style=get_config_value(_X)+get_config_value(_Bf),header=header,content=content,legend=legend_text)
...
def write_decomposition_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_decomposition_diagram():
		header=plantuml_footer_generator();title=_y.format(arguments.title)if arguments.title!=''else'';plant_uml_prepost=_z;content='';notes='';note_count=0;interface_count=0;relations='';descriptions=get_config_value(_Bj,number=_A);interface_mem={};handled_elems=[];indentation_prefix=_M
		...
		def interface_rel_pos(posindex):positions={0:'up',1:'down',2:_A9,3:_AA};return positions[posindex%4]if get_config_value(_AO,number=_A)else''
		...
		return plant_uml_prepost.format(style=(_CO if get_config_value(_AP,number=_A)else'')+get_config_value(_X)+get_config_value(_AT),header=header+title,content=content+_D+relations+_D+(_D+notes if descriptions or arguments.source else''))
...
def write_element_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_decomposition_diagram():
		...
		for elem in elements_ordered:handle_elem(elem)
		return plant_uml_prepost.format(style=(_CO if get_config_value(_AP,number=_A)else'')+get_config_value(_X)+get_config_value(_AT),header=header+title,content=content+_D+relations+_D+(_D+''))
	plantuml_writer(file_opener,output_file,plantuml_decomposition_diagram(),print_l);return errors
def write_function_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_function_diagram():
		...
		return plant_uml_prepost.format(style=get_config_value(_X)+get_config_value(_AU),header=header+title,content=content)
...
def write_process_graph_output(sbdl_ast,output_file,file_opener,_,print_l):
	...
	def plantuml_process_diagram():
		process_dia_cont='';process_dia_cont+='@startuml\n';process_dia_cont+=get_config_value(_X)+get_config_value(_AU)+_f
...
def write_state_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_state_diagram():
		...
		return plant_uml_prepost.format(style=get_config_value(_X)+get_config_value(_Bo),header=header+title,content=statedefs+content+_D+relations)
...
def write_usecase_graph_output(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	def plantuml_usecase_diagram():
		...
		for aspect in aspects:
			...
			for usecase_tuple in aspects[aspect]:content+='  usecase "{}" as {}\n'.format('**{}**\\n{}{}'.format(usecase_tuple[3],_H.join(textwrap.wrap(usecase_tuple[4],get_config_value(_c,number=_A))),_H+usecase_tuple[6]if arguments.source else'')if get_config_value(_AV,number=_A)else usecase_tuple[3],puml_id(usecase_tuple[3]));relations+='\n{} --> {}: {}{}\n'.format(puml_id(usecase_tuple[0]),puml_id(usecase_tuple[3]),'**{}**\\n{}'.format(usecase_tuple[1],_H.join(textwrap.wrap(usecase_tuple[2],get_config_value(_c,number=_A))))if get_config_value(_AV,number=_A)else usecase_tuple[1],_H+usecase_tuple[5]if arguments.source else'')
			content+='}\n'
		content+=relations;return plant_uml_prepost.format(style=get_config_value(_X)+get_config_value(_Bq),header=header+title,content=content)
...
def write_network_graph_output(sbdl_ast,output_file,_,arguments,print_l):
	...
	elements,errors=sbdl_ast.elements(print_l,prune_missing_references=_A);sbdl_graph=nx.MultiDiGraph();skip_types={x for x in get_config_value(_Bb,number=_C).split(_S)};node_name_sep=_m;node_text_wrap_width=25
	def node_name(elem_id):
		...
		if get_config_value(_BX,number=_A):node_text+=_f+_D.join(textwrap.wrap(elements[elem_id].description(),width=node_text_wrap_width))
...
class OpenFMEA:
	...
	def struct_from_table(self,title,sbdl_table,open_fmea_type=_Q):
		result_struct={_Ak:_AB,'version':'1.0',_K:open_fmea_type,_i:{'title':title,'application':name(),'application_version':version(),'cmd_line':str(sys.argv),'date':get_date_string()},_x:sbdl_table,'layout':{}};result_struct[_i]['user']=get_user_string()
	...
	def read_struct_from_file(self,open_sbdl_file):
		...
		if not(_Ak in open_sbdl_content and open_sbdl_content[_Ak]==_AB):raise Exception('Not an OpenFMEA file')
	...
	def elements_from_table(self,sbdl_table,_):
		multi_field_separator=get_config_value(_AW);elems_to_consider={_d:SBDL_Parser.Types.aspect,_Z:SBDL_Parser.Types.requirement,_L:SBDL_Parser.Types.mode,_b:SBDL_Parser.Types.effect,_R:SBDL_Parser.Types.cause,_o:SBDL_Parser.Types.current_control,_g:SBDL_Parser.Types.current_detection,_A0:SBDL_Parser.Types.action_control,_h:SBDL_Parser.Types.action_detection};elem_rating={_b:_t,_R:_u,_L:_s};elem_rating_post={_R:_A7,_L:_A6};elem_link={_R:[_L,_o,_A0,_g,_h],_L:[_d,_b,_g,_Z,_h,_R],_Z:[_d,_L],_b:[_L],_o:[_R],_g:[_L],_A0:[_R],_h:[_L]};elem_present_every_row={_g,_h};replace_str=SBDL_Parser.Tokens.replacement_string_default;elements={};id_lookup={}
		if not get_config_value(_C6,number=_A):elem_link[_L].remove(_R);elem_link[_b].remove(_L)
...
def write_template_fill(sbdl_ast,output_file,file_opener,arguments,print_l):
	...
	if get_config_value(_C5,number=_A):openfmea,errors_fmea=OpenFMEA.generate_sbdl_table_from_ast(sbdl_ast,id_in_body=arguments.identifier,source_in_body=arguments.source,print_l=print_l);errors+=errors_fmea;template_context[_AD]=openfmea
...
def affinity_graph():
	...
	def puml_id(input_str):return input_str.replace(_I,_U).replace(_V,_U)
...
def check_ast_cache(do_caching,source_files,print_l):
	...
	if do_caching:
		if len(source_files)==1:
			input_file=source_files[0];cache_file=f"{input_file}{get_config_value(_BL)}"
...
def main(arguments):
	...
	aggregate_methods={_Ao:OpenFMEA.aggregate_all_parser_elements_from_files,A:OpenFMEA.aggregate_all_parser_elements_from_quickshare,C:OpenFMEA.aggregate_all_parser_elements_from_files,B:OpenFMEA.aggregate_all_parser_elements_from_quickshare,_AG:aggregate_all_parser_elements_from_json,_Ax:aggregate_all_parser_elements_from_yaml,_AH:aggregate_all_parser_elements_from_csv};aggregation_method=aggregate_all_parser_elements_from_files if not arguments.mode in aggregate_methods else aggregate_methods[arguments.mode];parser_elements,read_errors=aggregation_method([fix_wsl_path(x)for x in arguments.source_files],arguments.recurse,arguments.hidden,print_l);check_step('File reading',read_errors);sbdl_ast=load_ast_cache(parser_elements,get_config_value(_AL,number=_A),arguments.source_files,print_l);check_step('Parsing',sbdl_ast.check_parsing(print_l))
	...
	if get_config_value(_AL,number=_A)and operations[arguments.mode]is write_aggregated_output:_,cache_path=check_ast_cache(_A,[arguments.output],print_l);store_ast_cache(sbdl_ast.ast(),cache_path,print_l)
...
def show_manual(_):
	...
	sbdl_info_struct=SBDL_Semantics.get_language_information_struct();sbdl_properties=sbdl_info_struct[_v];sbdl_types=sbdl_info_struct[_a];sbdl_directives=sbdl_info_struct[_Ai]
