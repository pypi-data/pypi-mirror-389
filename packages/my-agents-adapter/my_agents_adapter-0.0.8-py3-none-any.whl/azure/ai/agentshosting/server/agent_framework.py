...
class AgentFrameworkCBAgent(FoundryCBAgent):
    ...
    async def agent_run(
        ...
        logger.info(f"Starting agent_run with stream={context.request.stream}")
        logger.debug(f"Request body input type: {type(context.request.input)}")

        input_converter = AgentFrameworkInputConverter()
        message = input_converter.transform_input(context.request.input)
