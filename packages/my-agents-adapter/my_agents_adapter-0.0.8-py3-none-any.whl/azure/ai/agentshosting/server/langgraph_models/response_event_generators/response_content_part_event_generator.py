...
class ResponseContentPartEventGenerator(ResponseEventGenerator):
    ...
    def on_start(
        ...
        start_event = openai_models.ResponseContentPartAddedEvent(
    ...
    def on_end(self, event, run_details, stream_state: StreamEventState) -> List[openai_models.ResponseStreamEvent]:
        aggregated_content = self.item_content_helper.create_item_content()
        done_event = openai_models.ResponseContentPartDoneEvent(
            item_id=self.item_id,
            output_index=self.output_index,
            content_index=self.content_index,
    ...
    def create_child_processor(self, message) -> ResponseEventGenerator:
        if (
            self.item_content_helper.content_type == openai_models.ItemContentType.INPUT_TEXT
            or self.item_content_helper.content_type == openai_models.ItemContentType.OUTPUT_TEXT
        ):
