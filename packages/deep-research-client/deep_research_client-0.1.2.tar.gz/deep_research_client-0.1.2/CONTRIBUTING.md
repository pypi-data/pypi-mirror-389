# Contributing to deep-research-client

:+1: First of all: Thank you for taking the time to contribute!

The following is a set of guidelines for contributing to
deep-research-client. These guidelines are not strict rules.
...
The deep-research-client team strives to create a
...
#### Unit Tests (Fast)
Unit tests run quickly and don't make external API calls. They use mocked providers and isolated environments:
...
uv run pytest tests/test_client.py -k "not integration"

# Run with verbose output
uv run pytest tests/test_client.py -v -k "not integration"
```

#### Full Test Suite
To run all tests including both unit and integration tests:

```bash
# Run everything (this will be slower due to integration tests)
uv run pytest

# Use just command if available
just test
```
...
Integration tests are marked with `@pytest.mark.integration` and may make real API calls to external services. These tests:

- Are slower to run (may timeout if API calls fail)
- Require valid API keys in environment variables
- Test real interactions with external research providers
...
uv run pytest tests/test_client.py -m integration -v
```

#### Setting Up Integration Tests

Integration tests will only run meaningful tests if you have API keys configured:

```bash
# Optional: Set API keys for testing real providers
...
**Note**: Integration tests may make real API calls and consume API credits. Run them sparingly and only when testing provider integrations.

#### Test Development Guidelines

When writing tests:

- **Unit tests**: Should be fast (<2 seconds total), isolated, and not make external calls
- **Integration tests**: Mark with `@pytest.mark.integration`, may be slow, can make API calls
- **Async tests**: Use `async def` for test functions - pytest-asyncio is configured automatically
- **Mock external dependencies**: Use `patch.dict(os.environ, {}, clear=True)` to isolate unit tests
...
- Pull Requests
    ...
    - PRs should be reviewed and merged in a timely fashion by the deep-research-client technical leads
...
[issues]: https://github.com/ai4curation/deep-research-client/issues/
[pulls]: https://github.com/ai4curation/deep-research-client/pulls/
