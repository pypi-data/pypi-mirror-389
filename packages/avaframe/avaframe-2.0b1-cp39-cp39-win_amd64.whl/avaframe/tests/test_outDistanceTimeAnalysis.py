...
def test_plotRangeTime(tmp_path):
    ...
    mtiInfo = {
        "mti": mti,
        "rangeList": np.arange(50),
        "timeList": np.arange(50),
        "rangeGates": np.arange(45) - 25.0,
        "plotTitle": "testTitle",
        "referencePointName": "betaPoint",
        "type": "tt",
        "betaPointAngle": 9.987,
    }
...
def test_radarFieldOfViewPlot(tmp_path):
    ...
    # setup required inputs
    avaDir = pathlib.Path(tmp_path)
    radarFov = np.asarray([[0.0, 10.0], [5.0, 5.0]])
    radarRange = np.repeat([np.arange(50)], 45, axis=0)
    cfgRangeTime = configparser.ConfigParser()
    cfgRangeTime["GENERAL"] = {"avalancheDir": avaDir, "aperture": 5.0, "simHash": "simDI"}
    cfgRangeTime["PLOTS"] = {"gateContours": 5}

    rangeGates = np.arange(45)
    demData = np.zeros((45, 50))
    dem = {
        "header": {"nrows": 45, "ncols": 50, "cellsize": 5.0, "xllcenter": 0.0, "yllcenter": 0.0},
        "rasterData": demData,
    }
...
def test_animationPlot(tmp_path):
    ...
    mtiInfo = {
        "mti": mti,
        "rangeList": np.arange(12),
        "timeList": np.arange(12),
        "rangeGates": np.arange(543) - 25.0,
        "plotTitle": "testTitle",
        "referencePointName": "betaPoint",
        "type": "tt",
        "betaPointAngle": 9.987,
        "cLower": 2,
        "slRaster": np.ones((543, 121)),
        "rangeRaster": np.ones((543, 121)),
        "sType": "projected",
    }
    mtiInfo["rasterTransfo"] = {
        "x": np.arange(543),
        "y": np.ones(543),
        "cellSizeSL": 2.0,
        "DBXl": np.asarray([0.0, 10]),
        "DBXr": np.asarray([2.0, 20]),
        "DBYl": np.asarray([0, 9]),
        "DBYr": np.asarray([0, 2.0]),
        "indStartOfRunout": 4,
        "gridx": np.ones((543, 121)),
        "gridy": np.ones((543, 121)),
        "l": np.arange(121),
        "s": np.arange(543),
    }
    ...
    cfgRangeTime["GENERAL"] = {
        ...
        "simHash": "simDI",
        "minVelTimeStep": 2.0,
        "thresholdResult": 1.0e-2,
        "minVelTimeStep": 2,
        "maxOrMean": "max",
    }
    cfgRangeTime["PLOTS"] = {"width": 0.25, "height": 0.25, "lw": 0.25, "textsize": 7}
    cfgRangeTime["ANIMATE"] = {
        "resMin": 0.0,
        "resMax": 1.9,
        "xyEastNorth": True,
        "panelTitles": True,
