...
class NamespaceService:
    ...
    def _build_namespace_response(row) -> NamespaceResponse:
        ...
        这是一个内部辅助方法，用于将数据库行转换为 API 响应格式。
    ...
    async def list_namespaces(
        ...
        示例:
            # 获取所有命名空间
            all_namespaces = await NamespaceService.list_namespaces()

            # 只获取激活的命名空间
            active_namespaces = await NamespaceService.list_namespaces(is_active=True)
        """
    ...
    async def create_namespace(namespace: NamespaceCreate) -> NamespaceResponse:
        ...
        示例:
            # 使用 direct 模式创建
            namespace = await NamespaceService.create_namespace(NamespaceCreate(
                name="production",
                description="生产环境",
                config_mode=ConfigMode.DIRECT,
    ...
    async def get_namespace(namespace_name: str) -> NamespaceResponse:
        ...
        示例:
            namespace = await NamespaceService.get_namespace("default")
    ...
    async def update_namespace(namespace_name: str, namespace: NamespaceUpdate) -> NamespaceResponse:
        ...
        示例:
            # 更新描述
            namespace = await NamespaceService.update_namespace("default", NamespaceUpdate(
        ...
        async with session_factory() as session:
            ...
            # 优先使用独立的配置模式，否则使用统一的config_mode，最后使用当前模式
            redis_mode = None
            pg_mode = None
    ...
    async def delete_namespace(namespace_name: str) -> Dict[str, str]:
        ...
        示例:
            result = await NamespaceService.delete_namespace("staging")
            print(result['message'])  # "命名空间 'staging' 已删除"
        """
