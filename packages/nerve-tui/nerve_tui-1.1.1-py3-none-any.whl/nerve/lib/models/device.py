...
from typing import (NotRequired, TypedDict, override, Iterable, Iterator)
...
ENC_IDENTIFIER: str = 'ainst'
DEFAULT_ITER_TIME: int = 10000
...
class DiskLayoutConfiguration:
    ...
    def json(self) -> _DiskLayoutConfigurationSerialization:
        if self.config_type == DiskLayoutType.Pre_mount:
            return {
                'config_type': str(self.config_type.value),
    ...
    @classmethod
    def parse_arg(cls, disk_config: _DiskLayoutConfigurationSerialization, enc_password: Password | None = None) -> DiskLayoutConfiguration | None:
        from nerve.lib.disk.device_handler import device_handler

        device_modifications: list[DeviceModification] = []
        config_type: str = disk_config.get('config_type', None)

        if not config_type:
            raise ValueError('Missing disk layout configuration: config_type')

        config: DiskLayoutConfiguration = DiskLayoutConfiguration(config_type=DiskLayoutType(config_type), device_modifications=device_modifications)
        ...
        for entry in disk_config.get('device_modifications', []):
            device_path: Path | None = Path(entry['device']) if entry.get('device', None) else None

            if not device_path:
                continue

            device: BDevice | None = device_handler.get_device(device_path)

            if not device:
                continue

            device_modification: DeviceModification = DeviceModification(wipe=entry.get('wipe', False), device=device)
            device_partitions: list[PartitionModification] = []

            for partition in entry.get('partitions', []):
                flags: list[PartitionFlag] = [flag for f in partition.get('flags', []) if (flag := PartitionFlag.from_string(f))]
                device_partition: PartitionModification = PartitionModification(status=ModificationStatus(partition['status']), fs_type=FilesystemType(partition['fs_type']) if partition.get('fs_type') else None, start=Size.parse_args(partition['start']), length=Size.parse_args(partition['size']), mount_options=partition['mount_options'], mountpoint=Path(partition['mountpoint']) if partition['mountpoint'] else None, dev_path=Path(partition['dev_path']) if partition['dev_path'] else None, type=PartitionType(partition['type']), flags=flags, btrfs_subvols=SubvolumeModification.parse_args(partition.get('btrfs', [])))
        ...
        for dev_mod in device_modifications:
            ...
            non_delete_partitions: list[PartitionModification] = [part_mod for part_mod in dev_mod.partitions if not part_mod.is_delete()]

            if not non_delete_partitions:
                continue

            first: PartitionModification = non_delete_partitions[0]
            ...
            for (i, current_partition) in enumerate(non_delete_partitions[1:], start=1):
                previous_partition: PartitionModification = non_delete_partitions[i - 1]
            ...
            create_partitions: list[PartitionModification] = [part_mod for part_mod in non_delete_partitions if part_mod.status == ModificationStatus.Create]
            ...
            last: PartitionModification = create_partitions[-1]
            total_size: Size = dev_mod.device.device_info.total_size

            if dev_mod.using_gpt(device_handler.partition_table) and (last.end > total_size.gpt_end()):
    ...
    def has_default_btrfs_vols(self) -> bool:
        return (self.config_type == DiskLayoutType.Default) and any((part.fs_type == FilesystemType.Btrfs) and part.is_create_or_modify() and part.btrfs_subvols for mod in self.device_modifications for part in mod.partitions)
...
class SectorSize:
    ...
    def parse_args(cls, arg: _SectorSizeSerialization) -> SectorSize:
        return SectorSize(arg['value'], Unit[arg['unit']])
...
class Size:
    ...
    def parse_args(cls, size_arg: _SizeSerialization) -> Size:
        sector_size: _SectorSizeSerialization = size_arg['sector_size']
        return Size(size_arg['value'], Unit[size_arg['unit']], SectorSize.parse_args(sector_size))

    def convert(self, target_unit: Unit, sector_size: SectorSize | None = None) -> Size:
        if (target_unit == Unit.sectors) and not sector_size:
            raise ValueError('If target has unit sector, a sector size must be provided')

        match self.unit:
            case unit if unit == target_unit:
                return self

            case Unit.sectors:
                norm: int = self._normalize()
            ...
            case _:
                if (target_unit == Unit.sectors) and sector_size is not None:
                    norm:    int = self._normalize()
                    sectors: int = math.ceil(norm / sector_size.value)

                    return Size(sectors, Unit.sectors, sector_size)

                value: int = int(self._normalize() / target_unit.value)  # type: ignore
                return Size(value, target_unit, self.sector_size)

    def format_size(self, target_unit: Unit, sector_size: SectorSize | None = None, include_unit: bool = True) -> str:
        target_size: Size = self.convert(target_unit, sector_size)
        return (str(target_size.value) + ' ' + target_unit.name) if include_unit else str(target_size.value)

    def binary_unit_highest(self, include_unit: bool = True) -> str:
        binary_units: list[Unit] = Unit.get_binary_units()
        size: float = float(self._normalize())
        unit: Unit = Unit.KiB
        base_value: int = unit.value
        ...
        formatted_size: str = str(round(size, 1))

        if formatted_size.endswith('.0'):
            formatted_size = formatted_size[:-2]

        return formatted_size if not include_unit else (formatted_size + ' ' + unit.name)

    def si_unit_highest(self, include_unit: bool = True) -> str:
        si_units: list[Unit] = Unit.get_si_units()

        all_si_values: list[Size] = [self.convert(si) for si in si_units]
        filtered: Iterable[Size] = filter(lambda x: x.value >= 1, all_si_values)
        ...
        si_value: Size = max(filtered, key=lambda x: x.unit.value)
        return (str(si_value.value) + ' ' + str(si_value.unit.name)) if include_unit else str(si_value.value)

    def format_highest(self, include_unit: bool = True, units: Units = Units.BINARY) -> str:
        return self.binary_unit_highest(include_unit) if units == Units.BINARY else self.si_unit_highest(include_unit)

    def is_valid_start(self) -> bool:
        return self >= Size(1, Unit.MiB, self.sector_size)

    def align(self) -> Size:
        align_norm: int = Size(1, Unit.MiB, self.sector_size)._normalize()
        src_norm:   int = self._normalize()
    ...
    def _normalize(self) -> int:
        ...
        will normalize the value of the unit to Byte
        """
        return self.value * self.sector_size.normalize() if (self.unit == Unit.sectors) and self.sector_size is not None else int(self.value * self.unit.value)  # type: ignore

    def __sub__(self, other: Size) -> Size:
        src_norm:  int = self._normalize()
        dest_norm: int = other._normalize()
    ...
    def __eq__(self, other: object) -> bool:
        return self._normalize() == other._normalize()  # type: ignore[attr-defined]
...
class PartitionInfo:
    ...
    def sector_size(self) -> SectorSize:
        sector_size: int = self.partition.geometry.device.sectorSize
        return SectorSize(sector_size, Unit.B)

    def table_data(self) -> dict[str, str]:
        end: Size = self.start + self.length
        ...
        if self.btrfs_subvol_infos:
            part_info['Btrfs vol.'] = str(len(self.btrfs_subvol_infos)) + ' subvolumes'

        return part_info

    @classmethod
    def from_partition(cls, partition: Partition, lsblk_info: LsblkInfo, fs_type: FilesystemType | None, btrfs_subvol_infos: list[BtrfsSubvolumeInfo] | None = None) -> PartitionInfo:
        if not btrfs_subvol_infos:
            btrfs_subvol_infos = []

        partition_type: PartitionType = PartitionType.get_type_from_code(partition.type)
        flags: list[PartitionFlag] = [f for f in PartitionFlag if partition.getFlag(f.flag_id)]

        start:  Size = Size(partition.geometry.start, Unit.sectors, SectorSize(partition.disk.device.sectorSize, Unit.B))
        length: Size = Size(int(partition.getLength(unit='B')), Unit.B, SectorSize(partition.disk.device.sectorSize, Unit.B))

        return PartitionInfo(partition=partition, name=partition.get_name(), type=partition_type, fs_type=fs_type, path=Path(partition.path), start=start, length=length, flags=flags, partn=lsblk_info.partn, partuuid=lsblk_info.partuuid, uuid=lsblk_info.uuid, disk=partition.disk, mountpoints=lsblk_info.mountpoints, btrfs_subvol_infos=btrfs_subvol_infos)
...
class DeviceInfo:
    ...
    def table_data(self) -> dict[str, str | int | bool]:
        total_free_space: int = sum([region.get_length(unit=Unit.MiB) for region in self.free_space_regions])
    ...
    def from_disk(cls, disk: Disk) -> DeviceInfo:
        device: parted.device.Device = disk.device
        device_type: str
        ...
        if device.type == 18:
            ...
            debug('Device code unknown: ' + str(device.type))
            device_type = parted.devices[parted.DEVICE_UNKNOWN]

        sector_size: SectorSize = SectorSize(device.sectorSize, Unit.B)
        free_space: list[DeviceGeometry] = [DeviceGeometry(g, sector_size) for g in disk.getFreeSpaceRegions()]

        return DeviceInfo(model=device.model.strip(), path=Path(device.path), type=device_type, sector_size=sector_size, total_size=Size(int(device.getLength(unit='B')), Unit.B, sector_size), free_space_regions=free_space, read_only=device.readOnly, dirty=device.dirty)
...
class SubvolumeModification:
    ...
    def parse_args(cls, subvol_args: list[_SubvolumeModificationSerialization]) -> list[SubvolumeModification]:
        mods: list[SubvolumeModification] = []

        for entry in subvol_args:
            if not entry.get('name', None) or not entry.get('mountpoint', None):
                debug('Subvolume arg is missing name: ' + str(entry))
                continue

            mountpoint: Path | None = Path(entry['mountpoint']) if entry['mountpoint'] else None
    ...
    def is_root(self) -> bool:
        return self.mountpoint == Path('/') if self.mountpoint else False
...
class DeviceGeometry:
    ...
    def table_data(self) -> dict[str, str | int]:
        start:  Size = Size(self._geometry.start, Unit.sectors, self._sector_size)
        end:    Size = Size(self._geometry.end, Unit.sectors, self._sector_size)
        length: Size = Size(self._geometry.getLength(), Unit.sectors, self._sector_size)

        start_str:  str = str(self._geometry.start)       + ' / ' + str(start.format_size(Unit.B, include_unit=False))
        end_str:    str = str(self._geometry.end)         + ' / ' + str(end.format_size(Unit.B, include_unit=False))
        length_str: str = str(self._geometry.getLength()) + ' / ' + str(length.format_size(Unit.B, include_unit=False))
...
class PartitionType(Enum):
    ...
    def get_type_from_code(cls, code: int) -> PartitionType:
        ...
        debug('Partition code not supported: ' + str(code))
        return PartitionType.Unknown

    def get_partition_code(self) -> int | None:
        return {
            PartitionType.Primary: parted.PARTITION_NORMAL,
            PartitionType.Boot:    parted.PARTITION_BOOT
        }.get(self)
...
class PartitionFlag(PartitionFlagDataMixin, Enum):
    ...
    @classmethod
    def from_string(cls, string: str) -> PartitionFlag | None:
        string: str = string.lower()

        for partition_flag in cls:
            if string in (partition_flag.name.lower(), partition_flag.alias):
                return partition_flag

        debug('Partition flag not supported: ' + string)
        return None
...
class FilesystemType(Enum):
    ...
    # this is not an FS known to parted, so be careful
...
class PartitionModification:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.fs_type and (self.status == ModificationStatus.Modify):
    ...
    def from_existing_partition(cls, partition_info: PartitionInfo) -> PartitionModification:
        subvol_mods: list[SubvolumeModification] = [SubvolumeModification.from_existing_subvol_info(i) for i in partition_info.btrfs_subvol_infos] if partition_info.btrfs_subvol_infos else []
        mountpoint: Path | None = None if partition_info.btrfs_subvol_infos else (partition_info.mountpoints[0] if partition_info.mountpoints else None)

        return PartitionModification(status=ModificationStatus.Exist, type=partition_info.type, start=partition_info.start, length=partition_info.length, fs_type=partition_info.fs_type, dev_path=partition_info.path, partn=partition_info.partn, partuuid=partition_info.partuuid, uuid=partition_info.uuid, flags=partition_info.flags, mountpoint=mountpoint, btrfs_subvols=subvol_mods)
    ...
    def is_root(self) -> bool:
        return (self.mountpoint == Path('/')) if self.mountpoint is not None else any(subvol.is_root() for subvol in self.btrfs_subvols)

    def is_home(self) -> bool:
        return (self.mountpoint == Path('/home')) if self.mountpoint is not None else False
    ...
    def mapper_name(self) -> str | None:
        return 'root' if self.is_root() else ('home' if self.is_home() else ((ENC_IDENTIFIER + self.dev_path.name) if self.dev_path else None))
    ...
    def invert_flag(self, flag: PartitionFlag) -> None:
        self.flags = [f for f in self.flags if f != flag] if flag in self.flags else self.set_flag(flag)
    ...
    def table_data(self) -> dict[str, str]:
        ...
        if self.btrfs_subvols:
            part_mod['Btrfs vol.'] = str(len(self.btrfs_subvols)) + ' subvolumes'
...
class LvmVolumeGroup:
    ...
    def parse_arg(arg: _LvmVolumeGroupSerialization, disk_config: DiskLayoutConfiguration) -> LvmVolumeGroup:
        ...
        return LvmVolumeGroup(arg['name'], lvm_pvs, [LvmVolume.parse_arg(vol) for vol in arg['volumes']])
...
class LvmVolume:
    ...
    @property
    def mapper_name(self) -> str | None:
        return (ENC_IDENTIFIER + self.safe_dev_path.name) if self.dev_path else None

    @property
    def mapper_path(self) -> Path:
        if self.mapper_name:
            return Path('/dev/mapper/' + str(self.mapper_name))
    ...
    def parse_arg(arg: _LvmVolumeSerialization) -> LvmVolume:
        volume: LvmVolume = LvmVolume(status=LvmVolumeStatus(arg['status']), name=arg['name'], fs_type=FilesystemType(arg['fs_type']), length=Size.parse_args(arg['length']), mountpoint=Path(arg['mountpoint']) if arg['mountpoint'] else None, mount_options=arg.get('mount_options', []), btrfs_subvols=SubvolumeModification.parse_args(arg.get('btrfs', [])))
    ...
    def table_data(self) -> dict[str, str]:
        part_mod: dict[str, str] = {
            ...
            'Mountpoint': str(self.mountpoint) if self.mountpoint else '',
            'Mount options': ', '.join(self.mount_options),
            'Btrfs': str(len(self.btrfs_subvols)) + ' vol'
        }
...
class LvmConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        if any(pv in (seen := set()) or seen.add(pv) for g in self.vol_groups for pv in g.pvs):
    ...
    def parse_arg(arg: _LvmConfigurationSerialization, disk_config: DiskLayoutConfiguration) -> LvmConfiguration:
        ...
        return LvmConfiguration(config_type=LvmLayoutType(arg['config_type']), vol_groups=[LvmVolumeGroup.parse_arg(vol_group, disk_config) for vol_group in arg['vol_groups']])
    ...
    def get_root_volume(self) -> LvmVolume | None:
        for vg in self.vol_groups:
            filtered: LvmVolume = next(filter(lambda x: x.is_root(), vg.volumes), None)
...
class BtrfsOptions:
    ...
    def parse_arg(arg: _BtrfsOptionsSerialization) -> BtrfsOptions | None:
        snapshot_args: _SnapshotConfigSerialization | None = arg.get('snapshot_config')
        return BtrfsOptions(SnapshotConfig.parse_args(snapshot_args)) if snapshot_args else None
...
class DeviceModification:
    ...
    def using_gpt(self, partition_table: PartitionTable) -> bool:
        return partition_table.is_gpt() if self.wipe else (self.device.disk.type == PartitionTable.GPT.value)

    def add_partition(self, partition: PartitionModification) -> None:
        self.partitions.append(partition)

    def get_efi_partition(self) -> PartitionModification | None:
        filtered: Iterator[PartitionModification] = filter(lambda x: x.is_efi() and x.mountpoint, self.partitions)
        return next(filtered, None)

    def get_boot_partition(self) -> PartitionModification | None:
        filtered: Iterator[PartitionModification] = filter(lambda x: x.is_boot() and x.mountpoint, self.partitions)
        return next(filtered, None)

    def get_root_partition(self) -> PartitionModification | None:
        filtered: Iterator[PartitionModification] = filter(lambda x: x.is_root(), self.partitions)
        return next(filtered, None)
...
class EncryptionType(Enum):
    ...
    def text_to_type(cls, text: str) -> 'EncryptionType':
        mapping: dict[str, EncryptionType] = cls._encryption_type_mapper()
        return mapping[text]

    @classmethod
    def type_to_text(cls, type_: 'EncryptionType') -> str:
        mapping:      dict[str, EncryptionType] = cls._encryption_type_mapper()
        type_to_text: dict[EncryptionType, str] = {type_: text for (text, type_) in mapping.items()}
...
class DiskEncryption:
    ...
    def should_generate_encryption_file(self, dev: PartitionModification | LvmVolume) -> bool:
        return (dev.mountpoint != Path('/')) and ((isinstance(dev, PartitionModification) and dev in self.partitions) or (not isinstance(dev, PartitionModification) and dev in self.lvm_volumes))
    ...
    @classmethod
    def validate_enc(cls, modifications: list[DeviceModification], lvm_config: LvmConfiguration | None = None) -> bool:
        ...
        if (len(partitions) > 2) and lvm_config:  # assume one boot and at least 2 additional
            return False

        return True

    @classmethod
    def parse_arg(cls, disk_config: DiskLayoutConfiguration, disk_encryption: _DiskEncryptionSerialization, password: Password | None = None) -> 'DiskEncryption | None':
        ...
        enc_partitions: list[PartitionModification] = []
        ...
        enc: DiskEncryption = DiskEncryption(EncryptionType(disk_encryption['encryption_type']), password, enc_partitions, volumes)
...
class Fido2Device:
    ...
    def parse_arg(cls, arg: _Fido2DeviceSerialization) -> 'Fido2Device':
        return Fido2Device(Path(arg['path']), arg['manufacturer'], arg['product'])
...
class LsblkInfo(BaseModel):
    ...
    type: str | None  # maybe None for strange behavior with md devices
    mountpoint: Path | None
    mountpoints: list[Path]
    fsroots: list[Path]
    ...
    def fields(cls) -> list[str]:
        return [_field.alias or name for (name, _field) in cls.model_fields.items() if name != 'children']
