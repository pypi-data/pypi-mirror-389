...
from nerve.lib.utils.unicode import (unicode_ljust, unicode_rjust)
...
class FormattedOutput:
    @classmethod
    def _get_values(cls, o: 'DataclassInstance', class_formatter: str | Callable | None = None, filter_list: list[str] | None = None) -> dict[str, Any]:
        """
        the original values returned a dataclass as dict through the call to some specific methods
        this version allows through the parameter class_formatter to call a dynamically selected formatting method.
    ...
    @classmethod
    def as_table(cls, obj: list[Any], class_formatter: str | Callable | None = None, filter_list: list[str] | None = None, capitalize: bool = False) -> str:
        ...
        A general comment, the format selected for the output (a string where a newline separates every data record)
        ...
        raw_data: list[dict[str, Any]] = [cls._get_values(o, class_formatter, filter_list) for o in obj]
        ...
        for record in raw_data:
            ...
            for key in filter_list:
                ...
                obj_data.append(unicode_rjust(str(value), width)) if isinstance(value, int | float) or (isinstance(value, str) and value.isnumeric()) else obj_data.append(unicode_ljust(str(value), width))
...
class Journald:
    ...
    def log(message: str, level: int = logging.DEBUG) -> None:
        ...
        log_adapter: logging.Logger = logging.getLogger('nerve')
        log_fmt: logging.Formatter = logging.Formatter("[%(levelname)s]: %(message)s")
        log_ch: systemd.journal.JournalHandler = systemd.journal.JournalHandler()
...
class Logger:
    def __init__(self, path: Path = Path('/var/log/nerve')) -> None:
    ...
    def _check_permissions(self) -> None:
        ...
        try:
            ...
            warn('Not enough permission to place log file at ' + str(log_file) + ', creating it in ' + str(logger.path) + ' instead')
    ...
    def log(self, level: int, content: str) -> None:
        ...
        with self.path.open(mode='a') as f:
            ...
            f.write('[' + ts + '] - ' + level_name + ' - ' + content + '\n')

logger: Logger = Logger()

def _supports_color() -> bool:
    """
    Found the first reference here:
        https://stackoverflow.com/questions/7445658/how-to-detect-if-the-console-does-support-ansi-escape-codes-in-python
    And re-used this:
        https://github.com/django/django/blob/main/django/core/management/color.py#L12

    Return True if the running system's terminal supports color,
    and False otherwise.
    """
    supported_platform: bool = (sys.platform != 'win32') or 'ANSICON' in os.environ
...
def _stylize_output(text: str, fg: str, bg: str | None, reset: bool, font: list[Font] | None = None) -> str:
    """
    ...
    foreground: dict[str, str] = {key: ('3' + colors[key]) for key in colors}
    background: dict[str, str] = {key: ('4' + colors[key]) for key in colors}
    ...
    ansi: str = ';'.join(code_list)
    return '\033[' + ansi + 'm' + text + '\033[0m'

def info(*msgs: str, level: int = logging.INFO, fg: str = 'white', bg: str | None = None, reset: bool = False, font: list[Font] | None = None) -> None:
    log(*msgs, level=level, fg=fg, bg=bg, reset=reset, font=font)

def debug(*msgs: str, level: int = logging.DEBUG, fg: str = 'white', bg: str | None = None, reset: bool = False, font: list[Font] | None = None) -> None:
    log(*msgs, level=level, fg=fg, bg=bg, reset=reset, font=font)

def error(*msgs: str, level: int = logging.ERROR, fg: str = 'red', bg: str | None = None, reset: bool = False, font: list[Font] | None = None) -> None:
    log(*msgs, level=level, fg=fg, bg=bg, reset=reset, font=font)

def warn(*msgs: str, level: int = logging.WARNING, fg: str = 'yellow', bg: str | None = None, reset: bool = False, font: list[Font] | None = None) -> None:
    log(*msgs, level=level, fg=fg, bg=bg, reset=reset, font=font)
...
def log(*msgs: str, level: int = logging.INFO, fg: str = 'white', bg: str | None = None, reset: bool = False, font: list[Font] | None = None) -> None:
    if not font:
        font = []

    text: str = ' '.join([str(x) for x in msgs])
