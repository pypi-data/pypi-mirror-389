...
from nerve.lib.networking import (DownloadTimer, ping)
...
class MirrorStatusEntryV3(BaseModel):
    ...
    def server_url(self) -> str:
        return self.url + '$repo/os/$arch'

    @property
    def speed(self) -> float:
        if self._speed is None:
            if self._speedtest_retries is None:
            ...
            while (retry < self._speedtest_retries) and not self._speed:
                debug("Checking download speed of " + str(self._hostname) + "[" + str(self.score) + "] by fetching: " + self.url + "core/os/x86_64/core.db")
                req: urllib.request.Request = urllib.request.Request(url=self.url + "core/os/x86_64/core.db")

                try:
                    with urllib.request.urlopen(req, data=None, timeout=5) as handle, DownloadTimer(timeout=5) as timer:
                        size: int = len(handle.read())

                    assert timer.time is not None
                    self._speed = size / timer.time

                    debug("    speed: " + str(self._speed) + " (" + str(int(self._speed / 1024 / 1024 * 100) / 100) + "MiB/s)")

                # Do not retry error
                except urllib.error.URLError as error:
                    debug("    speed: <undetermined> (" + str(error) + "), skip")
                    self._speed = 0
                # Do retry error
                except (http.client.IncompleteRead, ConnectionResetError) as error:
                    debug("    speed: <undetermined> (" + str(error) + "), retry")
                # Catch all
    ...
    def latency(self) -> float | None:
        """
        Latency measures the miliseconds between one ICMP request and a response.
        It only does so once because we check if self._latency is None, and an ICMP timeout results in -1
        We do this because some hosts block ICMP, so we'll have to rely on .speed() instead, which is slower.
        """
        if self._latency is None:
            debug("Checking latency for " + self.url)
            assert self._hostname is not None

            self._latency = ping(self._hostname, timeout=2)
            debug("  latency: " + str(self._latency))
    ...
    def validate_score(cls, value: float) -> int | None:
        if value is not None:
            value = round(value)
            debug("    score: " + str(value))

        return value

    @model_validator(mode='after')
    def debug_output(self) -> 'MirrorStatusEntryV3':
        (self._hostname, *port) = urllib.parse.urlparse(self.url).netloc.split(sep=':', maxsplit=1)
        self._port              = int(port[0]) if port and len(port) >= 1 else None

        debug("Loaded mirror " + str(self._hostname) + (" with current score of " + str(self.score) if self.score else ''))
        return self
...
class MirrorRegion:
    ...
    def __eq__(self, other: object) -> bool:
        return NotImplemented if not isinstance(other, MirrorRegion) else self.name == other.name
...
class CustomRepository:
    ...
    def parse_args(cls, args: list[dict[str, str]]) -> list['CustomRepository']:
        configs: list['CustomRepository'] = []

        for arg in args:
            configs.append(CustomRepository(arg['name'], arg['url'], SignCheck(arg['sign_check']), SignOption(arg['sign_option'])))
...
class CustomServer:
    ...
    def parse_args(cls, args: list[dict[str, str]]) -> list['CustomServer']:
        configs: list['CustomServer'] = []

        for arg in args:
            configs.append(CustomServer(arg['url']))
...
class MirrorConfiguration:
    ...
    def regions_config(self) -> str:
        from nerve.lib.mirrors import mirror_list_handler
        config: str = ''

        for mirror_region in self.mirror_regions:
            sorted_static: list[MirrorStatusEntryV3] = mirror_list_handler.get_status_by_region(mirror_region.name)
            config += '\n\n## ' + mirror_region.name + '\n'

            for status in sorted_static:
                config += 'Server = ' + status.server_url + '\n'
    ...
    def repositories_config(self) -> str:
        ...
        for repo in self.custom_repositories:
            config += '\n\n[' + repo.name + ']\n'
            config += 'SigLevel = ' + str(repo.sign_check.value) + ' ' + str(repo.sign_option.value) + '\n'
            config += 'Server = ' + repo.url + '\n'

        return config

    @classmethod
    def parse_args(cls, args: dict[str, Any], backwards_compatible_repo: list[Repository] | None = None) -> 'MirrorConfiguration':
        if not backwards_compatible_repo:
            backwards_compatible_repo = []

        config: MirrorConfiguration = MirrorConfiguration()
        mirror_regions: dict[str, list[str]] = args.get('mirror_regions', {})
