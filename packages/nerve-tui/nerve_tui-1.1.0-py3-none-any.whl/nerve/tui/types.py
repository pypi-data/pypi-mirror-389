...
class MenuKeys(Enum):
    ...
    # Menu right: right, l
    MENU_RIGHT = frozenset({
        261,
        108
    })

    # Menu start: home CTRL-a
    MENU_START = frozenset({
        262,
        1
    })
    ...
    # Scroll down: PGDOWN
    SCROLL_DOWN = frozenset({
        338
    })

    @classmethod
    def from_ord(cls, key: int) -> list['MenuKeys']:
        return [group for group in MenuKeys if key in group.value]

    @classmethod
    def decode(cls, key: int) -> str:
        byte_str: bytes = curses.keyname(key)
...
class FrameProperties:
    ...
    def max(cls, header: str) -> 'FrameProperties':
        return FrameProperties(header, FrameStyle.MAX, FrameStyle.MAX)

    @classmethod
    def min(cls, header: str) -> 'FrameProperties':
        return FrameProperties(header, FrameStyle.MIN, FrameStyle.MIN)
