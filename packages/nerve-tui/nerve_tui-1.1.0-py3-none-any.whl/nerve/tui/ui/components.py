...
from nerve.tui.ui.result import (Result, ResultType)

ValueT: TypeVar = TypeVar('ValueT')
...
class BaseScreen(Screen[Result[ValueT]]):
    ...
    @staticmethod
    def _compose_header() -> ComposeResult:
...
class LoadingScreen(BaseScreen[None]):
    ...
    def __init__(self, timer: int, header: str | None = None) -> None:
        super().__init__()
...
class ConfirmationScreen(BaseScreen[ValueT]):
    ...
    def __init__(self, group: MenuItemGroup, header: str, allow_skip: bool = False, allow_reset: bool = False) -> None:
    ...
    def update_selection(self) -> None:
        focused: MenuItem | None = self._group.focus_item
        buttons: DOMQuery[Button] = self.query(Button)
    ...
    def on_key(self, event: Key) -> None:
        if event.key == 'enter':
            item: MenuItem | None = self._group.focus_item
...
class NotifyScreen(ConfirmationScreen[ValueT]):
    def __init__(self, header: str) -> None:
        group: MenuItemGroup = MenuItemGroup([MenuItem(tr('Ok'))])
...
class InputScreen(BaseScreen[str]):
    ...
    def __init__(self, header: str, placeholder: str | None = None, password: bool = False, default_value: str | None = None, allow_reset: bool = False, allow_skip: bool = False) -> None:
        super().__init__(allow_skip, allow_reset)
        ...
        with Center(classes='dialog-wrapper'):
            with Vertical(classes='input-dialog'):
                with Vertical(classes='input-content'):
                    ...
                    yield Input(placeholder=self._placeholder, password=self._password, value=self._default_value, id='main_input')

    def on_mount(self) -> None:
        input_field: Input = self.query_one('#main_input', Input)
        input_field.focus()

    def on_key(self, event: Key) -> None:
        if event.key == 'enter':
            input_field: Input = self.query_one('#main_input', Input)
            value: Any = input_field.value
...
class TableSelectionScreen(BaseScreen[ValueT]):
    ...
    def __init__(self, header: str | None = None, data: list[ValueT] | None = None, data_callback: Callable[[], Awaitable[list[ValueT]]] | None = None, allow_reset: bool = False, allow_skip: bool = False, loading_header: str | None = None) -> None:
        super().__init__(allow_skip, allow_reset)
    ...
    def action_cursor_down(self) -> None:
        ...
        if table.cursor_row is not None:
            next_row: int = min(table.cursor_row + 1, len(table.rows) - 1)
    ...
    def action_cursor_up(self) -> None:
        ...
        if table.cursor_row is not None:
            prev_row: int = max(table.cursor_row - 1, 0)
    ...
    def on_mount(self) -> None:
        ...
        self._put_data_to_table(data_table, self._data) if self._data else self._load_data(data_table)

    @work
    async def _load_data(self, table: DataTable[ValueT]) -> None:
        assert self._data_callback is not None
        data: list[ValueT] = await self._data_callback()
    ...
    def _display_header(self, is_loading: bool) -> None:
        ...
        try:
            loading_header: Static = self.query_one('#loading-header', Static)
            header:         Static = self.query_one('#header', Static)
    ...
    def _put_data_to_table(self, table: DataTable[ValueT], data: list[ValueT]) -> None:
        ...
        loader: Widget = self.query_one('#loader')
...
class TApp(App[Any]):
    ...
    async def _run_worker(self) -> None:
        try:
            ...
            debug('Error while running main app: ' + str(err))
            raise err from err
