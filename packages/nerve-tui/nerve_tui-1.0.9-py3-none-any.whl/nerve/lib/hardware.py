...
from nerve.lib.networking import (enrich_iface_types, list_interfaces)
...
class CpuVendor(Enum):
    ...
    def get_vendor(cls, name: str) -> 'CpuVendor':
        vendor: CpuVendor = getattr(cls, name, None)

        if vendor:
            return vendor

        debug("Unknown CPU vendor '" + name + "' detected.")
        return cls._Unknown
    ...
    def get_ucode(self) -> Path | None:
        return Path(str(self.value) + '-ucode.img') if self._has_microcode() else None
...
class GfxPackage(Enum):
    ...
    Lib32NvidiaUtils  = 'lib32-nvidia-utils'
...
class GfxDriver(Enum):
    ...
    def packages_text(self) -> str:
        pkg_names: list[str] = [p.value for p in self.gfx_packages()]
    ...
    def gfx_packages(self) -> list[GfxPackage]:
        packages: list[GfxPackage] = [GfxPackage.XorgServer, GfxPackage.XorgXinit]
...
class _SysInfo:
    ...
    def mem_info(self) -> dict[str, int]:
        ...
        """
        mem_info_path: Path = Path("/proc/meminfo")
    ...
    def loaded_modules(self) -> list[str]:
        ...
        """
        modules_path: Path = Path('/proc/modules')
        modules: list[str] = []

        with modules_path.open() as file:
            for line in file:
                module: str = line.split(maxsplit=1)[0]
...
class SysInfo:
    @staticmethod
    def has_wifi() -> bool:
        interfaces: list[str] = list(list_interfaces().values())
        return 'WIRELESS' in enrich_iface_types(interfaces).values()
    ...
    def graphics_devices() -> dict[str, str]:
        ...
        for line in SysCommand("lspci"):
            if b' VGA ' in line or b' 3D ' in line:
                (_, identifier) = line.split(sep=b': ', maxsplit=1)
    ...
    def has_nvidia_graphics() -> bool:
        return any('nvidia' in x.lower() for x in SysInfo.graphics_devices())

    @staticmethod
    def has_amd_graphics() -> bool:
        return any('amd' in x.lower() for x in SysInfo.graphics_devices())

    @staticmethod
    def has_intel_graphics() -> bool:
        return any('intel' in x.lower() for x in SysInfo.graphics_devices())

    @staticmethod
    def cpu_vendor() -> CpuVendor | None:
        return CpuVendor.get_vendor(vendor) if (vendor := _sys_info.cpu_info.get('vendor_id')) else None
    ...
    def virtualization() -> str | None:
        try:
            ...
            debug("Could not detect virtual system: " + str(err))
    ...
    def is_vm() -> bool:
        try:
            result: SysCommand = SysCommand("systemd-detect-virt")
            return b"none" not in b"".join(result).lower()
        except SysCallError as err:
            debug("System is not running in a VM: " + str(err))
