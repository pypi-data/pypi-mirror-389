...
class ProfileMenu(AbstractSubMenu):
    ...
    def _select_profile(self, preset: Profile | None) -> Profile | None:
        ...
        if profile:
            self._item_group.find_by_key('gfx_driver').enabled = True if profile.is_graphic_driver_supported() else False
            self._item_group.find_by_key('gfx_driver').value   = GfxDriver.AllOpenSource if profile.is_graphic_driver_supported() else None

            self._item_group.find_by_key('greeter').enabled = True if profile.is_greeter_supported() else False
            self._item_group.find_by_key('greeter').value   = profile.default_greeter_type if profile.is_greeter_supported() else None
