...
class U2FLoginMethod(Enum):
	...
	def display_value(self) -> str:
		match self:
			...
			case _:
				raise ValueError('Unknown type: ' + str(self))
...
class U2FLoginConfiguration:
	...
	def parse_arg(args: U2FLoginConfigSerialization) -> 'U2FLoginConfiguration | None':
		if not (u2f_login_method := args.get('u2f_login_method')):
			return None

		u2f_config: U2FLoginConfiguration = U2FLoginConfiguration(u2f_login_method=U2FLoginMethod(u2f_login_method))
...
class AuthenticationConfiguration:
	...
	def parse_arg(args: dict[str, Any]) -> 'AuthenticationConfiguration':
		auth_config: AuthenticationConfiguration = AuthenticationConfiguration()
