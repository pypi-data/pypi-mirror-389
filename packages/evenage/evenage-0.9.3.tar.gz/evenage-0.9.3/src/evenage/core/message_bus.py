...
class RedisBus:
    ...
    def __init__(self, redis_url: str):
        ...
        self._client = None
        self._cancel_prefix = "evenage:cancel:"
...
class KafkaBus:
    ...
    async def _get_consumer(self, agent_name: str) -> Any:
        ...
        if agent_name not in self._consumers:
            ...
            consumer = AIOKafkaConsumer(
                f"evenage.tasks.{agent_name}",
                bootstrap_servers=self.bootstrap_servers,
                group_id=f"evenage-{agent_name}-workers",
    ...
    async def publish_task(self, agent_name: str, task: dict) -> str:
        try:
            ...
            topic = f"evenage.tasks.{agent_name}"
    ...
    async def publish_response(self, task_id: str, response: dict) -> bool:
        ...
        try:
            print(f"[KafkaBus] Publishing to evenage.responses topic...")
            producer = await self._get_producer()
                            ...
                            try:
                                ...
                                if response_task_id == task_id:
                                    ...
                                    if data.get("offloaded"):
                                        ...
                                        storage = getattr(self, "_storage", None)
                                        if storage:
                                            bucket = data.get("bucket", "evenage")
                                            key = data.get("key")
                                            if key:
                                                try:
    ...
    async def get_registered_agents(self) -> dict[str, dict]:
        ...
        Topics follow the pattern: evenage.tasks.<agent_name>
        """
        try:
