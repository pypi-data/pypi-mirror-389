...
class MLDetectionModule2(BaseModule):
    ...
    def apply(self, context):
        ...
        def get_model(time, x_pos, y_pos, *flux):
            ...
            # dit = self.dit
            # wl_bin = self.wl_bin
            model = self.phringe.get_template(time, self.wavelengths, x_pos, y_pos, flux)
        ...
        # self._intensity_response = phringe.get_intensity_response(planet_name)
        # wavelength_index = 10
        ...
        flux_real = context._phringe.get_spectral_flux_density(planet_name).cpu().numpy()
        ...
        sky = [source for source in context._phringe._director._sources if source.name == planet_name][
            0].sky_brightness_distribution

        coord = [source for source in context._phringe._director._sources if source.name == planet_name][
            0].sky_coordinates

        i = np.nonzero(sky[6] > 0)
        print(i)

        x = i[0, 0]
        y = i[0, 1]

        posx = coord[0, x, y].cpu().numpy()

        posy = coord[1, x, y].cpu().numpy()

        plt.imshow(sky.cpu().numpy()[6])
        plt.show()

        plt.imshow(coord.cpu().numpy()[1])
        plt.colorbar()
        plt.show()
        ...
        def residual_data(params, target):
            ...
            if np.isnan(self.icov2).any():
                ...
                model = self.icov2 @ get_model(xdata, posx, posy, *flux).numpy()
            return model - target
        ...
        plt.subplot(3, 2, 3)
        plt.imshow(get_model(xdata, posx, posy, *flux_init)[0], cmap='Greys')
        plt.colorbar()
        plt.title('Original Model')

        plt.subplot(3, 2, 4)
        plt.imshow((self.icov2 @ get_model(xdata, posx, posy, *flux_real).numpy())[0], cmap='Greys')
        plt.colorbar()
        plt.title('Whitened Model')

        plt.subplot(3, 2, 5)
        plt.imshow(best_fit[0], cmap='Greys')
        plt.colorbar()
        plt.title('Best Fit')
        ...
        # snr
        bins = phringe._director._wavelength_bin_widths.cpu().numpy()[:-1]
        ...
        model = (self.icov2 @ get_model(xdata, posx, posy, *fluxes).cpu().numpy()).flatten()
