...
def analyse_pe_equity(pe_price: pd.Series,
    ...
    # compute pe premia
    pe_premia_nav = compute_pe_premia_factor(futures_prices=futures_prices, portfolio_vol_target=0.15, rebalancing_freq='ME')

    # compute pe excluding returns
    equity_returns = futures_prices['NDDUWI'].reindex(index=pe_excess_returns.index).ffill().pct_change()
    ...
    # betas[pe_premia_nav.name] = 0.5 + 0.5*betas[pe_premia_nav.name]
