...
def run_local_test(local_test: LocalTests):
    ...
    if local_test == LocalTests.CONSTRAINTS:
    ...
    elif local_test == LocalTests.LGT_PE:

        create_data = True
        if create_data:
            from bbg_fetch import fetch_field_timeseries_per_tickers
            #{'HLGPAFI LX Equity': 'Hamilton Lane',
                       #'FRLPSKA LX Equity': 'Franklin Lexington',
            ...
            eq_prices = fetch_field_timeseries_per_tickers(tickers={'NDDUWI Index': 'NDDUWI'}, freq='B', field='PX_LAST').ffill()
            joint_returns = qis.to_returns(pd.concat([pe_premia_nav, eq_prices], axis=1), freq='ME')
            replica = qis.returns_to_nav(joint_returns.sum(axis=1)).rename('PE replica')
            qis.save_df_to_csv(df=pe_fund_prices, file_name='pe_fund_prices', local_path=local_path)
        else:
            pe_fund_prices = qis.load_df_from_csv(file_name='pe_fund_prices', local_path=local_path)
        ...
        net_navs_unsmoothed = pd.concat([net_navs_unsmoothed, replica], axis=1)
        time_period = qis.TimePeriod('31Dec1999', '30Sep2023')
...
if __name__ == '__main__':
    ...
    run_local_test(local_test=LocalTests.LGT_PE)
