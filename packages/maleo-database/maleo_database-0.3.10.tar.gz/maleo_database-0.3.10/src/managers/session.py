...
from maleo.schemas.exception.exc import (
    MaleoException,
    Conflict,
    UnprocessableEntity,
    InternalServerError,
...
class SessionManager(Generic[SQLConfigT]):
    ...
    async def _async_session_handler(
        ...
        operation.log(self._logger, Level.DEBUG)
        ...
        try:
            ...
            exc.log_and_publish_operation(self._logger, Level.ERROR, self._publishers)
            if raise_exc:
            ...
            await session.rollback()
            me.log_and_publish_operation(self._logger, Level.ERROR, self._publishers)
            if raise_exc:
                raise me
