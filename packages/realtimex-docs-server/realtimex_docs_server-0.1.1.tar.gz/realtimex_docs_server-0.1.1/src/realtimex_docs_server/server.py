...
def list_documents(
    patterns: Optional[Union[str, List[str]]] = Field(
        default=None,
        description="Optional glob patterns (relative to the documentation root). Defaults to all files.",
    ),
) -> Dict[str, object]:
    ...
    for pattern in queries:
        query = (pattern or "**/*").replace("\\", "/")
        if ".." in query.split("/"):
            return {
                "status": "error",
                "message": "Parent directory references ('..') are not allowed in patterns.",
            }
        for candidate in root.glob(query):
...
def resolve_docs_root(args: argparse.Namespace) -> Path:
    ...
    if not raw:
        raise RuntimeError(
            "Documentation path not provided. Use --docs-path or set "
            f"the {DOCS_ENV_VAR} environment variable.",
        )
