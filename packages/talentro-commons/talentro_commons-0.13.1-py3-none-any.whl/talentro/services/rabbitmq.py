...
class RabbitMQ:
    ...
    async def consume(self, exchange_name: str, queue: str, callback: Callable):
        ...
        async for message in queue_object:
            async with message.process():
                # Gebruik de nieuwe Event.decode() methode
...
class QueueContext(metaclass=SingletonMeta):
    ...
    async def start_consuming(self, exchange_name: str, queue: str):
        asyncio.create_task(
            self._rabbit_mq.consume(exchange_name, queue, self._on_new_message)
        )
