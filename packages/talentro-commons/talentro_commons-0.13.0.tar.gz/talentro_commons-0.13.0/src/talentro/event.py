...
class EventMeta:
    ...
    correlation_id: Optional[UUID] = None # zet gelijk aan trace_id voor request/response chains
    ...
    def to_headers(self) -> Dict[str, Any]:
        ...
        if self.correlation_id:
            h["x-correlation-id"] = str(self.correlation_id)
        if self.source:
    ...
    def decode(data: bytes) -> "Event":
        ...
        ev = Event(
            ...
            meta=EventMeta(
                ...
                trace_id=UUID(meta["trace_id"]) if meta.get("trace_id") else uuid4(),
