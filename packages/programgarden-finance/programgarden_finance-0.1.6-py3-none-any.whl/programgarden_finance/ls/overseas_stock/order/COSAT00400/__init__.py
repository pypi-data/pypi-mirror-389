...
class TrCOSAT00400(TROrderAbstract):
    def __init__(self, request_data: COSAT00400Request):
        ...
        self._generic: GenericTR[COSAT00400Response] = GenericTR[COSAT00400Response](self.request_data, self._build_response, url=URLS.ORDER_URL)
    ...
    def _build_response(self, resp: Optional[object], resp_json: Optional[Dict[str, Any]], resp_headers: Optional[Dict[str, Any]], exc: Optional[Exception]) -> COSAT00400Response:
        ...
        block1_data = resp_json.get("COSAT00400OutBlock1")
        block2_data = resp_json.get("COSAT00400OutBlock2")
        ...
        if exc is None and not is_error_status:
            if block1_data is not None:
                parsed_block1 = COSAT00400OutBlock1.model_validate(block1_data)
            if block2_data is not None:
                parsed_block2 = COSAT00400OutBlock2.model_validate(block2_data)

        error_msg: Optional[str] = None
        if exc is not None:
            error_msg = str(exc)
            pg_logger.error(f"COSAT00400 request failed: {exc}")
        elif is_error_status:
            error_msg = f"HTTP {status}"
            if resp_json.get("rsp_msg"):
                error_msg = f"{error_msg}: {resp_json['rsp_msg']}"
            pg_logger.error(f"COSAT00400 request failed with status: {error_msg}")
