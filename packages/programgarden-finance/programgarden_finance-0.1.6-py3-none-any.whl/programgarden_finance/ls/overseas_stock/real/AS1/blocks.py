...
class AS1RealRequestBody(BaseModel):
    tr_cd: str = Field("AS1", description="거래 CD")
...
class AS1RealRequest(BaseModel):
    ...
    header: AS1RealRequestHeader = Field(
        ...
        title="요청 헤더 데이터 블록",
        description="AS1 API 요청을 위한 헤더 데이터 블록"
    )
    body: AS1RealRequestBody = Field(
        ...
        title="입력 데이터 블록",
        description="해외주식주문체결(미국) 입력 데이터 블록",
    )
...
class AS1RealResponseBody(BaseModel):
    ...
    """표준종목번호"""
    sBnsTp: str = Field(..., title="매매구분", description="매매구분 (1:매도, 2:매수)")
    """매매구분 (1:매도, 2:매수)"""
