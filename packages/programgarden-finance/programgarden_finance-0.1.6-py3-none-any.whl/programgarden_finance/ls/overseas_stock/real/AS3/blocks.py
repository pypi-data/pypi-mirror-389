...
class AS3RealRequestHeader(BlockRealRequestHeader):
    pass


class AS3RealResponseHeader(BlockRealResponseHeader):
    pass


class AS3RealRequestBody(BaseModel):
    tr_cd: str = Field("AS3", description="거래 CD")
    tr_key: Optional[str] = Field(None, max_length=8, description="단축코드")


class AS3RealRequest(BaseModel):
    """
    해외주식주문취소(미국) 실시간 요청
    """
    header: AS3RealRequestHeader = Field(
        AS3RealRequestHeader(
            token="",
            tr_type="1"
        ),
        title="요청 헤더 데이터 블록",
        description="AS3 API 요청을 위한 헤더 데이터 블록"
    )
    body: AS3RealRequestBody = Field(
        AS3RealRequestBody(
            tr_cd="AS3",
            tr_key=""
        ),
        title="입력 데이터 블록",
        description="해외주식주문취소(미국) 입력 데이터 블록",
    )


class AS3RealResponseBody(BaseModel):
...
class AS3RealResponse(BaseModel):
    header: Optional[AS3RealResponseHeader]
    body: Optional[AS3RealResponseBody]
