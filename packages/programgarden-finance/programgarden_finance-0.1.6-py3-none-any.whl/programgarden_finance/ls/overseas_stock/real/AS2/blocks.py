...
class AS2RealRequestHeader(BlockRealRequestHeader):
    pass


class AS2RealResponseHeader(BlockRealResponseHeader):
    pass


class AS2RealRequestBody(BaseModel):
    tr_cd: str = Field("AS2", description="거래 CD")
    tr_key: Optional[str] = Field(None, max_length=8, description="단축코드")


class AS2RealRequest(BaseModel):
    """
    해외주식주문정정(미국) 실시간 요청
    """
    header: AS2RealRequestHeader = Field(
        AS2RealRequestHeader(
            token="",
            tr_type="1"
        ),
        title="요청 헤더 데이터 블록",
        description="AS2 API 요청을 위한 헤더 데이터 블록"
    )
    body: AS2RealRequestBody = Field(
        AS2RealRequestBody(
            tr_cd="AS2",
            tr_key=""
        ),
        title="입력 데이터 블록",
        description="해외주식주문정정(미국) 입력 데이터 블록",
    )


class AS2RealResponseBody(BaseModel):
    """
    AS2 응답 바디 모델
    """
    ...
    # 주문/정정 관련 필드 (AS2 specific fields)
...
class AS2RealResponse(BaseModel):
    header: Optional[AS2RealResponseHeader]
    body: Optional[AS2RealResponseBody]
