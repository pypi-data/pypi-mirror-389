from __future__ import annotations

from typing import Callable, List
from .blocks import OVHRealResponse
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from programgarden_finance.ls.overseas_futureoption.real import Real


class RealOVH():
    def __init__(self, parent: Real):
        self._parent = parent

    def add_ovh_symbols(self, symbols: List[str]):
        return self._parent._add_message_symbols(symbols=symbols, tr_cd="OVH")

    def remove_ovh_symbols(self, symbols: List[str]):
        return self._parent._remove_message_symbols(symbols=symbols, tr_cd="OVH")

    def on_ovh_message(self, listener: Callable[[OVHRealResponse], None]):
        return self._parent._on_message("OVH", listener)

    def on_remove_ovh_message(self):
        return self._parent._on_remove_message("OVH")
