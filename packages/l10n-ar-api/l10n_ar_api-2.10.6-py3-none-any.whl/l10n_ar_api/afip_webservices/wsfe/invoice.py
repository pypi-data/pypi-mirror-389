...
class ElectronicInvoice(inv.Invoice):
    ...
    def add_iva(self, iva):
        ...
        # Buscar si ya existe un IVA con el mismo document_code
    ...
    def add_tribute(self, tribute):
        if not tribute.amount:
            return

        # Buscar si ya existe un Tribute con el mismo document_code
        existing = next((x for x in self.array_tributes if x.document_code == tribute.document_code), None)

        if existing:
            existing.amount += tribute.amount
            existing.taxable_base += tribute.taxable_base
        else:
