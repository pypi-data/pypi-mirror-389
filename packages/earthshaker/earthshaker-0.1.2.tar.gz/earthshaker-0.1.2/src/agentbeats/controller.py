...
def get_status():
    ...
    running_agents = len(os.listdir(agents_folder))
    return {
...
def list_agents():
    ...
    for agent_id in os.listdir(agents_folder):
        ...
        port = settings.port
        agents[agent_id] = {
            "url": f"{'https' if settings.https_enabled else 'http'}://{host}:{port}/to_agent/{agent_id}",
...
def maintain_agent_process(agent_id: str):
    ...
    while True:
        ...
        if state == "pending":
            ...
            pass  # all good, nothing to worry about
