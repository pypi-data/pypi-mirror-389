# Makefile for ansible-collection-path project
#
...
#   PIP_CONSTRAINTS: Path name of a pip constraints file to be used
...
# Pypi package name
pypi_package_name := ansible-collection-path

# Python package name (also used for dist archive names)
python_package_name := ansible_collection_path

# Package directory
package_dir := src/ansible_collection_path
...
# Docker image
docker_image_name := ansible-collection-path
...
ifdef PIP_CONSTRAINTS
  pip_level_opts := --upgrade --upgrade-strategy eager -c $(PIP_CONSTRAINTS)
else
...
help:
	...
	@echo "  PIP_CONSTRAINTS  - Path name of a pip constraints file to be used"
...
.PHONY: flake8
flake8: $(done_dir)/flake8_$(pymn).done
	@echo "Makefile: $@ done."
...
.PHONY: all
all: install develop flake8 ruff pylint bandit build authors
	@echo "Makefile: $@ done."
...
clean:
	...
	-$(call RMDIR_FUNC,build src/ansible_collection_path.egg-info .ruff_cache)
	@echo "Makefile: $@ done."
