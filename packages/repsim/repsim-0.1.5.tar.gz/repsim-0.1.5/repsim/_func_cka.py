...
def cka(
    ...
    kernel_type : str, optional
        Kernel type for CKA computation. Defaults to ``"rbf"``. See
        ``repsim.repsim_kernels()`` for the list of supported
        kernels at runtime.
    estimator: str, optional
        Estimator type for CKA computation. Defaults to ``"gretton"``. See
        ``repsim.repsim_hsic()`` for the list of supported estimators at runtime.

    Returns
    -------
    numpy.ndarray
        Array of shape ``(M, M)`` of CKA similarities.
    ...
    if kernel_type is None or (
        ...
        if par_kernel not in all_kernel:
            raise ValueError(f"* cka : kernel_type must be one of {all_kernel}.")
    ...
    if estimator is None or (
        ...
        if par_estimator not in all_estimator:
            raise ValueError(f"* cka : estimator must be one of {all_estimator}.")

    # run
    return _repsim.cpp_cka(_aux_checker(mats, "cka"), par_kernel, par_estimator)
