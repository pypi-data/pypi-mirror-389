...
@LastEditTime: 2025-10-29 11:14:37
...
class FrameBaseHandler(FrameBaseApiHandler):
    ...
    def get_http_log_extra_dict(self):
        ...
        """
        try:
            is_web = share_config.get_value("plat_type", PlatType.tb.value) == PlatType.web.value
            if is_web:
                open_id = self.get_cooke_user_id()
            else:
                code = share_config.get_value("http_log_user_code")
                open_id = (self.get_param(code) if code else None) or self.get_open_id() or self.get_user_id()
        except Exception:
            open_id = ''

        return {
            "open_id": open_id,
            "nick_name": self.get_user_nick(),
            "app_id": self.get_app_id() or "",
            "source_type": self._request_source_type(),
        }

    def prepare_executing(self, return_request_params_error=False, check_timestamp_validity=False):
        """
        :Description: prepare 执行前事件
        :param return_request_params_error: 是否返回参数转换错误
        :param check_timestamp_validity: 是否校验请求时间戳
        :last_editors: HuangJianYi
        """
        ...
        if self.request.method == 'OPTIONS':
            return False
        if self.check_current_limit_request() is True:
            return False
        invoke_result_data = self._convert_request_params()
        if return_request_params_error is True and invoke_result_data.success is False:
            ...
            return False
        # 验证超时 10秒过期
        if check_timestamp_validity is True and self.is_api_encrypt() is True and not self._check_timestamp_validity():
            return False
        # 防攻击校验
    ...
    def prepare_executed(self):
        """
        :Description: prepare 执行后事件
        :last_editors: HuangJianYi
        """
        # 标记日志请求关联
        self._build_http_log(self.get_http_log_extra_dict())
        # 记录请求参数明文
        if share_config.get_value("log_plain", True) is True and self.is_api_encrypt() is True:
    ...
    def prepare(self):
        ...
        try:
            ...
            if self.prepare_executing() is False:
                return
            if self.prepare_executed() is False:
                return
            is_build_http_log = True
        except Exception as ex:
            if is_build_http_log is False:
    ...
    def get_user_nick(self):
        ...
        :last_editors: HuangJianYi
        """
        if hasattr(self, 'context_user_nick'):
            return self.context_user_nick
        ...
        self.context_user_nick = user_nick
        return user_nick
    ...
    def get_open_id(self):
        ...
        :last_editors: HuangJianYi
        """
        if hasattr(self, 'context_open_id'):
            return self.context_open_id
    ...
    def get_user_id(self):
        ...
        :last_editors: HuangJianYi
        """
        if hasattr(self, 'context_user_id'):
    ...
    def get_app_id(self):
        ...
        :last_editors: HuangJianYi
        """
        if hasattr(self, 'context_app_id'):
            return self.context_app_id
        ...
        if plat_type == PlatType.tb.value:
            ...
            test_config = share_config.get_value("test_config", {})
            if request_source_type == 1 and test_config:
                if app_id == "":
    ...
    def get_access_token(self):
        ...
        :last_editors: HuangJianYi
        """
        if hasattr(self, 'context_access_token'):
            return self.context_access_token
        ...
        if plat_type == PlatType.tb.value:
            ...
            if self.get_param("source_app_id") == "":  #使用场景：通过IDE工具进行测试
                ...
                if store_user_nick and store_user_nick == test_config.get("user_nick", ""):
                    access_token = test_config.get("access_token", "")
        self.context_access_token = access_token
        return access_token
    ...
    def get_act_id(self):
        ...
        :last_editors: HuangJianYi
        """
        if hasattr(self, 'context_act_id'):
            return self.context_act_id
    ...
    def create_operation_log(self, operation_type=1, model_name="", handler_name="", old_detail=None, update_detail=None, operate_user_id="", operate_user_name="", operate_role_id="", title="", operation_desc=""):
        ...
        operate_base_model.add_operation_log(operation_type, title, operation_desc, model_name, handler_name, old_detail, update_detail, self.get_app_id(), self.get_act_id(), operate_user_id, operate_user_name, operate_role_id, {
            "request_params": self.request_params,
            "request": self.request
        }, self.get_remote_ip(), self.request_code)
    ...
    def check_current_limit_request(self):
        """
        :description: 全局请求限制校验(次数/分钟)，限制当前请求次数，超过次数直接返回错误提示
        ...
        if safe_config.get("is_current_control", False) == True:
            ...
            if SafeHelper.check_current_limit_by_time_window("global_current_limit", limit_count, 60) == True and limit_count > 0:
                self.response_json_error("global_current_limit", '当前活动过于火爆，请稍候再试')
                self.finish()
                return True
...
class ClientBaseHandler(FrameBaseHandler):
    ...
    def prepare(self):
        ...
        is_build_http_log = False
        try:
            if self.prepare_executing(return_request_params_error=True, check_timestamp_validity=True) is False:
                return
            if self.prepare_executed() is False:
                return
            is_build_http_log = True
