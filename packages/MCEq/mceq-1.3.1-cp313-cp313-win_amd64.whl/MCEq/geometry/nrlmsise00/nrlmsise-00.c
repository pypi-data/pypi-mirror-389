...
double densm(double alt, double d0, double xm, double *tz, int mn3, double *zn3, double *tn3,
		double *tgn3, int mn2, double *zn2, double *tn2, double *tgn2)
{
...
double densu(double alt, double dlb, double tinf, double tlb, double xm, double alpha, double *tz,
		double zlb, double s2, int mn1, double *zn1, double *tn1, double *tgn1)
{
...
/*    Eq. A24d */__inline_double g0(double a, double *p)
{
	return (a - 4.0
			+ (p[25] - 1.0)
					* (a - 4.0 + (exp(-sqrt(p[24] * p[24]) * (a - 4.0)) - 1.0) / sqrt(p[24] * p[24])));
}

/*    Eq. A24c */__inline_double sumex(double ex)
{
	return (1.0 + (1.0 - pow(ex, 19.0)) / (1.0 - ex) * pow(ex, 0.5));
}

/*    Eq. A24a */__inline_double sg0(double ex, double *p, double *ap)
{
	return (g0(ap[1], p)
			+ (g0(ap[2], p) * ex + g0(ap[3], p) * ex * ex + g0(ap[4], p) * pow(ex, 3.0)
					+ (g0(ap[5], p) * pow(ex, 4.0) + g0(ap[6], p) * pow(ex, 12.0))
							* (1.0 - pow(ex, 8.0)) / (1.0 - ex))) / sumex(ex);
}
...
double globe7(double *p, struct nrlmsise_input *input, struct nrlmsise_flags *flags)
{
	...
	if (flags->sw[14])
	{
		t[13] = f2
				* ((p[39] * plg[3][3]
						+ (p[93] * plg[3][4] + p[46] * plg[3][6]) * cd14 * flags->swc[5]) * s3tloc
						+ (p[40] * plg[3][3]
		...
		struct nrlmsise_output *output)
{
	...
	meso_tgn2[1] = pavgm[8] * pma[9][0]
			* (1.0 + flags->sw[20] * flags->sw[22] * glob7s(pma[9], input, flags)) * meso_tn2[3]
			* meso_tn2[3] / (pow((pma[2][0] * pavgm[2]), 2.0));
	...
	if (input->alt < zn3[0])
	{
		...
		meso_tgn3[1] = pma[7][0] * pavgm[7] * (1.0 + flags->sw[22] * glob7s(pma[7], input, flags))
				* meso_tn3[4] * meso_tn3[4] / (pow((pma[6][0] * pavgm[6]), 2.0));
	}
	...
	output->d[2] = densm(input->alt, dm28m, xmm, &tz, mn3, zn3, meso_tn3, meso_tgn3, mn2, zn2,
			meso_tn2, meso_tgn2);
	...
	/**** temperature at altitude ****/
	dd = densm(input->alt, 1.0, 0, &tz, mn3, zn3, meso_tn3, meso_tgn3, mn2, zn2, meso_tn2,
			meso_tgn2);
	output->t[1] = tz;
		...
		struct nrlmsise_output *output)
{
	...
	output->d[5] =
			1.66E-24
					* (4.0 * output->d[0] + 16.0 * output->d[1] + 28.0 * output->d[2]
		...
		struct nrlmsise_output *output, double press)
{
	...
	do
	{
		...
		gtd7(input, flags, output);
		z = input->alt;
		xn = output->d[0] + output->d[1] + output->d[2] + output->d[3] + output->d[4] + output->d[6]
				+ output->d[7];
		p = bm * xn * output->t[1];
		if (flags->sw[0])
			p = p * 1.0E-6;
		...
		struct nrlmsise_output *output)
{
	...
	if (input->alt < 300.0)
	{
		...
		meso_tgn1[1] = ptm[8] * pma[8][0]
				* (1.0 + flags->sw[18] * flags->sw[20] * glob7s(pma[8], input, flags)) * meso_tn1[4]
				* meso_tn1[4] / (pow((ptm[4] * ptl[3][0]), 2.0));
	}
	else
	{
	...
	zhf = pdl[1][24]
			* (1.0
					+ flags->sw[5] * pdl[0][24] * sin(dgtr * input->g_lat)
	...
	output->d[2] = densu(z, db28, tinf, tlb, 28.0, alpha[2], &output->t[1], ptm[5], s, mn1, zn1,
			meso_tn1, meso_tgn1);
	dd = output->d[2];
	/* Turbopause */
	zh28 = pdm[2][2] * zhf;
	...
	b28 = densu(zh28, db28, tinf, tlb, xmd, (alpha[2] - 1.0), &tz, ptm[5], s, mn1, zn1, meso_tn1,
			meso_tgn1);
	if ((flags->sw[15]) && (z <= altl[2]))
	{
		/*  Mixed density at Alt */
		dm28 = densu(z, b28, tinf, tlb, xmm, alpha[2], &tz, ptm[5], s, mn1, zn1, meso_tn1,
				meso_tgn1);
		/*  Net density at Alt */
	...
	/*  Diffusive density at Alt */
	output->d[0] = densu(z, db04, tinf, tlb, 4., alpha[0], &output->t[1], ptm[5], s, mn1, zn1,
			meso_tn1, meso_tgn1);
	dd = output->d[0];
	if ((flags->sw[15]) && (z < altl[0]))
	{
		...
		zh04 = pdm[0][2];
		/*  Mixed density at Zlb */
		b04 = densu(zh04, db04, tinf, tlb, 4. - xmm, alpha[0] - 1., &output->t[1], ptm[5], s, mn1,
				zn1, meso_tn1, meso_tgn1);
		/*  Mixed density at Alt */
		dm04 = densu(z, b04, tinf, tlb, xmm, 0., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
				meso_tgn1);
		zhm04 = zhm28;
		/*  Net density at Alt */
	...
	/*   Diffusive density at Alt */
	output->d[1] = densu(z, db16, tinf, tlb, 16., alpha[1], &output->t[1], ptm[5], s, mn1, zn1,
			meso_tn1, meso_tgn1);
	dd = output->d[1];
	if ((flags->sw[15]) && (z <= altl[1]))
	{
		...
		zh16 = pdm[1][2];
		/*  Mixed density at Zlb */
		b16 = densu(zh16, db16, tinf, tlb, 16.0 - xmm, (alpha[1] - 1.0), &output->t[1], ptm[5], s,
				mn1, zn1, meso_tn1, meso_tgn1);
		/*  Mixed density at Alt */
		dm16 = densu(z, b16, tinf, tlb, xmm, 0., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
				meso_tgn1);
		zhm16 = zhm28;
		/*  Net density at Alt */
	...
	/*   Diffusive density at Alt */
	output->d[3] = densu(z, db32, tinf, tlb, 32., alpha[3], &output->t[1], ptm[5], s, mn1, zn1,
			meso_tn1, meso_tgn1);
	dd = output->d[3];
	if (flags->sw[15])
	{
		if (z <= altl[3])
		{
			...
			zh32 = pdm[3][2];
			/*  Mixed density at Zlb */
			b32 = densu(zh32, db32, tinf, tlb, 32. - xmm, alpha[3] - 1., &output->t[1], ptm[5], s,
					mn1, zn1, meso_tn1, meso_tgn1);
			/*  Mixed density at Alt */
			dm32 = densu(z, b32, tinf, tlb, xmm, 0., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
					meso_tgn1);
			zhm32 = zhm28;
			/*  Net density at Alt */
	...
	/*   Diffusive density at Alt */
	output->d[4] = densu(z, db40, tinf, tlb, 40., alpha[4], &output->t[1], ptm[5], s, mn1, zn1,
			meso_tn1, meso_tgn1);
	dd = output->d[4];
	if ((flags->sw[15]) && (z <= altl[4]))
	{
		...
		zh40 = pdm[4][2];
		/*  Mixed density at Zlb */
		b40 = densu(zh40, db40, tinf, tlb, 40. - xmm, alpha[4] - 1., &output->t[1], ptm[5], s, mn1,
				zn1, meso_tn1, meso_tgn1);
		/*  Mixed density at Alt */
		dm40 = densu(z, b40, tinf, tlb, xmm, 0., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
				meso_tgn1);
		zhm40 = zhm28;
		/*  Net density at Alt */
	...
	/*   Diffusive density at Alt */
	output->d[6] = densu(z, db01, tinf, tlb, 1., alpha[6], &output->t[1], ptm[5], s, mn1, zn1,
			meso_tn1, meso_tgn1);
	dd = output->d[6];
	if ((flags->sw[15]) && (z <= altl[6]))
	{
		...
		zh01 = pdm[5][2];
		/*  Mixed density at Zlb */
		b01 = densu(zh01, db01, tinf, tlb, 1. - xmm, alpha[6] - 1., &output->t[1], ptm[5], s, mn1,
				zn1, meso_tn1, meso_tgn1);
		/*  Mixed density at Alt */
		dm01 = densu(z, b01, tinf, tlb, xmm, 0., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
				meso_tgn1);
		zhm01 = zhm28;
		/*  Net density at Alt */
	...
	/*   Diffusive density at Alt */
	output->d[7] = densu(z, db14, tinf, tlb, 14., alpha[7], &output->t[1], ptm[5], s, mn1, zn1,
			meso_tn1, meso_tgn1);
	dd = output->d[7];
	if ((flags->sw[15]) && (z <= altl[7]))
	{
		...
		zh14 = pdm[6][2];
		/*  Mixed density at Zlb */
		b14 = densu(zh14, db14, tinf, tlb, 14. - xmm, alpha[7] - 1., &output->t[1], ptm[5], s, mn1,
				zn1, meso_tn1, meso_tgn1);
		/*  Mixed density at Alt */
		dm14 = densu(z, b14, tinf, tlb, xmm, 0., &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
				meso_tgn1);
		zhm14 = zhm28;
		/*  Net density at Alt */
	...
	tho = pdm[7][9] * pdl[0][6];
	dd = densu(z, db16h, tho, tho, 16., alpha[8], &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
			meso_tgn1);
	zsht = pdm[7][5];
	zmho = pdm[7][4];
	...
	ddum = densu(z, 1.0, tinf, tlb, 0.0, 0.0, &output->t[1], ptm[5], s, mn1, zn1, meso_tn1,
			meso_tgn1);
	if (flags->sw[0])
	{
		for (i = 0; i < 9; i++)
