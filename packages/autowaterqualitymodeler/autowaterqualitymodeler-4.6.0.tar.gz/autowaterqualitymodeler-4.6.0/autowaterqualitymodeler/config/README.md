...
`features_config.py` 是 AutoWaterQualityModeler 的核心配置文件（Python 字典形式），用于定义光谱特征计算方法、模型参数以及不同水体类型下的特征组合。本文档将指导您如何正确配置此文件。若需向后兼容，也可提供同结构的 JSON 文件，但推荐使用 `.py` 以便随包编译并内嵌至二进制。
...
通过正确配置 features_config.py 文件，您可以为不同水体类型的各种水质指标定制最合适的特征组合，实现高精度的水质参数预测模型。
