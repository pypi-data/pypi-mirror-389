...
class ConfigManager:
    ...
    def __init__(self, config_path: str | None = None, system_config_path: str | None = None):
        ...
        if config_path is None:
            config_path = os.path.join(base_dir, 'config', 'features_config.py')
        logger.info(f"使用特征配置文件: {config_path} (存在: {os.path.exists(config_path)})")
        self.config_path = config_path
        self.config = self._load_config(config_path)

        # 加载系统配置（默认 .py，不存在则使用默认值）
        if system_config_path is None:
            system_config_path = os.path.join(base_dir, 'config', 'system_config.py')
