...
### 配置系统
- `config/features_config.py`: 特征计算配置（Python 字典，随包分发）
- `config/system_config.py`: 系统参数配置（Python 字典，随包分发）
- 说明：原 `.json` 已废弃并从仓库移除，仍保留解析兼容；建议始终使用 `.py` 配置以便在编译为 `.so` 时内嵌。
  支持运行时配置修改和自定义特征定义（通过传入模块路径或自定义模块）。
