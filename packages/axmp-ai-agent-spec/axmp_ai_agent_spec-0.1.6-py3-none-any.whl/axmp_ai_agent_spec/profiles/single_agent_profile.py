...
class NodeOfSingleAgent(BaseNode):
    """Node Entity. This entity is used to store the node of the single agent."""
...
class SingleAgentFlow(BaseFlow):
    """A flow of an AI agent."""

    nodes: list[NodeOfSingleAgent] | None = Field(
        None,
...
class SingleAgentProfile(BaseProfile):
    ...
    flow: SingleAgentFlow | None = None

    @model_validator(mode="after")
    def validate_root_node(self) -> SingleAgentProfile:
