...
class CameraFollowCursorCV:
    ...
    Returns
    -------
    :class:`CameraFollowCursorCVScene`

    Raises
    ------
    ValueError
        If video_name is empty, or interval_range is invalid, or camera_floating_max_value, camera_move_interval, camera_move_duration, or camera_scale is invalid.
    """
    ...
    def __init__(
        ...
        interval_range: tuple[
            Annotated[float, Field(ge=SHORTEST_POSSIBLE_DURATION)],
            Annotated[float, Field(ge=SHORTEST_POSSIBLE_DURATION)]
        ] = (DEFAULT_TYPE_INTERVAL, DEFAULT_TYPE_INTERVAL),
    ...
    def _create_scene(self):
        ...
        class CameraFollowCursorCVScene(MovingCameraScene):
            ...
            def construct(self):
                ...
                occupy = Code(
                    code_string=number_of_lines*(max_char_num_per_line*OCCUPY_CHARACTER + '\n'),
                    language=cfccv.language,
                    paragraph_config={
                        'font': DEFAULT_CODE_FONT,
                        'line_spacing': cfccv.line_spacing
                ...
                if all(check in "acegmnopqrsuvwxyz" + EMPTY_CHARACTER for check in cfccv.code_str_lines[0]):
                    code_mobject.shift(DOWN*CODE_OFFSET)
                    occupy.shift(DOWN*0.03)
