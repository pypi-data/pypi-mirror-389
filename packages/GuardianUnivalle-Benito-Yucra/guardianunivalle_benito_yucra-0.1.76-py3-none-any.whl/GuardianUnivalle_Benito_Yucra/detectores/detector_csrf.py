...
BLOCK_ON_HIGH = getattr(settings, "CSRF_DEFENSE_BLOCK_ON_HIGH", True)
...
class CSRFDefenseMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        if not has_csrf_token(request):
            ...
            # prefer double-submit check if header+cookie or form exist
            if not double_submit_valid(request):
                # only flag if header/cookie exist but do not match; absence already flagged above
        ...
        # 3) Suspicious Content-Type for state changing requests
        for patt in SUSPICIOUS_CONTENT_TYPES:
            if patt.search(content_type or ""):
                # flag but keep analyzing
        ...
        # 5) Sensitive params in querystring (GET) or in JSON for non-same-origin
        try:
            # query string keys
            qs_keys = []
            raw_qs = request.META.get("QUERY_STRING", "") or ""
            if raw_qs:
                # simple parse for keys: split on & and =
            ...
            for k in qs_keys:
                if k in SENSITIVE_PARAMS and method != "POST":  # sensitive params outside POST
        ...
        # payload fingerprinting for cross-ip detection
        ...
        if pf:
            ...
            if repeat_count >= PAYLOAD_REPEAT_THRESHOLD:
                # escalate score if identical payload seen from many IPs
                s_csrf += repeat_count * 0.5
                signals.append(f"payload_repeat_count:{repeat_count}")

        # Build attack info (redacted)
        ...
        # store on request for auditoria
        request.csrf_attack_info = attack_info if (total_signals >= MIN_SIGNALS_TO_FLAG or payload_score > 0) else None

        # record event for telemetry (always record low/medium events for training)
        try:
            record_event({
                "ip": client_ip,
                "path": path,
        ...
        # Policy: decide block / challenge / alert
        HIGH_THRESHOLD = getattr(settings, "CSRF_DEFENSE_HIGH_SCORE", 2.5)
        MED_THRESHOLD = getattr(settings, "CSRF_DEFENSE_MED_SCORE", 1.0)
        LOW_THRESHOLD = getattr(settings, "CSRF_DEFENSE_LOW_SCORE", 0.5)

        # First, if score very high -> block and cache
        if s_csrf >= HIGH_THRESHOLD and BLOCK_ON_HIGH:
            ...
            request.csrf_attack_info = {**attack_info, "blocked": True, "block_level": level, "block_timeout": timeout}
            return HttpResponseForbidden("Request blocked by CSRF defense")

        # Medium: increment counter and possibly auto-block on repeated alerts
        if s_csrf >= MED_THRESHOLD:
            ...
            request.csrf_attack_info = {**attack_info, "blocked": False, "counter": count}
            ...
            # optional challenge: return 403 with challenge header if configured
            if getattr(settings, "CSRF_DEFENSE_USE_CHALLENGE", False):
                resp = HttpResponse("Challenge required", status=403)
                resp["X-CSRF-Challenge"] = "captcha"
                return resp
            return None

        # Low: mark but don't block
        if s_csrf >= LOW_THRESHOLD:
            logger.info(f"[CSRFDefense][MONITOR] ip={client_ip} path={path} score={s_csrf} signals={signals}")
            request.csrf_attack_info = {**attack_info, "blocked": False}
            return None
