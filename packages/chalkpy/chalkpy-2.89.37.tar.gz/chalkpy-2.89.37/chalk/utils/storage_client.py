...
class AzureBlobStorageClient(StorageClient):
    ...
    def get_uri(self, filename: str) -> str:
        """Return a URI for a filename"""
        return f"{self.protocol}://{self.bucket}@{self._account_name}.dfs.core.windows.net/{filename}"

    def get_https_uri(self, filename: str) -> str:
        """Return an HTTPS URI for signed URLs and direct access"""
        return f"https://{self._account_name}.blob.core.windows.net/{self.bucket}/{filename}"
    ...
    def upload_object(
        ...
        if len(data) < 256 * 1024 * 1024:  # < 256 MB
            blob_client.upload_blob(
                data,
                overwrite=True,
                content_settings=azure.storage.blob.ContentSettings(content_type=content_type),
                metadata=metadata,
            )
        else:
            blob_client.upload_blob(
    ...
    def copy(self, source_filename: str, dest_filename: str) -> None:
        source_blob_url = self.get_https_uri(source_filename)
        dest_blob_client = self._container_client.get_blob_client(dest_filename)
