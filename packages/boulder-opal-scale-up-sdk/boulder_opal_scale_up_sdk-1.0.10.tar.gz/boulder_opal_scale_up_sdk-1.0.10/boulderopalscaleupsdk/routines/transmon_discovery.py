...
def _validate_transmon_field(transmon: str | list[str] | None) -> list[str] | None:
    """Convert single string to list for convenience and backward compatibility."""
    if isinstance(transmon, str):
        return [transmon]
    return transmon
...
class TransmonDiscovery(Routine):
    ...
    transmon: Annotated[list[str] | None, BeforeValidator(_validate_transmon_field)] = None
