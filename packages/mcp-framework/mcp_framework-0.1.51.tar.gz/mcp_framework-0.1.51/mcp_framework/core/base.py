...
class BaseMCPServer(ABC):
    ...
    async def startup(self) -> None:
        ...
        if not self._initialized:
            # 对于 EnhancedMCPServer，检查是否需要延迟配置加载
            if (hasattr(self, '_config_loaded') and not self._config_loaded and 
                (not hasattr(self, 'server_config_manager') or self.server_config_manager is None)):
                print(f"🔍 检测到延迟配置加载，创建默认配置管理器", file=sys.stderr)
