...
class ResilienceDataTransformer(DataTransformer):
    ...
    def transform(self, results: Dict[str, Any], model_name: str, timestamp: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Ensure feature importance fields are initialized
        if 'feature_importance' not in report_data:
            logger.warning("feature_importance not found, initializing as empty dict")
            report_data['feature_importance'] = {}

        if 'model_feature_importance' not in report_data:
            logger.warning("model_feature_importance not found, initializing as empty dict")
            report_data['model_feature_importance'] = {}

        if 'feature_subset' not in report_data:
            logger.warning("feature_subset not found, initializing as empty list")
            report_data['feature_subset'] = []

        # Extract feature list from feature_importance if available
        if 'features' not in report_data and report_data.get('feature_importance'):
            feature_list = list(report_data['feature_importance'].keys())
            report_data['features'] = feature_list
            logger.info(f"Extracted {len(feature_list)} features from feature_importance")

        # Generate boxplot data from distribution shift results
        if 'boxplot_data' not in report_data:
            logger.info("Generating boxplot data from shift scenarios...")
            boxplot_data = self._generate_boxplot_data(report_data)
            if boxplot_data:
                report_data['boxplot_data'] = boxplot_data
