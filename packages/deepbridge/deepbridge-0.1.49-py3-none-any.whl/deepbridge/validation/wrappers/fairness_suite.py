...
class FairnessSuite:
    ...
    def run_threshold_analysis(
        ...
        if optimal_metrics['disparate_impact_ratio'] < 0.8:
            recommendations.append(
                "🚨 CRÍTICO: Mesmo com threshold ótimo, modelo não atinge compliance - "
                "considere retreinar modelo"
            )
            ...
            for metric_name in self.current_config['metrics']:
                ...
                if metric_name == 'disparate_impact':
                    if not result.get('passes_threshold', True):
                        critical = (
                            f"{attr}: Disparate Impact CRÍTICO "
                            f"(ratio={result['ratio']:.3f} < 0.8) - RISCO LEGAL"
                        )
    ...
    def _assess_fairness(self, score: float) -> str:
        ...
        if score >= 0.95:
            return "EXCELENTE - Modelo altamente fair e compliant"
        elif score >= 0.85:
            return "BOM - Fairness adequada para produção"
        elif score >= 0.70:
            return "MODERADO - Requer atenção e possível remediação"
        else:
            return "CRÍTICO - Intervenção necessária antes de deploy"
