...
class ResourceLoader:
    ...
    def get_resource_classes_from_module(module, file_path=None):
        ...
        # 获取模块文件路径，用于判断类是否在当前模块中定义
        module_file = getattr(module, '__file__', None)
        
        for name, obj in inspect.getmembers(module):
            if inspect.isclass(obj) and name.endswith("Resource"):
                # 只识别在当前模块中定义的类，避免导入的类造成干扰
                obj_module = inspect.getmodule(obj)
                if obj_module is None:
                    continue
                
                # 检查类是否在当前模块中定义
                # 方法1: 检查模块名（适用于标准导入的模块）
                # 方法2: 检查模块文件路径（适用于动态加载的模块）
                obj_module_file = getattr(obj_module, '__file__', None)
                is_current_module = (
                    obj_module.__name__ == module_name or
                    (module_file is not None and obj_module_file is not None and 
                     os.path.abspath(obj_module_file) == os.path.abspath(module_file))
                )
