...
def setup_dot_files(OS='Linux',
                    overwrite=False,
                    git_url="https://git.smallhack.org/jessebot/dot_files.git",
                    branch="main",
    ...
    if git_url:
        if "github.com" in git_url:
            ...
            # get just the owner/repo_name like jessebot/dot_files
            user_repo = "/".join(git_url.split("/")[-2:]).replace(".git", "")
            # should cover cases such as codeberg and selfhosted forgejo/gitea
            git_domain = git_url.split(user_repo)[0]

    if "github.com" in git_url:
        git_raw = f"https://raw.githubusercontent.com/{user_repo}/{branch}"
    else:
        # should cover cases such as codeberg and selfhosted forgejo/gitea
        git_raw = f"{git_domain}/{user_repo}/raw/branch/{branch}"
