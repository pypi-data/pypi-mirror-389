...
class CommunicationComposeViewTests(TestCase):
    ...
    def test_compose_send_now_prepares_payloads(self):
        ...
        self.assertIn('Hi', payload['subject'])
        self.assertIn('Hello there', payload['html'])
    ...
    def test_compose_schedule_for_future(self):
        ...
        delivery = communication.messages.first()
        payload = delivery.metadata.get('payload')
        self.assertEqual(payload['subject'], f'Communication {communication.pk}')
        self.assertEqual(payload['html'].strip(), '<p>Scheduled</p>')
        self.assertEqual(delivery.metadata.get('status'), 'scheduled')
        self.assertEqual(delivery.status, 'scheduled')
        self.assertIsNotNone(delivery.scheduled_at)

    def test_generate_drafts_prefers_custom_content(self):
