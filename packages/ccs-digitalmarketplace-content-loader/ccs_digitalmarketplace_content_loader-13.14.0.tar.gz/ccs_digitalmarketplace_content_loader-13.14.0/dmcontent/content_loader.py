...
class ContentSection(object):
    ...
    def create(cls, section):
        if isinstance(section, ContentSection):
            ...
            return ContentSection(
                ...
                display_only=section.get('display_only'),
    ...
    def __init__(
        ...
        self.display_only = display_only or False
    ...
    def get_data(self, form_data):
        ...
        for question in self.questions:
            ...
            if isinstance(question, Multiquestion) and not isinstance(question, ListMultiquestion):
                sub_questions = question.get_questions_flat()
            else:
                sub_questions = [question]

            for sub_question in sub_questions:
                if sub_question.type in ['boolean', 'radios'] and sub_question.id not in section_data:
                    section_data[sub_question.id] = None
    ...
    def get_error_messages(self, errors: dict, question_descriptor_from: str = "label") -> OrderedDict:
        ...
        """
        if (set(errors.keys()) - set(self.get_field_names())) and ('_form' not in errors):
        ...
        for question in self.questions:
            ...
            for sub_question in questions:
                if ('_form' in errors) and (not sub_question.is_optional):
                    errors[sub_question.id] = errors['_form'][0]
