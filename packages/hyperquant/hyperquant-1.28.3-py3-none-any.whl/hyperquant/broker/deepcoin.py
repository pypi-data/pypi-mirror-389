...
class DeepCoin:
    ...
    async def update(
        self,
        update_type: Literal[
            "all", "detail", "ticker", "orders", "positions", "balances", "trades"
        ] = "all",
        *,
        ...
        if include_positions:
            ...
            # if inst_id is None and symbol is not None:
            #     raise ValueError("inst_id is required when updating positions by symbol")
            ...
            # if inst_id:
            #     params["instId"] = resolved_inst_id
            requests.append(
                self.client.get(
    ...
    async def place_order(
        ...
        *,
        ...
        - Provide contracts via ``qty_contract`` (number of lots/contracts) or
          base quantity via ``qty_base``; when both are supplied, base quantity wins.
        - Quantisation uses instrument ``lotSz``/``tickSz`` and ``ctVal`` to ensure
          sizes/prices align with exchange requirements.
        - Returns the exchange response (success indicated by ``sCode == '0'``).
        """
