...
class Ticker(DataStore):
    ...
    def _normalize(entry: dict[str, Any]) -> dict[str, Any] | None:
        inst_id = entry.get("instId") or entry.get("I")
        symbol = entry.get("s")
        ...
        normalized.setdefault("s", symbol)
        normalized.setdefault("I", symbol)
    ...
    def _normalize_ws(data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        for key, value in data.items():
            ...
            if target:
                ...
                normalized[key] = value
        if "instId" not in normalized and "I" in data:
            normalized["instId"] = data["I"]
...
class Orders(DataStore):
    ...
    ACTIVE_STATES = {"", "0",  "2", "3", "4", "10", "11", "live", "partially_filled"}
    ...
    def _normalize_ws(data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not normalized.get("ordId"):
            return None
        # state_tomap  # 4 live 6 cancel 1 filled
        state_map = {
            "4": "live",
            "6": "canceled",
            "1": "filled",
