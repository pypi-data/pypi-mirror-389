...
class ExecutionEngine:
    ...
    async def execute_single_config(
        self, config_name: str, custom_utils_module=None
    ) -> Dict[str, Any]:
        """
        ...
        for entity_name, metrics in config.entities.items():
            for metric_config in metrics:
                ...
                for resource_idx, resource in enumerate(metric_config.resources):
                    try:
                        value = await self.processor.process_resource(
                            resource, utils_module
                        )
                        ...
                        logger.error(
                            f"Resource failed for {entity_name}.{metric_config.name}: {e}"
                        )
        ...
        return {
            ...
            "failed_metrics": sum(len(metrics) for metrics in config.entities.values())
            - len(results),
        }

    async def execute_multiple_configs(
        self, config_names: List[str], custom_utils_module=None, parallel: bool = True
    ) -> Dict[str, Dict]:
        """
    ...
    async def execute_all_configs(
        ...
        return await self.execute_multiple_configs(
            config_names, custom_utils_module, parallel
        )
