...
class IntelligentScraper:
    ...
    def _build_user_message(
        self, url: str, description: str, content: Dict[str, Any]
    ) -> str:
        """
...
"""
    ...
    async def smartscraper(
        ...
        try:
            ...
            content_result = await self.tools.fetch_url_content(
                url, render_js=False, is_truncate=False
            )
            ...
            if content_result.get("error"):
                ...
                if "403" in error_msg or "HTTP 403" in error_msg:
                    self._log(
                        "WARNING",
                        f"HTTP 403 for {url} - browser rendering attempted, continuing with available content...",
                    )
            ...
            for iteration in range(max_iterations):
                ...
                try:
                    ...
                    self._log(
                        "DEBUG", f"AI response received ({len(ai_response)} chars)"
                    )
                    ...
                    if not parse_result["valid"]:
                        ...
                        self._log(
                            "WARNING", f"Invalid XML response: {parse_result['error']}"
                        )
                    ...
                    if parse_result.get("done"):
                        ...
                        if parse_result.get("data"):
                            ...
                            if parse_result.get("message"):
                                self._log(
                                    "INFO", f"AI message: {parse_result['message']}"
                                )
                            ...
                            result["error"] = (
                                "AI returned done status but no data was extracted"
                            )
                            if self.raise_exceptions:
                                raise ValueError(result["error"])
                            return result
                        ...
                        if parse_result.get("message"):
                            ...
                            user_message = (
                                f"Continue extraction: {parse_result['message']}"
                            )
                        continue
            ...
            if not result["success"]:
                result["error"] = (
                    f"Failed to extract data after {max_iterations} iterations"
                )
                if self.raise_exceptions:
                    raise ValueError(result["error"])
