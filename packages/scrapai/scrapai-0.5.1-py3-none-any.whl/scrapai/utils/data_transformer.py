...
class DataTransformer:
    ...
    def extract_from_json_path(
            ...
            try:
                if key == "reverse":
                    ...
                    if remaining_keys:
                        ...
                        sort_order = (
                            remaining_keys[1] if len(remaining_keys) > 1 else "asc"
                        )
                    ...
                    key == "filter" and filter_func is None and len(remaining_keys) >= 2
                ):
                    ...
                    current_date = (
                        datetime.now() - timedelta(days=filter_date_key_days)
                    ).strftime("%Y-%m-%d")
                    ...
                    value = [
                        ...
                        and current_date in str(row.get(date_key, ""))
                    ]
        ...
        if remaining_keys:
            return self.extract_from_json_path(
                value, remaining_keys, default, filter_func
            )
    ...
    def calculate_tps_from_transactions(
        self, total_transactions: Any, time_window_seconds: int = 86400
    ) -> Optional[float]:
        """
    ...
    def parse_timestamp(
        self, value: Any, fmt: str = "%Y-%m-%dT%H:%M:%S"
    ) -> Optional[datetime]:
        """
    ...
    def filter_last_24_hours(
        self, items: list, timestamp_key: str = "timestamp"
    ) -> list:
        """
