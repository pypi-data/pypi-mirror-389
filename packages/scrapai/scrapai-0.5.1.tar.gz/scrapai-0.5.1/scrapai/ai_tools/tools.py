...
class AgentTools:
    ...
    def __init__(
        ...
        Args:
            ...
            proxies: Optional proxy configuration (legacy, for backward compatibility)
        ...
        self.http_client = HTTPClient(proxies, proxy_manager=proxy_manager)
    ...
    def get_available_actions(self) -> Dict[str, List[str]]:
        ...
        for name, method in inspect.getmembers(
            self.data_transformer, predicate=inspect.ismethod
        ):
        ...
        def _reduce_recursive(obj: Any, depth: int = 0) -> Any:
            ...
            if isinstance(obj, list):
                ...
                if len(obj) > max_items:
                    ...
                    first_items = [
                        _reduce_recursive(item, depth + 1) for item in obj[:first_count]
                    ]

                    last_items = [
                        _reduce_recursive(item, depth + 1) for item in obj[-last_count:]
                    ]
    ...
    async def fetch_url_content(
        self, url: str, render_js: bool = False, is_truncate=True
    ) -> Dict[str, Any]:
        """
        ...
        **Example**:
        ...
        try:
            self._log(
                "Tool", "[fetch_url_content] Fetching content with smart rendering..."
            )
            ...
            if original_content is not None:
                ...
                if fetch_result.get("is_json"):
                    ...
                    def _has_reduction_info(obj: Any) -> bool:
                        ...
                        if isinstance(obj, list):
                            return any(
                                isinstance(item, dict) and item.get("_reduction_info")
                                for item in obj
                            )
                    ...
                    if _has_reduction_info(reduced_content):
                        self._log(
                            "Tool",
                            "[fetch_url_content] Large lists detected: showing first 5 + last 5 items (content reduced)",
                        )
            ...
            if fetch_result.get("rendered"):
                self._log(
                    "Tool", "[fetch_url_content] ✓ Content rendered with Playwright"
                )

            if fetch_result.get("is_json"):
                self._log("Tool", "[fetch_url_content] ✓ JSON response received")

                if isinstance(result.get("content"), dict):
                    keys = [
                        k for k in result["content"].keys() if not k.startswith("_")
                    ]
            ...
            self._log(
                "Tool",
                f"[fetch_url_content] ✓ Content processed ({content_size} chars)",
            )
    ...
    async def discover_apis(
        self, url: str, search_keywords: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        **Example**:
        ...
        try:
            ...
            self._log(
                "Tool", f"[discover_apis] Found {result.get('total_apis', 0)} API calls"
            )
            ...
            if matched:
                self._log(
                    "Tool", f"[discover_apis] Matched {len(matched)} APIs with keywords"
                )
    ...
    async def test_api_path_on_json(self, url: str, path: str) -> Dict[str, Any]:
        ...
        **Examples**:
        ...
        self._log(
            "Tool", f"[test_api_path_on_json] Testing path: '{path}' on URL: {url}"
        )
    ...
    def get_method_implementation(
        self, method_name: str, config_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        **Example**:
        ...
        try:
            if add_to_base_utils:
                ...
                result["error"] = (
                    "Must specify either config_name or add_to_base_utils=True"
                )
    ...
    def validate_results(
        self, data: Any, expected_type: str, description: str
    ) -> Dict[str, Any]:
        """
    ...
    def update_brain(
        self, content: str, brain_type: str = "ai_analyzer"
    ) -> Dict[str, Any]:
        """
        ...
        **Example**:
        ...
        result = {"brain_type": brain_type, "success": False, "error": None}
    ...
    def list_user_base_utils_methods(self) -> Dict[str, Any]:
        ...
        **Example**:
        ...
        result = {"exists": False, "methods": [], "error": None}
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                "user_base_utils", base_utils_path
            )
            ...
            for name, obj in inspect.getmembers(module):
                if inspect.isfunction(obj) and not name.startswith("_"):
                    ...
                    result["methods"].append(
                        {"name": name, "signature": f"{name}{sig}", "docstring": doc}
                    )
        ...
        try:
            ...
            if existing_metric is not None:
                ...
                entities[entity_name].append(
                    {"name": metric_name, "resources": resources}
                )
