...
"""
    ...
    def validate_and_parse(ai_response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if root.tag == "xml":
                ...
                result["error"] = (
                    f"Unknown root tag: {root.tag}. Expected <xml>, <scraper_config>, or <tool_call>"
                )
                return result
            ...
            if (
                ...
                and not result["configs"]
                and not result["done"]
            ):
                result["error"] = (
                    "XML is valid but contains no meaningful content (no message, tool_calls, configs, or status)"
                )
                result["valid"] = False
                return result
    ...
    def _clean_xml(response: str) -> Optional[str]:
        ...
        if not (
            ...
            or response.startswith("<scraper_config>")
            or response.startswith("<tool_call>")
        ):
            # Try to find XML tags
            xml_start_patterns = [r"<xml>", r"<scraper_config>", r"<tool_call>"]
...
<status>processing</status>
    ...
    def _parse_metadata_dict(metadata_elem: Optional[ET.Element]) -> Dict:
        ...
        return {
            "data_type": XMLConfigParser._get_text(
                metadata_elem, "data_type", "single_value"
            ),
            "expected_format": XMLConfigParser._get_text(
                metadata_elem, "expected_format", ""
            ),
            "sample_value": XMLConfigParser._get_text(metadata_elem, "sample_value"),
            "update_frequency": XMLConfigParser._get_text(
                metadata_elem, "update_frequency", "daily"
            ),
            "requires_auth": XMLConfigParser._get_text(
                metadata_elem, "requires_auth", "false"
            ).lower()
            == "true",
            "is_paginated": XMLConfigParser._get_text(
                metadata_elem, "is_paginated", "false"
            ).lower()
            == "true",
            "created_at": datetime.utcnow().isoformat(),
            "capture_date": XMLConfigParser._get_text(metadata_elem, "capture_date")
    ...
    def _parse_metric(metric_elem: ET.Element) -> Optional[Dict]:
        ...
        return {"name": metric_name, "resources": resources}
    ...
    def _parse_resource_dict(resource_elem: ET.Element) -> Optional[Dict]:
        ...
        if path_elem is not None:
            ...
            for step_elem in path_elem.findall("step"):
                step = {
                    ...
                    "attribute": {step_elem.get("attribute"): step_elem.get("value")}
                    if step_elem.get("attribute")
                    else {},
                }
                path.append(step)
        ...
        return {
            "url": url,
            "resource_type": XMLConfigParser._get_text(
                resource_elem, "resource_type", "html_static"
            ),
            ...
            "path": path,
            "is_render_required": XMLConfigParser._get_text(
                resource_elem, "is_render_required", "false"
            ).lower()
            == "true",
            "is_captcha_based": XMLConfigParser._get_text(
                resource_elem, "is_captcha_based", "false"
            ).lower()
            == "true",
            "sleep_time": int(
                XMLConfigParser._get_text(resource_elem, "sleep_time", "5")
            ),
            "locator_click": XMLConfigParser._get_text(resource_elem, "locator_click")
            or None,
            "pre_actions_methods": [],  # TODO: parse if present
            "actions_methods": actions,
            "extra_method": None,
            "extra_method_kwargs": {},
            "use_proxy": XMLConfigParser._get_text(
                resource_elem, "use_proxy", "false"
            ).lower()
            == "true",
    ...
    def format_pre_analysis_xml(analysis: Dict[str, Any]) -> str:
        ...
        lines.append(
            f"  <content_type>{analysis.get('content_type', 'unknown')}</content_type>"
        )
        lines.append(
            f"  <is_json>{str(analysis.get('is_json', False)).lower()}</is_json>"
        )
        lines.append(
            f"  <is_render_required>{str(analysis.get('is_render_required', False)).lower()}</is_render_required>"
        )
        ...
        if analysis.get("apis"):
            ...
            for api in analysis["apis"][:10]:  # Limit to 10
                ...
                if api.get("request_headers"):
                    lines.append(
                        f"      <request_headers>{json.dumps(api['request_headers'])}</request_headers>"
                    )

                if api.get("response_headers"):
                    lines.append(
                        f"      <response_headers>{json.dumps(api['response_headers'])}</response_headers>"
                    )

                if api.get("matched_keywords"):
                    lines.append(
                        f"      <matched_keywords>{', '.join(api['matched_keywords'])}</matched_keywords>"
                    )
