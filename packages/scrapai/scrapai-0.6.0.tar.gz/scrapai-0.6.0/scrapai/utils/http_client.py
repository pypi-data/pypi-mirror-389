...
class HTTPClient:
    ...
    async def make_api_request(
        ...
        DEPRECATED: This method exists only for backward compatibility with ResourceProcessor.
        Use fetch_content_smart() directly instead.

        Note: POST requests still use direct requests for body support.
        GET requests use fetch_content_smart() which handles proxy/rendering/CDP automatically.
        ...
        Args:
            ...
            use_proxy: (Deprecated for GET) Whether to use proxy - auto-handled
        ...
        # GET requests: Use unified flow (fetch_content_smart)
        for attempt in range(retries):
            try:
    ...
    async def fetch_html_content(
        ...
        Note: Now internally calls fetch_content_smart() which handles proxy/rendering/CDP automatically.
        The use_proxy parameter is ignored (proxy auto-handled for 403).

        Args:
            url: Target URL
            headers: Request headers
            use_proxy: (Deprecated) Ignored - proxy auto-handled
            timeout: Request timeout
