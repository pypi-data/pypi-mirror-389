...
class ResourceProcessor:
    ...
    async def process_resource(
        self, resource: ResourceConfig, utils_module=None
    ) -> Any:
        """
        ...
        try:
            ...
            if resource.resource_type in [
                ResourceType.API_JSON,
                ResourceType.API_GRAPHQL,
            ]:
    ...
    async def process_api_resource(
        self, resource: ResourceConfig, utils_module=None
    ) -> Any:
        """
        ...
        if resource.api_path:
            value = self.data_transformer.extract_from_json_path(
                response_data, resource.api_path
            )
    ...
    async def process_static_html_resource(
        self, resource: ResourceConfig, utils_module=None
    ) -> Any:
        """
        ...
        # Priority 3: Extract using path (automatic, direct)
            ...
            soup = (
                ...
                if resource.pre_actions_methods
                else None
            )
            ...
            for method_name in resource.pre_actions_methods:
                if (
                    soup is not None
                    and utils_module
                    and hasattr(utils_module, method_name)
                ):
                    ...
                    logger.warning(
                        f"Pre-action {method_name} requires soup object but not available"
                    )
    ...
    async def process_rendered_html_resource(
        self, resource: ResourceConfig, utils_module=None
    ) -> Any:
        """
    ...
    async def process_captcha_html_resource(
        self, resource: ResourceConfig, utils_module=None
    ) -> Any:
        """
    ...
    async def process_custom_method_resource(
        self, resource: ResourceConfig, utils_module=None
    ) -> Any:
        """
        ...
        if not utils_module or not resource.extra_method:
            logger.error(
                "Custom method resource requires utils_module and extra_method"
            )
