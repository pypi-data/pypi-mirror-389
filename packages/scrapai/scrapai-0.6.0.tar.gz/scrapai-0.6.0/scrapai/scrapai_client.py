...
class ScrapAIClient:
    ...
    def __init__(
        ...
        Args:
            ...
            auto_install: Automatically install Playwright browsers and system deps (default: True)
        """
        ...
        # Auto-install Playwright browsers and system deps if requested
        if auto_install:
            try:
                self.dependency_manager.install()
            except Exception as e:
                if raise_exceptions:
                    raise
                logger.warning(f"Auto-install failed: {e}. You can call client.install() manually.")
        ...
        self.proxy_manager = ProxyManager(str(self.base_path))

        # Load proxies (backward compatibility)
        ...
        self.execution_engine = ExecutionEngine(
            self.config_manager, proxies=self.proxies, browser=None, context=None
        )
        ...
        if self.enable_logging:
            ...
            if self.service_config:
                logger.info(
                    f"AI Service: {self.service_config.service_name} ({self.service_config.base_url})"
                )
    ...
    def _load_proxies(self, proxies: Optional[Any] = None) -> Dict:
        """
        Load proxy configuration (backward compatibility).

        Uses ProxyManager if available, falls back to old format.
        """
        if proxies is None:
            # Try to load from ProxyManager (new format)
            try:
                proxy_dict = self.proxy_manager.get_proxy_for_requests()
                if proxy_dict:
                    return proxy_dict
            except Exception as e:
                logger.debug(f"ProxyManager not available or no proxies: {e}")

            # Fallback: Try to load from old .scrapai/proxies.json format
            proxies_file = self.scrapai_dir / "proxies.json"
            if proxies_file.exists():
                try:
                    with open(proxies_file, "r") as f:
                        proxies_data = json.load(f)
                        # Check if it's old format (list)
    ...
    async def add_config(
        ...
        if not self.analyzer:
            if self.raise_exceptions:
                ...
                if self.enable_logging:
                    logger.error(
                        f"AI service required. Available services: {AIServiceRegistry.list_services()}"
                    )
                return {"error": "AI service required"}

        if self.enable_logging:
            mode = (
                f"adding to '{target_config}'"
                if target_config
                else "creating new config"
            )
            logger.info(f"AI analyzer {mode} for URL: {url}")

        # Use intelligent analyzer
        result = await self.analyzer.create_config_interactive(
            url, description, target_config, max_iterations=max_iterations
        )
    ...
    async def smartscraper(
        self, url: str, description: str, max_iterations: int = 3
    ) -> Dict[str, Any]:
        """
        ...
        result = await self.scraper.smartscraper(
            url, description, max_iterations=max_iterations
        )

        if self.enable_logging:
            if result.get("success"):
                logger.info(
                    f"✓ Data extracted successfully in {result['metadata'].get('execution_time', 0):.2f}s"
                )
            else:
    ...
    async def execute_config(
        self, name: str, custom_utils_module=None
    ) -> Dict[str, Any]:
        """
        ...
        result = await self.execution_engine.execute_single_config(
            name, custom_utils_module
        )

        if self.enable_logging:
            if result.get("success"):
                logger.info(
                    f"✓ Config '{name}' executed: {result['successful_metrics']}/{result['total_metrics']} metrics"
                )
            else:
                logger.warning(
                    f"✗ Config '{name}' execution had issues: {result.get('failed_metrics', 0)} failed"
                )

        return result

    async def execute_all_configs(
        self, custom_utils_module=None, parallel: bool = True
    ) -> Dict[str, Dict]:
        """Execute all configurations."""
        return await self.execution_engine.execute_all_configs(
            custom_utils_module, parallel
        )
    ...
    def add_proxy(
        ...
        Args:
            ...
            cdp_url: CDP WebSocket URL (for CDP type, e.g., "ws://localhost:9222")
            browser_type: Browser type for CDP (chromium, firefox, webkit)
            tags: Optional tags for categorization
            description: Optional description
            is_active: Whether proxy is active

        Returns:
            Created ProxyConfig object
    ...
    def get_proxy(self, name: str) -> Optional[ProxyConfig]:
        ...
        Returns:
            ProxyConfig object or None if not found
        """
        return self.proxy_manager.get_proxy(name)
    ...
    def list_proxies(self, active_only: bool = False) -> List[ProxyConfig]:
        ...
        """
        return self.proxy_manager.list_proxies(active_only=active_only)
    ...
    def update_proxy(self, name: str, **kwargs) -> Optional[ProxyConfig]:
        ...
        Args:
            name: Proxy name
            **kwargs: Fields to update (host, port, is_active, tags, etc.)
        ...
        Example:
            # Update proxy status
            client.update_proxy("my_proxy", is_active=False)

            # Update host and port
            client.update_proxy("my_proxy", host="new.proxy.com", port=9090)
        """
        return self.proxy_manager.update_proxy(name, **kwargs)
    ...
    def remove_proxy(self, name: str) -> bool:
        ...
        """
        return self.proxy_manager.remove_proxy(name)

    async def validate_proxy(
        self, name: str, test_url: str = "https://api.ipify.org?format=json"
    ) -> Dict[str, Any]:
        """
        ...
        Example:
            result = await client.validate_proxy("my_proxy")
            if result["is_valid"]:
                print(f"Proxy is working! Response time: {result['response_time_ms']}ms")
        """
        return await self.proxy_manager.validate_proxy(name, test_url)
    ...
    def get_proxy_for_requests(self, name: Optional[str] = None) -> Dict[str, str]:
        ...
        """
        return self.proxy_manager.get_proxy_for_requests(name)
    ...
    def get_proxy_for_playwright(self, name: Optional[str] = None) -> Dict[str, Any]:
        ...
        Returns:
            Dict with Playwright proxy config or CDP connection config
        """
        return self.proxy_manager.get_proxy_for_playwright(name)
    ...
    def install(self) -> Dict[str, bool]:
        ...
        - Handles Databricks/restricted environments automatically

        Note: Python packages (requests, beautifulsoup4, etc.) are handled by pip automatically.
        This method only handles Playwright browser installation and system dependencies.

        Returns:
            Dict with installation results:
            - browsers: bool - Browser binaries installation status
            - system_deps: bool - System dependencies installation status

        Example:
            # Manual installation
            results = client.install()
            if results["browsers"]:
