...
class GraphCreatorWithEmbeddings(GraphCreator):
    def __init__(
        ...
        self.logger.info("Reading entity descriptions...")
        ...
        self.logger.info("Entity descriptions read")
    ...
    def _find_entity_by_name(self, name: str) -> Entity:
        ...
        matched_entities = [self._alias_to_entities[alias] for alias in matched_aliases]

        matched_descriptions = []
        for entity_id in matched_entities:
            matched_descriptions.append(self._entity_descriptions[entity_id])

        self.logger.info(
            f"Matched descriptions for entity {name}: {matched_descriptions}"
        )
        self.logger.info("Generating embeddings for matched descriptions...")
    ...
    def __call__(self, model_answer: str) -> Graph:
        ...
        self.logger.info("triplets: %s", triplets)
        triplets_dict = self.parse_triplets(
            triplets
        )  # {"Donatus Djagom": {"religious affiliation": "Catholicism"}, "Catholicism": {"headquarters located": "Vatican City"}}
        self.logger.info("triplets_dict: %s", triplets_dict)
        self.logger.info("Generating query embedding...")
        detailed_instruct = GraphCreatorWithEmbeddings.get_detailed_instruct(
            "Given a wikidata search query, retrieve relevant passages that answer the query.",
            model_answer,
        )
