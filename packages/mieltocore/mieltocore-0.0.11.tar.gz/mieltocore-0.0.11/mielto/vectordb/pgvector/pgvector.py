...
class PgVector(VectorDb):
    ...
    def get_table_v1(self) -> Table:
        ...
        Index(f"idx_{self.table_name}_workspace_id", table.c.workspace_id)
        Index(f"idx_{self.table_name}_collection_id", table.c.collection_id)
        Index(f"idx_{self.table_name}_meta_data", table.c.meta_data, postgresql_using="gin")
        Index(f"idx_{self.table_name}_filters", table.c.filters, postgresql_using="gin")
        return table
    ...
    def insert(
        ...
        Args:
            ...
            collection_id (Optional[str]): Collection ID to insert the documents into.
        """
        try:
                ...
                for i in range(0, len(documents), batch_size):
                    ...
                    try:
                        ...
                        for doc in batch_docs:
                            try:
                                batch_records.append(self._get_document_record(doc, filters, content_hash, collection_id, workspace_id))
                            except Exception as e:
    ...
    def upsert(
        ...
        try:
            ...
            self._upsert(content_hash, documents, filters, batch_size, collection_id, workspace_id)
        except Exception as e:
    ...
    def _upsert(
        ...
        Args:
            ...
            collection_id (Optional[str]): Collection ID to upsert the documents into.
        """
        try:
            with self.Session() as sess:
                for i in range(0, len(documents), batch_size):
                    ...
                    try:
                        ...
                        for doc in batch_docs:
                            try:
                                batch_records_dict[doc.id] = self._get_document_record(doc, filters, content_hash, collection_id)  # type: ignore
                            except Exception as e:
                                logger.error(f"Error processing document '{doc.name}': {e}")
                        ...
                        upsert_stmt = insert_stmt.on_conflict_do_update(
                            ...
                            set_={
                                ...
                                "collection_id": insert_stmt.excluded.collection_id,
    ...
    async def async_upsert(
        ...
        try:
            if self.content_hash_exists(content_hash):
                self._delete_by_content_hash(content_hash)
            await self._async_upsert(content_hash, documents, filters, batch_size, collection_id, workspace_id)
        except Exception as e:
            logger.error(f"Error upserting documents by content hash: {e}")
            raise
    ...
    def search(self, query: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None, collection_id: Optional[str] = None, workspace_id: Optional[str] = None) -> List[Document]:
        ...
        Args:
            ...
            collection_id (Optional[str]): Collection ID to search the documents in.

        Returns:
            List[Document]: List of matching documents.
        """
        if self.search_type == SearchType.vector:
            return self.vector_search(query=query, limit=limit, filters=filters, collection_id=collection_id, workspace_id=workspace_id)
        elif self.search_type == SearchType.keyword:
            return self.keyword_search(query=query, limit=limit, filters=filters, collection_id=collection_id, workspace_id=workspace_id)
        elif self.search_type == SearchType.hybrid:
            return self.hybrid_search(query=query, limit=limit, filters=filters, collection_id=collection_id, workspace_id=workspace_id)
        else:
    ...
    async def async_search(
        ...
        return await asyncio.to_thread(self.search, query, limit, filters, collection_id, workspace_id)

    def vector_search(self, query: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None, collection_id: Optional[str] = None, workspace_id: Optional[str] = None) -> List[Document]:
        """
        ...
        Args:
            ...
            filters (Optional[Dict[str, Any]]): Filters to apply to the search.
            collection_id (Optional[str]): Collection ID to search the documents in.
            workspace_id (Optional[str]): Workspace ID to search the documents in.
        ...
        try:
            ...
            # Define the columns to select, including distance/score
            columns = [
                self.table.c.id,
                self.table.c.name,
            ...
            # Add distance/score column based on the distance metric
            if self.distance == Distance.l2:
                distance_column = self.table.c.embedding.l2_distance(query_embedding).label('distance')
            elif self.distance == Distance.cosine:
                distance_column = self.table.c.embedding.cosine_distance(query_embedding).label('distance')
            elif self.distance == Distance.max_inner_product:
                distance_column = self.table.c.embedding.max_inner_product(query_embedding).label('distance')
            else:
                logger.error(f"Unknown distance metric: {self.distance}")
                return []
            
            columns.append(distance_column)
            ...
            # Order the results by the distance label
            stmt = stmt.order_by(distance_column)
            ...
            for result in results:
                # Extract distance/score from the result
                distance_score = getattr(result, 'distance', None)

                # Add distance to metadata if available
                metadata = result.meta_data or {}
                if distance_score is not None:
                    metadata['distance'] = float(distance_score)
                    metadata['score'] = 1.0 - float(distance_score) if self.distance == Distance.cosine else float(distance_score)
    ...
    def keyword_search(self, query: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None, collection_id: Optional[str] = None, workspace_id: Optional[str] = None) -> List[Document]:
        """
        ...
        try:
            ...
            # Build the base statement, including the text rank
            stmt = select(*columns, text_rank.label("text_rank"))
            ...
            # Order by the text rank label
            stmt = stmt.order_by(desc('text_rank'))
            ...
            search_results: List[Document] = []
            for result in results:
                # Extract text rank score from the result
                text_rank_score = getattr(result, 'text_rank', None)
                
                # Add text rank to metadata if available
                metadata = result.meta_data or {}
                if text_rank_score is not None:
                    metadata['text_rank'] = float(text_rank_score)
                    metadata['score'] = float(text_rank_score)
    ...
    def hybrid_search(
        ...
        try:
            ...
            search_results: List[Document] = []
            for result in results:
                # Extract hybrid score from the result
                hybrid_score = getattr(result, 'hybrid_score', None)
                
                # Add hybrid score to metadata if available
                metadata = result.meta_data or {}
                if hybrid_score is not None:
                    metadata['hybrid_score'] = float(hybrid_score)
    ...
    def get_chunk(self, vector_id: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Optional[Dict[str, Any]]: Dictionary containing all chunk data, or None if not found.
                Contains: id, content_id, name, content, embedding, meta_data, filters,
                         collection_id, workspace_id, created_at
        """
        try:
            with self.Session() as sess, sess.begin():
                stmt = select(
    ...
    def list_chunks(self, content_id: str = None, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        Note: The base class signature shows List[float] as return type, but this 
        implementation returns List[Dict[str, Any]] which is more practical for 
        returning multiple chunks with their metadata.
        ...
        try:
            with self.Session() as sess, sess.begin():
                ...
                stmt = stmt.order_by(desc(self.table.c.created_at))
