...
class Knowledge:
    ...
    async def add_content_async(
        ...
        file_data = None
        if text_content:
            file_data = FileData(content=text_content, type="text/plain")
    ...
    async def insert_content(self, content: Content) -> None:
        await self._load_content(content, upsert=True, skip_if_exists=True, include=None, exclude=None)
    ...
    def content_hash_exists(self, content_hash: str, workspace_id: Optional[str] = None) -> KnowledgeRow:
        if self.contents_db is None:
            return None
        content = self.contents_db.get_knowledge_content_by_content_hash(content_hash, workspace_id)
        return content
    ...
    async def _load_from_path(
        ...
        if path.is_file():
            if self._should_include_file(str(path), include, exclude):
                ...
                if not content.skip_content_insert:
                    self._add_to_contents_db(content)
                else:
                    log_info(f"Skipping content insert for {content.id}, {content.name}, {content.path}, {content.description}")

                if content.reader:

                    print(f"Reading content with reader: {content.reader.__class__.__name__}")
                    ...
                    reader = ReaderFactory.get_reader_for_extension(path.suffix, provider=self.provider)
                ...
                for read_document in read_documents:
                    ...
                    read_document.id = generate_prefix_ulid("chunk")
    ...
    def _add_to_contents_db(self, content: Content) -> Optional[KnowledgeRow]:
        if self.contents_db:
            ...
            file_data = content.file_data.to_dict() if content.file_data else None
            ...
            content_row = KnowledgeRow(
                ...
                sync_status=content.sync_status if content.sync_status else ContentStatus.PROCESSING,
    ...
    def _update_content(self, content: Content) -> Optional[Dict[str, Any]]:
        if self.contents_db:
            ...
            if content.sync_status is not None:
                content_row.sync_status = content.sync_status
            if content.definition is not None:
                content_row.definition = content.definition
            if content.content_hash is not None:
                content_row.content_hash = content.content_hash
            if content.file_data is not None:
                content_row.file_data = content.file_data
            ...
            if content.auth is not None:
                content_row.config = { "auth": content.auth, **content_row.config }
