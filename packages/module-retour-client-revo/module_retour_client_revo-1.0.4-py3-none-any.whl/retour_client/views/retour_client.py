        ...
        if form_bug.is_valid():
            try:
                with transaction.atomic():
                    ...
                    date_fin = datetime.strptime(settings.DATE_FIN_GARANTIE, '%Y-%m-%d').date()
                    aujourd_hui = timezone.now().date()

                    if settings.ENVIRONNEMENT == RetourClientConstantes.Environnement.PRODUCTION and date_fin < aujourd_hui:
                        ...
                        # URL de l'API
                        if retour_client.utilisateur == 'info.marseille@alyseparking.com':
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if reponse_bug.temps_passe and reponse_bug.temps_passe > 0 and settings.ENVIRONNEMENT == RetourClientConstantes.Environnement.PRODUCTION:
                # URL de l'API
                if bug.utilisateur == 'info.marseille@alyseparking.com':
                    siren = settings.SIREN_CLIENT_MARSEILLE
                else:
                    siren = settings.SIREN_CLIENT
