...
class SendDataToServer(Node, ServiceAbstract):
    ...
    def save_locally_or_send(
        ...
        if f_is_local:
            ...
            self.logger.debug(f"Sending data for: {f_topic_name}")
    ...
    def create_listener_function(self, f_topic_details):
        ...
        def callback(msg):
            ...
            if f_is_subscribed:
                self.logger.debug(f"Processing and sending data for topic: {f_topic_name}")
            else:
            ...
            for destination, rates in f_destinations.items():
                self.logger.debug(f"Processing destination: {destination} -> {rates}")
                (
                    can_send_buffered,
                    can_send_live,
                    buf_freq,
