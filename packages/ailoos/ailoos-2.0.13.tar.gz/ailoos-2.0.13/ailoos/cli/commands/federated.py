...
def join(session_id, model, data_path, batch_size, learning_rate, local_epochs, max_rounds, coordinator_url):
    ...
    async def run_training():
        try:
            ...
            click.echo(f"📊 Final global accuracy: {results['final_accuracy']:.2f}%")
            click.echo(f"📉 Final global loss: {results['final_loss']:.4f}")
            click.echo(f"🔄 Total rounds completed: {results['rounds_completed']}")
            click.echo(f"💰 Total rewards earned: {results['total_rewards']} DRACMA")
