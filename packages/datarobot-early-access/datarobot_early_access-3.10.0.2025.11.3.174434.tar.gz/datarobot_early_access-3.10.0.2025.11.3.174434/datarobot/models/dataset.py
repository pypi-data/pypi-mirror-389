...
class Dataset(APIObject, BrowserMixin):
    ...
    def get_raw_data(self) -> pd.DataFrame:
        """
        Retrieves the raw data for the dataset as a pandas DataFrame.
        ...
        -------
        pd.DataFrame
            A DataFrame with the dataset's raw data.
        """
        url = f"{self._path}{self.id}/versions/{self.version_id}/rawData/"

        # Fetch one item to get columns names
        fetch_columns_resp = self._client.get(
            url,
            params={
                'limit': 1,
                'offset': 0,
        ...
        columns = fetch_columns_resp.json()['columns']
        ...
        raw_data = unpaginate(url, initial_params, self._client)
