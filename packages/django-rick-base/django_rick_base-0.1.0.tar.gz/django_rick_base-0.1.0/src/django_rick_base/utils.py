...
class DingCallbackCrypto3:
    def __init__(self, token: str = None, encodingAesKey: str = None, key: str = None):
        self.encodingAesKey = encodingAesKey or settings.DINGTALK_AES_KEY
        self.key = key or settings.DINGTALK_APP_KEY
        self.token = token or settings.DINGTALK_TOKEN
    ...
    def getDecryptMsg(self, msg_signature, timeStamp, nonce, content):
        ...
        _l = struct.unpack("!i", decodeRes[16:20])[0]
        # 获取去除初始向量，四位msg长度以及尾部corpid
        # _nl = len(decodeRes)

        if decodeRes[(20 + _l) :].decode() != self.key:
            raise ValueError("corpId 校验错误")
        return decodeRes[20 : (20 + _l)].decode()
    ...
    def encrypt(self, content):
        ...
        content = "".join(
            [self.generateRandomKey(16), msg_len.decode(), content, self.key]
        )
