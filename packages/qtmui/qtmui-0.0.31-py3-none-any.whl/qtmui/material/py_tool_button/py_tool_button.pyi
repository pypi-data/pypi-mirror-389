from typing import Optional, Union, Callable, Dict
import uuid
from qtpy.QtWidgets import QToolButton
from qtpy.QtGui import QPalette, QIcon
from qtpy.QtCore import QSize, QEvent
from qtmui.hooks import State
from qtmui.material.styles import useTheme
from qtmui.material.styles.create_theme.components.get_qss_styles import get_qss_style
from qtmui.i18n.use_translation import i18n, translate
from ..py_iconify import PyIconify
from ..widget_base import PyWidgetBase
class PyToolButton:
    def __init__(self, parent, color: str, icon: PyIconify, iconKey: str, size: Optional[Union[QSize, str]], iconSize: QSize, tooltip, text: str, width: int, sx: Optional[Union[Callable, str, Dict]], **kwargs): ...
    def _init_ui(self): ...
    def _py_svg_widget_destroy(self): ...
    def slot_set_stylesheet(self, value): ...
    def _setup_icon(self): ...
    def _set_stylesheet(self, component_styled): ...
    def reTranslation(self, value): ...
    def changeEvent(self, event: QEvent): ...
    def _set_text_color(self, color): ...
    def _set_icon(self, key): ...