from functools import lru_cache
import uuid
from typing import Optional, Union, Callable, Dict, List
from qtmui.utils.calc import timer
from qtpy.QtWidgets import QFrame, QWidget, QBoxLayout
from qtpy.QtCore import Qt
from qtmui.hooks import State
from qtmui.material.styles.create_theme.components.get_qss_styles import get_qss_style
from qtmui.material.styles import useTheme
from qtmui.material.widget_base import PyWidgetBase
from qtmui.material.typography import Typography
from qtmui.errors import PyMuiValidationError
from ..utils.validate_params import _validate_param
class Box:
    def __init__(self, key: Optional[str], direction: Union[State, str], spacing: Union[State, int], children: Optional[Union[State, List, str]], visible: Optional[Union[State, bool]], sx: Optional[Union[State, Callable, str, Dict]], **kwargs): ...
    def _set_key(self, value): ...
    def _get_key(self): ...
    def _set_direction(self, value): ...
    def _get_direction(self): ...
    def _set_spacing(self, value): ...
    def _get_spacing(self): ...
    def _set_children(self, value): ...
    def _get_children(self): ...
    def _set_visible(self, value): ...
    def _get_visible(self): ...
    def _set_sx(self, value): ...
    def _validate_children(self, children: list): ...
    def _update_layout_direction(self): ...
    def _update_layout_spacing(self): ...
    def _update_layout_children(self): ...
    def _update_layout_visible(self, init: bool): ...
    def _connect_signals(self): ...
    def _get_direction_enum(self, direction): ...
    def _get_align_enum(self, direction): ...
    def _clear_layout(self): ...
    def _init_ui(self): ...
    def _get_stylesheet(cls, _variant: str, _size: str, _color: str, _theme_mode: str): ...
    def _get_qss(cls, sx: str): ...
    def _set_stylesheet(self, component_styled): ...
    def _on_destroyed(self, ev): ...
    def paintEvent(self, arg__1): ...
    def resizeEvent(self, event): ...