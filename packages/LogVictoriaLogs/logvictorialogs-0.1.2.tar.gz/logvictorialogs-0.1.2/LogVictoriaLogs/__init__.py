...
class VictoriaLogsClient:
    ...
    def create_client(self, name: str = "logging-demo") -> 'VictoriaLogsServiceClient':
        """
        创建一个服务级别的子客户端
...
class VictoriaLogsServiceClient:
    ...
    def sent(self, message: str, service: str = None,error:bool = False,warning:bool = False, debug:bool = False,**kwargs) -> bool:
...
VictoriaLogsClient.sent = lambda self, message, service=None, **kwargs: self.create_client(service).sent(message, **kwargs)
VictoriaLogsClient.query_logs = lambda self, query="*": self.create_client().query_logs(query)
VictoriaLogsClient.print_logs = lambda self, query="*": self.create_client().print_logs(query)
VictoriaLogsClient.setup_logging_handler = lambda self, service="python-app", level=logging.INFO: self.create_client(service).setup_logging_handler(service, level)
