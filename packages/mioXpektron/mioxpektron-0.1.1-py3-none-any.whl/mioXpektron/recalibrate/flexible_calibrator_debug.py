...
*** DEBUG VERSION WITH DIAGNOSTIC LOGGING ***
...
This debug version adds extensive logging to diagnose differences between
'max' and 'parabolic' peak picking methods.
...
def _parabolic_peak_center(x: npt.NDArray[np.float64], y: npt.NDArray[np.float64],
    ...
    if peak_idx == 0 or peak_idx == len(x) - 1:
        logger.debug(f"Parabolic fit failed: peak at edge (peak_idx={peak_idx}, len={len(x)})")
        return None
    ...
    if abs(denom) < 1e-10:
        logger.debug(f"Parabolic fit failed: denominator too small ({abs(denom):.2e})")
        return None
    ...
    if abs(A) < 1e-10:
        logger.debug(f"Parabolic fit failed: A coefficient too small ({abs(A):.2e})")
        return None

    xc = -B / (2 * A)

    if xc < x1 or xc > x3:
        logger.debug(f"Parabolic fit failed: center out of range (xc={xc:.4f}, range=[{x1:.4f}, {x3:.4f}])")
        return None

    logger.debug(f"Parabolic fit success: peak_mz={x2:.4f} -> refined_mz={xc:.4f} (shift={xc-x2:.4f})")
    return float(xc)
...
def _enhanced_pick_channels(
    ...
    logger.info(f"[DIAGNOSTIC] Peak picking method: {method}")
    ...
    for target_idx, xi in enumerate(targets):
        ...
        if not mask.any():
            logger.debug(f"[DIAGNOSTIC] Target {xi:.4f}: No peaks found in window")
            out.append(np.nan)
            continue
        ...
        logger.debug(f"[DIAGNOSTIC] Target {xi:.4f}: Found {len(mzw)} points in window, max at mz={peak_mz:.4f}, ch={max_ch}")

        if method == "max":
            final_ch = chw[k_local]
            logger.debug(f"[DIAGNOSTIC] Target {xi:.4f}: Using MAX method -> ch={final_ch}")
            ...
            if wsum > 0:
                ...
                logger.debug(f"[DIAGNOSTIC] Target {xi:.4f}: CENTROID mz={mz_c:.4f} -> ch={final_ch} (vs max ch={max_ch})")
            else:
                final_ch = chw[k_local]
                logger.debug(f"[DIAGNOSTIC] Target {xi:.4f}: CENTROID fallback to MAX -> ch={final_ch}")
            ...
            if center is not None:
                ...
                logger.info(f"[DIAGNOSTIC] Target {xi:.4f}: PARABOLIC refined_mz={center:.4f} -> ch={final_ch} (max_ch={max_ch}, SAME={final_ch==max_ch})")
            else:
                parabolic_fallback += 1
                final_ch = chw[k_local]
                logger.info(f"[DIAGNOSTIC] Target {xi:.4f}: PARABOLIC FALLBACK to MAX -> ch={final_ch}")
    ...
    if method == "parabolic":
        ...
        logger.info(f"[DIAGNOSTIC SUMMARY] Total targets: {total_targets}")
        logger.info(f"[DIAGNOSTIC SUMMARY] Parabolic fit succeeded: {parabolic_success}/{total_targets} ({100*parabolic_success/max(1,total_targets):.1f}%)")
        logger.info(f"[DIAGNOSTIC SUMMARY] Parabolic fit failed (fallback to max): {parabolic_fallback}/{total_targets} ({100*parabolic_fallback/max(1,total_targets):.1f}%)")
        if parabolic_success > 0:
            logger.info(f"[DIAGNOSTIC SUMMARY] When parabolic succeeded:")
            logger.info(f"[DIAGNOSTIC SUMMARY]   - Same channel as max: {parabolic_same_as_max}/{parabolic_success} ({100*parabolic_same_as_max/max(1,parabolic_success):.1f}%)")
            logger.info(f"[DIAGNOSTIC SUMMARY]   - Different channel from max: {parabolic_different_from_max}/{parabolic_success} ({100*parabolic_different_from_max/max(1,parabolic_success):.1f}%)")
        logger.info(f"[DIAGNOSTIC SUMMARY] ==========================================")
