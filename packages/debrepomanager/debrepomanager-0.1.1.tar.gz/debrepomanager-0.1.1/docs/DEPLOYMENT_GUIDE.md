...
**URL для клиентов**: `http://repo.jethome.ru/beta`
...
sudo tee /etc/repomanager/config.yaml > /dev/null <<EOF
aptly:
...
sudo sed -i 's/1234567890ABCDEF/ВАШ_РЕАЛЬНЫЙ_KEY_ID/' /etc/repomanager/config.yaml
```
...
sudo tee /etc/nginx/sites-available/repo.jethome.ru <<'EOF'
server {
    listen 80;
    server_name repo.jethome.ru repo.site.com;
...
sudo ln -s /etc/nginx/sites-available/repo.jethome.ru /etc/nginx/sites-enabled/
sudo nginx -t
...
curl -I http://repo.jethome.ru/beta/bookworm/jethome-bookworm/dists/jethome-bookworm/Release
# Должен вернуть: HTTP/1.1 200 OK

# Проверка публичного ключа
curl http://repo.jethome.ru/pubkey.gpg
# Должен вывести GPG ключ

# Проверка Packages файла
curl http://repo.jethome.ru/beta/bookworm/jethome-bookworm/dists/jethome-bookworm/main/binary-amd64/Packages | head -20
```
...
```bash
# Импорт GPG ключа
wget -qO - http://repo.jethome.ru/pubkey.gpg | sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/jethome.gpg

# Добавление репозитория
echo "deb http://repo.jethome.ru/beta bookworm jethome-bookworm" | \
...
# Добавление репозитория
echo "deb http://repo.jethome.ru/beta noble jethome-noble" | \
    sudo tee /etc/apt/sources.list.d/jethome.list
...
# Добавление репозитория
echo "deb http://repo.jethome.ru/beta jammy jethome-jammy" | \
    sudo tee /etc/apt/sources.list.d/jethome.list
...
**ВАЖНО**: Ваш URL `http://repo.jethome.ru/beta` требует специальной настройки!

### Обновите config.yaml:

```bash
sudo nano /etc/repomanager/config.yaml
```
...
**URL для клиентов:**
```
deb http://repo.jethome.ru/beta bookworm jethome-bookworm
```
...
echo "✓ Конфигурация создана: /etc/repomanager/config.yaml"
echo ""
...
echo "2. Проверьте HTTP доступ: curl -I http://repo.jethome.ru/beta/bookworm/jethome-bookworm/dists/jethome-bookworm/Release"
...
wget -qO - http://repo.jethome.ru/pubkey.gpg | \
    sudo gpg --dearmor -o /etc/apt/trusted.gpg.d/jethome.gpg

# Добавление репозитория
echo "Добавление репозитория JetHome..."
echo "deb http://repo.jethome.ru/beta $CODENAME jethome-$CODENAME" | \
    sudo tee /etc/apt/sources.list.d/jethome.list
...
```bash
# На клиенте:
curl -O http://repo.jethome.ru/setup_jethome_repo.sh
...
cat /etc/repomanager/config.yaml
...
- [ ] Конфигурация создана: `cat /etc/repomanager/config.yaml`
...
- [ ] HTTP доступ работает: `curl -I http://repo.jethome.ru/beta/bookworm/jethome-bookworm/dists/jethome-bookworm/Release`
- [ ] Публичный ключ доступен: `curl http://repo.jethome.ru/pubkey.gpg`
...
**Ваш формат:** `deb http://repo.jethome.ru/beta bookworm jethome-bookworm`

Разбор:
- `http://repo.jethome.ru/beta` - базовый URL (nginx root: /opt/repo, запрос к /beta)
...
```
deb http://repo.jethome.ru/beta/dists bookworm jethome-bookworm
```
