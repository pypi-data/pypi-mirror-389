...
**URL схема**: `http://repo.jethome.ru/{codename}/{component}`
...
aptly repo add → snapshot → publish switch
    ↓
http://repo.jethome.ru/bookworm/jethome-tools (обновлен атомарно)
```
...
4. Опубликовать: aptly publish snapshot ... trixie/jethome-tools
    ↓
http://repo.jethome.ru/trixie/jethome-tools (готов к использованию)
```
...
3. **CDN**: CloudFlare/CloudFront перед repo.jethome.ru
