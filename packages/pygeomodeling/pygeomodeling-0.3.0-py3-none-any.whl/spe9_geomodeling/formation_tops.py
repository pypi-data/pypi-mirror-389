...
class FormationTopDetector:
    ...
    def compute_boundary_score(
        ...
        for curve in curves:
            ...
            if method in ['composite', 'variance']:
                ...
                local_var = pd.Series(values).rolling(
                    window=window,
                    center=True,
                    min_periods=1
                ).var().values
    ...
    def detect_boundaries(
        ...
        if use_peak_detection:
            ...
            min_distance = int(self.min_formation_thickness / np.median(np.diff(data.index)))
    ...
    def train_boundary_classifier(
        ...
        for well_data, formation_tops in training_data:
            if curves is None:
                curves = [col for col in well_data.columns 
                         if not col.endswith(('_DERIV', '_RATIO', '_ROLL', '_SPATIAL'))]
            ...
            for idx in range(len(well_data)):
                ...
                window_size = max(1, int(self.min_formation_thickness / 2 / np.median(np.diff(well_data.index))))
    ...
    def classify_boundaries(
        ...
        for depth in detected_boundaries:
            ...
            # Add context
            window_size = max(1, int(self.min_formation_thickness / 2 / np.median(np.diff(data.index))))
            start_idx = max(0, idx - window_size)
    ...
    def correlate_with_stratigraphy(
        ...
        if regional_tops:
            ...
            for i, boundary_depth in enumerate(boundaries):
                ...
                formation_tops.append(FormationTop(
...
def compare_tops_with_reference(
    ...
    for ref_top in reference_tops:
        ...
        matching_pred = [
            p for p in predicted_tops 
            if p.formation_name == ref_top.formation_name
        ]
