...
class OrdinaryKriging:
    ...
    def fit(self, coordinates: np.ndarray, values: np.ndarray):
        ...
        try:
            ...
            warnings.warn(
                "Kriging matrix is singular. Increasing regularization.",
                UserWarning
            )
    ...
    def predict(
        ...
        if self.K_inv is None:
            raise DataValidationError(
                "Kriging system not fitted",
                suggestion="Call fit() before predict()"
            )
    ...
    def cross_validate(self, n_folds: int = 5) -> Dict[str, float]:
        ...
        kf = KFold(n_splits=min(n_folds, len(self.values)), shuffle=True, random_state=42)
...
class UniversalKriging:
    ...
    def predict(
        ...
        if self.K_inv is None:
            raise DataValidationError(
                "Universal kriging system not fitted",
                suggestion="Call fit() first"
            )
    ...
    def predict(
        ...
        for i in range(n_targets):
            ...
            if return_variance:
                ...
                variances[i] = max(C_0 - np.dot(weights_p, k_p) - np.dot(weights_s, k_s), 0.0)
