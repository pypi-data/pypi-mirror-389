...
CURVE_SIGNATURES = {
    ...
    'RESISTIVITY': {
        'names': ['ILD', 'ILM', 'ILS', 'RT', 'RES', 'RESD', 'RESS', 
                  'LLD', 'LLS', 'AT90', 'RACELM'],
...
class ProcessedWellLogs:
    ...
    def __str__(self) -> str:
        good_curves = sum(1 for q in self.quality_report.values() if q.quality_flag == 'good')
        return (
...
class WellLogProcessor:
    ...
    def identify_curve_type(
        ...
        if unit:
            ...
            for curve_type, signature in CURVE_SIGNATURES.items():
                if any(pattern.upper() in unit_upper for pattern in signature['unit_patterns']):
                    if self._verify_curve_statistics(data, signature['typical_range']):
                        return curve_type
    ...
    def impute_missing_values(
        ...
        for col in data.columns:
            ...
            if method == 'linear':
                ...
                series = series.fillna(series.rolling(window=5, center=True, min_periods=1).median())
            elif method == 'forward':
    ...
    def process_well_logs(
            ...
            for old_name, curve_type in curve_types.items():
                for new_name in processed_data.columns:
                    if new_name == old_name or (
                        curve_type in CURVE_SIGNATURES and 
                        new_name in CURVE_SIGNATURES[curve_type]['names']
                    ):
        ...
        acceptable_count = sum(1 for q in quality_report.values() if q.quality_flag == 'acceptable')
