...
class LASExporter:
    ...
    def export_with_interpretation(
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f" VERS.                  {self.version}:   CWLS LOG ASCII STANDARD - VERSION {self.version}\n")
            f.write(f" WRAP.                  NO:   ONE LINE PER DEPTH STEP\n")
            f.write(f" GENERATED BY.          PyGeomodeling:   Automated Well Log Interpretation\n")
            f.write(f" DATE.                  {datetime.datetime.now().strftime('%Y-%m-%d')}:   Export Date\n")
            
            # Write well information section
            f.write(f"~WELL INFORMATION\n")
            if well_header:
                f.write(f" WELL.                  {well_header.get('well_name', 'UNKNOWN')}:   Well Name\n")
                f.write(f" UWI.                   {well_header.get('uwi', '')}:   Unique Well Identifier\n")
                f.write(f" FLD.                   {well_header.get('field', '')}:   Field\n")
                f.write(f" LOC.                   {well_header.get('location', '')}:   Location\n")
                f.write(f" COMP.                  {well_header.get('operator', '')}:   Operator\n")
            f.write(f" STRT.M                 {depth.min():.2f}:   Start Depth\n")
            f.write(f" STOP.M                 {depth.max():.2f}:   Stop Depth\n")
            f.write(f" STEP.M                 {np.median(np.diff(depth)):.4f}:   Step\n")
            ...
            for curve_name in interpreted_curves.keys():
                unit = "code"
                desc = curve_descriptions.get(curve_name, curve_name) if curve_descriptions else curve_name
            ...
            for i, d in enumerate(depth):
                ...
                # Original curves
                for col in original_curves.columns:
                    val = original_curves[col].iloc[i] if i < len(original_curves) else null_value
                    row.append(f"{val:10.4f}")
                
                # Interpreted curves
                for curve_name in interpreted_curves.keys():
                    val = interpreted_curves[curve_name][i] if i < len(interpreted_curves[curve_name]) else null_value
...
class FormationTopExporter:
    ...
    def export_to_petrel_format(
        ...
        with open(output_file, 'w') as f:
            ...
            for well_name, tops in tops_by_well.items():
                for top in tops:
                    f.write(f"{well_name}\t{top.formation_name}\t{top.depth:.2f}\t{top.depth:.2f}\t{top.confidence:.3f}\t{top.method}\n")
    ...
    def export_to_ascii(
        ...
        with open(output_file, 'w') as f:
            ...
            f.write("\n")
            f.write(f"{'Formation':<30} {'Depth (m)':>12} {'Confidence':>12} {'Method':<20}\n")
            f.write("-" * 80 + "\n")
            
            for top in tops:
                f.write(f"{top.formation_name:<30} {top.depth:>12.2f} {top.confidence:>12.3f} {top.method:<20}\n")
...
class PetrelProjectExporter:
    ...
    def export_interpretation_package(
        ...
        with open(readme, 'w') as f:
            ...
            f.write(f"Contents:\n")
            f.write(f"- interpreted_logs/: LAS files with ML-interpreted facies curves\n")
