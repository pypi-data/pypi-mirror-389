...
class LogFeatureEngineer:
    ...
    def compute_derivatives(
        ...
        """
        if curves is None:
            curves = [col for col in data.columns 
        ...
        for curve in curves:
            ...
            if method == 'gradient':
                ...
                window_length = int(5 * smooth_sigma) if int(5 * smooth_sigma) % 2 == 1 else int(5 * smooth_sigma) + 1
                window_length = max(5, min(window_length, len(smoothed) // 2))
                deriv = savgol_filter(smoothed, window_length=window_length, polyorder=2, deriv=1, delta=depth_step)
            else:
        ...
        for ratio_name, (num_curve, den_curve, transform) in ratio_definitions.items():
            ...
            if valid_mask.sum() > 0:
                ...
                ratio_values[valid_mask] = transform(
                    num_values[valid_mask],
                    den_values[valid_mask]
                )
    ...
    def _get_standard_ratios(self) -> Dict[str, Tuple[str, str, Callable]]:
        ...
        return {
            ...
            'PORO_DENSITY': ('RHOB', 'RHOB', lambda rhob, _: self._density_to_porosity(rhob)),
    ...
    def _gamma_ray_to_vshale(self, gr: np.ndarray, gr_clean: float = 20, gr_shale: float = 150) -> np.ndarray:
    ...
    def compute_spatial_features(
        ...
        if curves is None:
            ...
            curves = [c for c in all_curves if not c.endswith(('_DERIV', '_RATIO', '_ROLL', '_SPATIAL'))]
    ...
    def create_feature_set(
        ...
        **kwargs,
        ...
        if include_derivatives:
            derivatives = self.compute_derivatives(data, **kwargs.get('derivative_params', {}))
        ...
        if include_rolling_stats:
            rolling = self.compute_rolling_statistics(data, **kwargs.get('rolling_params', {}))
            features = pd.concat([features, rolling], axis=1)
        
        if include_spatial:
            if offset_wells is None or well_locations is None or target_location is None:
    ...
    **feature_kwargs,
    ...
    for well_name, well_data in train_wells.items():
        ...
        feature_set = feature_engineer.create_feature_set(
            well_data.drop(columns=[target_column]),
            **feature_kwargs
        )
