...
class ConfidenceScorer:
    ...
    def create_well_report(
        ...
        high_conf_count = sum(1 for s in confidence_scores if s.confidence_level == 'high')
        high_confidence_fraction = high_conf_count / len(confidence_scores)
        
        # Identify depths needing review
        low_confidence_depths = [
            depth for depth, score in zip(depths, confidence_scores)
        ...
        summary = {
            ...
            'medium_confidence_fraction': sum(1 for s in confidence_scores if s.confidence_level == 'medium') / len(confidence_scores),
            'low_confidence_fraction': sum(1 for s in confidence_scores if s.confidence_level == 'low') / len(confidence_scores),
    ...
    def confidence_by_depth_interval(
        ...
        for bin_idx in range(len(bins) - 1):
            ...
            intervals.append({
                ...
                'n_samples': mask.sum(),
                'needs_review_count': sum(1 for s in interval_scores if s.needs_review),
                'dominant_prediction': pd.Series([s.prediction for s in interval_scores]).mode()[0] if interval_scores else -1,
            })
    ...
    def confidence_by_facies(
        ...
        for facies_code, stats in facies_stats.items():
            ...
            rows.append({
                ...
                'facies_name': facies_names.get(facies_code, f'Facies_{facies_code}') if facies_names else f'Facies_{facies_code}',
                'count': stats['count'],
