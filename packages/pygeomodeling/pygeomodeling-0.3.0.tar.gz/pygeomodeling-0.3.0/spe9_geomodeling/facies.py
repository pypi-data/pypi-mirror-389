...
FACIES_LABELS = {
    ...
    2: {"name": "CSiS", "description": "Non-marine coarse siltstone", "adjacent": [1, 3]},
...
class FaciesClassifier:
    ...
    def active_learning_query(
        ...
        if not self.is_fitted:
            raise DataValidationError(
                "Model not fitted",
                "Train model on labeled data before active learning"
            )
    ...
    def semi_supervised_fit(
        ...
        print(f"Semi-supervised learning: Added {n_pseudo} high-confidence pseudo-labels")
    ...
    def transfer_learning_fit(
        ...
        print(f"Transfer learning complete: {len(X_source)} source + {len(X_target)} target samples")
