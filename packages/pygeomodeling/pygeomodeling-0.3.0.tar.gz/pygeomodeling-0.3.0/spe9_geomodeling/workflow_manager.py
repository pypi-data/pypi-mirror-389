...
class WorkflowManager:
    ...
    def record_well_interpreted(
        ...
        print(f"Interpreted well: {well_name} ({self.state.interpreted_wells}/{self.state.total_wells})")
        self.save_state()
    ...
    def get_training_data_with_corrections(
        ...
        print(f"Training with {len(X_original)} original + {len(corrections_df)} corrected samples")
    ...
    def complete_iteration(
        ...
        iteration = WorkflowIteration(
            ...
            corrections_received=len([c for c in self.state.corrections 
    ...
    def _get_confusion_pairs(self, corrections_df: pd.DataFrame) -> List[Tuple[int, int, int]]:
        """Get most common prediction-correction pairs."""
        pairs = corrections_df.groupby(['original_prediction', 'corrected_label']).size()
        pairs = pairs.sort_values(ascending=False).head(5)
        
        return [(int(orig), int(corr), int(count)) 
                for (orig, corr), count in pairs.items()]
...
def create_workflow_dashboard(workflow_manager: WorkflowManager) -> Dict:
    ...
    review_rate = state.reviewed_wells / state.interpreted_wells if state.interpreted_wells > 0 else 0
    correction_rate = state.total_corrections / state.interpreted_wells if state.interpreted_wells > 0 else 0
    ...
    dashboard = {
        ...
        'progress_percent': (state.interpreted_wells / state.total_wells * 100) if state.total_wells > 0 else 0,
