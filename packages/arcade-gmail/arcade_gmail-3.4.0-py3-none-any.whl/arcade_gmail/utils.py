...
def build_gmail_service(context: ToolContext) -> Any:
    """
    ...
    try:
        ...
        raise ToolExecutionError(message="Failed to build Gmail service.", developer_message=str(e))
...
def parse_draft_email(draft_email_data: dict[str, Any]) -> dict[str, str]:
    ...
    internal_date = draft_email_data.get("internalDate")

    return {
        "id": draft_email_data.get("id", ""),
        "thread_id": draft_email_data.get("threadId", ""),
        "from": headers.get("from", ""),
        "date": _format_internal_date(internal_date),
