        ...
        for key in ordered_items:
            if values.get(key):
                if isinstance(values[key], dict):
                    ...
                    self.__dict__[key].set_data(values[key])
                else:
...
class SomeIpFixedSizeArray(Generic[T]):
    ...
    def set_data(self, values: list):
        for index, _ in enumerate(self.data):
            self.data[index] = self.data[index](values[index])
...
class SomeIpDynamicSizeArray(Generic[T]):
    ...
    def serialize(self) -> bytes:
        ...
        for element in self.data:
            if hasattr(element, "_include_struct_len") and element._include_struct_len:
                result += struct.pack(">L", length_data_in_bytes)
            result += element.serialize()
        return result
    ...
    def deserialize(self, payload: bytes):
        ...
        fixed_single_element_length = self._single_element_length
        if (
            hasattr(class_reference, "_include_struct_len")
            and class_reference._include_struct_len
        ):
            fixed_single_element_length = fixed_single_element_length + 4

        number_of_elements = length / fixed_single_element_length
        for i in range(int(number_of_elements)):
            start_idx = (i * fixed_single_element_length) + self._length_field_length
    ...
    def set_data(self, values: list):
        for value in values:
            item = self._class_reference()
            if isinstance(value, dict):
                for k, v in value.items():
                    if hasattr(item, k):
                        getattr(item, k).value = v
            else:
                item.value = value
