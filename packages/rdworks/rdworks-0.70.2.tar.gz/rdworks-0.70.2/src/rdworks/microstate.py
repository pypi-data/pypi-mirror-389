...
class StateEnsemble:
    ...
    def get_macro_pKa(self, ph_values: np.ndarray, p: np.ndarray) -> list[tuple[float, float]]:
        ...
        charge = np.sum([q * p[st_indices].sum(axis=0) for q, st_indices in charge_groups.items()], axis=0)
        results = InflectionDetector(ph_values, charge).analyze(require_plateaus=True, high_confidence=True)
    ...
    def get_plot_data_pH_vs_population(self, 
    ...
    def get_plot_data_pH_vs_charge(self, ph_values: np.ndarray, p: np.ndarray) -> tuple[dict, list]:
        charge_groups = self.get_charge_groups()
        charge = np.sum([q * p[st_indices].sum(axis=0) for q, st_indices in charge_groups.items()], axis=0)
        results = InflectionDetector(ph_values, charge).analyze(require_plateaus=True, high_confidence=True)
        inflection_points = [(float(r['x']), float(r['y'])) for r in results]
...
class StateNetwork:
    ...
    def get_plot_data_pH_vs_population(self, 
                                       ph_values: np.ndarray, 
                                       p: np.ndarray, 
                                       threshold: float = 0.0) -> tuple[dict, list[int]]:
