...
class InflectionDetector:
    ...
    def _calculate_derivatives(self) -> None:
        ...
        self.dy_dx = signal.savgol_filter(self.y, window, polyorder, deriv=1,
                                         delta=np.mean(np.diff(self.x)))
        self.d2y_dx2 = signal.savgol_filter(self.y, window, polyorder, deriv=2,
                                           delta=np.mean(np.diff(self.x)))

    
    def detect_plateaus(self, derivative_threshold=None, min_plateau_length=5) -> list[dict]:
        """
            ...
            if len(zero_crossings) > 0:
                ...
                if idx_global < len(self.x) - 1:
                    ...
                    if y1 != y2:
                        ...
                        plateau_separation = abs(plateau2['mean_y'] - plateau1['mean_y'])
...
class State:
    def __init__(self, 
        ...
        self.transformation : str | None = transformation  # how this state is generated from origin
...
class StateEnsemble:
    ...
    def trim(self, ph_values: np.ndarray, C: float = ln10, beta: float = 1.0, threshold: float = 0.05) -> Self:
        ...
        Returns:
            Self: StateEnsemble
        """
        pH_dependent_populations = self.get_population(ph_values, C, beta)
    ...
    def sort(self, ph_values: np.ndarray, C: float = ln10, beta: float = 1.0) -> Self:
    ...
    def set_energies(self, energies: list[float] | np.ndarray, ref_ph: float = 7.0) -> Self:
    ...
    def get_population(self, ph_values: np.ndarray, C: float = ln10, beta: float = 1.0) -> np.ndarray:
        ...
        assert isinstance(ref.energy, float), 'reference state should have defined energy'
        assert isinstance(ref.charge, int), 'reference state should have defined charge'

        ph_values = np.array(ph_values)
        
        dG_pH = []
        for st in self.states:
            assert isinstance(st.energy, float), 'state should have defined energy'
            assert isinstance(st.charge, int), 'state should have defined charge'
            assert isinstance(st.ref_ph, float), 'state should have defined reference pH'
        ...
        assert p.shape == (self.size(), ph_values.shape[0]), "population array has wrong shape"
        
        return p
    

    def get_micro_pKa(self, ph_values: np.ndarray, p: np.ndarray) -> dict[int, list[tuple[float,float]]]:
        """
        ...
        for atom_idx, charge_groups in charge_groups_per_site.items():
            ...
            site_charge = np.sum([q * p[st_indices].sum(axis=0) for q, st_indices in charge_groups.items()], axis=0)
            results = InflectionDetector(ph_values, site_charge).analyze(require_plateaus=True, high_confidence=True)
            inflection_points[atom_idx] = [(float(r['x']), float(r['y'])) for r in results]
        
        return inflection_points    


    def get_macro_pKa(self, ph_values: np.ndarray, p: np.ndarray) -> list[tuple[float, float]]:
        """
        Compute inflection points in pH vs mean_charge that could be macroscopic pKa values.
        ...
        mean_charge = np.sum([q * p[st_indices].sum(axis=0) for q, st_indices in charge_groups.items()], axis=0)
        results = InflectionDetector(ph_values, mean_charge).analyze(require_plateaus=True, high_confidence=True)
    ...
    def get_plot_data_pH_vs_microstate_population(self, 
        ...
        Args:
            ...
            beta (float, optional): \\( \\beta = \\frac{1}{k_{B} T} \\). Defaults to 1.0.
            threshold (float) : threshold to ignore low populated microstates. Defaults to 0.01.
        ...
        for i, pH_dependent_populations in enumerate(p):
            ...
            for k, pop in enumerate(pH_dependent_populations):
                plot_data['microstate'].append(j) # for colors, 1,2,... continuous numbers
    ...
    def get_plot_data_pH_vs_site_charge(self, ph_values: np.ndarray, p: np.ndarray) -> tuple[dict, dict]:
        charge_groups_per_site = self.get_charge_groups_per_site()
        ...
        for atom_idx, charge_groups in charge_groups_per_site.items():
            ...
            for ph, q in zip(ph_values, site_charge):
                plot_data['pH'].append(ph)
                plot_data['charge'].append(q)
                plot_data['site'].append(atom_idx)

        return (plot_data, inflection_points)
    

    def get_plot_data_pH_vs_mean_charge(self, ph_values: np.ndarray, p: np.ndarray) -> tuple[dict, list]:
        charge_groups = self.get_charge_groups()
        mean_charge = np.sum([q * p[st_indices].sum(axis=0) for q, st_indices in charge_groups.items()], axis=0)
        results = InflectionDetector(ph_values, mean_charge).analyze(require_plateaus=True, high_confidence=True)
        inflection_points = [(float(r['x']), float(r['y'])) for r in results]
        plot_data = {'pH': ph_values, 'charge': mean_charge}
...
class StateNetwork:
    ...
    def get_micro_pKa(self, beta: float=1.0) -> dict[int,np.ndarray]:
        ...
        return state_ens.get_micro_pKa(beta=beta)

            
    def get_macro_pKa(self, ph_values: np.ndarray, p: np.ndarray) -> list[float]:
        ...
        """
        state_ens = StateEnsemble(self.visited_states)
        return state_ens.get_macro_pKa(ph_values=ph_values, p=p)
    ...
    def get_population(self, ph_values: np.ndarray, C: float = ln10, beta: float = 1.0) -> np.ndarray:
        ...
        """
        state_ens = StateEnsemble(self.visited_states)
        return state_ens.get_population(ph_values, C, beta=beta)
...
.            beta (float, optional): \\( \\beta = \\frac{1}{k_{B} T} \\). Defaults to 1.0.
        ...
        """
        state_ens = StateEnsemble(self.visited_states)
        return state_ens.get_plot_data_pH_vs_microstate_population(ph_values, p, threshold)
    ...
    def get_plot_data_pH_vs_mean_charge(self, ph_values: np.ndarray, p: np.ndarray) -> tuple[dict, list]:
        state_ens = StateEnsemble(self.visited_states)
        return state_ens.get_plot_data_pH_vs_mean_charge(ph_values, p)
