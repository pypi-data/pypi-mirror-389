...
class StateEnsemble:
    ...
    def trim(self, p: np.ndarray, threshold: float = 0.05) -> Self:
    ...
    def sort(self, p: np.ndarray) -> Self:
        ...
        Returns:
            Self: StateEnsemble
        """        
        _ = sorted([(max(ph_p), state_idx) for state_idx, ph_p in enumerate(p)], reverse=True)
    ...
    def set_energies(self, dG: list[float] | np.ndarray, ref_ph: float = 7.0) -> Self:
        ...
        Returns:
            Self: StateEnsemble
        """
        assert len(dG) == self.size(), "The number of energies does not match the number of states"
