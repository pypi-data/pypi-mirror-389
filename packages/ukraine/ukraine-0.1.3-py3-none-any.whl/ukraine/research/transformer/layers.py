...
class MultiheadAttention(nn.Module):
    ...
    def forward(self,
        ...
        if self.use_flash:
            ...
            if mask is not None:
                assert mask.ndim == 2, "mask should be 2D tensor."
                mask = (mask == float("-inf")).unsqueeze(0).unsqueeze(0) # (Lq, Lk) -> (1, 1, Lq, Lk)
                attn_mask_bool = mask
            ...
            if key_padding_mask is not None:
                ...
                attn_mask_bool = key_padding_mask if attn_mask_bool is None else (attn_mask_bool | key_padding_mask)
