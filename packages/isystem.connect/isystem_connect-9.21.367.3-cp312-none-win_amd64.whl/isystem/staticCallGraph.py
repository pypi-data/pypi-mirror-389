...
def _writeCallGraph(outf,
    ...
    if isShowCallsTo:
        outf.write("bottom to top direction\n")
    
    outf.write("\n")
    outf.write("' Nodes\n")

    funcUnderTestColor = 'cadetblue'
    ...
    for funcNode in callGraphNodesSet:
        ...
        outf.write(f'usecase "{funcNode}"') # quotes are needed because scope and params: ::, ...
        ...
        if funcNode == functionUnderTest:
            ...
            if isNumber:
                ...
                color = "firebrick" # invalid type, color should never be used!

        outf.write(f" #{color}\n")
        
    # if the function is not called, add it as a special case
    if not functionUnderTest in callGraphNodesSet:
        outf.write(f'usecase "{functionUnderTest}" #{funcUnderTestColor}\n')
    ...
    for caller, setOfCalled in callGraphMap.items():
        for called in setOfCalled:
            if called:
                ...
                outf.write(f'"{caller}" -down-> "{called}"\n')
...
def _main(cmdLineArgs):
    ...
    graphFileName = diagutils.createPumlGraphFileName(opts.outFileName,
                                                  'statcallg')
