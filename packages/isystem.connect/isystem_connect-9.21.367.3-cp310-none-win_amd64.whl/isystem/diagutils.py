...
def createPumlGraphFileName(imageFileName, graphType):
    """ Creates file name by removing type and adding 'graphType' as an extension. """
    imageFileName = Path(imageFileName)
    if imageFileName.suffix:
        imageFileNameWithoutType = str(imageFileName.with_suffix(""))
    else:
        raise ValueError("File name must be specified with desired type. EX: graph.svg, graph.png")
    
    return imageFileNameWithoutType + '.' + graphType
...
def createGraphImage(dotDir, graphFileName, outFileName):
    ...
    try:
        ...
        msg = f"\nERROR: Can not run dot utility."
        msg += f"\nException: {ex}"
        msg += f"\nPlease make sure that it is installed, and that input file has correct syntax."
        msg += f"\nSee help (option -h) for instructions."
        msg += f"\nYour command: {cmd}"
        raise Exception(msg)


def createPumlGraphImage(puml_fpath, graphFileName, outFileName, verbose = False):
    """
    This function runs PlantUML utility, which creates image
    file out of text description in puml format. Format of output
    file depends on outFileName extension. Extenstions "png" and
    "svg" are recommended as they are supported by puml, testIDEA
    and web browsers.

    Parameters:

    puml_fpath - directory, where plantuml.jar is located
    ...
    imgType = getImageTypeFromExtension(outFileName) # do not use print statements in libraries
    pumlJar = puml_fpath

    cmd = f'java -jar "{pumlJar}" -t{imgType} "{graphFileName}"'
    if verbose:
        print(cmd)
...
def parseArgs(cmdLineArgs, userArgDefs):
    ...
    parser.add_argument('--puml', dest="puml_fpath", default='',
                        help="Specifies path to 'plantuml.jar'.")
