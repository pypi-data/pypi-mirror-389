...
class ItemsApi(object):
    ...
    def search_items_post(self, **kwargs):
        """
        Get a collection of items
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.search_items_post(body=body_value, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param ItemSearchInputV1 body: (required)
        :return: ItemSearchPreviewPaginatedListV1
                 If the method is called asynchronously,
                 returns the request thread.
        :rtype: ItemSearchPreviewPaginatedListV1
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.search_items_post_with_http_info(**kwargs)
        else:
            (data) = self.search_items_post_with_http_info(**kwargs)
            return data

    def search_items_post_with_http_info(self, **kwargs):
        """
        Get a collection of items
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.search_items_post_with_http_info(body=body_value, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method search_items_post" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `search_items_post`")
