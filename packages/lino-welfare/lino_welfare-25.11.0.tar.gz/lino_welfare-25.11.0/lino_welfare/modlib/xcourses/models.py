...
class Course(dd.Model):
    ...
    print_candidates = DirectPrintAction(label=_("List of candidates"),
                                         tplname='candidates')
    print_participants = DirectPrintAction(label=_("List of participants"),
    ...
    def participants(self):
        ...
        """
        return ParticipantsByCourse.create_request(self).data_iterator
    ...
    def candidates(self):
        ...
        kommen.
        """
        return CandidatesByCourse.create_request(self).data_iterator
...
class RegisterCandidate(dd.ChangeStateAction):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        if not obj.course:
            return ar.error(_("Cannot register to unknown course."),
                            alert=True)
        kw = super(RegisterCandidate, self).run_from_ui(ar, **kw)
        kw.update(refresh_all=True)
        kw.update(message=_("%(person)s has been registered to %(course)s") %
                  dict(person=obj.person, course=obj.course))
        ar.success(**kw)
...
class CourseRequest(UploadController):
    ...
    course = dd.ForeignKey("xcourses.Course",
                           blank=True,
                           null=True,
                           help_text=_(
                               "The course which satisfies this request. "
    ...
    def setup_parameters(cls, fields):
        fields.update(request_state=CourseRequestStates.field(blank=True),
                      course_content=dd.ForeignKey("xcourses.CourseContent",
                                                   blank=True),
                      course_offer=dd.ForeignKey("xcourses.CourseOffer",
                                                 blank=True),
                      course_provider=dd.ForeignKey('xcourses.CourseProvider',
                                                    blank=True))
...
class PendingCourseRequests(CourseRequests):
    ...
    def setup_columns(self):
        ...
        age_slices = [(16, 24), (25, 30), (31, 40), (41, 50), (51, 60),
                      (61, None)]
