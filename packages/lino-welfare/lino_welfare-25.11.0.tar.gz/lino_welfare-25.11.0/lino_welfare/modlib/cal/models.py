...
def you_are_busy_messages(ar):
    ...
    if events.count() > 0:
        ...
        for evt in events:
            ...
            chunks.append(evt.as_summary_item(ar))
            chunks += [' (', ar.instance_action_button(evt.close_meeting), ')']
            sep = ', '
        chunks.append('. ')
        yield E.span(*chunks)
...
class Event(Event):
    ...
    def __str__(self):
        ...
        if when:
            s = "{} ({})".format(s, when)
        if self.project:
            s = _("{label} with {client}").format(label=s, client=self.project)
        return s

    def as_summary_item(self, ar, text=None, **kwargs):
        # We specify the default value for `text` explicitly here because
        # we have the client's name shown in __str__() and don't want
        # ProjectRelated.as_summary_item() to repeat it.
        if text is None:
            text = str(self)
    ...
    def when_text(self, ar):
        ...
        if self.start_time is not None:
            txt = "%s %s %s" % (txt, pgettext("(time)", "at"),
...
if False:
    ...
    class CreateClientEvent(dd.Action):
        label = _("Appointment")
        custom_handler = True
        parameters = dict(date=models.DateField(_("Date"),
                                                blank=True,
                                                null=True),
                          user=dd.ForeignKey(settings.SITE.user_model),
                          summary=models.CharField(verbose_name=_("Summary"),
                                                   blank=True))
        params_layout = """
    date user
    summary
    """
        ...
        def get_notify_subject(self, ar, obj):
            return _(
                "Created appointment for %(user)s with %(partner)s") % dict(
                    event=obj, user=obj.event.user, partner=obj.partner)
...
class Task(Task):
    ...
    delegated = models.BooleanField(_("Delegated to client"), default=False)
...
TasksByController.default_display_modes = {None: constants.DISPLAY_MODE_GRID}
TasksByProject.default_display_modes = {None: constants.DISPLAY_MODE_GRID}
