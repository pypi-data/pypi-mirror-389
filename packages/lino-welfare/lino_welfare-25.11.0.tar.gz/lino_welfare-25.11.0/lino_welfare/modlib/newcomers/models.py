...
def faculty_weight(user, client):
    ...
    if not client or not client.faculty:
        ...
        try:
            w = Competence.objects.get(faculty=client.faculty,
                                       user=user).weight
...
class AvailableCoaches(Users):
    ...
    parameters = dict(
        for_client=dd.ForeignKey('pcsw.Client',
        ...
        since=models.DateField(_("New clients since"),
                               ...
                               default=amonthago,
                               help_text=_(
                                   "New clients are those whose coaching "
    ...
    def for_client_choices(cls):
        return NewClients.create_request().data_iterator
    ...
    def get_data_rows(self, ar):
        ...
        for user in qs:
            if client:
                if client.faculty:
                    r = Competence.objects.filter(user=user,
                                                  faculty=client.faculty)
                    if r.count() == 0:
                        continue

            user.new_clients = NewClients.create_request(
                param_values=dict(coached_by=user,
        ...
        if len(data) == 0:
            if client and client.faculty:
                raise Warning(
                    _("No coaches available for %s.") % client.faculty)
        elif total_weight != 0:
            for user in data:
    ...
    def primary_clients(self, obj, ar):
        # ~ return pcsw.ClientsByCoach1.create_request(ar.ui,master_instance=obj)
        return rt.models.coachings.CoachingsByUser.create_request(master_instance=obj)

    # ~ @dd.requestfield(_("Active clients"))
    # ~ def active_clients(self,obj,ar):
    # ~ return integ.Clients.create_request(param_values=dict(coached_by=obj,only_active=True))
    ...
    @dd.virtualfield(
        models.DecimalField(_("Current workload"),
                            max_digits=8,
                            decimal_places=0,
                            help_text=u"""\
...
Summe der Belastungspunkte pro neuem Klient."""))
    ...
    @dd.virtualfield(
        models.DecimalField(_("Added workload"),
                            max_digits=8,
                            decimal_places=0,
                            help_text=u"""\
...
class AssignCoach(NotifyingAction):
    ...
    def get_notify_subject(self, ar, obj, **kw):
        ...
        if client:
            return _('%(client)s assigned to %(coach)s') % dict(client=client,
                                                                coach=obj)
    ...
    def get_notify_body(self, ar, obj, **kw):
        ...
        if client:
            ...
            return tpl % dict(client=client.obj2memo(), coach=obj, faculty=client.faculty)
...
settings.SITE.add_user_field(
    'newcomer_quota',
    models.IntegerField(_("Newcomers Quota"),
                        default=0,
                        help_text=u"""\
...
dd.inject_field(
    'pcsw.Client', 'broker',
    dd.ForeignKey('newcomers.Broker',
                  blank=True,
                  null=True,
