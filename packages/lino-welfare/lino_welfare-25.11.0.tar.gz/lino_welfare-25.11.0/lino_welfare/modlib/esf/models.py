...
class ClientSummary(Certifiable, DateSummarized, SlaveSummarized):
    ...
    children_at_charge = models.BooleanField(_("Children at charge"),
                                             default=False)
    certified_handicap = models.BooleanField(_("Certified handicap"),
                                             default=False)
    other_difficulty = models.BooleanField(_("Other difficulty"),
                                           default=False)
    result = ParticipationCertificates.field(blank=True)
    remark = models.CharField(_("Remark"), blank=True, max_length=200)
    user_modified = models.BooleanField(_("User modified"), default=False)
    ...
    def get_summary_collectors(self):
        ...
        if dd.is_installed('immersion'):
            ...
            qs = self.add_date_filter(qs, 'applies_from', client=self.master)
    ...
    def results(self, ar):
        ...
        for sf in StatisticalFields.objects():
            ...
            cells.append(
                E.td(str(sf.text), E.br(), str(v), **ar.renderer.cellattrs))
...
dd.inject_field('pcsw.Client', 'has_esf',
                models.BooleanField(_("ESF data"), default=True))
