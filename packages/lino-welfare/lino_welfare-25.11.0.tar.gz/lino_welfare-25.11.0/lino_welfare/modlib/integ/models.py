...
class ClientIsAvailable(ObservedEvent):
    ...
    def add_filter(self, qs, pv):
        ...
        if dd.is_installed('immersion'):
            flt |= has_contracts_filter('immersion_contract_set_by_client',
                                        period)
        qs = qs.exclude(flt)
        return qs
...
class UsersWithClients(dd.VirtualTable):
    ...
    def get_data_rows(self, ar):
        ...
        for user in qs.order_by('username'):
            r = Clients.create_request(param_values=dict(coached_by=user))
    ...
    def primary_clients(self, obj, ar):
        t = dd.today()
        return Clients.create_request(param_values=dict(
            only_primary=True, coached_by=obj, start_date=t, end_date=t))

    @dd.requestfield(_("Active clients"))
    def active_clients(self, obj, ar):
        # ~ return MyActiveClients.create_request(ar.ui,subst_user=obj)
        t = dd.today()
        return Clients.create_request(param_values=dict(only_active=True,
                                                      only_primary=True,
                                                      coached_by=obj,
                                                      start_date=t,
                                                      end_date=t))
...
def on_database_ready(sender, **kw):
    ...
    TODO: This requires the web server to restart every morning, otherwise the
    start_date and end_date of the pv of spawned requests have the old value for
    today. See #5834 (Kunde erscheint nicht in Liste).
        ...
        for pg in pcsw.PersonGroup.objects.exclude(
            ...
            def w(pg):
                ...
                def func(self, obj, ar):
                    ...
                    return Clients.create_request(param_values=pv)
...
class CompareRequestsTable(dd.VirtualTable):
    ...
    def get_data_rows(self, ar):
        ...
        def add(A, **kw):
            pva = dict(**kw)
            ar = A.create_request(param_values=pva)
            cells = [ar.get_title()]
        ...
        yield add(pcsw.CoachedClients, observed_event=pcsw.ClientEvents.active)
        ...
        all_contracts = isip.Contracts.create_request(
            param_values=dict(start_date=pv.start_date,
...
class PeriodicNumbers(dd.VirtualTable):
    ...
    def get_data_rows(self, ar):
        ...
        def add(A, **pva):
            ...
            ar = A.create_request(param_values=pva)
            cells = [ar.get_title()]
            ar = A.create_request(param_values=dict(
                pva, start_date=mi.start_date, end_date=mi.end_date))
            cells.append(ar)
            return cells
        ...
        # ~ ar = A.create_request(param_values=pv)
        # ~ cells.append(ar)
        # ~ return cells
        yield add(rt.models.coachings.Coachings,
                  observed_event=PeriodEvents.started,
                  coaching_type=DSBE)
        yield add(rt.models.coachings.Coachings,
                  observed_event=PeriodEvents.active,
                  coaching_type=DSBE)
        yield add(rt.models.coachings.Coachings,
                  observed_event=PeriodEvents.ended,
                  coaching_type=DSBE)
    ...
    def get_ventilated_columns(self):
        ...
        def w(user):
            ...
            def func(fld, obj, ar):
                ...
                return rt.models.coachings.Coachings.create_request(param_values=pv)

            return func

        user_types = [
            p for p in UserTypes.items() if p.has_required_roles([IntegUser])
        ]
    ...
    def get_ventilated_columns(self):
        ...
        def w(ct):
            ...
            def func(fld, obj, ar):
                ...
                return self.contracts_table.create_request(param_values=pv)
