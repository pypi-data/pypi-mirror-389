...
def objects():
    ...
    def isip_contract(**kw):
        ...
        kw.update(type=ct,
                  ...
                  applies_until=af +
                  datetime.timedelta(days=ISIP_DURATIONS.pop()))
        if not 'type' in kw:
            raise Exception("20150711, %s" % kw)
    ...
    if dd.is_installed('art61'):
        ...
        SUBS_STORIES = Cycler([[Subsidizations.hiring],
        ...
        def f(**kw):
            kw.update(type=ART61_CONTRACT_TYPES.pop(),
    ...
    for i, ctr in enumerate(isip.Contract.objects.all()):
        ...
        if i % 3:
            yield isip.ContractPartner(contract=ctr, company=COMPANIES.pop())
            if i % 4:
                yield isip.ContractPartner(contract=ctr,
                                           company=COMPANIES.pop())

        if dd.is_installed('aids'):
            # create an income Granting for each isip contract:
            kw = dict(start_date=ctr.applies_from, aid_type=INTEG_DUTIES.pop())
