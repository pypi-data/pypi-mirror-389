...
def demo_polls():
    ...
    choice_type = ContentType.objects.get_for_model(polls.Choice)
...
""")
    ...
    yield rae
    temps = polls.ChoiceSet(name="Temps de travail", choice_type=choice_type)
    yield temps
    for s in """
    temps-plein
    3/4
    ...
    def response(date_offset, isreg, partner, poll):
        ...
        for q in polls.Question.objects.filter(poll=poll):
            ...
            if cs is not None:
                ...
                i = i % choices.count()
                c = choices[i]
                yield polls.AnswerChoice(response=r, question=q, **gfk2lookup(polls.AnswerChoice.choice, c))
                i += 1
