...
class DisbursementOrdersByJournal(accounting.ByJournal, DisbursementOrders):
    insert_layout = """
    item_account
    voucher_date
    """
...
VoucherTypes.add_item_lazy(DisbursementOrdersByJournal,
                           _("Disbursement orders"))


@dd.receiver(dd.pre_analyze)
def override_field_names(sender=None, **kwargs):
    for m in rt.models_by_base(FinancialVoucher):
        dd.update_field(m, 'narration', verbose_name=_("Internal reference"))
        dd.update_field(m, 'item_remark', verbose_name=_("External reference"))
    for m in rt.models_by_base(FinancialVoucherItem):
        # dd.update_field(
        #     m, 'narration', verbose_name=_("Internal reference"))
        dd.update_field(m, 'remark', verbose_name=_("External reference"))
