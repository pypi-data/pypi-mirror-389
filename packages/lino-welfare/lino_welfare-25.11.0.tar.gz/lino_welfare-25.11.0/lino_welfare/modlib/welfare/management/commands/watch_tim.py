...
def checkcc(person, pk, nType):
    ...
    # ~ return
        ...
        raise Exception("%s : Pharmacy or Health Insurance %s doesn't exist" %
                        (dd.obj2str(person), pk))
        # ~ dblogger.warning(u"%s : Company %s doesn't exist (please create manually in Lino).",
        # ~ dd.obj2str(person),pk)
        # ~ return
    qs = rt.models.clients.ClientContact.objects.filter(client=person,
                                                        type__id=nType)
    if qs.count() == 0:
            ...
            try:
                ...
                if activity:
                    try:
                        ...
                        activity = pcsw.Activity(id=activity,
                                                 name=str(activity))
...
class Controller:
    ...
    def old_validate_and_save(self, obj):
        ...
        try:
            ...
            dblogger.warning("Validation failed for %s : %s", dd.obj2str(obj),
                             e)
    ...
    def DELETE(self, **kw):
        ...
        if msg:
            dblogger.warning("%s:%s : DELETE failed: %s", kw['alias'],
                             kw['id'], msg)
            return
        ...
        obj.delete()
        dblogger.info("%s:%s (%s) : DELETE ok", kw['alias'], kw['id'],
                      dd.obj2str(obj))
    ...
    def POST(self, **kw):
        ...
        if obj is None:
            ...
            if obj is None:
                dblogger.warning("%s:%s (%s) : ignored POST %s", kw['alias'],
                                 kw['id'], obj, kw['data'])
                return
            # ~ watcher = changes.Watcher(obj,True)
            self.set_timestamp(kw['time'])
            self.applydata(obj, kw['data'])
            dblogger.info("%s:%s (%s) : POST %s", kw['alias'], kw['id'],
                          dd.obj2str(obj), kw['data'])
    ...
    def PUT(self, **kw):
        ...
        if obj is None:
            if self.allow_put2post:
                dblogger.info("%s:%s : PUT becomes POST", kw['alias'],
                              kw['id'])
                kw['method'] = 'POST'
                return self.POST(**kw)
            else:
                dblogger.warning("%s:%s : PUT ignored (row does not exist)",
                                 kw['alias'], kw['id'])
                return
        watcher = ChangeWatcher(obj)
        ...
        self.set_timestamp(kw['time'])
        self.applydata(obj, kw['data'])
        dblogger.info("%s:%s (%s) : PUT %s", kw['alias'], kw['id'],
                      dd.obj2str(obj), kw['data'])
        self.validate_and_save(obj)
        watcher.send_update(REQUEST)
        # ~ watcher.log_diff(REQUEST)
...
class PAR(Controller):
                    ...
                    try:
                        ...
                        try:
                            ...
                            if u is not None:
                                ...
                                coaching.save()
                                dd.on_ui_created.send(sender=coaching,
                                                      request=REQUEST)
                                # ~ changes.log_create(REQUEST,coaching)
                        except Exception as e:
                            raise Exception(
                                "More than one active coaching for %r by %r" %
                                (obj, u))

                    except Exception as e:
                        raise Exception(
                            "More than one primary coaching for %r : %s" %
                            (obj, e))
    ...
    def PUT_special(self, watcher, **kw):
        ...
        if obj.__class__ != model:
            dblogger.info("%s:%s (%s) : %s becomes %s", kw['alias'], kw['id'],
                          dd.obj2str(obj), obj.__class__.__name__,
                          model.__name__)
    ...
    def swapclass(self, watcher, new_class, data):
        ...
        if old_class is Client:
            ...
            # Person
            dd.pre_remove_child.send(sender=obj,
                                     request=REQUEST,
                                     child=old_class)
        ...
        if new_class is Client:
            ...
            try:
                ...
                dd.pre_add_child.send(sender=partner,
                                      request=REQUEST,
                                      child=Person)
...
def watch(data_dir):
    ...
    if not os.path.exists(watching):
        try:
            ...
            dblogger.debug("Could not rename %s to %s: %s", infile, watching,
                           e)
            return
