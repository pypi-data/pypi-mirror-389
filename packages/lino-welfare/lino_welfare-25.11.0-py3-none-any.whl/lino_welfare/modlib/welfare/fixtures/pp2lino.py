...
def get_by_id(model, pk, offset=0, warn=True):
    ...
    try:
        ...
        if warn:
            dblogger.warning("%s %r does not exist?!", full_model_name(model),
                             pk)
        return None
...
def unused_get_or_create_job(provider, contract_type, job_type, sector,
                             function):
    try:
        #~ if provider_id:
        ...
        job = jobs.Job(provider=provider,
                       contract_type=contract_type,
                       type=job_type,
                       name=name,
                       sector=sector,
...
class PersonLoader(LinoMdbLoader):
    ...
    headers = [
        u'IDClient', u'DateArrivee', u'NumeroDossier', u'Titre', u'Nom',
        u'Prénom', u'Rue', u'Adresse', u'Numero', u'Boite',
        u'IDCommuneCodePostal', u'Tel1', u'Tel2', u'GSM1', u'GSM2', u'Email',
        u'DateNaissance', u'IDPays', u'IDNationalite', u'NumeroNational',
        u'Conjoint', u'NEnfant', u'IBIS', u'Sexe', u'Statut', u'DateFin',
        u'RISEQRIS', u'DateOctroi', u'MontantRISEQRIS', u'Qualification',
        u'Phase', u'PIIS', u'Tutorat', u'IDASISP', u'IDASSSG', u'Remarques',
        u'IDTokAns', u'RPE', u'Art 35', u'DateDebutArt35', u'DateFinArt35',
        u'ALE', u'Update', u'PermisDeTravail'
    ]
...
class RechercheProfilLoader(LinoMdbLoader):
    ...
    def row2obj(self, row):
        ...
        kw.update(provider=get_by_id(jobs.JobProvider,
                                     row['IDEndroitMiseAuTravail']))
...
class TBMiseEmploisLoader(LinoMdbLoader):
    ...
    def row2obj(self, row):
        ...
        provider = get_by_id(jobs.JobProvider, row[u'IDEndroitMiseAuTravail'],
                             OFFSET_JOBPROVIDER)
        ...
        if statut == "Fiche Candidature" or ct is None:
            if ct is not None:
                dblogger.warning("TBMiseEmplois %s : ignored contract type",
                                 row)
            ...
            dblogger.warning("Ignored TBMiseEmplois %s : unknown statut %r",
                             row, statut)
...
def objects():
    ...
    for name in (u"1er Convocation", u"Suivi", u"Rdv Manqué",
                 ...
                 u"Attestation EQRIS", u"Eval 1er", u"Eval Inter", u"Eval Fin",
                 u"Eval Inter SIAJ"):
