# -*- coding: UTF-8 -*-
# Copyright 2009-2013 Rumma & Ko Ltd
        ...
        if i is not None:
            ...
            if i is not None:
                ...
                try:
                    ...
                    dblogger.warning("Failed to save %s from %s : %s",
                                     obj2str(i), dbfrow, e)
        ...
        if is_company(row):
            ...
            if row['IDUSR']:
                ...
                if username is not None:
                    try:
                        kw.update(coach1=users.User.objects.get(
                            username=username))
                        # ~ kw.update(user=users.User.objects.get(username=username))
                    except users.User.DoesNotExist as e:
                        dblogger.warning(
                            "PAR:%s PAR->IdUsr %r (converted to %r) doesn't exist!",
                            row['IDPAR'], row['IDUSR'], username)
