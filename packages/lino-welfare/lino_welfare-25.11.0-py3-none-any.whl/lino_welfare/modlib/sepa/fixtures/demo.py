...
def objects():
    ...
    CLIENTS = Cycler(
        Client.objects.filter(client_state=ClientStates.coached)[:5])
    if len(CLIENTS) == 0:
    ...
    refs = ('832/3331/01', '832/330/01', '832/330/03F', '832/330/03',
            '832/3343/21', '832/334/27')
    ACCOUNTS = list(rt.models.accounting.Account.objects.filter(ref__in=refs))
    ...
    for i in range(months + 1):
        ...
        for acc in ACCOUNTS:
            ...
            for j in range(5):
                cli = CLIENTS.pop()
                yield PaymentOrderItem(seqno=j + 1,
                                       voucher=obj,
                                       amount=-AMOUNTS.pop(),
                                       project=cli)
