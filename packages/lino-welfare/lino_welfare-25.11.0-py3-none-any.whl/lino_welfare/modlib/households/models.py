...
class RefundsByPerson(SiblingsByPerson):
    ...
    def get_adults_and_children(cls, person, today):
        ...
        """
        children = adults = 0
        for obj in cls.create_request(master_instance=person):
...
def get_household_summary(person, today=None, adult_age=None):
    ...
    ar = SiblingsByPerson.create_request(master_instance=person)
    ...
    adults = children = 0
    for m in ar:
        if m.birth_date is not None and ((m.get_age(today) or 0) >= adult_age):
            adults += 1
        else:
            children += 1
    l = []
    if adults:
        l.append(
            ngettext("%(count)d adult", "%(count)d adults", adults) %
            dict(count=adults))
    if children:
        l.append(
            ngettext("%(count)d child", "%(count)d children", children) %
            dict(count=children))
    return _(" and ").join(l)
