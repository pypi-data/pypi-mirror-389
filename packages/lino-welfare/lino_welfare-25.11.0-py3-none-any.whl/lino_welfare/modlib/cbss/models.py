...
from .choicelists import (RequestStates, ManageActions, QueryRegisters,
                          RequestLanguages)
...
class IdentifyPersonRequest(SSDNRequest, WithPerson):
    ...
    xsd_filename = xsdpath('SSDN', 'OCMW_CPAS', 'IdentifyPerson',
                           'IdentifyPersonRequest.xsd')
...
class ManageAccessRequest(SSDNRequest, WithPerson):
    ...
    xsd_filename = xsdpath('SSDN', 'OCMW_CPAS', 'ManageAccess',
                           'ManageAccessRequest.xsd')
...
for register/unregister it is mandatory.""")
    ...
    def save(self, *args, **kw):
        if not self.sector_id:
            self.sector = settings.SITE.get_config_value('sector')
        super(ManageAccessRequest, self).save(*args, **kw)

    @dd.chooser()
    def purpose_choices(cls, sector):
        if not sector:
            sector = settings.SITE.get_config_value('sector')
        if not sector:
            raise Exception("SiteConfig.sector is not set!")
        Q = models.Q
        return Purpose.objects.filter(
            Q(sector_code=sector.code)
            | Q(sector_code__isnull=True)).order_by('code')
    ...
    def build_request(self):
        ...
        main.append(period)
        period.append(
            E('common:StartDate', ns=NSCOMMON).setText(str(self.start_date)))
        if self.end_date:
            period.append(
                E('common:EndDate', ns=NSCOMMON).setText(str(self.end_date)))
        ...
        if self.query_register:
            main.append(
                E('mar:QueryRegister').setText(self.query_register.name))
...
dd.update_field(ManageAccessRequest,
                'national_id',
                blank=False,
                help_text="""\
...
def reply_has_result(reply):
    if reply.status.value == "NO_RESULT":
        ...
        msg += '\n'.join([k + ' : ' + getattr(reply.status, k) for k in keys])
...
class RetrieveTIGroupsRequest(NewStyleRequest, SSIN):
    ...
    history = models.BooleanField(verbose_name=_("History"),
                                  default=True,
                                  help_text="Whatever this means.")
    ...
    def execute_newstyle(self, client, infoCustomer, simulate_response):
        client.add_prefix("rtg",
                          "http://kszbcss.fgov.be/types/RetrieveTIGroups/v2")
        ...
        if simulate_response is not None:
            ...
            try:
                ...
                reply = client.service.retrieveTI(infoCustomer, None,
                                                  legalContext, si)
...
def customize_system(sender, **kw):
    ...
    dd.inject_field(
        'system.SiteConfig', 'cbss_org_unit',
...
def customize_pcsw(sender, **kw):
    ...
    dd.inject_quick_add_buttons(pcsw.Client, 'cbss_identify_person',
                                IdentifyRequestsByPerson)
    dd.inject_quick_add_buttons(pcsw.Client, 'cbss_manage_access',
                                ManageAccessRequestsByPerson)
    dd.inject_quick_add_buttons(pcsw.Client, 'cbss_retrieve_ti_groups',
...
def cbss_summary(self, ar):
    ...
    for t in (IdentifyRequestsByPerson, ManageAccessRequestsByPerson,
        ...
        if n > 0:
            html += "<li>%d %s</li>" % (n,
                                        str(t.model._meta.verbose_name_plural))
    html += '</ul></p>'
    return ar.html_text(html)
    # return html


dd.inject_field(pcsw.Client, 'cbss_summary',
