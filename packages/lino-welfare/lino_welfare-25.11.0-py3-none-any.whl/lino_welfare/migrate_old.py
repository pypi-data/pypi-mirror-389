...
def migrate_from_1_1_16(globals_dict):
    ...
    def create_contacts_person(
            country_id, city_id, name, addr1, street_prefix, street, street_no,
            street_box, addr2, zip_code, region, language, email, url, phone,
            gsm, fax, remarks, first_name, last_name, title, id, is_active,
            activity_id, bank_account1, bank_account2, remarks2, gesdos_id,
            is_cpas, is_senior, group_id, coached_from, coached_until,
            coach1_id, coach2_id, sex, birth_date, birth_date_circa,
            birth_place, birth_country_id, civil_state, national_id,
            health_insurance_id, pharmacy_id, nationality_id, card_number,
            card_valid_from, card_valid_until, card_type, card_issuer,
            noble_condition, residence_type, in_belgium_since,
            unemployed_since, needs_residence_permit, needs_work_permit,
            work_permit_suspended_until, aid_type_id, income_ag, income_wg,
            income_kg, income_rente, income_misc, is_seeking,
            unavailable_until, unavailable_why, native_language_id, obstacles,
            skills, job_agents, job_office_contact_id):
        p = Person(country_id=country_id,
                   city_id=city_id,
                   name=name,
                   addr1=addr1,
                   ...
                   is_senior=is_senior,
                   group_id=group_id,
                   coached_from=coached_from,
                   coached_until=coached_until,
                   coach1_id=coach1_id,
                   coach2_id=coach2_id,
                   sex=sex,
                   birth_date=birth_date,
                   birth_date_circa=birth_date_circa,
...
def migrate_from_1_1_17(globals_dict):
    ...
    def get_or_create_job(provider_id, contract_type_id):
        try:
            return Job.objects.get(provider__id=provider_id,
                                   contract_type__id=contract_type_id)
    ...
    def create_dsbe_contract(
            id, user_id, reminder_date, reminder_text, delay_value, delay_type,
            reminder_done, must_build, person_id, company_id, contact_id,
            language, type_id, applies_from, applies_until, date_decided,
            date_issued, duration, regime, schedule, hourly_rate, refund_rate,
            reference_person, responsibilities, stages, goals, duties_asd,
            duties_dsbe, duties_company, duties_person, user_asd_id,
            exam_policy_id, ending_id, date_ended):
        ...
        obj = Contract(
            ...
            stages=stages,
            goals=goals,
            duties_asd=duties_asd,
            duties_dsbe=duties_dsbe,
            duties_company=duties_company,
            duties_person=duties_person,
            user_asd_id=user_asd_id,
            exam_policy_id=exam_policy_id,
            ending_id=ending_id,
            date_ended=date_ended)
        REMINDERS.append((obj, (reminder_date, reminder_text, delay_value,
    ...
    def create_links_link(id, user_id, reminder_date, reminder_text,
                          delay_value, delay_type, reminder_done, person_id,
                          company_id, type_id, date, url, name):
        obj = Link(
            id=id,
            user_id=user_id,
    ...
    def create_notes_note(id, user_id, reminder_date, reminder_text,
                          delay_value, delay_type, reminder_done, must_build,
                          person_id, company_id, date, type_id, event_type_id,
    ...
    def create_uploads_upload(id, user_id, owner_type_id, owner_id,
                              reminder_date, reminder_text, delay_value,
                              delay_type, reminder_done, file, mimetype,
                              created, modified, description, type_id):
        obj = Upload(
            id=id,
            user_id=user_id,
...
def migrate_from_1_2_1(globals_dict):
    ...
    def create_lino_siteconfig(id, default_build_method, site_company_id,
                               job_office_id, propgroup_skills_id,
                               propgroup_softskills_id, propgroup_obstacles_id,
                               residence_permit_upload_type_id,
                               work_permit_upload_type_id,
                               driving_licence_upload_type_id,
                               next_partner_id):
        next_partner_id = new_next_partner_id
        return SiteConfig(
            id=id,
            default_build_method=default_build_method,
            site_company_id=site_company_id,
            job_office_id=job_office_id,
            propgroup_skills_id=propgroup_skills_id,
            propgroup_softskills_id=propgroup_softskills_id,
            propgroup_obstacles_id=propgroup_obstacles_id,
            residence_permit_upload_type_id=residence_permit_upload_type_id,
            work_permit_upload_type_id=work_permit_upload_type_id,
            driving_licence_upload_type_id=driving_licence_upload_type_id,
            next_partner_id=next_partner_id)

    globals_dict.update(create_lino_siteconfig=create_lino_siteconfig)

    def create_dsbe_study(id, country_id, city_id, person_id, type_id, content,
                          started, stopped, success, language_id, school,
                          remarks):
        if school is None:
            school = ''
        if remarks is None:
    ...
    def create_users_user(id, username, first_name, last_name, email, is_staff,
                          is_expert, is_active, is_superuser, last_login,
                          date_joined):
        if email is None:
            email = ''
        return User(id=new_user_id(id),
                    username=username,
                    first_name=first_name,
                    last_name=last_name,
                    email=email,
    ...
    def create_uploads_upload(id, user_id, owner_type_id, owner_id, file,
                              mimetype, created, modified, description,
                              type_id, valid_until):
    ...
    def create_notes_note(id, user_id, must_build, person_id, company_id, date,
                          type_id, event_type_id, subject, body, language):
        if subject is None:
            subject = ''
        return Note(id=id,
                    user_id=new_user_id(user_id),
    ...
    def create_lino_textfieldtemplate(id, user_id, name, description, text):
        return TextFieldTemplate(id=id,
                                 user_id=new_user_id(user_id),
                                 name=name,
                                 description=description,
                                 text=text)
    ...
    def create_links_link(id, user_id, person_id, company_id, type_id, date,
                          url, name):
        return Link(id=id,
                    user_id=new_user_id(user_id),
                    person_id=person_id,
    ...
    def create_jobs_contract(
            id, user_id, must_build, person_id, provider_id, contact_id,
            language, job_id, type_id, applies_from, applies_until,
            date_decided, date_issued, duration, regime, schedule, hourly_rate,
            refund_rate, reference_person, responsibilities, stages, goals,
            duties_asd, duties_dsbe, duties_company, duties_person,
            user_asd_id, exam_policy_id, ending_id, date_ended):
        if regime is None:
            regime = ''
        if schedule is None:
            schedule = ''
    ...
    def create_dsbe_personsearch(id, user_id, title, aged_from, aged_to, sex,
                                 only_my_persons, coached_by_id, period_from,
                                 period_until):
        user_id = new_user_id(user_id)
        if sex is None:
            sex = ''
        ...
        return PersonSearch(id=id,
                            ...
                            aged_to=aged_to,
                            sex=sex,
                            only_my_persons=only_my_persons,
                            coached_by_id=coached_by_id,
    ...
    def create_contacts_company(country_id, city_id, name, addr1,
                                ...
                                addr2, zip_code, region, language, email, url,
                                phone, gsm, fax, remarks, vat_id, type_id, id,
                                is_active, activity_id, bank_account1,
                                bank_account2, prefix, hourly_rate):
        if email is None:
            email = ''
        if remarks is None:
            remarks = ''
        ...
        if bank_account2 is None:
            bank_account2 = ''
        return Company(country_id=country_id,
                       city_id=city_id,
                       name=name,
                       addr1=addr1,
                       street_prefix=street_prefix,
    ...
    def create_contacts_person(
            country_id, city_id, name, addr1, street_prefix, street, street_no,
            street_box, addr2, zip_code, region, language, email, url, phone,
            gsm, fax, remarks, first_name, last_name, title, sex, id,
            is_active, activity_id, bank_account1, bank_account2, remarks2,
            gesdos_id, is_cpas, is_senior, group_id, coached_from,
            coached_until, coach1_id, coach2_id, birth_date, birth_date_circa,
            birth_place, birth_country_id, civil_state, national_id,
            health_insurance_id, pharmacy_id, nationality_id, card_number,
            card_valid_from, card_valid_until, card_type, card_issuer,
            noble_condition, residence_type, in_belgium_since,
            ...
            work_permit_suspended_until, aid_type_id, income_ag, income_wg,
            income_kg, income_rente, income_misc, is_seeking,
            unavailable_until, unavailable_why, obstacles, skills, job_agents,
    ...
    def create_dsbe_wantedlanguageknowledge(id, search_id, language_id, spoken,
                                            written):
        if spoken is None:
            spoken = ''
        if written is None:
            written = ''
        return WantedLanguageKnowledge(id=id,
                                       search_id=search_id,
                                       language_id=language_id,
                                       spoken=spoken,
                                       written=written)
    ...
    def create_dsbe_languageknowledge(id, person_id, language_id, spoken,
                                      written, native, cef_level):
        if spoken is None:
            spoken = ''
        if written is None:
            written = ''
        if cef_level is None:
            cef_level = ''
        return LanguageKnowledge(id=id,
                                 person_id=person_id,
                                 language_id=language_id,
                                 spoken=spoken,
                                 written=written,
    ...
    def create_cal_task(id, user_id, created, modified, owner_type_id,
                        owner_id, person_id, company_id, start_date,
                        start_time, summary, description, access_class,
                        sequence, alarm_value, alarm_unit, dt_alarm, due_date,
                        due_time, done, percent, status, auto_type):
        owner_type_id = new_contenttype(owner_type_id)
        user_id = new_user_id(user_id)
        ...
        if person_id:
            ...
            if company_id:
                dblogger.warning(
                    "create_cal_task looses company_id %s for task #%",
                    company_id, id)
        else:
    ...
    def create_jobs_contracttype(id, build_method, template, ref, name,
                                 name_fr, name_en):
        if name.startswith('VSE'):
            obj = isip.ContractType(id=id,
    ...
    def create_jobs_job(id, name, type_id, provider_id, contract_type_id,
                        hourly_rate, capacity, remark):
        ctype = CONTRACT_TYPES.get(contract_type_id)
        if ctype.__class__ != jobs.ContractType:
            dblogger.warning("Dropping VSE Job %s" % list(
                (id, name, type_id, provider_id, contract_type_id, hourly_rate,
                 capacity, remark)))
            return None
        if remark is None:
            remark = ''
    ...
    def create_cal_event(id, user_id, created, modified, must_build, person_id,
                         company_id, start_date, start_time, summary,
                         description, access_class, sequence, alarm_value,
                         alarm_unit, dt_alarm, end_date, end_time, transparent,
                         type_id, place_id, priority, status, duration_value,
                         duration_unit, repeat_value, repeat_unit):
        user_id = new_user_id(user_id)
        return Event(id=id,
                     user_id=user_id,
                     created=created,
...
def migrate_from_1_2_2(globals_dict):
    ...
    def create_notes_notetype(id, build_method, template, name, important,
                              remark):
    ...
    def create_jobs_jobrequest(id, person_id, job_id, date_submitted,
                               contract_id, remark):
        return jobs_JobRequest(id=id,
                               person_id=person_id,
                               job_id=job_id,
...
def migrate_from_1_2_3(globals_dict):
    ...
    def create_jobs_candidature(id, person_id, job_id, date_submitted,
                                contract_id, remark):
        return jobs_Candidature(id=id,
                                person_id=person_id,
                                job_id=job_id,
                                date_submitted=date_submitted,
                                remark=remark)
...
def migrate_from_1_2_4(globals_dict):
    ...
    def create_cal_event(id, user_id, created, modified, project_id,
                         must_build, calendar_id, uid, start_date, start_time,
                         summary, description, access_class, sequence,
                         alarm_value, alarm_unit, dt_alarm, user_modified,
                         rset_id, end_date, end_time, transparent, type_id,
                         place_id, priority, status, duration_value,
                         duration_unit):
        return cal_Event(id=id,
                         user_id=user_id,
                         created=created,
                         modified=modified,
                         project_id=project_id,
    ...
    def create_cal_task(id, user_id, created, modified, owner_type_id,
                        owner_id, project_id, calendar_id, uid, start_date,
                        start_time, summary, description, access_class,
                        sequence, alarm_value, alarm_unit, dt_alarm,
                        user_modified, rset_id, due_date, due_time, done,
                        percent, status, auto_type):
        owner_type_id = new_content_type_id(owner_type_id)
        return cal_Task(id=id,
                        user_id=user_id,
                        created=created,
                        modified=modified,
                        owner_type_id=owner_type_id,
    ...
    def create_dsbe_course(id, title, content_id, provider_id, start_date,
                           remark):
        o = dsbe_CourseOffer(id=id,
                             title=title,
    ...
    def create_dsbe_courserequest(id, person_id, content_id, date_submitted,
                                  course_id, remark, date_ended, ending_id):
        return dsbe_CourseRequest(id=id,
                                  person_id=person_id,
                                  content_id=content_id,
                                  date_submitted=date_submitted,
                                  course_id=course_id,
...
def migrate_from_1_2_6(globals_dict):
    ...
    def create_contacts_person(
            contact_ptr_id, first_name, last_name, title, sex, birth_date,
            birth_date_circa, is_active, activity_id, bank_account1,
            bank_account2, remarks2, gesdos_id, is_cpas, is_senior, group_id,
            coached_from, coached_until, coach1_id, coach2_id, birth_place,
            birth_country_id, civil_state, national_id, health_insurance_id,
            pharmacy_id, nationality_id, card_number, card_valid_from,
            card_valid_until, card_type, card_issuer, noble_condition,
            residence_type, in_belgium_since, unemployed_since,
            ...
            job_office_contact_id):
        return insert_child(
            contacts_Contact.objects.get(pk=contact_ptr_id),
            contacts_Person,
            first_name=first_name,
            last_name=last_name,
            title=title,
    ...
    def create_users_user(contact_ptr_id, first_name, last_name, title, sex,
                          username, is_staff, is_expert, is_active,
                          is_superuser, last_login, date_joined, is_spis):
    ...
    def create_dsbe_personsearch(id, user_id, title, aged_from, aged_to, sex,
                                 only_my_persons, coached_by_id, period_from,
                                 period_until):
        return dsbe_PersonSearch(id=id,
                                 user_id=user_id,
                                 title=title,
                                 aged_from=aged_from,
                                 aged_to=aged_to,
    ...
    def create_cal_event(id, user_id, created, modified, project_id,
                         must_build, calendar_id, uid, start_date, start_time,
                         summary, description, access_class, sequence,
                         user_modified, rset_id, end_date, end_time,
                         transparent, type_id, place_id, priority, status,
    ...
    def create_cal_task(id, user_id, created, modified, owner_type_id,
                        owner_id, project_id, calendar_id, uid, start_date,
                        start_time, summary, description, access_class,
                        sequence, user_modified, rset_id, due_date, due_time,
                        done, percent, status, auto_type):
        owner_type_id = new_content_type_id(owner_type_id)
...
def migrate_from_1_2_7(globals_dict):
    ...
    def create_contacts_person(
            contact_ptr_id, birth_date, birth_date_circa, first_name,
            last_name, title, gender, is_active, activity_id, bank_account1,
            bank_account2, remarks2, gesdos_id, is_cpas, is_senior, group_id,
            coached_from, coached_until, coach1_id, coach2_id, birth_place,
            birth_country_id, civil_state, national_id, health_insurance_id,
            pharmacy_id, nationality_id, card_number, card_valid_from,
...
- 
    ...
    def create_jobs_contract(id, user_id, must_build, person_id, contact_id,
                             language, applies_from, applies_until,
                             date_decided, date_issued, user_asd_id,
                             exam_policy_id, ending_id, date_ended, type_id,
                             provider_id, job_id, duration, regime, schedule,
                             hourly_rate, refund_rate, reference_person,
                             responsibilities, remark):
        schedule = convert(Schedule, schedule)
        regime = convert(Regime, regime)
        return jobs_Contract(id=id,
                             user_id=user_id,
                             must_build=must_build,
                             person_id=person_id,
                             contact_id=contact_id,
...
def migrate_from_1_3_2(globals_dict):
    ...
    def create_cal_event(id, user_id, created, modified, owner_type_id,
                         owner_id, project_id, must_build, calendar_id, uid,
                         start_date, start_time, summary, description,
                         access_class_id, sequence, auto_type, user_modified,
                         rset_id, end_date, end_time, transparent, type_id,
                         place_id, priority_id, status_id, duration_value,
                         duration_unit):
        owner_type_id = new_content_type_id(owner_type_id)
        obj = cal_Event(
            id=id,
    ...
    def create_cal_guest(id, must_build, contact_id, language, event_id,
                         role_id, status_id, remark):
        obj = cal_Guest(
            id=id,
            #~ must_build=must_build,
            contact_id=contact_id,
    ...
    def create_isip_contract(id, user_id, must_build, person_id, contact_id,
                             language, applies_from, applies_until,
                             date_decided, date_issued, user_asd_id,
                             exam_policy_id, ending_id, date_ended, type_id,
                             company_id, stages, goals, duties_asd,
                             duties_dsbe, duties_company, duties_person):
        obj = isip_Contract(
            id=id,
            user_id=user_id,
            #~ must_build=must_build,
    ...
    def create_jobs_contract(id, user_id, must_build, person_id, contact_id,
                             ...
                             date_decided, date_issued, user_asd_id,
                             exam_policy_id, ending_id, date_ended, type_id,
                             provider_id, job_id, duration, regime_id,
                             schedule_id, hourly_rate, refund_rate,
                             reference_person, responsibilities, remark):
        obj = jobs_Contract(
            id=id,
            user_id=user_id,
            #~ must_build=must_build,
    ...
    def create_jobs_contractssituation(id, must_build, date, contract_type_id,
                                       job_type_id):
        obj = jobs_ContractsSituation(
            id=id,
            #~ must_build=must_build,
            date=date,
            contract_type_id=contract_type_id,
    ...
    def create_mails_mail(id, must_build, type_id, sender_id, subject, body,
                          received, sent):
        obj = mails_Mail(
            id=id,
            #~ must_build=must_build,
...
def migrate_from_1_3_3(globals_dict):
    ...
    def create_contacts_person(
            contact_ptr_id, birth_date, first_name, last_name, title, gender,
            is_active, newcomer, is_deprecated, activity_id, bank_account1,
            bank_account2, remarks2, gesdos_id, is_cpas, is_senior, group_id,
            coached_from, coached_until, coach1_id, coach2_id, birth_place,
            birth_country_id, civil_state, national_id, health_insurance_id,
            pharmacy_id, nationality_id, card_number, card_valid_from,
...
def migrate_from_1_3_7(globals_dict):
    ...
    def create_newcomers_faculty(id, name, body, body_fr, body_en, name_fr,
                                 name_en):
        return newcomers_Faculty(id=id,
                                 name=name,
                                 name_fr=name_fr,
                                 name_en=name_en)
...
def migrate_from_1_3_8(globals_dict):
    ...
    def create_cal_event(id, user_id, created, modified, owner_type_id,
                         owner_id, project_id, build_time, calendar_id, uid,
                         start_date, start_time, summary, description,
                         access_class_id, sequence, auto_type, user_modified,
                         rset_id, end_date, end_time, transparent, type_id,
                         place_id, priority_id, status_id, duration_value,
...
def migrate_from_1_4_2(globals_dict):
    ...
    def create_jobs_contract(id, user_id, build_time, person_id, contact_id,
                             language, applies_from, applies_until,
                             date_decided, date_issued, user_asd_id,
                             exam_policy_id, ending_id, date_ended, type_id,
                             provider_id, job_id, duration, regime_id,
...
def migrate_from_1_4_3(globals_dict):
    ...
    globals_dict.update(dsbe_WantedLanguageKnowledge=resolve_model(
        "pcsw.WantedLanguageKnowledge"))

    mails_Recipient = resolve_model("mails.Recipient")

    def create_mails_recipient(id, mail_id, contact_id, type, address, name):
        return mails_Recipient(id=id,
                               mail_id=mail_id,
    ...
    def create_cal_event(id, user_id, created, modified, owner_type_id,
                         ...
                         access_class_id, sequence, auto_type, user_modified,
                         rset_id, end_date, end_time, transparent, type_id,
                         place_id, priority_id, status_id):
        owner_type_id = new_content_type_id(owner_type_id)
    ...
    def create_cal_task(id, user_id, created, modified, owner_type_id,
                        owner_id, project_id, calendar_id, uid, start_date,
                        start_time, summary, description, access_class_id,
                        sequence, auto_type, user_modified, rset_id, due_date,
                        due_time, done, percent, status_id):
        owner_type_id = new_content_type_id(owner_type_id)
    ...
    def create_cal_guest(id, build_time, contact_id, language, event_id,
                         role_id, status_id, remark):
        return cal_Guest(
            id=id,
            build_time=build_time,
    ...
    def create_users_user(contact_ptr_id, first_name, last_name, title, gender,
                          username, is_staff, is_expert, is_active,
                          is_superuser, last_login, date_joined, is_spis,
                          is_newcomers, newcomer_quota):
        kw = dict()
    ...
    def create_isip_contract(id, user_id, build_time, person_id, company_id,
                             contact_id, language, applies_from, applies_until,
                             date_decided, date_issued, user_asd_id,
                             exam_policy_id, ending_id, date_ended, type_id,
                             stages, goals, duties_asd, duties_dsbe,
    ...
    def create_jobs_contract(id, user_id, build_time, person_id, company_id,
                             contact_id, language, applies_from, applies_until,
                             date_decided, date_issued, user_asd_id,
                             exam_policy_id, ending_id, date_ended, type_id,
                             job_id, duration, regime_id, schedule_id,
                             hourly_rate, refund_rate, reference_person,
                             responsibilities, remark):
    ...
    def create_bcss_identifypersonrequest(id, user_id, project_id, birth_date,
                                          first_name, last_name, title, gender,
                                          sent, status, request_xml,
                                          response_xml, national_id,
                                          middle_name, tolerance):
        if not birth_date:
            return None
        return bcss_IdentifyPersonRequest(id=id,
                                          user_id=user_id,
    ...
    def create_dsbe_courserequest(id, person_id, offer_id, content_id,
                                  date_submitted, urgent, course_id, remark,
                                  date_ended, ending_id):
    ...
    def create_contacts_company(contact_ptr_id, prefix, vat_id, type_id,
                                is_active, newcomer, is_deprecated,
                                activity_id, bank_account1, bank_account2,
                                hourly_rate):
        if prefix == 'Eheleute' and vat_id.endswith('.999'):
            type = households_Type.objects.get(pk=1)
            return create_child(contacts_Contact,
                                contact_ptr_id,
    ...
    def create_notes_notetype(id, name, build_method, template, important,
                              remark, name_fr, name_en):
        return notes_NoteType(id=id,
                              name=name,
                              build_method=build_method,
    ...
    def create_notes_note(id, user_id, build_time, person_id, company_id, date,
                          type_id, event_type_id, subject, body, language):
        return notes_Note(
            id=id,
            user_id=user_id,
    ...
    def create_cal_eventtype(id, name, build_method, template, name_fr,
                             name_en):
        return cal_Calendar(id=id,
                            name=name,
                            build_method=build_method,
                            template=template,
                            email_template='Default.eml.html',
    ...
    def create_cal_calendar(id, user_id, type, name, description, url_template,
                            username, password, readonly, is_default,
                            is_hidden, start_date, color):
        return None
        #~ return cal_Calendar(id=id,user_id=user_id,type=type,name=name,description=description,url_template=url_template,username=username,password=password,readonly=readonly,is_default=is_default,is_private=is_hidden,start_date=start_date,color=color)
...
class Migrator(Migrator):
    ...
    def migrate_from_1_4_10(self, globals_dict):
        ...
        def convert_region(region):
            ...
            if not region:
                return None
            try:
                return countries_City.objects.get(name=region,
                                                  country__id='BE')
        ...
        def create_contacts_partner(id, country_id, city_id, name, addr1,
                                    street_prefix, street, street_no,
                                    street_box, addr2, zip_code, region,
                                    language, email, url, phone, gsm, fax,
                                    remarks):
            region = convert_region(region)
            return contacts_Partner(id=id,
                                    country_id=country_id,
                                    city_id=city_id,
                                    name=name,
                                    addr1=addr1,
        ...
        def create_debts_account(id, name, seqno, group_id, type,
                                 required_for_household, required_for_person,
                                 periods, help_text, name_fr, name_en):
            if periods is not None:
                periods = Decimal(periods)
            return debts_Account(id=id,
                                 name=name,
                                 seqno=seqno,
                                 group_id=group_id,
                                 type=type,
                                 required_for_household=required_for_household,
                                 required_for_person=required_for_person,
                                 periods=periods,
                                 help_text=help_text,
                                 name_fr=name_fr,
                                 name_en=name_en)
        ...
        def create_debts_accountgroup(id, name, seqno, account_type, help_text,
                                      name_fr, name_en):
            ...
            return debts_AccountGroup(id=id,
                                      chart_id=1,
                                      name=name,
                                      ref=str(seqno),
        ...
        globals_dict.update(
            create_debts_accountgroup=create_debts_accountgroup)

        def create_contacts_company(partner_ptr_id, prefix, vat_id, type_id,
                                    is_active, newcomer, is_deprecated,
                                    activity_id, bank_account1, bank_account2,
                                    hourly_rate):
        ...
        def create_households_household(partner_ptr_id, is_active, newcomer,
                                        is_deprecated, activity_id,
                                        bank_account1, bank_account2, prefix,
                                        type_id):
            p = contacts_Partner.objects.get(pk=partner_ptr_id)
            p.is_obsolete = is_deprecated
            p.activity_id = activity_id
        ...
        def create_contacts_person(
                partner_ptr_id, birth_date, first_name, last_name, title,
                gender, is_active, newcomer, is_deprecated, activity_id,
                bank_account1, bank_account2, remarks2, gesdos_id, is_cpas,
                is_senior, group_id, coached_from, coached_until, coach1_id,
                coach2_id, birth_place, birth_country_id, civil_state,
                national_id, health_insurance_id, pharmacy_id, nationality_id,
                card_number, card_valid_from, card_valid_until, card_type,
                card_issuer, noble_condition, residence_type, in_belgium_since,
                unemployed_since, needs_residence_permit, needs_work_permit,
                work_permit_suspended_until, aid_type_id, income_ag, income_wg,
                income_kg, income_rente, income_misc, is_seeking,
                unavailable_until, unavailable_why, obstacles, skills,
                job_agents, job_office_contact_id, broker_id, faculty_id):
            p = contacts_Partner.objects.get(pk=partner_ptr_id)
            p.is_obsolete = is_deprecated
            p.activity_id = activity_id
            p.bank_account1 = bank_account1
            ...
            if national_id or gesdos_id or remarks2:
                ...
                if coach2_id and coach2_id != coach1_id:
                    ...
                    kw.update(user_id=coach2_id, type_id=ti)
                ...
                if coach1_id:
                    ...
                    if ti == 2:
                        ...
                        kw.update(start_date=SINCE_ALWAYS)
                    kw.update(user_id=coach1_id, primary=True, type_id=ti)
                    #~ if ti == 1:
                    #~ kw.update(start_date=coached_from or SINCE_ALWAYS,
        ...
        def create_cal_event(id, owner_type_id, owner_id, user_id, created,
                             modified, project_id, build_time, start_date,
                             start_time, end_date, end_time, uid, summary,
                             description, calendar_id, access_class, sequence,
                             auto_type, transparent, place_id, priority_id,
                             state):
            owner_type_id = new_content_type_id(owner_type_id)
            if not state:
        ...
        def create_cal_task(id, owner_type_id, owner_id, user_id, created,
                            modified, project_id, start_date, start_time, uid,
                            summary, description, calendar_id, access_class,
                            sequence, auto_type, due_date, due_time, percent,
                            state):
            owner_type_id = new_content_type_id(owner_type_id)
            if not state:
        ...
        def create_outbox_attachment(id, owner_type_id, owner_id, mail_id):
            owner_type_id = new_content_type_id(owner_type_id)
            return outbox_Attachment(id=id,
                                     owner_type_id=owner_type_id,
                                     owner_id=owner_id,
                                     mail_id=mail_id)
        ...
        def create_outbox_mail(id, owner_type_id, owner_id, user_id,
                               project_id, date, subject, body, sent):
            owner_type_id = new_content_type_id(owner_type_id)
            return outbox_Mail(id=id,
                               owner_type_id=owner_type_id,
                               owner_id=owner_id,
                               user_id=user_id,
                               project_id=project_id,
        ...
        def create_uploads_upload(id, owner_type_id, owner_id, user_id,
                                  created, modified, file, mimetype, type_id,
                                  valid_until, description):
            owner_type_id = new_content_type_id(owner_type_id)
            return uploads_Upload(id=id,
                                  owner_type_id=owner_type_id,
                                  owner_id=owner_id,
                                  user_id=user_id,
                                  created=created,
        ...
        def create_isip_contract(id, user_id, build_time, person_id,
                                 company_id, contact_id, language,
                                 applies_from, applies_until, date_decided,
                                 date_issued, user_asd_id, exam_policy_id,
                                 ending_id, date_ended, type_id, stages, goals,
                                 duties_asd, duties_dsbe, duties_company,
                                 duties_person):
            #~ contact = find_contact(contact_id)
            obj = isip_Contract(
                id=id,
        ...
        def create_jobs_contract(id, user_id, build_time, person_id,
                                 company_id, contact_id, language,
                                 applies_from, applies_until, date_decided,
                                 date_issued, user_asd_id, exam_policy_id,
                                 ending_id, date_ended, type_id, job_id,
                                 duration, regime_id, schedule_id, hourly_rate,
                                 refund_rate, reference_person,
                                 responsibilities, remark):
            if hourly_rate is not None:
                hourly_rate = Decimal(hourly_rate)
            obj = jobs_Contract(id=id,
                                user_id=user_id,
                                build_time=build_time,
                                client_id=person_id,
                                company_id=company_id,
        ...
        def create_properties_proptype(id, name, choicelist, default_value,
            ...
            return properties_PropType(id=id,
                                       name=name,
                                       choicelist=choicelist,
                                       default_value=default_value,
                                       limit_to_choices=limit_to_choices,
                                       multiple_choices=multiple_choices,
                                       name_fr=name_fr,
                                       name_en=name_en)

        globals_dict.update(
            create_properties_proptype=create_properties_proptype)

        def create_isip_exampolicy(id, name, project_id, start_date,
                                   start_time, end_date, end_time, uid,
                                   summary, description, every, every_unit,
                                   calendar_id, name_fr, name_en):
            return isip_ExamPolicy(id=id,
                                   name=name,
                                   start_date=start_date,
                                   start_time=start_time,
                                   end_date=end_date,
    ...
    def migrate_from_1_0_2(self, globals_dict):
        ...
        def create_countries_country(name, isocode, short_code, iso3, name_fr,
                                     name_en, nationalities, inscode):
    ...
    def migrate_from_1_0_5(self, globals_dict):
        ...
        def create_cal_event(id, owner_type_id, owner_id, user_id, created,
                             modified, project_id, build_time, start_date,
                             start_time, end_date, end_time, summary,
                             description, uid, calendar_id, access_class,
                             sequence, auto_type, transparent, place_id,
                             priority_id, state):
            owner_type_id = new_content_type_id(owner_type_id)
    ...
    def migrate_from_1_0_9(self, globals_dict):
        ...
        def create_lino_change(id, time, type, user_id, object_type_id,
                               object_id, master_type_id, master_id, diff):
            object_type_id = new_content_type_id(object_type_id)
            master_type_id = new_content_type_id(master_type_id)
            return lino_Change(id=id,
                               time=time,
                               type=type,
                               user_id=user_id,
    ...
    def migrate_from_1_0_10(self, globals_dict):
        ...
        def create_pcsw_client(
                person_ptr_id, remarks2, gesdos_id, is_cpas, is_senior,
                ...
                income_kg, income_rente, income_misc, is_seeking,
                unavailable_until, unavailable_why, obstacles, skills,
                job_agents, job_office_contact_id, client_state,
                refusal_reason, broker_id, faculty_id):
            if client_state == '60':
                client_state = '30'
            return create_child(
                contacts_Person,
    ...
    def migrate_from_1_0_17(self, globals_dict):
        ...
        def create_jobs_candidature(id, sector_id, function_id, person_id,
                                    job_id, date_submitted, remark, active):
            kw = dict()
            kw.update(id=id)
    ...
    def migrate_from_1_1_0(self, globals_dict):
        ...
        def create_cal_calendar(id, name, build_method, template,
                                attach_to_email, email_template, type,
                                description, url_template, username, password,
                                readonly, invite_team_members, start_date,
                                color):
            kw = dict()
            kw.update(id=id)
            if name is not None:
    ...
    def migrate_from_1_1_2(self, globals_dict):
        ...
        def create_debts_budget(id, user_id, build_time, date, partner_id,
                                print_todos, print_empty_rows,
                                ignore_yearly_incomes, intro, conclusion,
                                dist_amount):
            kw = dict()
            kw.update(id=id)
    ...
    def migrate_from_1_1_6(self, globals_dict):
        ...
        def create_cal_event(id, owner_type_id, owner_id, user_id, created,
                             ...
                             description, uid, calendar_id, access_class,
                             sequence, auto_type, transparent, place_id,
                             priority_id, state, assigned_to_id):
            kw = dict()
            kw.update(id=id)
            owner_type_id = new_content_type_id(owner_type_id)
        ...
        def create_isip_exampolicy(id, name, start_date, start_time, end_date,
                                   end_time, summary, description, every,
                                   every_unit, calendar_id):
            kw = dict()
            kw.update(id=id)
            if name is not None:
                kw.update(bv2kw('name', name))
    ...
    def migrate_from_1_1_7(self, globals_dict):
        ...
        def create_isip_exampolicy(id, name, start_date, start_time, end_date,
                                   end_time, every, every_unit, monday,
                                   tuesday, wednesday, thursday, friday,
                                   saturday, sunday, max_occurences,
                                   calendar_id):
            kw = dict()
            kw.update(id=id)
            if name is not None:
        ...
        def create_cal_event(id, owner_type_id, owner_id, user_id, created,
                             ...
                             start_time, end_date, end_time, summary,
                             description, uid, calendar_id, access_class,
                             sequence, auto_type, transparent, room_id,
                             priority_id, state, assigned_to_id):
        ...
        def create_cal_room(id, name, company_id, contact_person_id,
                            contact_role_id):
            kw = dict()
            kw.update(id=id)
            if name is not None:
        ...
        def create_pcsw_coaching(id, user_id, client_id, start_date, end_date,
    ...
    def migrate_from_1_1_8(self, globals_dict):
        ...
        globals_dict.update(
            countries_Language=resolve_model('languages.Language'))
    ...
    def migrate_from_1_1_10(self, globals_dict):
        ...
        def create_cal_calendar(id, name, seqno, build_method, template,
                                attach_to_email, email_template, type,
                                description, url_template, username, password,
                                readonly, is_appointment, start_date, color,
                                event_label, invite_team_members_id,
                                invite_client):
            kw = dict()
            kw.update(id=id)
            if name is not None:
        ...
        def create_users_user(id, created, modified, username, password,
                              profile, initials, first_name, last_name, email,
                              remarks, language, partner_id, access_class,
                              calendar_id, coaching_type_id,
                              coaching_supervisor, newcomer_quota):
            kw = dict()
            kw.update(id=id)
            kw.update(created=created)
            kw.update(modified=modified)
        ...
        def create_cal_task(id, owner_type_id, owner_id, user_id, created,
                            modified, project_id, start_date, start_time,
                            summary, description, uid, calendar_id,
                            access_class, sequence, auto_type, due_date,
        ...
        def create_isip_exampolicy(id, name, start_date, start_time, end_date,
                                   end_time, every_unit, every, monday,
                                   tuesday, wednesday, thursday, friday,
                                   saturday, sunday, max_events, calendar_id):
            kw = dict()
            kw.update(id=id)
            if name is not None:
                kw.update(bv2kw('name', name))
        ...
        def create_accounts_account(id, name, seqno, chart_id, group_id, ref,
                                    type, help_text, required_for_household,
        ...
        def target_name(obj):
            ...
            if bm is None:
                return None
            return bm.get_target_file(None, obj).name

        def after_load(loader):
            "Convert CVs and write migrate_from_1_1_10.py script."
            fname = os.path.join(settings.SITE.project_dir,
                                 'migrate_from_1_1_10.py')
            fd = file(fname, 'w')
            fd.write("""\
#!/usr/bin/env python
import os
...
""")
    ...
    def migrate_from_1_1_11(self, globals_dict):
        ...
        def create_uploads_upload(id, owner_type_id, owner_id, user_id,
                                  modified, created, file, mimetype, type_id,
                                  valid_until, description):
            kw = dict()
            kw.update(id=id)
            owner_type_id = new_content_type_id(owner_type_id)
        ...
        def create_notes_notetype(id, name, build_method, template,
                                  attach_to_email, email_template, important,
                                  remark, body_template):
            kw = dict()
            kw.update(id=id)
            if name is not None: kw.update(bv2kw('name', name))
            kw.update(build_method=build_method)
        ...
        def create_aids_aid(id, client_id, aid_regime, aid_type_id,
                            decided_date, decider_id, applies_from,
                            applies_until, category_id, amount):
            kw = dict()
            kw.update(id=id)
            kw.update(client_id=client_id)
        ...
        def create_cal_eventtype(id, name, seqno, build_method, template,
                                 attach_to_email, email_template, description,
                                 is_appointment, all_rooms, locks_user,
                                 start_date, event_label, invite_client):
            kw = dict()
            kw.update(id=id)
            if name is not None: kw.update(bv2kw('name', name))
            kw.update(seqno=seqno)
        ...
        def create_attestations_attestation(id, owner_type_id, owner_id,
                                            user_id, project_id, build_time,
                                            build_method, company_id,
        ...
        def create_system_textfieldtemplate(id, user_id, name, description,
                                            text):
            kw = dict()
            kw.update(id=id)
            kw.update(user_id=user_id)
        ...
        def create_contacts_partner(id, created, modified, country_id, city_id,
                                    region_id, zip_code, addr1, street_prefix,
                                    street, street_no, street_box, addr2, name,
                                    language, email, url, phone, gsm, fax,
                                    remarks, is_obsolete, activity_id):
            kw = dict()
            kw.update(id=id)
            kw.update(created=created)
            kw.update(modified=modified)
        ...
        def create_households_member(id, role_id, household_id, person_id,
                                     start_date, end_date):
            kw = dict()
            kw.update(id=id)
        ...
        globals_dict.update(create_households_member=create_households_member)
        ...
        def create_cal_guestrole(id, name, build_method, template,
                                 attach_to_email, email_template):
            kw = dict()
            kw.update(id=id)
            if name is not None: kw.update(bv2kw('name', name))
            # kw.update(build_method=build_method)
        ...
        def create_isip_contracttype(id, name, build_method, template, ref,
                                     exam_policy_id, needs_study_type):
            kw = dict()
            kw.update(id=id)
            if name is not None: kw.update(bv2kw('name', name))
            # kw.update(build_method=build_method)
        ...
        globals_dict.update(create_isip_contracttype=create_isip_contracttype)

        jobs_ContractType = resolve_model('jobs.ContractType')

        def create_jobs_contracttype(id, name, build_method, template, ref,
                                     exam_policy_id):
            kw = dict()
            kw.update(id=id)
            if name is not None: kw.update(bv2kw('name', name))
        ...
        def create_debts_budget(id, user_id, build_time, date, partner_id,
                                print_todos, print_empty_rows,
                                include_yearly_incomes, intro, conclusion,
                                dist_amount):
            kw = dict()
            kw.update(id=id)
            kw.update(user_id=user_id)
        ...
        def create_isip_contract(id, user_id, build_time, signer1_id,
                                 signer2_id, company_id, contact_person_id,
                                 contact_role_id, client_id, language,
                                 applies_from, applies_until, date_decided,
                                 date_issued, user_asd_id, exam_policy_id,
                                 ending_id, date_ended, type_id, stages, goals,
                                 duties_asd, duties_dsbe, duties_company,
                                 duties_person, study_type_id):
            ...
            if build_time:
                ...
                e = Excerpt(build_time=build_time,
                            user_id=user_id,
                            owner_type=isip_ot,
                            owner_id=id,
        ...
        def create_jobs_contract(
                id, user_id, build_time, signer1_id, signer2_id, company_id,
                contact_person_id, contact_role_id, client_id, language,
                applies_from, applies_until, date_decided, date_issued,
                user_asd_id, exam_policy_id, ending_id, date_ended, type_id,
                job_id, duration, regime_id, schedule_id, hourly_rate,
                refund_rate, reference_person, responsibilities, remark):
            kw = dict()
            kw.update(id=id)
            kw.update(user_id=user_id)
            # kw.update(build_time=build_time)
        ...
        globals_dict.update(create_pcsw_coachingtype=create_pcsw_coachingtype)
    ...
    def migrate_from_1_1_12(self, globals_dict):
        ...
        """
        globals_dict.update(aids_Decider=resolve_model("boards.Board"))
    ...
    def migrate_from_1_1_13(self, globals_dict):
        ...
        def create_isip_contract(
                id, user_id, signer1_id, signer2_id, company_id,
                contact_person_id, contact_role_id, printed_by_id, client_id,
                language, applies_from, applies_until, date_decided,
                date_issued, user_asd_id, exam_policy_id, ending_id,
                date_ended, type_id, stages, goals, duties_asd, duties_dsbe,
    ...
    def migrate_from_1_1_14(self, globals_dict):
        ...
        def create_aids_aid(id, start_date, end_date, decided_date, board_id,
                            client_id, aid_regime, aid_type_id, category_id,
                            amount):
            kw = dict()
            kw.update(id=id)
            kw.update(start_date=start_date)
    ...
    def migrate_from_1_1_15(self, globals_dict):
        ...
        def create_aids_aid(id, start_date, end_date, client_id, aid_regime,
                            aid_type_id, category_id, amount, remark):
            kw = dict()
            kw.update(id=id)
            kw.update(start_date=start_date)
            kw.update(end_date=end_date)
...
- Removed the different UploadAreas (medical, jobs...)
        ...
        def create_uploads_uploadtype(id, name, upload_area, max_number,
                                      wanted, warn_expiry_unit,
                                      warn_expiry_value):
            kw = dict()
            kw.update(id=id)
            if name is not None: kw.update(bv2kw('name', name))
        ...
        globals_dict.update(
            create_uploads_uploadtype=create_uploads_uploadtype)

        def create_uploads_upload(id, owner_type_id, owner_id, user_id,
                                  project_id, file, mimetype, company_id,
                                  contact_person_id, contact_role_id,
                                  upload_area, type_id, description,
                                  valid_from, valid_until, remark):
            kw = dict()
            kw.update(id=id)
            owner_type_id = new_content_type_id(owner_type_id)
            kw.update(owner_type_id=owner_type_id)
        ...
        def create_aids_aidtype(id, name, company_id, contact_person_id,
                                contact_role_id, aid_regime, confirmation_type,
                                long_name, short_name, board_id,
...
"""
        ...
        def create_jobs_experience(id, sector_id, function_id, person_id,
                                   started, stopped, company, title,
                                   country_id, status_id, is_training,
                                   regime_id, remarks):
            kw = dict()
            kw.update(id=id)
            kw.update(sector_id=sector_id)
        ...
        def create_jobs_study(id, country_id, city_id, zip_code, person_id,
                              started, stopped, study_regime, type_id, content,
                              success, language_id, school, remarks):
            kw = dict()
            kw.update(id=id)
            kw.update(country_id=country_id)
            kw.update(city_id=city_id)
...
valid_until to end_date.
        ...
        def create_uploads_upload(id, project_id, file, mimetype, user_id,
    ...
    def migrate_from_1_1_21(self, globals_dict):
        ...
        def create_accounts_account(id, seqno, name, chart_id, group_id, ref,
                                    type, required_for_household,
                                    required_for_person, periods,
                                    default_amount):
            kw = dict()
            kw.update(id=id)
