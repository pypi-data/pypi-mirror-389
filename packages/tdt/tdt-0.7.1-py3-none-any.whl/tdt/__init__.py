...
ALLOWED_FORMATS = [np.float32, np.int32, np.int16, np.int8, np.float64, np.int64, None, None, 'rawpacked']
ALLOWED_EVTYPES = ['all','epocs','snips','streams','scalars']

def get_files(dir, ext, ignore_mac=False):
    """
    Recursively get files with a specific extension from directory and subdirectories.

    Args:
        dir: Directory path to search
        ext: File extension to search for (e.g., '.sev')
        ignore_mac: If True, ignore Mac hidden files starting with '._'
