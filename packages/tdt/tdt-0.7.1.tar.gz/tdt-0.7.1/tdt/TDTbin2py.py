...
def time2sample(ts, fs=195312.5, t1=False, t2=False, to_time=False):
    ...
    if (sample.size == 1 and sample.ndim == 2):
        sample = sample[0][0]
...
upgrade to OpenEx v2.18 or above\n'''.format(file['name'],
            ...
            # Handle rawpacked format
            if stream_header.data_format == 'rawpacked':
                file['itemsize'] = np.uint64(np.dtype(np.int32).itemsize)
            else:
    ...
    for this_event in event_names:
        ...
        if export is None:
            if data_format == 'rawpacked':
                # Allocate separate arrays for SU_data and LFP_data for rawpacked format
                data[varname].SU_data = [[] for i in range(num_ranges)]
                data[varname].LFP_data = [[] for i in range(num_ranges)]
                for jj in range(num_ranges):
                    data[varname]['SU_data'][jj] = np.zeros((len(channels), absolute_end_sample[jj] - absolute_start_sample[jj] + np.uint64(2 + this_start_sample)), dtype=np.int16)
                    data[varname]['LFP_data'][jj] = np.zeros((len(channels), absolute_end_sample[jj] - absolute_start_sample[jj] + np.uint64(2 + this_start_sample)), dtype=np.int16)
            else:
            ...
            if export in ['csv', 'interlaced']:                
                for kk in range(start_hour_file[ii], end_hour_file[ii] + np.uint64(1)):
                    ...
                    for chan in channels:
                        ...
                        # handle gaps with memory-mapped file approach
                        if len(file_list_temp[file_num]['gaps']) > 0:
                            # Use numpy memmap to read the file
                            path = file_list_temp[file_num]['fullname']
                            n = file_list_temp[file_num]['npts']

                            # Determine dtype for memmap
                            if data_format == 'rawpacked':
                                map_dtype = np.int32
                            else:
                                map_dtype = data_format

                            # Memory map the file (skip 40-byte header)
                            mapped_data = np.memmap(path, dtype=map_dtype, mode='r', offset=int(40), shape=(int(n),))

                            # Get gap information
                            ggg = file_list_temp[file_num]['gaps']  # shape: (n_gaps, 2)

                            # Matlab: ggg(:)' flattens column-wise then transposes
                            # In numpy with row-major order, transpose first then flatten
                            ggg_flat = ggg.T.flatten()  # [gap1_start, gap2_start, gap1_end, gap2_end, ...]

                            # Calculate data indices
                            # Matlab: diff([1 ggg(:)'-file_list_temp(file_num).start_sample])+1
                            start_sample = file_list_temp[file_num]['start_sample']
                            temp_diff = np.diff(np.concatenate([[1], ggg_flat - start_sample])) + 1
                            # Take every other element starting from index 0 (the data segments, not the gaps)
                            data_ind = np.cumsum(np.concatenate([[1], temp_diff[0::2]]))
                            data_ind[0] = 0
                            data_ind = data_ind.astype(np.int64)

                            # Calculate gap sizes
                            # Matlab diff(ggg) on 2D array computes differences along first non-singleton dimension
                            # For shape (n_gaps, 2), this is along the columns (within each row)
                            gap_pts = np.diff(ggg, axis=1).flatten() - 1

                            # Create pieces array following Matlab logic exactly
                            num_gaps = len(gap_pts)
                            pieces = [None] * (num_gaps * 2 + 1)

                            # First piece of data
                            first_end = int(ggg[0, 0])
                            pieces[0] = mapped_data[0:first_end]

                            # Add zero gaps
                            for mm in range(num_gaps):
                                pieces[mm*2 + 1] = np.zeros(int(gap_pts[mm]), dtype=map_dtype)

                            # Add data segments (skip mm=0 since pieces[0] is already filled)
                            for mm in range(1, len(data_ind) - 1):
                                seg_start = int(data_ind[mm])
                                seg_end = int(data_ind[mm+1])
                                piece_idx = mm * 2
                                pieces[piece_idx] = mapped_data[seg_start:seg_end]

                            # Add final data piece
                            final_start = int(data_ind[-1])
                            final_piece_idx = (len(data_ind) - 1) * 2
                            pieces[final_piece_idx] = mapped_data[final_start:]

                            # Concatenate all pieces
                            ddd = np.concatenate(pieces)

                            # Extract requested sample range
                            ...
                            ddd = ddd[int(firstSample):int(lastSample)]

                        else:
                            # No gaps - use regular file reading
                            # open file
                            with open(file_list_temp[file_num]['fullname'], 'rb') as f:
                                ...
                                if lastSample == MAX_UINT64:
                                    if data_format == 'rawpacked':
                                        ddd = np.frombuffer(f.read(), dtype=np.int32)
                                    else:
                                        ddd = np.frombuffer(f.read(), dtype=data_format)
                                else:
                                    if data_format == 'rawpacked':
                                        ddd = np.frombuffer(f.read(int((lastSample - firstSample)*file_list_temp[file_num]['itemsize'])), dtype=np.int32)
                                    else:
                                        ddd = np.frombuffer(f.read(int((lastSample - firstSample)*file_list_temp[file_num]['itemsize'])), dtype=data_format)

                            if temp_data is None:
                                if data_format == 'rawpacked':
                                    temp_data = np.zeros((len(channels), len(ddd)), dtype=np.int32)
                                else:
                ...
                for chan in channels:
                    ...
                    for kk in range(start_hour_file[ii], end_hour_file[ii] + np.uint64(1)):
                        ...
                        if len(file_list_temp[file_num]['gaps']) > 0:
                            ...
                            # Matlab: ggg(:)' flattens column-wise (down each column) then transposes to row
                            # For a matrix [[a,b],[c,d]], ggg(:)' gives [a,c,b,d]
                            ggg_flat = ggg.flatten('F')  # Fortran-order (column-major) flattening

                            # Calculate data indices
                            # Matlab: diff([1 ggg(:)'-file_list_temp(file_num).start_sample])+1
                            temp_array = np.concatenate([[1], ggg_flat - start_sample])
                            temp_diff = np.diff(temp_array) + 1
                            # Take every other element starting from index 0 (the data segments)
                            data_ind = np.cumsum(np.concatenate([[1], temp_diff[0::2]]))
                            data_ind[0] = 0
                            data_ind = data_ind.astype(np.int64)
                            ...
                            # Create pieces array following Matlab logic exactly
                            # Total pieces = num_gaps * 2 + 1 (alternating data and gaps, with data at start and end)
                            num_gaps = len(gap_pts)
                            pieces = [None] * (num_gaps * 2 + 1)
                            ...
                            # Add zero gaps (in even indices: 2, 4, 6, ...)
                            # Matlab: for mm = 1:length(gap_pts), pieces{mm*2} = zeros(...)
                            for mm in range(num_gaps):
                                pieces[mm*2 + 1] = np.zeros(int(gap_pts[mm]), dtype=map_dtype)

                            # Add data segments (in odd indices after 0: 2, 4, 6, ...)
                            # Matlab: for mm = 1:length(data_ind)-1, pieces{mm*2-1} = data(...)
                            # When mm=1: pieces{1} is already filled (the first data segment)
                            # When mm=2: pieces{3} gets filled (index 2 in Python)
                            # So we start from mm=2 in Matlab (mm=1 in Python, since 0-indexed)
                            for mm in range(1, len(data_ind) - 1):
                                # Matlab index: mm*2-1 where mm starts at 2
                                # mm=2: pieces{3} (Python index 2)
                                # mm=3: pieces{5} (Python index 4)
                                seg_start = int(data_ind[mm])
                                seg_end = int(data_ind[mm+1])
                                piece_idx = mm * 2
                                pieces[piece_idx] = mapped_data[seg_start:seg_end]
                            ...
                            # mm is the last value from the loop above
                            # After the loop, mm = length(data_ind)-1 (in Matlab terms)
                            # Final piece index: mm*2+1
                            final_start = int(data_ind[-1])
                            final_piece_idx = (len(data_ind) - 1) * 2
                            pieces[final_piece_idx] = mapped_data[final_start:]
                            ...
                            with open(file_list_temp[file_num]['fullname'], 'rb') as f:
                                ...
                                if lastSample == MAX_UINT64:
                                    ...
                                    if data_format == 'rawpacked':
                                        ddd = np.frombuffer(f.read(int((lastSample - firstSample+1)*file_list_temp[file_num]['itemsize'])), dtype=np.int32)
                                    else:
                                        ddd = np.frombuffer(f.read(int((lastSample - firstSample+1)*file_list_temp[file_num]['itemsize'])), dtype=data_format)

                        # Write data to output array (handles both gap-filled and regular data)
                        if export in ['binary']:
                            if scale != 1:
                            ...
                            if data_format == 'rawpacked':
                                # Split int32 data into SU_data (upper 16 bits) and LFP_data (lower 16 bits)
                                data[varname].SU_data[ii][arr_index, int(chan_index):(int(chan_index + len(ddd)))] = np.int16(ddd // 65536)
                                data[varname].LFP_data[ii][arr_index, int(chan_index):(int(chan_index + len(ddd)))] = np.int16(ddd % 65536)
                            else:
        ...
        if export is None:
            if data_format == 'rawpacked':
                if num_ranges == 1:
                    data[varname].SU_data = data[varname].SU_data[ii]
                    data[varname].LFP_data = data[varname].LFP_data[ii]
            else:
                if num_ranges == 1:
        ...
        for ii in range(len(epocs.name)):
            if epocs.type[ii] == 'offset':
                ...
                if var_name not in header.stores.keys():
                    warnings.warn(epocs.buddies[ii] + ' buddy epoc not found, skipping', Warning, stacklevel=2)
                    continue
        ...
        if current_type_str == 'epocs':
            ...
            if nchan > 1:
                ...
                if min_length != max_length:
                    warnings.warn('Truncating store {0} to {1} values (from {2})'.format(current_name, min_length, max_length), Warning, stacklevel=2)
            ...
            if current_name in sev_names:
                ...
                if d is not None: # exporter returns None
                    ...
                    if expected_fs > 0 and (np.abs(expected_fs - detected_fs) > 1):
                        warnings.warn('Detected fs in SEV files was {0} Hz, expected {1} Hz. Using {2} Hz.'.format(detected_fs, expected_fs, expected_fs), Warning, stacklevel=2)
                ...
                if data[current_type_str][current_name].ucf == 1:
                    warnings.warn('Expecting SEV files for {0} but none were found, skipping...'.format(current_name), Warning, stacklevel=2)
                    continue
                    ...
                    for f in range(len(arr)):
                        ...
                        for kk in range(len(relative_offsets)):
                            if nchan > 1:
                                ...
                                arr_index = 0
                            channel_offset += 1
                            if np.any(ind[kk,:] > len(tev_data)):
                                if not found_empty:
                                    warnings.warn(f"Data missing from tev file for store:{current_name} channel:{arr_index+1} time:{t1 + chan_index[arr_index] / current_freq:.2f}", Warning, stacklevel=2)
                                    found_empty = True
                            ...
                            if export in ['csv', 'interlaced', 'binary']:
                                ...
                                if ready_for_export[arr_index] == True:
                                    warnings.warn('This channel already ready? tev file out of order?', Warning, stacklevel=2)
...
if __name__ == '__main__':
    ...
    start = time.time()
    data = read_block(BLOCK_PATH, channel=1)
