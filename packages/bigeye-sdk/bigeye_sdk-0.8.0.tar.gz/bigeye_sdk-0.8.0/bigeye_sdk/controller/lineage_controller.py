...
class LineageController:
    ...
    def process_all_edge_requests(self, purge_lineage: bool = False, skip_existing_check: bool = False):
        ...
        with typer.progressbar(length=len(self.edge_requests), label=loading_text) as progress:
            ...
            for r in self.edge_requests:
                try:
                    if purge_lineage:
                        ...
                        # Skip existing check if flag is set - just attempt to create all edges
                        ...
                        # Only check existing relations if skip_existing_check is False
                        if not skip_existing_check:
                            self._get_existing_relations_for_nodes([task.data_node_id, r.upstream.data_node_id])
                        source_to_etl_deps = self.existing_relations.get(r.upstream.data_node_id, None) if not skip_existing_check else None
                        etl_downstream_deps = self.existing_relations.get(task.data_node_id, None) if not skip_existing_check else None
            ...
            for r in final_edge_requests:
                if r.upstream_data_node_id != r.downstream_data_node_id \
                    and r.upstream_data_node_id is not None and r.upstream_data_node_id != 0 \
                    and r.downstream_data_node_id is not None and r.downstream_data_node_id != 0:
                    cleansed_requests.append(r)

            # batch the requests into chunks of 500
            for i in range(0, len(cleansed_requests), 500):
                self.client.bulk_create_lineage_edges(cleansed_requests[i:i+500])
            progress.update(1)
    ...
    def infer_relations_from_database_tables(self,
        ...
        if r.table_overrides is not None:
            for t_override in r.table_overrides:
                try:
                    ...
                    # Skip expensive downstream graph calls if flag is set
                    if not skip_existing_check:
        ...
        if match_by_name:
            # match column level relations by name and then process all requests
    ...
    def infer_column_level_lineage_from_file(
        ...
        for r in lineage_configuration_file.relations:
            if not r.has_custom:
                self.infer_relations_from_database_tables(r, match_by_name=match_by_name, skip_existing_check=skip_existing_check)
            else:
                ...
                # Skip this if skip_existing_check is enabled
                if not skip_existing_check:
                    # Get all the relationships for tables and store in index
                    self._get_existing_relations_for_nodes(table_node_ids)

        self.process_all_edge_requests(purge_lineage=purge_lineage, skip_existing_check=skip_existing_check)
