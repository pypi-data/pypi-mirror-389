...
information for the Teradata sessions established via langchain-teradata.
"""  
from functools import wraps
from teradatasqlalchemy.telemetry.queryband import (
    QueryBand, 
    collect_queryband as tdsqlalchemy_collect_queryband
)

# Create a global variable to manage querybands for langchain-teradata 
# package.
...
def collect_queryband(*qb_deco_pos_args, **qb_deco_kwargs):
    """
    DESCRIPTION:
        Decorator for calling collect_queryband decorator in telemetry 
        utility in teradatasqlalchemy using session_queryband object and 
        other positional and keyword arguments expected by collect_queryband.

    PARAMETERS:
        qb_deco_pos_args:
            Optional Argument.
            Specifies the positional arguments accepted by 
            collect_queryband decorator in telemetry utility in 
            teradatasqlalchemy.
    ...
    EXAMPLES:
        >>> from langchain_teradata.telemetry_utils.queryband import \
        ...     collect_queryband
    ...
    def outer_wrapper(func):
        ...
        def inner_wrapper(*func_args, **func_kwargs):
            # Pass the required argument 'session_queryband' along with 
            # other expected arguments to collect_queryband() decorator which 
            # is imported as tdsqlalchemy_collect_queryband.
            return tdsqlalchemy_collect_queryband(
                session_queryband, *qb_deco_pos_args, **qb_deco_kwargs
