...
class LocalCodeExecutorTool(CodeExecutorTool):
    ...
    def execute(self, python_script: str, export_files: Optional[List[str]] = None):
        """
        Execute Python script in local mode.

        Args:
            python_script: Python code to execute
            export_files: Optional list of file paths to export after execution

        Returns:
            Execution result as string
        """
        # Delegate to parent's execute method with code parameter
        return super().execute(code=python_script, export_files=export_files)
