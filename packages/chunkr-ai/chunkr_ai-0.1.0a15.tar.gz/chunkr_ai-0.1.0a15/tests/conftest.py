...
from chunkr_ai import Chunkr, AsyncChunkr, DefaultAioHttpClient
from chunkr_ai._utils import is_dict
...
logging.getLogger("chunkr_ai").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Chunkr]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Chunkr(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncChunkr]:
