...
from tiledb.client.array import ArrayList
from tiledb.client.array import apply as array_apply
from tiledb.client.array import exec_multi_array_udf
from tiledb.client.dag.dag import DAG
from tiledb.client.dag.dag import Mode
from tiledb.client.dag.dag import Node
from tiledb.client.sql import exec as sql_exec
from tiledb.client.udf import exec as udf_exec
...
class DelayedBase(Node):
    ...
    def compute(
        ...
        :param workspace: The workspace to execute tasks under, if different
        ...
        if self.dag is None:
            ...
            self.__set_all_parent_nodes_same_dag(
                DAG(workspace=workspace, name=name, mode=mode, max_workers=max_workers)
            )
        else:
            if workspace is not None:
                self.dag.workspace = workspace
            if name is not None:
                self.dag.name = name
...
class Delayed(DelayedBase):
    ...
    Extends [DelayedBase](#tiledb.client.compute.delayed.DelayedBase)
    """
...
class DelayedSQL(DelayedBase):
    ...
    [Delayed API](#tiledb.client.compute.delayed.Delayed)
