...
class DatabasePool:
                ...
                if return_ids and operation == 'insert':
                    for value in values:
                        try:
                            ...
                            # self.logger.info(f"{operation} success【{table}】-> {ids}")
                    ...
                    if values:  # 判断values是否有数据（非空）
                        ...
                        # self.logger.info(f"{operation} executed without values【{table}】")
                        conn.commit()
        except cursor.IntegrityError as err:
            pass
        except Exception as e:
    ...
    def query_table_data_value_count(self, id, table, search_value):
