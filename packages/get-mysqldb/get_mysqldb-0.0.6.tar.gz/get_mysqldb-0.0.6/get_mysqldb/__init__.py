...
class DatabasePool:
    def __init__(self, DB_HOST, DB_PORT, DB_DATABASE, DB_USER, DB_PASSWORD):
        self.pool = PooledDB(creator=pymysql, maxconnections=8, mincached=2, maxcached=5, maxshared=3, blocking=True,
    ...
    def insert_exception(self, table, data_dict: dict = None, data_list=None):
        data_dicts = self._prepare_data_list(data_dict, data_list)
        keys = data_dicts[0].keys()
        values = [[item[key] for key in keys] for item in data_dicts]
        placeholders = ', '.join(['%s'] * len(keys))
