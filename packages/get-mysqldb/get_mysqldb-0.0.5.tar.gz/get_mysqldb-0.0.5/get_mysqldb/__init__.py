...
class DatabasePool:
    def __init__(self, DB_HOST, DB_PORT, DB_DATABASE, DB_USER, DB_PASSWORD):
        self.pool = PooledDB(creator=pymysql,maxconnections=8,mincached=2,maxcached=5,maxshared=3,blocking=True,host=DB_HOST,port=DB_PORT,database=DB_DATABASE,user=DB_USER,password=DB_PASSWORD,charset='utf8mb4')
        self.lock = Lock()
    ...
    def to_dict(data):
        if isinstance(data, dict):
            ...
            return {column.name: getattr(data, column.name) for column in data.__table__.columns if getattr(data, column.name) is not None}
        else:
    ...
    def execute_many_or_loop(self, table, sql, values, return_ids=False, operation='insert', raise_exception=False):
        """
        执行批量操作或循环操作以返回ID。
        :param table: 表名。
        :param sql: SQL语句模板。
                ...
                if return_ids and operation == 'insert':
                    for value in values:
                        try:
                            ...
                            # logger.info(f"{operation} success【{table}】-> {ids} -> {value}")
                            # logger.info(f"{operation} success【{table}】-> {ids}")
                        except cursor.IntegrityError as err:
                            if raise_exception:  # 单个数据错误也透传（可根据需求调整）
                                raise
                            else:
                                logger.warning(f"{operation} fail【{table} -> {value} 数据存在！")
                        except Exception as e:
                            conn.rollback()
                            logger.error(f"{operation} failed: {e} for {value}")
                    ...
                    try:
                        ...
                        logger.error(f"{operation} failed: {e} for {values}")
                else:
                    if values:  # 判断values是否有数据（非空）
                        cursor.executemany(sql, values)
                        # logger.info(f"{operation} success【{table}】-> {values[0]} ")
                        conn.commit()
                    else:
                        cursor.execute(sql)  # 无数据时直接执行SQL
                        # logger.info(f"{operation} executed without values【{table}】")
                        conn.commit()
        except cursor.IntegrityError as err:
            pass
        except Exception as e:
    ...
    def _get_ids_for_update(self, table, condition):
        ...
        try:
            ...
            logger.error("Failed to get IDs for update: %s", e)
        finally:
            conn.close()
            return ids

    def insert(self, table, data_dict: dict = None, data_list=None, return_ids: bool = False, raise_exception=False):
        """
            插入mysql数据库。
            :param table: 表名。
        ...
        try:
            ...
            ids = self.execute_many_or_loop(table, sql, values, return_ids, 'insert', raise_exception)
        except Exception as E:
    ...
    def update(self, table, condition, data_dict=None, data_list=None, return_ids=False):
        ...
        if return_ids:
            ...
            if not ids:
                logger.warning(f"update 【{table}】失败，未找到需要更新的数据, 更新key -> {condition}")
                return
        try:
            data_dicts = self._prepare_data_list(data_dict, data_list)
    ...
    def query_value_count(self, id, table, search_value):
