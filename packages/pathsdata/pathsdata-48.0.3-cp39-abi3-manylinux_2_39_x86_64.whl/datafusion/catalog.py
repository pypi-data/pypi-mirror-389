...
# Import both factory and actual catalog from internal module
_IcebergCatalogProviderFactory = df_internal.catalog.IcebergCatalogProvider

class IcebergCatalogProvider:
    """Iceberg Catalog Provider factory."""
    
    @staticmethod
    def create_iceberg_session_context():
        """Create a SessionContext pre-configured with IcebergQueryPlanner.
        
        This method creates a SessionContext that can handle Iceberg DDL operations
        without getting "No installed planner" errors.
        
        Returns:
            SessionContext: A SessionContext configured with IcebergQueryPlanner
        """
        # Import here to avoid circular imports
        from datafusion.context import SessionContext
        
        # Create the raw internal context with IcebergQueryPlanner
        internal_ctx = _IcebergCatalogProviderFactory.create_iceberg_session_context()
        
        # Create a proper SessionContext wrapper
        ctx = SessionContext.__new__(SessionContext)
        ctx.ctx = internal_ctx
        
        return ctx
    
    def __new__(cls, warehouse_name: str, region: str = "us-east-1"):
        """Create a new IcebergCatalog instance."""
        return _IcebergCatalogProviderFactory.create_catalog(warehouse_name, region)
    
    @staticmethod
    def from_glue_catalog(warehouse_name: str, region: str = "us-east-1", branch: str | None = None):
        """Create an IcebergCatalog from AWS Glue catalog."""
        return _IcebergCatalogProviderFactory.from_glue_catalog(warehouse_name, region, branch)
    
    @staticmethod
    def from_file_catalog(path: str, branch: str | None = None):
        """Create an IcebergCatalog from file catalog."""
        return _IcebergCatalogProviderFactory.from_file_catalog(path, branch)
    
    @staticmethod  
    def from_rest_catalog(endpoint: str, access_token: str | None = None, branch: str | None = None):
        """Create an IcebergCatalog from REST catalog."""
        return _IcebergCatalogProviderFactory.from_rest_catalog(endpoint, access_token, branch)
