...
impl PySessionContext {
    ...
    pub fn register_catalog_provider(
        ...
        let provider = if provider.hasattr("__iceberg_catalog__")? {
            // Special handling for IcebergCatalog to avoid FFI wrapping
            let iceberg_capsule = provider.call_method0("__iceberg_catalog__")?;
            let capsule = iceberg_capsule.downcast::<PyCapsule>().map_err(py_datafusion_err)?;
            validate_pycapsule(capsule, "iceberg_catalog")?;
            let catalog = unsafe { capsule.reference::<Arc<IcebergCatalog>>() };
            catalog.clone() as Arc<dyn CatalogProvider>
        } else if provider.hasattr("__datafusion_catalog_provider__")? {
