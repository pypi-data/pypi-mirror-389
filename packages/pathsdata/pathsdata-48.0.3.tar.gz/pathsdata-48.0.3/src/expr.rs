...
impl PyExpr {
    ...
    fn to_variant<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
        Python::with_gil(|_| {
            ...
            Expr::Literal(value, metadata ) => {
                let metadata_map = metadata.as_ref().map(|m| m.inner().clone());
                Ok(PyLiteral::new_with_metadata(value.clone(), metadata_map).into_bound_py_any(py)?)
            },
    ...
    pub fn literal_with_metadata(
        ...
        lit_with_metadata(value.0, Some(FieldMetadata::try_from(metadata).unwrap())).into()
    }
    ...
    pub fn alias(&self, name: &str, metadata: Option<HashMap<String, String>>) -> PyExpr {
        self.expr.clone().alias_with_metadata(name, Some(FieldMetadata::try_from(metadata.unwrap()).unwrap())).into()
    }
