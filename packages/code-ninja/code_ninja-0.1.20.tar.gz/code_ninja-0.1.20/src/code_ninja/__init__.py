...
def _auto_patch_function(function, corrected_code: str):
  
    src_file = inspect.getsourcefile(function)
    src_lines, start_line = inspect.getsourcelines(function)
    end_line = start_line + len(src_lines)

    with open(src_file, "r", encoding="utf-8") as f:
        original_code = f.readlines()

    original_code[start_line - 1:end_line - 1] = [corrected_code + "\n"]

    with open(src_file, "w", encoding="utf-8") as f:
        f.writelines(original_code)

    console.print(
        Panel(Text("Function automatically corrected and saved in your code file.",
                   style="bold green"), title="[bold yellow]Auto Patch", border_style="yellow")
    )
...
def catch(function):
    def wrapper(*args, **kwargs):
        try:
            ...
            _auto_patch_function(function, ai_data.corrected_code)
    return wrapper
