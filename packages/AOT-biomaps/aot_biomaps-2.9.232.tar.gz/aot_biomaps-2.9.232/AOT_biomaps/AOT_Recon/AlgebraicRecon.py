...
class AlgebraicRecon(Recon):
    ...
    """
    def __init__(self, opti = OptimizerType.MLEM, numIterations = 10000, numSubsets = 1, isSavingEachIteration=True, maxSaves = 5000, alpha = None, denominatorThreshold = 1e-6, device = None, **kwargs):
    ...
    def _sparseSMatrix(AcousticFields, threshold_factor=0.01, show_logs=True):
        # 1. Sparsifier chaque champ et stocker les indices/valeurs
        sparse_fields = []
        for ac_field in AcousticFields:
            field = ac_field.field
            threshold = threshold_factor * np.max(field)
            sparse_mask = field > threshold
            sparse_indices = np.nonzero(sparse_mask)
            sparse_values = field[sparse_mask]
            sparse_fields.append((sparse_indices, sparse_values, field.shape))

        # 2. Construire directement le tenseur creux PyTorch 4D
        all_indices = []
        all_values = []
        total_nnz = sum(len(values) for _, values, _ in sparse_fields)
        n_fields = len(AcousticFields)
        field_offsets = np.cumsum([0] + [len(values) for _, values, _ in sparse_fields])

        coo_indices = np.zeros((total_nnz, 4), dtype=np.int64)
        coo_values = np.zeros(total_nnz, dtype=np.float32)

        for i, (indices, values, _) in enumerate(sparse_fields):
            coo_indices[field_offsets[i]:field_offsets[i+1], :3] = np.column_stack(indices)
            coo_indices[field_offsets[i]:field_offsets[i+1], 3] = i  # Index du champ
            coo_values[field_offsets[i]:field_offsets[i+1]] = values

        coo_indices = torch.LongTensor(coo_indices).t().contiguous()
        coo_values = torch.FloatTensor(coo_values)
        shape_4d = (AcousticFields[0].field.shape[0], AcousticFields[0].field.shape[1], AcousticFields[0].field.shape[2], n_fields)
        A_sparse = torch.sparse_coo_tensor(
            coo_indices,
            coo_values,
            size=shape_4d,
            device=f"cuda:{config.select_best_gpu()}"
        )

        # 3. Calculer la taille mémoire (pour comparaison)
        dense_A = torch.zeros(shape_4d, dtype=torch.float32, device=A_sparse.device)
        taille_A_Mo = dense_A.numel() * dense_A.element_size() / 1024**2
        taille_A_sparse_Mo = A_sparse._values().numel() * A_sparse._values().element_size() / 1024**2

        if show_logs:
            print(f"Size of dense A matrix: {taille_A_Mo:.2f} Mo")
            print(f"Size of sparse S matrix: {taille_A_sparse_Mo:.2f} Mo")
            reduction = (1 - taille_A_sparse_Mo / taille_A_Mo) * 100
            print(f"Size reduction: {reduction:.2f} %")


    def _AlgebraicReconPython(self,show_logs, withTumor):
    ...
    def _AlgebraicReconCASToR(self, show_logs, withTumor):
        # Définir les chemins
