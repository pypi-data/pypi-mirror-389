...
class AlgebraicRecon(Recon):
    ...
    def _sparseSMatrix(self, threshold_factor=0.01, show_logs=True):
        sparse_fields = []
        for ac_field in self.experiment.AcousticFields:
        ...
        stacked_fields = []
        for indices, values, shape in sparse_fields:
            field_sparse = np.zeros(shape)
            field_sparse[indices] = values
            stacked_fields.append(field_sparse)

        self.SMatrix = np.stack(stacked_fields, axis=-1)  

        A = torch.tensor(self.SMatrix, dtype=torch.float32).to(device=f"cuda:{config.select_best_gpu()}")
        A_reshaped = A.reshape(-1, A.shape[-1])
        mask = A_reshaped > (threshold_factor * A.max())
        indices = mask.nonzero()
        values = A_reshaped[mask]
        A_sparse = torch.sparse_coo_tensor(
            indices.t(),
            values,
            size=A_reshaped.shape,
            device=f"cuda:{config.select_best_gpu()}"
        )

        # 6. Affichage des tailles
        taille_A_Mo = A.numel() * A.element_size() / 1024**2
        taille_A_sparse_Mo = A_sparse._values().numel() * A_sparse._values().element_size() / 1024**2
        if show_logs:
            # in English
            print(f"Size of A matrix: {taille_A_Mo:.2f} Mo")
