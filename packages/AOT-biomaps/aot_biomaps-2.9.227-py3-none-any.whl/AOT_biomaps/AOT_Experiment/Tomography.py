...
class Tomography(Experiment):
    ...
    def check_experimentalAO(self, activeListPath, withTumor=True):
        ...
        if self.AcousticFields is not None:
            for field in self.AcousticFields:
                if field.field.shape[0] != self.AOsignal_withTumor.shape[0]:
                    if self.AOsignal_withTumor.shape[0] > field.field.shape[0]:
                        if withTumor:
                            self.AOsignal_withTumor = self.AOsignal_withTumor[:field.field.shape[0], :]
                        else:
                            self.AOsignal_withoutTumor = self.AOsignal_withoutTumor[:field.field.shape[0], :]
                    else:
                        self.cutAcousticFields(time_max=field.field.shape[0]/float(self.params.acoustic['f_saving']))
