    ...
    def save_datapoints_bulk(
        ...
        try:
            ...
            for dp in datapoints:
                # Serialize metadata to JSON with custom encoder for date objects
                context_json = json.dumps(dp.metadata, cls=DateTimeEncoder) if dp.metadata else "{}"
    ...
    def get_last_loaded_timestamp(self, metric_name: str) -> datetime | None:
        ...
        try:
            ...
            if result and result[0] and result[0][0] is not None:
                ...
                # ClickHouse returns epoch 0 (1970-01-01) for MAX() on empty table
                # Treat this as None
                epoch_zero = datetime(1970, 1, 1, 0, 0, 0)
                if last_timestamp == epoch_zero:
                    logger.debug(f"No data loaded yet for {metric_name} (got epoch 0)")
                    return None
