...
class ClickHouseCollector(BaseCollector):
    ...
    def collect_bulk(
        ...
        try:
            ...
            # Execute rendered query with column types to get column names
            logger.debug(
                f"Executing ClickHouse query for period {period_start} to {period_finish}"
            )
            logger.debug(f"Rendered query: {rendered_query[:200]}...")
            result, columns_info = client.execute(rendered_query, with_column_types=True)

            # Build column name to index mapping
            column_names = [col[0] for col in columns_info]
            try:
                timestamp_idx = column_names.index(self.timestamp_column)
            except ValueError:
                raise CollectionError(
                    f"Timestamp column '{self.timestamp_column}' not found in query result. "
                    f"Available columns: {', '.join(column_names)}",
                    source="clickhouse",
                )

            try:
                value_idx = column_names.index(self.value_column)
            except ValueError:
                raise CollectionError(
                    f"Value column '{self.value_column}' not found in query result. "
                    f"Available columns: {', '.join(column_names)}",
                    source="clickhouse",
                )

            # Build context column indices
            context_indices = {}
            if self.context_columns:
                for col in self.context_columns:
                    try:
                        context_indices[col] = column_names.index(col)
                    except ValueError:
                        logger.warning(
                            f"Context column '{col}' not found in query result, skipping. "
                            f"Available columns: {', '.join(column_names)}"
                        )
            ...
            for row_num, row in enumerate(result):
                try:
                    # Extract timestamp using column index
                    ...
                    # Extract context columns (if configured)
                    context = None
                    if context_indices:
                        context = {col: row[idx] for col, idx in context_indices.items()}
