...
def _convert_struct_column(
    ...
    if logger:
        logger.info(f"Struct '{col_name}' evoluído recursivamente com campos ausentes adicionados")
    
    return result_df

def _build_aligned_struct(col_expr, target_type, logger=None):
    """
    Constrói struct alinhado, tratando primitivos corretamente sem recursão errada.
    """
    from pyspark.sql.functions import col, lit, coalesce, struct as F_struct
    from pyspark.sql.types import StructType
    import pyspark.sql.functions as F
    ...
    for field in target_type.fields:
        ...
        field_value = col_expr.getField(field_name) if col_expr is not None else lit(None)
        
        if field_value is None or field_value == 'null':  # Seguro para ausentes
            if isinstance(target_field_type, StructType):
                # Cria struct vazio recursivo apenas se target for struct
                field_value = _build_aligned_struct(lit(None), target_field_type, logger)
            else:
                field_value = lit(_get_default_value_for_type(target_field_type))
        
        if isinstance(target_field_type, StructType):
            # Recursão só para structs reais
            sub_value = _build_aligned_struct(field_value, target_field_type, logger)
            struct_fields.append(sub_value.alias(field_name))
        else:
            # Cast primitivo sem getField extra
