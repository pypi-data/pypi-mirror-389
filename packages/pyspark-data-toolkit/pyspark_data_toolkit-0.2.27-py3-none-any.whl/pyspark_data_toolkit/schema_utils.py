...
def _build_aligned_struct(col_expr, target_type, logger=None, build_default=False):
    ...
    for field in target_type.fields:
        ...
        if isinstance(target_field_type, StructType):
            # Recursão normal (não default aqui, pois field_value já tem tipo de target)
            final_field = _build_aligned_struct(field_value, target_field_type, logger, build_default=False).alias(field_name)
        else:
            # Cast primitivo safe (infer schema sem VOID)
