...
def _build_aligned_struct(col_expr, target_type, logger=None):
    ...
    Usa col_expr.isNotNull() (método Column) para wrapping getField – evita ImportError e extract errors.
    ...
    if not isinstance(target_type, StructType):
        # Cast simples se target primitivo (safe para nulls)
    ...
    for field in target_type.fields:
        ...
        # Extração safe: when(col_expr.isNotNull(), getField) otherwise lit(None) – evita VOID/null extract
        # Nota: isNotNull() é método de Column, não import – direto no col_expr
        ...
        if logger:
            logger.debug(f"Extração {field_name} em {col_expr}: safe handling com isNotNull()")
