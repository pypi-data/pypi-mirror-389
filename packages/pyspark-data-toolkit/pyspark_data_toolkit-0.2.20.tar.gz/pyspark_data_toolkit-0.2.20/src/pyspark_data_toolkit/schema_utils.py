...
def _convert_struct_column(
    ...
    """
    Converte uma coluna do tipo struct alinhando campos ausentes recursivamente.
    Usa getField para acesso seguro a campos que podem não existir.
    """
    from pyspark.sql.types import StructType
    ...
    # Usa o helper recursivo para construir o struct alinhado
    new_col_expr = _build_aligned_struct(F.col(col_name), target_type, logger)
    result_df = df.withColumn(col_name, new_col_expr)
    
    if logger:
        logger.info(f"Coluna '{col_name}' convertida recursivamente para o schema alinhado")
...
def _build_aligned_struct(col_expr, target_type, logger=None):
    ...
    Usa getField() - mais robusto que indexação. Adiciona defaults para campos ausentes.
    """
    ...
    for field in target_type.fields:
        ...
        # getField retorna null safely se campo ausente
        field_value = col_expr.getField(field_name)
        
        # Default não-null para campos ausentes ou nulos (use _get_default_value_for_type)
        default_val = lit(_get_default_value_for_type(target_field_type))
        field_value = coalesce(field_value, default_val)
        
        # Recursão para sub-structs
        if isinstance(target_field_type, StructType):
            sub_struct = _build_aligned_struct(field_value, target_field_type, logger).alias(field_name)
            struct_fields.append(sub_struct)
        else:
