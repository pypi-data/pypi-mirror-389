                ...
                try:
                    ...
                    if _is_array_struct_conversion(current_type, target_type):
                        ...
                        if isinstance(df.schema[col_name].dataType, StructType) and isinstance(target_type, StructType):
                            # Usa df.schema para check inicial, mas converte result_df
...
def _convert_struct_column(df: DataFrame, col_name: str, target_type, logger=None) -> DataFrame:
    ...
    # Log schemas para debug
    if logger:
        logger.info(f"Antes - {col_name}: {df.schema[col_name].simpleString()}")
        logger.info(f"Target - {col_name}: {target_type.simpleString()}")
    
    # Reconstrói sempre para nested como st_article_live
    new_col_expr = _build_aligned_struct(F.col(col_name), target_type, logger)
    result_df = df.withColumn(col_name, new_col_expr)
    
    if logger:
        logger.info(f"Após - {col_name}: {result_df.schema[col_name].simpleString()}")
    
    # Testa acesso safe pós-build
    try:
        result_df.select(f"{col_name}.slogan.date").limit(1).collect()  # Deve retornar sem erro
    except Exception as test_err:
        if logger:
            logger.error(f"Teste de acesso falhou: {test_err}")
        raise
...
def _build_aligned_struct(col_expr, target_type, logger=None):
    """
    Reconstrói struct do zero, adicionando todos target fields com defaults;
    extrai do original só se field existe e tipo match (evita tratar string como struct).
    """
    from pyspark.sql.functions import col, lit, when, struct as F_struct
    ...
    # Extrai o expr pai se necessário (ex: st_article_live.slogan)
    slogan_expr = col_expr.getField("slogan") if "slogan" in col_expr.toString() else col_expr
    
    for field in target_type.fields:
        field_name = field.name
        target_field_type = field.dataType
        
        # Default sempre primeiro (vazio para string/struct)
        if isinstance(target_field_type, StringType):
            default_val = lit("")
        elif isinstance(target_field_type, (F.types.LongType, F.types.IntegerType)):
            default_val = lit(0)
        elif isinstance(target_field_type, F.types.BooleanType):
            default_val = lit(False)
        elif isinstance(target_field_type, StructType):
            # Struct vazio recursivo
            default_val = _build_aligned_struct(lit(None), target_field_type, logger)
        else:
            default_val = lit(None).cast(target_field_type)
        
        # Se field existe no original e é primitivo matching, usa valor; senão default
        existing_val = None
        if field_name == "component" and slogan_expr is not None:  # Caso específico para string
            existing_val = when(slogan_expr.isNotNull(), slogan_expr.getField("component")).otherwise(lit(""))
        elif slogan_expr is not None:
            # Para outros fields (date, event, title): tenta getField safe, default se null
            temp_val = slogan_expr.getField(field_name)
            existing_val = when(temp_val.isNotNull(), temp_val).otherwise(default_val)
        else:
            existing_val = default_val
        
        if isinstance(target_field_type, StructType):
            # Para image: recursão no default ou existing (se fosse struct)
            field_value = _build_aligned_struct(existing_val, target_field_type, logger).alias(field_name)
        else:
            field_value = existing_val.cast(target_field_type).alias(field_name)
        
        struct_fields.append(field_value)
    
    new_slogan = F_struct(*struct_fields)
    # Reconstrói full st_article_live com slogan novo
    return F_struct(new_slogan.alias("slogan"))
