...
def _convert_struct_column(df: DataFrame, col_name: str, target_type, logger=None) -> DataFrame:
    """
    Converte coluna struct genérica, com handling safe para nulls/VOID.
    Neutra para qualquer coluna; wrapping previne extract errors em bronze parciais.
    """
    from pyspark.sql.types import StructType
    ...
    # Se current não struct mas target é, cria struct vazio safe (evita VOID em cast)
    if not isinstance(current_type, StructType) and isinstance(target_type, StructType):
        if logger:
            logger.info(f"{col_name} não struct no input, criando vazio alinhado ao target.")
        new_col_expr = _build_aligned_struct(F.lit(None), target_type, logger)  # Safe para não-struct
    elif isinstance(current_type, StructType) and isinstance(target_type, StructType):
        # Build normal com wrapping
    ...
    if logger:
        ...
        logger.info(f"Struct '{col_name}' alinhado safe para nulls/VOID")
...
def _build_aligned_struct(col_expr, target_type, logger=None):
    """
    Reconstrói struct genérico expressionalmente, com extração safe de nulls/VOID.
    Wrapping getField em when(isNotNull) evita extract errors em structs parciais/null.
    Genérico para qualquer nested level/coluna.
    """
    from pyspark.sql.functions import lit, when, isnull, struct as F_struct, isNotNull
    from pyspark.sql.types import StructType, StringType, LongType, BooleanType
    import pyspark.sql.functions as F
    
    if not isinstance(target_type, StructType):
        # Cast simples se target primitivo (ex: string para int, safe para nulls)
    ...
    for field in target_type.fields:
        ...
        # Extração safe: when(col_expr não null, getField) otherwise lit(None) – evita VOID/null extract
        existing_val = when(isNotNull(col_expr), col_expr.getField(field_name)).otherwise(lit(None))
        
        # Log se null frequente (opcional, remova após debug)
        if logger:
            logger.debug(f"Extração {field_name} em {col_expr}: safe handling")
        
        # Default baseado no tipo (lit(None) como fallback para structs, cast safe)
        if isinstance(target_field_type, StringType):
            ...
            # Para sub-struct ausente/null: recursão com lit(None) safe (cria vazio)
            default_val = _build_aligned_struct(lit(None), target_field_type, logger)
        else:
            default_val = lit(None).cast(target_field_type)
        
        # When para escolher: existing se não null, senão default (expressional)
        field_value = when(isnull(existing_val), default_val).otherwise(existing_val)
        
        if isinstance(target_field_type, StructType):
            # Recursão com wrapping safe (ex: image com nulls vira vazio sem extract error)
            final_field = _build_aligned_struct(field_value, target_field_type, logger).alias(field_name)
        else:
            # Cast primitivo safe (coalesce nulls se necessário)
            final_field = field_value.cast(target_field_type).alias(field_name)
        
        struct_fields.append(final_field)
    
    # Novo struct raiz safe (genérico, sem issues em null roots)
