...
def _convert_struct_column(df: DataFrame, col_name: str, target_type, logger=None) -> DataFrame:
    ...
    # Genérico: F.col(col_name) como root expr para qualquer struct top-level
    new_col_expr = _build_aligned_struct(F.col(col_name), target_type, logger)
    result_df = df.withColumn(col_name, new_col_expr)
    
    # Teste genérico (ajuste path para fields conhecidos, ou remova para neutralidade total)
    if logger:
        logger.info(f"Struct '{col_name}' alinhado genéricamente")
...
def _build_aligned_struct(col_expr, target_type, logger=None):
    ...
    Genérico: não assume nomes como 'slogan' – aplica a qualquer nested level.
    """
    ...
    for field in target_type.fields:
        ...
        # Extrai field do col_expr original (safe: null se ausente)
        existing_val = col_expr.getField(field_name)
        
        # Default baseado no tipo (neutro para primitivos e structs)
