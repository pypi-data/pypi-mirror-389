...
def _convert_struct_column(
    ...
    Versão corrigida para structs aninhados.
    """
    ...
    # CORREÇÃO: Recursivamente processar structs aninhados
    result_df = df
    struct_fields = []
    ...
    for field in target_type.fields:
        ...
        if field_name in current_field.dataType.fieldNames():
            ...
            if isinstance(target_field_type, StructType) and isinstance(current_nested_type, StructType):
                # Criar uma coluna temporária com o struct aninhado
                temp_col = f"_temp_{col_name}_{field_name}"
                result_df = result_df.withColumn(
                    temp_col, 
                    col(f"{col_name}.{field_name}")
                )
                # Converter o struct aninhado
                result_df = _convert_struct_column(
                    result_df, temp_col, target_field_type, logger
                )
                # Adicionar à lista de campos para reconstruir
                struct_fields.append(col(temp_col).alias(field_name))
            else:
    ...
    result_df = result_df.withColumn(col_name, struct(*struct_fields))
    
    # Limpar colunas temporárias
    temp_cols = [col for col in result_df.columns if col.startswith("_temp_")]
    if temp_cols:
