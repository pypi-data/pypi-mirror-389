...
def _convert_struct_column(
    ...
    Usa SQL TRANSFORM para segurança ao acessar campos que podem não existir.
    """
    from pyspark.sql.types import StructType
    ...
    # ✅ Usar SQL para safe field access
    df.createOrReplaceTempView(f"_temp_struct_{col_name}")
    
    # Construir expressão SQL para cada campo do target type
    select_fields = []
    for field in target_type.fields:
        field_name = field.name
        target_field_type = str(field.dataType)
        
        # Verificar se o campo existe na estrutura atual
        current_field_names = [f.name for f in current_field.dataType.fields]
        
        if field_name in current_field_names:
            # Campo existe - tentar extrair com segurança
            select_fields.append(f"`{col_name}`.`{field_name}` AS `{field_name}`")
        else:
            # Campo não existe - criar com NULL
            if logger:
                logger.warning(f"Campo ausente: {col_name}.{field_name}, preenchendo com NULL")
            select_fields.append(f"CAST(NULL AS {target_field_type}) AS `{field_name}`")
    
    field_list = ", ".join(select_fields)
    
    # SQL query para reconstruir o struct
    sql_query = f"""
        SELECT *, 
               STRUCT({field_list}) AS `{col_name}_new`
        FROM `_temp_struct_{col_name}`
    """
    ...
    if logger:
        logger.info(f"Coluna '{col_name}' convertida com sucesso para o schema alinhado")
