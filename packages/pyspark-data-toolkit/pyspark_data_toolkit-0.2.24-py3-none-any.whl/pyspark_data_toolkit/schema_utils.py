...
def _convert_struct_column(df: DataFrame, col_name: str, target_type, logger=None) -> DataFrame:
    """
    Converte coluna struct genérica, alinhando recursivamente ao target_type.
    Neutra para qualquer coluna; sem testes hard-coded ou callable errors.
    """
    from pyspark.sql.types import StructType
    ...
    if logger:
        ...
        logger.info(f"Struct '{col_name}' alinhado sem erros")
...
def _build_aligned_struct(col_expr, target_type, logger=None):
    """
    Reconstrói struct genérico expressionalmente, alinhando ao target_type recursivamente.
    Extrai fields do col_expr original onde possível; adiciona defaults para ausentes.
    Genérico: não assume nomes – aplica a qualquer nested level sem callable issues.
    """
    ...
    for field in target_type.fields:
        ...
        # Extrai field safe: getField retorna Column ou null se ausente
        existing_val = col_expr.getField(field_name)
        
        # Log debug se callable persistir (remova após fix)
        if logger and isinstance(existing_val, F.Column):
            logger.debug(f"Extraído {field_name}: Column type OK")
        
        # Default baseado no tipo (corrigido para Python 3: lit(0) para long)
        if isinstance(target_field_type, StringType):
            default_val = lit("")
        elif isinstance(target_field_type, LongType):
            default_val = lit(0)  # Não 0L – evita syntax/callable misinterp
        elif isinstance(target_field_type, BooleanType):
            default_val = lit(False)
        elif isinstance(target_field_type, StructType):
            # Recursão para sub-struct vazio (ex: image._id, url)
            default_val = _build_aligned_struct(lit(None), target_field_type, logger)
        else:
            default_val = lit(None).cast(target_field_type)
        
        # When expressional: escolhe existing se não null, senão default (sem Python if em Column)
        field_value = when(isnull(existing_val), default_val).otherwise(existing_val)
        
        if isinstance(target_field_type, StructType):
            # Recursão segura para nested (sem callable em subcalls)
            final_field = _build_aligned_struct(field_value, target_field_type, logger).alias(field_name)
        else:
            # Cast primitivo (evita lit callable)
            final_field = field_value.cast(target_field_type).alias(field_name)
        
        struct_fields.append(final_field)
    
    # Retorna novo struct raiz (genérico, sem F_struct call issues)
