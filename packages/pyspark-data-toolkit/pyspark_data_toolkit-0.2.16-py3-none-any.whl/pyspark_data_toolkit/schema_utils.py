...
def _convert_struct_column(
    ...
    for field in target_type.fields:
        ...
        full_path = f"{col_name}.{field_name}"
        
        # se o campo aninhado não existe, adiciona como None
        if field_name not in current_field.dataType.fieldNames():
            if logger:
                logger.warning(f"Campo ausente: {full_path}, criando com valor nulo.")
            df = df.withColumn(full_path.replace('.', '_'), F.lit(None).cast(field.dataType))
            struct_fields.append(F.col(full_path.replace('.', '_')).alias(field_name))
        else:
            if isinstance(field.dataType, StructType):
                df = _convert_struct_column(df, full_path, field.dataType, logger)
            struct_fields.append(F.col(full_path).alias(field_name))

    return df.withColumn(col_name, F.struct(*struct_fields))
