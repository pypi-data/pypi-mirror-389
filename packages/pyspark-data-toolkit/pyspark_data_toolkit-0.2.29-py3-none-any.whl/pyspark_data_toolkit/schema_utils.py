...
def _align_struct_fields(col_expr, target_struct, logger):
    ...
    for field in target_struct.fields:
        ...
        try:
            # Campo existe na origem
            if col_expr.getField(field_name) is not None:
                sub_col = F.col(f"{col_expr._jc.toString()}.{field_name}")
            ...
            # Campo ausente → cria campo nulo compatível
            aligned_field = _get_default_value_for_type(field_type).cast(field_type)
            logger.debug(f"Campo '{field_name}' ausente — preenchido como NULL ({field_type.simpleString()})")
