...
def _convert_struct_column(
    ...
    preenchendo-os com valores padrão (NULL ou default coerente).
    """
    from pyspark.sql.functions import col, struct, lit
    from pyspark.sql.types import StructType, StructField
    
    if not isinstance(target_type, StructType):
        raise ValueError(f"Tipo de destino deve ser StructType, recebido: {target_type}")
    
    current_struct = df.schema[col_name].dataType
    if not isinstance(current_struct, StructType):
        raise ValueError(f"Coluna {col_name} não é StructType, recebido: {current_struct}")
    ...
    for field_name, field in target_fields.items():
        if field_name in current_fields:
            # campo existe → mantém o cast
            if isinstance(field.dataType, StructType):
                # alinhamento recursivo de structs aninhadas
                nested_col = f"{col_name}.{field_name}"
                df = _convert_struct_column(df, nested_col, field.dataType, logger)
                struct_fields.append(col(nested_col).alias(field_name))
            else:
                struct_fields.append(col(f"{col_name}.{field_name}").cast(field.dataType).alias(field_name))
        else:
            # campo ausente → adiciona valor padrão
            default_value = _get_default_value_for_type(field.dataType)
            struct_fields.append(lit(default_value).cast(field.dataType).alias(field_name))
    
    df = df.withColumn(col_name, struct(*struct_fields))
    
    if logger:
        added = set(target_fields.keys()) - set(current_fields.keys())
        logger.info(f"Evoluído struct '{col_name}' com campos adicionados: {list(added)}")
