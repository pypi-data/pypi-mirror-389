...
class CostFunction:
    ...
    def eval(self, x, freq_ghz, kwargs):
        self.fn(x, freq_ghz, self.score_fn, kwargs)
...
class InverseProblem:
    def __init__(self, config_file, cost_fn, score_fn, unknown_layer: int = 0):
        ...
        self.frequencies_ghz = linspace(self.config['sweep']['range'][0],
                                           self.config['sweep']['range'][1],
                                           int(self.config['sweep']['num_points']), True)
    ...
    def fit(self, x, freq_idx, model, args=None):
        ...
        if len(x) == 2:
            ...
            bounds = ([1, 20], [-1, -0.00001])
        elif len(x) == 3:
            # thickness, permittivity (real, imag)
            bounds = ([0.0001, 50], [1, 10.], [-1., -1e-6])
        else:
            # thickness1, thickness2, permittivity (real, imag)
            bounds = ([0.0001, 50], [0.0001, 50], [1, 10.], [-1., -1e-6])
    ...
    def load_experimental_data(self,
        ...
        if s2p_path.endswith('.s2p'):
            ...
            self.data =  self.filtered_data[0].s
        else:
