...
class Trainer:
    ...
    def train(self, epoch=None, new_op=True, no_val=False, step=None, reset_loss_tracker=False, device=None):
        ...
        if self.val_dataset is not None:
            if 'cv_step' not in self.cfg_val:
                self.print('val_dataset is given but "cv_step" not specified in cfg_val. Defaults to 1 epoch')
                self.cfg_val['cv_step'] = len(self.loader)
    ...
    def _update_epoch(self, T, no_val=False):
        ...
        for epoch in range(1, T+1):
            ...
            for batch_i, data in enumerate(self.loader, 1):
                ...
                # Validation
                if (self.val_dataset is not None) and (not no_val) and (_iter % self.cfg_val['cv_step']==0):
