...
class EncoderConfig(BaseModel):
    ...
    output_group: Optional[OutputGroup] = None

    @model_validator(mode="after")
    def _validate_outputs(self) -> "EncoderConfig":
        # Support both new format (Outputs) and legacy format (output_group)
        if not self.Outputs and not self.output_group:
            raise ValueError("Encoder must define either Outputs or output_group")
        if self.Outputs and self.output_group:
            raise ValueError("Provide either Outputs or output_group, not both")
        return self
...
class PipelineStatusResponse(BaseModel):
    ...
    detected_users: Optional[List[str]] = None
...
class ComponentStatus(BaseModel):
    """Status of a pipeline component (MediaConnect, EC2, or MediaLive)."""
    model_config = ConfigDict(extra="allow")

    status: str
    # MediaConnect specific
    flowStatus: Optional[str] = None
    enabledOutputs: Optional[int] = None
    totalOutputs: Optional[int] = None
    # EC2 specific
...
class StateComponents(BaseModel):
    ...
    mediaconnect: Optional[ComponentStatus] = None
    ec2: Optional[ComponentStatus] = None
    medialive: Optional[ComponentStatus] = None
...
class StateResponse(BaseModel):
    ...
    components: Optional[StateComponents] = None
    note: Optional[str] = None
