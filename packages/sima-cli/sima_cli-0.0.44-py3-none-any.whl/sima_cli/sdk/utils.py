...
def baseline_is_present(installed_refs: set[str], version: str, images_user_will_remove: set[str]) -> bool:
...
def get_all_containers(running_containers_only: bool = False):
    """
    Return a list of Docker containers with detailed info (in JSON format).

    Args:
        running_containers_only (bool): If True, return only running containers (default: False).

    Returns:
        list[dict]: Each entry is a parsed JSON object from `docker ps --format '{{json .}}'`
                    containing keys like: ID, Image, Names, Status, Ports, etc.
    """
    try:
        cmd = ["docker", "ps", "--format", "{{json .}}"]
        if not running_containers_only:
            cmd.insert(2, "-a")  # equivalent to docker ps -a
        ...
        for line in result.stdout.strip().splitlines():
            ...
            try:
                ...
                continue  # skip malformed line just in case
...
def get_local_sima_images():
    """Return a list of local SDK images containing 'sima-docker'."""
    try:
        output = subprocess.check_output(
    ...
    images = [line for line in output.splitlines() if "sima-docker" in line]
    return sorted(set(images))
...
def prompt_image_selection(images):
    ...
    # Example: "sima-docker/sdk:1.8" → version "1.8"
    #          "sima-docker/sdk:latest" → version "latest"
    # ─────────────────────────────────────────────
    version_map = defaultdict(list)
    for img in images:
        # Extract version tag safely (after last colon)
        parts = img.split(":")
    ...
    # ─────────────────────────────────────────────
    # 2. If multiple versions found, prompt user to choose
    # ─────────────────────────────────────────────
    if len(versions) > 1:
        version_choices = [{"name": v, "value": v} for v in sorted(versions)]
        selected_version = (
            inquirer.select(
        ...
        console.print(f"[cyan]ℹ️  Showing images for version [bold]{selected_version}[/bold][/cyan]")
    else:
        console.print(f"[dim]Single SDK version detected: {versions[0]}[/dim]")

    # ─────────────────────────────────────────────
    # 3. Show image selection prompt
    # ─────────────────────────────────────────────
    choices = (
    ...
    selected = (
        inquirer.checkbox(
            ...
            transformer=lambda res: (
                f"[bold green]{len(res)} selected[/bold green]" if res else "[dim]None selected[/dim]"
            ),
        ).execute()
    )
    ...
    if container_name in existing:
        ...
        if yes_to_all:
            ...
            if resp in {"", "y", "yes"}:
                ...
                console.print(f"⏩ Skipping '{container_name}'.", style="yellow")
                return
