...
from sima_cli.sdk.uninstall import remove_containers
...
def sdk(ctx, version_filter):
    """Manage and launch SiMa SDK tool containers."""
...
def start(ctx, yes):
    ...
    setup_and_start(start_only=True, yes_to_all=yes)
...
def list_sdk(ctx):
    ...
    for c in containers:
        ...
        # ──────────────────────────────────────────────
        # 3. Skip non-SDK containers (optional)
        # ──────────────────────────────────────────────
        if not sdk_name or "sima-docker" not in image:
            continue
