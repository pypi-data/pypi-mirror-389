"""
Dynamic tool generation for Polygon agent clients.
...
SPEC_FILE = Path(__file__).parent / "tool_specs.json"
...
def _import_class(module_path: str, class_name: str):
    ...
    Args:
        module_path: Module path (e.g., "polygon.clients.agent_market_data_client")
...
def create_tool_from_spec(spec: Dict[str, Any]) -> StructuredTool:
    ...
    # Import client class from market_data_sdk.epoch_polygon.clients
    # All agent clients are exported there
    client_class = _import_class(
        "market_data_sdk.epoch_polygon.clients",
        client_class_name
    )

    # Import request model from market_data_sdk.epoch_polygon.models.requests
    request_model = _import_class(
        "market_data_sdk.epoch_polygon.models.requests",
        request_model_name
    )
