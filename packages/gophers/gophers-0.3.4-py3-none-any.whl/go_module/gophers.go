...
func (df *DataFrame) Flatten(flattenCols []string) *DataFrame {
    ...
    for _, fcol := range flattenCols {
        // Collect all nested keys we will create as columns
        ...
        // Create new columns (once) with full length
        newCols := []string{}
        for k := range keySet {
            name := fcol + "." + k
            if !colSet[name] {
        ...
        for i := 0; i < df.Rows; i++ {
            ...
            for _, name := range newCols {
                // name is fcol.key → extract key part after fcol+"."
                key := name[len(fcol)+1:]
                if nested != nil {
...
func (df *DataFrame) explodePrealloc(column string) *DataFrame {
    ...
    // First pass: compute total output rows
    total := 0
    for i := 0; i < df.Rows; i++ {
        val := df.Data[column][i]
         if arr, ok := val.([]interface{}); ok {
             if len(arr) == 0 { total++ } else { total += len(arr) }
         } else {
             total++
         }
     }
