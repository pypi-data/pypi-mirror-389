...
class Spillage(Component):
    """
    A class representing demand for a resource in the simulation.

    This class is used to represent the demand for a specific resource in
    a simulation environment. It encapsulates data attributes such as the
    resource being demanded, demand values, load shedding cost and capacity,
    and spillage cost and capacity. The input data for demand or other
    ...
    def __init__(
            ...
            max_capacity: Optional[Union[float, Dict[int, float], pl.DataFrame]] = 0.,
    ):
        """
        ...
        :type resource: str
        :param demand: Demand value or a DataFrame defining the demand.
        :type demand: Union[float, pl.DataFrame]
        :param load_shedding_cost: Cost associated with load shedding, can be a single
            value, a dictionary of costs indexed by time, or a DataFrame.
        :type load_shedding_cost: Optional[Union[float, Dict[int, float], pl.DataFrame]]
        :param load_shedding_max_capacity: Maximum allowable capacity for load shedding,
            can be a single value, a dictionary indexed by time, or a DataFrame.
        :type load_shedding_max_capacity: Optional[Union[float, Dict[int, float], pl.DataFrame]]
        :param spillage_cost: Cost associated with spillage, can be a single value,
            a dictionary of costs indexed by time, or a DataFrame.
        :type spillage_cost: Optional[Union[float, Dict[int, float], pl.DataFrame]]
        :param spillage_max_capacity: Maximum allowable spillage capacity, can be a
            single value, a dictionary indexed by time, or a DataFrame.
        :type spillage_max_capacity: Optional[Union[float, Dict[int, float], pl.DataFrame]]
        """
        super().__init__(name)
