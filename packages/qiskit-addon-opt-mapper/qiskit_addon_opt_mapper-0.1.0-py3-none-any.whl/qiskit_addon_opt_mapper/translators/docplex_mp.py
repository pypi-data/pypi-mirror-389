...
def to_docplex_mp(quadratic_problem: OptimizationProblem) -> Model:
    ...
    Args:
        quadratic_problem: The optimization problem to be translated.
    ...
    """
    # initialize model
    mdl = Model(quadratic_problem.name)

    # add variables
    var = {}
    for idx, x in enumerate(quadratic_problem.variables):
    ...
    if quadratic_problem.objective.higher_order:
        raise OptimizationError(
            "Higher-order objective terms are not supported in docplex.md model export."
        )

    if quadratic_problem.get_num_higher_order_constraints() > 0:
        raise OptimizationError(
            "Higher-order constraints are not supported in docplex.md model export."
        )
    ...
    objective = (
        ...
        + mdl.sum(
            ...
            for (i, j), v in quadratic_problem.objective.quadratic.to_dict().items()
        )
    )
    if quadratic_problem.objective.sense == OptimizationObjective.Sense.MINIMIZE:
        mdl.minimize(objective)
    else:
        mdl.maximize(objective)

    # add linear constraints
    for l_constraint in quadratic_problem.linear_constraints:
...
class _FromDocplexMp:
    ...
    def quadratic_program(self, indicator_big_m: float | None) -> OptimizationProblem:
        ...
        """
        self._quadratic_program = OptimizationProblem(self._model.name)
...
def from_docplex_mp(model: Model, indicator_big_m: float | None = None) -> OptimizationProblem:
    ...
    Higher-order objective terms and constraints are not supported in a docplex.mp model.
    Also spin variables are not supported.
    ...
    if model.number_of_user_cut_constraints > 0:
        raise OptimizationError("User cut constraints are not supported")

    if model.number_of_lazy_constraints > 0:
        raise OptimizationError("Lazy constraints are not supported")

    if model.number_of_sos > 0:
        raise OptimizationError("SOS sets are not supported")
