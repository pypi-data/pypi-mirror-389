...
"""An interface definition for solvers in the qiskit-addon-opt-mapper package."""
...
from ..converters.optimization_problem_to_qubo import (
    OptimizationProblemConverter,
    OptimizationProblemToQubo,
)
from ..exceptions import OptimizationError
from ..problems.optimization_problem import OptimizationProblem, Variable

logger = getLogger(__name__)


class SolverResultStatus(Enum):
    """Termination status of an optimization solver."""

    SUCCESS = 0
    """the optimization solver succeeded to find a feasible solution."""

    FAILURE = 1
    """the optimization solver ended in a failure."""

    INFEASIBLE = 2
    """the optimization solver obtained an infeasible solution."""
...
class SolverResult:
    ...
    The optimization algorithms return an object of the type ``SolverResult``
    with the information about the solution obtained.

    ``SolverResult`` allows users to get the value of a variable by specifying an index or
    a name as follows.
    ...
    Note:
        ...
        optimization algorithms. Optimization algorithms and converters of ``OptimizationProblem``
        should maintain the order when generating a new ``SolverResult`` object.
    """
    ...
    @property
    def status(self) -> SolverResultStatus:
...
class OptimizationSolver(ABC):
    ...
    def _get_feasibility_status(
        problem: OptimizationProblem, x: list[float] | np.ndarray
    ) -> SolverResultStatus:
    ...
    def _prepare_converters(
        ...
        Raises:
            ...
            :class:`~qiskit_addon_opt_mapper.converters.OptimizationProblemConverter type.
        """
    ...
    def _interpret(
        ...
        **kwargs,
        ...
        Raises:
            OptimizationError: if result_class is not a sub-class of SolverResult.
            TypeError: if converters are not OptimizationProblemConverter or a list of
                OptimizationProblemConverter.
        """
        if not issubclass(result_class, SolverResult):
            raise OptimizationError(
                f"Invalid result class, not derived from SolverResult: {result_class}"
            )
