...
class OptimizationProblemToQubo(OptimizationProblemConverter):
    ...
    The resulting HUBO problem can be directly mapped to an Ising Hamiltonian using the `to_ising()`
    function.
    ...
    def __init__(self, penalty: float | None = None) -> None:
        ...
        Args:
            penalty: Penalty factor to scale equality constraints that are added to objective.
                ...
                conversion.
        """
        self._penalize_lin_eq_constraints = EqualityToPenalty(penalty=penalty)
    ...
    def get_compatibility_msg(problem: OptimizationProblem) -> str:
        ...
        # check whether there are incompatible objective functions
        if len(problem.objective.higher_order) > 0:
            msg += "Higher order objective functions are not supported. "
        ...
        if len(problem.higher_order_constraints) > 0:
            msg += "Higher order constraints are not supported. "
