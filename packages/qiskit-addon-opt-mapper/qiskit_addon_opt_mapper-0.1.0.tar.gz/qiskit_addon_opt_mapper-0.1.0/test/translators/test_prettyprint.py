...
from qiskit_addon_opt_mapper.translators.prettyprint import expr2str, prettyprint

from ..optimization_test_case import OptimizationTestCase


class TestPrettyprint(OptimizationTestCase):
    """Test prettyprint"""
    ...
    def test_prettyprint(self):
        ...
        with self.subTest("maximize 2"):
            q_p = OptimizationProblem("problem 1")
        ...
        with self.subTest("minimize 3 (spin var)"):
            q_p = OptimizationProblem("spin var")
            q_p.spin_var("s0")
            q_p.spin_var("s1")
            q_p.minimize(linear={"s0": 1}, quadratic={("s0", "s1"): 1})
            expected = "\n".join(
                [
                    "Problem name: spin var",
                    "",
                    "Minimize",
                    "  s0*s1 + s0",
                    "",
    ...
    def test_prettyprint2(self):
        ...
        q_p = OptimizationProblem("+ test - test *")
        q_p.binary_var(" x ")
    ...
    def test_error(self):
        ...
        name = "\n"
        with self.subTest("problem name - func"), self.assertRaises(OptimizationError):
            q_p = OptimizationProblem(name)
            _ = prettyprint(q_p)

        with self.subTest("problem name - meth"), self.assertRaises(OptimizationError):
            q_p = OptimizationProblem(name)
            _ = q_p.prettyprint()
...
class TestExpr2Str(OptimizationTestCase):
    """Test expr2str"""
