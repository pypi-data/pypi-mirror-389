...
class TestDocplexMpTranslator(OptimizationTestCase):
    """Test from_docplex_mp and to_docplex_mp"""
    ...
    def test_unsupported_features(self):
        ...
        with self.subTest("semiinteget_var"), self.assertRaises(OptimizationError):
            mod = Model()
            mod.semiinteger_var(lb=1, name="x")
            _ = from_docplex_mp(mod)

        with self.subTest("range constraint"), self.assertRaises(OptimizationError):
            mod = Model()
            x = mod.binary_var("x")
        ...
        with self.subTest("not equal constraint"), self.assertRaises(OptimizationError):
            mod = Model()
            x = mod.binary_var("x")
            y = mod.binary_var("y")
            mod.add(x != y)
        ...
        with self.subTest("PWL constraint"), self.assertRaises(OptimizationError):
            mod = Model()
            x = mod.binary_var("x")
            mod.add(mod.piecewise(-1, [(0, 0)], 1)(x) <= 1)
            _ = from_docplex_mp(mod)

        with self.subTest("lazy constraint"), self.assertRaises(OptimizationError):
            mod = Model()
            x = mod.binary_var("x")
            y = mod.binary_var("y")
        ...
        with self.subTest("user cut constraint"), self.assertRaises(OptimizationError):
            mod = Model()
            x = mod.binary_var("x")
            y = mod.binary_var("y")
        ...
        with self.subTest("sos1"), self.assertRaises(OptimizationError):
            mod = Model()
            x = mod.binary_var("x")
            y = mod.binary_var("y")
            mod.add_sos1([x, y])
        ...
        with self.subTest("sos2"), self.assertRaises(OptimizationError):
            mod = Model()
            x = mod.binary_var("x")
            y = mod.binary_var("y")
            z = mod.binary_var("z")
