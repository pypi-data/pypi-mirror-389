...
def get_access_token():
    if os.path.exists(TOKEN_PATH):
        with open(TOKEN_PATH, 'r') as token_file:
            ...
            log("Loaded token data (masked):\n {}".format(_mask_token_payload(token_data)), level="DEBUG")
            if 'access_token' in token_data and 'expires_in' in token_data:
                ...
                if token_expiry_time > time.time():
                    log("Access token is still valid. Expires in {} seconds.".format(int(token_expiry_time - time.time())), level="DEBUG")
                    return token_data['access_token']
                else:
                    log("Access token has expired. Current time: {}, Expiry time: {}".format(time.time(), token_expiry_time), level="DEBUG")
                    ...
                    if 'access_token' in new_token_data:
                        ...
                        log("Access token refreshed successfully. New token data (masked): {}".format(_mask_token_payload(new_token_data)), level="DEBUG")
