...
def collect_detailed_patient_data(selected_files, config, crosswalk):
    ...
    MediLink_ConfigLoader.log("Starting insurance type cache overlay for {} patient(s)".format(len(detailed_patient_data)), level="INFO")
    try:
        if cache_lookup is not None:
            csv_dir = get_csv_dir_from_config(config) if get_csv_dir_from_config else ''
            MediLink_ConfigLoader.log("Insurance type cache lookup: csv_dir='{}'".format(csv_dir), level="INFO")
            if csv_dir:
                lookup_count = 0
                success_count = 0
                for idx, data in enumerate(detailed_patient_data):
                    try:
                        ...
                        # Optional fallback inputs when available in this flow
                        dob = data.get('dob', '')
                        member_id = data.get('member_id', '')
                        lookup_count += 1
                        # Extract additional fields that might help with matching
                        patient_name = data.get('patient_name', '')
                        service_date = data.get('surgery_date', '') or data.get('surgery_date_iso', '')
                        insurance_name = data.get('primary_insurance', '')
                        # Log lookup attempt (no PHI - patient_id only, no DOB/member_id)
                        MediLink_ConfigLoader.log("Cache lookup attempt {}: patient_id='{}', has_dob={}, has_member_id={}".format(
                            idx + 1, pid, bool(dob), bool(member_id)), level="INFO")
                        api_code = cache_lookup(patient_id=pid, dob=dob, member_id=member_id, csv_dir=csv_dir, 
                                               patient_name=patient_name, payer_id=None, 
                                               service_date=service_date, insurance_name=insurance_name, config=config)
                        if api_code:
                            success_count += 1
                            MediLink_ConfigLoader.log("Cache lookup SUCCESS for patient_id='{}': code='{}'".format(pid, api_code), level="INFO")
                            data['insurance_type'] = api_code
                            data['insurance_type_source'] = 'API'
                        else:
                            MediLink_ConfigLoader.log("Cache lookup MISS for patient_id='{}' (not found in cache)".format(pid), level="INFO")
                    except Exception as e:
                        MediLink_ConfigLoader.log("Cache lookup exception for patient {}: {}".format(idx + 1, str(e)), level="WARNING")
                        continue
                MediLink_ConfigLoader.log("Insurance type cache overlay complete: {} lookup(s), {} success(es)".format(lookup_count, success_count), level="INFO")
            else:
                MediLink_ConfigLoader.log("Insurance type cache lookup skipped: csv_dir is empty", level="WARNING")
        else:
            MediLink_ConfigLoader.log("Insurance type cache lookup skipped: cache_lookup function not available", level="WARNING")
    except Exception as e:
...
def extract_and_suggest_endpoint(file_path, config, crosswalk):
    ...
    for personal_info, insurance_info, service_info, service_info_2, service_info_3 in MediLink_DataMgmt.read_fixed_width_data(file_path):
        ...
        # Extract DOB and member_id if available in fixed-width file (for cache matching)
        dob_from_fixed = parsed_data.get('DOB', '') or parsed_data.get('BIRTH', '') or parsed_data.get('BIRTHDATE', '')
        member_id_from_fixed = parsed_data.get('MEMBER', '') or parsed_data.get('MEMBERID', '') or parsed_data.get('POLICY', '') or parsed_data.get('POLICYNUM', '')
        ...
        detailed_data.update({
            ...
            # Add DOB and member_id if found in fixed-width file (for cache matching)
            'dob': dob_from_fixed,
            'member_id': member_id_from_fixed
        })
