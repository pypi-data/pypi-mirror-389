...
if __name__ == "__main__":
    ...
    try:
        ...
        if proceed:
            ...
            # Identify dual-date NEW patients for anticipatory display BEFORE notepad generation
            dual_date_new_patients = []
            for row in csv_data:
                if len(row.get('_all_surgery_dates', [])) > 1:
                    # This patient has multiple surgery dates and is NEW (not existing)
                    patient_entries = create_patient_entries_from_row(row, reverse_mapping)
                    dual_date_new_patients.extend(patient_entries)

            # Generate and open notepad file with existing patients table (and dual-date anticipatory table if applicable)
            try:
                # Use combined function if we have dual-date patients, otherwise use original function
                if dual_date_new_patients:
                    patient_type = "MEDICARE" if is_medicare else "PRIVATE"
                    dual_date_title = "{} PATIENTS - DUAL DATE-OF-SERVICE (ANTICIPATORY): These patients have multiple dates of service.\n      After the first date is entered via AHK, they will become EXISTING patients and may need manual diagnosis updates:".format(patient_type)
            ...
            # Display anticipatory dual-date patients table if any exist (on-screen only, already included in notepad)
            if dual_date_new_patients:
                patient_type = "MEDICARE" if is_medicare else "PRIVATE"
                dual_date_title = "{} PATIENTS - DUAL DATE-OF-SERVICE (ANTICIPATORY): These patients have multiple dates of service.\n      After the first date is entered via AHK, they will become EXISTING patients and may need manual diagnosis updates:".format(patient_type)
                MediBot_UI.display_enhanced_patient_table(
