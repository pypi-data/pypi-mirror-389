...
def _lookup_csv_for_dob_member_id(csv_dir, patient_name, service_date, insurance_name, config=None):
    ...
    try:
        ...
        try:
            ...
            for row in csv_data:
                # Build patient name from CSV fields (try multiple field name variations)
                csv_first = _normalize_str(row.get('Patient First', row.get('First Name', row.get('First', ''))))
                csv_last = _normalize_str(row.get('Patient Last', row.get('Last Name', row.get('Last', ''))))
                csv_middle = _normalize_str(row.get('Patient Middle', row.get('Middle Name', row.get('Middle', ''))))
                csv_name = ' '.join([csv_first, csv_middle, csv_last]).strip()
                if not csv_name:
                    # Fallback: try single 'Patient Name' field if it exists
                    csv_name = _normalize_str(row.get('Patient Name', row.get('Name', '')))
                csv_name = csv_name.upper()
                
                csv_dob = _normalize_str(row.get('Patient DOB', row.get('DOB', '')))
                csv_member_id = _normalize_str(row.get('Primary Policy Number', row.get('Ins1 Member ID', row.get('Member ID', ''))))
                csv_service_date = _normalize_str(row.get('Surgery Date', row.get('Service Date', row.get('Date', ''))))
                csv_insurance = _normalize_str(row.get('Primary Insurance', row.get('Insurance Name', row.get('Ins1 Name', '')))).upper()
                
                # Simple name matching (check if key parts match)
                csv_name_parts = csv_name.split()
                if not csv_name_parts or not name_parts:
                    continue
                
                # Match if first and last name parts are found in CSV name
                first_match = name_parts[0] in csv_name_parts if name_parts else False
                last_match = name_parts[-1] in csv_name_parts if len(name_parts) > 1 else False
                
                # Require at least first name match, ideally both first and last
                if first_match and (last_match or len(name_parts) == 1):
                    # Name matches, check service date and insurance if available
                    # Normalize dates for comparison (handle different formats)
                    date_match = True  # Default to match if dates not available
                    if service_date and csv_service_date:
                        # Simple string comparison (normalize both to same format if possible)
                        service_date_norm = service_date.replace('-', '').replace('/', '').strip()
                        csv_service_date_norm = csv_service_date.replace('-', '').replace('/', '').strip()
                        date_match = service_date_norm == csv_service_date_norm or service_date in csv_service_date or csv_service_date in service_date
                    elif not service_date or not csv_service_date:
                        date_match = True  # If either is missing, don't exclude on date
                    
                    # Insurance matching (fuzzy - check if key parts match)
                    ins_match = True  # Default to match if insurance not available
                    if insurance_name and csv_insurance:
                        # Extract key words from insurance names for comparison
                        ins_words = set(insurance_name.split())
                        csv_ins_words = set(csv_insurance.split())
                        # Match if there's significant overlap or if one contains the other
                        ins_match = (len(ins_words & csv_ins_words) >= 2 or 
                                    insurance_name in csv_insurance or 
                                    csv_insurance in insurance_name or
                                    any(word in csv_insurance for word in ins_words if len(word) > 3))
                    elif not insurance_name or not csv_insurance:
                        ins_match = True  # If either is missing, don't exclude on insurance
                    
                    if date_match and ins_match and csv_dob and csv_member_id:
                        _log("CSV bridge lookup found match: name='{}', dob='{}', member_id='{}'".format(
                            csv_name[:20] + '...' if len(csv_name) > 20 else csv_name,
                            csv_dob[:10] + '...' if len(csv_dob) > 10 else csv_dob, 
                            csv_member_id[:10] + '...' if len(csv_member_id) > 10 else csv_member_id), level="INFO")
