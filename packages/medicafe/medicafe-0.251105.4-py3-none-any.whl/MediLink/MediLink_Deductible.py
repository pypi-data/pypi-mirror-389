                ...
                if enhanced_result:
                    ...
                    try:
                        if put_entry is not None:
                            ...
                            # Prefer derived patient_id when available
                            try:
                            ...
                            if eligibility_data is not None:
                                ...
                                if enhanced_result:
                                    ...
                                    try:
                                        if put_entry is not None:
                                            code_to_persist = str(enhanced_result.get('insurance_type', '')).strip()
                                            # Prefer explicit patient_id from CSV row when available
                                            pid_for_cache = ''
                                            try:
                                                pid_for_cache = str(patient_info.get('Patient ID', '')) if patient_info else ''
                                            except Exception:
                                                pid_for_cache = ''
