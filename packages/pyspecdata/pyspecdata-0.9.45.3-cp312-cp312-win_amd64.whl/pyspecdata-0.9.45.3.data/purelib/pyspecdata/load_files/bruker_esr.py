...
def xepr(filename, exp_type=None, dimname="", verbose=False):
    ...
    if filename[-4:] == ".DTA":
        ...
        raise ValueError(
            strm(
                "When guessing that the filename is a"
    ...
    x_axis += v.pop(
        "XMIN"
    )  # actually using pop is better than calling these, so that we don't have
    #    redundant information
    harmonics = np.array(
        [[False] * 2] * 5
    )  # inner dimension for the 90 degree phase
    ...
    if n_harmonics > 1:
        logger.debug(
            "there are %d harmonics, first is of type %s"
            % (n_harmonics, ikkf[0])
        )
    # }}}
    ...
    if n_harmonics > 1:
        ...
        logger.debug(
            "I found multiple harmonics, and am loading them into the"
            " 'harmonics' axis.  This is experimental.  You most likely will"
            " want to select the 0th element of the harmonic axis."
        )
    ...
    if y_points_calcd > 1:
        if "YPTS" in list(v.keys()):
            assert v["YPTS"] == y_points_calcd, (
                "y points left over after"
                " reshaping according to harmonics (y_points_calcd="
                + str(y_points_calcd)
                + ") doesn't match the number of data points"
            )
            assert "YTYP" in list(v.keys()), (
                "No parameter YTYP -- how do you expect me to know the type of"
                " 2D dataset??"
            )
            if v["YTYP"] == "IGD":
                logger.debug("Found YTYP=IGD, assuming this is a power series")
                assert "YNAM" in list(v.keys()), (
                    "No parameter YNAM -- how do you expect me to know the"
                ...
                if isinstance(y_dim_name, list):
                    y_dim_name = " ".join(
                        y_dim_name
                    )  # it gets split into a list, which for XEpr files
                    #    shouldn't be happening, but fix later
                    if y_dim_name[0] == "'":
                        y_dim_name = y_dim_name.replace("'", "")
                if y_dim_name.startswith("'") and y_dim_name.endswith("'"):
                    y_dim_name = y_dim_name[1:-1]
                assert "YUNI" in list(v.keys()), (
                    "No parameter YUNI -- how do you expect me to know the"
                    " units of the second dimension??"
                )
            ...
            if v["YTYP"] == "IDX":
                ...
                raise ValueError(
                    strm(
                        "found YTYP=",
                        v["YTYP"],
                        " which is not currently programmed",
                    )
                )
        else:
            logger.debug(
                "y_points_calcd is greater than 1, but YPTS is not set, so"
                " assuming WinEPR style"
            )
            assert y_points_calcd == v["REY"], (
                "Trying WinEPR style and I thought REY was the indirect dim,"
                " guess not"
            )
            if dimname == "":
                y_dim_name = v["JEY"]
            ...
            y_axis = 10 ** (-y_axis / 10.0)  # convert to linear power
            y_axis *= v["MP"] / y_axis[0]  # the initial power
    ...
    assert len(extra_dims) == 0, strm(
        "You seem to have one or more extra dimension(s)called",
        str(extra_dims),
        "(Code below is just copied from winepr -- need to update)",
    )
...
def winepr(filename, dimname="", exp_type=None):
    ...
    xlabels = np.linspace(
        v["HCF"] - v["HSW"] / 2.0, v["HCF"] + v["HSW"] / 2.0, xpoints
    )
    if len(data.dimlabels) > 1:
        ...
        if dimname == "mw-power-sweep":
            ...
            yaxis = 10 ** (-yaxis / 10.0)  # convert to linear power
    ...
    for line in lines:
        ...
        if m is None:
            raise RuntimeError(
                "Warning:",
                lsafen(repr(line)),
                "does not appear to be a valid WinEPR format line, and I"
                " suspect this is a problem with the terminators!",
            )
        else:
            name = m.groups()[0]
            value = m.groups()[1]
        ...
        if len(x):
            try:
                result = np.genfromtxt(
                    StringIO(x), dtype=None, encoding="utf-8"
                ).tolist()
            except Exception:
            ...
            if m:
                ...
                if m:
                    ...
                    if which_block is None:
                        raise ValueError(
                            "Appears to be stuff outside the first hashed"
                            " block which, as far as I know, should not be"
                            " allowed.  The first non-comment line I see is: "
                            + repr(line)
                        )
                    else:
