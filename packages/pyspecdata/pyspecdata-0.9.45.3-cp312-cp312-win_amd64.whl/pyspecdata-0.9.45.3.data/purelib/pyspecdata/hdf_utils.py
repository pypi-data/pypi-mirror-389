...
def gensearch(labelname, format="%0.3f", value=None, precision=None):
    ...
    if precision is None:
        precision = (
            value * 0.01
        )  # the precision is 1% of the value, if we don't give an argument
...
def h5searchstring(*args, **kwargs):
    ...
    search AROUND a certain value (overcomes some type conversion issues)
    optional arguments are the format specifier and the fractional
    precision:
    **OR**
    :parameter field_and_value_dictionary:
    generate a search string that matches one or more criteria"""
    format, precision = process_kwargs(
        [("format", "%g"), ("precision", 0.01)], kwargs
    )
    if len(args) == 2:
        ...
        for k, v in dict_arg.items():
            condlist.append(
                h5searchstring(k, v, format=format, precision=precision)
            )
    ...
    if isinstance(value, str):
        raise ValueError(
            "string matching in pytables is broken -- search by hand and then"
            " use the index"
        )
    precision *= value
...
def h5loaddict(thisnode):
    ...
    retval = dict([
        (x, thisnode._v_attrs.__getattribute__(x))
    ...
    for (
        ...
        retval.items()
    ):  # {{{ search for record arrays that represent normal lists
    ...
    if isinstance(thisnode, tables.table.Table):  # {{{ load any table data
        ...
        if "data" in list(retval.keys()):
            raise AttributeError(
                "There's an attribute called data --> this should not happen!"
            )
        ...
        for thischild in list(mychildren.keys()):
            if thischild in list(retval.keys()):
                raise AttributeError(
                    ...
                    thischild,
                    " and also a sub-node called the",
                    thischild,
        ...
        raise AttributeError(
            strm("I don't know what to do with this node:", thisnode)
        )
    # }}}
    return retval
...
def h5child(thisnode, childname, clear=False, create=None):
    r"""grab the child, optionally clearing it and/or (by default)
    creating it"""
    # {{{ I can't create and clear at the same time
    if create and clear:
        raise ValueError(
            "You can't call clear and create at the same time!\nJust call"
            " h5child twice, once with clear, once with create"
        )
    if create is None:
        if clear is True:
            create = False
    ...
    try:
        ...
        if create is False and not clear:
            top_level_node_names = [
                node._v_name for node in h5file.list_nodes("/")
            ]
            raise RuntimeError(
                "Trying to grab a node that does not exist with create ="
                " False.\nHere are the nodes available:"
                + str(top_level_node_names)
            )
        elif clear:
            childnode = None
        else:
...
def h5remrows(bottomnode, tablename, searchstring):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                strm(
                    "Problem trying to remove rows using search string",
                    searchstring,
                    "in",
                    thistable,
                )
        ...
        for row in thistable.where(searchstring):
            if len(thistable) == 1:
                ...
                try:
                    thistable.remove_rows(
                        row.nrow - counter, row.nrow - counter + 1
                    )  # counter accounts for rows I have already removed.
                except Exception:
                    ...
                    raise RuntimeError(
                        "length of thistable is "
                        + repr(len(thistable))
                        + " calling remove_rows with "
                        + repr(row.nrow - counter)
                    )
                counter += 1
        return counter, data
    except tables.NoSuchNodeError:
...
def h5addrow(bottomnode, tablename, *args, **kwargs):
    """add a row to a table, creating it if necessary, but don\'t add if
    the data matches the search condition indicated by `match_row`
    `match_row` can be either text or a dictionary -- in the latter case
    it's passed to h5searchstring
    """
    match_row, only_last = process_kwargs(
        [("match_row", None), ("only_last", True)], kwargs
    )
    ...
    if tableexists:
        ...
        if match_row is not None:
            ...
            logger.debug(
                strm("trying to match row according to", lsafen(match_row))
            )
            mytable.flush()
            try:
                ...
                if only_last:
                    logger.debug(
                        strm(
                            ...
                            lsafen(
                                len(matches),
                                "rows match your search criterion, returning"
                                " the last row",
                            ),
                            "}",
                        )
    ...
    if len(args) == 1 and (isinstance(args[0], dict)):
        listofnames, listofdata = list(
            map(list, list(zip(*tuple(args[0].items()))))
        )
    elif (
        len(args) == 2
        ...
        raise TypeError(
            "h5addrow takes either a dictionary for the third argument or a"
            " list for the third and fourth arguments"
        )
    try:
        listofdata = [newindex] + listofdata
    except Exception:
        raise TypeError(
            "newindex is" + repr(newindex) + "listofdata is" + repr(listofdata)
        )
    ...
    if tableexists:
        try:
            ...
            print(
                lsafen(
                    "I'm about to flag an error, but it looks like there was"
                    " an issue appending",
                    myrowdata,
                )
            )
                ...
                strm(
                    ...
                    "\n".join(
                        map(
                            ...
                            list(
                                zip(
                                    list(mytable.read().dtype.fields.keys()),
        ...
        try:
            mytable = h5table(bottomnode, tablename, recorddata)
        except Exception:
            raise RuntimeError(
                strm(
                    "Error trying to write record np.array:",
                    repr(recorddata),
                    "from listofdata",
                    listofdata,
                    "and names",
            ...
            raise RuntimeError(
                " ".join(
                    map(
                        str,
                        [
                            "You passed no data, so I can't create table",
                            tablename,
                            "but it doesn't exist in",
                            bottomnode,
                            "which has children",
                            list(bottomnode._v_children.keys()),
                        ],
                    )
                )
            )
...
def h5nodebypath(
    ...
    if h5path[0] in listdir(directory):
        ...
        if check_only:
            errmsg = log_fname(
                ...
                h5path[0],
                directory,
                unknown_exp_type_name,
            )
            raise AttributeError(
                "You're checking for a node in a file (%s) that does not exist"
                % (h5path[0])
                + "\n"
                + errmsg
            )
    ...
    try:
        h5file = tables.open_file(
            os.path.join(directory, h5path[0]), mode=mode, title="test file"
        )
    except IOError:
        raise IOError(
            "I think the HDF5 file has not been created yet, and there is a"
            " bug pytables that makes it freak out, but you can just run"
            " again."
        )
    # }}}
    ...
    logger.debug(
        strm(
            ...
            "of file",
            h5file,
            "ready to step down search path",
        )
    )
    for pathlevel in range(1, len(h5path)):  # {{{ step down the path
        ...
        try:
            ...
            logger.debug(
                strm(
                    lsafen(
                        "searching for node path: descended to node",
                        currentnode,
                    )
                )
            )
            logger.debug(
                strm("searching for node path: descended to node", currentnode)
            )
        except BaseException:
            logger.debug(
                strm(
                    lsafen(
                        "searching for node path: got caught searching for"
                        " node",
                        h5path[pathlevel],
                    )
                )
            ...
            logger.debug(
                strm(
                    "searching for node path: got caught searching for node",
                    h5path[pathlevel],
                )
            )
            h5file.close()
...
def h5attachattributes(node, listofattributes, myvalues):
    listofattributes = [
        j
        for j in listofattributes  # need to exclude the properties
    ...
    if hasattr(myvalues, "dimlabels"):  # use as a proxy for being nddata
        ...
        raise TypeError(
            "I don't understand the type of myvalues, which much be a list or"
            " a nddata object, from which the attribute values are retrieved"
        )
    ...
    for j, thisattr in enumerate(listofattributes):
        ...
        if type(thisval) in [dict]:
            dictnode = h5child(node, thisattr, clear=True)
            dictnode = h5child(node, thisattr, create=True)
            h5attachattributes(
                dictnode, list(thisval.keys()), list(thisval.values())
            )
            thisval = None
            ...
            if type(thisval) in [list, tuple]:
                if np.any([isinstance(x, str) for x in thisval]):
                    logger.debug(
                        strm("going to convert", thisval, "to strings")
                    )
                    thisval = [
                        str(x) if isinstance(x, str) else x for x in thisval
                    ]
        ...
        if thisval is not None:
            try:
                ...
                raise RuntimeError(
                    "PyTables freaks out when trying to attach attribute "
                    + repr(thisattr)
                    + " with value "
                    + repr(thisval)
                    + "\nOriginal error was:\n"
...
def h5inlist(columnname, mylist):
    ...
    if np.any([type(x) in [np.double, np.float64] for x in mylist]):
        if all([
            type(x) in [np.double, np.float64, int, np.int32, np.int64]
            for x in mylist
        ]):
            return (
                "("
                + "|".join(["(%s == %g)" % (columnname, x) for x in mylist])
                + ")"
            )
        ...
        return (
            "("
            + "|".join(["(%s == '%s')" % (columnname, x) for x in mylist])
            + ")"
        )
    else:
        raise TypeError(
            "I can't figure out what to do with this list --> I know what to"
            " do with a list of numbers or a list of strings, but not a list"
            " of type"
            + repr(list(map(type, mylist)))
        )
...
def h5join(
    ...
    # {{{ process the first argument as the hdf5 table and indices, and
    #     process the second one as the structured np.array to join onto
    if not (
        (isinstance(firsttuple, tuple)) and (isinstance(secondtuple, tuple))
    ):
        raise ValueError("both the first and second arguments must be tuples!")
    if not ((len(firsttuple) == 2) and (len(secondtuple) == 2)):
        raise ValueError(
            "The length of the first and second arguments must be two!"
        )
    ...
    if len(tableindices) != len(mystructarrayindices):
        raise ValueError(
            "You must pass either a string or a list for the second element of"
            " each tuple!\nIf you pass a list, they must be of the same"
            " length, since the field names need to line up!"
        )
    # this can't use h5inlist, because the and needs to be on the inside
    # {{{ this loop creates a list of lists, where the inner lists are a
    #     set of conditions that need to be satisfied this is actually not
    #     causing  any trouble right now, but needs to be fixed, because
    #     of the way that it's doing the type conversion
    for thistableindex, thisstructarrayindex in zip(
        tableindices, mystructarrayindices
    ):
        if thisstructarrayindex not in mystructarray.dtype.names:
            raise ValueError(
                repr(thisstructarrayindex)
                + " is not in "
                + repr(mystructarray.dtype.names)
            )
        if isinstance(mystructarray[thisstructarrayindex][0], str):
            search_string.append([
                "(%s == '%s')" % (thistableindex, x)
                for x in mystructarray[thisstructarrayindex]
            ])
        elif type(mystructarray[thisstructarrayindex][0]) in [
            int,
            np.double,
            float,
            np.float64,
            ...
            search_string.append([
                "(%s == %s)" % (thistableindex, str(x))
                for x in mystructarray[thisstructarrayindex]
            ])
        else:
            raise TypeError(
                "I don't know what to do with a structured np.array that has a"
                " row of type"
                + repr(type(mystructarray[thisstructarrayindex][0]))
            )
    # }}}
    search_string = [
        " & ".join(x) for x in zip(*tuple(search_string))
    ]  # this "and"s together the inner lists, since all conditions must
    #    be matched
    search_string = (
        "(" + "|".join(search_string) + ")"
    )  # then, it "or"s the outer lists, since I want to collect data for
    ...
    try:
        retval = decorate_rec(
            (retval, tableindices), (mystructarray, mystructarrayindices)
        )  # this must be the problem, since the above looks fine
    except Exception:
        raise Exception(
            strm(
                "Some problems trying to decorate the table",
                retval,
                "of dtype",
                retval.dtype,
                "with the structured np.array",
                mystructarray,
                "of dtype",
                mystructarray.dtype,
            )
    ...
    if pop_fields is not None:
        if select_fields is not None:
            raise ValueError(
                "It doesn't make sense to specify pop_fields and select_fields"
                " at the same time!!"
            )
