...
def nnls_regularized(A, b, l=0, maxiter=None):
    ...
    if len(b.shape) > 2:
        raise ValueError(
            "Expected vector! It's allowed to have indirect, but you gave me, "
            + str(b.shape)
        )
    ...
    if m != b.shape[-1]:
        raise ValueError(
            strm(
                "incompatible dimensions (rows of A",
                m,
                " do not match size of data",
                b.shape,
                ")",
            )
        )
    ...
    if isscalar(l):
        if l == 0.0:
            ...
            logger.debug(
                "about to execute a non-regularized (least squares) fit"
            )
            ...
            if len(b.shape) == 1:
                logger.debug(
                    "about to execute a regularized fit using the compiled"
                    " nnls_regularized routine"
                )
        ...
        logger.debug(
            "about to run a parallelized loop over different values of l"
        )
        nCPU = cpu_count()
        p = mpd.Pool(nCPU)
        if len(b.shape) == 1:
