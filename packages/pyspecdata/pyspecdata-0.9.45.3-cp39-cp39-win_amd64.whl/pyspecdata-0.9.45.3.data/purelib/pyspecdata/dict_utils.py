...
def make_ndarray(array_to_conv, name_forprint="unknown"):
    if type(array_to_conv) in [
        ...
        array_to_conv = rec.fromarrays(
            [array_to_conv], names="LISTELEMENTS"
        )  # list(rec.fromarrays([b])['f0']) to convert back
    elif type(array_to_conv) in [list, np.ndarray] and len(array_to_conv) == 0:
        array_to_conv = None
        ...
        raise TypeError(
            strm(
                ...
                ") for attribute name",
                name_forprint,
                "passed to make_ndarray is not currently supported",
            )
        )
...
def unmake_ndarray(array_to_conv, name_forprint="unknown"):
    ...
    if (isinstance(array_to_conv, np.recarray)) or (
        isinstance(array_to_conv, np.ndarray)
        and array_to_conv.dtype.names is not None
        and len(array_to_conv.dtype.names) > 0
    ):
        # {{{ if it's a record/structured np.array, it should be either a list
        #     or dictionary
        if "LISTELEMENTS" in array_to_conv.dtype.names:
            if array_to_conv.dtype.names == tuple(["LISTELEMENTS"]):
                ...
                raise ValueError(
                    strm(
                        ...
                        "also has other dimensions:",
                        array_to_conv.dtype.names,
                        "not",
                        tuple(["LISTELEMENTS"]),
                    )
                )
            ...
            retval = dict(
                list(zip(array_to_conv.dtype.names, array_to_conv.tolist()[0]))
            )
        else:
            raise ValueError(
                "You passed a structured np.array, but it has more",
        ...
        logger.debug(
            strm(
                ...
                "=",
                type(array_to_conv),
                "is a numpy np.array of length one",
            )
        )
        # }}}
        ...
        logger.debug(
            strm(
                "name_forprint", "=", type(array_to_conv), "is a numpy scalar"
            )
        )
        # }}}
        ...
        if all([x is np.bytes_ for x in typeofall]):
            ...
            logger.debug(
                strm(name_forprint, "=", typeofall, "are not all numpy string")
            )
        ...
        logger.debug(
            strm(
                ...
                "=",
                type(array_to_conv),
                "is not a numpy string or record np.array",
            )
        )
        retval = array_to_conv
    return retval
