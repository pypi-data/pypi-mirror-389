        ...
        def on_modified(self, event):
            ...
            if event is None:  # if I called it manually
                ...
                if event.src_path in [
                    "." + os.path.sep + j for j in self.dependencies
                ]:
                    print(
                        "Scons signaled on",
                        event.src_path,
                    ...
                    print(
                        "Modified", event.src_path, "which is not a dependency"
                    )
                    runornot = False
            if runornot:
                outstring = check_output(
                    ["scons", "--tree=all"] + sys.argv[1:], shell=True
                )
                # print "outstring:\n",outstring
                self.dependencies = []
                for line in outstring.split(os.linesep):
                    # here, I grab the tree, and then I should be able to use
                    # watchdog to implement latexmk-like functionality
                    x = line.find("+-")
                    if x > -1:
                        x += 1
                        while line[x] == "-":
...
def det_new_pdf_name(thisargv):
    """based on an original tex or pdf name, determine the original basename
    (i.e., no extension), as well as one with the final word after the
    underscore removed"""
    tex_basename = list(filter(lambda x: x[0] != "-", thisargv))[-1]
    ...
    if (len(tex_basename) > 1) and (
        tex_basename[-1] in ["basic", "fancy", "georgia"]
    ):
...
def wraplatex():
    """runs the python scripts after running latex also creates a copy of latex
    without the final portion under the underscore This prevents the viewer
    from hanging while it's waiting for a refresh. This can be used in
    combination with wrapviewer() and latexmk by using a ``~/.latexmkrc`` file
    that looks like this:

    If you pass the ``--xelatex`` argument, xelatex is used instead of pdflatex
    (note that if you're using latexmk, you need to add this in the latexmkrc
    file).

    .. code-block:: perl

        $pdflatex=q/pdflatex_notebook_wrapper %O -synctex=1 %S/;# calls this
        function $pdf_previewer=q/pdflatex_notebook_view_wrapper/;# calls the
        wrapviewer function
    """
    ...
    with open(orig_tex_basename + ".tex", "r", encoding="utf-8") as fp:
        ...
        while thisline.startswith(
            "%!"
        ):  # in case we want to allow multiple directives
    ...
    if orig_tex_basename != new_pdf_basename:
        print(
            "preparing to:",
            "cp " + orig_tex_basename + ".pdf " + new_pdf_basename + ".pdf",
        )
        os.system(
            "cp " + orig_tex_basename + ".pdf " + new_pdf_basename + ".pdf"
        )
...
def wrapviewer():
    ...
    if os.name == "posix":
        # {{{ this plays the role of the function that I used to call
        #     "new_evince" with argument "b"
        ...
        os.system(
            "start sumatrapdf -reuse-instance " + new_pdf_basename + ".pdf"
        )
    if new_pdf_basename == "lists":
        os.system(
            'cp lists.pdf "'
            + os.path.join(os.path.expanduser("~"), "Dropbox", "lists.pdf")
        )
    return
...
def cached_filename(hashstring, returndir=False):
    ...
    we use the first two characters as directory names (so there are just 16 of
    them
    """
    return (
        get_scripts_dir()
...
def flush_script(number):
    tex_name = os.path.normpath(
        os.path.join(os.getcwd(), "scripts", number + ".tex")
    )
...
if haswatchdog:
    ...
    def repeat_scons():
        r"""This is just a function to run scons over and over again (e.g. so
        it can replace latexmk)"""
        for j in os.listdir("."):
            if j.endswith(".sconslog"):
                os.remove(
                    j
                )  # by removing these, we force the PDF reader to open
        observer = Observer()
        ...
        for j in dependencies:
            ...
            observer.schedule(
                myhandler, path=os.path.dirname(fname), recursive=False
            )
        observer.start()
        try:
            while True:
...
def main():
    r"""This looks for `scripts/scriptsUsed.csv` inside the notebook directory,
    ...
    if not os.path.exists(get_scripts_dir()):
        return
