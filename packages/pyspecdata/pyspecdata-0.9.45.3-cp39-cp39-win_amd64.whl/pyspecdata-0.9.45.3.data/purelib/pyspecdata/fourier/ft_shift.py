...
def _ft_shift(self, thisaxis, p2, shift_axis=None, verbose=False):
    (
        ...
        " first index\n this is derived from the numpy fftshift routine, but"
        " defines slices instead of index numbers\n `shift_axis` is only used"
        " after the (i)fft.  It assumes that the axis labels start at zero,"
        " and it aliases them over in the same way the data was aliased"
    )
...
def ft_new_startpoint(self, axis, which_domain, value=None, nearest=False):
    ...
    which_domain: "t" or "f"
        ...
        cm,cm⁻¹ or u,B₀, etc.)
    value: float, 'current', 'reset', or None
        can be set by (1) manually setting
        the start point (2) 'reset', which
        clears the startpoint, allowing the
        (I)FT to decide from scratch
    nearest: bool
    ...
    if orig_u is None:
        if self.get_ft_prop(axis) is thisconditional:
            ...
            raise ValueError(
                "I don't know"
                "how to pick the startpoint if"
                "it's not given and I'm not in"
                "that dimension"
            )
            ...
            if nearest is None:
                raise ValueError(
                    strm(
                        ...
                        " the start point from",
                        orig_u,
                        "to",
                        value,
                        "which is a non-integral number ",
                        abs(orig_u - value) / du,
                        " of du=",
                        du,
                        "intervals (n_du ",
