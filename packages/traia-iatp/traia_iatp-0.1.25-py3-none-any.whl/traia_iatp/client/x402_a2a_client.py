"""X402-enabled A2A client for IATP with payment support."""
...
from ..x402.client import X402IATPClient, create_iatp_payment_client
from ..x402.models import X402PaymentInfo
...
class X402A2AClient:
    """A2A client with x402 payment support for IATP.
    
    This client automatically handles x402 payments when communicating with
    ...
    def __init__(
        self,
        agent_endpoint: str,
        payment_client: Optional[X402IATPClient] = None,
        max_payment_usd: Optional[float] = None
    ):
        """Initialize the x402-enabled A2A client.
        
        Args:
            agent_endpoint: URL of the utility agent
            payment_client: Optional pre-configured X402IATPClient
        ...
        # Extract x402 payment information if available
        self.x402_info = self._extract_x402_info()
        
        # Initialize A2A client
        self.a2a_client = A2AClient(
            agent_card=self.agent_card,
            credentials=None  # x402 handles payment authorization
        )
    
    def _extract_x402_info(self) -> Optional[X402PaymentInfo]:
        """Extract x402 payment information from agent card."""
        metadata = self.agent_card.metadata or {}
        x402_data = metadata.get("x402")
        
        if x402_data and x402_data.get("enabled"):
            return X402PaymentInfo(**x402_data)
        return None
    ...
    async def send_message_with_payment(
        ...
        """Send a message to the agent, automatically handling x402 payment if required.
    ...
    async def _handle_payment_required(
        ...
        if not self.payment_client:
            raise ValueError(
                ...
                "Please provide a payment_client when initializing X402A2AClient."
            )
        
        # Parse payment requirements
        try:
            payment_response = x402PaymentRequiredResponse(**response.json())
