"""Example: Utility agent with x402 payment support."""
...
x402_config = X402Config(
    ...
    skill_prices={
        "sentiment_analysis": X402ServicePrice(
            usd_amount="0.01",
            network="base-mainnet",
            asset_address="0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",
            max_timeout_seconds=300
        ),
        "entity_extraction": X402ServicePrice(
            usd_amount="0.02",
            network="base-mainnet",
            asset_address="0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",
            max_timeout_seconds=300
    ...
    facilitator_url=os.getenv("FACILITATOR_URL", "https://api.traia.io/x402/facilitator"),
...
async def agent_card():
    """Return agent card with x402 payment information."""
    card = {
    ...
    # Add x402 payment information
    card = await add_x402_info_to_agent_card(card, x402_config)
    return card
...
def main():
    ...
    print(f"X402 Enabled: {x402_config.enabled}")
    print(f"Pay-to Address: {x402_config.pay_to_address}")
    print(f"Default Price: ${x402_config.default_price.usd_amount} USD")
    print(f"Facilitator: {x402_config.facilitator_url}")
    print()
