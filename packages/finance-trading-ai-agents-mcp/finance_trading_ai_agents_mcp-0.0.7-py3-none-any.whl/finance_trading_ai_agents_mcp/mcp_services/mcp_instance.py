...
async def combined_lifespan(app: FastAPI):
    async with AsyncExitStack() as stack:
        ...
        for custom_mcp_app in custom_mcp_server._mcp_apps:
...
def add_custom_mcp():
    for custom_mcp in  custom_mcp_server.mcp_list:
        server_name=custom_mcp.name or "unknown name"
        name_=server_name.replace(' ', '_')
        name=name_.lower()
        ...
        custom_mcp_server._mcp_apps.append(custom_mcp_app)
