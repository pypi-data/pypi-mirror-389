...
| `resume_errors()`  | Resume or restart calculations with errors in a given folder |  
...
def resume_unfinished(
    ...
    for calc in supercells_out:
        ...
        if data['Success']:
            ...
            continue
        print(f'  x   {basename}')
        unfinished.extend(basename)
    for calc in supercells_in:
        if not calc in finished or not calc in unfinished:
            not_started.append(calc)
    ...
    if not testing:
        api_slurm.sbatch(files=unfinished, template=template, folder=folder)
    return unfinished
