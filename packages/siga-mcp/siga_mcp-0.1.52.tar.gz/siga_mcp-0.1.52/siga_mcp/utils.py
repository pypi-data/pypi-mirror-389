...
def send_message_to_user(user_id: str, message_text: str):
    ...
    if message_response.status_code == 201:
        print("Mensagem enviada com sucesso para o usuário!")
...
def montar_string(items: dict[str, Any], separator: str = "-") -> str:
    ...
    return f"\n        {separator} ".join(
        [f'"{key}" (Código {value})' for key, value in items.items()]
    )
...
def normalizar_parametro(
    ...
    # Busca o tipo correto na constante ignorando maiúsculas/minúsculas
    ...
    tipo_final = constant_mapping[parametro_normalizado]
    return tipo_final
