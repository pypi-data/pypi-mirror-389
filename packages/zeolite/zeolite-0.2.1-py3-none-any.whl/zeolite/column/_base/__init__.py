...
class Col:
    ...
    def temporary(self, is_temp: bool = True) -> "Col":
        """Mark this column as temporary, excluding it from the final refined output.

        Args:
            is_temp: Whether the column should be temporary (default True).

        Returns:
            Col: A new Col instance with the temporary flag set.

        Note:
            If an alias is set on this column, it will override the temporary flag
            and the column will be included in the output.
        """
    ...
    def get_nodes(self, schema: str, stage: str | None = None) -> List[ColumnNode]:
        ...
        for v in p.validations:
            nodes.append(v.get_validation_node(ref, is_temporary=True))
