...
mcp = FastMCP(
    ...
    dependencies=["pyautogui", "Pillow", "pyperclip"],
)
...
def type_text(
    ...
    interval: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Delay between characters to mimic natural typing (ignored when using clipboard paste).",
    ),
) -> Dict[str, str]:
    """Type a string of characters."""
    try:
        _release_modifiers()
        # Use clipboard paste method to avoid "stuck shift" issues with special characters
        # This is more reliable than typewrite() especially on macOS
        pyperclip.copy(text)
        time.sleep(0.05)  # Brief pause to ensure clipboard is ready

        # Use appropriate paste command based on platform
