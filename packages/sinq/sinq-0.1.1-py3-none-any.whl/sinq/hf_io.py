...
def _detect_sinq_from_quant_obj(qobj: Any) -> bool:
    ...
    if isinstance(qobj, dict):
        ...
        for key in ("method", "backend", "approach", "name", "quantization_method", "quantization_method", "nbits", "group_size"):
            if hasattr(qobj, key):
                try:
...
def _download_if_repo_id(
    ...
    *,
    ...
    print(f"[SINQ IO] cfg_is_sinq={cfg_is_sinq}, use_sinq_downloader={use_sinq_downloader}", flush=True)
...
class SinqHFIO:
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        use_sinq = _is_sinq_sharded_dir(local_path) or _path_has_sinq_flag(local_path)
        print(f"[SINQ IO] use_sinq_loader={use_sinq}", flush=True)
...
def patch_hf_pretrained_io() -> None:
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        ...
        _from_pretrained.__func__.__module__ = "sinq.hf_io" # optional, but makes prints clearer
        ...
        try:
            ...
            if _is_sinq_sharded_dir(local_path) or _path_has_sinq_flag(local_path):
        ...
        try:
            ...
            def _wrap_file(orig):
                def _w(*args, **kwargs):
                    ...
                    hinted = kwargs.pop("_hinted_sinq", False)
                    # If filename is config.json we can let HF handle it normally;
                    # custom downloader is mainly for full snapshots. Keep it simple.
                    if filename and os.path.basename(filename) == HF_CONFIG:
                        return orig(*args, **kwargs)
                    # Try custom downloader if hinted (by env/repo naming) to fetch single files as well
