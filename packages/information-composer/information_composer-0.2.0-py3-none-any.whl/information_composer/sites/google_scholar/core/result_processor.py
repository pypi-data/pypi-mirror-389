...
class ResultProcessor:
    ...
    def _clean_single_paper(self, paper: GoogleScholarPaper) -> GoogleScholarPaper:
        ...
        # Clean DOI
        doi = self._clean_doi(paper.doi) if paper.doi else None
    ...
    def _clean_venue(self, venue: str) -> Optional[str]:
    ...
    def _clean_doi(self, doi: str) -> Optional[str]:
        ...
        if doi_match:
            cleaned_doi = doi_match.group(0)
            # Remove trailing punctuation
            cleaned_doi = re.sub(r"[.,;:]+$", "", cleaned_doi)
            return cleaned_doi

        return None

    def _extract_arxiv_id(self, paper: GoogleScholarPaper) -> Optional[str]:
    ...
    async def _resolve_doi(self, paper: GoogleScholarPaper) -> Optional[str]:
    ...
    async def _find_pubmed_id(self, paper: GoogleScholarPaper) -> Optional[str]:
