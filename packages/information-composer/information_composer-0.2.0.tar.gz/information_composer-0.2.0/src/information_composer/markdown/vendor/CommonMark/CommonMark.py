...
This module implements a parser and renderer for the CommonMark markdown specification.
It is a port of @jgm's JavaScript stmd.js implementation.
"""
...
def ASTtoJSON(block: Block) -> str:
    ...
    def prepare(block: Block) -> Block:
        ...
        if (
            "label" in block.__dict__
            and isinstance(block.label, list)
            and len(block.label) > 0
        ):
            for i, child in enumerate(block.label):
                if isinstance(child, Block):
                    block.label[i] = prepare(child)
        if "c" in block.__dict__ and isinstance(block.c, list) and len(block.c) > 0:
    ...
    return json.dumps(
        prepare(block), default=lambda o: o.__dict__
    )  # sort_keys=True) # indent=4)


def dumpAST(obj: Block, ind: int = 0) -> None:
    ...
    if obj.label:
        ...
        if isinstance(obj.label, list):
            ...
            print("\t" + indChar + "Label: " + str(obj.label))
...
def detabLine(text: str) -> str:
    ...
    if re.match("\t", text) and text.index("\t") == -1:
        ...
        def tabber(m: re.Match[str]) -> str:
            nonlocal last_stop
            result = "    "[(m.end() - 1 - last_stop) % 4 :]
            last_stop = m.end()
            return result
...
class Block:
    """Base class for block-level elements in the AST."""

    def __init__(
        self,
        t: str = "",
        c: Union[str, List[Block]] = "",
        destination: str = "",
        label: Union[str, List[Block]] = "",
        start_line: int = 0,
        start_column: int = 0,
        title: str = "",
    ) -> None:
        ...
        Args:
            ...
            label: Link label (can be string or list of Blocks)
        ...
        self.inline_content: List[Block] = []
        self.list_data: Dict[str, Any] = {}
        self.title = title
        self.info = ""
        self.tight = False
        # Additional attributes for specific block types
        self.level: int = 0
        self.fence_offset: int = 0
        self.fence_char: str = ""
        self.fence_length: int = 0

    @staticmethod
    def makeBlock(tag: str, start_line: int, start_column: int) -> Block:
        """Create a new Block with the given tag and position."""
...
class InlineParser:
    ...
    def match(self, regexString: str, reCompileFlags: int = 0) -> Optional[str]:
        """If re matches at current position in the subject, advance
        position in subject and return the match; otherwise return null."""
    ...
    def parseBackticks(self, inlines: List[Block]) -> int:
    ...
    def parseEscaped(self, inlines: List[Block]) -> int:
    ...
    def parseAutoLink(self, inlines: List[Block]) -> int:
    ...
    def parseHtmlTag(self, inlines: List[Block]) -> int:
        """Attempt to parse a raw HTML tag."""
        m = self.match(reHtmlTag.pattern)
        if m:
    ...
    def scanDelims(self, c: str) -> Dict[str, Any]:
        ...
        can_open = (
            (numdelims > 0) and (numdelims <= 3) and (not re.match(r"\s", char_after))
        )
        can_close = (
            (numdelims > 0) and (numdelims <= 3) and (not re.match(r"\s", char_before))
        )

        if c == "_":
            can_open = can_open and (
                not re.match(r"[a-z0-9]", char_before, re.IGNORECASE)
            )
            can_close = can_close and (
                not re.match(r"[a-z0-9]", char_after, re.IGNORECASE)
            )
        self.pos = startpos
        return {"numdelims": numdelims, "can_open": can_open, "can_close": can_close}

    def parseEmphasis(self, inlines: List[Block]) -> int:
        ...
        if startpos > 0:
            inlines.append(
                Block(
                    t="Str", c=self.subject[self.pos - numdelims : numdelims + startpos]
                )
            )
        else:
            inlines.append(
        ...
        if numdelims == 1:
            while True:
                ...
                if res["numdelims"] >= 1 and res["can_close"]:
                    ...
                    if len(inlines) > 1:
                        for _x in range(delimpos + 1, len(inlines)):
            ...
            while True:
                ...
                if (
                    ...
                    if first_close_delims == 1 and numdelims > 2:
                        res["numdelims"] = 2
                    elif first_close_delims == 2 or res["numdelims"] == 3:
                        res["numdelims"] = 1
                    self.pos += res["numdelims"]

                    if first_close > 0:
                        inlines[delimpos].t = (
                            "Strong" if first_close_delims == 1 else "Emph"
                        )
                        temp = "Emph" if first_close_delims == 1 else "Strong"
                        inlines[delimpos].c = [
                            Block(t=temp, c=inlines[delimpos + 1 : first_close])
                        ] + inlines[first_close + 1 :]
                        if len(inlines) > 1:
                            for _x in range(delimpos + 1, len(inlines)):
                                inlines.pop(len(inlines) - 1)
                        break
                        ...
                        inlines.append(
                            Block(
                                t="Str",
                                c=self.subject[self.pos - res["numdelims"] : self.pos],
                            )
                        )
    ...
    def parseLinkTitle(self) -> Optional[str]:
        """Attempt to parse link title (sans quotes), returning the string
        or null if no match."""
        title = self.match(reLinkTitle.pattern)
        if title:
            return unescape(title[1 : len(title) - 1])
        else:
            return None

    def parseLinkDestination(self) -> Optional[str]:
        """Attempt to parse link destination, returning the string or
        null if no match."""
        res = self.match(reLinkDestinationBraces.pattern)
        if res is not None:
            return unescape(res[1 : len(res) - 1])
        else:
            res2 = self.match(reLinkDestination.pattern)
            if res2 is not None:
    ...
    def parseRawLabel(self, s: str) -> List[Block]:
    ...
    def parseLink(self, inlines: List[Block]) -> int:
    ...
    def parseEntity(self, inlines: List[Block]) -> int:
    ...
    def parseString(self, inlines: List[Block]) -> int:
    ...
    def parseNewline(self, inlines: List[Block]) -> int:
    ...
    def parseImage(self, inlines: List[Block]) -> int:
    ...
    def parseReference(self, s: str, refmap: Dict[str, Dict[str, str]]) -> int:
    ...
    def parseInline(self, inlines: List[Block]) -> int:
    ...
    def parseInlines(
        self, s: str, refmap: Optional[Dict[str, Dict[str, str]]] = None
    ) -> List[Block]:
...
class DocParser:
    def __init__(self, subject: Optional[str] = None, pos: int = 0) -> None:
    ...
    def acceptsLines(self, block_type: str) -> bool:
        """Returns true if block type can accept lines of text."""
        return (
            block_type == "Paragraph"
            or block_type == "IndentedCode"
            or block_type == "FencedCode"
        )

    def endsWithBlankLine(self, block: Block) -> bool:
    ...
    def breakOutOfLists(self, block: Block, line_number: int) -> None:
        ...
        if last_list:
            while not block == last_list:
                ...
                block = block.parent  # type: ignore[assignment]
            self.finalize(last_list, line_number)
            self.tip = last_list.parent  # type: ignore[assignment]

    def addLine(self, ln: str, offset: int) -> None:
    ...
    def addChild(self, tag: str, line_number: int, offset: int) -> Block:
    ...
    def listsMatch(self, list_data: Dict[str, Any], item_data: Dict[str, Any]) -> bool:
    ...
    def parseListMarker(self, ln: str, offset: int) -> Optional[Dict[str, Any]]:
        ...
        if match:
            ...
            blank_item = len(match2.group(0)) == len(rest)
        else:
            return None
    ...
    def incorporateLine(self, ln: str, line_number: int) -> None:
        ...
        container: Optional[Block] = self.doc
        oldtip: Optional[Block] = self.tip

        ln = detabLine(ln)

        while container and len(container.children) > 0:
            ...
            if container.t == "BlockQuote":
                ...
                if blank:
                    ...
                    all_matched = False
            if not all_matched:
                container = container.parent if container else None  # type: ignore[assignment]
                break
        last_matched_container: Optional[Block] = container

        def closeUnmatchedBlocks(
            self, already_done: bool, oldtip: Optional[Block]
            ...
            while (
                not already_done
                and oldtip
                and last_matched_container
                and not oldtip == last_matched_container
            ):
                self.finalize(oldtip, line_number)
                oldtip = oldtip.parent
            return True, oldtip

        if blank and container and container.last_line_blank:
            self.breakOutOfLists(container, line_number)
        while (
            container
            and not container.t == "FencedCode"
            and not container.t == "IndentedCode"
            and not container.t == "HtmlBlock"
            and matchAt(r"^[ #`~*+_=<>0-9-]", ln, offset) is not None
        ):
            ...
            if indent >= CODE_INDENT:
                ...
                if re.search(r"\\#", ln[offset:]) is not None:
                    container.strings = [
                        re.sub(r"(?:(\\#) *#*| *#+) *$", r"\g<1>", ln[offset:])
                    ]
                else:
                    container.strings = [
                        re.sub(r"(?:(\\#) *#*| *#+) *$", "", ln[offset:])
                ...
                break
            elif matchAt(reHtmlBlockOpen.pattern, ln, first_nonspace) is not None:
                already_done, oldtip = closeUnmatchedBlocks(self, already_done, oldtip)
                container = self.addChild("HtmlBlock", line_number, first_nonspace)
                break
            elif (
                container.t == "Paragraph" and len(container.strings) == 1 and PARmatch
            ):
                already_done, oldtip = closeUnmatchedBlocks(self, already_done, oldtip)
                container.t = "SetextHeader"
                container.level = 1 if PARmatch.group(0)[0] == "=" else 2
                offset = len(ln)
            elif matchAt(reHrule.pattern, ln, first_nonspace) is not None:
                already_done, oldtip = closeUnmatchedBlocks(self, already_done, oldtip)
                ...
                break
            if container and self.acceptsLines(container.t):
                break
        ...
        if (
            ...
            container.last_line_blank = blank and not (
                ...
                or (
                    ...
                    and len(container.children) == 0
                    and container.start_line == line_number
                )
            )
            cont = container
            while cont.parent:
            ...
            if container.t == "IndentedCode" or container.t == "HtmlBlock":
                ...
                if (
                    match
                    and FENmatch
                    and len(FENmatch.group(0)) >= container.fence_length
                ):
                ...
                if self.acceptsLines(container.t):
                    ...
                    not container.t == "HorizontalRule"
                    and not container.t == "SetextHeader"
                ):
    ...
    def finalize(self, block: Block, line_number: int) -> None:
        ...
        if block.t == "Paragraph":
            ...
            while i < numitems:
                ...
                while j < numsubitems:
                    ...
                    if self.endsWithBlankLine(subitem) and not (
                        last_item and last_subitem
                    ):
                        block.tight = False
                        break
                    j += 1
        ...
        self.tip = block.parent  # type: ignore[assignment]

    def processInlines(self, block: Block) -> None:
    ...
    def parse(self, input: str) -> Block:
...
class HTMLRenderer:
    ...
    innersep = "\n"
    softbreak = "\n"
    escape_pairs: Tuple[Tuple[str, str], ...] = (
        ("[&]", "&amp;"),
        ("[<]", "&lt;"),
        ("[>]", "&gt;"),
        ('["]', "&quot;"),
    ...
    def inTags(
        ...
        if len(attribs) > 0:
            i = 0
            while (len(attribs) > i) and (attribs[i] is not None):
                attrib = attribs[i]
    ...
    def escape(self, s: str, preserve_entities: Optional[bool] = None) -> str:
    ...
    def renderInline(self, inline: Block) -> str:
        """Render an inline element as HTML."""
        attrs: Optional[List[List[str]]] = None
        if inline.t == "Str":
            return self.escape(inline.c if isinstance(inline.c, str) else "")  # type: ignore[arg-type]
            ...
            return self.inTags(
                "em",
                [],
                self.renderInlines(inline.c if isinstance(inline.c, list) else []),
            )
            ...
            return inline.c if isinstance(inline.c, str) else ""  # type: ignore[return-value]
        elif inline.t == "Entity":
            if inline.c == "&nbsp;":
                return " "
            else:
                return self.escape(inline.c if isinstance(inline.c, str) else "", True)  # type: ignore[arg-type]
        elif inline.t == "Link":
            ...
            return self.inTags(
                ...
                attrs,
                self.renderInlines(
                    inline.label if isinstance(inline.label, list) else []
                ),
            ...
            return self.inTags(
                "code", [], self.escape(inline.c if isinstance(inline.c, str) else "")
            )  # type: ignore[arg-type]
        else:
            warn("Unknown inline type " + inline.t, stacklevel=2)
            return ""

    def renderInlines(self, inlines: List[Block]) -> str:
    ...
    def renderBlock(self, block: Block, in_tight_list: Optional[bool]) -> str:
        """Render a single block element."""
        tag: Optional[str] = None
        attr: Optional[List[List[str]]] = None
        info_words: Optional[List[str]] = None
        if block.t == "Document":
            ...
            return self.inTags(
                "li", [], self.renderBlocks(block.children, in_tight_list).strip()
            )
        elif block.t == "List":
            ...
            return HTMLRenderer.inTags(
                "pre",
                [],
                HTMLRenderer.inTags("code", [], self.escape(block.string_content)),
            )
        elif block.t == "FencedCode":
            info_words = []
            if block.info:
            ...
            attr = (
                []
                if len(info_words) == 0
                else [["class", "language-" + self.escape(info_words[0], True)]]
            )
            return self.inTags(
                "pre", [], self.inTags("code", attr, self.escape(block.string_content))
            )
            ...
            warn("Unknown block type" + block.t, stacklevel=2)
            return ""

    def renderBlocks(
        self, blocks: List[Block], in_tight_list: Optional[bool] = None
    ) -> str:
    ...
    def render(self, block: Block, in_tight_list: Optional[bool] = None) -> str:
