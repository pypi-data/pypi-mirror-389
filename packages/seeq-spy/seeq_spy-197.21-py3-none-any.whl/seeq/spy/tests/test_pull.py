...
def test_seeq_server_error():
    session = test_common.get_session_copy(Sessions.nonadmin)
    session.options.retry_timeout_in_seconds = 1
    ...
    try:
        spy.pull(search_results, start='2019-01-01', end='2019-03-07', session=session)
    except ApiException as e:
        assert "Connection to \'SPy Tests: SPy Tests\' is offline" in str(e)

    status = Status(errors='catalog')
    spy.pull(search_results, start='2019-01-01', end='2019-03-07', status=status, session=session)
