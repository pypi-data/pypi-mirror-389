...
def test_path_search_pagination():
    ...
    pagination_levels = [5, 20, 30, 50, 75, 100, 101]
    ...
    for pagination_level in pagination_levels:
        ...
        assert len(search_results) == 100, search_results.to_string()
        _assert_result(search_results, 'Success')
        assert search_results.spy.status.df.iloc[0]['Count'] > 0, search_results.spy.status.df.to_string()
        # expected_pages_count = 100 // pagination_level + 1
        assert search_results.spy.status.df.iloc[0]['Pages'] > 0, search_results.spy.status.df.to_string()
...
def test_search_does_not_override_asset_and_path_properties():
    tree_name = f"My Tree {_common.new_placeholder_guid()}"
    items_api = ItemsApi(spy.client)
    my_tree = spy.assets.Tree(tree_name, workbook='Search Override Test Workbook')
    my_tree.insert(children='Cooling Tower 1', parent=tree_name)
    ...
    assert search_results.at[0, 'Path'] == tree_name
...
def test_include_properties():
    session = test_common.get_session_copy(Sessions.nonadmin)
    with ApiClientRecorder(session) as recorder:
    ...
    if _login.is_server_version_at_least(62):
        assert search_result['Maximum Interpolation'].dropna().to_list() == ['5.4d']
        assert search_result['Maximum Duration'].dropna().to_list() == ['2.2h']

        session.options.compatibility = 195
        search_result = spy.search({'Name': f'* {workbook}', 'Scoped To': workbook_id},
                                   include_properties=['Maximum Interpolation', 'Maximum Duration'],
                                   order_by='Name', session=session)
...
@pytest.mark.system
def test_include_unitful_properties():
    # Properties that are unitful should include the units in the search results when requested and be round-trip-able
    if not _login.is_server_version_at_least(62):
        # R62+ has the batch responses we need, so skip the test if not available
        return

    workbook = 'test_include_unitful_properties'
    session = test_common.get_session_copy(Sessions.nonadmin)
    spy.push(metadata=pd.DataFrame([{
        'Name': f'Stored Signal {workbook}',
        'Type': 'Signal',
        'Maximum Interpolation': '3.5d',
        'Value Unit Of Measure': '°F',
    }, {
        'Name': f'Calculated Signal {workbook}',
        'Type': 'Signal',
        'Formula': '$s.setMaxInterpolation(2.5d)',
        'Formula Parameters': {
            '$s': f'Stored Signal {workbook}',
        }
    }, {
        'Name': f'Stored Condition {workbook}',
        'Type': 'Condition',
        'Maximum Duration': '3.5d',
    }, {
        'Name': f'Calculated Condition {workbook}',
        'Type': 'Condition',
        'Formula': '$c.removeLongerThan(2.5d)',
        'Formula Parameters': {
            '$c': f'Stored Condition {workbook}',
        }
    }]), workbook=workbook, datasource=workbook, worksheet=None, session=session)

    signals = spy.search({'Name': f'Signal {workbook}', 'Type': 'Signal'}, workbook=workbook,
                         include_properties=['Maximum Interpolation'], session=session, order_by='Name')
    assert signals['Maximum Interpolation'].to_list() == ['2.5d', '3.5d']
    conditions = spy.search({'Name': f'Condition {workbook}', 'Type': 'Condition'}, workbook=workbook,
                            include_properties=['Maximum Duration'], session=session, order_by='Name')
    assert conditions['Maximum Duration'].to_list() == ['2.5d', '3.5d']

    # In version 195 and below, we should maintain compatibility that calculated items return the value as a raw
    # number. The stored items always return a string.
    session.options.compatibility = 195
    signals = spy.search({'Name': f'Signal {workbook}', 'Type': 'Signal'}, workbook=workbook,
                         include_properties=['Maximum Interpolation'], session=session, order_by='Name')
    assert signals['Maximum Interpolation'].to_list() == [2.5, '3.5d']
    conditions = spy.search({'Name': f'Condition {workbook}', 'Type': 'Condition'}, workbook=workbook,
                            include_properties=['Maximum Duration'], session=session, order_by='Name')
    assert conditions['Maximum Duration'].to_list() == [2.5, '3.5d']
...
def test_search_by_id():
    ...
    with ApiClientRecorder(spy.session) as recorder:
        ...
        if spy.utils.is_sdk_module_version_at_least(62):
            ...
            assert recorder.calls.get('GET /items/{id}', 0) == 0
            assert recorder.calls.get('GET /formulas/items/{id}', 0) == 0
            assert recorder.calls.get('GET /folders', 0) == 0  # CRAB-28129 we shouldn't call /folders searching by ID
        else:
            assert recorder.calls.get('GET /trees/assets', 0) == 0
            assert recorder.calls.get('GET /items', 0) == 0
