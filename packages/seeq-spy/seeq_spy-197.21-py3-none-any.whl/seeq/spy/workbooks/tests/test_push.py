...
from seeq.spy._metadata_push_results import PushMethod
...
def _get_push_methods(workbook_push_results_df, ignore_charts=True) -> set:
    pushed_inventory = workbook_push_results_df.spy.pushed_inventory
    push_methods = set()
    for pushed_inventory_for_workbook in pushed_inventory.values():
        # since Charts are never pushed Batched, most of the time we want to ignore them
        if ignore_charts:
            pushed_inventory_for_workbook = pushed_inventory_for_workbook[
                pushed_inventory_for_workbook['Type'] != 'Chart']
        push_methods_for_workbook = set(pushed_inventory_for_workbook['Push Method'].unique())
        push_methods.update(push_methods_for_workbook)
    return push_methods
...
def test_export_example_and_test_data():
    ...
    # Note: before copying the contents of "spy-example-and-test-data-folder.zip" into "sq-run-data-dir",
    # make sure the zip doesn't include the "/configuration" directory.
    #
    ...
    # this test gets executed. Run the sdk-system-tests test. It will copy everything into the right spot.
    #
    ...
    _pull_and_save_as_job('Workbook Job Tests', test_load.get_workbook_job_tests_path())
...
def _remove_if_exists(path):
    if util.safe_exists(path):
        if util.safe_isdir(path):
            util.safe_rmtree(path)
        else:
            util.safe_remove(path)


def _pull_and_save_as_job(workbook_folder_path, destination_file_path):
    search_df = spy.workbooks.search({
        'Path': workbook_folder_path
    }, content_filter='ALL')

    _remove_if_exists(destination_file_path)

    job_folder = destination_file_path
    spy.workbooks.job.pull(job_folder, search_df)
...
@pytest.mark.system
@pytest.mark.exclude_from_earliest_dependencies_test_run  # Because file times are messed up due to copying stuff around
...
def test_push_workbook_seeq_internal_datasource():
    session = test_common.get_session_copy(Sessions.agent)
    session.options.include_push_method = True
    ...
    push_result_2 = spy.workbooks.push(workbooks, path=path, datasource=spy.INHERIT_FROM_WORKBOOK, session=session)
    ...
    assert not error
    # we should be using batch pushes for everything other than the Chart
    assert _get_push_methods(push_result_2) == {PushMethod.BATCH}
    ...
    push_result_3 = spy.workbooks.push(workbook_pull, path=path, datasource=spy.INHERIT_FROM_WORKBOOK, session=session)
    ...
    assert not error
    # we should be using batch pushes for everything other than the Chart
    assert _get_push_methods(push_result_3) == {PushMethod.BATCH}
...
def test_push_local_asset_tree():
    ...
    test_name = 'test_push_local_asset_tree'

    pushed_metadata_df = spy.push(metadata=pd.DataFrame([{
        'Type': 'StoredSignal',
        'Name': 'Spin Rate',
        'Path': f'{test_name} >> Building 12',
        'Asset': 'Mangler',
    }, {
        'Type': 'StoredSignal',
        'Name': 'Spin Rate',
        'Path': f'{test_name} >> Building 42',
        'Asset': 'Mangler'
    }]), workbook=f'{test_name} >> First Workbook >> The Workbook', worksheet=None, datasource=test_name)
    ...
    workbook_1 = workbooks_1[0]
    sorted_inventory_1 = sorted(workbook_1.item_inventory.values(), key=lambda x: x.fqn)
    assert workbook_id in sorted_inventory_1[3]['Data ID']

    pushed_2_df = spy.workbooks.push(workbook_1, path=f'{test_name} >> Second Workbook', label='new', refresh=False,
                                     session=session)
    pushed_2_workbook_id = pushed_2_df.iloc[0]['Pushed Workbook ID']
    workbooks_2 = spy.workbooks.pull(pushed_2_workbook_id)
    assert len(workbooks_2) == 1
    workbook_2 = workbooks_2[0]
    assert len(workbook_2.item_inventory) == 10
    sorted_inventory_2 = sorted(workbook_2.item_inventory.values(), key=lambda x: x.fqn)
    assert [item['ID'] for item in sorted_inventory_1] != [item['ID'] for item in sorted_inventory_2]
    assert sorted_inventory_1[3]['ID'] in sorted_inventory_2[3]['Data ID']

    reprs = [re.sub(_common.GUID_REGEX, 'GUID', str(item)) for item in sorted_inventory_2]
    assert reprs == [
        'Folder "Agent API Key" (GUID)',
        'Folder "Second Workbook" (GUID)',
        'Folder "test_push_local_asset_tree" (GUID)',
    ...
    assert sorted_inventory_2[3]['Parent ID'] == Item.ROOT
    assert sorted_inventory_2[4]['Parent ID'] == sorted_inventory_2[3]['ID']
    assert sorted_inventory_2[5]['Parent ID'] == sorted_inventory_2[4]['ID']
    assert sorted_inventory_2[6]['Parent ID'] == sorted_inventory_2[5]['ID']
    assert sorted_inventory_2[7]['Parent ID'] == sorted_inventory_2[3]['ID']
    assert sorted_inventory_2[8]['Parent ID'] == sorted_inventory_2[7]['ID']
    assert sorted_inventory_2[9]['Parent ID'] == sorted_inventory_2[8]['ID']

    assert _get_push_methods(pushed_2_df) == {PushMethod.BATCH}


@pytest.mark.system
def test_round_trip_local_asset_tree_with_metadata():
    session = test_common.get_session_copy(Sessions.agent)
    session.options.include_push_method = True

    workbook = 'test_round_trip_local_metadata_asset_tree'
    ...
    workbooks_1 = spy.workbooks.pull(workbook_id)
    assert len(workbooks_1) == 1
    workbook_results = spy.workbooks.push(workbooks_1, datasource=workbook, errors='raise', refresh=False,
                                          session=session)
    assert _get_push_methods(workbook_results) == {PushMethod.BATCH}


@pytest.mark.system
def test_push_with_mixed_datasources():
    # when pushing items with a different datasource than specified in the push, we can't batch it
    # make sure we can see that in the push results

    session = test_common.get_session_copy(Sessions.agent)
    session.options.include_push_method = True

    workbook = 'test_push_with_mixed_datasources'
    ...
    }]), workbook=workbook, worksheet=None, datasource=f'{workbook} but cool')
    ...
    workbook_results = spy.workbooks.push(workbooks_1, datasource=workbook, errors='raise', refresh=False,
                                          session=session)
    assert _get_push_methods(workbook_results) == {PushMethod.SINGLE}


@pytest.mark.system
def test_round_trip_stored_items():
    session = test_common.get_session_copy(Sessions.agent)
    session.options.include_push_method = True

    workbook = 'test_round_trip_stored_items'
...
def test_push_tree_with_mixed_scopes():
    ...
    workbook_results = spy.workbooks.push(workbooks_2, datasource=test_name, refresh=True,
                                          reconcile_inventory_by='name', session=session)
    ...
    # ensure everything was pushed via batch endpoints
    assert _get_push_methods(workbook_results) == {PushMethod.BATCH}
...
def test_reconcile_inventory_by_name():
    ...
    workbook_results = spy.workbooks.push(workbook, label=test_name, reconcile_inventory_by='name', session=session)
...
@pytest.mark.system
def test_assume_dependencies_exist_with_asset_selection_asset_id_not_found():
    test_name = f'{inspect.stack()[0].function} {_common.new_placeholder_guid()}'
    signal_search = spy.search({
        'Path': 'Example >> Cooling Tower 1 >> Area G',
        'Name': 'Temperature'
    })
    assert not signal_search.empty
    signal_id = signal_search.iloc[0]['ID']

    workbook = spy.workbooks.Analysis(f'Analysis {test_name}')
    worksheet = workbook.worksheet('Sheet 1')
    worksheet.display_items = pd.DataFrame([signal_search[signal_search['ID'] == signal_id].iloc[0]])

    topic = spy.workbooks.Topic(f'Topic {test_name}')
    doc = topic.document('Test Document')

    content = spy.workbooks.Content({
        'Name': 'Test Content',
        'Type': 'Worksheet',
        'Worksheet ID': worksheet.id,
        'Width': 100,
    ...
    asset_id = spy.search({'Path': 'Example >> Cooling Tower 1', 'Name': 'Area G'}).iloc[0]['ID']
    now = pd.Timestamp.now()
    start = now - pd.Timedelta(days=5)

    asset_selection = spy.workbooks.AssetSelection(
        {'Name': 'Test Selection', 'Asset ID': asset_id, 'Path Levels': 1},
        report=doc.report
    )
    date_range = spy.workbooks.DateRange(
        {'Name': 'Test Date Range', 'Start': start.isoformat(), 'End': now.isoformat()},
        report=doc.report
    )

    content.asset_selection = asset_selection
    content.date_range = date_range
    doc.asset_selections[asset_selection.id] = asset_selection
    doc.date_ranges[date_range.id] = date_range

    def push_workbooks(assume_dependencies_exist: bool):
        return spy.workbooks.push(
            [workbook, topic],
            include_inventory=False,
            assume_dependencies_exist=assume_dependencies_exist
        )

    result = push_workbooks(assume_dependencies_exist=True)

    assert len(result) == 2
    assert result['Pushed Workbook ID'].notna().all()
    assert result['Workbook Type'].isin(['Topic', 'Analysis']).all()
    assert result['Result'].str.contains('Success').all()

    with pytest.raises(SPyRuntimeError, match=f'Could not find the following dependencies:\nAsset {asset_id}'):
        push_workbooks(assume_dependencies_exist=False)
