...
from seeq.spy.workbooks import Analysis, Workbook, Topic, TopicDocument, DateRange
...
@pytest.mark.system
def test_pull_date_range_with_no_start_end():
    name = 'test_pull_date_range_with_no_start_end'
    # Setup: Create an empty DateRange manually. It must use a Formula that results in metadata that should
    # be a capsule, but should not actually have a result.
    formula = 'condition(1day, capsule("2024-01-01")).toGroup(capsule("2025-01-01")).first()'
    content_api = sdk.ContentApi(spy.session.client)
    date_range_input = sdk.DateRangeInputV1(name=name, formula=formula)
    date_range_sdk = content_api.create_date_range(body=date_range_input)

    # Pulling the DateRange without errors=catalog should raise an error.
    with pytest.raises(SPyRuntimeError,
                       match=f'Date Range "{date_range_sdk.name}" {date_range_sdk.id} does not have a start or end'):
        DateRange.pull(date_range_sdk.id, session=spy.session)

    # Pulling the DateRange when errors=catalog should work, but the start and end should be the last day
    status = spy.Status(errors='catalog')
    date_range_spy = DateRange.pull(date_range_sdk.id, session=spy.session, status=status)
    assert date_range_spy.id == date_range_sdk.id
    assert date_range_spy.name == name
    now = pd.Timestamp.now(tz='UTC')
    tolerance = pd.Timedelta(minutes=1)
    assert abs(date_range_spy.start + pd.Timedelta(days=1) - now) <= tolerance
    assert abs(date_range_spy.end - now) <= tolerance
...
def test_pull_from_url_with_session():
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=DeprecationWarning, message=r".*get_analysis_worksheet_from_url.*")
