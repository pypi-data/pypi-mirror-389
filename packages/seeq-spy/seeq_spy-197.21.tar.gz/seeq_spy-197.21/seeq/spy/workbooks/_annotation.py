...
class Annotation(Item):
    """
    The base class of all documents in the spy.workbooks module.
    """
    _html: str
    _images: dict
    ...
    def pull(self, session: Session, *, include_images=True, include_archived: Optional[bool] = None,
...
class Report(Annotation):
    """
    The SPy representation of a Report, the document present in each TopicDocument.
    """
    ...
    def pull(self, session: Session, *, include_images=True, include_archived: Optional[bool] = None,
        ...
        for content in contents.content_items:  # type: ContentWithMetadataOutputV1
            # The default (include_archived=None) means that archived content is INCLUDED.
            if content.is_archived and include_archived is False:
                continue
        ...
        for date_range_id in annotation_output.date_range_ids:
            if date_range_id not in self.date_ranges.keys():
                ...
                # The default (include_archived=None) means that archived date ranges are INCLUDED.
                if new_date_range.get('Archived', False) and include_archived is False:
                    continue
        ...
        for asset_selection_id in annotation_output.asset_selection_ids:
            if asset_selection_id not in self.asset_selections.keys():
                ...
                # The default (include_archived=None) means that archived asset selections are INCLUDED.
                if new_asset_selection.get('Archived', False) and include_archived is False:
                    continue
