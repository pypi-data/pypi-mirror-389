#!/usr/bin/env python3
"""MCP Client Wrapper for Claude Desktop integration.

This script bridges Claude Desktop's stdio-based MCP protocol to MyAlly's
Share Server HTTP-based MCP endpoint.

Usage:
    # Via environment variable (recommended - more secure)
    ALLY_SHARE_KEY=YOUR_SHARE_KEY python mcp_client_wrapper.py --url http://localhost:8081/share/mcp

    # Via command-line argument (backward compatible)
    python mcp_client_wrapper.py --share-key YOUR_SHARE_KEY --url http://localhost:8081/share/mcp
...
Configuration for Claude Desktop (claude_desktop_config.json):
{
  "mcpServers": {
    "myally-workspace": {
      "command": "python",
      "args": [
        "C:\\path\\to\\mcp_client_wrapper.py",
        "--url", "http://localhost:8081/share/mcp"
      ],
      "env": {
        "ALLY_SHARE_KEY": "ally_share_xxxxxxxxxxxxx"
      }
    }
  }
}
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('mcp_client_wrapper.log'), logging.StreamHandler(sys.stderr)]
)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='MCP Client Wrapper for MyAlly Share Server',
    ...
    parser.add_argument(
        ...
        help='Share key for authentication (x-ally-share-key header). Can also be set via ALLY_SHARE_KEY environment variable.'
    )
    parser.add_argument(
        '--url',
        default='http://localhost:8081/share/mcp',
        help='Share Server MCP endpoint URL (default: http://localhost:8081/share/mcp)'
    )
    parser.add_argument(
        '--timeout',
        type=int,
    ...
    # Get share key from environment variable or argument
    share_key = args.share_key or os.getenv('ALLY_SHARE_KEY')

    if not share_key:
        logger.error("Share key not provided via --share-key argument or ALLY_SHARE_KEY environment variable")
        parser.error("--share-key argument or ALLY_SHARE_KEY environment variable is required")

    # Validate share key format
    if not share_key.startswith('ally_share_'):
        logger.warning(f"Share key doesn't match expected format (ally_share_*)")

    # Create and run wrapper
    wrapper = MCPClientWrapper(share_key, args.url, timeout=args.timeout)
    wrapper.run()
