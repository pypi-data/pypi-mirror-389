...
from os2borgerpc.client.config import has_config, get_config
...
OS2BORGERPC_CLIENT_VERSION = version("os2borgerpc-client")
...
def get_last_update_times():
    """Return the last update times.

    Returns the last time automatic updates were run and
    the last time full updates were run (which might be the same time).
    """
    config = OS2borgerPCConfig()

    # Find the timestamp for the latest automatic updates
    ...
    elif has_config("_last_automatic_update_time"):
        last_automatic_update_time = config.get_value("_last_automatic_update_time")
    else:
        last_automatic_update_time = ""
    # Find the timestamp for the latest full updates
    # This is updated by the "Update all" script, but will also be
    # updated here if all automatic updates are enabled
    if os.path.isfile("/etc/apt/apt.conf.d/90os2borgerpc-automatic-upgrades"):
        with open("/etc/apt/apt.conf.d/90os2borgerpc-automatic-upgrades", "r") as f:
            update_settings = f.read()
        # Matches the line '; "${distro_id}:${distro_codename}"', which is
        # only present if all automatic updates have been enabled
        if '{distro_id}:${distro_codename}"' in update_settings:
            # Prevent erroneously changing "_last_full_update_time" to an earlier time
            if (
                has_config("_last_full_update_time")
                and config.get_value("_last_full_update_time")
                > last_automatic_update_time
            ):
                last_full_update_time = config.get_value("_last_full_update_time")
            else:
                last_full_update_time = last_automatic_update_time
        elif has_config("_last_full_update_time"):
            last_full_update_time = config.get_value("_last_full_update_time")
        else:
    ...
    return last_automatic_update_time, last_full_update_time
...
def update_configuration_from_server(configurations):
    ...
    try:
        hostname = get_config("hostname")  # Get hostname from config
        current_hostname = socket.gethostname()  # Get current system hostname
...
def send_config_values(config_dict):
    ...
    remote.push_config_keys(uid, config_dict, read_only=True)
...
def update_and_run():
    ...
    try:
        ...
        kernel_version = ""
    last_automatic_update_time, last_full_update_time = get_last_update_times()
    if has_config("job_timeout"):
        try:
            job_timeout = int(config.get_value("job_timeout"))
        except ValueError:
        ...
        with filelock(LOCK_FILE, max_age=job_timeout):
            try:
                ...
                if last_full_update_time:
                    send_config_values(
                        {"_last_full_update_time": last_full_update_time}
                    )
