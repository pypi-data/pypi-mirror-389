...
def scan_all_items_dual(
    ...
    recursive: bool = True,
    absolute: bool = True,
    allowed_for: Optional[str] = "files",  # 'files', 'dirs', or 'both'
    **kwargs
) -> dict[str, list[str]]:
    """
    Scan separately for files and directories, applying ScanConfig rules.

    Returns:
        dict[str, list[str]] -> {'files': [...], 'dirs': [...]}
    """
    allowed = make_allowed_predicate(cfg)

    def _collect(files: bool, dirs: bool) -> list[str]:
        if user_at_host and run_cmd:
            # build find expression
            find_types = []
            if files: find_types.append("-type f")
            if dirs: find_types.append("-type d")
            type_clause = " -o ".join(find_types)
            depth_opt = "" if recursive else "-maxdepth 1"
            cmd = f"find $(pwd) {depth_opt} \\( {type_clause} \\) -print0"
            raw = run_cmd(cmd, cwd=base_path, user_at_host=user_at_host)
            raw = raw.encode() if isinstance(raw, str) else raw
            paths = [p for p in raw.decode(errors="ignore").split("\0") if p]
        else:
            base = Path(base_path)
            iterator = base.rglob("*") if recursive else base.glob("*")
            paths = [str(p.resolve() if absolute else p) for p in iterator
                     if (p.is_file() and files) or (p.is_dir() and dirs)]

        return [p for p in paths if allowed(p)]

    # perform per-type scans
    results = {}
    if allowed_for in ("files", "both"):
        results["files"] = _collect(files=True, dirs=False)
    if allowed_for in ("dirs", "both"):
        results["dirs"] = _collect(files=False, dirs=True)
