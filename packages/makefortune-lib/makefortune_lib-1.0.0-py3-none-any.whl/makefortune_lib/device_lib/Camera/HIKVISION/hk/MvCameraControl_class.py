...
if system == "Linux":
    ...
    MvCamCtrldll = ctypes.cdll.LoadLibrary(os.getenv('GENICAM_GENTL64_PATH') + "/MvCameraControl.dll")
...
class MvCamera():
    ...
    # 设置String型属性值
    def MV_CC_SetExposureTime(self, strKey):
        MvCamCtrldll.MV_CC_SetStringValue.argtype = (c_void_p, c_void_p, c_void_p)
        MvCamCtrldll.MV_CC_SetStringValue.restype = c_uint
        # C原型:int MV_CC_SetStringValue(void* handle,char* strKey,char * sValue)
        return MvCamCtrldll.MV_CC_SetExposureTime(self.handle, strKey.encode('ascii'))
