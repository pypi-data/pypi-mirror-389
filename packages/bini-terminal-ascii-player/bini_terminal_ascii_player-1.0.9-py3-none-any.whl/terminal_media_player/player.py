...
class WebcamASCIICapture:
    ...
    def frame_to_ascii(self, frame, width=None):
        """Convert webcam frame to ASCII art"""
        if width is None:
    ...
    def start_webcam(self, camera_index=0, fps=15):
        """Start webcam ASCII capture"""
        self.get_terminal_size()
        ...
        if not cap.isOpened():
            ...
            print("Please check:")
            print("  1. Webcam is connected and not in use by another app")
            print("  2. Camera drivers are installed")
            print("  3. Try different camera index: bini webcam --camera 1")
            return False
        
        # Try to set camera resolution for better performance
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        cap.set(cv2.CAP_PROP_FPS, fps)
        
        print(f"📹 Starting webcam ASCII capture...")
        print(f"   Camera: {camera_index}")
        print(f"   Target FPS: {fps}")
        print(f"   Terminal size: {self.terminal_width}x{self.terminal_height}")
        print("   Press 'q' to quit, 'c' to change camera, 'f' to toggle FPS info")
        time.sleep(2)
        ...
        try:
            while self.playing:
                ...
                if show_info:
                    ...
                    info_line = (f"Webcam Live - Frame: {frame_count} | "
                ...
                if show_info:
                    print("═" * self.terminal_width)
                    print("Controls: [Q]uit [C]amera [F]PS info [S]ave frame")
                ...
                if msvcrt.kbhit():
                    ...
                    if key == 'q':
                        ...
                        cap = cv2.VideoCapture(camera_index)
                        if not cap.isOpened():
                            print(f"❌ Could not access camera {camera_index}, switching back...")
            ...
            print("\n🛑 Webcam capture interrupted by user")
        except Exception as e:
