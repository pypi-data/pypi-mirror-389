...
class CZSessionOptions:
    ...
    for status in RetryStatus:
        DEFAULT_REGISTER_STATUS.register_code(status)
...
class RealtimeOptionsBuilder:
    ...
    def with_request_failed_retry_status(self, retry_status) -> 'RealtimeOptionsBuilder':
        """
        Set retry status codes. Can accept multiple RetryStatus arguments or a list of RetryStatus.
        
        Examples:
            # Multiple arguments
            .with_request_failed_retry_status(RetryStatus.THROTTLED, RetryStatus.INTERNAL_ERROR)
            
            # List argument
            .with_request_failed_retry_status([RetryStatus.THROTTLED, RetryStatus.INTERNAL_ERROR])
        """
        # Handle both individual arguments and list/tuple arguments
        statuses = []
        ...
        for status in statuses:
            self._options.request_failed_retry_register_status.register_code(status)
        return self
