...
def _gate_way_call(client, request, method):
    ...
    # Add version info like Java implementation
    gate_way_request.versionInfo.name = VERSION_NAME
    ...
    try:
        api_response = requests.post(
            ...
            headers=headers,
            timeout=30  # Add timeout like Java
        )
        api_response.encoding = "utf-8"

        if api_response.status_code != 200:
            raise CZException(f"Gateway call failed with status code: {api_response.status_code}")
        ...
        if not result_status or result_status.code != ingestion_pb2.Code.SUCCESS:
            error_msg = result_status.message if result_status else "Unknown error"
            raise CZException(f"Gateway call failed: {error_msg}")

        return result_dict["message"]

    except requests.exceptions.RequestException as e:
        log.error(f"Gateway request failed: {e}")
        raise CZException(f"Gateway request failed: {e}")
    except Exception as e:
