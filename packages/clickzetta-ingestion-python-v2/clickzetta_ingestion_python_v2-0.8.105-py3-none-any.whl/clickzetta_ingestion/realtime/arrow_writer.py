...
class ArrowFieldWriter:
    ...
    def __init__(self, vector: pa.Array):
        self.vector = vector
        self.values = []
        self.name = vector.type.name if hasattr(vector.type, 'name') else None
        self.nullable = vector.type.nullable if hasattr(vector.type, 'nullable') else True
        self.count = 0
        ...
        try:
            if is_set:
                ...
                if not self.nullable and row.is_null_at(ordinal):
                    raise ValueError(f"Column {self.name} is not nullable but got null value")
...
class StructWriter(ArrowFieldWriter):
    ...
    def __init__(self, vector: pa.StructArray, children: List[ArrowFieldWriter]):
        super().__init__(vector)
        self.children = children
        self.field_names = [field.name for field in vector.type]
...
class ListWriter(ArrowFieldWriter):
    ...
    def __init__(self, vector: pa.ListArray, element_writer: ArrowFieldWriter):
        super().__init__(vector)
...
class MapWriter(ArrowFieldWriter):
    ...
    def __init__(self, vector: pa.MapArray, key_writer: ArrowFieldWriter, value_writer: ArrowFieldWriter):
        super().__init__(vector)
...
class DecimalWriter(ArrowFieldWriter):
    ...
    def __init__(self, vector: pa.Decimal128Array):
        super().__init__(vector)
...
class ArrowRecordBatchWriter:
    ...
    def _init_field_writers(self, initial_row_size: int):
        ...
        try:
            ...
            for i in range(self.arrow_schema.get_column_count()):
                ...
                vector = self._create_empty_array(field.type)
                if initial_row_size != -1:
                    # TODO: implement pre-allocation if needed
                    pass
                self.field_writers.append(self._create_field_writer(vector))
        except Exception as e:
    ...
    @staticmethod
    def _create_field_writer(vector: pa.Array) -> ArrowFieldWriter:
        """Create appropriate field writer based on type"""
        if isinstance(vector.type, pa.StructType):
            child_writers = [
                ArrowRecordBatchWriter._create_field_writer(
                    pa.array([], type=field.type)
                )
                for field in vector.type
            ]
            return StructWriter(vector, child_writers)
        elif isinstance(vector.type, pa.ListType):
            element_writer = ArrowRecordBatchWriter._create_field_writer(
                pa.array([], type=vector.type.value_type)
            )
            return ListWriter(vector, element_writer)
        elif isinstance(vector.type, pa.MapType):
            key_writer = ArrowRecordBatchWriter._create_field_writer(
                pa.array([], type=vector.type.key_type)
            )
            value_writer = ArrowRecordBatchWriter._create_field_writer(
                pa.array([], type=vector.type.item_type)
            )
            return MapWriter(vector, key_writer, value_writer)
        elif isinstance(vector.type, pa.Decimal128Type):
            return DecimalWriter(vector)
            ...
            return FixedSizeBinaryWriter(vector)
        else:
            return ArrowFieldWriter(vector)
    ...
    def write(self, row: ArrowRow):
        ...
        for i, writer in enumerate(self.field_writers):
            ...
            if (i in self.arrow_schema.get_key_columns_index() and
                ...
                raise ValueError(
                    f"Key column {self.arrow_schema.get_column_by_index(i).name} "
                    "must have value"
                )
