...
from .core import b64url_decode, gzip_event, redact_event, sign
...
class SenderPool:
    def __init__(
        ...
        *,
        ...
        self.q: queue.Queue[tuple[Dict[str, Any], Dict[str, str]]] = queue.Queue(maxsize=max_queue)
    ...
    def submit(self, event: Dict[str, Any], extra_headers: Dict[str, str] | None = None) -> None:
        self.q.put((event, extra_headers or {}), block=True)
    ...
    def _worker(self, http2: bool) -> None:
        ...
        while not self.stop.is_set():
            try:
                event, extra_headers = self.q.get(timeout=0.2)
            except queue.Empty:
                continue

            ok = self._send(client, event, extra_headers)
            if ok:
                self.q.task_done()
    ...
    def _send(
        ...
        body = gzip_event(redact_event(event))
