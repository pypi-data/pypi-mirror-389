...
from ..core import body_from_content_type, ensure_bytes, extract_client_ip, normalize_headers
...
def instrument(monitor: Monitor, app, _AIKO_PKG_VERSION: str) -> None:
    ...
    async def _aiko_mw(request: Request, call_next):
        ...
        client_ip = extract_client_ip(req_headers, request.client.host if request.client else None)
        try:
        ...
        try:
            ...
            extra_headers = {"X-Client-IP": client_ip} if client_ip else None
            monitor.add_event(evt, extra_headers=extra_headers)
            return response
