...
from .rac5 import closest_ref, fetch_daily


class Rac5DailyServer(WeatherServerApi):

    def __init__(self):
        super().__init__()
        self.storage = self.storage_root() / "rac5_daily"
    ...
    def fetch(self, latitude, longitude, year):
        ...
        Args:
            latitude (float): [deg] latitude of place
            longitude (float): [deg] longitude of place
            year (int): [-] year to fetch from YYYY-01-01T00:00 till YYYY-12-31 included
        ...
        # fetch
        df, header = fetch_daily(ref_lat, ref_long, datetime(year, 1, 1), datetime(year, 12, 31))
