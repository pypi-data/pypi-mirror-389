...
def record_system(thread_state, immutable_types, config):
    ...
    if config["trace_calls"]:
        class CallTracer:
            def __init__(self):
                self.file = open(recording_path / 'call_trace.txt', 'w')
            ...
            def close(self):
                ...
                # json.dump(obj, self.file, indent = 2)
                # self.file.write('\n')

            # def on_call(self, function_name):
            #     self.file.write(f'call: {function_name}\n')

            # def on_return(self, function_name):
            #     self.file.write(f'return: {function_name}\n')

            # def on_result(self, function_name, result):
            #     self.file.write(f'result: {function_name} {result}\n')
