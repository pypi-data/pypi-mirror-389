...
class Sheet(WordFile, ComponentMixin):
    ...
    def _process_image_data(self, xml_string, xml_file):
        """处理XML字符串中的图片数据
    ...
    def _process_header_footer(self, xml_string, data):
...
{content}
    ...
    def render_xml(self, xml_filename, data, use_internal_template=False):
        ...
        try:
            ...
            xml_content = self._process_image_data(xml_content, f'{xml_filename}.xml')
            
            if xml_filename == 'document':
                xml_content = self._process_header_footer(xml_content, data)
    ...
    def render_and_add_xml(self, xml_file, data, relation_id=None, use_internal_template=False):
        ...
        self[f'word/{xml_file}'] = self._process_image_data(xml_content, xml_file)
