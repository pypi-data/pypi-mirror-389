...
from pygdsdesign.materials_color import colors
colors_names = [color.lower() for color in colors.keys()]


from pygdsdesign.gdsiiformat import (
    _record_reader,
    _raw_record_reader,
    _eight_byte_real,
...
class Cell(object):
    ...
    def __init__(self, name, exclude_from_current=False, overwrite_duplicate=False):
        self.name = name
        self.polygons = []
        self.paths = []
        self.labels = []
        ...
        #if library.use_current_library and not exclude_from_current:
            #library.current_library.add(self, include_dependencies=False, overwrite_duplicate=overwrite_duplicate)
    ...
    def to_gds(self, outfile, multiplier, timestamp=None) -> None:
        """
    ...
    def add(self, element):
        ...
        if isinstance(element, PolygonSet):
            ...
            self.polygons.append(element)
        elif isinstance(element, MicroStrip):
            self.polygons.append(element)
            ...
            for e in element:
                if isinstance(e, PolygonSet):
                    ...
                    raise ValueError(
                        "[pygdsdesign] Only instances of `PolygonSet`, `FlexPath`, "
...
class CellReference(object):
    ...
    def __init__(
        ...
        if not isinstance(self.ref_cell, Cell) and not ignore_missing:
            warnings.warn(
                "[pygdsdesign] Cell {0} not found; operations on this "
...
class GdsLibrary(object):
    ...
    def export_gds(self, filename: str,
                     fill: bool=False):
        """
        Export a GDS file and its associated  lyp (Layer Properties) file.

        Inspired by:
            https://github.com/amccaugh/phidl/blob/master/phidl/utilities.py

        Args:
            filename : Filename of the exported layer file.
            fill : If True the layers dither pattern will be filled.
                If False layers will alternate between pattern 5 and 9.
                Defaults to False.
        """

        self.write_gds(filename)

        # Get all layers properties in a nice dictionnary
        ...
        with open('{}.lyp'.format(filename[:-4]),'w+') as f:
            ...
            for i, property in enumerate(properties):
                ...
                # Writing properties header for specific layer
                f.write(' <properties>\n')

                # The color is determine
                # 1. If the name has a specified color
                # 2. If the user has specified a color
                # 3. default gds color
                if property['color']!='':
                    if property['color'][0]=='#':
                        color = property['color']
                    elif property['name'].lower() in colors_names:
                        color = colors[property['color']]
                    else:
                        color = colors['']
                elif property['name'].lower() in colors_names:
                    color = colors[property['name']]
                else:
                    color = colors['']

                # Writing line to specify frame colour
                f.write('  <frame-color>{}</frame-color>\n'.format(color))

                # Writing line to specify fill colour
                f.write('  <fill-color>{}</fill-color>\n'.format(color))

                # Writing line to specify brightness (value between [-255, 255])
                f.write('  <frame-brightness>0</frame-brightness>\n')

                # Writing line to specify brightness (value between [-255, 255])
                f.write('  <fill-brightness>0</fill-brightness>\n')

                # Writing line to specify dither pattern
                if fill:
                    f.write('  <dither-pattern>I0</dither-pattern>\n')
                elif i%2==0:
                    f.write('  <dither-pattern>I9</dither-pattern>\n')
                else:
                    f.write('  <dither-pattern>I5</dither-pattern>\n')
                ...
                f.write('  <source>{}/{}@1</source>\n'.format(property['layer'], property['datatype']))
    ...
    def new_cell(self, name, overwrite_duplicate=False, update_references=True):
        ...
        cell = Cell(name, overwrite_duplicate=overwrite_duplicate)
        self.add(cell, False, overwrite_duplicate, update_references)
        return cell
    ...
    def add(
        ...
        for c in cell_set:
            if (
                ...
                raise ValueError(
                    "[pygdsdesign] Cell named {0} already present in library.".format(c.name)
                )
            if (
                overwrite_duplicate
    ...
    def write_gds(self, outfile, cells=None, timestamp=None, binary_cells=None):
        ...
        if len(cells) == 0:
            warnings.warn("[pygdsdesign] Creating a GDSII file without any cells.")
        for cell in cells:
        ...
        for record in _record_reader(infile):
            ...
            # BOX
                ...
                if record[0] not in emitted_warnings:
                    warnings.warn(
                        "[pygdsdesign] GDSII elements of type BOX are imported as polygons.",
                        stacklevel=2,
                    )
            ...
            # STRANS
                ...
                if (int(record[1][0]) & 0x0006) and record[0] not in emitted_warnings:
                    warnings.warn(
                        "[pygdsdesign] Absolute magnification or rotation of "
            ...
            # STRNAME
                ...
                if name in self.cells:
                    raise ValueError("[pygdsdesign] Multiple cells with name: {0} in GDSII file".format(name))
            ...
            # UNITS
                ...
                if units == "skip":
                    ...
                    raise ValueError(
                        "[pygdsdesign] units must be one of 'convert', 'import' or 'skip'."
                    )
            # LIBNAME
            elif record[0] == 0x02:
                self.name = record[1]
            ...
            # Not supported
                ...
                warnings.warn(
                    "[pygdsdesign] Record type {0} ({1:02X}) is not supported.".format(
    ...
    def rename_cell(self, cell, name, update_references=True):
        ...
        if isinstance(cell, Cell):
            ...
            if old_name not in self.cells:
                raise ValueError(
                    "[pygdsdesign] Cell named {0} not present in library.".format(old_name)
                )
            if self.cells[old_name] is not cell:
                raise ValueError(
                    "[pygdsdesign] Cell named {0} doesn't match library's.".format(old_name)
                )
        else:
            old_name = cell
            if old_name not in self.cells:
        ...
        if name in self.cells:
            raise ValueError(
                "[pygdsdesign] Cell named {0} already present in library.  "
    ...
    def extract(self, cell, overwrite_duplicate=False):
        ...
        """
        warnings.warn(
            "[pygdsdesign] extract and the use of the global library is deprecated.",
