...
class MoesifMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        if self.config.have_governance_rules():
            ...
            user_id = await self.logger_helper.get_user_id(self.settings, request, None, dict(request.headers),
                                                           self.DEBUG)
        ...
        if 'blocked_by' in governed_response:
            ...
            response = _StreamingResponse(content=generate_data(), status_code=governed_response['status'],
                                          headers=headers)
        else:
