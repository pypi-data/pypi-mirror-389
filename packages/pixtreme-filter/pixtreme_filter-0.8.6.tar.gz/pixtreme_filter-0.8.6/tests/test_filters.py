...
class TestFilters:
    ...
    def test_gaussian_blur_basic(self, sample_image):
        ...
        with px.Device(0):
            ...
            blurred = px.gaussian_blur(sample_image, ksize=5, sigma=1.0)
    ...
    def test_gaussian_blur_different_kernels(self, sample_image):
        ...
        with px.Device(0):
            # Small kernel
            blur_small = px.gaussian_blur(sample_image, ksize=3, sigma=0.5)

            # Large kernel
            blur_large = px.gaussian_blur(sample_image, ksize=15, sigma=5.0)
    ...
    def test_gaussian_blur_sigma_effect(self, sample_image):
        ...
        with px.Device(0):
            ...
            blur_low_sigma = px.gaussian_blur(sample_image, ksize=9, sigma=1.0)
            blur_high_sigma = px.gaussian_blur(sample_image, ksize=9, sigma=3.0)
    ...
    def test_gaussian_blur_edge_handling(self, sample_image):
        ...
        with px.Device(0):
            ...
            blurred = px.gaussian_blur(sample_image, ksize=7, sigma=2.0)
    ...
    def test_gaussian_blur_custom_kernel(self, sample_image):
        ...
        with px.Device(0):
            # Create a custom Gaussian kernel (1D, separable)
            kernel_size = 5
            sigma = 1.5
            ...
            # Separable filter: pass tuple of 1D kernels
            kernel = (kernel_1d, kernel_1d)

            # Apply blur with custom kernel
            blurred = px.gaussian_blur(sample_image, ksize=kernel_size, sigma=sigma, kernel=kernel)
    ...
    def test_gaussian_blur_performance(self, sample_image):
        ...
        with px.Device(0):
            ...
            sizes = [
                ...
                100,
                500,
            ]  # Larger size difference for more reliable comparison
            times = []
            ...
            for size in sizes:
                ...
                # Time the blur operation with warmup
                cp.cuda.Stream.null.synchronize()
                # Warmup run to eliminate cold start effects
                _ = px.gaussian_blur(resized, ksize=9, sigma=2.0)
                cp.cuda.Stream.null.synchronize()
            ...
            # Larger images should take more time
            # Only check extremes due to GPU overhead variability
            assert times[2] > times[0], (
                f"Expected {sizes[2]}x{sizes[2]} ({times[2]}ms) > {sizes[0]}x{sizes[0]} ({times[0]}ms)"
            )
    ...
    def test_gaussian_blur_multichannel(self):
        ...
        with px.Device(0):
            ...
            for channels in [1, 3, 4]:
                ...
                blurred = px.gaussian_blur(image, ksize=5, sigma=1.0)
    ...
    def test_gaussian_blur_preserves_range(self, sample_image):
        ...
        with px.Device(0):
            # Apply blur
            blurred = px.gaussian_blur(sample_image, ksize=9, sigma=3.0)
