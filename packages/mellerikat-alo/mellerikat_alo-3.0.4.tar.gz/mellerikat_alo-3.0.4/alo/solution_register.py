...
REGISTER_EXPPLAN = os.path.join(REGISTER_HOME, "experimental_plan.yaml")
REGISTER_SOLUTION_EXPPLAN = os.path.join(REGISTER_SOLUTION_PATH, "experimental_plan.yaml")
...
def gpu_dockerfile_message():
    msg = """
    ================================================================================
    <주의> - GPU 사용자
    solution_info.yaml 의 train 혹은 inference의 gpu=true로 설정하신 경우,
    사전에 gpu 용 Dockerfile을 미리 셋팅해두셔야 합니다. alo register를 실행하기 전에,
    아래 명령어를 통해 가이드에 따라 GPU Dockerfile을 작성해주세요.
...
class SolutionRegister:
    ...
    def __init__(self, user_id: str, user_pw: str, command_update=None):
        ...
        try:
            ...
            self.allowed_pipes = ['inference'] if self.solution_info.inference.only else ['train', 'inference']
        ...
        if self.solution_info.contents_type.support_labeling:
            try:
                ...
                if os.path.exists(TRAIN_SUMMARY):
                    logger.info(f"Skip validating {TRAIN_SUMMARY}")
                else:
    ...
    def init_setup(self, nalo=True):
        ...
        if not nalo:
            for file in os.listdir(PROJECT_HOME):
                if file == 'alo' and os.path.isdir(file):
                    raise AloErrors['ALO-SSA-012']('Registration has been stopped due to limits',
                                                   doc={'message': "The 'alo' directory cannot be registered. Remove or rename alo directory."})
    ...
    def _set_alo(self, nalo=True):
        ...
        # Check if Dockerfiles and required files exist before copying
        requirements_path = os.path.join(SOURCE_HOME, 'Dockerfiles', 'requirements.txt')
        main_path = os.path.join(SOURCE_HOME, 'Dockerfiles', 'main.py')

        if os.path.exists(requirements_path):
            shutil.copy2(requirements_path, REGISTER_SOURCE_PATH)
        else:
            logger.warning("File not found: %s", requirements_path)

        if os.path.exists(main_path):
            shutil.copy2(main_path, REGISTER_SOURCE_PATH)
        else:
            logger.warning("File not found: %s", main_path)

        # copy files of user
        exclude_dirs = ['.workspace', 'setting']
        if hasattr(settings.experimental_plan.solution, 'train') and settings.experimental_plan.solution.train and hasattr(settings.experimental_plan.solution.train, 'artifact_uri'):
            exclude_dirs.extend([Path(storage.url()).parts[0] for storage in settings.experimental_plan.solution.train.artifact_uri if isinstance(storage, LocalFile)])
        if hasattr(settings.experimental_plan.solution, 'inference') and settings.experimental_plan.solution.inference and hasattr(settings.experimental_plan.solution.inference, 'artifact_uri'):
            exclude_dirs.extend([Path(storage.url()).parts[0] for storage in settings.experimental_plan.solution.inference.artifact_uri if isinstance(storage, LocalFile)])
        #venc는 복사 하지 않는다
        ...
        for source_file in os.listdir(PROJECT_HOME):
            ...
            if os.path.isfile(source_path):
                ...
                logger.info("copy file : %s -->  %s ", source_file, f'{REGISTER_SOLUTION_PATH}{os.sep}')
        ...
        if not nalo:
            if settings.experimental_plan.solution.pip.requirements:
                with open(os.path.join(REGISTER_SOLUTION_PATH, 'requirements.txt'), 'w', encoding='utf-8') as f:
                    if settings.experimental_plan.solution.pip.requirements == True:
                        # requirements.txt 파일이 있는 경로 (solution_meta.yaml이 있는 폴더)
                        req_file = os.path.join(os.path.dirname(settings.config), 'requirements.txt')
                        if not os.path.exists(req_file):
                            raise AloErrors["ALO-INI-003"](req_file)
                        # requirements.txt 파일 읽어서 내용 복사
                        with open(req_file, 'r', encoding='utf-8') as req:
                            for line in req:
                                if line.strip() and not line.startswith('#'):  # 빈 줄과 주석 제외
                                    f.write(line)
                    else:
                        for package in settings.experimental_plan.solution.pip.requirements:
                            f.write(f"{package}\n")
        ...
        if not nalo:
            for pipe in self.allowed_pipes:
                if not getattr(settings.experimental_plan.solution, pipe):
                    raise ValueError(f"{pipe} pipeline not defined in experimental_plan.yaml")
                for uri_type in ['dataset_uri', 'model_uri']:
                    local_files = getattr(getattr(settings.experimental_plan.solution, pipe), uri_type)
                    if not local_files:
                        continue
                    replace_files = []
                    for local_file in local_files:
                        if isinstance(local_file, LocalFile):
                            ...
                            if not os.path.isabs(local_file.path):
                                ...
                                continue
                            target_path = os.path.join(REGISTER_SOLUTION_PATH,
                                                    os.path.basename(local_file.path.rstrip(os.sep))) + (os.sep if os.path.isdir(local_file.path.rstrip(os.sep)) else "")
                    ...
                    if replace_files:
                        with open(REGISTER_EXPPLAN, 'r', encoding='utf-8') as file:
                            yaml_content = yaml.safe_load(file)
                        yaml_content['solution'][pipe][uri_type] = replace_files if len(replace_files) > 1 else replace_files[0]
                        with open(REGISTER_EXPPLAN, 'w', encoding='utf-8') as file:
    ...
    def set_description(self):
        ...
        # detail 검증
        detail_list = []
        if hasattr(self.solution_info, 'detail') and self.solution_info.detail:
            assert isinstance(self.solution_info.detail, list), "detail must be a list"
            for d in self.solution_info.detail:
                assert isinstance(d.title, str) and isinstance(d.content, str), \
                    "title and content must be strings"
                if (not check_str_bytes(d.title, bytes_limit=5000)) or (not check_str_bytes(d.content, bytes_limit=5000)):
                    raise AloErrors['ALO-SSA-002']("The title and content in detail must be under 5000 bytes.",
                                                doc={'field': d})

                # ContentTitle 객체를 딕셔너리로 변환하여 리스트에 추가
                detail_list.append({
                    'title': d.title,
                    'content': d.content
                })
    ...
    def set_user_parameters(self, display_table=False):
        ...
        # API 설정이 있는 경우 처리
        if exp_yaml.get('api') is not None:
            self.__api_mode = True
            api_config = deepcopy(exp_yaml['api'])
            api_structure = {
                'type': 'api',
                'config': {
            ...
            self._sol_me.pipeline[self.sm_pipe_pointer].type = 'api'
        ...
        # 기존 train/inference 파이프라인 처리
            ...
            for pipe_name in self.allowed_pipes:
                if pipe_name in params:
                    ...
                    if sm_pipe_type == pipe_name:
                        ...
                        for step in pipeline['pipeline']:
                            ...
                            output_data = {'step': step, 'args': {}}
                            selected_user_parameters.append(output_data.copy()) # Edgeconductor를 위해 비워둠
                            # user_parameters.append(output_data.copy()) # UI detail값
                            # output_data = {'step': step, 'args': params['function'][step]['argument']}
                            ...
                            user_param = {'step': step, 'args': []}
                            if 'function' in self.solution_info.ui_args and step in self.solution_info.ui_args['function']:
                                args_list = self.solution_info.ui_args['function'][step]
                                # 기존 딕셔너리를 리스트로 변환합니다.
                                # user_param = {'step': step, 'args': [args_list]}
                                user_param['args'] = args_list
                            # if 'function' in params and step in params['function']:
                            #     if 'argument' in params['function'][step]:
                            #         for key, value in params['function'][step]['argument'].items(): # key가 value인 경우 없에서 cadidate에 작성
                            #             if isinstance(value, dict) and 'value' in value:
                            #                 user_param['args'][key] = value['value'] # FIXME detail 값을 넣어야됨
                        ...
                        try:
                            if step in params['function'] and 'argument' in params['function'][step]:
                                ...
                                for key, value in args.items():
                                    ...
                                    self.candidate_format[pipe_name][step_idx]['args'] = new_dict2
                        except:
                            self.candidate_format[pipe_name][step_idx]['args'] = {}
                        step_idx += 1
    ...
    def s3_upload_data(self, is_eva=False):
        ...
        if is_eva or self.__api_mode and not os.path.exists(local_folder):
            try:
                # 디렉토리 생성
            ...
            raise AloErrors['ALO-SSA-012']('Registration has been stopped due to limits',
                                        doc={'message': "After executing the `alo run` command at least once, re-register."})
    ...
    def _set_dockerfile(self):
        ...
        with open(REGISTER_EXPPLAN, 'r', encoding='utf-8') as file:
            yaml_content = yaml.safe_load(file)
        try:
            all_pipes = ['train', 'inference']
            del_pipe = all_pipes[(all_pipes.index(self.pipeline) + 1) % 2]
            if del_pipe in yaml_content['solution']:
                del yaml_content['solution'][del_pipe]
            with open(REGISTER_SOLUTION_EXPPLAN, 'w', encoding='utf-8') as file:
                yaml.safe_dump(yaml_content, file)
            # Check if Dockerfile exists in current directory - when alo docker command executed earlier
            current_dockerfile = os.path.join(os.getcwd(), 'Dockerfile')
            use_gpu =  self.solution_info.train.gpu if self.pipeline == 'train' else self.solution_info.inference.gpu
            if os.path.exists(current_dockerfile):
                if use_gpu:
                    logger.warning(gpu_dockerfile_message())
                ...
                if use_gpu:
                    raise AloErrors['ALO-SSA-009']("Failed Dockerfile setting.", doc={'phase': self.pipeline, 'message': gpu_dockerfile_message()})
                # Use default Dockerfile if no user Dockerfile exists ~ CPU Dockerfile
                shutil.copy(os.path.join(SOURCE_HOME, "Dockerfiles", "register", "dockerfile_cpu", "Dockerfile"),
    ...
    def _make_buildspec_commands(self):
        ...
        if self.solution_info.update is True:
            ...
            # 신규 등록 모드
            pre_command.append(
                f'aws ecr describe-repositories --repository-names {self.ecr_repo} '
                f'--region {self.infra_config["REGION"]} || '
                f'echo "Repository does not exist"'
            )

            pre_command.append(
                f'if aws ecr describe-repositories --repository-names {self.ecr_repo} '
                f'--region {self.infra_config["REGION"]}; then '
                f'aws ecr delete-repository --repository-name {self.ecr_repo} '
                f'--region {self.infra_config["REGION"]} --force; '
                f'fi'
            )
    ...
    #         buildspec   (dict): codebuild buildspec
    #     """
    #     def get_arg_value(dockerfile_str, arg_name):
    #         arg_prefix = f'ARG {arg_name}='
    #         for line in dockerfile_str.split('\n'):
    #             if line.startswith(arg_prefix):
    #                 return line.split('=')[1]
    #         return None

    #     with open(os.path.join(AWS_CODEBUILD_ZIP_PATH, "Dockerfile"), 'r') as file:
    #         file_content = file.read()

    #     docker_args = {
    #         "ALO_BASE_IMAGE": get_arg_value(file_content, 'ALO_BASE_IMAGE'),
    #         "ALO_PYTHON_VER": "",
    #         "ALO_CUDA_VER": "",
    #         "ALO_CUDNN_VER": "",
    #     }

    #     build_arg = " ".join([f"--build-arg {k}={v}" for k, v in docker_args.items() if v])
    ...
    #         pre_command.append(f'docker pull {self.ecr_full_url}:v{self.__version_num - 1}')
    #         build_command.append(
    #             f'docker build --build-arg BUILDKIT_INLINE_CACHE=1 {build_arg} --cache-from {self.ecr_full_url}:v{self.__version_num - 1} -t {self.ecr_full_url}:v{self.__version_num} .')
    #     else:
    ...
    #         build_command.append(f'docker build --build-arg BUILDKIT_INLINE_CACHE=1 {build_arg} -t {self.ecr_full_url}:v{self.__version_num} .')
    ...
    def _make_codebuild_s3_project(self, bucket_uri, codebuild_role):
        ...
        codebuild_project_json['tags'] = self.infra_config["REPOSITORY_TAGS"] if ("REPOSITORY_TAGS" in self.infra_config and self.infra_config["REPOSITORY_TAGS"]) else []
        codebuild_project_json['tags'] =  [{k.lower(): v for k, v in d.items()} for d in codebuild_project_json['tags']] # Key, Value --> key, value
    ...
    def make_docker(self, skip_build=False):
        ...
        """
        assert self.infra_config['BUILD_METHOD'] in ['docker', 'buildah', 'codebuild', 'local']
        is_local = (self.infra_config['BUILD_METHOD'] == 'local')
        if is_local:
            if not self.infra_config['CONTAINER_NAME'] or not self.infra_config['CONTAINER_TAG'] :
                raise Exception(f"CONTAINER_NAME or CONTAINER_TAG is empty.") from e
            ...
            if not skip_build:
                ...
                if not is_remote:
                    ...
                    self._create_ecr_repository(self.infra_config["REPOSITORY_TAGS"] if "REPOSITORY_TAGS" in self.infra_config else [])
                else:
                    pass
                    # build docker image
    ...
    def docker_push(self):
        ...
        """
        image_tag = f"{self.ecr_full_url}:v{self.__version_num}"
        if self.infra_config['BUILD_METHOD'] == 'docker' or self.infra_config['BUILD_METHOD'] == 'local':
            try:
                if self.infra_config['BUILD_METHOD'] == 'local':
                    # Tag the external image with the desired ECR tag
                    self.docker_client.images.get(f"{self.infra_config['CONTAINER_NAME']}:{self.infra_config['CONTAINER_TAG']}").tag(image_tag)
        ...
        try:
            ...
            try:
                ...
                try:
                    if is_local:
                        # Docker 작업
                        # 현재 컨테이너 이미지가 있으면 docker를 만들지 않고 push 하는 코드 추가 필요
                        build_id = self.make_docker(False)
                        self._set_aws_ecr()  # set aws ecr
                        self._ecr_login(is_docker='Docker')
                        logger.info("======= Create ECR For EVA Repository =======")
                        self._create_ecr_repository(self.infra_config["REPOSITORY_TAGS"] if "REPOSITORY_TAGS" in self.infra_config else [])
                        self.docker_push()
    ...
    def _validate_summary_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if any(summary.get(key) == value for key, value in default_values.items()):
    ...
    def cleanup_all_resources(self):
        ...
        try:
            ...
            raise AloErrors['ALO-SSA-015']("Docker cleanup error",
        ...
        try:
            ...
            raise AloErrors['ALO-SSA-016']("Artifacts cleanup error",
                                        doc={'solution_name': self.__name,
                                                'errors': str(e)})
        ...
        try:
            ...
            raise AloErrors['ALO-SSA-017']("S3 data cleanup error",
                                        doc={'solution_name': self.__name,
                                                'errors': str(e)})
