...
class AloInitNameError(AloError):
    ...
    에러 원인
        제작하신 AI Soltuion의 {file}이 python 파일 이름 혹은 함수 명에 'alo' 혹은 'main'을 포함합니다.

    조치 가이드
        | {file}의 'alo' 혹은 'main' 부분을 수정해주세요.
        | Ex) titanic.main -> titanic.train # titanic.py 내 main 함수 명 수정 시
        | Ex) main.train -> titanic.train   # main.py 파일 명 수정 시
    """
...
class AloDatasetKeyNotFoundError(AloError):
    ...
    조치 가이드
        | Key는 experimental_plan.yaml 파일의 solution.{stage}.dataset_uri에 설정된 경로 이하 또는 압축 파일 내에 폴더명 및 확장자명을 포함한 경로 형태여야 합니다.
...
class AloModelFileStorageLimitError(AloError):
    ...
    참고 사항
        - experimental_plan.yaml 상에 solution.train.dataset_uri은 및 solution.inference.dataset_uri의 경로는 폴더까지만 입력합니다.
          string, list type 입력이 가능합니다.
            >>> dataset_uri: sample_dataset/                           # 정상
            >>> dataset_uri: ['sample_dataset/']                       # 정상
            >>> dataset_uri: ['sample_dataset1/', 'sample_dataset2/']  # 정상
            >>> dataset_uri: sample_dataset/dataset.csv                # 오류 - 폴더가 아닌 파일 경로 입력 불가

        - 로컬 경로의 파일 지정시 experimental_plan.yaml 파일 기준 상대 경로로 작성해야 합니다.
            >>> Ex) 폴더 트리 인 경우
            >>> ./
            ...
            >>> └── titanic.py
            >>> dataset_uri: sample_dataset/                            # 정상
            >>> dataset_uri: /home/user/alo/titanic/sample_dataset/     # 오류 - 절대 경로로 지정함
    """
...
class AloContextInvalidStrError(AloError):
    ...
    조치 가이드
        ...
        아래와 같이 사용가능한 문자열로 변경하세요.
            >>> context['model']['key\\test.csv'] = 'test'  # 역슬래쉬(\) 사용에 의한 오류
...
class AloExperimentalPlanFileNotFoundError(AloError):
    ...
    에러 원인(제약 사항)
        experimental_plan.yaml 파일을 찾을 수 없습니다.
        조치 가이드 내용과 같이 파일 경로 확인 또는 옵션을 지정하세요.

    조치 가이드
        experimental_plan.yaml 파일 우선순위는 아래와 같습니다.

        1. --config 옵션 지정시
          Ex) alo --config /mellerikat/experimental_plan.yaml
        2. git repository url 옵션 지정시
          Ex) alo --git.url http://mellerikat.com/repository/titanic.git --git.branch main
          (titanic 이하 폴더에 experimental_plan.yaml 파일이 존재해야만 함)
        3. 사용자 현재 경로 이하에 experimental_plan.yaml 파일이 존재하는 경우
        4. python alo package에 설치된 경로에 solution/experimental_plan.yaml 파일이 존재하는 경우

    참고 사항
        사용자 python 파일을 로딩하기 위해서 experimental_plan.yaml 파일이 존재하는 폴더 이하에 py파일 생성 후
        yaml의 solution.function 항목에 실행 대상 함수를 "모듈명.함수명" 형식으로 정의하세요.
            >>> Ex) 폴더 트리
            >>> ./
            >>> ├── experimental_plan.yaml
            >>> └── titanic.py
        Ex) experimental_plan.yaml 사용자 함수 정의 작성 방법
            >>> function:
            >>>   preprocess:                  # 함수 이름
            >>>     def: titanic.preprocess    # 모듈명.함수명(python 파일명.def 함수명)
            >>>   train:
            >>>     def: titanic.train
            >>>     argument:
            >>>       x_columns: [ 'Pclass', 'Sex', 'SibSp']
            >>>       y_column: Survived
            >>>   inference:
...
class AloSolutionStreamApiStatusCodeError(AloError):
    """
     ...
     에러 원인
        ...
        [주의] Solution 등록 도중에 AI Conductor 웹 브라우저에 로그인하면 등록이 중단됩니다.
     """
...
class AloSolutionStreamFileNotFoundError(AloError):
    ...
    참고 사항
        ALO 실행 및 솔루션 등록을 위해서는 아래 예제와 같이 파일 구성이 필요합니다.
            >>> ./
            >>> ├── experimental_plan.yaml      # ALO 실행 환경 설정 파일
            >>> ├── inference/
...
class AloSolutionStreamDockerFailError(AloError):
    """
     ...
     에러 원인
        Dockerfile 생성 작업 중 오류가 확인되었습니다.
        ALO 관리자에게 에러 메시지와 함께 문의하세요.

     참고사항
        solution_info.yaml에 gpu 사용을 true로 설정하신 경우, 사전에
        alo docker --gpu 를 통해 gpu 용 dockerfile 작성을 수행해야 합니다.
...
class AloSolutionStreamDefaultSummaryError(AloError):
    """
    에러 코드
        {code}
    ...
    에러 원인
        Summary 파일이 기본값을 가지고 있어 솔루션 등록이 불가능합니다.
        Train/Inference가 정상적으로 실행되지 않았거나, 실행 결과가 저장되지 않았을 수 있습니다.

    조치 가이드
        1. Train/Inference pipeline에서 inference summary를 제작하도록 하세요.
        2. Summary 파일의 각 필드가 아래 기본값과 같은지 확인하세요:
           - probability: {{}}
           - result: None
           - score: None
        3. 기본값과 같은 경우 문제가 생길 수 있습니다

    참고 사항
        Summary 파일은 Train/Inference 실행 시 사용자가 제작하지 않으면 자동으로 default 값으로 생성됩니다.
        support_labeling을 사용하시려면 default 값이 아니고 사용자가 직접 summary를 제작하셔야 합니다
    """

    code = "ALO-SSA-014"

class AloSolutionStreamDockerCleanupError(AloError):
    """
    에러 코드
        {code}
    ...
    에러 원인
        Docker 리소스를 정리하는 동안 에러가 발생한 경우

    조치 가이드
        1. LOG 파일을 확인 후 ALO 관리자엑게 문의 하세요

    참고 사항
    """
    code = "ALO-SSA-015"

class AloSolutionStreamArtifactsCleanupError(AloError):
    """
    에러 코드
        {code}
...
class AloSolutionStreamS3DataCleanupError(AloError):
    """
    에러 코드
        {code}
...
class AloApiInitError(AloError):
    """
    에러 코드
        {code}
    ...
    에러 원인
        API 서버 초기화 중에 오류가 발생했습니다.
        에러 메시지를 확인하고 조치하세요.

    조치 가이드
        1. experimental_plan.yaml 파일이 존재하는지 확인
        2. 필요한 모델 파일들이 올바른 위치에 있는지 확인
        3. 설정값들이 올바르게 지정되었는지 확인
    """
    code = "ALO-API-001"


class AloApiRequestError(AloError):
    """
    에러 코드
        {code}
    ...
    에러 원인
        ...
        요청 데이터가 올바르지 않거나 필수 파라미터가 누락되었을 수 있습니다.

    조치 가이드
        1. 요청 데이터의 형식이 올바른지 확인
        2. 필수 파라미터가 모두 포함되어 있는지 확인
        3. 파라미터 값들이 허용된 범위 내인지 확인
    """
    code = "ALO-API-002"


class AloApiInferenceError(AloError):
    """
    에러 코드
        {code}
    ...
    에러 원인
        API를 통한 inference 실행 중 오류가 발생했습니다.
        모델 실행 또는 결과 처리 과정에서 문제가 발생했을 수 있습니다.

    조치 가이드
        1. 모델 파일이 올바르게 로드되었는지 확인
        2. 입력 데이터가 모델의 요구사항과 일치하는지 확인
        3. 메모리나 GPU 리소스가 충분한지 확인
    """
    code = "ALO-API-003"


class AloApiAuthenticationError(AloError):
    """
    에러 코드
        {code}
    ...
    에러 원인
        API 인증 과정에서 오류가 발생했습니다.
        인증 정보가 올바르지 않거나 만료되었을 수 있습니다.
...
class AloApiResourceError(AloError):
    """
    에러 코드
        {code}
    ...
    에러 원인
        API 서버의 리소스 관리 중 오류가 발생했습니다.
        시스템 리소스가 부족하거나 접근 권한이 없을 수 있습니다.

    조치 가이드
        1. 시스템 리소스(메모리, CPU, 디스크 등) 사용량 확인
        2. 파일 시스템 권한 확인
        3. 임시 파일들이 적절히 정리되고 있는지 확인
    """
    code = "ALO-API-005"
