...
class EditCommandDialog(QDialog):
    ...
    def _create_ui(self):
        ...
        # 删除按钮（在保存按钮左侧）
        self.delete_button = QPushButton("🗑️ 删除指令")
        self.delete_button.setMinimumSize(100, 35)
    ...
    def _delete_command(self):
        ...
        reply = QMessageBox.question(
            self, "确认删除", 
            f"确定要删除指令文件 '{self.command_data.get('title', '未知')}' 吗？\n\n此操作不可撤销！",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No
        )
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.information(
                    self, "删除成功", 
                    f"指令文件 '{self.command_data.get('title', '未知')}' 已删除"
                )
