import { _ as w, D as te, E as ae, H as he, e as ue, l as J, aP as P, d as Y, b as pe, a as fe, p as ge, q as me, g as ye, s as Se, F as ve, aQ as xe, y as be } from "./mermaid.core-DqXbFyRH.js";
import { s as we } from "./chunk-QN33PNHL-BVOxQlDm.js";
import { p as Ce } from "./chunk-4BX2VUAB-C4zYEg_Z.js";
import { p as Te } from "./treemap-KMMF4GRG-CSQK1e7L.js";
import { b as O } from "./defaultLocale-DlTZ34it.js";
import { o as Z } from "./ordinal-CDDIbzuo.js";
function Le(t) {
...
function Ue(t, a, l, n, o, s) {
  for (var d = [], h = a.children, g, c, u = 0, b = 0, r = h.length, x, S, v = a.value, p, m, N, k, V, R, M; u < r; ) {
    ...
    d.push(g = {
      value: p,
      dice: x < S,
      children: h.slice(u, b)
    }), g.dice ? je(g, l, n, o, v ? n += S * p / v : s) : Ye(g, l, n, v ? l += x * p / v : o, s), v -= p, u = b;
  }
  return d;
}
...
var Ke = /* @__PURE__ */ w((t, a) => {
  ...
  const o = n.db, s = o.getConfig(), d = s.padding ?? tt, h = o.getDiagramTitle(), g = o.getRoot(), {
    themeVariables: c
  } = ae();
  if (!g)
    return;
  ...
  const V = b.append("g").attr("transform", `translate(0, ${u})`).attr("class", "treemapContainer"), R = K(g).sum((e) => e.value ?? 0).sort((e, i) => (i.value ?? 0) - (e.value ?? 0)), ee = Je().size([r, x]).paddingTop((e) => e.children && e.children.length > 0 ? X + B : 0).paddingInner(d).paddingLeft((e) => e.children && e.children.length > 0 ? B : 0).paddingRight((e) => e.children && e.children.length > 0 ? B : 0).paddingBottom((e) => e.children && e.children.length > 0 ? B : 0).round(!0)(R), se = ee.descendants().filter((e) => e.children && e.children.length > 0), W = V.selectAll(".treemapSection").data(se).enter().append("g").attr("class", "treemapSection").attr("transform", (e) => `translate(${e.x0},${e.y0})`);
  W.append("rect").attr("width", (e) => e.x1 - e.x0).attr("height", X).attr("class", "treemapSectionHeader").attr("fill", "none").attr("fill-opacity", 0.6).attr("stroke-width", 0.6).attr("style", (e) => e.depth === 0 ? "display: none;" : ""), W.append("clipPath").attr("id", (e, i) => `clip-section-${a}-${i}`).append("rect").attr("width", (e) => Math.max(0, e.x1 - e.x0 - 12)).attr("height", X), W.append("rect").attr("width", (e) => e.x1 - e.x0).attr("height", (e) => e.y1 - e.y0).attr("class", (e, i) => `treemapSection section${i}`).attr("fill", (e) => m(e.data.name)).attr("fill-opacity", 0.6).attr("stroke", (e) => N(e.data.name)).attr("stroke-width", 2).attr("stroke-opacity", 0.4).attr("style", (e) => {
    ...
    if (e.depth === 0)
      return "display: none;";
    const i = "dominant-baseline: middle; font-size: 12px; fill:" + k(e.data.name) + "; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;", f = P({
      cssCompiledStyles: e.data.cssCompiledStyles
    });
    ...
    if (e.depth === 0)
      return "display: none;";
    const i = "text-anchor: end; dominant-baseline: middle; font-size: 10px; fill:" + k(e.data.name) + "; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;", f = P({
      cssCompiledStyles: e.data.cssCompiledStyles
    });
    return i + f.labelStyles.replace("color:", "fill:");
  });
  ...
  j.append("rect").attr("width", (e) => e.x1 - e.x0).attr("height", (e) => e.y1 - e.y0).attr("class", "treemapLeaf").attr("fill", (e) => e.parent ? m(e.parent.data.name) : m(e.data.name)).attr("style", (e) => P({
    cssCompiledStyles: e.data.cssCompiledStyles
  }).nodeStyles).attr("fill-opacity", 0.3).attr("stroke", (e) => e.parent ? m(e.parent.data.name) : m(e.data.name)).attr("stroke-width", 3), j.append("clipPath").attr("id", (e, i) => `clip-${a}-${i}`).append("rect").attr("width", (e) => Math.max(0, e.x1 - e.x0 - 4)).attr("height", (e) => Math.max(0, e.y1 - e.y0 - 4)), j.append("text").attr("class", "treemapLabel").attr("x", (e) => (e.x1 - e.x0) / 2).attr("y", (e) => (e.y1 - e.y0) / 2).attr("style", (e) => {
    const i = "text-anchor: middle; dominant-baseline: middle; font-size: 38px;fill:" + k(e.data.name) + ";", f = P({
      cssCompiledStyles: e.data.cssCompiledStyles
    });
    return i + f.labelStyles.replace("color:", "fill:");
    ...
    let I = Math.max(z, Math.min(T, Math.round(y * L))), U = y + H + I;
    for (; U > F && y > _ && (y--, I = Math.max(z, Math.min(T, Math.round(y * L))), !(I < z && y === _)); )
    ...
    const f = "text-anchor: middle; dominant-baseline: hanging; font-size: 28px;fill:" + k(i.data.name) + ";", C = P({
      cssCompiledStyles: i.data.cssCompiledStyles
    });
    ...
    const A = parseFloat($.style("font-size")), D = 28, F = 0.6, y = 6, _ = 2, T = Math.max(y, Math.min(D, Math.round(A * F)));
