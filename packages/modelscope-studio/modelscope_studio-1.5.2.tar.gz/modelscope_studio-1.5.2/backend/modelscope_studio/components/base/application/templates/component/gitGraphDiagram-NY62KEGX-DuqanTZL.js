import { p as Z } from "./chunk-4BX2VUAB-C4zYEg_Z.js";
import { I as F } from "./chunk-QZHKN3VN-D6qaqmcX.js";
import { _ as h, q as U, p as rr, s as er, g as tr, a as ar, b as nr, l as m, c as sr, d as or, u as cr, C as ir, y as dr, k as B, D as hr, E as lr, F as $r, G as fr } from "./mermaid.core-DqXbFyRH.js";
import { p as gr } from "./treemap-KMMF4GRG-CSQK1e7L.js";
var u = {
  NORMAL: 0,
  REVERSE: 1,
  HIGHLIGHT: 2,
...
var Cr = /* @__PURE__ */ h(function() {
  ...
  if (n === u.HIGHLIGHT)
    ...
    n === u.REVERSE && t.append("path").attr("d", `M ${a.x - 5},${a.y - 5}L${a.x + 5},${a.y + 5}M${a.x - 5},${a.y + 5}L${a.x + 5},${a.y - 5}`).attr("class", `commit ${s} ${r.id} commit${e % O}`);
  }
  ...
  if (r.tags.length > 0) {
    ...
    for (const {
      ...
      ${a.posWithOffset + c / 2 + k},${p - y - L}
      ${a.posWithOffset + c / 2 + k},${p + y + L}
      ${a.posWithOffset - c / 2 - k},${p + y + L}`), f.attr("cy", p).attr("cx", s - c / 2 + k / 2).attr("r", 1.5).attr("class", "tag-hole"), x === "TB" || x === "BT") {
        const w = s + d;
        ...
        ${a.x + R + c + 4},${w - y - 2}
        ${a.x + R + c + 4},${w + y + 2}
        ${a.x + R},${w + y + 2}`).attr("transform", "translate(12,12) rotate(45, " + a.x + "," + s + ")"), f.attr("cx", a.x + k / 2).attr("cy", w).attr("transform", "translate(12,12) rotate(45, " + a.x + "," + s + ")"), l.attr("x", a.x + 5).attr("y", w + 3).attr("transform", "translate(14,14) rotate(45, " + a.x + "," + s + ")");
      }
    }
  }
  ...
  x === "BT" && (o && Dr(l, r, n), l = l.reverse()), l.forEach((f) => {
    ...
    x === "TB" || x === "BT" ? E.set(g.id, {
      x: d.x,
      y: d.posWithOffset
    }) : E.set(g.id, {
      x: d.posWithOffset,
      y: d.y
    }), n = x === "BT" && o ? n + I : n + I + R, n > M && (M = n);
  });
  ...
  r.forEach((s, e) => {
    ...
    const y = l.getBBox();
    f.attr("class", "branchLabelBkg label" + n).attr("rx", 4).attr("ry", 4).attr("x", -y.width - 4 - ((b == null ? void 0 : b.rotateCommitLabel) === !0 ? 30 : 0)).attr("y", -y.height / 2 + 8).attr("width", y.width + 18).attr("height", y.height + 4), d.attr("transform", "translate(" + (-y.width - 14 - ((b == null ? void 0 : b.rotateCommitLabel) === !0 ? 30 : 0)) + ", " + (c - y.height / 2 - 1) + ")"), x === "TB" ? (f.attr("x", c - y.width / 2 - 10).attr("y", 0), d.attr("transform", "translate(" + (c - y.width / 2 - 5) + ", 0)")) : x === "BT" ? (f.attr("x", c - y.width / 2 - 10).attr("y", M), d.attr("transform", "translate(" + (c - y.width / 2 - 5) + ", " + M + ")")) : f.attr("transform", "translate(-19, " + (c - y.height / 2) + ")");
  });
}, "drawBranches"), te = /* @__PURE__ */ h(function(t, r, a, s, e) {
  return C.set(t, {
    pos: r,
    index: a
  }), r += 50 + (e ? 40 : 0) + (x === "TB" || x === "BT" ? s.width / 2 : 0), r;
