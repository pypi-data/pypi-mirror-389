...
impl PdfDocument {
        ...
        let (encrypt_ref, file_id) = {
            ...
            let file_id = match trailer_dict.get("ID") {
                Some(Object::Array(arr)) => {
                    if let Some(first_id) = arr.get(0) {
                        if let Some(id_bytes) = first_id.as_string() {
                            id_bytes.to_vec()
                        } else {
                            log::warn!("Invalid /ID array entry (not a string), using empty file ID");
                            vec![]
                        }
                    } else {
    ...
    fn decode_stream_with_encryption(&self, stream_obj: &Object, obj_ref: ObjectRef) -> Result<Vec<u8>> {
            ...
            if window == pattern_bytes {
                ...
                if distance >= -100 && distance <= 10 {
                    ...
                    let is_better = best_match
                        .as_ref()
                        .map_or(true, |(_, best_dist)| distance.abs() < best_dist.abs());
    ...
    fn extract_text_structure_order(
        ...
        for span in all_spans {
            if let Some(mcid) = span.mcid {
                mcid_map.entry(mcid).or_insert_with(Vec::new).push(span);
            } else {
