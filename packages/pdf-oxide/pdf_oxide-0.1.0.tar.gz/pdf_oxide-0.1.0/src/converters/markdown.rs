...
impl MarkdownConverter {
    ...
    fn format_links(text: &str) -> String {
        ...
        result = RE_URL.replace_all(&result, |caps: &Captures| {
            let url = &caps[1];
        ...
        result = RE_EMAIL.replace_all(&result, |caps: &Captures| {
            let email = &caps[1];
            // Don't format if already part of a markdown link or URL
            if result.contains(&format!("[{}]", email)) || result.contains(&format!("//{}",email)) {
                email.to_string()
            } else {
...
fn should_insert_bold_marker(prev_char: Option<char>, next_char: Option<char>) -> bool {
    match (prev_char, next_char) {
        ...
        // Common cases: )**=, )**-, )**+, )**<, )**>, etc.
        (Some(')'), Some(n)) if matches!(n, '=' | '-' | '+' | '<' | '>' | '*' | '/' | '&' | '|' | '^') => false,
        (Some(']'), Some(n)) if matches!(n, '=' | '-' | '+' | '<' | '>' | '*' | '/' | '&' | '|' | '^') => false,
        (Some('}'), Some(n)) if matches!(n, '=' | '-' | '+' | '<' | '>' | '*' | '/' | '&' | '|' | '^') => false,
