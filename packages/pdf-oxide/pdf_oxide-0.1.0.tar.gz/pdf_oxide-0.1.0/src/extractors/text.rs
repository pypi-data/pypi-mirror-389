...
fn fallback_char_to_unicode(char_code: u16) -> String {
    match char_code {
        ...
        code if (code <= 0xD7FF || (code >= 0xE000 && code <= 0xF8FF)) => {
...
fn decode_text_to_unicode(
    ...
    if let Some(font) = font {
        if font.base_font == "F1" {
            let hex_bytes: String = bytes.iter()
                .map(|b| format!("{:02X}", b))
                .collect::<Vec<_>>()
                .join(" ");
            log::warn!(
        ...
        if is_type0 && bytes.len() >= 2 {
            ...
            while i < bytes.len() {
                if i + 1 < bytes.len() {
                    ...
                    let char_str = font.char_to_unicode(char_code)
...
impl TextExtractor {
    ...
    fn deduplicate_overlapping_chars(&mut self) {
        ...
        for ch in self.chars.iter() {
            ...
            if !should_skip {
                ...
                log::trace!("Deduplicating overlapping char '{}' at X={:.1}, Y={:.1} (too close to previous)",
                           ch.char, x, ch.bbox.y);
            }
        }

        log::debug!("Deduplicated {} overlapping characters ({} -> {} chars)",
                   self.chars.len() - deduplicated.len(), self.chars.len(), deduplicated.len());
    ...
    fn sort_spans_by_reading_order(&mut self) {
        ...
        log::info!("Column detection: found {} columns from {} spans", columns.len(), self.spans.len());
        for (i, (left, right)) in columns.iter().enumerate() {
            log::info!("  Column {}: X range [{:.1}, {:.1}] (width: {:.1})", i, left, right, right - left);
        }
    ...
    fn deduplicate_overlapping_spans(&mut self) {
        ...
        for span in self.spans.iter() {
            ...
            if !should_skip {
                ...
                log::trace!("Deduplicating overlapping span '{}' at X={:.1}, Y={:.1} (too close to previous)",
                           span.text, x, span.bbox.y);
            }
        }

        log::debug!("Deduplicated {} overlapping spans ({} -> {} spans)",
                   self.spans.len() - deduplicated.len(), self.spans.len(), deduplicated.len());
    ...
    fn merge_adjacent_spans(&mut self) {
        ...
        for span in self.spans.iter() {
            ...
            let should_merge = same_line && gap >= -0.5 && gap < 3.0 && !large_gap_indicates_column;
            ...
            if should_merge {
                ...
                let merged_text = if needs_space {
                    ...
                    if needs_space_by_heuristic && !needs_space_by_gap {
                        log::trace!("Heuristic space insertion: '{}' | '{}'", current.text, span.text);
                    } else if gap > 0.1 && gap <= space_threshold {
                        log::trace!("Aggressive space insertion (gap={:.2}pt < threshold): '{}' | '{}'",
                ...
                log::trace!("Merged spans: '{}' + '{}' -> '{}' (gap={:.1}pt)",
                           old_text, span.text, current.text, gap);
                ...
                if same_line {
                    log::trace!("Not merging spans (gap={:.1}pt > 3pt): '{}' | '{}'",
                               gap, current.text, span.text);
                }
                merged.push(current);
                current_span = Some(span.clone());
    ...
    fn execute_operator(&mut self, op: Operator) -> Result<()> {
        match op {
                    ...
                    for element in array {
                        match element {
                            ...
                            TextElement::Offset(offset) => {
                                ...
                                let tx = -offset / 1000.0 * state.font_size * state.horizontal_scaling
                                    / 100.0;
            ...
            Operator::SetFillColor { components } => {
                ...
                match state.fill_color_space.as_str() {
                    ...
                    "Lab" if components.len() == 3 => {
                        ...
                        log::debug!("Lab color space simplified to grayscale (full conversion not yet implemented)");
                    },
                    ...
                    "ICCBased" => {
                        ...
                        log::debug!("ICCBased color space using simplified conversion (ICC profile not processed)");
                    },
                    ...
                    _ => {
                        ...
                        log::warn!("Unsupported fill color space: {} with {} components",
                                   state.fill_color_space, components.len());
                    }
                }
            },
            Operator::SetStrokeColor { components } => {
                ...
                match state.stroke_color_space.as_str() {
                    ...
                    _ => {
                        ...
                        log::warn!("Unsupported stroke color space: {} with {} components",
                                   state.stroke_color_space, components.len());
                    }
                }
            },
            ...
            Operator::SetExtGState { dict_name } => {
                ...
                log::debug!("ExtGState '{}' referenced (transparency/blend modes not yet fully supported)", dict_name);
            ...
            Operator::PaintShading { name } => {
                ...
                log::debug!("Shading pattern '{}' referenced (gradients not rendered in text extraction)", name);
            },
            Operator::InlineImage { dict, data } => {
                ...
                let width = dict.get("W").and_then(|obj| {
                    match obj {
                        Object::Integer(i) => Some(*i),
                        _ => None,
                    }
                }).unwrap_or(0);
                let height = dict.get("H").and_then(|obj| {
                    match obj {
                        Object::Integer(i) => Some(*i),
                        _ => None,
                    }
    ...
    fn flush_tj_buffer(&mut self, buffer: &TjBuffer) -> Result<()> {
        ...
        let span = TextSpan {
            ...
            font_name: buffer.font_name.clone().unwrap_or_else(|| "Unknown".to_string()),
            font_size: effective_font_size,
            font_weight,
            color: Color::new(
        ...
        for element in array {
            ...
            match element {
                TextElement::String(s) => {
                    ...
                    let unicode_text = if let Some(font_name) = self.state_stack.current().font_name.as_ref() {
                        if let Some(font) = self.fonts.get(font_name) {
    ...
    fn show_text(&mut self, text: &[u8]) -> Result<()> {
        for &byte in text {
            ...
            let unicode_string = if let Some(font) = font {
                let result = font.char_to_unicode(char_code)
                    .unwrap_or_else(|| "?".to_string());

                // DEBUG: Log when we get 'd' or ρ to trace the issue
                if result == "d" || result.contains('ρ') || result.contains('r') && char_code == 0x72 {
                    log::info!(
