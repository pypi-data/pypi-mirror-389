...
fn export_pdf_to_html(
    ...
    let file_stem = pdf_path.file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("unknown");
...
fn main() {
    ...
    println!("Mode: {}", if config.preserve_layout { "Layout-preserved" } else { "Semantic" });
    println!();
    ...
    for (i, (pdf_path, category)) in pdfs.iter().enumerate() {
        ...
        match export_pdf_to_html(pdf_path, category, &config.output_dir, config.preserve_layout, config.verbose) {
            Ok(bytes) => {
                println!("✓ ({} bytes)", bytes);
                success_count += 1;
            },
