...
fn xy_cut_with_sigma(
    ...
    let child1 = xy_cut_with_sigma(region1, blocks, &blocks1, depth + 1, max_depth, min_region_size, sigma);
    let child2 = xy_cut_with_sigma(region2, blocks, &blocks2, depth + 1, max_depth, min_region_size, sigma);
...
fn horizontal_projection(region: &Rect, blocks: &[TextBlock], indices: &[usize], sigma: f32) -> Vec<f32> {
...
///
/// * `sigma` - Gaussian smoothing parameter (default: 2.0 per Meunier ICDAR 2005)
fn vertical_projection(region: &Rect, blocks: &[TextBlock], indices: &[usize], sigma: f32) -> Vec<f32> {
    if region.width <= 0.0 {
        return vec![0.0];
    }
