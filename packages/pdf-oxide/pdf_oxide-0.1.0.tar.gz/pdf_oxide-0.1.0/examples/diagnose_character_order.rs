...
fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        ...
        eprintln!("\nExample:");
        eprintln!(
            "  cargo run --example diagnose_character_order test_datasets/pdfs/academic/arxiv_2510.21165v1.pdf"
        );
        std::process::exit(1);
    }
    ...
    let extraction_order_text: String = chars.iter().take(max_chars).map(|ch| ch.text.as_str()).collect();
    ...
    for ch in chars.iter().take(max_chars) {
        ...
        y_groups.entry(y_rounded).or_insert_with(Vec::new).push(ch);
    }
