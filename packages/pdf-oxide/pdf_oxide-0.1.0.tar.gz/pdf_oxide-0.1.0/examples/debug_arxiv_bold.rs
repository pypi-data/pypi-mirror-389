...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut pdf = PdfDocument::open("test_datasets/pdfs/academic/arxiv_2510.21165v1.pdf")?;
    ...
    for (i, span) in spans.iter().enumerate() {
        ...
        if text_lower.contains("chinese") || text_lower.contains("stock") || text_lower.contains("market") {
            let is_bold = span.font_weight.is_bold();
            println!(
                "{:3}. weight={:?} bold={:5} text={:?}",
                i, span.font_weight, is_bold, span.text
