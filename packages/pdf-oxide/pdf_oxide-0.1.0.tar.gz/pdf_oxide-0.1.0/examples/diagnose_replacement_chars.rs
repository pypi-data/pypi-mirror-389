...
fn main() {
    let pdf_path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "test_datasets/pdfs/government/CFR_2024_Title33_Vol1_Navigation_and_Navigable_Waters.pdf".to_string());
    ...
    if !repl_spans.is_empty() {
        ...
        for (i, span) in repl_spans.iter().take(10) {
            ...
            let repl_pos: Vec<usize> = span.text.char_indices()
                .filter(|(_, c)| *c == '\u{FFFD}')
                .map(|(pos, _)| pos)
                .collect();
    ...
    if repl_count > 0 {
        ...
        let lines_with_repl: Vec<&str> = markdown.lines()
