...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let pdf_path = "test_datasets/pdfs/academic/arxiv_2510.25760v1.pdf";
    let page_num = 12;
    ...
    for (i, pos) in replacement_positions.iter().take(20).enumerate() {
        ...
        if let Some(char_idx) = chars.iter().position(|(byte_pos, _)| *byte_pos == *pos) {
            ...
            let end_byte = chars.get(end_char_idx).map(|(b, _)| *b).unwrap_or(markdown.len());
    ...
    let unicode_non_ascii = markdown.chars().filter(|c| !c.is_ascii() && *c != '\u{FFFD}').count();
    let replacement_chars = replacement_positions.len();

    println!("{}", "=".repeat(80));
    println!("CHARACTER DISTRIBUTION:");
    println!("  Total: {} chars", total_chars);
    println!("  ASCII: {} ({:.1}%)", ascii_chars, (ascii_chars as f64 / total_chars as f64) * 100.0);
    println!("  Unicode (non-ASCII): {} ({:.1}%)", unicode_non_ascii, (unicode_non_ascii as f64 / total_chars as f64) * 100.0);
    println!("  Replacement (�): {} ({:.1}%)", replacement_chars, (replacement_chars as f64 / total_chars as f64) * 100.0);
    println!();
    ...
    let mut font_names: Vec<_> = spans.iter()
    ...
    for span in spans_with_issues {
        *fonts_with_issues.entry(span.font_name.as_str()).or_insert(0) += 1;
    }
