...
fn main() -> Result<(), Error> {
    ...
    let test_files = vec![
        "test_datasets/pdfs/newspapers/IA_0001-cdc-2015-american-tour.pdf",
        "test_datasets/pdfs/newspapers/IA_0001-cdc-2018-american-tour.pdf",
        "test_datasets/pdfs/newspapers/IA_0001-chambers-museum.pdf",
    ];
                ...
                match doc.catalog() {
                    Ok(catalog) => {
                        ...
                        if let Some(catalog_dict) = catalog.as_dict() {
                            println!("   Catalog keys: {:?}", catalog_dict.keys().collect::<Vec<_>>());
                                    ...
                                    match doc.load_object(pages_ref) {
                                        Ok(pages_obj) => {
                                            ...
                                            if let Some(pages_dict) = pages_obj.as_dict() {
                                                println!("   Pages dict keys: {:?}", pages_dict.keys().collect::<Vec<_>>());

                                                // Check for /Count
                                                if let Some(count_entry) = pages_dict.get("Count") {
                                                    println!("   ✅ /Count entry found: {:?}", count_entry);

                                                    if let Some(count_int) = count_entry.as_integer() {
                                                        println!("   ✅ /Count value: {}", count_int);
                                                    } else {
                                                        println!("   ❌ /Count is not an integer: {:?}", count_entry);
                                                    }
                                                } else {
                                                    println!("   ❌ /Count entry not found in /Pages");
                                                }

                                                // Check for /Kids
                                                if let Some(kids_entry) = pages_dict.get("Kids") {
                                                    println!("   ✅ /Kids entry found: {:?}", kids_entry);

                                                    if let Some(kids_array) = kids_entry.as_array() {
                                                        println!("   ✅ /Kids is an array with {} items", kids_array.len());
                                                    } else {
