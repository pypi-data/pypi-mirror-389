...
fn main() {
    let mut doc = PdfDocument::open("test_datasets/pdfs/government/CFR_2024_Title33_Vol1_Navigation_and_Navigable_Waters.pdf")
        ...
        match doc.extract_spans(page_num) {
            Ok(spans) => {
                ...
                for (i, span) in spans.iter().enumerate() {
                    ...
                    if text.contains("0 16") || text.contains("0–16") ||
                       text.contains("20402") || text.contains("96 511") {
                        ...
                        println!("    Spaces: {}, Dashes: {}, Replacements: {}",
        ...
        match doc.to_markdown(page_num, &ConversionOptions::default()) {
            Ok(markdown) => {
                ...
                if repl_count > 0 {
                    ...
                    let problem_lines: Vec<&str> = markdown.lines()
