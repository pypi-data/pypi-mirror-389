...
def create_pp_mvlv_lines(net, lines):
    ...
    create_lines(
        ...
        std_type=lines.apply(
            lambda x: (
                x.std_type
                if ("std_type" in x.keys() and str(x.std_type) != "nan")
                else {
...
def create_pp_trafos(net, grid_data):  # TODO: Umschreiben auf pp.create_trafos
    # pp funktionen
    ...
    if not trafos_mvlv.empty:
        # TODO: Anpassung an create_transformer function from pandapower
        ...
        trafos_mvlv["std_type"] = trafos_mvlv.apply(
            lambda x: (
                x.std_type
                if "std_type" in x.keys() and str(x.std_type) != "nan"
                else {
...
def create_pandapower(grid_data, opt_model, output_folder, save_data=True):
    ...
    """
    # set progress bar
    pbar = create_tqdm(desc="create pandapower network")
    ...
    if not all_buses.empty:
        # create bus names
        create_pp_buses(net, all_buses.reset_index(drop=True))
    # update progress
    pbar.update(15)
...
def power_processing(
    ...
    """
    # set progress bar
    pbar = create_tqdm(desc="run pandapower model processing")
    ...
    if opt_model:
        # set progress bar
        pbar = create_tqdm(desc="run power grid optimization")
        ...
        while "overload" in pp_diagnostic.keys():
            if (pp_diagnostic["overload"]["generation"]) and (net.sgen.scaling.min() >= 0.1):
                ...
                pp_diagnostic = diagnostic(net, report_style="None")
            elif (pp_diagnostic["overload"]["load"]) and (net.load.scaling.min() >= 0.1):
                # scale down sgens about 10%
