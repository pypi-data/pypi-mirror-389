...
def build_extended_attributes(revision, fields):
    ...
    rev_value = revision.get('rev')
    if rev_value is not None:
        extended_attrs['rev'] = rev_value
...
def main():
    ...
    if args.product is None:
        ...
        config = {
            'organization': org_url.replace('https://dev.azure.com/', '').replace('https://', ''),
            'personal_access_token': config.get('AZDO_TOKEN'),
