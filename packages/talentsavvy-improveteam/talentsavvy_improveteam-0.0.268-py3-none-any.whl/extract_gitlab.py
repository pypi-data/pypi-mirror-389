...
class GitLabExtractor:
    ...
    def main_with_connection(self, conn, start_date_str: Optional[str] = None):
        """Main extraction function using existing database connection."""
        self.db_connection = conn
        
        # Get configuration
        with conn.cursor() as cursor:
            config = self.get_config_from_database(cursor)
        ...
        # Process repositories with enrichment if in enriched mode
        all_events = self.process_repositories(base_url, repos, api_token, start_date, project, is_enriched_mode)
        logger.info(f"Processed {len(all_events)} total events")
        ...
        if not all_events:
            ...
            with self.db_connection.cursor() as cursor:
                total_events, inserted_count, duplicate_count = CodeCommitEvent.save_events_to_database(
                    all_events, cursor, self.db_connection
                )
...
def main():
    ...
    if args.product is None:
        ...
        config = {
            ...
            'project': config.get('GITLAB_PROJECT', '').split('/')[0] if config.get('GITLAB_PROJECT') else None,
            'repos': repos_list
        }
