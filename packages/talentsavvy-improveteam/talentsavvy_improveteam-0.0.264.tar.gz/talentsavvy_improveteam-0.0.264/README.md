...
To locate where the package files are installed, use:

```bash
# Method 1: Using pip show (shows Location field)
pip show talentsavvy-improveteam

# Method 2: Using Python directly
python -c "import site; print(site.getsitepackages()[0])"

# Method 3: Get the exact config.json path
python -c "import site; import os; print(os.path.join(site.getsitepackages()[0], 'config.json'))"
```

The `config.json` file is installed at:
```
<site-packages>/config.json
```
...
1. **Copy the config file to your working directory:**
   ```bash
   ...
   # Windows PowerShell:
   Copy-Item "$(python -c 'import site; import os; print(os.path.join(site.getsitepackages()[0], \"config.json\"))')" config.json
   
   # Linux/macOS:
   cp $(python -c 'import site; import os; print(os.path.join(site.getsitepackages()[0], "config.json"))') config.json
   ```

2. **Update `config.json`** in your working directory with your specific configuration values for each data source you plan to use.

3. Refer to each extraction script's documentation for the specific configuration keys it requires.
