...
def filter_molecule_species(system, species):
    """Callback to filter particles by species.
    ...
    """
    s = copy.copy(system)
    # TODO: we should pop the particles from the system...
    if species is not None:
        s.molecule = [p for p in system.molecule if p.species == species]
    return s
...
def setup_t_grid(trajectory, t_grid, offset=True):
    # TODO: document this crucial piece of crap
    ...
    i_grid = {int(round(t/trajectory.timestep)) for t in t_grid}
    offsets = [off_samp[t] for t in _templated(sorted(off_samp.keys()), sorted(i_grid))]
    return offsets

def _itemize(data):
    """Itemize numpy arrays in data dict"""
    new_data = {}
    for key in data:
...
# Species handling (adapted from atooms.system.System)

def _get_species_layout(species):
    """
    Return species layout (A=alphabetical, C=C indexed, F=fortran indexed)
    """
    try:
        species = numpy.array(species, dtype=int)
    except ValueError:
        # The species cannot be converted to int, thus layout is alphabetical
        layout = 'A'
    else:
...
def _set_species_layout(species, layout):
    """
    Set species layout
    """
    # Sanity checks
    ...
    current_layout = _get_species_layout(species)
