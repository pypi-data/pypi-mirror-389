...
def analyze_module(
    ...
    """
    This analysis is for OB loaded module cell metrology.
    """
    ...
    qc_config = get_qc_config(
        qc_criteria_path or data_path / "analysis_cuts_OBLoadedModuleCell.json",
        TEST_TYPE,
    )
    ...
    results = {key: df[key]["Values"] for key in df if key in keysList}

    # Average Glue Thickness
    results["AVERAGE_GLUE_THICKNESS"] = np.average(
        results["GLUE_THICKNESS_MODULE_CORNERS"]
    )

    # Loading Accuracy & Rotation
    if metadata.get("LOADING_METHOD") in ["1", "2"]:
        la_x = max(
            abs(results["COORDINATE_MODULE_CORNER_TOP_LEFT"][0] - (-21111)),
            abs(results["COORDINATE_MODULE_CORNER_BOTTOM_LEFT"][0] - (-21111)),
        )
    elif metadata.get("LOADING_METHOD") == "3":
        la_x = max(
            abs(results["COORDINATE_MODULE_CORNER_TOP_RIGHT"][0] - 21111),
            abs(results["COORDINATE_MODULE_CORNER_BOTTOM_RIGHT"][0] - 21111),
        )
    else:
        la_x = np.nan
    results["LOADING_ACCURACY"] = [
        la_x,
        max(
            abs(results["COORDINATE_MODULE_CORNER_TOP_RIGHT"][1] - 20562.5),
            abs(results["COORDINATE_MODULE_CORNER_TOP_LEFT"][1] - 20562.5),
        ),
    ]
    results["LOADING_ROTATION"] = np.atan(
        abs(
            results["COORDINATE_MODULE_CORNER_TOP_RIGHT"][1]
            - results["COORDINATE_MODULE_CORNER_TOP_LEFT"][1]
        )
        / abs(
            results["COORDINATE_MODULE_CORNER_TOP_RIGHT"][0]
            - results["COORDINATE_MODULE_CORNER_TOP_LEFT"][0]
        )
    )
