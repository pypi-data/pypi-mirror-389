...
class NodeRegistry:
    ...
    def get_user_nodes(self, user_id: str) -> List[Dict]:
        """Get all nodes registered for a user (based on authorized_users config)."""
        self.load_registry()
        
        # New format: registry[user_id] = [node1, node2, ...]
        user_nodes = self._nodes_cache.get(user_id, [])
        
        # Update status based on last_heartbeat
        for node in user_nodes:
            if "last_heartbeat" in node:
                try:
                    last_seen = datetime.fromisoformat(node["last_heartbeat"])
        ...
        user_nodes.sort(key=lambda x: x.get("last_heartbeat", ""), reverse=True)
