...
class NodeConnectionManager:
    ...
    def _get_or_create_node_id(self) -> str:
        """Get persistent node ID or create one if it doesn't exist"""
        # Check environment variable first
        node_id = os.environ.get("NODE_ID")
        if node_id:
            return node_id
        
        # Check for stored node ID
        try:
            app_dir = Path(__file__).parent.parent
            node_id_file = app_dir / "data" / "node_id.txt"
            
            if node_id_file.exists():
                with open(node_id_file, 'r') as f:
                    stored_id = f.read().strip()
                    if stored_id:
                        logger.info(f"📋 Using stored node ID: {stored_id}")
                        return stored_id
            
            # Create new persistent node ID
            new_id = f"node-{int(time.time())}"
            node_id_file.parent.mkdir(parents=True, exist_ok=True)
            with open(node_id_file, 'w') as f:
                f.write(new_id)
            logger.info(f"🆕 Created new persistent node ID: {new_id}")
            return new_id
            
        except Exception as e:
            logger.warning(f"⚠️  Could not persist node ID: {e}")
    ...
    def _store_registration_locally(self, registration_data: Dict):
        ...
        try:
            ...
            for user_id in self.config.authorized_users:
                ...
                # Find existing registration for this node
                existing_idx = None
                for idx, node in enumerate(registry[user_id]):
                ...
                if existing_idx is not None:
                    ...
                    registry[user_id][existing_idx] = registration_data
                    logger.info(f"🔄 Updated existing node registration for user {user_id}")
                else:
                    # Add new registration
                    registry[user_id].append(registration_data)
                    logger.info(f"🆕 Added new node registration for user {user_id}")
