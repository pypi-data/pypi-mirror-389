...
async def list_user_nodes(request: Request):
    ...
    try:
        ...
        # Use local registry - discovery service is deprecated (using tunnel-based proxy now)
        registry = get_registry()
        nodes = registry.get_user_nodes(user_id)
        
        logger.info(f"Found {len(nodes)} registered nodes for user {user_id}")
...
async def register_execution_node_manually(request: Request):
    ...
    This establishes a WebSocket tunnel to the cloud API and registers
    the node with the discovery service.
    
    Can be called from the UI when user clicks "Connect Tunnel" button.
    """
    try:
        # Get user from auth
        user = getattr(request.state, "user", None)
        ...
        logger.info(f"📡 Tunnel connection requested by user {user_id}")
        
        # Use the new tunnel manager
        import sys
        from pathlib import Path
        tunnel_manager_path = Path(__file__).parent.parent
        if str(tunnel_manager_path) not in sys.path:
            sys.path.insert(0, str(tunnel_manager_path))
        ...
        if success:
            logger.info(f"✅ Tunnel established successfully for user {user_id}!")
            return {
                "success": True,
                "message": "Tunnel connected successfully",
                "user_id": user_id,
                "note": "Your local node is now accessible via api.oneaurica.com"
            }
        else:
            logger.warning(f"⚠️  Tunnel connection failed for user {user_id}")
            return {
                "success": False,
                "message": "Failed to establish tunnel connection",
                "note": "Check that the cloud API (api.oneaurica.com) is accessible and has the /tunnel/connect endpoint available.",
                "suggestion": "Verify network connectivity and ensure the cloud API is running"
            }
