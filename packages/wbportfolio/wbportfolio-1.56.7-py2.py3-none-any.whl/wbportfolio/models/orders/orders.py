        ...
        **extra_param,
        ...
        with suppress(InstrumentPrice.DoesNotExist):
            ...
            if fx_rate := instrument_price.currency_fx_rate_to_usd:
                price_data = {
                    "volume_usd": instrument_price.volume * float(instrument_price.net_value) / float(fx_rate.value)
                }
                ...
                if shares is not None:
                    if instrument_price.market_capitalization:
                        price_data["market_share"] = (
                            float(shares) * float(price) / instrument_price.market_capitalization
                        )
                    if instrument_price.volume_50d:
                        price_data["daily_liquidity"] = float(shares) / instrument_price.volume_50d / 0.33
        return PositionDTO(
