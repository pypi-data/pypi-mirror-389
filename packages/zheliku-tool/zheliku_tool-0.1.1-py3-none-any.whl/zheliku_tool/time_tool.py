...
# 在 helpers 区域增加一个专门找调用点文件的函数
def _find_caller_src_path() -> Path:
    """
    在上下文管理器场景下，沿调用栈寻找“用户代码”的文件路径：
    - 排除本模块 (__file__)
    - 排除标准库/平台标准库
    - 排除 site-packages/dist-packages
    - 排除 contextlib.py
    找不到则退回到本模块路径。
    """
    ...
    # 收集标准库路径
    stdlib_dirs: set[Path] = set()
    try:
        paths = sysconfig.get_paths()
        for key in ("stdlib", "platstdlib"):
            p = paths.get(key)
            if p:
                stdlib_dirs.add(Path(p).resolve())
    except Exception:
        pass
...
class TimeLogger(ContextDecorator):
    ...
    def _resolve_log_path(self, func: Optional[Callable]) -> Path:
        ...
        if func is not None:
            ...
            src_path = _find_caller_src_path()
                ...
                try:
                    ...
                    logger.log(
                        ...
                        (
                            ...
                            f"abs='{src_path}', line={start_line}, "
