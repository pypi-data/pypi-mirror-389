    ...
    while True:
        ...
        if key == 'q':
            ...
            try:
                ...
                folder = choose_save_path(args.files)
            ...
            while True:
                ...
                if rot_cmd == 'c' or rot_cmd == 'a':
                    try:
                        ...
                        # Swap X and Y data for all curves - just swap, don't negate
                        for i in range(len(labels)):
                            xdata = x_data_list[i].copy()  # COPY to avoid mutation
                            ydata = y_data_list[i].copy()  # COPY to avoid mutation
                            
                            # Simply swap X and Y without negation
                            # Rotation direction is handled by inverting axis limits instead
                            new_x = ydata
                            new_y = xdata
                            ...
                            if i < len(orig_y):
                                # For orig_y, swap with the original x_data (before offset was applied)
                                offset_val = offsets_list[i] if i < len(offsets_list) else 0.0
                                # orig_y should be the normalized version of what was x_data
                                orig_y[i] = xdata - offset_val
                        
                        # Also rotate full data - just swap without negation
                        ...
                        # Swap axis limits and handle rotation direction by inverting one axis
                        xlim = ax.get_xlim()
                        ylim = ax.get_ylim()
                        
                        # For both clockwise and anti-clockwise, swap the limits
                        # Then invert one axis to achieve the rotation direction
                        if rot_cmd == 'c':
                            # Clockwise: swap limits and invert the new X-axis (which was Y)
                            ax.set_xlim(ylim[1], ylim[0])  # Inverted
                            ax.set_ylim(xlim[0], xlim[1])  # Normal
                        else:
                            # Anti-clockwise: swap limits and invert the new Y-axis (which was X)
                            ax.set_xlim(ylim[0], ylim[1])  # Normal
                            ax.set_ylim(xlim[1], xlim[0])  # Inverted
            ...
            try:
                ...
                # Choose base path (terminal cwd vs file directories)
                base_path = choose_save_path(args.files)
                ...
                if filename.isdigit() and files:
                    ...
                    if os.path.isabs(filename):
                        ...
                        export_target = get_organized_path(filename, 'figure', base_path)
