...
def operando_ec_interactive_menu(fig, ax, im, cbar, ec_ax):
    ...
    while True:
        ...
        if cmd == 'e':
            try:
                import os
                from .utils import list_files_in_subdirectory, get_organized_path, _confirm_overwrite as _co, choose_save_path
                
                # Choose base path (terminal cwd vs file directories)
                file_paths = []
                if hasattr(args, 'files'): file_paths.extend(args.files)
                if hasattr(args, 'ec_file') and args.ec_file: file_paths.append(args.ec_file)
