...
def dump_session(
    ...
    *,
    ...
    try:
        ...
        # Save rotation angle
        sess['rotation_angle'] = int(getattr(ax, '_rotation_angle', 0))
        if skip_confirm:
            target = filename
        else:
        ...
        for cyc, parts in cycle_lines.items():
            ...
            if isinstance(parts, dict):
                ...
                for role in ("charge", "discharge"):
                    ...
                    # Save smoothing state if present
                    orig_x = getattr(ln, '_original_xdata', None)
                    orig_y = getattr(ln, '_original_ydata', None)
                    if orig_x is not None and orig_y is not None:
                        # Line has been smoothed, save original data
                        entry[role] = {
                            'x': x, 
                            'y': y, 
                            'style': st,
                            'original_x': np.asarray(orig_x, float),
                            'original_y': np.asarray(orig_y, float),
                        }
                    else:
            ...
            for role in ("charge", "discharge"):
                ...
                if isinstance(rec, dict) and isinstance(rec.get('x'), np.ndarray) and isinstance(rec.get('y'), np.ndarray):
                    ...
                    try:
                        ...
                        # Restore smoothing state if present
                        if 'original_x' in rec and 'original_y' in rec:
                            ln_obj._original_xdata = np.asarray(rec['original_x'], float)
                            ln_obj._original_ydata = np.asarray(rec['original_y'], float)
                    except Exception:
                        pass
