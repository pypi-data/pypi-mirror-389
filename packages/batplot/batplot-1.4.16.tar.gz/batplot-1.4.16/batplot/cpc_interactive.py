...
def _apply_style(fig, ax, ax2, sc_charge, sc_discharge, sc_eff, cfg: Dict, file_data=None):
    ...
    try:
        ...
        if is_multi_file:
            # Restore per-file visibility from multi_files if available
            multi_files_cfg = cfg.get('multi_files', [])
            for i, f in enumerate(file_data):
                # Restore visibility for this file if saved
                if i < len(multi_files_cfg):
                    file_vis = multi_files_cfg[i].get('visible', True)
                    f['visible'] = file_vis
                    f['sc_charge'].set_visible(file_vis)
                    f['sc_discharge'].set_visible(file_vis)
                    f['sc_eff'].set_visible(file_vis)
    ...
    while True:
        ...
        if key == 'q':
            ...
            try:
                ...
                file_paths = [f['filepath'] for f in file_data] if is_multi_file else ([file_data['filepath']] if isinstance(file_data, dict) else [])
                base_path = choose_save_path(file_paths)
            ...
            try:
                ...
                # Choose base path (terminal cwd vs file directories)
                file_paths = [f['filepath'] for f in file_data] if is_multi_file else ([file_data['filepath']] if isinstance(file_data, dict) else [])
                folder = choose_save_path(file_paths)
