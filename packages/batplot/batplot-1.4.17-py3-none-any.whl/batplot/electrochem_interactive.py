...
def _print_menu(n_cycles: int, is_dqdv: bool = False):
    ...
    # Add smooth command for dQdV mode
    if is_dqdv:
...
def _rebuild_legend(ax):
    ...
    if handles:
        fig = ax.figure
        # Preserve the existing legend title (default to 'Cycle' for EC plots)
        old_legend = ax.get_legend()
        legend_title = 'Cycle'  # Default title for EC plots
        if old_legend is not None:
            old_title = old_legend.get_title()
            if old_title and old_title.get_text():
                legend_title = old_title.get_text()
        ...
        if xy_in is not None:
            try:
                ...
                new_legend = ax.legend(handles, labels, loc='center', bbox_to_anchor=(fx, fy), 
                                       bbox_transform=fig.transFigure, borderaxespad=1.0, title=legend_title)
            except Exception:
                new_legend = ax.legend(handles, labels, loc='best', borderaxespad=1.0, title=legend_title)
        else:
            new_legend = ax.legend(handles, labels, loc='best', borderaxespad=1.0, title=legend_title)
        
        # Set the title font size to match the original
        if new_legend:
            new_legend.get_title().set_fontsize('medium')
    else:
        leg = ax.get_legend()
        if leg is not None:
            try:
...
def electrochem_interactive_menu(fig, ax, cycle_lines: Dict[int, Dict[str, Optional[object]]]):
    ...
    while True:
        ...
        if key == 'q':
            ...
            try:
                from .utils import list_files_in_subdirectory, get_organized_path, choose_save_path
                
                # Choose base path (terminal cwd vs file directories)
                base_path = choose_save_path([args.file] if hasattr(args, 'file') else [])
                
                # List existing figure files in Figures/ subdirectory
                fig_extensions = ('.svg', '.png', '.jpg', '.jpeg', '.pdf', '.eps', '.tif', '.tiff')
                file_list = list_files_in_subdirectory(fig_extensions, 'figure', base_path)
                ...
                if fname.isdigit() and files:
                    ...
                    if os.path.isabs(fname):
                        ...
                        target = get_organized_path(fname, 'figure', base_path)
            ...
            try:
                ...
                # Choose base path (terminal cwd vs file directories)
                folder = choose_save_path([args.file] if hasattr(args, 'file') else [])
            ...
            # Smooth dQ/dV curves by filtering out data points with small voltage steps
            # Based on Neware BTSDA approach: removes points where ΔV between consecutive points is too small
            # Only available in dQdV mode
            if not is_dqdv:
            ...
            while True:
                ...
                if sub == 'r':
                    ...
                    try:
                        for cyc, parts in cycle_lines.items():
                            for role in ("charge", "discharge"):
                                line = parts.get(role)
                                if line and hasattr(line, '_original_xdata'):
                                    line.set_xdata(line._original_xdata)
                                    line.set_ydata(line._original_ydata)
                                    restored_count += 1
                        if restored_count > 0:
                            print(f"Reset {restored_count} curves to original data.")
                            try:
                                fig.canvas.draw()
                            except Exception:
                                fig.canvas.draw_idle()
                ...
                if sub == 'a':
                    # Get minimum voltage step threshold from user
                    try:
                        threshold_input = input("Enter minimum voltage step in mV (default 0.5 mV, larger = more filtering): ").strip()
                        if not threshold_input:
                            threshold_mv = 0.5
                        else:
                            threshold_mv = float(threshold_input)
                        
                        # Convert mV to V for comparison
                        ...
                        # Apply filtering to all visible curves
                        ...
                        try:
                            for cyc, parts in cycle_lines.items():
                                for role in ("charge", "discharge"):
                                    ...
                                    if line and line.get_visible():
                                        # Store original data on first use
                                        if not hasattr(line, '_original_xdata'):
                                            line._original_xdata = np.array(line.get_xdata()).copy()
                                            line._original_ydata = np.array(line.get_ydata()).copy()
                                        
                                        # ALWAYS filter from the original raw data, not the already-filtered data
                                        xdata = line._original_xdata
                                        ydata = line._original_ydata
                                        
                                        # Filter points where |ΔV| is too small
                                        # Keep first point, then only keep points where voltage change exceeds threshold
                                        if len(xdata) < 2:
                                            continue
                                        
                                        mask = [True]  # Always keep first point
                                        for i in range(1, len(xdata)):
                                            dv = abs(xdata[i] - xdata[i-1])
                                            if dv >= threshold_v:
                                                mask.append(True)
                                            else:
                                                mask.append(False)
                                        ...
                                        points_before = len(xdata)
                                        points_after = len(filtered_x)
                                        
                                        if points_after < points_before:
                                            line.set_xdata(filtered_x)
                            ...
                            if filtered_count > 0:
                                removed = total_points_before - total_points_after
                                pct = 100 * removed / total_points_before if total_points_before > 0 else 0
                                print(f"Filtered {filtered_count} curves with ΔV ≥ {threshold_mv} mV.")
                                print(f"Removed {removed} of {total_points_before} points ({pct:.1f}%).")
                                print("Tip: Try different threshold values to adjust smoothness (always applies to raw data).")
                                try:
                                    fig.canvas.draw()
                                except Exception:
                                    fig.canvas.draw_idle()
                            else:
                                print("No curves were filtered. All voltage steps already exceed threshold.")
                ...
                if sub == 'o':
                    # Remove outliers: points where dQ/dV value changes abruptly
                    try:
                        method_input = input("Method: (1) Z-score threshold, (2) Median absolute deviation? [1]: ").strip()
                        method = method_input if method_input in ('1', '2') else '1'
                        
                        if method == '1':
                            # Z-score method: remove points where |z-score| > threshold
                            threshold_input = input("Enter Z-score threshold (default 3.0, larger = fewer removed): ").strip()
                            if not threshold_input:
                                z_threshold = 3.0
                            else:
                                z_threshold = float(threshold_input)
                            
                            if z_threshold <= 0:
                                print("Error: threshold must be positive.")
                                continue
                        else:
                            # MAD method: remove points where deviation > threshold * MAD
                            threshold_input = input("Enter MAD multiplier (default 3.0, larger = fewer removed): ").strip()
                            if not threshold_input:
                                mad_threshold = 3.0
                            else:
                                mad_threshold = float(threshold_input)
                            
                            if mad_threshold <= 0:
                                print("Error: threshold must be positive.")
                                continue
                        
                        # Apply outlier removal to all visible curves
                        ...
                        try:
                            for cyc, parts in cycle_lines.items():
                                for role in ("charge", "discharge"):
                                    ...
                                    if line and line.get_visible():
                                        ...
                                        # ALWAYS filter from the original raw data
                                        xdata = line._original_xdata
                                        ydata = line._original_ydata
                                        ...
                                        # Detect outliers based on dQ/dV values
                                        if method == '1':
                                            # Z-score method
                                            ...
                                            z_scores = np.abs((ydata - mean_y) / std_y)
                                            mask = z_scores <= z_threshold
                                        else:
                                            # MAD (Median Absolute Deviation) method - more robust to outliers
                                            median_y = np.median(ydata)
                                            mad = np.median(np.abs(ydata - median_y))
                                            if mad == 0:
                                                # Fallback to standard deviation if MAD is zero
                                                std_y = np.std(ydata)
                                                if std_y == 0:
                                                    continue
                                                deviations = np.abs(ydata - median_y) / std_y
                            ...
                            if filtered_count > 0:
                                ...
                                threshold_val = z_threshold if method == '1' else mad_threshold
                                print(f"Removed outliers from {filtered_count} curves using {method_name} (threshold={threshold_val}).")
                                print(f"Removed {removed} of {total_points_before} points ({pct:.1f}%).")
                                print("Tip: Adjust threshold to control outlier sensitivity (always applies to raw data).")
                                try:
                                    fig.canvas.draw()
                                except Exception:
                                    fig.canvas.draw_idle()
                ...
                print("Unknown command. Use: a=apply voltage filter, o=remove outliers, r=reset, or q=back")
