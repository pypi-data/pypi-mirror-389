...
def export_style_config(
    ...
    try:
        ...
        import os
        from .utils import list_files_in_subdirectory, get_organized_path, choose_save_path, ensure_subdirectory
        
        # Choose base path (terminal cwd vs file directories)
        base_path = choose_save_path(args.files if hasattr(args, 'files') else [])
        
        file_list = list_files_in_subdirectory((default_ext, '.bpcfg'), 'style', base_path)
        ...
        if choice.isdigit() and style_files and 1 <= int(choice) <= len(style_files):
            ...
            if os.path.isabs(filename_with_ext):
                ...
                target_path = get_organized_path(filename_with_ext, 'style', base_path)
