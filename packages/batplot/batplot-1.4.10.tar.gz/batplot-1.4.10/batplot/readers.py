...
def read_ec_csv_file(fname: str, prefer_specific: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    Required columns (at least one voltage/current variant):
        ...
        - Cycle Index: 'Cycle Index' or '循环序号' (if missing, inferred from segments)
    ...
    2. Merge logic:
        - If 'Cycle Index' column exists: use it directly (cycler already paired segments)
        - If missing: pair consecutive segments as complete cycles
            * Segment 1 (charge) + Segment 2 (discharge) = Cycle 1
            * Segment 3 (charge) + Segment 4 (discharge) = Cycle 2
            * etc.
        - Handles odd number of segments (incomplete final cycle)
    ...
    Returns:
        Tuple of 5 numpy arrays for galvanostatic cycling plots:
            ...
            cycle_numbers (np.ndarray): Cycle index for each data point
                ...
                - If 'Cycle Index' column present: use directly
                - If missing: inferred by pairing alternating charge/discharge segments
                  (Segment 1+2 = Cycle 1, Segment 3+4 = Cycle 2, etc.)
    ...
    if cyc_idx is None:
        ...
        # Cycle Index column exists, but some cyclers (Neware) use inconsistent numbering:
        # - Style 1: Charge=1, Discharge=2, Charge=3, Discharge=4, ... (simple half-cycle numbering)
        # - Style 2: Charge=1, Discharge+Charge=2, Discharge+Charge=3, ... (overlapping segments)
        # 
        # The old merging logic fails for Style 2 because a single Cycle Index contains both
        # discharge and charge, which is mistaken for a "complete cycle".
        # 
        # New strategy: Ignore the unreliable Cycle Index and infer cycles from contiguous
        # charge/discharge blocks. This is more robust and works for both styles.
        
        # Detect contiguous charge and discharge segments
        def _contiguous_blocks(mask):
            """Find contiguous True blocks in boolean mask. Returns list of (start, end) index pairs."""
            blocks = []
            in_block = False
            start = 0
        ...
        # Determine the starting half-cycle type (charge or discharge)
        # by checking which comes first in the data
        starts_with_charge = True
        if len(ch_blocks) > 0 and len(dch_blocks) > 0:
            first_ch_idx = ch_blocks[0][0]
            first_dch_idx = dch_blocks[0][0]
            starts_with_charge = (first_ch_idx < first_dch_idx)
        elif len(dch_blocks) > 0:
            starts_with_charge = False
        
        # Pair charge and discharge blocks into cycles based on starting type
        inferred_cycles = np.zeros_like(cycles)
        
        if starts_with_charge:
            # Starts with charge: Charge → Discharge → Charge → Discharge → ...
            # Cycle 1 = Charge[0] + Discharge[0]
            # Cycle 2 = Charge[1] + Discharge[1]
            # etc.
            N = max(len(ch_blocks), len(dch_blocks))
            for i in range(N):
                cyc = i + 1
                # Assign charge block i to cycle i+1
                if i < len(ch_blocks):
                    a, b = ch_blocks[i]
                    inferred_cycles[a:b+1] = cyc
                # Assign discharge block i to cycle i+1
                if i < len(dch_blocks):
                    a, b = dch_blocks[i]
                    inferred_cycles[a:b+1] = cyc
        else:
            # Starts with discharge: Discharge → Charge → Discharge → Charge → ...
            # Cycle 1 = Discharge[0] + Charge[0]
            # Cycle 2 = Discharge[1] + Charge[1]
            # etc.
            N = max(len(ch_blocks), len(dch_blocks))
            for i in range(N):
                cyc = i + 1
...
def read_ec_csv_dqdv_file(fname: str, prefer_specific: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, str]:
    ...
    Cycle Inference:
    ----------------
    If file lacks explicit 'Cycle Index' column, cycles are inferred by pairing alternating
    charge/discharge segments:
        - Segment 1 (charge) + Segment 2 (discharge) = Cycle 1
        - Segment 3 (charge) + Segment 4 (discharge) = Cycle 2
        - etc.
    
    This assumes standard galvanostatic cycling protocol (charge → discharge → repeat).
    ...
    Returns:
        Tuple of 6 elements for differential capacity plotting:
            ...
            cycles (np.ndarray): Cycle number for each data point
                ...
                - Inferred from segment pairing if 'Cycle Index' column absent
