    ...
    for i, v in enumerate(value):
        if not (0 <= v <= 254):
            raise exception_class(
                f"The PID value for {class_name} needs to be 0 ~ 254, but received the {pid_str[i]} is {v}"
            )
...
def check_value_type(parameter, value_type, exception_class, _type):
    if value_type is not _type:
        raise exception_class(
            f"The acceptable parameter {parameter} should be an {_type}, but the received {value_type}"
        )
...
def check_coords(
    ...
    if not isinstance(value, list):
        raise exception_class(
            f"`{parameter_name}` must be a list, but the received {type(value)}"
        )
    if len(value) != 6:
        raise exception_class(
            f"The length of `{parameter_name}` must be 6, but the received length is {len(value)}"
        )
    if serial_port:
...
def check_angles(angle_value, robot_limit, class_name, exception_class):
    ...
    if not isinstance(angle_value, list):
        raise exception_class(
            f"`angles` must be a list, but the received {type(angle_value)}"
        )
    # Check angles
    if len(angle_value) != 6:
        raise exception_class(
            f"The length of `angles` must be 6, but received length is {len(angle_value)}"
        )
    for idx, angle in enumerate(angle_value):
        if (
            not robot_limit[class_name]["angles_min"][idx]
            <= angle
            <= robot_limit[class_name]["angles_max"][idx]
        ):
            raise exception_class(
                "Has invalid angle value, error on index {0}. Received {3} but angle should be {1} ~ {2}.".format(
                    idx,
                    robot_limit[class_name]["angles_min"][idx],
                    robot_limit[class_name]["angles_max"][idx],
                    angle,
                )
            )
...
def check_0_or_1(
    parameter, value, range_data, value_type, exception_class, _type
):
    check_value_type(parameter, value_type, exception_class, _type)
    if value not in range_data:
        error = f"The data supported by parameter {parameter} is "
        len_data = len(range_data)
        for idx in range(len_data):
            error += str(range_data[idx])
            if idx != len_data - 1:
                error += " or "
        error += f", but the received value is {value}"
        raise exception_class(error)


def check_id(value, id_list, exception_class):
    raise exception_class(
        f"The joint_id not right, should be in {id_list}, but received {value}."
    )


def public_check(
    parameter_list, kwargs, robot_limit, class_name, exception_class
):
    for parameter in parameter_list[1:]:
        ...
        if (
            parameter == "id"
            and value not in robot_limit[class_name][parameter]
        ):
            check_id(
                value, robot_limit[class_name][parameter], exception_class
            )
        elif parameter == "rgb":
            ...
            if value < 0 or value > 4096:
                raise exception_class(
                    f"The range of {parameter} is 0 ~ 4096, but the received value is {value}"
                )
        elif parameter == "pin_mode":
            check_value_type(parameter, value_type, exception_class, int)
            if value not in [0, 1, 2]:
                raise exception_class(
                    f"The data supported by parameter {parameter} is 0 or 1 or 2, but the received value is {value}"
                )
        elif parameter == "pin_signal":
            check_0_or_1(
                parameter, value, [0, 1], value_type, exception_class, int
            )
            ...
            check_0_or_1(
                parameter, value, [0, 1, 254], value_type, exception_class, int
            )
        elif parameter == "gripper_type":
            check_0_or_1(
                parameter, value, [1, 3, 4], value_type, exception_class, int
            )
            ...
            if value < 0 or value > 100:
                raise exception_class(
                    f"The range of {parameter} is 0 ~ 100, but the received value is {value}"
                )
        ...
        #     check_coords(value, robot_limit, class_name, exception_class)
            ...
            if not 1 <= value <= 100:
                raise exception_class(
                    f"{parameter} value not right, should be 1 ~ 100, the received is {value}"
                )
            ...
            if (
                value < robot_limit[class_name]["angles_min"][index]
                or value > robot_limit[class_name]["angles_max"][index]
            ):
                raise exception_class(
            ...
            for data in value:
                ...
                if data < 0 or data > 4096:
                    raise exception_class(
                        f"The range of encoder is 0 ~ 4096, but the received value is {data}"
                    )
        elif parameter == "speeds":
            if "MyCobot" in class_name or "MechArm" in class_name:
                if len(value) not in [6, 7]:
                    raise exception_class(
                        f"The length of `speeds` must be 6. but the received value is {value}"
                    )
            elif class_name in ["MyPalletizer", "MyPalletizer260"]:
                if len(value) != 4:
            ...
            for data in value:
                ...
                if data < 0 or data > 6000:
                    raise exception_class(
                        f"The range of speed is 0 ~ 6000, but the received value is {data}"
                    )
            ...
            if "MyCobot" in class_name or "MechArm" in class_name:
                if value < 1 or value > 7:
                    raise exception_class(
                        f"The range of id is 1 ~ 6 or 7, but the received is {value}"
                    )
            if class_name in ["MyPalletizer", "MyPalletizer260"]:
                if value < 1 or value > 4:
                    raise exception_class(
                        f"The range of id is 1 ~ 4, but the received is {value}"
                    )
            elif (
                "MyArm" in class_name
                or "MyCobot" in class_name
                or "MechArm" in class_name
            ):
                if value < 1 or value > 7:
                    raise exception_class(
                        f"The range of joint_id is 1 ~ 7, but the received is {value}"
                    )
            ...
            if value < torque_min or value > torque_max:
                raise exception_class(
                    f"The range of torque is {torque_min} ~ {torque_max}, but the received is {value}"
                )
            ...
            if value < current_min or value > current_max:
                raise exception_class(
                    f"The range of current is {current_min} ~ {current_max}, but the received is {value}"
                )
        elif parameter == "end_direction":
            check_0_or_1(
                parameter, value, [1, 2, 3], value_type, exception_class, int
            )
            ...
            if class_name in ["Mercury"]:
                ...
                check_0_or_1(
                    parameter, value, [1, 2], value_type, exception_class, int
                )
            elif class_name in [
                "MyCobot280",
                "MyCobot280Socket",
    ...
    if class_name in ["Mercury", "MercurySocket"]:
        for parameter in parameter_list[1:]:
            ...
            if parameter == "joint_id":
                ...
                if joint_id in [11, 12, 13]:
                    index = (
                        robot_limit[class_name]["joint_id"][joint_id - 4] - 4
                    )
                else:
                    index = (
                        robot_limit[class_name]["joint_id"][joint_id - 1] - 1
                    )
                if (
                    value < robot_limit[class_name]["angles_min"][index]
                    or value > robot_limit[class_name]["angles_max"][index]
                ...
                if len(value) not in [7, 10]:
                    raise MercuryDataException(
                        "The length of `angles` must be 7 or 10."
                    )
                for idx, angle in enumerate(value):
                    ...
                    if angle < angle_min or angle > angle_max:
                        raise MercuryDataException(
                            f"Joint {joint_id} angle value of {angle} exceeds the limit, with a limit range of {angle_min} ~ {angle_max}."
                        )
                ...
                if (
                    ...
                    or value > robot_limit[class_name]["coords_max"][index]
                ):
                    raise MercuryDataException(
                ...
                if isinstance(coord_id, int):
                    ...
                    if serial_port == "/dev/left_arm":
                        ...
                        min_coord = robot_limit[class_name][
                            "right_coords_min"
                        ][index]
                        max_coord = robot_limit[class_name][
                            "right_coords_max"
                        ][index]
                    else:
                        min_coord = robot_limit[class_name]["coords_min"][
                    ...
                    if value < min_coord or value > max_coord:
                        raise MercuryDataException(
                            f"The `base_coord` value of {value} exceeds the limit, and the limit coord is {min_coord} ~ {max_coord}"
                        )
            ...
            # if direction is not None:
                ...
                if value not in [0, 1]:
                    raise MercuryDataException(
                        f"{parameter} only supports 0 or 1, but received {value}"
                    )

            elif parameter == "coord_id":
                if value < 1 or value > 6:
                    raise MercuryDataException(
                        f"coord_id only supports 1 ~ 6, but received {value}"
                    )

            elif parameter == "solution_angle":
                if value > 90 or value < -90:
                    raise MercuryDataException(
                        f"The angle range is -90° ~ 90°, but received {value}"
                    )
            elif parameter == "address":
                if value < 32 or value > 34:
                    raise MercuryDataException(
                        f"The angle address is 32 ~ 34, but received {value}"
                    )
            elif parameter == "value":
                if value < 1 or value > 32000:
                    raise MercuryDataException(
                        f"The angle value is 1 ~ 32000, but received {value}"
                    )
            elif parameter == "servo_restore":
                if value not in [1, 2, 3, 4, 5, 6, 7, 13, 254]:
                    raise MercuryDataException(
                        f"The joint_id should be in [1,2,3,4,5,6,7,13,254], but received {value}"
                    )
            elif parameter == "data_len":
                if value < 1 or value > 45:
                    raise MercuryDataException(
                        f"The parameter data_len data range only supports 1 ~ 45, but received {value}"
                    )
            elif parameter == "max_time":
                if value < 0:
                    raise MercuryDataException(
                        f"The parameter max_time must be greater than or equal to 0, but received {value}"
                    )
                ...
                if value not in [1, 2, 3, 4]:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 1, 2, 3, 4, but received {value}"
                    )
            elif parameter == "axis":
                if value not in [1, 2, 3]:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 1, 2, 3, but received {value}"
                    )
            elif parameter == "threshold_value":
                if value < 50 or value > 250:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 50 ~ 250, but received {value}"
                    )
            elif parameter == "comp_value":
                if value < 0 or value > 250:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 0 ~ 250, but received {value}"
                    )
            elif parameter == "shoot_value":
                if value < -300 or value > 300:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports -300 ~ 300, but received {value}"
                    )
            elif parameter == "head_id":
                if value not in [11, 12]:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 11, 12, but received {value}"
                    )
                ...
                if value < 0 or value > 655.5:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 0 ~ 655.5, but received {value}"
                    )
            elif parameter == "rank":
                if value not in [0, 1, 2]:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 0 or 1 or 2, but received {value}"
                    )
            elif parameter == "move_type":
                if value not in [0, 1, 2, 3, 4]:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 0 or 4, but received {value}"
                    )
            elif parameter == "trajectory":
                if value not in [0, 1, 2, 3, 4]:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports [0,1,2,3,4], but received {value}"
                    )
            elif parameter in ["gripper_id", "new_hand_id"]:
                if value < 1 or value > 254:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 1 ~ 254, but received {value}"
                    )
            elif parameter == "gripper_address":
                if value < 1 or value > 44 or value in [15, 17, 19]:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 1 ~ 44 (except 15, 17, and 19), but received {value}"
                    )
            elif parameter == "gripper_angle":
                if value < 0 or value > 100:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 0 ~ 100, but received {value}"
                    )
            elif parameter == "torque":
                if value < 1 or value > 100:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 1 ~ 100, but received {value}"
                    )
            elif parameter == "hand_id":
                if value < 1 or value > 6:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 1 ~ 6, but received {value}"
                    )
                ...
                if value != 1:
                    raise MercuryDataException(
                        f"The parameter {parameter} only supports 1, but received {value}"
                    )
            else:
                public_check(
                    parameter_list,
            ...
            if parameter == "rgb":
                ...
                if value not in [1, 2]:
                    raise MyAgvDataException(
                        f"led mode only supports 1 or 2, but received is {value}"
                    )
            elif parameter == "direction_1":
                if value < 0 or value > 255:
                    raise MyAgvDataException(
                        f"The range of direction_1 is 0 ~ 255, but the received value is {value}"
                    )
            elif parameter == "direction_2":
                if value < 0 or value > 255:
                    raise MyAgvDataException(
                        f"The range of direction_2 is 0 ~ 255, but the received value is {value}"
                    )
            elif parameter == "direction_3":
                if value < 0 or value > 255:
                    raise MyAgvDataException(
                        f"The range of direction_3 is 0 ~ 255, but the received value is {value}"
                    )
            elif parameter == "data":
                if value < 1 or value > 127:
                    raise MyAgvDataException(
                        f"The range of {parameter} is 1 ~ 127, but the received value is {value}"
                    )
        ...
        for parameter in parameter_list[1:]:
            ...
            if (
                ...
                check_id(
                    value,
                    robot_limit[class_name][parameter],
                    MyCobot280DataException,
                )
            elif parameter in ["coord_id"]:
                check_value_type(
                    parameter, value_type, MyCobot280DataException, int
                )
                if value not in robot_limit[class_name][parameter]:
                    raise MyCobot280DataException(
                        f"The coord_id not right, should be in {[1, 2, 3, 4, 5, 6]}, but received {value}."
                    )
            elif parameter == "servo_data_id" and value not in [
                1,
                2,
                3,
                4,
                ...
                raise MyCobot280DataException(
                    f"The id not right, should be in {[1, 2, 3, 4, 5, 6, 7]}, but received {value}."
                )
            elif parameter == "address_id" and value in range(0, 5):
                raise MyCobot280DataException(
                    f"The address_id is invalid: should not be in [0, 1, 2, 3, 4], but received {value}."
                )
            elif parameter == "rgb":
                ...
                check_value_type(
                    parameter, value_type, MyCobot280DataException, str
                )
            elif parameter == "coords":
                check_coords(
                    parameter,
                ...
                check_angles(
                    value, robot_limit, class_name, MyCobot280DataException
                )
            elif parameter == "angle":
                ...
                id = kwargs.get("id", None)
                index = (
                    robot_limit[class_name]["id"][id - 1] - 1
                ...
                if not 0 <= value <= 4096:
                    raise MyCobot280DataException(
                        f"The range of encoder is 0 ~ 4096, but the received value is {value}"
                    )
            elif parameter == "encoders":
                if len(value) != 6:
                    raise MyCobot280DataException(
                ...
                for data in value:
                    data_type = type(data)
                    check_value_type(
                        parameter, data_type, MyCobot280DataException, int
                    )
                    if data < 0 or data > 4096:
                        raise MyCobot280DataException(
                            f"The range of encoder is 0 ~ 4096, but the received value is {data}"
        ...
        for parameter in parameter_list[1:]:
            ...
            if (
                ...
                check_id(
                    value,
                    robot_limit[class_name][parameter],
                    MyCobot320DataException,
                )
            elif (
                parameter in ("servo_id", "joint_id", "coord_id")
                and value not in limit_info[parameter]
            ):
                raise ValueError(
                ...
                check_value_type(
                    parameter, value_type, MyCobot320DataException, int
                )
                if value < 0 or value > 4096:
                    raise MyCobot320DataException(
                        f"The range of {parameter} is 0 ~ 4096, but the received value is {value}"
                ...
                check_value_type(
                    parameter, value_type, MyCobot320DataException, str
                )
            elif parameter == "coords":
                check_coords(
                    parameter,
                ...
                check_angles(
                    value, robot_limit, class_name, MyCobot320DataException
                )
            elif parameter == "angle":
                id = kwargs.get("id", None)
                index = robot_limit[class_name]["id"][id - 1] - 1
                ...
                for data in value:
                    data_type = type(data)
                    check_value_type(
                        parameter, data_type, MyCobot320DataException, int
                    )
                    if data < 0 or data > 4096:
                        raise MyCobot320DataException(
                            f"The range of encoder is 0 ~ 4096, but the received value is {data}"
                ...
                if value < 1 or value > 6:
                    raise MyCobot320DataException(
                        f"The range of id is 1 ~ 6, but the received is {value}"
                    )
            elif parameter == "torque":
                torque_min = 150
                torque_max = 980
                ...
                if not isinstance(gripper_id, int) or not isinstance(
                    gripper_angle, int
                ):
                    raise MyCobot320DataException(
                        f"Both 'gripper_id' and 'gripper_angle' in {parameter} must be integers"
                    )
                if gripper_id < 1 or gripper_id > 254:
                    raise MyCobot320DataException(
                        f"The range of 'gripper_id' in {parameter} is 1 ~ 254, but the received value is {gripper_id}"
                    )
                if gripper_angle < 0 or gripper_angle > 100:
                    raise MyCobot320DataException(
                        f"The range of 'gripper_angle' in {parameter} is 0 ~ 100, but the received value is {gripper_angle}"
                    )
                ...
                if value < 1 or value > 254:
                    raise MyCobot320DataException(
                        f"The range of 'gripper_id' in {parameter} is 1 ~ 254, but the received value is {value}"
                    )
                ...
                if not isinstance(gripper_id, int) or not isinstance(
                    ...
                    raise MyCobot320DataException(
                        f"Both 'gripper_id' and 'torque_value' in {parameter} must be integers"
                    )
                if gripper_id < 1 or gripper_id > 254:
                    raise MyCobot320DataException(
                        f"The range of 'gripper_id' in {parameter} is 1 ~ 254, but the received value is {gripper_id}"
                ...
                if torque_value < 0 or torque_value > 100:
                    raise MyCobot320DataException(
                        f"The range of 'torque_value' in {parameter} is 0 ~ 100, but the received value is {torque_value}"
                    )
                ...
                if not isinstance(gripper_id, int) or not isinstance(
                    ...
                    raise MyCobot320DataException(
                        f"Both 'gripper_id' and 'speed' in {parameter} must be integers"
                    )
                if gripper_id < 1 or gripper_id > 254:
                    raise MyCobot320DataException(
                        f"The range of 'gripper_id' in {parameter} is 1 ~ 254, but the received value is {gripper_id}"
                ...
                if speed < 1 or speed > 100:
                    raise MyCobot320DataException(
                        f"The range of 'speed' in {parameter} is 1 ~ 100, but the received value is {speed}"
                    )

            elif parameter == "set_gripper_args":
                if len(value) != 3:
                    raise ValueError(
                        f"Expected 3 arguments, but got {len(value)}"
                    )
                gripper_id, address, data = value
                if (
                    not isinstance(gripper_id, int)
                    or not isinstance(address, int)
                    or not isinstance(data, int)
                ):
                    raise MyCobot320DataException(
                        f"All arguments in {parameter} must be integers"
                    )
                if gripper_id < 1 or gripper_id > 254:
                    raise MyCobot320DataException(
                        f"The range of 'gripper_id' in {parameter} is 1 ~ 254, but the received value is {gripper_id}"
                ...
                if address < 1 or address > 44:
                    raise MyCobot320DataException(
                        f"The range of 'address' in {parameter} is 1 ~ 44, but the received value is {address}"
                    )
                if address in invalid_addresses:
                    raise MyCobot320DataException(
                        f"'address' in {parameter} cannot be one of the following values: {invalid_addresses}, but the received value is {address}"
                    )
                # Process value according to address
                if address in [3]:
                    if data < 1 or data > 254:
                        raise MyCobot320DataException(
                            f"Error in parameter '{parameter}': The range of 'value' for address={address} is 1 ~ 254, but the received value is {data}"
                        )
                elif address in [43]:
                    if data < 100 or data > 300:
                        raise MyCobot320DataException(
                            f"Error in parameter '{parameter}': The range of 'value' for address={address} is 100 ~ 300, but the received value is {data}"
                        )
                elif address == 10:
                    if data not in [0, 1]:
                        raise MyCobot320DataException(
                            f"Error in parameter '{parameter}': Value for address={address} must be 0 or 1, but the received value is {data}"
                        )
                elif address in [25]:
                    if data < 0 or data > 254:
                        raise MyCobot320DataException(
                            f"Error in parameter '{parameter}': The range of 'value' for address={address} is 0 ~ 254, but the received value is {data}"
                        )
                elif address in [21, 23]:
                    if data < 0 or data > 16:
                        raise MyCobot320DataException(
                            f"Error in parameter '{parameter}': The range of 'value' for address={address} is 0 ~ 16, but the received value is {data}"
                        )
                elif address in [30, 31, 41]:
                    if data < 0 or data > 100:
                        raise MyCobot320DataException(
                            f"Error in parameter '{parameter}': The range of 'value' for address={address} is 0 ~ 100, but the received value is {data}"
                        )
                elif address == 29:
                    if data not in [0, 1, 16, 17]:
                        raise MyCobot320DataException(
                            f"Error in parameter '{parameter}': The range of 'value' for address={address} is 0 or 1 or 16 or 17, but the received value is {data}"
                        )
                ...
                if value < 1 or value > 6:
                    raise MyCobot320DataException(
                        f"The range of 'gripper_id' in {parameter} is 1 ~ 6, but the received value is {value}"
                    )
            elif parameter == "gripper_angles":
                if len(value) != 6:
                    raise MyCobot320DataException(
                        f"{class_name}: 'gripper_angles' must contain exactly 6 values, but received {len(value)}."
                    )

                for i, angle in enumerate(value):
                    if not isinstance(angle, int):
                        raise MyCobot320DataException(
                            f"{class_name}: Value at position {i + 1} in 'gripper_angles' must be an integer, but received {angle}."
                        )
                    if angle < 0 or angle > 100:
                        raise MyCobot320DataException(
                            f"{class_name}: Value {angle} at position {i + 1} in 'gripper_angles' is out of range (0 ~ 100)."
                        )
            elif parameter == "pinch_mode":
                check_0_or_1(
                    parameter,
                ...
                if value < 0 or value > 16:
                    raise MyCobot320DataException(
                        f"The range of 'value' in {parameter} is 0 ~ 16, but the received value is {value}"
                    )
                ...
                if value < 0 or value > 254:
                    raise MyCobot320DataException(
                        f"The range of 'value' in {parameter} is 0 ~ 254, but the received value is {value}"
                    )
        ...
        for parameter in parameter_list[1:]:
            ...
            if (
                ...
                check_value_type(
                    parameter, value_type, MechArmDataException, int
                )
                if value < 0 or value > 4096:
                    raise MechArmDataException(
                        f"The range of {parameter} is 0 ~ 4096, but the received value is {value}"
                ...
                check_value_type(
                    parameter, value_type, MechArmDataException, str
                )
            elif parameter in [
                "rftype",
                "move_type",
            ...
            if parameter == "angles":
                check_angles(
                    value, robot_limit, class_name, MechArmDataException
                )
            elif parameter == "angle":
                id = kwargs.get("id", None)
                index = robot_limit[class_name]["id"][id - 1] - 1
                ...
                for data in value:
                    data_type = type(data)
                    check_value_type(
                        parameter, data_type, MechArmDataException, int
                    )
                    if data < 0 or data > 4096:
                        raise MechArmDataException(
                            f"The range of encoder is 0 ~ 4096, but the received value is {data}"
        ...
        for parameter in parameter_list[1:]:
            ...
            if (
                ...
                check_value_type(
                    parameter, value_type, MyArmDataException, int
                )
                if value < 0 or value > 4096:
                    raise MyArmDataException(
                        f"The range of {parameter} is 0 ~ 4096, but the received value is {value}"
                ...
                check_value_type(
                    parameter, value_type, MyArmDataException, str
                )
            elif parameter == "coords":
                check_coords(
                    parameter,
                ...
                if len(value) != 7:
                    raise MyArmDataException(
                        "The length of `angles` must be 7."
                    )
                for idx, angle in enumerate(value):
                    if (
                        not robot_limit[class_name]["angles_min"][idx]
                ...
                for data in value:
                    data_type = type(data)
                    check_value_type(
                        parameter, data_type, MyArmDataException, int
                    )
                    if data < 0 or data > 4096:
                        raise MyArmDataException(
                            f"The range of encoder is 0 ~ 4096, but the received value is {data}"
                ...
                if len(value) != 7:
                    raise MyArmDataException(
                        "The length of `speeds` must be 7."
                    )
                for data in value:
                    data_type = type(data)
                    check_value_type(
                ...
                if value < 1 or value > 8:
                    raise MyArmDataException(
                        f"The range of id is 1 ~ 7 or 8, but the received is {value}"
                    )
            elif parameter == "torque":
                torque_min = 150
                torque_max = 980
        ...
        for parameter in parameter_list[1:]:
            ...
            if (
                ...
                check_rgb_value(
                    value, MyPalletizer260DataException, class_name
                )
            elif parameter == "value":
                check_value_type(
                    parameter, value_type, MyPalletizer260DataException, int
                )
                if value < 0 or value > 4096:
                    raise MyPalletizer260DataException(
                        f"The range of {parameter} is 0 ~ 4096, but the received value is {value}"
                ...
                check_value_type(
                    parameter, value_type, MyPalletizer260DataException, str
                )
            elif parameter in [
                "rftype",
                "move_type",
                ...
                if len(value) != 4:
                    raise MyPalletizer260DataException(
                        "The length of `angles` must be 4."
                    )
                for idx, angle in enumerate(value):
                    if (
                        not robot_limit[class_name]["angles_min"][idx]
                ...
                for idx, coord in enumerate(value):
                    if (
                        not robot_limit[class_name]["coords_min"][idx]
                        <= coord
                        <= robot_limit[class_name]["coords_max"][idx]
                    ):
                        raise MyPalletizer260DataException(
                            "Has invalid coord value, error on index {0}. received {3} .but angle should be {1} ~ {2}.".format(
                                idx,
                                robot_limit[class_name]["coords_min"][idx],
                                robot_limit[class_name]["coords_max"][idx],
                                coord,
                            )
                        )
                ...
                for data in value:
                    data_type = type(data)
                    check_value_type(
                        parameter, data_type, MyPalletizer260DataException, int
                    )
                    if data < 0 or data > 4096:
                        raise MyPalletizer260DataException(
                            f"The range of encoder is 0 ~ 4096, but the received value is {data}"
                ...
                if len(value) != 4:
                    raise MyPalletizer260DataException(
                        "The length of `speeds` must be 4."
                    )
                for data in value:
                    data_type = type(data)
                    check_value_type(
                ...
                if value < 1 or (value > 4 and value != 7):
                    raise MyPalletizer260DataException(
                        f"The range of id is 1 ~ 4 or 7, but the received is {value}"
                    )
            elif parameter == "torque":
                torque_min = 150
                torque_max = 980
        ...
        for parameter in parameter_list[1:]:
            ...
            if (
                ...
                if len(value) != joint_length:
                    raise ValueError(
                        f"The length of `angles` must be {joint_length}."
                    )
                ...
                if (2048 - value[1]) + (2048 - value[2]) != 0:
                    raise ValueError(
                        "The 2 and 3 servo encoder values must be reversed"
                    )
                ...
                if value in (0, 1, 2, 3, 4):
                    if class_name == "MyArmMControl":
                        raise ValueError(
                            f"modification is not allowed between 0~4, current data id: {value}"
                        )
                ...
                if not re.match(r"^[A-Za-z0-9]{8,63}$", value):
                    raise ValueError(
                        "The password must be 8-63 characters long and contain only letters and numbers."
                    )
                ...
                if not 1 <= value <= pin:
                    raise ValueError(
                        f"The pin number must be between 1 and {pin}."
                    )
                ...
                if value not in (1, 2, 3):
                    raise ValueError(
                        f"end_direction not right, should be 1 ~ 3, the received end_direction is {value}"
                    )
            elif parameter == "gripper_flag":
                if value not in (0, 1, 254):
                    raise ValueError(
                        f"gripper_flag not right, should be in (0, 1, 254), the received gripper_flag is {value}"
                    )
            elif parameter == "gripper_value":
                if not 0 <= value <= 100:
                    raise ValueError(
                        f"gripper_value not right, should be 0 ~ 100, the received gripper_value is {value}"
                    )
                ...
                if not 1 <= value <= 6:
                    raise ValueError(
                        "The basic pin number must be between 1 ~ 6."
                    )
            elif parameter == "rgb":
                for v in value:
                    if not 0 <= v <= 255:
                        raise ValueError(
                            f"rgb value not right, should be 0 ~ 255, the received rgb is {value}"
                        )
        ...
        for parameter, value in kwargs.items():
            ...
            if parameter in ("servo_id", "joint_id", "coord_id"):
                ...
                if value not in servo_ids:
                    raise ValueError(
                        f"The {parameter} not right, should be in {servo_ids}, but received {value}."
                    )
                ...
                index = _id - 1
                minimum_position = robotic_limit_table[f"{parameter}s_min"][
                    index
                ]
                maximum_position = robotic_limit_table[f"{parameter}s_max"][
                    index
                ]
                ...
                for speed in value:
                    if not 1 <= speed <= 100:
                        raise ValueError(
                            f"speed value not right, should be 1 ~ 100, the received speed is {speed}"
                        )
                ...
                if not len(value) == 6:
                    raise ValueError(
                        f"The length of encoders is 6, but the received value is {len(value)}"
                    )

                for encoder in value:
                    if not 0 <= encoder <= 4096:
                        raise ValueError(
                            f"The range of encoder is 0 ~ 4096, but the received value is {encoder}"
                        )
                ...
                for speed in value:
                    if not 1 <= speed <= 10000:
                        raise ValueError(
                            f"speed value not right, should be 1 ~ 10000, the received speed is {speed}"
                        )
                ...
                if value not in (0, 1):
                    raise ValueError(
                        f"The {parameter} not right, should be in (0, 1), but received {value}."
                    )

            elif parameter == "gripper_state":
                if value not in (0, 1, 254):
                    raise ValueError(
                        f"The {parameter} not right, should be in (0, 1, 254), but received {value}."
                    )

            elif parameter == "gripper_speed":
                if not 0 <= value <= 100:
                    raise ValueError(
                        f"The gripper_speed value not right, should be 0 ~ 100, the received speed is {value}"
                    )
            elif parameter == "gripper_torque":
                if not 150 <= value <= 980:
                    raise ValueError(
                        f"The gripper_torque value not right, should be 150 ~ 980, the received speed is {value}"
                    )
            elif parameter == "gripper_type":
                gripper_types = (1, 2, 3, 4)
                if value not in gripper_types:
                    raise ValueError(
                        f"The gripper_type not right, should be in {gripper_types}, but received {value}."
                    )
            elif parameter == "is_torque":
                torque_gripper_types = (0, 1)
                if value not in torque_gripper_types:
                    raise ValueError(
                        f"The is_torque not right, should be in {torque_gripper_types}, but received {value}."
                    )
            elif parameter == "value":
                if not 0 <= value <= 4096:
                    raise ValueError(
                        f"The value not right, should be 0 ~ 4096, but received {value}."
                    )
            elif parameter == "protect_current":
                if not 1 <= value <= 500:
                    raise ValueError(
                        f"The protect_current not right, should be 1 ~ 500, but received {value}."
                    )
            elif parameter == "end_direction":
                if value not in (1, 2, 3):
                    raise ValueError(
                        f"The end_direction not right, should be in (1, 2, 3), but received {value}."
                    )
                ...
                for color in value:
                    if not 0 <= color <= 255:
                        raise ValueError(
                            f"The color not right, should be 0 ~ 255, but received {color}."
                        )
        ...
        for parameter in parameter_list[1:]:
            ...
            if parameter == "pin_no_base":
                ...
                check_value_type(
                    parameter, value_type, MyCobotPro450DataException, int
                )
                if value not in [0, 1, 2, 3, 4]:
                ...
                coord_id = kwargs.get("coord_id", None)
                index = (
                    robot_limit[class_name]["coord_id"][coord_id - 1] - 1
                )  # Get the index based on the ID
                ...
                if not 1 <= value <= 100:
                    raise MyCobotPro450DataException(
                        f"speed value not right, should be 1 ~ 100, the error speed is {value}"
                    )
            elif parameter == "angles":
                check_angles(
                    value, robot_limit, class_name, MyCobotPro450DataException
                )
            elif parameter == "coords":
                check_coords(
                    parameter,
                ...
                if value < 0 or value > 10000:
                    raise MyCobotPro450DataException(
                        f"The parameter {parameter} only supports 0 ~ 10000, but received {value}"
                    )
            elif parameter == "rank":
                check_value_type(
                    parameter, value_type, MyCobotPro450DataException, int
                ...
                if value < 1 or value > 5:
                    raise MyCobotPro450DataException(
                        f"The parameter {parameter} only supports 1 ~ 5, but received {value}"
                    )
                ...
                if not 1 <= value <= 100:
                    raise MyCobotPro450DataException(
                        f"rank value not right, should be 1 ~ 100, the error speed is {value}"
                    )
            elif parameter == "axis":
                check_value_type(
                    parameter, value_type, MyCobotPro450DataException, int
                ...
                if value not in [0, 1]:
                    raise MyCobotPro450DataException(
                        f"The parameter {parameter} only supports [0,1], but received {value}"
                    )
            elif parameter == "add":
                check_value_type(
                    parameter, value_type, MyCobotPro450DataException, int
                ...
                if value < 0 or value > 62:
                    raise MyCobotPro450DataException(
                        f"The parameter {parameter} only supports 0 ~ 62, but received {value}"
                    )
            elif parameter == "rgb":
                ...
                if value < 0 or value > 254:
                    raise MyCobotPro450DataException(
                        f"The parameter {parameter} only supports 0 ~ 254, but received {value}"
                    )
                ...
                if value < 100 or value > 300:
                    raise MyCobotPro450DataException(
                        f"The parameter {parameter} only supports 100 ~ 300, but received {value}"
                    )
                ...
                check_value_type(
                    parameter, value_type, MyCobotPro450DataException, list
                )
                if len(value) > 64:
                    raise MyCobotPro450DataException(
                ...
                check_value_type(
                    parameter, value_type, MyCobotPro450DataException, str
                )
                ...
                check_world_tool_coords(
                    parameter, value, MyCobotPro450DataException
                )
        ...
        for parameter in parameter_list[1:]:
            ...
            if parameter == "id":
                ...
                check_value_type(
                    parameter, value_type, ultraArmP340DataException, str
                )
                ...
                check_value_type(
                    parameter, value_type, ultraArmP340DataException, int
                )
                ...
                if not 0 <= value <= 255:
                    raise ultraArmP340DataException(
                        f"pwm value not right, should be 0 ~ 255, the error p_value is {value}"
                    )
            elif parameter == "gripper_value":
                check_value_type(
                    parameter, value_type, ultraArmP340DataException, int
                ...
                if not 0 <= value <= 100:
                    raise ultraArmP340DataException(
                        f"gripper value not right, should be 0 ~ 100, the error gripper_value is {value}"
                    )
            elif parameter == "gripper_speed":
                check_value_type(
                    parameter, value_type, ultraArmP340DataException, int
                ...
                if not 1 <= value <= 1500:
                    raise ultraArmP340DataException(
                        f"gripper speed not right, should be 1 ~ 1500, the error gripper_speed is {value}"
                    )
            elif parameter == "address":
                check_value_type(
                    parameter, value_type, ultraArmP340DataException, int
                ...
                if not 7 <= value <= 69:
                    raise ultraArmP340DataException(
                        f"address not right, should be 7 ~ 69, the error address is {value}"
                    )
                ...
                if not 0 <= value <= 69:
                    raise ultraArmP340DataException(
                        f"get address not right, should be 0 ~ 69, the error address is {value}"
                    )
                ...
                if not 0 <= value <= 4096:
                    raise ultraArmP340DataException(
                        f"system value not right, should be 0 ~ 4096, the error system value is {value}"
                    )
            elif parameter == "angles":
                if not isinstance(value, list):
                    raise ultraArmP340DataException(
                ...
                if len(value) not in [3, 4]:
                    raise ultraArmP340DataException(
                        f"The length of `radians` must be 3 or 4, but received length is {len(value)}"
                    )
                ...
                for idx, radian in enumerate(value):
                    if not min_radian[idx] <= radian <= max_radian[idx]:
                        raise ultraArmP340DataException(
                            f"Has invalid radian value, error on index {idx}. Received {radian} but radian should be {min_radian[idx]} ~ {max_radian[idx]}."
                        )
    ...
    def wrapper(self, *args, **kwargs):
        try:
            if hasattr(self, "_serial_port"):
                if self._serial_port.port not in [
                    "/dev/left_arm",
                    "/dev/right_arm",
                ]:
