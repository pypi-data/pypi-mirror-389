...
from modusched.core import ArkAdapter
...
class Interviews():
    ...
    def get_status(self,user_id):
        ...
        try:
            ...
            result_1 = json.loads(wk)
        except Exception as e:
            logger.error(f"Error get_status-解析状态表时出现错误,{e}")
            result_1 = result
    ...
    async def chat_with_deepmodel(self,prompt_with_history: str,user_id: str,topic: str):
        logger.info(f'函数输入 & {type(prompt_with_history)} &  {prompt_with_history} user_id{user_id} topic {topic}')
        try:
            ...
            output = await ark.aproduct(prompt = input_wok)
            try:
            ...
            logger.error(f"Error 深度模型错误了,{e}")
            raise Exception(f"深度模型错误了 {e}") from e
        logger.info(f'函数输出 & {type('success')} & {"success"}')
    ...
    async def chat_with_iv(self,prompt_with_history,user_id,topic):
        try:
            ...
            logger.error(f"Error 解析状态表时出现错误,{e}")
            raise Exception(f'解析状态表时出现错误,{e}') from e

        output_generate = ark.aproduct_stream(prompt = prompt.format(topic = topic) + input_data) # 加工
...
class ChatBox():
    ...
    async def init_chatbox(self):
        self.interview.init_role_prompt()
    ...
    async def astream_product(self,prompt_with_history: str,user_id:str, model: str,topic:str) -> Any:
        ...
        # 只需要修改这里
        """
        logger.info(f'函数输入 & {type(prompt_with_history)} &  {prompt_with_history} model: {model},user_id{user_id} topic {topic}')
