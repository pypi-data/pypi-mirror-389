...
class AsyncTaskManager:
    ...
    def wait_for_done(self, task_id: str,
                      get_task_progress=None,
                      check_interval: float = 0.5,  # 进度检查间隔（秒）
                      wait_timeout: Optional[float] = None,  # 等待超时时间（秒，None表示不限制）
                      show_progress: bool = True) -> TaskInfo:
        """
        阻塞等待任务完成，并返回最终任务信息
        Args:
            task_id: 要等待的任务ID
            get_task_progress: 获取任务进度方法
            check_interval: 每次检查进度的时间间隔（默认0.5秒，间隔越小实时性越高）
            wait_timeout: 等待超时时间（超过此时间抛出TimeoutError，默认不限制）
            show_progress: 是否实时打印进度（默认True，控制台显示当前进度和步骤）
        Returns:
            TaskInfo: 任务最终状态信息（包含result/error）
        Raises:
            ValueError: 任务ID不存在
            TimeoutError: 等待超时
        """
        ...
        logger.info(f"开始等待任务完成（任务ID：{task_id}），检查间隔：{check_interval}秒")

        # 3. 循环检查任务状态，直到任务结束
        while True:
            # 获取最新任务状态（调用已有的 get_task_progress）
