...
from ..configs.config_pytorch_model_eval_step import PyTorchModelEvalConfig
...
try:
    from ..specs.pytorch_model_eval_spec import PYTORCH_MODEL_EVAL_SPEC

    SPEC_AVAILABLE = True
except ImportError:
    PYTORCH_MODEL_EVAL_SPEC = None
...
class PyTorchModelEvalStepBuilder(StepBuilderBase):
    """
    Builder for a PyTorch Model Evaluation ProcessingStep.
    ...
    Uses FrameworkProcessor to support source_dir parameter for PyTorch model evaluation.
    """
    ...
    def __init__(
        ...
        """
        Initializes the builder with a specific configuration for the PyTorch model evaluation step.

        Args:
            config: A PyTorchModelEvalConfig instance containing all necessary settings.
            sagemaker_session: The SageMaker session object to manage interactions with AWS.
            role: The IAM role ARN to be used by the SageMaker Processing Job.
            registry_manager: Optional registry manager for dependency injection
            dependency_resolver: Optional dependency resolver for dependency injection
        """
        if not isinstance(config, PyTorchModelEvalConfig):
            raise ValueError(
                "PyTorchModelEvalStepBuilder requires a PyTorchModelEvalConfig instance."
            )

        # Use the model evaluation specification if available
        spec = PYTORCH_MODEL_EVAL_SPEC if SPEC_AVAILABLE else None
        ...
        self.config: PyTorchModelEvalConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        """
        self.log_info("Validating PyTorchModelEvalConfig...")
        ...
        for attr in required_attrs:
            if not hasattr(self.config, attr) or getattr(self.config, attr) in [
                ...
                raise ValueError(
                    f"PyTorchModelEvalConfig missing required attribute: {attr}"
                )
        ...
        self.log_info("PyTorchModelEvalConfig validation succeeded.")
    ...
    def _create_processor(self) -> FrameworkProcessor:
        ...
        while providing a Python environment suitable for PyTorch model evaluation.
    ...
    def _get_outputs(self, outputs: Dict[str, Any]) -> List[ProcessingOutput]:
        ...
        for _, output_spec in self.spec.outputs.items():
            ...
            if logical_name in outputs:
                ...
                destination = Join(on="/", values=[base_output_path, "pytorch_model_evaluation", logical_name])
                self.log_info(
    ...
    def _get_job_arguments(self) -> List[str]:
        ...
        # For PyTorch model evaluation, we always return just the job_type argument
        # This is because pytorch_model_evaluation_contract has empty expected_arguments
    ...
    def create_step(self, **kwargs) -> ProcessingStep:
        ...
        """
        self.log_info("Creating PyTorchModelEval ProcessingStep...")
