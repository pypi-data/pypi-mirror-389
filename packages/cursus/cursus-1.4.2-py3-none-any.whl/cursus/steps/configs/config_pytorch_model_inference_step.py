...
This module implements the configuration class for the PyTorch model inference step
...
class PyTorchModelInferenceConfig(ProcessingStepConfigBase):
    """
    Configuration for PyTorch model inference step with self-contained derivation logic.

    This class defines the configuration parameters for the PyTorch model inference step,
    which generates predictions from trained PyTorch models without computing evaluation metrics.
    ...
    processing_entry_point: str = Field(
        default="pytorch_inference.py",
        description="Entry point script for PyTorch model inference.",
    )
    ...
    def initialize_derived_fields(self) -> "PyTorchModelInferenceConfig":
    ...
    @model_validator(mode="after")
    def validate_inference_config(self) -> "PyTorchModelInferenceConfig":
        ...
        if not self.id_name:
            raise ValueError(
                "id_name must be provided (required by PyTorch model inference contract)"
            )
    ...
    def get_environment_variables(self) -> Dict[str, str]:
        ...
        # Add PyTorch model inference specific environment variables
        env_vars.update(
            {
    ...
    def get_script_contract(self) -> "ScriptContract":
        ...
        Returns:
            The PyTorch model inference script contract
        """
    ...
    def get_public_init_fields(self) -> Dict[str, Any]:
        ...
        # Add PyTorch model inference specific fields
        inference_fields = {
