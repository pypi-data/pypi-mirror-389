...
class DictionaryEncodingProcessor(Processor):
    ...
    def __init__(self,
        ...
        if unknown_strategy not in ["error", "default", "ignore"]:
            raise ValueError(f"unknown_strategy must be one of ['error', 'default', 'ignore'], got {unknown_strategy}")
    ...
    def fit(self, data: Union[Dict, np.ndarray, pd.DataFrame]) -> 'DictionaryEncodingProcessor':
        ...
        if not self.categorical_map:
            if isinstance(data, pd.DataFrame):
                ...
                for key, values in data.items():
                    if isinstance(values, list) and all(isinstance(v, str) for v in values if pd.notna(v)):
        ...
        self.is_fitted = True
        logger.info(f"DictionaryEncodingProcessor fitted with {len(self.categorical_map)} categorical mappings")
        return self
        ...
        for col, mapping in self.categorical_map.items():
            if col in result.columns:
                if self.unknown_strategy == "default":
                    ...
                    unknown_values = set(result[col].dropna().unique()) - set(mapping.keys())
                    if unknown_values:
                        raise ValueError(f"Unknown categories found in column {col}: {unknown_values}")
        ...
        for key, values in input_data.items():
            if key in self.categorical_map:
                ...
                if isinstance(values, list):
                    if self.unknown_strategy == "default":
                        result[key] = [mapping.get(v, self.default_value) for v in values]
                    elif self.unknown_strategy == "error":
                        unknown_values = set(v for v in values if pd.notna(v)) - set(mapping.keys())
                        if unknown_values:
                            raise ValueError(f"Unknown categories found in key {key}: {unknown_values}")
                    ...
                    if self.unknown_strategy == "default":
                        ...
                        if values not in mapping:
                            raise ValueError(f"Unknown category found in key {key}: {values}")
    ...
    def _process_numpy_array(self, input_data: np.ndarray) -> np.ndarray:
        ...
        if input_data.dtype.kind in ['U', 'S', 'O']:  # Unicode, byte string, or object
            ...
            if self.categorical_map:
                ...
                if self.unknown_strategy == "default":
                    vectorized_map = np.vectorize(lambda x: mapping.get(x, self.default_value))
                elif self.unknown_strategy == "error":
    ...
    def inverse_transform(self, encoded_data: Union[Dict, np.ndarray, pd.DataFrame]) -> Union[Dict, np.ndarray, pd.DataFrame]:
        ...
        if isinstance(encoded_data, pd.DataFrame):
            ...
            for key, values in encoded_data.items():
                if key in inverse_maps:
                    ...
                    if isinstance(values, list):
                        result[key] = [inverse_map.get(v, f"UNKNOWN_{v}") for v in values]
                    else:
            ...
            if inverse_maps:
                ...
                vectorized_inverse = np.vectorize(lambda x: inverse_map.get(x, f"UNKNOWN_{x}"))
    ...
    def get_vocab_size(self, column_name: str) -> int:
        ...
        return max(self.categorical_map[column_name].values()) + 1 if self.categorical_map[column_name] else 1
    ...
    def __repr__(self) -> str:
        return (f"DictionaryEncodingProcessor(n_mappings={len(self.categorical_map)}, "
