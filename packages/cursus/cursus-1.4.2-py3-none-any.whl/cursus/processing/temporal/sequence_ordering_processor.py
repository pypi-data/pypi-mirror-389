...
class SequenceOrderingProcessor(Processor):
    ...
    def fit(self, data: Any) -> 'SequenceOrderingProcessor':
        ...
        self.is_fitted = True
        logger.info(f"SequenceOrderingProcessor fitted with sort_field: {self.sort_field}")
        return self
        ...
        if self.validate_order:
            ...
            if self.sort_order == "ascending":
                if not np.all(timestamps[:-1] <= timestamps[1:]):
                    logger.warning("Sequence ordering validation failed for ascending order")
            else:
                if not np.all(timestamps[:-1] >= timestamps[1:]):
                    logger.warning("Sequence ordering validation failed for descending order")
    ...
    def _process_dataframe(self, input_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.sort_field not in input_data.columns:
            raise ValueError(f"Sort field '{self.sort_field}' not found in DataFrame columns")
        
        ascending = self.sort_order == "ascending"
        result = input_data.sort_values(by=self.sort_field, ascending=ascending).reset_index(drop=True)
        ...
        if self.validate_order:
            ...
            if self.sort_order == "ascending":
                if not np.all(timestamps[:-1] <= timestamps[1:]):
                    logger.warning(f"Sequence ordering validation failed for ascending order on field '{self.sort_field}'")
            else:
                if not np.all(timestamps[:-1] >= timestamps[1:]):
                    logger.warning(f"Sequence ordering validation failed for descending order on field '{self.sort_field}'")
    ...
    def _process_dict(self, input_data: Dict) -> Dict:
        ...
        if self.sort_field not in input_data:
            raise ValueError(f"Sort field '{self.sort_field}' not found in input dictionary")
        ...
        if self.validate_order:
            ...
            if self.sort_order == "ascending":
                if not all(sorted_timestamps[i] <= sorted_timestamps[i+1] for i in range(len(sorted_timestamps)-1)):
                    logger.warning(f"Sequence ordering validation failed for ascending order on field '{self.sort_field}'")
            else:
                if not all(sorted_timestamps[i] >= sorted_timestamps[i+1] for i in range(len(sorted_timestamps)-1)):
                    logger.warning(f"Sequence ordering validation failed for descending order on field '{self.sort_field}'")
    ...
    def __repr__(self) -> str:
        return (f"SequenceOrderingProcessor(sort_field='{self.sort_field}', "
