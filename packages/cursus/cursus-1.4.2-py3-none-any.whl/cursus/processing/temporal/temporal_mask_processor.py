...
class TemporalMaskProcessor(Processor):
    ...
    def __init__(self,
        ...
        if output_format not in ["boolean", "float", "int"]:
            raise ValueError(f"output_format must be one of ['boolean', 'float', 'int'], got {output_format}")
    
    def fit(self, data: Any) -> 'TemporalMaskProcessor':
        """No fitting required for masking"""
        self.is_fitted = True
        logger.info(f"TemporalMaskProcessor fitted with output_format: {self.output_format}")
        return self
    
    def process(self, input_data: Union[np.ndarray, List, pd.DataFrame]) -> Union[np.ndarray, List]:
    ...
    def create_padding_mask(self, sequence_lengths: List[int], max_length: int) -> np.ndarray:
        """
    ...
    def __repr__(self) -> str:
        return (f"TemporalMaskProcessor(mask_value={self.mask_value}, "
