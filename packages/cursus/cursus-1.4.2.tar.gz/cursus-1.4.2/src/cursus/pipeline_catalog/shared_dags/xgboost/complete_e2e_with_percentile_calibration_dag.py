"""
Shared DAG definition for XGBoost Complete End-to-End Pipeline with Percentile Model Calibration and Testing Paths
...
def create_xgboost_complete_e2e_with_percentile_calibration_dag() -> PipelineDAG:
    """
    Create a DAG for complete XGBoost E2E pipeline with percentile model calibration and testing paths.
    ...
    dag.add_node("PercentileModelCalibration_calibration")  # Percentile model calibration step with calibration variant
    ...
    dag.add_edge("XGBoostModelEval_calibration", "PercentileModelCalibration_calibration")
    ...
    logger.info(
        f"Created XGBoost complete E2E with percentile calibration and testing DAG with {len(dag.nodes)} nodes and {len(dag.edges)} edges"
    )
    return dag
...
def get_dag_metadata() -> DAGMetadata:
    """
    Get metadata for the XGBoost complete end-to-end DAG with percentile model calibration and testing paths.
    ...
    return DAGMetadata(
        ...
        extra_metadata={
            ...
            "exit_points": ["Registration", "XGBoostModelEval_calibration", "ModelWikiGenerator"],
            "required_configs": [
                "CradleDataLoading_training",
                "CradleDataLoading_calibration",
                "CradleDataLoading_testing",
...
def validate_dag_structure(dag: PipelineDAG) -> Dict[str, Any]:
    """
    Validate the structure of the XGBoost complete end-to-end DAG with percentile model calibration and testing paths.
