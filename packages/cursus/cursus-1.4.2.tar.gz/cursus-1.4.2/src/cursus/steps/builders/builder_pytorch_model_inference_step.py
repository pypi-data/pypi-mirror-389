...
from ..configs.config_pytorch_model_inference_step import PyTorchModelInferenceConfig
...
try:
    from ..specs.pytorch_model_inference_spec import PYTORCH_MODEL_INFERENCE_SPEC

    SPEC_AVAILABLE = True
except ImportError:
    PYTORCH_MODEL_INFERENCE_SPEC = None
    SPEC_AVAILABLE = False

logger = logging.getLogger(__name__)


class PyTorchModelInferenceStepBuilder(StepBuilderBase):
    """
    Builder for a PyTorch Model Inference ProcessingStep.
    ...
    Uses FrameworkProcessor to support source_dir parameter for PyTorch model inference.
    """
    ...
    def __init__(
        ...
        """
        Initializes the builder with a specific configuration for the PyTorch model inference step.

        Args:
            config: A PyTorchModelInferenceConfig instance containing all necessary settings.
            sagemaker_session: The SageMaker session object to manage interactions with AWS.
            role: The IAM role ARN to be used by the SageMaker Processing Job.
            registry_manager: Optional registry manager for dependency injection
            dependency_resolver: Optional dependency resolver for dependency injection
        """
        if not isinstance(config, PyTorchModelInferenceConfig):
            raise ValueError(
                "PyTorchModelInferenceStepBuilder requires a PyTorchModelInferenceConfig instance."
            )

        # Use the model inference specification if available
        spec = PYTORCH_MODEL_INFERENCE_SPEC if SPEC_AVAILABLE else None
        ...
        self.config: PyTorchModelInferenceConfig = config
    ...
    def validate_configuration(self) -> None:
        ...
        """
        self.log_info("Validating PyTorchModelInferenceConfig...")
        ...
        for attr in required_attrs:
            if not hasattr(self.config, attr) or getattr(self.config, attr) in [
                ...
                raise ValueError(
                    f"PyTorchModelInferenceConfig missing required attribute: {attr}"
                )
        ...
        self.log_info("PyTorchModelInferenceConfig validation succeeded.")
    ...
    def _create_processor(self) -> FrameworkProcessor:
        ...
        This defines the execution environment for the PyTorch script, including the instance
        type, framework version, and environment variables.

        Uses FrameworkProcessor with PyTorch estimator to support source_dir parameter
        while providing a PyTorch environment suitable for model inference.
    ...
    def _get_environment_variables(self) -> Dict[str, str]:
        ...
        These variables are used to control the behavior of the PyTorch inference script
        ...
        self.log_info("PyTorch inference environment variables: %s", env_vars)
        return env_vars
    ...
    def _get_job_arguments(self) -> List[str]:
        ...
        # For PyTorch model inference, we always return just the job_type argument
        # This is because pytorch_model_inference_contract has empty expected_arguments
    ...
    def create_step(self, **kwargs) -> ProcessingStep:
        ...
        using the specification-driven approach with source_dir support.
        ...
        """
        self.log_info("Creating PyTorchModelInference ProcessingStep...")
