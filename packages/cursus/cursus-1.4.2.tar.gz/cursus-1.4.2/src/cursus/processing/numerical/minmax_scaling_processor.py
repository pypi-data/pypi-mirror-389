...
class MinMaxScalingProcessor(Processor):
    ...
    def __init__(self,
        ...
        if feature_range[0] >= feature_range[1]:
            raise ValueError(f"feature_range[0] must be less than feature_range[1], got {feature_range}")
    ...
    def fit(self, data: Union[np.ndarray, pd.DataFrame]) -> 'MinMaxScalingProcessor':
        ...
        self.is_fitted = True
        logger.info(f"MinMaxScalingProcessor fitted with feature_range: {self.feature_range}")
        return self
    ...
    def inverse_transform(self, scaled_data: Union[np.ndarray, pd.DataFrame]) -> Union[np.ndarray, pd.DataFrame]:
    ...
    def get_data_range(self, column: Optional[str] = None) -> Union[Tuple[float, float], Dict[str, Tuple[float, float]]]:
        ...
        if column is not None:
            ...
            # Return all ranges
            if isinstance(self.scale_params, dict) and any(isinstance(v, dict) for v in self.scale_params.values()):
                # DataFrame case
                return {col: (params['data_min_'], params['data_max_']) 
                       for col, params in self.scale_params.items() if isinstance(params, dict)}
            else:
                # Single array case
                return (self.scale_params['data_min_'], self.scale_params['data_max_'])
    ...
    def __repr__(self) -> str:
        return (f"MinMaxScalingProcessor(feature_range={self.feature_range}, "
