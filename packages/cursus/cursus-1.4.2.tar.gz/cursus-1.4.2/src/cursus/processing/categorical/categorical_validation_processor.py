...
class CategoricalValidationProcessor(Processor):
    ...
    def __init__(self,
        ...
        if validation_strategy not in ["strict", "warn", "filter"]:
            raise ValueError(f"validation_strategy must be one of ['strict', 'warn', 'filter'], got {validation_strategy}")
    ...
    def fit(self, data: Union[Dict, pd.DataFrame]) -> 'CategoricalValidationProcessor':
        ...
        self.is_fitted = True
        logger.info(f"CategoricalValidationProcessor fitted with {len(self.allowed_values)} predefined and {len(self.learned_values)} learned value sets")
        return self
    ...
    def _process_dataframe(self, input_data: pd.DataFrame) -> pd.DataFrame:
            ...
            if allowed_set:
                ...
                if len(invalid_values) > 0:
                    ...
                    if self.validation_strategy == "strict":
                        raise ValueError(f"Invalid values found in column {col}: {invalid_values}")
                    elif self.validation_strategy == "warn":
                        logger.warning(f"Invalid values found in column {col}: {invalid_values} (count: {invalid_mask.sum()})")
                    elif self.validation_strategy == "filter":
                        result = result[~invalid_mask]
                        logger.info(f"Filtered {invalid_mask.sum()} rows with invalid values in column {col}")
        
        # Apply custom validation rules
        for col, rule_func in self.validation_rules.items():
            if col in result.columns:
                try:
                    rule_violations = result[col].apply(lambda x: not rule_func(x) if pd.notna(x) else False)
                    if rule_violations.any():
                        ...
                        if self.validation_strategy == "strict":
                            raise ValueError(f"Custom validation rule failed for column {col}: {violation_count} violations")
                        elif self.validation_strategy == "warn":
                            logger.warning(f"Custom validation rule failed for column {col}: {violation_count} violations")
                        elif self.validation_strategy == "filter":
                            result = result[~rule_violations]
                            logger.info(f"Filtered {violation_count} rows failing custom rule for column {col}")
                except Exception as e:
                    logger.error(f"Error applying custom validation rule for column {col}: {e}")
        ...
        if self.max_violations is not None:
            ...
            if total_violations > self.max_violations:
                raise ValueError(f"Total violations ({total_violations}) exceed maximum allowed ({self.max_violations})")
    ...
    def _process_dict(self, input_data: Dict) -> Dict:
        ...
        for key, values in result.items():
            ...
            if allowed_set and isinstance(values, list):
                invalid_values = [v for v in values if v not in allowed_set and pd.notna(v)]
                ...
                if invalid_values:
                    ...
                    if self.validation_strategy == "strict":
                        raise ValueError(f"Invalid values found in key {key}: {set(invalid_values)}")
                    elif self.validation_strategy == "warn":
                        logger.warning(f"Invalid values found in key {key}: {set(invalid_values)} (count: {len(invalid_values)})")
                    elif self.validation_strategy == "filter":
                        result[key] = [v for v in values if v in allowed_set or pd.isna(v)]
                        logger.info(f"Filtered {len(invalid_values)} invalid values from key {key}")
        
        # Apply custom validation rules
        for key, rule_func in self.validation_rules.items():
            if key in result and isinstance(result[key], list):
                try:
                    invalid_indices = [i for i, v in enumerate(result[key]) 
                    ...
                    if invalid_indices:
                        ...
                        if self.validation_strategy == "strict":
                            raise ValueError(f"Custom validation rule failed for key {key}: {violation_count} violations")
                        elif self.validation_strategy == "warn":
                            logger.warning(f"Custom validation rule failed for key {key}: {violation_count} violations")
                        elif self.validation_strategy == "filter":
                            result[key] = [v for i, v in enumerate(result[key]) if i not in invalid_indices]
                            logger.info(f"Filtered {violation_count} values failing custom rule for key {key}")
                except Exception as e:
                    logger.error(f"Error applying custom validation rule for key {key}: {e}")
    ...
    def add_validation_rule(self, field: str, rule_func: callable, rule_name: Optional[str] = None) -> None:
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validation_rules": list(self.validation_rules.keys()),  # Can't serialize functions
            "validation_strategy": self.validation_strategy,
    ...
    def __repr__(self) -> str:
        return (f"CategoricalValidationProcessor(strategy='{self.validation_strategy}', "
