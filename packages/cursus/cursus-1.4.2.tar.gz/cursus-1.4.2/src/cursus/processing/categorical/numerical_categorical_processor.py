...
class NumericalCategoricalProcessor(Processor):
    ...
    def __init__(self,
        ...
        if binning_strategy not in ["equal_width", "equal_frequency", "custom", "threshold"]:
            raise ValueError(f"binning_strategy must be one of ['equal_width', 'equal_frequency', 'custom', 'threshold'], got {binning_strategy}")
    
    def fit(self, data: Union[np.ndarray, pd.DataFrame]) -> 'NumericalCategoricalProcessor':
        ...
        self.is_fitted = True
        logger.info(f"NumericalCategoricalProcessor fitted with strategy: {self.binning_strategy}")
        return self
    ...
    def _fit_numpy_array(self, data: np.ndarray) -> None:
        ...
        if self.binning_strategy == "equal_width":
            ...
            self.learned_bins['edges'] = np.linspace(data_min, data_max, self.n_bins + 1)
        elif self.binning_strategy == "equal_frequency":
            self.learned_bins['edges'] = np.percentile(data, np.linspace(0, 100, self.n_bins + 1))
    ...
    def _process_numpy_array(self, input_data: np.ndarray) -> np.ndarray:
        ...
        if self.labels:
            if input_data.ndim == 1:
                result = np.array([self.labels[min(idx, len(self.labels)-1)] for idx in result])
            else:
                for i in range(result.shape[1]):
                    result[:, i] = [self.labels[min(int(idx), len(self.labels)-1)] for idx in result[:, i]]
    ...
    def _process_dataframe(self, input_data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self.learned_bins.keys():
            if col in result.columns:
                ...
                # Apply binning
                result[col] = pd.cut(result[col], bins=edges, labels=False, include_lowest=True)
                
                # Apply custom labels if provided
                if self.labels:
                    result[col] = result[col].map(lambda x: self.labels[min(int(x), len(self.labels)-1)] if pd.notna(x) else x)
        
        return result
    
    def get_bin_info(self, column: Optional[str] = None) -> Union[Dict[str, Any], Dict[str, Dict[str, Any]]]:
    ...
    def __repr__(self) -> str:
        return (f"NumericalCategoricalProcessor(strategy='{self.binning_strategy}', "
