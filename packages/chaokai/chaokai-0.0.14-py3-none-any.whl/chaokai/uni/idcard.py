...
class IDCARD:
    ...
    def id_address(self,add):
        where = ID_address[add]
        # print('您的身份证归属地为：', where)
        return where
    ...
    def get_idcard_info(self,idcard):
        ...
        try:
            address = self.id_address(idcard_add)
            _dict['address'] = address
        except:
            _dict['address'] = ''

        birthday_result = self.birthday(idcard[6:14])
        _dict.update(birthday_result)

        _dict['constellation'] = self.constellations(idcard)

        sex = self.your_sex(idcard[16:17])
        _dict['gender'] = sex

        birthday_array = time.strptime('{}'.format(idcard[6:14]), "%Y%m%d")
        birthday_tsp = int(time.mktime(birthday_array))
        _dict['birthday_tsp'] = birthday_tsp

        _dict['age'] = (int(time.time()) - birthday_tsp) // (86400 * 365) + 1
    ...
    def birthday(self,date):
        ...
        return {'year': year, 'month': month, 'day': day, 'shengxiao': shengxiao[int(year) % 12]}


    def constellations(self,x):
        month = int(x[4:6])
        day = int(x[6:8])
        con = ('摩羯座', '水瓶座', '双鱼座', '白羊座', '金牛座', '双子座',
               '巨蟹座', '狮子座', '处女座', '天秤座', '天蝎座', '射手座')
        d = ((1, 20), (2, 19), (3, 21), (4, 21), (5, 21), (6, 22),
             (7, 23), (8, 23), (9, 23), (10, 23), (11, 23), (12, 23))
        constellation = con[len(list(filter(lambda y: y < (month, day), d))) % 12]
        # print('您的星座为：', constellation)
        return constellation


    def your_sex(self,sx):
        if int(sx) % 2 != 0:
            sex = '男'
            # print('您的性别为：', sex)
            return sex
...
def get_info(idcard_number):
    i = IDCARD()
    return i.get_idcard_info(idcard_number)

if __name__ == '__main__':

    # 获取身份证基本信息
    r = get_info('412923197405182152')
    print(r)
