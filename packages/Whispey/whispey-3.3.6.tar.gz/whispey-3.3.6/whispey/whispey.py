...
def generate_whispey_data(session_id: str, status: str = "in_progress", error: str = None) -> Dict[str, Any]:
    ...
    # Check for call_ended_reason with fallback to alternate field names
    call_ended_reason = (
        dynamic_params.get('call_ended_reason') or 
        dynamic_params.get('fail_called_reason') or 
        dynamic_params.get('fail_call_reason') or 
        'completed'
    )
