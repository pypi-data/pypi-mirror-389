...
async def send_to_whispey(data, apikey=None, api_url=None):
    ...
    # Handle call_ended_reason - check for alternate field names
    if "call_ended_reason" not in data:
        # Check for alternate field names
        if "fail_called_reason" in data:
            data["call_ended_reason"] = data.pop("fail_called_reason")
        elif "fail_call_reason" in data:
            data["call_ended_reason"] = data.pop("fail_call_reason")
        else:
