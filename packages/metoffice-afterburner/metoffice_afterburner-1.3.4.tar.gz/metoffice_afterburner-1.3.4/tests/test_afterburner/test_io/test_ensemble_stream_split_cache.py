...
"""
Script for testing various operations against an EnsembleStreamSplit data cache.
"""
...
from afterburner.io.datacaches import DataCache, DATA_CACHE_SCHEMES, ENSEMBLE_STREAM_SPLIT_SCHEME
from afterburner.exceptions import DataCacheError


class TestEnsembleStreamSplitCache(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cache = DataCache.create_cache(ENSEMBLE_STREAM_SPLIT_SCHEME,
            self.massds, base_dir=self.base_dir)
    ...
    def test_null_realization_dir(self):
        cache = DataCache.create_cache(ENSEMBLE_STREAM_SPLIT_SCHEME,
            self.massds, base_dir=self.base_dir, null_realization_dir='r0i0p0')

        var = UmMetaVariable('8.5', 'abcde', stream_id='apy', stash_code='m01s03i236')
        self.assertEqual(cache.get_cache_dir_for_variable(var), 'abcde/r0i0p0/apy')
        var.realization_id = 'r1i1p1'
        self.assertEqual(cache.get_cache_dir_for_variable(var), 'abcde/r1i1p1/apy')

        var = NemoMetaVariable('8.5', 'abcde', stream_id='ony', var_name='sosstsst')
        self.assertEqual(cache.get_cache_dir_for_variable(var), 'abcde/r0i0p0/ony')
        var.realization_id = 'r1i1p1'
        self.assertEqual(cache.get_cache_dir_for_variable(var), 'abcde/r1i1p1/ony')

        var = CiceMetaVariable('8.5', 'mi-ab123', stream_id='iny', var_name='snoice')
        self.assertEqual(cache.get_cache_dir_for_variable(var), 'mi-ab123/r0i0p0/iny')
        var.realization_id = 'r1i1p1'
        self.assertEqual(cache.get_cache_dir_for_variable(var), 'mi-ab123/r1i1p1/iny')

    def test_cache_type_mismatch(self):
        with self.assertRaises(DataCacheError):
            for scheme in [x for x in DATA_CACHE_SCHEMES if x != ENSEMBLE_STREAM_SPLIT_SCHEME]:
                DataCache.create_cache(scheme, self.massds, self.base_dir)

    def test_cache_type_no_mismatch(self):
        cache = DataCache.create_cache(ENSEMBLE_STREAM_SPLIT_SCHEME, self.massds, self.base_dir)
    ...
    def test_init_in_readonly_mode(self):
        ...
        cache = DataCache.create_cache(ENSEMBLE_STREAM_SPLIT_SCHEME, self.massds, base_dir,
            read_only=True)
        ...
        with self.assertRaises(DataCacheError):
            DataCache.create_cache(ENSEMBLE_STREAM_SPLIT_SCHEME, self.massds, base_dir,
                read_only=True)
    ...
    def test_fetch_stream_in_readonly_mode(self):
        cache = DataCache.create_cache(ENSEMBLE_STREAM_SPLIT_SCHEME, self.massds, self.base_dir,
            read_only=True)
        with self.assertRaises(DataCacheError):
            cache.fetch_stream('UM', 'expid', 'apy', realization_id='r1i1p1')
