...
"""
Test the afterburner.apps.model_monitor2.ModelMonitor2 application using simple 3D
diagnostics.
"""
...
class TestVerticalLevels(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patch = mock.patch('afterburner.apps.model_monitor2.ModelMonitor2._update_diag_data_cache')
        self.mock_update_cache = patch.start()
        self.addCleanup(patch.stop)

        patch = mock.patch('afterburner.apps.model_monitor2.ModelMonitor2._generate_plot_images')
        self.mock_gen_images = patch.start()
        self.addCleanup(patch.stop)

        patch = mock.patch('afterburner.apps.model_monitor2.ModelMonitor2._generate_html_page')
    ...
    def test_diag_with_levels(self):
        ...
        self.assertEqual(cube.var_name, 'm01s08i223')
...
class TestMultipleLevels(unittest.TestCase):
    """Test correct handling of multiple levels for the same diagnostic."""
    ...
    def test_diag_with_two_levels(self):
        "Test the generation of a diagnostic on two vertical levels."
        ...
        app_config = """
            ...
            [namelist:diags(soilmoist_l2)]
            enable=true
            level=2
            stashcode=m01s08i223

            [namelist:diags(soilmoist_l4)]
            enable=true
            level=4
            stashcode=m01s08i223
        """
        ...
        self.mock_load_model_data.assert_called()

        # test for existence of 2 netcdf files
        nc_files = os.listdir(os.path.join(app.nc_output_dir, 'awmean'))
...
class TestPressureLevels(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patch = mock.patch('afterburner.apps.ModelMonitor2._load_latest_model_data')
        self.mock_load_model_data = patch.start()
        self.addCleanup(patch.stop)
    ...
    def test_diag_with_levels(self):
        ...
        self.assertTrue(cube is not None)
        self.assertEqual(cube.ndim, 1)
        self.assertEqual(cube.var_name, 'm01s30i201')
