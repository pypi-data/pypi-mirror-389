...
class PreParation:
                ...
                if os.path.isfile(filepath):
                    if inputdict_cif:
                        ...
                        if 'mask' in inputdict_cif:
                            ...
                            mask_contours = inputdict_cif.get('mask_contour', None)
                            ...
                            masks = _parse_value(mask_path)
                            contours = _parse_value(mask_contours)

                            # Handle single mask case
                            if not isinstance(masks, (list, tuple)):
                                masks = [masks]

                            # Handle contours - convert to list with same length as masks
                            if isinstance(contours, (list, tuple)) and len(contours) == len(masks):
                                contour_values = contours
                            else:
                                # Use single contour value for all masks, default to 1.0
                                try:
                                    contour_value = float(contours) if contours is not None else 1.0
                                except Exception:
                                    contour_value = 1.0
                                contour_values = [contour_value] * len(masks)

                            # Create masks dictionary with proper float conversion
                            self.masks = {}
                            for mask, contour in zip(masks, contour_values):
                                try:
                                    self.masks[mask] = float(contour)
                                except Exception:
                                    self.masks[mask] = 1.0
                        else:
                            self.masks = {}
    ...
    def parse_emdb_cif(file_path):
        ...
        for map_file, contour, map_type in zip(map_files, contour_levels, map_types):
            ...
            if 'msk' in map_file:
                masks = map_data.get('mask', {})
                mask_list = masks.get('mask', [])[:]
                mask_contours = masks.get('mask_contour', [])[:]
                mask_list.append(map_key)
                mask_contours.append(contour)
                map_data['mask'] = {'mask': mask_list, 'mask_contour': mask_contours}
