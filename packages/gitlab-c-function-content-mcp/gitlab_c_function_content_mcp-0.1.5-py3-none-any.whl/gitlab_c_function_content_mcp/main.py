...
def get_file_content(filepath: str) -> str:
    """Get file content from filepath. Sample filepath: `SonicOS/firmware/fw/fwCore/handleIpPacket.c`"""
...
def get_function_content(filepath: str, function_name: str) -> str:
    """Get function content in specified file. Sample filepath: `SonicOS/firmware/fw/fwCore/handleIpPacket.c`."""
    file_content = request_file(gitlab_base_url, gitlab_repo, gitlab_branch, filepath)
    tsc = TscParse(file_content)
    functions = tsc.get_function(function_name.lstrip('/'))
    content = ''
    if len(functions) == 0:
