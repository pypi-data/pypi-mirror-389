...
def main(input_dir: Path, output_dir: Path, do_overwrite: bool = False):
    """Performs pre-MEDS data wrangling for eICU.

    Inputs are the raw eICU files, read from the `input_dir` config parameter. Output files are either
    ...
    Args (all as part of the config file):
        input_dir: The directory containing the raw eICU files.
    ...
    if patient_out_fp.is_file():
        ...
        hospital_fp = input_dir / "hospital.csv.gz"
