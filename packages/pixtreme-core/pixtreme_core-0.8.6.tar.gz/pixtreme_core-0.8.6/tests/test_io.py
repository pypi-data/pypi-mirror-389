...
class TestIO:
    ...
    def test_imwrite_imread_uint8(self, temp_dir, sample_image_uint8):
        ...
        with px.Device(0):
            ...
            # Read back and verify (imread returns float32 by default)
            loaded_image = px.imread(png_path, dtype="uint8")
    ...
    def test_imwrite_different_formats(self, temp_dir, sample_image_uint8):
        ...
        with px.Device(0):
            ...
            # Test TIFF format
            tiff_path = os.path.join(temp_dir, "test_format.tiff")
            success = px.imwrite(tiff_path, sample_image_uint8)
            assert success
            assert os.path.exists(tiff_path)

            # Test EXR format (requires float32)
            exr_path = os.path.join(temp_dir, "test_format.exr")
            sample_float32 = px.to_float32(sample_image_uint8)
            success = px.imwrite(exr_path, sample_float32)
            assert success
