...
def ndi_uyvy422_to_ycbcr444(uyvy_data: cp.ndarray, use_bilinear: bool = True) -> cp.ndarray:
    ...
    if channels != 2:
        raise ValueError(f"Input must have 2 channels (UV, Y), got {channels}")
    ...
    if use_bilinear:
        ndi_uyvy422_to_ycbcr444_bilinear_kernel(
            (blocks_per_grid,),
            (threads_per_block,),
            (uyvy_flat, yuv444_flat, height, width),
        )
    else:
...
ndi_uyvy422_to_ycbcr444_bilinear_kernel = cp.RawKernel(
    code=ndi_uyvy422_to_ycbcr444_bilinear_kernel_code,
    name="ndi_uyvy422_to_ycbcr444_bilinear_kernel",
)
