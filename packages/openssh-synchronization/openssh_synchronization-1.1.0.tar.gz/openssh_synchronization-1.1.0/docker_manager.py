...
class DockerManager:
    ...
    def run_container(self, 
        ...
        run_cmd = [
            ...
            "-v", f"{self.data_dir}/openssh:/data/openssh",
...
使用示例:
  ...
  python docker_manager.py run            # 运行容器
  python docker_manager.py run --debug     # 以调试模式运行
  ...
  python docker_manager.py cleanup        # 清理所有资源
    ...
    parser.add_argument("--check-interval", type=int, default=24, 
                       help="检查间隔(小时) (默认: 24)")
    parser.add_argument("--min-version", default="10.2.1", 
    ...
    if args.command == "build":
        manager.build_image(tag=args.tag, no_cache=args.no_cache)
