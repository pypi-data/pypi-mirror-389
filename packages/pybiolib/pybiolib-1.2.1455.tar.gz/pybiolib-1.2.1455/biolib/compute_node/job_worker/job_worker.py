...
def _allocate_network_with_retries(
    ...
    job_hash = int(hashlib.sha256(job_id.encode()).hexdigest(), 16)
    starting_offset = (job_hash % 256) + network_index
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            logger_no_user_data.debug(
                f'Created network {name} with subnet {candidate_subnet} and static IP {static_ip}'
            )
            return network
    ...
    raise BioLibError(
        f'Failed to allocate and create network for job {job_id} after {max_attempts} attempts. '
        f'Base CIDR: 172.28.0.0/16'
    )
        ...
        if len(remote_hosts) > 0:
            ...
            try:
                ...
                for hostname, ports in hostname_to_ports.items():
                    ...
                    network_name = f'biolib-remote-host-network-{job_id}-{network_index}'
