...
def test_submit_task_includes_openrouter_key() -> None:
    ...
    task_id = client.submit_task("Check payload", open_router_key="custom-key")

    assert task_id == "task_789"
    assert captured_payload["prompt"] == "Check payload"
    assert captured_payload["openRouterKey"] == "custom-key"
...
def test_wait_for_completion_success() -> None:
    ...
    result = client.wait_for_completion("task_123", poll_interval=0.01, max_attempts=5)
...
async def test_async_run_task_success() -> None:
    ...
    assert result["taskId"] == "task_async"
...
def test_submit_task_missing_task_id() -> None:
    ...
    with pytest.raises(TaskForceAIError, match="did not return a taskId"):
...
@pytest.mark.asyncio
async def test_async_submit_task_includes_openrouter_key() -> None:
    captured_payload: Dict[str, Any] = {}
    ...
    task_id = await client.submit_task("Check async payload", open_router_key="async-key")

    assert task_id == "task_async_router"
    assert captured_payload["openRouterKey"] == "async-key"
    await client.close()
