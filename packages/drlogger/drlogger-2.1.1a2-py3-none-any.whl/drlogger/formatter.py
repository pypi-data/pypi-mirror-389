...
from drlogger.instrumentation import (
    get_current_span,
    get_tracer_provider,
    get_logger_provider,
)
...
class DrFormatter(jsonlogger.JsonFormatter):
    ...
    def add_fields(self, log_record, record, message_dict):
        ...
        if span := get_current_span():
            ...
            try:
                ...
                pass
        elif logger_provider := get_logger_provider():
            try:
                resource_attributes = logger_provider.resource.attributes

                for key, value in resource_attributes.items():
                    log_record[key] = value
            except AttributeError:
                pass
            log_record["trace_id"] = "unknown_trace_id"
        else:
            # Fallback to environment variables if OpenTelemetry is not configured
            log_record["service.name"] = os.getenv("SERVICE_NAME", "unknown_service")
            log_record["service.version"] = os.getenv(
                "SERVICE_VERSION", "unknown_version"
            )
            log_record["trace_id"] = "unknown_trace_id"
    ...
    def __get_request_data(self, request_context):
        ...
        try:
            parameters["request_args"] = json.dumps(
                request_context.args.to_dict(flat=False), default=str
            )
        ...
        if request_context.method in ["POST", "PUT"]:
            try:
                if request_context.json:
                    parameters["req_data"] = json.dumps(
                        request_context.json, default=str
                    )
            except Exception as err:
