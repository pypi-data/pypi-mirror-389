    ...
    try:
        ...
        if current_tool_call["name"]:
            ...
            try:
                ...
                yield f"data: {json.dumps({'type': 'tool_result', 'tool': current_tool_call['name'], 'result': result})}\n\n"
                
                # Stream a follow-up message with the tool result
                if result.get("success"):
                    yield f"data: {json.dumps({'type': 'content', 'content': f'\n\n✅ Executed: {current_tool_call[\"name\"]}'})}\n\n"
                else:
                    yield f"data: {json.dumps({'type': 'content', 'content': f'\n\n❌ Error: {result.get(\"error\", \"Unknown error\")}'})}\n\n"
                    
            except Exception as e:
                yield f"data: {json.dumps({'type': 'error', 'error': f'Tool execution error: {str(e)}'})}\n\n"

        yield f"data: {json.dumps({'type': 'done', 'metadata': {'dt_active': True, 'model': LLM_MODEL, 'tools_available': len(dt.execution_node.tools)}})}\n\n"
...
async def stream_think(request: Request, req: StreamThinkRequest):
    ...
    try:
        ...
        # Initialize full Digital Twin with ExecutionNode
        dt = DigitalTwin(identity)
        print(f"🤖 Digital Twin initialized with ExecutionNode")
