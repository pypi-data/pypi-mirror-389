...
@router.post("/refresh")
async def refresh_capabilities(request: Request):
    """
    Refresh tool and app discovery.
    
    Useful when new apps are installed or existing apps are updated.
    Forces a re-scan of all available apps and tools.
    ...
    try:
        ...
        # Force re-discovery
        discovery_result = await dt.execution_node.refresh_discovery()
        dt.tools_discovered = True
        ...
        raise HTTPException(status_code=500, detail=f"Error refreshing capabilities: {str(e)}")
        capabilities = dt.get_capabilities()
