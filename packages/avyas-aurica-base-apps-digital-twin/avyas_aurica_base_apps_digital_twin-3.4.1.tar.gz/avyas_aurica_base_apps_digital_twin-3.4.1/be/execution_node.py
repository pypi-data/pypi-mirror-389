        ...
        try:
            ...
            if request_body:
                ...
                if json_schema:
                    ...
                    parameters["properties"].update(body_props)
                    
                    # Add required fields from body
                    body_required = json_schema.get("required", [])
                    parameters["required"].extend(body_required)
            
            # Determine autonomy level (default to "ask" for safety)
            # GET requests with no side effects can be "full"
            # POST/PUT/DELETE should be "ask" or "restricted"
            if method == "GET":
                autonomy_level = "full"
                side_effects = "none"
