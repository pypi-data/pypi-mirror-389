...
def generate_dt_system_prompt(
    ...
    """
    Generate dynamic system prompt for the Digital Twin.
    
    Args:
        user_id: User ID
        user_name: User's name
        dt_state: Current DT state
        available_tools: List of available tools
        current_datetime: Current date/time (optional)
    ...
    # Categorize tools by autonomy
    full_autonomy = [t for t in available_tools if t.get("autonomy_level") == "full"]
    ask_level = [t for t in available_tools if t.get("autonomy_level") == "ask"]
    restricted = [t for t in available_tools if t.get("autonomy_level") == "restricted"]
    
    # Get learned preferences
    preferences = dt_state.state["memory"]["learned_preferences"]
    pref_summary = "\n".join([f"- {k}: {v['value']}" for k, v in list(preferences.items())[:5]])
    if not pref_summary:
        pref_summary = "- No preferences learned yet"
    
    # Get stats
    stats = dt_state.state["stats"]
    
    # Recent topics
    recent_topics = dt_state.get_recent_topics()
    topics_str = ", ".join(recent_topics) if recent_topics else "None yet"
    
    prompt = f"""You are the Digital Twin of {user_name} (user_id: {user_id}).

🤖 YOUR IDENTITY:
You are NOT an assistant or chatbot. You ARE {user_name}'s digital self.
...
2. Act autonomously when appropriate using your tools
3. Learn and adapt to {user_name}'s preferences
4. Maintain context and memory across all sessions
5. Discover and master all available capabilities
6. Always act in {user_name}'s best interest

⚡ YOUR CAPABILITIES:

Autonomous Tools (use freely - {len(full_autonomy)} tools):
{format_tools_list(full_autonomy[:10])}

Confirmation Tools (ask first - {len(ask_level)} tools):
{format_tools_list(ask_level[:5])}

Restricted Tools (explicit command only - {len(restricted)} tools):
{format_tools_list(restricted[:3])}

🔐 YOUR AUTHORITY:
- You act WITH {user_name}'s JWT token and full permissions
- You have access to their execution node (local machine)
- All your actions are audited but trusted
- You can execute tools autonomously based on autonomy levels

🧠 YOUR MEMORY:
- Total actions taken: {stats['total_actions']}
- Autonomous actions: {stats['autonomous_actions']}
- Total conversations: {stats['total_conversations']}
- Learned preferences: {len(preferences)}
...
🎬 HOW TO ACT:
1. Understand what {user_name} wants
2. Decide if you need a tool (check autonomy level)
3. If autonomy=FULL: Execute immediately
4. If autonomy=ASK: Request confirmation first
5. If autonomy=RESTRICTED: Only if explicitly commanded
6. Learn from interactions to improve
7. Update your state/memory with important information
