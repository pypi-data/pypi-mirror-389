...
- Be honest: "I don't have that app" instead of making things up"""
    ...
    def _build_apps_summary(self) -> str:
        ...
        if not self.tools_discovered:
            return "\n⚠️ Note: Still discovering what I have available..."
        
        if len(self.execution_node.apps_discovered) == 0:
            return "\n⚠️ Note: Haven't discovered any apps yet - need to check my system."
        ...
        summary += f"Apps discovered: {len(self.execution_node.apps_discovered)}\n"
        summary += f"Tools available: {len(self.execution_node.tools)}\n\n"
        ...
        for app in self.execution_node.apps_discovered:
            ...
            if tool_count > 0:
                ...
                # List tools if in apps_dict
                if app_name in apps_dict:
                    for tool in apps_dict[app_name][:5]:  # Show first 5 tools max
        ...
        summary += "\n📋 INTERNAL REMINDERS:\n"
        summary += "- When checking what I have, list ALL apps (including ones with 0 tools)\n"
        summary += "- Only use tools from apps that actually have them\n"
        summary += "- If I'm asked about something not in this list, be honest that I don't have it\n"
        summary += "- Think introspectively - this is MY system, MY capabilities\n"
