...
def prepare_co_settings(keyword: str,
                     years: list[int],
    ...
    def _prepare_years_setting():
        for y in years:
            ...
            if not path.exists():
                # 인덱스 포함 '빈 파일' 생성
                pd.DataFrame(columns=years_label).to_csv(path, index=False)
                dfs.append(pd.DataFrame())
                msgs.append(f"[알림] 파일이 없어 빈 파일을 만들었습니다: {path}")
                continue
            ...
            try:
                ...
                if df.empty:
                    warn_bits.append("데이터가 없음")
                if _schema_mismatch(df, index_label=years_label[0], expected_cols=years_label[1:]):
                    warn_bits.append("잘못된 형식의 설정 파일")
                if warn_bits:
                    msgs.append(f"[경고] {path} → {', '.join(warn_bits)}")
                dfs.append(df)
