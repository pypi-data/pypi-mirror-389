...
class RackConfig:
    ...
    df_goods: pd.DataFrame = None
    # 레벨별 픽킹 시간(분)
    level_pick_minutes: Dict[int, int] = field(
        default_factory=lambda: {1: 5, 2: 10, 3: 20, 4: 15}
    )
...
class Racks:
    ...
    def fill_from(
        ...
        *,
        ...
        """
        self.so_cost = int((so_df.get("수량", 0) * self.cfg.df_goods.get("가격", 0)).sum())
    ...
    def consume(self, product: str, quantity: int, *, when, order: str = "bottom") -> dict:
        """
        ...
        if quantity <= 0 or quantity > total_available:
            return {
                ...
                "customer_order": {"product": product, "quantity": quantity,
                                   "cost": int(self.cfg.df_goods.get("가격")[product]) * quantity
                },
        ...
        if order.lower() == "fefo":
            ...
            sub = sub.sort_values(["level", "rack", "cell"])
        else:
        ...
        pick_minutes_total = 0
        level_cost = self.cfg.level_pick_minutes  # {1:5, 2:10, 3:20, 4:15}
        ...
        for idx, row in sub.iterrows():
            ...
            if take > 0:
                lvl = int(row["level"])
                pick_minutes_total += int(level_cost.get(lvl, 0))  # 정의 없으면 0
        ...
        self.co_profit += int(self.cfg.df_goods.get("가격")[product]) * quantity
        ...
        return {
            ...
            "economic_status": {
                "month": when.month,
                "day": when.day,
                "co_profit": self.co_profit,
            },
            "time_cost": {
                "pick_minutes": int(pick_minutes_total)
            },
    ...
    def state_current(self, *, at=None) -> dict:
        ...
        return {
            ...
            "stock_status": None,
            "product_definitions": self.cfg.df_goods.to_dict(orient="records"),
            "economic_status": {
                "month": now_ts.month,
                "day": now_ts.day,
