...
class MasterScene:
    ...
    def add(self, scene: Scene, layer: Literal["top", "bottom"] = "top"):
        ...
        if scene.start_time is None:
            ...
            if was_empty and not self._has_content_at_start:
                # 最初のシーンが0秒に配置された場合は、以降のシーンが重ならないようフラグを立てる
                self._has_content_at_start = True
    ...
    def _update_master_bgm_durations(self):
        """マスターシーンサイドでループBGMの長さを補正"""
        from .audio_element import AudioElement
        for audio_element in self.audio_elements:
            if not isinstance(audio_element, AudioElement):
                continue
            if not getattr(audio_element, 'loop_until_scene_end', False):
                continue
            if getattr(audio_element, '_duration_overridden', False):
                continue
            content_end = getattr(audio_element, '_content_end_time', audio_element.start_time + audio_element.duration)
            target_duration = max(0.0, content_end - audio_element.start_time)
            if target_duration > 0.0 and abs(audio_element.duration - target_duration) > 1e-6:
                audio_element.duration = target_duration
    ...
    def _collect_audio_elements(self, scene: Scene, time_offset: float = 0.0):
        ...
        for element in scene.elements:
            if isinstance(element, Scene):
                ...
                audio_copy._content_end_time = min(scene_end, audio_copy.start_time + element.duration)
                ...
                if audio_element is not None:
                    ...
                    # Adjust timing only on the copy
                    audio_copy.start_time += total_offset
                    audio_copy._content_end_time = min(scene_end, audio_copy.start_time + audio_element.duration)
