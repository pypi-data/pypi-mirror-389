...
def _load_token():
    ...
    if TOKEN_FILE.exists():
        try:
            ...
            typer.echo(f"Debug: Loaded token from file: {AUTH_TOKEN[:20] if AUTH_TOKEN else 'None'}...")
        except Exception as e:
            typer.echo(f"Debug: Failed to load token: {e}")
            AUTH_TOKEN = None
    else:
        AUTH_TOKEN = None
    return AUTH_TOKEN
...
def _save_token(token):
    ...
    try:
        ...
        typer.echo(f"Debug: Token saved to {TOKEN_FILE}")
    except Exception as e:
        typer.echo(f"Debug: Failed to save token: {e}")
        pass
...
def _is_token_expired(token):
    ...
    try:
        ...
        if len(parts) not in [3, 4, 5]:
            typer.echo(f"Debug: Invalid token format - {len(parts)} parts")
            return True
        ...
        typer.echo(f"Debug: Token exp={exp}, current={current_time}, expired={is_expired}")
        return is_expired
    except Exception as e:
        typer.echo(f"Debug: Token decode error: {e}")
        return True
...
def login(
    ...
    try:
        ...
        typer.echo(f"Debug: Found token: {AUTH_TOKEN[:20] if AUTH_TOKEN else 'None'}...")
        typer.echo(f"Debug: Full token: {AUTH_TOKEN}")
        typer.echo(f"Debug: Token parts: {AUTH_TOKEN.split('.') if AUTH_TOKEN else 'None'}")
...
def whoami():
    ...
    try:
        ...
        typer.echo(f"Debug: API response: {user_data}")
