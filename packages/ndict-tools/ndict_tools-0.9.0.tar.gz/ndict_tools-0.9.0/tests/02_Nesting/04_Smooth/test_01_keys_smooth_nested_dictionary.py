...
def test_smooth_nested_dictionary_keys(smooth_f_snd, keys, leaf):
    value = smooth_f_snd[keys[0]]
    for key in keys[1:]:
        value = value[key]
    assert value == leaf


class TestKeysSmoothNestedDictionary:
    ...
    def test_change_keys(self, smooth_c_snd, keys, end_key, old_value, new_value):
        d = smooth_c_snd
        for key in keys:
            d = d[key]
    ...
    def test_change_control(self, smooth_c_snd, keys, leaf):
        value = smooth_c_snd[keys[0]]
        for key in keys[1:]:
            value = value[key]
        assert value == leaf
    ...
    def test_key_type_failed(self, smooth_c_snd, false_keys_type, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            smooth_c_snd[false_keys_type] = None
    ...
    def test_hybrid_keys(self, smooth_c_snd, key_a, key_b, type):
        assert (
            smooth_c_snd["global_settings"][key_a]
            != smooth_c_snd["global_settings"][key_b]
        )
        assert isinstance(smooth_c_snd["global_settings"][key_a], type)
    ...
    def test_key_list(self, smooth_c_snd, key, associated_list):
        assert smooth_c_snd.key_list(key) == associated_list


class TestUnpackSmoothNestedDictionary:
    ...
    def test_upack_keys(self, smooth_c_snd, unpacked_keys):
        assert unpacked_keys in smooth_c_snd.unpacked_keys()
    ...
    def test_unpack_items(self, smooth_c_snd, unpacked_items):
        assert unpacked_items in smooth_c_snd.unpacked_items()
    ...
    def test_unpack_values(self, smooth_c_snd, unpacked_values):
        assert unpacked_values in smooth_c_snd.unpacked_values()
    ...
    def test_key_list_false(self, smooth_c_snd, false_key, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            smooth_c_snd.key_list(false_key)
    ...
    def test_items_list(self, smooth_c_snd, key, items_list):
        assert smooth_c_snd.items_list(key) == items_list
    ...
    def test_items_list_false(self, smooth_c_snd, false_key, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            if false_key:
                smooth_c_snd.items_list(false_key)
            else:
                smooth_c_snd.items_list()
    ...
    def test_leaves(self, smooth_c_snd, value, confirm, first_occurrence):
        if confirm:
            assert value in smooth_c_snd.leaves()
            assert smooth_c_snd.leaves().index(value) == first_occurrence - 1
        else:
            assert value not in smooth_c_snd.leaves()
