...
def test_strict_nested_dictionary_keys(strict_f_snd, keys, leaf):
    value = strict_f_snd[keys[0]]
    for key in keys[1:]:
        value = value[key]
    assert value == leaf


class TestKeysStrictNestedDictionary:
    ...
    def test_change_value(self, strict_c_snd, keys, end_key, old_value, new_value):
        d = strict_c_snd
        for key in keys:
            d = d[key]
    ...
    def test_change_control(self, strict_c_snd, keys, leaf):
        value = strict_c_snd[keys[0]]
        for key in keys[1:]:
            value = value[key]
        assert value == leaf
    ...
    def test_change_keys_failed(
        self, strict_c_snd, keys, false_end_key, error, error_msg
    ):
        d = strict_c_snd
        for key in keys:
            d = d[key]
    ...
    def test_key_type_failed(self, strict_c_snd, false_keys_type, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            strict_c_snd[false_keys_type] = None
    ...
    def test_hybrid_keys(self, strict_c_snd, key_a, key_b, type):
        assert (
            strict_c_snd["global_settings"][key_a]
            != strict_c_snd["global_settings"][key_b]
        )
        assert isinstance(strict_c_snd["global_settings"][key_a], type)


class TestUnpackStrictNestedDictionary:
    ...
    def test_upack_keys(self, strict_c_snd, unpacked_keys):
        assert unpacked_keys in strict_c_snd.unpacked_keys()
    ...
    def test_unpack_items(self, strict_c_snd, unpacked_items):
        assert unpacked_items in strict_c_snd.unpacked_items()
    ...
    def test_unpack_values(self, strict_c_snd, unpacked_values):
        assert unpacked_values in strict_c_snd.unpacked_values()
    ...
    def test_key_list(self, strict_c_snd, key, associated_list):
        assert strict_c_snd.key_list(key) == associated_list
    ...
    def test_key_list_false(self, strict_c_snd, false_key, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            strict_c_snd.key_list(false_key)
    ...
    def test_items_list(self, strict_c_snd, key, items_list):
        assert strict_c_snd.items_list(key) == items_list
    ...
    def test_items_list_false(self, strict_c_snd, false_key, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            if false_key:
                strict_c_snd.items_list(false_key)
            else:
                strict_c_snd.items_list()
    ...
    def test_leaves(self, strict_c_snd, value, confirm, first_occurrence):
        if confirm:
            assert value in strict_c_snd.leaves()
            assert strict_c_snd.leaves().index(value) == first_occurrence - 1
        else:
            assert value not in strict_c_snd.leaves()


class TestBuildStrictNestedDictionary:
    ...
    def test_build_with_keys(
        self, strict_c_snd, empty_c_strict_snd, nested_strict_c_setup, keys, value
    ):
        stacked_dictionary = empty_c_strict_snd
        d_path = []
        for key in keys[:-1]:
            stacked_dictionary = stacked_dictionary[key]
            d_path.append(key)
        if isinstance(value, dict):
            stacked_dictionary[keys[-1]] = StrictNestedDictionary(
                value, default_setup={"indent": 3}
            )
        else:
            stacked_dictionary[keys[-1]] = value
        d_path.append(keys[-1])
        assert empty_c_strict_snd[d_path] == stacked_dictionary[keys[-1]]

    def test_compare(self, strict_c_snd, empty_c_strict_snd):
        assert strict_c_snd == empty_c_strict_snd
    ...
    def test_delete_with_key(self, empty_c_strict_snd, keys):
        del empty_c_strict_snd[keys]
        with pytest.raises(KeyError):
            assert empty_c_strict_snd[keys]
    ...
    def test_update_with_key(self, empty_c_strict_snd, strict_c_snd, keys, value):
        empty_c_strict_snd[keys[:-1]].update({keys[-1]: value})
    ...
    def test_kwargs_update(self, empty_c_strict_snd, strict_c_snd):
        ...
        with pytest.raises(KeyError):
            assert empty_c_strict_snd[[("env", "production"), "api", "timeout"]]
            assert empty_c_strict_snd[[("env", "production"), "api", "rate_limit"]]
        empty_c_strict_snd[[("env", "production")]].update(api={"rate_limit": 10000})
        empty_c_strict_snd[[("env", "production"), "api"]].update(timeout=30)
        assert (
            empty_c_strict_snd[[("env", "production"), "api", "timeout"]]
            == strict_c_snd[[("env", "production"), "api", "timeout"]]
        )

    def test_compare_again(self, empty_c_strict_snd, strict_c_snd):
        assert empty_c_strict_snd == strict_c_snd
