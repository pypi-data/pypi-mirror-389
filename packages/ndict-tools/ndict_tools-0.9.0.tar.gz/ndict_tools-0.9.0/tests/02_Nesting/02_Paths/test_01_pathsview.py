...
def test_path_strict_sd(strict_f_nd, path, leaf):
    assert strict_f_nd[path] == leaf
...
class TestPathStrictSD:
    ...
    def test_get_depth(self, strict_c_nd):
        assert strict_c_nd.paths().get_depth() == 5
    ...
    def test_change_value(self, strict_c_nd, path, old_value, new_value):
        assert strict_c_nd[path] == old_value
        strict_c_nd[path] = new_value
        assert strict_c_nd[path] != old_value
    ...
    def test_change_control(self, strict_c_nd, path, leaf):
        assert strict_c_nd[path] == leaf
    ...
    def test_hierarchical(self, strict_c_nd, a_path, b_path, v_type):
        assert strict_c_nd[a_path] != strict_c_nd[b_path]
        assert isinstance(strict_c_nd[a_path], v_type)
    ...
    def test_paths_type_failed(self, strict_c_nd, false_keys_type, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
    ...
    def test_paths_list_failed(self, strict_c_nd, path, error_msg):
        with pytest.raises(StackedKeyError, match=re.escape(error_msg)):
            strict_c_nd.is_key(path)
...
def test_path_smooth_sd(smooth_f_nd, path, leaf):
    assert smooth_f_nd[path] == leaf


class TestPathSmoothSD:

    def test_get_depth(self, smooth_c_nd):
        assert smooth_c_nd.paths().get_depth() == 5
    ...
    def test_change_value(self, smooth_c_nd, path, old_value, new_value):
        assert smooth_c_nd[path] == old_value
        smooth_c_nd[path] = new_value
        assert smooth_c_nd[path] != old_value
    ...
    def test_change_control(self, smooth_c_nd, path, leaf):
        assert smooth_c_nd[path] == leaf
    ...
    def test_hierarchical(self, smooth_c_nd, a_path, b_path, v_type):
        assert smooth_c_nd[a_path] != smooth_c_nd[b_path]
        assert isinstance(smooth_c_nd[a_path], v_type)
    ...
    def test_path_type_failed(self, smooth_c_nd, false_keys_type, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
...
class TestDictPathsStrictSD:
    ...
    def test_path(self, strict_c_nd, path):
        assert path in strict_c_nd.dict_paths()

    def test_eq_path(self, strict_c_nd, smooth_c_nd):
        assert strict_c_nd.dict_paths() == smooth_c_nd.dict_paths()

    def test_neq_path(self, strict_c_nd, nested_strict_c_setup):
        assert strict_c_nd.dict_paths() != NestedDictionary(
            {}, default_setup=nested_strict_c_setup
        )
...
class TestChildrenPaths:
    ...
    def test_get_children(self, strict_c_nd, path, children):
        assert children == strict_c_nd.paths().get_children(path)
    ...
    def test_has_children(self, strict_c_nd, path, children):
        assert strict_c_nd.paths().has_children(path) is children
    ...
    def test_filter_paths(self, strict_c_nd, key, length, expected):
        assert expected == strict_c_nd.paths().filter_paths(
    ...
    def test_leaf_paths(self, smooth_c_nd, leaf):
        assert leaf in smooth_c_nd.paths().get_leaf_paths()
