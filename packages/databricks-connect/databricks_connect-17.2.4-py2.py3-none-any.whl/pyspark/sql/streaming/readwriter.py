...
class DataStreamReader(OptionUtils):
    ...
    def excel(
        ...
        headerRows: Optional[Union[int, str]] = None,
        listSheets: Optional[Union[bool, str]] = None,
        dateFormat: Optional[str] = None,
        timestampFormat: Optional[str] = None,
    ) -> "DataFrame":
        """Loads a excel file stream, returning the result as a :class:`DataFrame`.
        ...
        Examples
        --------
        Load a data stream from a temporary excel file.

        >>> import tempfile
        >>> import time
        >>> with tempfile.TemporaryDirectory(prefix="excel") as d:
        ...     # Write a temporary excel file to read it.
        ...     spark.range(10).write.mode("overwrite").excel(d)
        ...
        ...     # Start a streaming query to read the excel file.
        ...
        if isinstance(path, str):
            return self._df(self._jreader.excel(path))
        else:
            raise PySparkTypeError(
                errorClass="NOT_STR",
                messageParameters={"arg_name": "path", "arg_type": type(path).__name__},
...
class DataStreamWriter:
    ...
    def trigger(
        ...
        *,
        ...
        realTime: Optional[str] = None, # EDGE
    ) -> "DataStreamWriter":
        ...
        <...streaming.readwriter.DataStreamWriter object ...>
        """
        params = [processingTime, once, continuous, availableNow, realTime]  # EDGE

        if params.count(None) == 5: # EDGE
            raise PySparkValueError(
                errorClass="ONLY_ALLOW_SINGLE_TRIGGER",
                messageParameters={},
            )
        elif params.count(None) < 4: # EDGE
            raise PySparkValueError(
                errorClass="ONLY_ALLOW_SINGLE_TRIGGER",
                messageParameters={},
            )
