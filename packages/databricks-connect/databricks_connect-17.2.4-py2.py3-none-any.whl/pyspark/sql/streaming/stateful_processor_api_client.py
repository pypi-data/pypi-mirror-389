...
class StatefulProcessorApiClient:
    def __init__(
        ...
        is_isolated_tws_enabled: bool = False,
        is_realtime_mode: bool = False, # EDGE
        ttl_cleanup_threshold: int = 10000, # EDGE
    ) -> None:
        self.key_schema = key_schema
        # BEGIN-EDGE
        if is_isolated_tws_enabled:
            self._client_socket = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            state_server_dir = "/databricks/sparkconnect/state-server"
            server_address = f"{state_server_dir}/tws_{state_server_id}.sock"

            try:
                self._client_socket.connect(server_address)
            except Exception as e:
                raise Exception(f"StatefulProcessorApiClient Failed to connect to state server. "
                                f"State socket file path is: {server_address}. "
                                f"Received exception: {e}")
        else:
            self._client_socket = socket.socket()
            self._client_socket.connect(("localhost", state_server_id))
        ...
        # BEGIN-EDGE
        # Below fields are only used in real-time mode.
        self.is_realtime_mode = is_realtime_mode
        self.ttl_cleanup_threshold = ttl_cleanup_threshold
        self.last_ttl_cleanup_timestamp = -1
        # END-EDGE

        # statefulProcessorApiClient is initialized per batch per partition,
        # so we will have new timestamps for a new batch
        # NOTE: The clock is advanced within the batch in real-time mode, hence in real-time mode,
        # these values can be updated multiple times.
    ...
    def update_clock(
        ...
        if not self.is_realtime_mode:
            raise PySparkRuntimeError(
                "update_clock should only be called in real-time mode. "
                "Please check if the query is running in real-time mode."
            )

        old_batch_timestamp = self._batch_timestamp
        old_watermark_timestamp = self._watermark_timestamp

        if old_batch_timestamp > batch_timestamp or old_watermark_timestamp > watermark_timestamp:
            raise PySparkRuntimeError(
                "New batch or watermark timestamp should be equal or greater than the "
                "previous one."
            )

        self._batch_timestamp = batch_timestamp
        self._watermark_timestamp = watermark_timestamp
    ...
    def cleanup_ttl(self) -> None:
        # Skip cleaning up TTL if the batch timestamp is advanced less than threshold.
        if self._batch_timestamp < self.last_ttl_cleanup_timestamp + self.ttl_cleanup_threshold:
            return

        import pyspark.sql.streaming.proto.StateMessage_pb2 as stateMessage

        cleanup_call = stateMessage.CleanupTtlRequest(
            expiryTimestampMs=self._batch_timestamp
        )
        timer_call = stateMessage.TimerRequest(
            cleanupTTLRequest=cleanup_call
        )
        message = stateMessage.StateRequest(timerRequest=timer_call)
        self._send_proto_message(message.SerializeToString())
        response_message = self._receive_proto_message()
        status = response_message[0]
        if status != 0:
            # TODO(SPARK-49233): Classify user facing errors.
            raise PySparkRuntimeError(f"Error cleaning up TTL: " f"{response_message[1]}")

        self.last_ttl_cleanup_timestamp = self._batch_timestamp
