...
class ArrowStreamPandasUDFSerializer(ArrowStreamPandasSerializer):
    ...
    def arrow_to_pandas(self, arrow_column, idx):
        ...
        # Variant, Geometry or Geography , which is an atomic type, treat it as a non-struct
        # arrow type.
        if (
            self._df_for_struct
            and types.is_struct(arrow_column.type)
            and not is_variant(arrow_column.type)
            and not is_geometry(arrow_column.type)
            and not is_geography(arrow_column.type)
        ):
            import pandas as pd
    ...
    def _create_batch(self, series):
        ...
        for s, arrow_type, spark_type in series:
            # Variants, Geometry and Geography are represented in arrow as structs with additional
            # metadata (checked by is_variant). If the data type is Variant, return a VariantVal
            # atomic type instead of a dict of two binary values.
            if (
                self._struct_in_pandas == "dict"
                and arrow_type is not None
                and pa.types.is_struct(arrow_type)
                and not is_variant(arrow_type)
                and not is_geometry(arrow_type)
                and not is_geography(arrow_type)
            ):
...
class ArrowStreamArrowUDFSerializer(ArrowStreamSerializer):
    ...
    def dump_stream(self, iterator, stream):
        ...
        return ArrowStreamSerializer.dump_stream(
            ...
            stream,
            # BEGIN-EDGE
            self.timely_flush_enabled,
            self.timely_flush_timeout_ms
            # END-EDGE
        )
...
class TransformWithStateInPySparkRowRealTimeModeSerializer(ArrowStreamUDFSerializer):
    """
    Serializer used by Python worker to evaluate UDF for
    :meth:`pyspark.sql.GroupedData.transformWithState`.
    Parameters
    ...
    def __init__(self, arrow_max_records_per_batch, timely_flush_enabled, timely_flush_timeout_ms):
        super(TransformWithStateInPySparkRowRealTimeModeSerializer, self).__init__(
    ...
    def load_stream(self, stream):
        ...
        def generate_data_batches(batches):
            ...
            NOTE: We send the control message in separate Arrow RecordBatch, since we need ordering
            between data and control message, and Arrow RecordBatch does not give the original
            index of the column if we write "either" data column or control message column in the
            same row.

            The arrow batch is written in the schema:
            schema: StructType = new StructType()
                .add("inputData", dataSchema)
                .add("clock", schemaForClock)

            schemaForClock: StructType = new StructType()
                .add("batchTimestampMs", LongType())
                .add("eventTimeWatermarkForEviction", LongType())

            We'll parse batch into Tuples of (key, inputData, clock) and pass into the Python
            data generator.
            """
            DataRow = None
            for batch in batches:
                data_column: pa.StructArray = batch.column("inputData")
                clock_column: pa.StructArray = batch.column("clock")

                if DataRow is None:
                    DataRow = Row(*data_column.type.names)

                try:
                    batch_ts = clock_column[0][0].as_py()
                except (IndexError, pa.lib.ArrowIndexError) as e:
                    batch_ts = None
                ...
                if batch_ts is not None:
                    ...
                    watermark_ts = clock_column[0][1].as_py()
                    yield (
                        TransformWithStateInPandasFuncMode.UPDATE_CLOCK,
                        (batch_ts, watermark_ts),
                        None
                    )
                else:
                    for row_idx in range(batch.num_rows):
                        # Data row
                        batch_key = tuple(
                            data_column[row_idx][o].as_py() for o in self.key_offsets
                        )
                        row = DataRow(
                            *(
                                data_column[row_idx][i].as_py()
                                for i in range(data_column.type.num_fields)
                            )
                        )
                        yield (TransformWithStateInPandasFuncMode.PROCESS_DATA, batch_key, row)

        _batches = super(ArrowStreamUDFSerializer, self).load_stream(stream)
        data_batches = generate_data_batches(_batches)

        for m, k, v in data_batches:
            yield (m, k, iter([v]))
    ...
    def dump_stream(self, iterator, stream):
        ...
        def flatten_iterator():
            ...
            for packed in iterator:
                ...
                fields = []
                for idx in range(pdf_type.num_fields):
                    fields.append(pdf_type.field(idx))
