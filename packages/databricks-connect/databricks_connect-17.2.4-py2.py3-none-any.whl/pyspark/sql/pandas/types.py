...
if IS_ENGINE_PYSPARK:
    from pyspark._engine_adapters import isinstance, type
# END-EDGE
...
def from_arrow_type(at: "pa.DataType", prefer_timestamp_ntz: bool = False) -> DataType:
    ...
    if types.is_boolean(at):
        ...
        if is_variant(at):
            ...
            if srid == GeometryType.GEOMETRY_MIXED_SRID:
                return GeometryType("ANY")
            else:
                return GeometryType(srid)
        elif is_geography(at):
            srid = int(at.field("wkb").metadata.get(b"srid"))
            if srid == GeographyType.GEOGRAPHY_MIXED_SRID:
    ...
    *,
    ...
    def _converter(
        ...
        if isinstance(dt, ArrayType):
            ...
            return convert_udt
        # BEGIN-EDGE
        elif isinstance(dt, VariantType) and IS_ENGINE_PYSPARK:
            # It would be nicer if these functions called some more central
            # function like, .fromInternal, but here we are.
            from pyspark._engine_adapters import client_pyspark_sql_types
            ...
            def convert_variant(value: Any) -> Any:
                if (
                    ...
                    return client_pyspark_sql_types.VariantVal(value["value"], value["metadata"])
                else:
        ...
        # END-EDGE
            ...
            def convert_geography(value: Any) -> Any:
                if value is None:
                    ...
                    if IS_ENGINE_PYSPARK:
                        # In the engine_pyspark, explicitly use client classes.
                        from pyspark._engine_adapters import client_pyspark_sql_types
                        return client_pyspark_sql_types.\
                            Geography.fromWKB(value["wkb"], value["srid"])
                    else:
            ...
            def convert_geometry(value: Any) -> Any:
                if value is None:
                    ...
                    if IS_ENGINE_PYSPARK:
                        ...
                        from pyspark._engine_adapters import client_pyspark_sql_types
                        return client_pyspark_sql_types.\
                            Geometry.fromWKB(value["wkb"], value["srid"])
                    else:
