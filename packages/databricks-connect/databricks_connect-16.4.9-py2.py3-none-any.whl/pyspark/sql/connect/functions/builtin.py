...
def lit(col: Any) -> Column:
    if isinstance(col, Column):
        ...
        # BEGIN-EDGE
        if isinstance(dt, ByteType) and os.getenv("LEGACY_LIT_NPINT8_ARRAY", "") == "1":
            dt = ShortType()
        # END-EDGE
...
def ai_parse(col: "ColumnOrName", options: Optional[Mapping[str, str]] = None) -> Column:
    if options is None:
        return _invoke_function("ai_parse", _to_col(col))
    else:
        return _invoke_function("ai_parse", _to_col(col), _options_to_col(options))


ai_parse.__doc__ = pysparkfuncs.ai_parse.__doc__

def ai_parse_document(col: "ColumnOrName", options: Optional[Mapping[str, str]] = None) -> Column:
    if options is None:
        return _invoke_function("ai_parse_document", _to_col(col))
    else:
        return _invoke_function("ai_parse_document", _to_col(col), _options_to_col(options))


ai_parse_document.__doc__ = pysparkfuncs.ai_parse_document.__doc__


def ai_extract_table_schema(col: "ColumnOrName", options: Optional[Mapping[str, str]] = None) -> Column:
    if options is None:
        return _invoke_function("ai_extract_table_schema", _to_col(col))
    else:
        return _invoke_function("ai_extract_table_schema", _to_col(col), _options_to_col(options))


ai_extract_table_schema.__doc__ = pysparkfuncs.ai_extract_table_schema.__doc__
...
def ai_extract_table_data(
    ...
    if options is None:
        return _invoke_function("ai_extract_table_data", _to_col(col), _schema)
    else:
        return _invoke_function("ai_extract_table_data", _to_col(col), _schema, _options_to_col(options))


ai_extract_table_data.__doc__ = pysparkfuncs.ai_extract_table_data.__doc__
