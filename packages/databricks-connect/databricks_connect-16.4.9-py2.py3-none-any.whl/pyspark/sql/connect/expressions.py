...
# BEGIN-EDGE
class PythonUDFEnvironment:
    """Represents the environment of a Python user-defined function."""
    ...
    def to_proto(self):
        return proto.PythonEnvironment(dependencies=self._dependencies)
# END-EDGE
...
class PythonUDF:
    ...
    def to_plan(self, session: "SparkConnectClient") -> proto.PythonUDF:
        ...
        # BEGIN-EDGE
        if session._pythonEnv is not None:
            expr.environment.CopyFrom(session._pythonEnv.to_proto())
        # END-EDGE
        return expr
