...
class DefaultPolicy(RetryPolicy):
    ...
    def can_retry(self, e: BaseException) -> bool:
        ...
        if e.code() == grpc.StatusCode.UNAVAILABLE:
            ...
            return msg_connection_failed not in str(e)
            # END-EDGE
...
class DatabricksRetryUnknownsPolicy(RetryPolicy):
    # Please synchronize this policy with RetryPolicy module in Scala.
    # Retries UNKNOWN errors for about 30 seconds.
    def __init__(
        self,
        max_retries: Optional[int] = 7,
        backoff_multiplier: float = 2.0,
        max_backoff: Optional[int] = 5000,
        jitter: int = 100,
        **kwargs,
    ...
    def can_retry(self, e: BaseException) -> bool:
        ...
        msg_connection_failed = "failed to connect to all addresses"
        if e.code() == grpc.StatusCode.UNAVAILABLE and msg_connection_failed in str(e):
            # SASP-2800: This should only happen if the endpoint is completely wrong
            # However it also seems to occasionally happen if the connection gets completely
            # killed due to network problem
            return True

        return False


class DatabricksClusterWaitPolicy(RetryPolicy):
    # Waits up to about 28-30 minutes for clusters to start up.
    # Please synchronize this policy with RetryPolicy module in Scala.
    def __init__(
        self,
        ...
        backoff_multiplier: float = 2.0,
        jitter: int = 100,
        max_backoff: Optional[int] = 30000,
        recognize_server_retry_delay: bool = True,
        max_server_retry_delay: Optional[int] = 10 * 60 * 1000,  # 10 minutes
        **kwargs,
    ):  # type: ignore[no-untyped-def]
    ...
    def can_retry(self, e: BaseException) -> bool:
        ...
        if e.code() in [
            ...
            if any(map(lambda x: x in str(e), [msg_single_user_policy, msg_pending_state])):
                return True
