...
class SparkSession:
    ...
    class Builder:
        ...
        def create(self) -> "SparkSession":
            ...
            if has_channel_builder:
                ...
                session = SparkSession(connection=self._channel_builder, pythonEnv=self._pythonEnv)  # EDGE
            else:
                spark_remote = to_str(self._options.get("spark.remote"))
                assert spark_remote is not None
                session = SparkSession(connection=spark_remote, pythonEnv=self._pythonEnv)  # EDGE
    ...
    def __init__(
        ...
        """
        self._init_client = SparkConnectClient(connection=connection, user_id=userId, pythonEnv=pythonEnv)  # EDGE
