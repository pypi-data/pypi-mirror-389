...
class GeographyType(AtomicType):
    ...
    def _from_crs(cls, crs, algorithm=None) -> "GeographyType":
        ...
        cm = sc._jvm.com.databricks.geo.CrsMappings
        srid = cm.get().getSrid(crs)
        if srid is None:
            raise IllegalArgumentException(
...
class GeometryType(AtomicType):
    ...
    GEOMETRY_DEFAULT_SRID = 0
    GEOMETRY_DEFAULT_CRS = "SRID:0"
    GEOMETRY_MIXED_SRID = -1
    GEOMETRY_MIXED_CRS = "SRID:ANY"

    def __init__(self, srid=None):
        # Unparameterized GEOMETRY type syntax is an alias for the GEOMETRY type with SRID 0.
        if srid is None:
            self.srid = GeometryType.GEOMETRY_DEFAULT_SRID
            self._crs = GeometryType.GEOMETRY_DEFAULT_CRS
        ...
        # Parameterized GEOMETRY type syntax requires SRID value lookup in the CRS mappings.
            ...
            if not isinstance(srid, int) or cm.get().getStringId(srid) is None:
                raise IllegalArgumentException(
                    errorClass="ST_INVALID_SRID_VALUE",
                    messageParameters={
                        "srid": str(srid),
            ...
            self.srid = srid
            self._crs = cm.get().getStringId(srid)
