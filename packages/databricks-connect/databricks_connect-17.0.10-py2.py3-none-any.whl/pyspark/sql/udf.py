...
def _create_py_udf(
    ...
    if is_arrow_enabled:
        return _create_arrow_udf(f, returnType)  # Edge

    return _create_udf(f, returnType, eval_type)


# BEGIN-EDGE
def _create_arrow_udf(f: Callable[..., Any], returnType: "DataTypeOrString"):
    return _create_udf(f, returnType, PythonEvalType.SQL_ARROW_BATCHED_UDF)
