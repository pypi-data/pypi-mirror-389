...
def default_session(*, check_ansi_mode: bool = True) -> SparkSession:
    ...
    if is_testing():
        spark.conf._allow_confs(lambda: spark.conf.set("spark.sql.ansi.enabled", False))
...
def sql_conf(pairs: Dict[str, Any], *, spark: Optional[SparkSession] = None) -> Iterator[None]:
    ...
    # BEGIN-EDGE
    origin = spark.conf.get("spark.testing.databricks.runtimeConfigAllowlist.enabled")
    spark.conf.set("spark.testing.databricks.runtimeConfigAllowlist.enabled", False)
    # END-EDGE
    ...
    try:
        ...
        # BEGIN-EDGE
        spark.conf.set("spark.testing.databricks.runtimeConfigAllowlist.enabled", origin)
        # END-EDGE
