...
# BEGIN-EDGE
def _get_new_job_group(uid):
    return "%s_job_group" % uid


def _cancel_on_failure(session, uid, sub_task_failed, f):
    """
    Utility that cancels the Spark jobs running via given function ``f``.
    """
    import time
    import warnings

    if is_remote():
        new_job_group = _get_new_job_group(uid)
        session.addTag(new_job_group)

        try:
            return f()
        except Exception as e:
            # Hack: this is best effort for other tasks to fail fast. Futures will
            #   read this and fail before running jobs. There's no guarantee here.
            sub_task_failed[0] = True
            try:
                # Hack: sleep for other Spark jobs in Future are launched.
                # Again, it doesn't guarantee that the future runs Spark jobs.
                time.sleep(1)
                session.interruptTag(new_job_group)
            except Exception:
                pass
            raise e
        finally:
            session.removeTag(new_job_group)

    else:
        from py4j.clientserver import ClientServer
        from pyspark.core.context import SparkContext

        if not isinstance(SparkContext._gateway, ClientServer):
            # Here's when the pinned-thread mode (PYSPARK_PIN_THREAD) is off
            try:
                return f()
            except Exception as e:
                warnings.warn(
                    "{} fit call failed but some spark jobs "
                    "may still running for unfinished trials. To address this "
                    "issue, you should enable pyspark pinned thread mode.".format(uid)
                )
                raise e
        ...
        new_job_group = _get_new_job_group(uid)
        sc.setLocalProperty("spark.jobGroup.id", new_job_group)
        try:
            return f()
        except Exception as e:
            # Hack: this is best effort for other tasks to fail fast. Futures will
            ...
            try:
                ...
                # Again, it doesn't guarantee that the future runs Spark jobs.
                time.sleep(1)
                sc.cancelJobGroup(new_job_group)
            except Exception:
                pass
            raise e
        finally:
...
def _cancel_on_ipython_post_run_cell(session, uid, sub_task_failed):
    def decorator(f):
        # If IPython is unavailable, we don't need to decorate f
        try:
        ...
        ipython = get_ipython()
        if ipython is None:
            # If IPython package installed but not running in ipython notebook,
            # the get_ipython return None.
            return f

        def cancel(session, uid):
            if is_remote():
                session.interruptTag(_get_new_job_group(uid))
            else:
                session._sc.cancelJobGroup(_get_new_job_group(uid))
        ...
        def wrapper(*args, **kwargs):
            def on_cancel(result):
                try:
                    ...
                    # We need unregister on_cancel event inside "on_cancel" function.
                    # otherwise before triggerring "on_cancel" the event will
                    # be unregistered.
...
def _remove_dfs_dir(path: str, spark_session: "SparkSession") -> None:
    ...
    if is_remote():
        # BEGIN-EDGE
        if _is_uc_volume_path():
            uc_fuse_path = _get_uc_volume_mount_path(path)
            shutil.rmtree(uc_fuse_path, ignore_errors=True)
        else:
...
def _is_uc_volume_path(path):
    parsed_url = urlparse(path)
    return parsed_url.scheme in ['', 'dbfs'] and parsed_url.path.startswith('/Volumes')


def _get_uc_volume_mount_path(uc_path):
    assert _is_uc_volume_path(uc_path), f"{uc_path} is not a UC volume path."
    return urlparse(uc_path).path
...
def _cache_spark_dataset(
    ...
    if (
        is_remote()
        and os.environ.get("DATABRICKS_RUNTIME_VERSION", "").startswith("client.")
        and (tmp_dfs_path is None or not _is_uc_volume_path(tmp_dfs_path))
    ):
        raise PySparkException(
            "In shared or serverless cluster, 'SPARKML_TEMP_DFS_PATH' environmental variable "
            "must be set to a UC volume path like '/Volumes/...' in order to support Saprk "
            "DataFrame caching."
        )
    # END-EDGE
