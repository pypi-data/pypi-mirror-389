...
def _convert_exception(
    ...
    sql_state = info.metadata.get("sqlState", default=ErrorCode.CLIENT_UNEXPECTED_MISSING_SQL_STATE) # EDGE
...
class ErrorCode:
    ...
    SQL_STATE_PERMISSION_DENIED = "42501"
    SQL_STATE_UNAUTHENTICATED = "08000"
    ERROR_CLASS_PERMISSION_DENIED = "INSUFFICIENT_PERMISSIONS"
    ERROR_CLASS_UNAUTHENTICATED = "UNAUTHENTICATED"


def _convert_exception_without_status(rpc_error: grpc.Call) -> SparkConnectException:
    """Convert exception without a google.rpc.status_pb2.Status by relying on grpc.StatusCode"""
    status_code: grpc.StatusCode = rpc_error.code()
    if status_code == StatusCode.PERMISSION_DENIED:
        sql_state = ErrorCode.SQL_STATE_PERMISSION_DENIED
        error_class = ErrorCode.ERROR_CLASS_PERMISSION_DENIED
    elif status_code == StatusCode.UNAUTHENTICATED:
        sql_state = ErrorCode.SQL_STATE_UNAUTHENTICATED
        error_class = ErrorCode.ERROR_CLASS_UNAUTHENTICATED
    else:
        sql_state = ErrorCode.CLIENT_INTERNAL_ERROR
        error_class = None
