...
from pyspark.sql.pandas.serializers import (
    ...
    TransformWithStateInPySparkRowRealTimeModeSerializer, # EDGE
...
from pyspark.worker_util import (  # noqa: F401
    ...
    monkey_patch_credential_chain_if_needed,
)
...
if IS_ENGINE_PYSPARK:
    from pyspark._engine_adapters import convert_return_type, engine_pyspark_function_converter, isinstance, type, client_pyspark_taskcontext
    BarrierTaskContext = client_pyspark_taskcontext.BarrierTaskContext
    TaskContext = client_pyspark_taskcontext.TaskContext
# END-EDGE
...
def _supports_udf_logging(eval_type: int) -> bool:
    return eval_type not in (
        PythonEvalType.SQL_SCALAR_PANDAS_ITER_UDF,
        PythonEvalType.SQL_MAP_PANDAS_ITER_UDF,
        PythonEvalType.SQL_MAP_ARROW_ITER_UDF,
...
def wrap_loggable_func(f, udf_id, default_context):
    if PySparkUDFLogger._max_entries:
...
def read_single_udf(pickleSer, infile, eval_type, runner_conf, udf_index, profiler):
    ...
    for i in range(read_int(infile)):
        ...
        if IS_ENGINE_PYSPARK:
            f = engine_pyspark_function_converter(eval_type, f)
    ...
    # BEGIN-EDGE
    if IS_ENGINE_PYSPARK:
        return_type = convert_return_type(return_type)
    # END-EDGE
    ...
    if _supports_udf_logging(eval_type):
    ...
    elif eval_type == PythonEvalType.SQL_TRANSFORM_WITH_STATE_PYTHON_ROW_UDF_REALTIME_MODE:
        return args_offsets, wrap_grouped_transform_with_state_udf(func, return_type, runner_conf)
...
def read_udfs(pickleSer, infile, eval_type):
    ...
    if eval_type in (
        ...
        PythonEvalType.SQL_TRANSFORM_WITH_STATE_PYTHON_ROW_UDF_REALTIME_MODE,
    ):
        ...
        if eval_type == PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF_WITH_STATE:
            ...
            or eval_type == PythonEvalType.SQL_TRANSFORM_WITH_STATE_PYTHON_ROW_UDF_REALTIME_MODE
        ):
            is_isolated_tws_enabled = read_bool(infile)
        ...
        if eval_type == PythonEvalType.SQL_COGROUPED_MAP_PANDAS_UDF and should_use_edge:
            ...
            ser = TransformWithStateInPySparkRowRealTimeModeSerializer(
                arrow_max_records_per_batch,
                # BEGIN-EDGE
                timely_flush_enabled=timely_flush_enabled,
                timely_flush_timeout_ms=timely_flush_timeout_ms,
        ...
        # BEGIN-EDGE
        timely_flush_enabled = (
            os.environ.get("PYTHON_UDF_TIMELY_FLUSH_ENABLED", "false").lower() == "true"
        )
        timely_flush_timeout_ms = int(os.environ.get("PYTHON_UDF_TIMELY_FLUSH_TIMEOUT_MS", "100"))
        # END-EDGE
        ser = BatchedSerializer(
            CPickleSerializer(),
            batch_size,
            timely_flush_enabled=timely_flush_enabled, # EDGE
            timely_flush_timeout_ms=timely_flush_timeout_ms, # EDGE
        )
    ...
    if eval_type == PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF:
        ...
        stateful_processor_api_client = stateful_processor_api_client = StatefulProcessorApiClient(
            state_server_id, key_schema, is_driver=False, is_isolated_tws_enabled=is_isolated_tws_enabled)
        ...
        ttl_cleanup_conf_key = (
            "spark.databricks.streaming.realTimeMode.transformWithState.ttlCleanupMsThreshold"
        )
        ttl_cleanup_threshold = int(runner_conf.get(ttl_cleanup_conf_key, "10000"))
        ...
        stateful_processor_api_client = StatefulProcessorApiClient(
            ...
            key_schema,
            is_driver=False,
            is_isolated_tws_enabled=is_isolated_tws_enabled,
            is_realtime_mode=True,
            ttl_cleanup_threshold=ttl_cleanup_threshold,
        )
        ...
        def mapper(a):
            ...
            if mode == TransformWithStateInPandasFuncMode.PROCESS_DATA:
                ...
                # This must be generator comprehension - do not materialize.
                return f(stateful_processor_api_client, mode, key, values)
            elif mode == TransformWithStateInPandasFuncMode.UPDATE_CLOCK:
                clock_ts_tuple = a[1]

                return f(stateful_processor_api_client, mode, clock_ts_tuple, iter([]))
            else:
                # mode == PROCESS_TIMER or mode == COMPLETE
                return f(stateful_processor_api_client, mode, None, iter([]))
...
def main(infile, outfile):
    ...
    faulthandler_use_stderr = os.environ.get("PYTHON_FAULTHANDLER_USE_STDERR", False) # Edge
    ...
    try:
        ...
        # BEGIN-EDGE
        should_check_py_ver = read_bool(infile)
        if should_check_py_ver:
