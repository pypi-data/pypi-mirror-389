...
if IS_ENGINE_PYSPARK:
    from pyspark._engine_adapters import isinstance
# END-EDGE
...
class CharType(AtomicType):
    ...
    def __init__(self, length: int, collation: str = "UTF8_BINARY"):
        self.length = length
        self.collation = collation
...
class GeographyType(AtomicType):
    ...
    Open Geospatial Consortium Simple Feature Access specification
    (https://portal.ogc.org/files/?artifact_id=25355), with a geographic coordinate system.
    Currently these values are generated only by ST functions that output GEOGRAPHY such as
    ST_GeogFromText.

    Parameters
    ----------
    srid : int or str
        The Spatial Reference System Identifier (SRID) value for the GEOGRAPHY.

    .. versionadded:: 3.5.0
    """

    GEOGRAPHY_MIXED_SRID = -1
    GEOGRAPHY_MIXED_CRS = "SRID:ANY"
    GEOGRAPHY_DEFAULT_CRS = "OGC:CRS84"
    GEOGRAPHY_DEFAULT_ALGORITHM = "SPHERICAL"

    def __init__(self, srid):
        # Note that nonparameterized GEOGRAPHY type syntax is currently not supported.
        # Special string value "ANY" is used to represent the mixed SRID GEOGRAPHY type.
        if srid == "ANY":
            self.srid = GeographyType.GEOGRAPHY_MIXED_SRID
            self._crs = GeographyType.GEOGRAPHY_MIXED_CRS
        # TODO(GEO-560): Account for more SRIDs here, once they are supported.
        elif srid != 4326:
            ...
            self.srid = srid
            self._crs = GeographyType.GEOGRAPHY_DEFAULT_CRS

    @classmethod
    def _from_crs(cls, crs, algorithm=None) -> "GeographyType":
        from pyspark.sql.crs_mappings import CrsMappings
        if algorithm is None:
            algorithm = cls.GEOGRAPHY_DEFAULT_ALGORITHM
        srid = CrsMappings.get_srid(crs)
        if srid is None:
            raise IllegalArgumentException(
                errorClass="INVALID_CRS_VALUE",
                messageParameters={
                    "crs": str(crs),
                },
            )
        elif algorithm.lower() != cls.GEOGRAPHY_DEFAULT_ALGORITHM.lower():
            raise IllegalArgumentException(
    ...
    def simpleString(self) -> str:
        if self.srid == GeographyType.GEOGRAPHY_MIXED_SRID:
    ...
    def jsonValue(self) -> Union[str, Dict[str, Any]]:
        return f"geography({self._crs}, {GeographyType.GEOGRAPHY_DEFAULT_ALGORITHM})"
...
class GeometryType(AtomicType):
    ...
    Open Geospatial Consortium Simple Feature Access specification
    (https://portal.ogc.org/files/?artifact_id=25355), with a Cartesian coordinate system. Currently
    these values are generated only by ST functions that output GEOMETRY such as ST_GeomFromText.
    ...
    GEOMETRY_MIXED_SRID = -1
    GEOMETRY_MIXED_CRS = "SRID:ANY"

    def __init__(self, srid):
        # Note that nonparameterized GEOMETRY type syntax is currently not supported.
        # Special string value "ANY" is used to represent the mixed SRID GEOMETRY type.
        if srid == "ANY":
            self.srid = GeometryType.GEOMETRY_MIXED_SRID
            self._crs = GeometryType.GEOMETRY_MIXED_CRS
        # Parameterized GEOMETRY type syntax requires SRID value lookup in the CRS mappings.
        else:
            from pyspark.sql.crs_mappings import CrsMappings
            # Only integer-based, valid SRID GEOMETRY type construction is allowed for users.
            if not isinstance(srid, int) or \
               CrsMappings.get_string_id(srid) is None:
                raise IllegalArgumentException(
                    errorClass="ST_INVALID_SRID_VALUE",
                    messageParameters={
                        "srid": str(srid),
            ...
            # If the integer SRID value represents a valid CRS (in the map), it is allowed.
            self.srid = srid
            self._crs = CrsMappings.get_string_id(srid)

    @classmethod
    def _from_crs(cls, crs) -> "GeometryType":
        from pyspark.sql.crs_mappings import CrsMappings
        if crs.lower() == cls.GEOMETRY_MIXED_CRS.lower():
    ...
    def simpleString(self) -> str:
        if self.srid == GeometryType.GEOMETRY_MIXED_SRID:
            # The mixed SRID type is displayed with a special specifier value "ANY".
            return "geometry(any)"
        else:
            # The fixed SRID type is always displayed with the appropriate SRID value.
...
class CalendarIntervalType(DataType, metaclass=DataTypeSingleton):
    ...
    def needConversion(self) -> bool:
        # If PYSPARK_CAL_INTERVAL_LEGACY is not set, needConversion is true,
        # 'df.collect' fails with PySparkNotImplementedError;
        # otherwise, no conversion is needed, and 'df.collect' returns the internal values.
        return not os.environ.get("PYSPARK_CAL_INTERVAL_LEGACY") == "1"
...
class StructField(DataType):
    ...
    def _isTypeWithCollation(self, dt: DataType) -> bool:
        return isinstance(dt, (StringType, CharType, VarcharType)) and not dt.isUTF8BinaryCollation()
...
class VariantVal:
    ...
    >>> v = df.select(sf.parse_json(df.json).alias("var")).head().var # doctest: +SKIP
    >>> v.toPython() # doctest: +SKIP
    {'a': 1}
    >>> v.toJson() # doctest: +SKIP
    '{"a":1}'
    """
...
class Geography:
    ...
    >>> g = df.select(sf.geogfromwkt(df.geogwkt).alias("geog")).head().geom # doctest: +SKIP
...
class Geometry:
    ...
    >>> g = df.select(sf.geomfromwkt(df.geomwkt).alias("geom")).head().geom # doctest: +SKIP
    >>> g.getBytes() # doctest: +SKIP
    b'\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x001@\x00\x00\x00\x00\x00\x00\x1c@'
    >>> g.getSrid() # doctest: +SKIP
    0
...
_LENGTH_CHAR_WITH_COLLATION = re.compile(r"char\(\s*(\d+)\s*\)\s+collate\s+(\w+)")
_LENGTH_VARCHAR = re.compile(r"varchar\(\s*(\d+)\s*\)")
_LENGTH_VARCHAR_WITH_COLLATION = re.compile(r"varchar\(\s*(\d+)\s*\)\s+collate\s+(\w+)")
...
def _parse_datatype_json_value(
    ...
    if not isinstance(json_value, dict):
        if collationsMap is not None and fieldPath in collationsMap:
            ...
            return type_with_collation(json_value, collation_name)
        if json_value in _all_mappable_types.keys():
        ...
        elif _LENGTH_CHAR_WITH_COLLATION.match(json_value):
            m = _LENGTH_CHAR_WITH_COLLATION.match(json_value)
            return CharType(int(m.group(1)), m.group(2)) # type: ignore[union-attr]
        elif _LENGTH_CHAR.match(json_value):
            m = _LENGTH_CHAR.match(json_value)
            return CharType(int(m.group(1)))  # type: ignore[union-attr]
        elif _LENGTH_VARCHAR_WITH_COLLATION.match(json_value):
            m = _LENGTH_VARCHAR_WITH_COLLATION.match(json_value)
            return VarcharType(int(m.group(1)), m.group(2))  # type: ignore[union-attr]
            ...
            return GeographyType._from_crs(crs.group(1))
        elif _GEOGRAPHY_CRS_ALG.match(json_value):
            crs = _GEOGRAPHY_CRS_ALG.match(json_value)
            return GeographyType._from_crs(crs.group(1), crs.group(2))
        elif _GEOGRAPHY_ALG.match(json_value):
            crs = _GEOGRAPHY_ALG.match(json_value)
            return GeographyType._from_crs(GeographyType.GEOGRAPHY_DEFAULT_CRS, crs.group(1))
        else:
            raise PySparkValueError(
...
def type_with_collation(name, collation):
    if name == "string":
        return StringType(collation)
    elif _LENGTH_CHAR.match(name):
        m = _LENGTH_CHAR.match(name)
        return CharType(int(m.group(1)), collation)
    elif _LENGTH_VARCHAR.match(name):
        m = _LENGTH_VARCHAR.match(name)
        return VarcharType(int(m.group(1)), collation)
    else:
        raise PySparkTypeError(
            errorClass="INVALID_JSON_DATA_TYPE_FOR_COLLATIONS",
            messageParameters={"jsonType": name},
        )

def _assert_valid_type_for_collation(
    fieldPath: str, fieldType: Any, collationMap: Dict[str, str]
) -> None:
    if fieldPath in collationMap and (
            fieldType != "string" and not _LENGTH_CHAR.match(fieldType) and not _LENGTH_VARCHAR.match(fieldType)):
        raise PySparkTypeError(
            errorClass="INVALID_JSON_DATA_TYPE_FOR_COLLATIONS",
