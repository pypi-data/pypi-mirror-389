        ...
        if jconf.arrowPySparkEnabled():
            ...
            if use_arrow:
                try:
                    ...
                    warn_inefficient_columns_for_conversion(self.schema)  # Edge
...
class SparkConversionMixin:
    ...
    def _create_from_pandas_with_arrow(
        ...
        if isinstance(schema, StructType):
            ...
            # BEGIN-EDGE
            arrow_types = to_arrow_schema(
                StructType(
                    [
                        StructField(
                            f.name, _deduplicate_field_names(f.dataType), f.nullable, f.metadata
                        )
            ...
            arrow_types = [
                ...
                if t is not None else None for t in spark_types
            ]
            # END-EDGE
        ...
        arrow_data = [
            # BEGIN-EDGE
            [(c, at, st) for (_, c), at, st in zip(pdf_slice.items(), arrow_types, spark_types)]
            # END-EDGE
        ...
        if self._sc._conf.get(  # type: ignore[attr-defined]
            ...
            jiter = self._jvm.PythonSQLUtils.readArrowStreamFromFile(
                self._write_to_trusted_path(arrow_data, ser)
            )
        else:
            # END-EDGE
