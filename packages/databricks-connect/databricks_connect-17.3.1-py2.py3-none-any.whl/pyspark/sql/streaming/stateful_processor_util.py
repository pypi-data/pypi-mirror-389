...
class TransformWithStateInPandasUdfUtils:
    ...
    def transformWithStateUDF(
        ...
        if stateful_processor_api_client.is_realtime_mode:
            return self._transformWithStateUDF_realtime_mode(
            ...
            return self._transformWithStateUDF_microbatch_mode(
                stateful_processor_api_client,
                mode,
                key,
                input_rows
    ...
    def _transformWithStateUDF_microbatch_mode(
        self,
        stateful_processor_api_client: StatefulProcessorApiClient,
        mode: TransformWithStateInPandasFuncMode,
        key: Any,
        ...
        if mode == TransformWithStateInPandasFuncMode.PROCESS_TIMER:
            ...
            raise PySparkRuntimeError(
                "UPDATE_CLOCK mode is not supported in micro-batch mode for transformWithStateUDF."
            )
        else:
    ...
    def _transformWithStateUDF_realtime_mode(
        self,
        stateful_processor_api_client: StatefulProcessorApiClient,
        mode: TransformWithStateInPandasFuncMode,
        key: Any,
        ...
        if mode == TransformWithStateInPandasFuncMode.PROCESS_TIMER:
            ...
            # key should be (batch_timestamp, watermark_timestamp)
            batch_timestamp, watermark_timestamp = key

            stateful_processor_api_client.update_clock(
                batch_timestamp, watermark_timestamp
            )

            # Need to consume expired timers
            for result in self._handle_expired_timers(stateful_processor_api_client):
                yield result

            if self._time_mode.lower() == "processingtime":
                stateful_processor_api_client.cleanup_ttl()
        else:
            # mode == TransformWithStateInPandasFuncMode.PROCESS_DATA
            for result in self._handle_data_rows(stateful_processor_api_client, key, input_rows):
                yield result
    ...
    def transformWithStateWithInitStateUDF(
        ...
        """
        if stateful_processor_api_client.is_realtime_mode:
            # Only when batchId > 0; for the first batch, we will use microbatch
            # python runner to load the initial state only
            return self._transformWithStateUDF_realtime_mode(
                stateful_processor_api_client,
                mode,
                key,
            ...
            return self._transformWithStateWithInitStateUDF_microbatch_mode(
                stateful_processor_api_client,
                mode,
                key,
                input_rows,
    ...
    def _transformWithStateWithInitStateUDF_microbatch_mode(
        self,
        stateful_processor_api_client: StatefulProcessorApiClient,
        mode: TransformWithStateInPandasFuncMode,
        key: Any,
