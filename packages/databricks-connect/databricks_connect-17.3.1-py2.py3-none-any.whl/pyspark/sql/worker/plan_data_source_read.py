...
if IS_ENGINE_PYSPARK:
    from pyspark._engine_adapters import client_pyspark_datasource, isinstance, type
# END-EDGE
    ...
    try:
        ...
        if is_streaming:
            ...
            is_pushdown_implemented = (
                hasattr(reader, "pushFilters") and # EDGE: skip this check if pushFilters is not implemented
                getattr(reader.pushFilters, "__func__", None) is not DataSourceReader.pushFilters
            )
            # BEGIN-EDGE
            if IS_ENGINE_PYSPARK:
                client_pushFilters = getattr(client_pyspark_datasource.DataSourceReader, "pushFilters", None)
                is_pushdown_implemented = (
                    hasattr(reader, "pushFilters") and
                    getattr(reader.pushFilters, "__func__", None) is not client_pushFilters
                )
            # END-EDGE
