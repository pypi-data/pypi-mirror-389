...
from pyspark.databricks.tvfs.utils import AnalyzeResultWithDependencies
...
if IS_ENGINE_PYSPARK:
    from pyspark._engine_adapters import is_client_image_v1, isinstance, client_pyspark_sql_udtf, client_pyspark_sql_types
    # Provide the customer function with the client pyspark types
    AnalyzeArgument = client_pyspark_sql_udtf.AnalyzeArgument
    AnalyzeResult = client_pyspark_sql_udtf.AnalyzeResult
    _parse_datatype_json_string = client_pyspark_sql_types._parse_datatype_json_string

is_engine_pyspark_client_image_v1 = IS_ENGINE_PYSPARK and is_client_image_v1()
# END-EDGE
...
def read_arguments(infile: IO) -> Tuple[List[AnalyzeArgument], Dict[str, AnalyzeArgument]]:
    ...
    for _ in range(num_args):
        ...
        # BEGIN-EDGE
        # ES-1557964: isConstantExpression is not available in client image v1.
        if is_engine_pyspark_client_image_v1:
            argument = AnalyzeArgument(
                dataType=dt, value=value, isTable=is_table
            )
        else:
            argument = AnalyzeArgument(
...
def main(infile: IO, outfile: IO) -> None:
    ...
    try:
        ...
        # BEGIN-EDGE
        # AnalyzeResult.select was introduced in DBR 15+, not existing in client image v1.
        elif not is_engine_pyspark_client_image_v1 and (
            not isinstance(result.select, (list, tuple)) or not all(
        ...
        if is_engine_pyspark_client_image_v1:
            # No AnalyzeResult.select is supported in client image v1.
            write_int(0, outfile)
        else:
        ...
        # BEGIN-EDGE: TVF Environment Support - send environment info to Scala
        # Dependencies are sent in the format of a pip requirements.txt file, for example:
        # numpy==1.26.4
        # Always send dependency count - Scala side always expects it
        if isinstance(result, AnalyzeResultWithDependencies):
            dependencies = result.dependencies
            if dependencies:
                write_int(len(dependencies), outfile)
                for dep in dependencies:
                    write_with_length(dep.encode("utf-8"), outfile)
            else:
                write_int(0, outfile)
        else:
            # For regular AnalyzeResult, send 0 dependencies
            write_int(0, outfile)
        # END-EDGE

    except BaseException as e:
        # BEGIN-EDGE
        # ES-1231564: hide stacktrace for AI_FORECAST
        hide_traceback = (udtf_name == "ai_forecast") or None
        handle_worker_exception(e, outfile, hide_traceback)
        # END-EDGE
        sys.exit(-1)
    finally:
        if faulthandler_log_path:
