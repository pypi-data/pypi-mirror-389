...
from pyspark.loose_version import LooseVersion # EDGE
...
from pyspark.errors.exceptions.connect import (
    ...
    _convert_exception_without_status,
)
...
class StreamWrapper:
    def __init__(self, call, process_headers_func, process_trailers_func):
        self._call = call
        self._process_headers_func = process_headers_func
        self._process_trailers_func = process_trailers_func
    ...
    def __iter__(self):
        try:
            ...
            # sent at the beginning of the stream, before any response data
            if hasattr(self._call, 'initial_metadata'):
                headers = self._call.initial_metadata()
                if headers:
                    self._process_headers_func(headers)
            # sent at the end of the stream
            if hasattr(self._call, 'trailing_metadata'):
                trailers = self._call.trailing_metadata()
                if trailers:
                    self._process_trailers_func(trailers)

    def cancel(self):
        return self._call.cancel()

class MetadataInterceptor(UnaryUnaryClientInterceptor, UnaryStreamClientInterceptor):
    LAST_SEEN_SERVER_SESSION_VERSION_KEY = "x-databricks-last-seen-server-session-version"
    LAST_SEEN_EFFECTIVE_USAGE_POLICY_ID_KEY = "x-databricks-effective-usage-policy-id"

    def __init__(self, server_session_version: AtomicInteger, effective_usage_policy_id: AtomicString):
        self._server_session_version = server_session_version
        self._effective_usage_policy_id = effective_usage_policy_id
    ...
    def _augment_metadata(self, client_call_details):
        ...
        metadata.extend([(self.LAST_SEEN_SERVER_SESSION_VERSION_KEY, str(self._server_session_version.get()))])
        return client_call_details._replace(metadata=metadata)

    def _process_headers(self, trailers):
        for key, value in trailers:
            # Extract the effective usage policy ID from the trailers if it exists
            if key == self.LAST_SEEN_EFFECTIVE_USAGE_POLICY_ID_KEY:
                current_policy = self._effective_usage_policy_id.get()
                latest_policy = value
                if current_policy is None:
                    logger.warn(f"Effective usage policy for this session is {latest_policy}.")
                elif current_policy != latest_policy:
                    logger.warn(
                        f"Effective usage policy for this session has changed from {current_policy} to {latest_policy}."
                    )
                self._effective_usage_policy_id.set(value)

    def _process_trailers(self, trailers):
        for key, value in trailers:
            # Extract the last seen server version from the trailers if it exists
            if key == self.LAST_SEEN_SERVER_SESSION_VERSION_KEY:
                received_version = int(value)
                self._server_session_version.compare_and_set_if_larger(received_version)
    ...
    def intercept_unary_stream(self, continuation, client_call_details, request):
        ...
        wrapped_call = StreamWrapper(call, self._process_headers, self._process_trailers)
...
class SparkConnectClient(object):
    ...
    def __init__(
        ...
        # BEGIN-EDGE
        self._last_seen_server_session_version = AtomicInteger()
        self._last_seen_effective_usage_policy_id = AtomicString()
        self._metadata_interceptor = MetadataInterceptor(
            self._last_seen_server_session_version,
            self._last_seen_effective_usage_policy_id
        )
        self._channel = grpc.intercept_channel(
            self._builder.toChannel(),
            self._metadata_interceptor
        )
        # END-EDGE
    ...
    def set_last_seen_server_session_version(self, version: int) -> None:
        self._last_seen_server_session_version.set(version)
    ...
    def to_pandas(
        ...
        if table.num_rows == 0:
            ...
            # BEGIN-EDGE
            # UDFs on client image can run using older versions of PyArrow.
            # OSS code removes this check since the code is de facto unreachable, but UDFs running on older
            # client image versions can still hit this check.
        ...
        if status:
            for d in status.details:
                if d.Is(error_details_pb2.ErrorInfo.DESCRIPTOR):
                    ...
                    self._handle_rpc_error_with_error_info(info, status.message, status_code)  # EDGE
            ...
            raise SparkConnectGrpcException(
                ...
                sql_state=ErrorCode.CLIENT_UNEXPECTED_MISSING_SQL_STATE,  # EDGE
                grpc_status_code=status_code,
            ) from None
        else:
            raise _convert_exception_without_status(error) from None  # EDGE

    # BEGIN-EDGE
    def _handle_rpc_error_with_error_info(
        self,
        info: google.rpc.error_details_pb2.ErrorInfo,
        message: str,
        status_code: grpc.StatusCode,
    ) -> None:
        """Handle special cases of Spark Connect errors that contain ErrorInfo and
        require additional handling outside of convert_exception.
        In contrast to convert_exception, this method has access to the SparkConnectClient
        and can modify its state, e.g. close the session if it is expired.

        Parameters
        ----------
        info : google.rpc.error_details_pb2.ErrorInfo
            The ErrorInfo from the gRPC error details.
        message : str
            The status of the RPC call.
        status_code: grpc.StatusCode
            The status code of the RPC call.

        Returns
        -------
        Either throws an exception or returns None and lets convert_exception handle the exception.
        """
        # INVALID_HANDLE.SESSION_CHANGED also present in OSS, the rest are EDGE errors
        error_class = info.metadata.get("errorClass")
        if error_class == "INVALID_HANDLE.SESSION_CHANGED":
            self._closed = True
        if error_class == "INVALID_HANDLE.SESSION_CLOSED":
            warnings.warn(
                "Spark Connect Session expired on the server. Please generate a new "
                "session by detaching and reattaching the compute if in a Databricks notebook or job "
                "or by calling DatabricksSession.builder.getOrCreate() if using Databricks Connect.")
            self._closed = True
        if error_class == "INVALID_PARAMETER_VALUE.INVALID_CLIENT_IMAGE_VERSION":
            raise SparkConnectGrpcException(
                ...
                sql_state=info.metadata.get("sqlState", default=ErrorCode.CLIENT_UNEXPECTED_MISSING_SQL_STATE),
                grpc_status_code=status_code,
            ) from None
        if '[PYTHON_VERSION_MISMATCH]' in message:
            raise SparkConnectGrpcException(
        ...
        if error_class == "CONNECT_INVALID_PLAN.CANNOT_PARSE":
