...
from pyspark.sql.connect.expressions import PythonUDFEnvironment  # EDGE
...
class SparkSession:
    ...
    class Builder:
        ...
        def __init__(self) -> None:
            ...
            self._pythonEnv: Optional[PythonUDFEnvironment] = None  # EDGE
        ...
        # BEGIN-EDGE
        def pythonUDFEnv(self, pythonEnv: PythonUDFEnvironment) -> "SparkSession.Builder":
            """Specify PythonUDFEnvironment and dependencies to be used for Python UDFs in the current session."""
            with self._lock:
                self._pythonEnv = pythonEnv
                return self
        # END-EDGE
        ...
        def enableHiveSupport(self) -> "SparkSession.Builder":
            raise PySparkNotImplementedError(
                ...
                messageParameters={"feature": "enableHiveSupport"},
            )
        ...
        def create(self) -> "SparkSession":
            ...
            if has_channel_builder:
                ...
                session = SparkSession(
                    ...
                    pythonEnv=self._pythonEnv,  # EDGE
    ...
    def __init__(
        ...
        # WARNING/TODO(SC-192095): This is a short-term solution.
        self._streaming_start_call_back = None # EDGE
    ...
    def _register_streaming_callback(self, start_callback: Callable[["StreamingQuery"], None]):
        """
        Set the callback fn to be called for streaming queries before query.start() returns.
        Guarantees exactly-once execution for each streaming query returned from start()
        (StreamingQueryListener.onQueryStart guarantees at-most-once).
        For internal use only.
        """
        self._streaming_start_call_back = start_callback
        logger.info(f"Registered streaming start call back for query: {start_callback}")
    # END-EDGE
    ...
    @property
    def _pythonEnv(self) -> Optional[PythonUDFEnvironment]:
        return self._client._pythonEnv

    @property
    def _effective_usage_policy_id(self) -> Optional[str]:
        return self._client._last_seen_effective_usage_policy_id.get()
    ...
    def createDataFrame(
        ...
        if cache_threshold <= _table.nbytes:
            plan = self._cache_local_relation(local_relation, max_chunk_size_rows, max_chunk_size_bytes)
    ...
    def _cache_local_relation(
        ...
        data_chunks = local_relation._serialize_table_chunks(max_chunk_size_rows, max_chunk_size_bytes)
