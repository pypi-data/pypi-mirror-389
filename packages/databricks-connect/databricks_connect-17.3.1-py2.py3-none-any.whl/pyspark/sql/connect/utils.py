...
def check_dependencies(mod_name: str) -> None:
    if mod_name == "__main__":
        ...
        # Classic DBR 17 doesn't have zstd dependency, the dependency check only applies to DBR 18+.
...
def normalize_workspace_includes(paths: List[str] = sys.path) -> List[str]:
    """
    Given a list of paths normalize them and return only the paths that are
    either under /Workspace or /Volumes.

    Parameters
    ----------
    paths - list of str
         List of paths to normalize and filter

    Returns
    -------
    Final list of paths that are either under /Workspace or /Volumes
    """
    result = []
    for p in paths:
        norm = os.path.abspath(p)
        if norm.startswith("/Workspace") or norm.startswith("/Volumes"):
            result.append(norm)
    return result
