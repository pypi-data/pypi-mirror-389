...
from pyspark.errors import PySparkRuntimeError, PySparkCodeLiteralCompilationException
...
if typing.TYPE_CHECKING:
    ...
    from pyspark.sql.pandas._typing import (
        ...
        GroupedMapUDFTransformWithStateRealTimeModeType, # EDGE
        ArrowScalarUDFType,
...
def handle_worker_exception(
    ...
    # BEGIN-EDGE
    def is_uc_udf_exception_from_function_execution(exc) -> bool:
        """
        Check if exception is from UC UDF by looking for <udfbody> filename.
        A UC UDF exception will be of the form:
        File "serializers.py", line 2412, in main():
            return f(o)
         File "<udfbody>", line 11, in main
           return level_one()
         File "<udfbody>", line 9, in level_one
           return level_two() + 1
         File "<udfbody>", line 6, in level_two
           return level_three() * 2
         File "<udfbody>", line 3, in level_three
           return 1 / 0  # This will cause ZeroDivisionError'
        
        Returns:
            bool: True if this is a UC UDF exception
        """
        # If a compilation error occurs, traceback frames will not have the <filename> in the frame, but in the "filename" attribute
        if getattr(exc, "filename", None) == UDF_BODY_NAME:
            return True
        tb = exc.__traceback__
        while tb:
            if tb.tb_frame.f_code.co_filename == UDF_BODY_NAME:
                return True
            tb = tb.tb_next
            
        return False

    def transform_uc_udf_frame(frame, offset=0):
        """
        Normalize frame info for UC UDFs by adjusting function name, and line numbers.

        This function transforms stack frames from UC UDF execution to provide user-friendly
        error reporting with correct line numbers.

        Args:
            frame: A traceback.FrameSummary object from the original stack trace
            offset: An integer offset to adjust the line number for UDF body frames

        Returns:
            traceback.FrameSummary: A normalized frame with adjusted filename, function name,
            and line numbers that correspond to the user's original code
        """
        # UDFs without handler are defined as part of a larger function:
        # def _udf_handler_func_entry():
        #
        #   actual user code
        name = "main" if frame.name == "_udf_handler_func_entry" else frame.name
        lineno = frame.lineno - offset if frame.filename == UDF_BODY_NAME else frame.lineno
        return traceback.FrameSummary(frame.filename, lineno, name)
    ...
    try:
        # BEGIN-EDGE
        is_uc_udf_exception = False
        is_compilation_error = False
        if isinstance(e, PySparkCodeLiteralCompilationException):
            # This is a UC UDF exception; use the underlying cause for traceback
            is_uc_udf_exception = True
            is_compilation_error = True
            # Only use the cause if it exists, otherwise keep the original exception
            if e.__cause__ is not None:
                e = e.__cause__
        elif is_uc_udf_exception_from_function_execution(e):
            is_uc_udf_exception = True

        if is_uc_udf_exception:
            # Use structured exception format for UC UDFs
            from pyspark.serializers import write_structured_exception

            udf_start_found = False
            user_frames = []
            # All frames before UDF code are from pyspark internals. Afterwards, we can reliably say
            # that exceptions originate from user code.
            if e.__traceback__ is not None:
                for live_tb, f in zip(walk_tb(e.__traceback__), traceback.extract_tb(e.__traceback__)):
                    if not udf_start_found and f.filename == UDF_BODY_NAME:
                        udf_start_found = True
                        offset = int(live_tb.tb_frame.f_globals.get("_udf_traceback_offset", 0))
                    if not udf_start_found:
                        continue

                    user_frames.append(transform_uc_udf_frame(f, offset))

            if hide_traceback or not user_frames:
                stack_lines = []
            else:
                stack = traceback.StackSummary.from_list(user_frames)
                stack_lines = [line.rstrip() for line in stack.format()]

            # Extract error class and message from error line
            error_class = type(e).__name__
            error_message = str(e)

            write_structured_exception(
...
class PythonEvalType:
    ...
    # BEGIN-EDGE
    SQL_TRANSFORM_WITH_STATE_PYTHON_ROW_UDF_REALTIME_MODE: "GroupedMapUDFTransformWithStateRealTimeModeType" = (  # noqa: E501
        500
    )
    # END-EDGE
