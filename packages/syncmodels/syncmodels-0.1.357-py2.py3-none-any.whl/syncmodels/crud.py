...
class iStorage(iCRUD):
    ...
    async def delete_old_data(self, context):
        "delete old data from tables for reducing disk presure and speed"
        if age_days := os.getenv("DELETE_DATA_OLDER_THAN"):
            try:
                uri = context["crawler__"].prefix_template.render(**context)
                _uri = parse_duri(uri)
                # 'transport://adif/trains'
                query = "{fscheme}://{xhost}/{table}".format_map(_uri)

                cut_wave = time.time() * 10**9  # nanoseconds

                delta_wave = int(age_days) * 24 * 3600 * 10**9
                cut_wave -= delta_wave

                params = {
                    MONOTONIC_SINCE_KEY: MONOTONIC_KEY,
                    MONOTONIC_SINCE_VALUE: cut_wave,
                    MONOTONIC_SINCE_OPERATOR: "<",
                }

                await self.delete_where(query, **params)
    ...
    async def delete(self, uri: URI, params):
        "Delete an object from URI"
        try:
            pool = self.connection_pool
            conn, table, _uri = await pool.prepare_call(uri)
    ...
    async def delete_where(self, uri: URI, params):
        "Delete an object from URI"
        try:
            pool = self.connection_pool
            conn, table, _uri = await pool.prepare_call(uri)
