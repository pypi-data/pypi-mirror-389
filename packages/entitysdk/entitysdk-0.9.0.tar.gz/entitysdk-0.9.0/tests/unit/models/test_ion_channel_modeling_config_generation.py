...
@pytest.fixture
def json_data():
    return json.loads(Path(DATA_DIR / "ion_channel_modeling_config_generation.json").read_bytes())


@pytest.fixture
def ion_channel_modeling_config_generation(json_data):
    return IonChannelModelingConfigGeneration.model_validate(json_data)


def test_read_ion_channel_modeling_config_generation(client, httpx_mock, auth_token, json_data):
...
def test_register_ion_channel_modeling_config_generation(
    client, httpx_mock, auth_token, ion_channel_modeling_config_generation, json_data
):
    httpx_mock.add_response(
        method="POST",
        json=ion_channel_modeling_config_generation.model_dump(mode="json")
        | {"id": str(MOCK_UUID)},
    )
