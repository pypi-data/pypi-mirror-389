...
async def get_bilibili_subtitle(
    ...
    try:
        ...
        return {"subtitles": formatted_subtitle.strip()}
...
async def search_bilibili_videos(
    ...
    for v in r["result"]:
        ...
        title = re.sub(r'<em class="keyword">(.*?)</em>', r"\1", v["title"])
...
async def get_bilibili_video_desc(bvid: str) -> dict | str:
    try:
        r = await video.Video(bvid=bvid).get_info()
        desc = r["desc"]
        return {"description": desc.strip()}
    except Exception as e:
        return f"Error getting video description: {e}"
