...
def merge_and_prepare(events, shifts):
    ...
    game = game.assign(
        ...
        event_detail = game.apply(lambda row: 
            '\xa0' if pd.isna(row['description']) else
            row['description'].split(', ')[1].strip() if row['event'] in ['SHOT', 'BLOCK', 'MISS', 'GOAL'] and len(row['description'].split(', ')) > 1 else
            row['description'].split(': ')[1].strip() if row['event'] in ["PSTR", "PEND", "SOC", "GEND"] and len(row['description'].split(': ')) > 1 else
            re.search(r'\((.*?)\)', row['description']).group(1).strip() if row['event'] == 'PENL' and re.search(r'\((.*?)\)', row['description']) else
            row['description'].split(' - ')[0].strip() if row['event'] == 'CHANGE' and len(row['description'].split(' - ')) > 0 else
            '\xa0', axis=1))
