...
def __insert_start_from_previous_event(
    ...
    concat_df = concat_df[
        concat_df[case_id_key] == concat_df[case_id_key + "_2"]
...
def average_duration_activity(
    ...
    return float(
        pandas_utils.get_total_seconds(
            df[timestamp_key] - df[start_timestamp_key]
        ).mean()
    )
...
def average_case_duration(
    ...
    df = timestamp_filter.filter_traces_intersecting(
        df, t1, t2, parameters=parameters
    )
