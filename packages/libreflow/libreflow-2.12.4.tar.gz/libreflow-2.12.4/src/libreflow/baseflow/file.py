...
class UploadPlayblastToKitsu(flow.Action):
    ...
    def _check_kitsu_params(self):
        ...
        if shot_data['nb_frames'] is not None :
            ...
            if frames < shot_data['nb_frames'] or frames > shot_data['nb_frames']:
                ...
                msg += f"<br><br>Parameter check: <i>File > Export To</i> in the tab <i>Clip:Display</i><br>The <b>Convert To</b> parameter must be set to <b>No Conversion</b>."
                self.message.set(msg)
                return False
