...
def get_2d_world_point(point: List[float]) -> TaskType.WorldPoint2D:
    """Get the 2D WorldPoint from a point."""
    return {
        "x": point[0],
        "y": point[1],
    }
...
def dicom_rb_series(
    ...
    for idx, label_map in enumerate(  # pylint: disable=too-many-nested-blocks
        labels_map
    ):
        volume_index: int = (
        ...
        if label_map and label_map.get("labelName"):
            ...
            if isinstance(segmentations, str):
                ...
                for segmentation in segmentations:
                    seg = os.path.basename(segmentation)
                    if seg.endswith("raw"):
                        continue

                    if seg.endswith(".mhd"):
                        segmentation_mapping[volume_index][seg] = [segmentation]
                        for s in segmentations:
                            if s != segmentation and s.startswith(segmentation[:-4]):
                                segmentation_mapping[volume_index][seg].append(s)  # type: ignore
                                break
                    else:
                        segmentation_mapping[volume_index][seg] = segmentation
    ...
    for label in labels:  # pylint: disable=too-many-nested-blocks
        ...
        if label.get("tasklevelclassify") or label.get("studyclassify"):
            ...
            for volume_index in volume_indices:
                ...
                if instance is not None:
                    if series[volume_index].get("pngMask"):
                        ...
                        if mask is None and mask_key is not None:
                            mask = (
                                segmentation_mapping.get(volume_index, {}) or {}
                            ).get(mask_key.removesuffix(".nii.gz") + ".dcm")
                        if mask is None and mask_key is not None:
                            mask = (
                                segmentation_mapping.get(volume_index, {}) or {}
                            ).get(mask_key.removesuffix(".nii.gz") + ".mhd")
            ...
            if label["bbox2d"].get("computedpoint1world"):
                bbox["absolutePoint1"] = get_2d_world_point(
                    label["bbox2d"]["computedpoint1world"]
                )
            if label["bbox2d"].get("computedpoint2world"):
                bbox["absolutePoint2"] = get_2d_world_point(
                    label["bbox2d"]["computedpoint2world"]
                )
            volume["boundingBoxes"].append(bbox)
