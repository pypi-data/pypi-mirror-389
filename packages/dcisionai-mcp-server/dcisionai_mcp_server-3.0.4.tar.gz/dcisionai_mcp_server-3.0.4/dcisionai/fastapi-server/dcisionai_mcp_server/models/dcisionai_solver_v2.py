...
Available domains:
- customer_onboarding: Portfolio optimization, asset allocation, client onboarding, wealth management
...
- portfolio: Portfolio rebalancing, investment optimization
...
Respond with ONLY the domain ID (e.g., "customer_onboarding", "retail_layout", etc.)"""
    ...
    async def solve(
        ...
        if validation_mode in ['auto', 'parallel'] and domain_id in ['portfolio', 'retail_layout']:
            ...
            try:
                ...
                if domain_id == 'portfolio':
                    adapter = PortfolioHiGHSAdapter()
                    highs_result = await adapter.solve(parsed_data)  # FIX: Await async call
                elif domain_id == 'retail_layout':
                    adapter = StoreLayoutHiGHSAdapter()
                    highs_result = await adapter.solve(parsed_data)  # FIX: Await async call
    ...
    async def _parse_universal(self, problem_description: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not generator_method:
                logger.error(f"❌ Missing synthetic_generator_method in config for {domain_id}")
                return {'error': f'Missing synthetic_generator_method for {domain_id}'}
