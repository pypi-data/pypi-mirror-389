...
class Qdrant(VectorStoreBase):
    ...
    def create_col(self, vector_size: int, on_disk: bool, distance: Distance = Distance.COSINE):
        ...
        """
        # Skip creating collection if already exists with matching dimensions
        ...
        for collection in response.collections:
            ...
            try:
                ...
                if isinstance(vectors_config, Mapping):
                    ...
                    existing_dim = first_config.size if first_config else None
                else:
                    existing_dim = vectors_config.size
            except Exception as dimension_error:  # pragma: no cover - defensive
                logger.warning(f"Unable to inspect existing collection dimensions: {dimension_error}")

            if existing_dim is not None and existing_dim != vector_size:
                logger.warning(
                    "Qdrant collection %s has dimension %s but %s is required. Recreating collection.",
            ...
            logger.debug(f"Collection {self.collection_name} already exists with matching vector size.")
