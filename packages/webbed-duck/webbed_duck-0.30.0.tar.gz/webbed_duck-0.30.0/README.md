...
- **Preprocessors:** Register callables (e.g., `myapp.preprocess.resolve_shift_window`) and reference them in TOML metadata to derive or validate parameters before the SQL runs. Each `[[preprocess]]` block now accepts either `callable_module = "pkg.module"` or `callable_path = "path/to/plugin.py"` paired with `callable_name = "function"`. The compiler resolves the callable during `webbed-duck compile`, raising a `RouteCompilationError` if the module or attribute cannot be loaded, so runtime requests never trip over missing plugins. Legacy `module:function` strings continue to work and are normalised into the new shape during compilation.
