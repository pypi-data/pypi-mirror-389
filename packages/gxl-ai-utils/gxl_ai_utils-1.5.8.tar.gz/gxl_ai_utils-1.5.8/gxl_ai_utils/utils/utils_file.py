...
def load_list_file_clean(path: str):
    ...
    try:
        with codecs.open(path, 'r', encoding='utf-8') as f:
            ...
            logging_info(f"load_list_file_clean() - 加载成功, 数据总条数为: {len(cat_to_name)}, 加载文件: {path}, 耗时: {global_timer.elapsed_time()}s")
...
def load_first_row_clean(path: str):
    ...
    try:
        with codecs.open(path, 'r', encoding='utf-8') as f:
            ...
            logging_info(f"load_first_row_clean() - 加载成功, 第一行数据: {cat_to_name}, 加载文件: {path}, 耗时: {global_timer.elapsed_time()}s")
    except Exception as e:
...
def load_dict_from_json(path) -> dict:
    ...
    try:
        with codecs.open(path, 'r', encoding='utf-8') as f:
            ...
            logging_info(f"load_dict_from_json() - 加载成功, 数据总条数为: {len(cat_to_name)}, 加载文件: {path}, 耗时: {global_timer.elapsed_time()}s")
    except Exception as e:
        ...
        with codecs.open(jsonl_file_path, 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                try:
                    ...
                    continue
            logging_info(
                f"load_dict_list_from_jsonl() - 加载成功, 数据总条数为: {len(lines_res)}, 加载文件: {jsonl_file_path}, 耗时: {global_timer.elapsed_time()}s")
    except Exception as e:
...
def load_dict_from_scp(label_scp_file: str, silence: bool = False) -> dict:
    ...
    try:
        ...
        logging_info(f"load_dict_from_scp() - 加载成功, 数据总条数为: {len(res)}, 加载文件: {label_scp_file}, 耗时: {global_timer.elapsed_time()}s")
    except Exception as e:
    ...
    try:
        with codecs.open(path, 'w' if not is_append else 'a', encoding='utf-8') as f:
            for data in data_list:
                f.write(data + '\n')
        logging_info(f"write_list_to_file() - 写入成功, 数据总条数为: {len(data_list)}, 写入文件: {path}, 耗时: {global_timer.elapsed_time()}s")
    except Exception as e:
    ...
    try:
        if not is_append:
            ...
            with jsonlines.open(jsonl_file_path, mode='a') as f:
                f.write_all(dict_list)
        logging_info(
            f"write_dict_list_to_jsonl() - 写入成功, 数据总条数为: {len(dict_list)}, 写入文件: {jsonl_file_path}, 耗时: {global_timer.elapsed_time()}s")
    except Exception as e:
    ...
    try:
        with codecs.open(scp_file_path, 'w', encoding='utf-8') as f:
            for k, v in dic.items():
                f.write(f"{k} {v}\n")
        logging_info(f"write_dict_to_scp() - 写入成功, 数据总条数为: {len(dic)}, 写入文件: {scp_file_path}, 耗时: {global_timer.elapsed_time()}s")
    except Exception as e:
