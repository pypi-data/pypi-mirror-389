...
class CompactMetricsDisplay(Static):
    ...
    def update_metrics(self, metrics: dict, rate: int, window: int, paused: bool, url: str, timeout: float):
        ...
        # Stuck requests (never completed, pending > 3x timeout)
        if metrics["stuck_count"] > 0:
            text.append("Stuck     ", style=f"bold {NORD_COLORS['aurora_red']}")
            text.append(f"{metrics['stuck_count']} requests never responded (>", style="dim")
            text.append(f"{int(timeout * 3)}s", style="dim")
            text.append(")", style="dim")
            text.append("\n")

        # Status code breakdown (if we have errors with status codes)
        status_codes = metrics.get("status_codes", {})
        if status_codes:
            text.append("\n")
            text.append("Status    ", style=f"bold {NORD_COLORS['frost_blue']}")
            for code in sorted(status_codes.keys()):
                count = status_codes[code]
                # Color code by type
                if 200 <= code < 300:
                    code_color = NORD_COLORS["aurora_green"]
                elif 400 <= code < 500:
                    code_color = NORD_COLORS["aurora_orange"]
                elif 500 <= code < 600:
                    code_color = NORD_COLORS["aurora_red"]
                else:
                    code_color = NORD_COLORS["snow_dark"]

                text.append(f"{code}: ", style="dim")
                text.append(f"{count}", style=code_color)
                text.append("  ", style="dim")
...
class HealthzApp(App):
        ...
        while True:
            try:
                ...
                metrics = MetricsCalculator.calculate(
                    self.monitor.completions,
                    self.monitor.pending_requests,
                    self.monitor.total_sent,
                    self.monitor.window_seconds,
                    self.monitor.timeout_seconds,
                )
                ...
                try:
                    self.query_one("#latency", LatencyStatsDisplay).update_latency(metrics, self.monitor.window_seconds)
                except Exception as e:
    ...
    def action_export(self):
        ...
        with open(filename, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["second", "request_count", "p50", "p95", "p99", "avg", "min", "max"])
            ...
            for second in sorted(self.monitor.buckets.keys()):
                ...
                if len(bucket.response_times) > 0:
                    ...
                    p50 = p95 = p99 = avg = min_val = max_val = 0

                writer.writerow(
                    [
                        second,
                        len(bucket.response_times),
                        f"{p50:.2f}",
                        f"{p95:.2f}",
                        f"{p99:.2f}",
                        f"{avg:.2f}",
