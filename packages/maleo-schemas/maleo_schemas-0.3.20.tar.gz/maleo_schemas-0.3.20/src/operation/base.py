...
class BaseOperation(
    ...
    @classmethod
    def _is_allowed_to_publish(cls) -> bool:
        # 1. Check explicit override
        publish_override = os.getenv("PUBLISH_OPERATION")
        if publish_override is not None:
            return publish_override.strip().lower() in ("true", "1", "yes", "y", "on")
        ...
        # 3. Convert to Enum (safe)
        try:
            env_enum = Environment(env_value.strip().lower())
        except ValueError:
            return False  # unknown environment string

        # 4. Allow only for staging or production
        return env_enum in (Environment.STAGING, Environment.PRODUCTION)
