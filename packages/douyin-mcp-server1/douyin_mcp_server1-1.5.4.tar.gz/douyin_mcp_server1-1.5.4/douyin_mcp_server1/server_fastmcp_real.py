...
def extract_douyin_text(share_text: str, model: str = "paraformer-realtime-v1") -> str:
    ...
    try:
        ...
        try:
            ...
            # 使用 file:// URI 格式（Linux需要三个斜杠）
            file_path = os.path.abspath(audio_path)
            file_uri = f"file://{file_path}" if file_path.startswith('/') else f"file:///{file_path}"
            print(f"[DEBUG] 提交给API的文件URI: {file_uri}")

            result = transcription.async_call(
                model=model,
                file_urls=[file_uri],  # 使用file:// URI格式
