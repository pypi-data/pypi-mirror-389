    ...
    try:
        ...
        try:
            ...
            task_response = transcription.async_call(
                model=model,
                file_urls=[file_path],  # 使用普通路径，不是file://
                language_hints=["zh", "zh-CN", "en"]
            )

            print(f"[DEBUG] 任务创建状态: {task_response.status_code}")
            if hasattr(task_response, 'output') and hasattr(task_response.output, 'task_id'):
                print(f"[DEBUG] 任务ID: {task_response.output.task_id}")

                # 第二步：等待任务完成
                print(f"[DEBUG] 等待转录任务完成...")
                transcription_response = transcription.wait(
                    task=task_response.output.task_id
                )

                print(f"[DEBUG] 转录响应状态: {transcription_response.status_code}")

                if transcription_response.status_code == 200:
                    # 获取转录结果
                    if hasattr(transcription_response, 'output') and 'results' in transcription_response.output:
                        # 从transcription_url获取实际结果
                        import urllib.request
                        import json as json_module

                        result_url = transcription_response.output['results'][0]['transcription_url']
                        print(f"[DEBUG] 结果URL: {result_url}")

                        result_data = json_module.loads(urllib.request.urlopen(result_url).read().decode('utf8'))

                        if 'transcripts' in result_data and len(result_data['transcripts']) > 0:
                        ...
                        error_msg = getattr(transcription_response, 'message', '未知错误')
                        return json.dumps({
                            "status": "error",
                            "message": f"转录结果格式错误: {error_msg}",
                            "share_link": share_text,
                            "model": model,
                            "debug_info": {
                                "response_output": str(getattr(transcription_response, 'output', 'N/A'))
                            }
                        }, ensure_ascii=False)
                else:
                    error_msg = getattr(transcription_response, 'message', '未知错误')
                ...
                return json.dumps({
                    "status": "error",
                    "message": f"创建转录任务失败: {error_msg}",
                    "share_link": share_text,
                    "model": model,
                    "debug_info": {
                        "status_code": task_response.status_code,
                        "output": str(getattr(task_response, 'output', 'N/A'))
                    }
                }, ensure_ascii=False)
