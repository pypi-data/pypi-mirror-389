...
class JsonSqlitePriorityQueue(SqliteMixin):
    ...
    def has(self, id):
        row = self.conn.execute(f"SELECT id, message FROM {self.table} WHERE id = ? LIMIT 1", (id, )).fetchone()
        if row is None:
            return False
        else:
            return False
    ...
    def remove_one(self, id):
        res = self.conn.execute(f"DELETE FROM {self.table} WHERE id = ?", (_id,)).rowcount
        self.conn.commit()
        return res
    ...
    def get_list(self, limit: int = -1):
        sql_str = f"SELECT message, priority FROM {self.table} ORDER BY priority DESC"
        if limit and limit != -1:
            sql_str += f" LIMIT {limit}"
        return (
            (self.decode(message), priority)
            for message, priority in self.conn.execute(sql_str)
        )
