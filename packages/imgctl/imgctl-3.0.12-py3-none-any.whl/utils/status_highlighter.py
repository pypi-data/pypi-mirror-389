...
class Highlighter:
    ...
    @classmethod
    def create_tag_text(cls, tag: str) -> Text:
        """
        Создает Text объект с правильной раскраской тега

        Args:
            tag: Тег для форматирования

        Returns:
            Text объект с Rich markup
        """
        text = Text()
        ...
        if "__" in tag:
            ...
            if len(parts) == 2:
                before_part = parts[0]
                after_part = "__" + parts[1]
                text.append(before_part, style="bold white")
                text.append(after_part, style="white")
            else:
                text.append(tag)
        else:
            text.append(tag)
    ...
    @classmethod
    def format_value_by_type(cls, value: Any) -> str:
        """
        Форматирует значение в зависимости от типа с цветовым кодированием
        Подкрашивает числа, ссылки и boolean значения, строки без изменений

        Args:
            value: Значение для форматирования (может быть любого типа)
        ...
        # Для строк проверяем, является ли это ссылкой/URL или boolean
        str_value = str(value)
