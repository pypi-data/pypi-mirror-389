...
class _Canvas:
    ...
    def grids(self, config, tile_size, remove_ticks, ylim, xlim):
        """
        Generate 2D-grids like canvas based on the configuration.
...
class Fig:
    ...
    @classmethod
    def grids(cls, config, tile_size=2, remove_ticks=True, ylim=None, xlim=None):
        """
        ...
        fig_obj._xlim = xlim
        fig_obj._canvas.grids(config, tile_size, remove_ticks, ylim, xlim)
...
class Animation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        self._save()
        display(Image(filename=self._path))
