    ...
    *args: ndarray
    ...
    if args[0].ndim == 1:
        ...
        if n > 1:  # User also passed the xs
            cv.paint.signal(args[0], args[1])
        else:
            cv.paint.signal(args[0])

    ## 2D Array
    if args[0].ndim in [2, 3]:
        M = args[0]
        if n == 1:
            cv = Canvas.cartesian2D(xlim=(0, M.shape[0]), ylim=(M.shape[1], 0))
            cv.paint.image(M)
        elif n == 2:  # User also passed the xs
            cv = Canvas.cartesian2D(xlim=(0, args[1][-1]), ylim=(M.shape[1], 0))
            cv.paint.image(M, args[1])
        elif n == 3:
            cv = Canvas.cartesian2D(xlim=(0, args[1][-1]), ylim=(args[2][-1], 0))
            cv.paint.image(args[0], args[1], args[2])
