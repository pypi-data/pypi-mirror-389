...
def _load_devanagari_font():
    ...
    font_path = (
        Path(__file__).resolve().parents[1] / "fonts" / "NotoSansDevanagari-Regular.ttf"
    )
...
class _Canvas:
    ...
    def tiers(self, config, xlim, fig_width, abc, fig_num):
        ...
        fig_height = (
            (n_aux_tier * height["a"])
            + (n_signal_tier * height["s"])
            + (n_matrix_tier * height["m"])
        )
        ...
        for i, char in enumerate(
            config
        ):  # Loop through each tier and adjust the layout
            ...
            axs[i, 1].axis(
                "off"
            )  # Turn off the column 2, only turn it on when matrix is plotted with colorbar

            axs[i, 0].sharex(
                axs[0, 0]
            )  # Share the x-axis to make all the tiers aligned
    ...
    def grids(self, config, tile_size, remove_ticks, ylim, xlim, abc):
        ...
        fig, axs = plt.subplots(
            n_rows, n_cols, figsize=(tile_size * n_cols, tile_size * n_rows)
        )
        ...
        if abc:
            ...
            for i, ax in enumerate(axs_flat):
                if i < len(labels):
                    ax.set_title(f"({labels[i]})", fontsize=10, loc="center")
    ...
    def stacks(self, config, cell_size, overlap, remove_ticks, focus, offset, facecolor, edgecolor):
        ...
        base_positions = [
            (0.1 + i * overlap * 0.1, 0.1 + i * overlap * 0.1) for i in range(config)
        ]
        ...
        for i in range(config):
            ...
            ax = fig.add_axes(
                [left, bottom, width, height], facecolor=facecolor, zorder=(config - i)
            )
    ...
    def cartesian(self, tile_size, ylim, xlim):
        """
...
class Fig:
    ...
    def _get_curr_tile(self):
        ...
        if self._canvas.type == "tiers":
            ...
            self._curr_tile_idx += 1
        elif self._canvas.type == "cartesian":
            curr_tile = self._canvas.axs
        return curr_tile
    ...
    @classmethod
    def stacks(
        cls, config, tile_size=1, overlap=0.3, remove_ticks=True, focus=None, offset=4.0
    ):
        """
        ...
        fig_obj._canvas.stacks(
            config, tile_size, overlap, remove_ticks, focus, offset, "white", "black"
        )

        return fig_obj
    
    @classmethod
    def cartesian(cls, tile_size=5, ylim=None, xlim=None):
        """
        Generate 2D-cartesian like canvas.
        """
        ...
        fig_obj._canvas.cartesian(tile_size, ylim, xlim)
    ...
    def paint_annotation(
        self, ann, patterns=None, ylim=(0, 1), text_loc="m", grid=True, same_tile=False
    ):
        """
            ...
            if xlim is not None:
                if xlim[0] <= event <= xlim[1]:
                    if i == 0:  # Label should be set only once for all the events
                        tile.axvline(
                            x=event, color=c, linestyle=ls, linewidth=lw, label=label
                        )
                    else:
    ...
    def paint_pointers(
        self, xys, labels, text_offset=(0, 0), c="r", fontsize=12, same_tile=True
    ):
        """
    ...
    def paint_displacement_vector( self, start, end, color="black", head_size=0.05, start_label=None, end_label=None, arrow_label=None, label_offset=0.05, same_tile=True):
        """
        Paint a clean, labeled displacement vector with auto label positioning.
        ...
        # Label start and end points (offset perpendicular to arrow)
        if start_label:
            tile.text(x_start + ox, y_start + oy, start_label, color=color, fontsize=10)
        if end_label:
            tile.text(x_end + ox, y_end + oy, end_label, color=color, fontsize=10)
    ...
    def title(self, title=None, s=10):
        """
        ...
        if title is not None:
            self._canvas.fig.suptitle(title, fontsize=s, y=0.98)
...
class Animation:
    ...
    def _save(self):
        ...
        imageio.mimsave(
            self._path, resized_frames, fps=self._fps, loop=0 if self._loop else 1
        )
        self._saved = True
