...
if __name__ == "__main__":
    ...
    print(f"MD5: {crypto.md5_hash(test_text)}")
    print(f"SHA-256: {crypto.sha256_hash(test_text)}")
    print(f"SHA-512: {crypto.sha512_hash(test_text)}")
    print()
    
    # 测试 AES 加密
    print("=== AES 加密测试 ===")
    plaintext = "这是一个秘密消息"
    ciphertext, key, iv = crypto.aes_encrypt(plaintext)
    decrypted = crypto.aes_decrypt(ciphertext, key, iv)
    print(f"原始: {plaintext}")
    print(f"加密: {crypto.bytes_to_base64(ciphertext)}")
    print(f"解密: {decrypted}")
    print(f"结果: {'成功' if decrypted == plaintext else '失败'}")
    print()
    ...
    decrypted = crypto.rsa_decrypt(ciphertext, private_key)
    print(f"原始: {plaintext}")
    print(f"解密: {decrypted}")
    print(f"结果: {'成功' if decrypted == plaintext else '失败'}")
    print()
    ...
    print("=== DES 加密测试 ===")
    plaintext = "DES 测试"
    ciphertext, key, iv = crypto.des_encrypt(plaintext)
    decrypted = crypto.des_decrypt(ciphertext, key, iv)
    print(f"原始: {plaintext}")
    print(f"解密: {decrypted}")
    print(f"结果: {'成功' if decrypted == plaintext else '失败'}")
    print()
    ...
    # 测试无效签名
    fake_data = "被篡改的数据"
    is_valid_fake = crypto.verify_signature(fake_data, signature, public_key)
    print(f"伪造数据验证: {'有效' if is_valid_fake else '无效'}")
    print()
