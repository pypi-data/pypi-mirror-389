...
# ========================================
# QUERIES PARA COMISSÕES
# ========================================

def get_comissao_query(connection_type: str, sql_customizado: str = None):
    """
    Retorna query para buscar comissões do ERP.
    Usado em: comission_jobs.job_sincroniza_comissao()
    
    Args:
        connection_type: Tipo de banco ('oracle', 'firebird', 'sql')
        sql_customizado: SQL personalizado (se fornecido, usa ele ao invés do padrão)
        
    Returns:
        Query SQL para buscar comissões
        
    Nota:
        Se sql_customizado for fornecido, retorna ele diretamente.
        Caso contrário, usa query padrão baseada no tipo de banco.
        
    IMPORTANTE - Estrutura da Query Customizada:
        A query DEVE usar LEFT JOIN com openk_semaforo.semaforo para trazer:
        1. Registros NOVOS (que não estão no semáforo com tipo_id = 27)
        2. Registros ATUALIZADOS (foram modificados após data_alteracao do semáforo)
        
        Exemplo de estrutura:
        ```sql
        SELECT 
            c.CODTRANSACAO,           -- IDENTIFICADOR (obrigatório)
            c.REFERENCIA,             -- IDENTIFICADOR2 (obrigatório)
            c.numero_nota_fiscal,
            c.codigo_vendedor,
            c.data_emissao,
            ... (demais campos)
        FROM sua_tabela_comissoes c
        LEFT JOIN openk_semaforo.semaforo s 
            ON CAST(c.CODTRANSACAO AS VARCHAR) = s.identificador 
            AND c.REFERENCIA = s.identificador2 
            AND s.tipo_id = 27  -- Tipo COMISSAO
        WHERE (
            -- Novos: não estão no semáforo
            s.identificador IS NULL
            OR
            -- Atualizados: data da comissão > data_alteracao do semáforo
            c.data_ultima_atualizacao > s.data_alteracao
        )
        ORDER BY c.data_emissao DESC
        ```
        
        ATENÇÃO: Os campos CODTRANSACAO e REFERENCIA são obrigatórios na query.
    """
    # Se tem SQL customizado, usa ele
    if sql_customizado:
        return sql_customizado
    
    # Caso contrário, usa query padrão (pode ser ajustada conforme estrutura do banco cliente)
    if connection_type.lower() == 'oracle':
        return """
            SELECT 
                numero_nota_fiscal,
                codigo_vendedor,
                TO_CHAR(data_emissao, 'YYYY-MM-DD HH24:MI:SS') as data_emissao,
                codigo_da_filial,
                nome_da_filial,
                codordtrans,
                cfop_nota_fiscal,
                ...
                preco_manual,
                quantidade_original,
                codigo_multiplicador,
                descricao_multiplicador,
                pureza,
                total_convertido
            FROM tabela_comissoes
            WHERE enviado = 'N'
              AND data_emissao >= SYSDATE - 30
        ...
        return """
            SELECT 
                numero_nota_fiscal,
                codigo_vendedor,
                CONVERT(VARCHAR(19), data_emissao, 120) as data_emissao,
                codigo_da_filial,
                nome_da_filial,
                codordtrans,
                cfop_nota_fiscal,
                preco_unitario,
                CONVERT(VARCHAR(10), data_vencimento, 120) as data_vencimento,
                quantidade,
                serie,
                referencia,
                numero_cupom,
            ...
            FROM tabela_comissoes
            WHERE enviado = 'N'
              AND data_emissao >= DATEADD(DAY, -30, GETDATE())
            ORDER BY data_emissao DESC
        """
