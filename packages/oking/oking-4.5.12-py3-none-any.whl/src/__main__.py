...
def get_job_from_name(job_name: str):
    ...
    for module_data in src.client_data.get('modulos', []):
        if module_data.get('job_name') == job_name:
            if hasattr(oking.Module(**module_data), 'job_generico'):
                module = oking.Module(**module_data)
                if hasattr(module, 'job_generico') and module.job_generico == 'S':
