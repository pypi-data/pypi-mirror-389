...
def list_all_files_under_cos_dir(
    ...
    if verbose:
        print(f"Total {len(file_count)} files have been found! {dir_count} directories have been found!")
...
def delete_cos_file_via_coscli(cos_path: str, config: dict, force: bool = False):
    # cos path does not start with cos://
    cos_path = cos_path.replace("cos://", "")
    
    delete_command_str = f"{config['coscli_path']} rm cos://{cos_path} -f {force}"
    result = subprocess.run(delete_command_str, shell=True, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error deleting {cos_path}: {result.stderr}")
        return result.returncode
    return result.returncode

def delete_cos_dir_via_coscli(cos_dir: str, config: dict, force: bool = False):
    # cos dir does not start with cos://
    cos_dir = cos_dir.replace("cos://", "")
    delete_command_str = f"{config['coscli_path']} rm cos://{cos_dir} -f {force} -r"
    result = subprocess.run(delete_command_str, shell=True, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error deleting {cos_dir}: {result.stderr}")
        return result.returncode
    return result.returncode
...
def download_cos_file_via_coscli(
    ...
    download_command_str = f"{config['coscli_path']} cp \
    cos://{cos_path} {local_file_path} \
    --thread-num {thread_num} \
    --routines {parallel_file_download_num} \
    --part-size {part_size} \
    --fail-output {fail_output}"

    result = subprocess.run(download_command_str, shell=True, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error downloading {cos_path} to {local_file_path}: {result.stderr}")
        return result.returncode
    return result.returncode
...
def download_cos_dir_via_coscli(
    ...
    download_command_str = f"{config['coscli_path']} cp \
    cos://{cos_dir} {local_dir} \
    --thread-num {thread_num} \
    --routines {parallel_file_download_num} \
    --part-size {part_size}"
    result = subprocess.run(download_command_str, shell=True, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error downloading {cos_dir} to {local_dir}: {result.stderr}")
        return result.returncode
    return result.returncode
...
def upload_file2cos_via_coscli(
    ...
    upload_command_str = f"{config['coscli_path']} cp \
        {local_file_path} cos://{cos_path} \
        --part-size {part_size} \
        --thread-num {thread_num} \
        --routines {parallel_file_upload_num}"
    result = subprocess.run(upload_command_str, shell=True, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error uploading {local_file_path} to {cos_save_path}: {result.stderr}")
        return result.returncode
    return result.returncode
...
def upload_dir2cos_via_coscli(
    ...
    cos_dir = cos_dir.replace("cos://", "")
    upload_command_str = f"{config['coscli_path']} cp -r \
        {local_upload_dir} cos://{cos_dir} \
        --part-size {part_size} \
        --thread-num {thread_num} \
        --routines {parallel_file_upload_num}"
    result = subprocess.run(upload_command_str, shell=True, capture_output=True, text=True)
    if result.returncode != 0:
        print(f"Error uploading {local_upload_dir} to {cos_dir}: {result.stderr}")
        return result.returncode
    return result.returncode
