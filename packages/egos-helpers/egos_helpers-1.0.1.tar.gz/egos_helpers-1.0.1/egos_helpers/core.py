    ...
    * `redact`: boolean flag to have the value of this key replaced with a
    ...
    for key, key_attributes in keys.items():
        if not isinstance(key_attributes, dict):
            log.warning(f"Invalid configuration for key '{key}'. Skipping.")
            continue
        ...
        try:
            ...
            log.warning(f"Error retrieving {key.upper()}: {e}")
            continue
        ...
        if env_value is not None:
            try:
                ...
                log.warning(f'Error processing {key.upper()}: {e}. Default value will be used.')
                continue
