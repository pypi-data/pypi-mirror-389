...
class ClaudeCodeProvider(ProviderAdapter):
    ...
    def install(self):  # type: ignore[override]
        ...
        if request is None:
            return self.build_result(
                False,
                "Install metadata missing for Claude Code provider.",
        ...
        channel = os.environ.get("AWARE_TERMINAL_CLAUDE_CHANNEL", SUPPORTED_CHANNEL)
        channel_info = get_channel_info("claude-code", channel=channel)
    ...
    def resume(self, *, session_id=None, context: ProviderContext | None = None):  # type: ignore[override]
        ...
        resolved_id = session_id or self.resolve_session_id(context)
        ...
        command = [binary]
        if resolved_id:
            command.extend(["resume", resolved_id])
            sid = resolved_id
        else:
