...
#
# Taken from: https://github.com/JasonStiefel/changelog-parser
#
"""
A tool for managing changelog formatted data and objects (see keepachangelog.com).

In Python, the data is managed as a list of
...
class ChangelogParsingError(Exception):
    ...
    def __init__(
        self, msg: str, line_number: Optional[int] = None, column_number: Optional[int] = None
    ):
        """Initialize a ChangelogParsingError object."""
        specifications = (
            *((f"line {line_number}",) if isinstance(line_number, int) else ()),
            *((f"column {column_number}",) if isinstance(column_number, int) else ()),
        )
        super().__init__(
            msg + (f' (at {", ".join(specifications)})' if specifications else "")
        )


# =============================================================================
def load(fp: IOBase, encoding: str = "utf-8") -> list[dict[str, Any]]:      # noqa: C901
    """
    ...
    while line := fp.readline():
        ...
        if isinstance(line, str):
            ...
            raise ChangelogParsingError(
                f'Parameter\'s "readline" function call returned unreadable type, '
        ...
        if line.startswith("## ") and not in_compare_urls:
            ...
            if line.rstrip() != line:
                raise ChangelogParsingError(
                    "Extra space(s) after date", line_no, len(line.rstrip()) + 1
                )
            line, sep, change_date = line.partition(" - ")
            if change_date.lstrip() != change_date:
                raise ChangelogParsingError(
                    "Extra space(s) before date", line_no, len(line + sep) + 1
                )
            ...
            for change in changes:
                ...
                raise ChangelogParsingError(
                    msg=(
                        'No corresponding record for compare url with version, '
                        f'"{match.group(1)}"'),
                    line_number=line_no,
                    column_number=2,
                )
...
# =============================================================================
def dump(
    obj: list[dict[str, Any]], fp: IOBase, header: str = DEFAULT_HEADER, encoding: str = "utf-8"
) -> None:
    """
    ...
    encode = lambda i: i if isinstance(fp, TextIOBase) else i.encode(encoding)  # noqa: E731
    ...
    for number, change in enumerate(obj, start=1):
        ...
        if isinstance(version, str) and version.lower() == "unreleased":
            ...
            raise ValueError(
                f'The value associated with the "version" key of changelog entry #{number}, '
                f'"{version}", was not a semver Version or the string "Unreleased" (ignoring case)'
            )
        line = f"## [{version}]"
...
__all__ = ["__version__", "ChangelogParsingError", "load", "loads", "dump", "dumps"]
