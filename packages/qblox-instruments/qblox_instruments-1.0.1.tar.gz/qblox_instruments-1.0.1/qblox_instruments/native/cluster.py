...
from qblox_instruments.constants import MAX_SAFE_VOLTAGE, MIN_SAFE_VOLTAGE
...
class Cluster(ClusterScpi):
    ...
    def get_waveforms(self, slot: int, sequencer: int, *, as_numpy: bool = False) -> dict:
        """
        Get all waveforms in the AWG waveform list of indexed
    ...
    # -------------------------------------------------------------------------
    def set_qsm_outputs_to_zero(self, slot: int) -> None:
        """
        ...
        return self._set_qsm_outputs_to_zero(slot)
    ...
    def _set_qsm_channel_to_zero(self, slot: int, io_channel: int) -> None:
        ...
        """
        check_is_valid_type(self._is_qsm_type(slot))
        return super()._set_qsm_channel_to_zero(slot, io_channel)
    ...
    # --------------------------------------------------------------------------
    def _set_safe_voltage_range(
        self, slot: int, io_channel: int, min_voltage: float, max_voltage: float
    ) -> None:
        """
        Set the safe voltage range for a specific channel of the QSM module at the specified slot.
        ...
        io_channel_obj = self.modules[slot - 1].io_channels[io_channel]
        io_channel_obj._safe_voltage_range = (min_voltage, max_voltage)

    # --------------------------------------------------------------------------
    def set_safe_voltage_range(self, min_voltage: float, max_voltage: float) -> None:
        """
        Set the safe voltage range for all the channels of all the QSM modules
        ...
        max_voltage : float
            The desired maximum voltage in volts.
        """
        for slot_idx in self._get_slots_with_module_connected():
            if self._is_qsm_type(slot_idx):
                self.modules[slot_idx - 1].set_safe_voltage_range(min_voltage, max_voltage)
