...
class SQLAlchemyMaterializer:
    ...
    def _materialize_with_cte(
        ...
        with Session(self.engine) as session:
            # Convert to list immediately to force evaluation of all values
            # This ensures we get actual Python values, not SQLAlchemy proxies
            raw_results = session.execute(text(cte_query)).all()
            results = []
            for row in raw_results:
                # Force evaluation by converting to dict - use _asdict() first as it's more reliable
                if hasattr(row, "_asdict"):
                    # _asdict() forces evaluation of all values
                    results.append(row._asdict())
                elif hasattr(row, "_mapping"):
                    # Fallback: convert _mapping to dict
                    results.append(dict(row._mapping))
                elif isinstance(row, (list, tuple)):
                    # Already a sequence - convert to dict with column names
                    # Get column names from first row if available
                    if raw_results and len(raw_results) > 0:
                        first_row = raw_results[0]
                        if hasattr(first_row, "_mapping"):
                            column_names = list(first_row._mapping.keys())
                        elif hasattr(first_row, "keys"):
                            column_names = list(first_row.keys())
                        else:
                            column_names = [f"col_{i}" for i in range(len(row))]
                        results.append(
                            {col_name: val for col_name, val in zip(column_names, row)}
                        )
                    else:
                        results.append({f"col_{i}": val for i, val in enumerate(row)})
                else:
                    # Already evaluated or unknown type
                    results.append(row)
        ...
        # Build row_dict from result - results are now dicts after forced evaluation
        result_rows = []
        for result in results:
            row_dict = {}

            # Results are now dicts (from _asdict() or dict(_mapping))
            if isinstance(result, dict):
                # Direct dict access - values are already evaluated
                # Force evaluation by accessing values explicitly
                for col_name in result.keys():
                    col_value = result[col_name]
                    # Convert None to 0.0 for coalesce operations with numeric literals
                    if "coalesce" in col_name.lower():
                ...
                # Still a Row object - convert to dict
                for col_name, col_value in result._mapping.items():
                    if "coalesce" in col_name.lower() and col_value is None:
                        row_dict[col_name] = 0.0
                    else:
                        row_dict[col_name] = col_value
            elif hasattr(result, "keys"):
                # SQLAlchemy Row objects support dict-like access
                for key in result.keys():
                    col_value = result[key]
                    if "coalesce" in key.lower() and col_value is None:
                        row_dict[key] = 0.0
                    else:
                        row_dict[key] = col_value
            else:
                ...
                if results and len(results) > 0:
                    ...
                    if isinstance(first_result, dict):
                        column_names = list(first_result.keys())
                    elif hasattr(first_result, "_mapping"):
                        column_names = list(first_result._mapping.keys())
                    elif hasattr(first_result, "keys"):
                        column_names = list(first_result.keys())
                    else:
                        column_names = [f"col_{i}" for i in range(len(first_result))]

                for i, val in enumerate(result):
                    col_name = column_names[i] if i < len(column_names) else f"col_{i}"
                    if "coalesce" in col_name.lower() and val is None:
                        row_dict[col_name] = 0.0
                    else:
    ...
    def _materialize_with_tables(
        ...
        for op_name, op_val in operations:
            ...
            if op_name == "filter":
                ...
                # Drop operation: select all columns except the ones to drop
                self._apply_drop(current_table_name, next_table_name, op_val)
    ...
    def _apply_drop(
        self, source_table: str, target_table: str, cols_to_drop: Tuple[str, ...]
    ) -> None:
        """Apply a drop operation by selecting all columns except the ones to drop."""
        source_table_obj = self._created_tables[source_table]
        ...
        for col in cols_to_drop:
            if isinstance(col, str):
                col_names_to_drop.append(col)
            elif hasattr(col, "name"):
                col_names_to_drop.append(col.name)
            else:
                col_names_to_drop.append(str(col))

        # Get all columns from source table except the ones to drop
        from ...functions import MockColumn

        remaining_cols = [
            MockColumn(col.name)
            for col in source_table_obj.columns
            if col.name not in col_names_to_drop
        ]

        # Use _apply_select to select remaining columns
        self._apply_select(source_table, target_table, tuple(remaining_cols))
    ...
    def _apply_select(
        ...
        # Check if we have function operations like coalesce, nvl, etc. that need raw SQL
        has_function_operations = any(
            hasattr(col, "operation")
            and col.operation in ["coalesce", "nvl", "concat", "regexp_replace"]
            for col in columns
        )

        # Combine the checks
        has_window_functions = (
            has_window_functions or has_complex_arithmetic or has_function_operations
        )
    ...
    def _apply_select_with_window_functions(
        ...
        with Session(self.engine) as session:
            ...
            # Convert to list of dicts to force evaluation of all values
            # This ensures we get actual Python values, not SQLAlchemy proxies
            results = []
            for row in raw_results:
                if hasattr(row, "_asdict"):
            ...
            if results:
                ...
                for i, col in enumerate(columns):
                    if isinstance(col, str):
                        ...
                        # Infer column type based on the operation
                        from ...functions import MockLiteral

                        col_type = String
                        if hasattr(col, "operation"):
                            if col.operation == "coalesce":
                                # Coalesce typically returns the type of the first non-null argument
                                # For numeric coalesce, use Float
                                has_numeric_literal = False
                                if hasattr(col, "value"):
                                    # Check if value is a list (multiple arguments)
                                    if isinstance(col.value, (list, tuple)):
                                        for item in col.value:
                                            if isinstance(
                                                item, MockLiteral
                                            ) and isinstance(item.value, (int, float)):
                                                has_numeric_literal = True
                                                break
                                    elif isinstance(col.value, MockLiteral):
                                        if isinstance(col.value.value, (int, float)):
                                            has_numeric_literal = True
                                ...
                                if hasattr(col, "column"):
                                    ...
                                    if col_name in source_table_obj.c:
                                        ...
                                        if isinstance(
                                            source_col.type,
                                            (Integer, Float, BigInteger),
                                        ):
                                            col_type = Float

                                # If we have a numeric literal, also use Float type
                                if has_numeric_literal:
                                    col_type = Float
                        new_columns.append(
                            Column(col.name, col_type, primary_key=False)
                        )
                    else:
                        new_columns.append(
                ...
                for result in results:
                    ...
                    if isinstance(result, dict):
                        ...
                        # Fallback: assume tuple/list result - convert to dict
                        result_dict = {}
                        for i, column in enumerate(new_columns):
                            if i < len(result):
                                value = result[i]
                                # Ensure Float columns preserve 0.0 (not None)
                                # DuckDB coalesce with CAST returns float, but we need to ensure
                                # integer 0 is converted to 0.0 for Float columns
                                if isinstance(column.type, Float):
                                    # Check if this is a coalesce operation with numeric literal
                                    # If so, None should be converted to 0.0 (coalesce should never return None with a default)
                                    is_coalesce_with_numeric = False
                                    # Try to match column by index first
                                    if i < len(columns) and hasattr(
                                        columns[i], "operation"
                                    ):
                                        if columns[i].operation == "coalesce":
                                            # Check if there's a numeric literal in the coalesce
                                            if hasattr(columns[i], "value"):
                                                if isinstance(
                                                    columns[i].value, (list, tuple)
                                                ):
                                                    for item in columns[i].value:
                                                        if isinstance(
                                                            item, MockLiteral
                                                        ) and isinstance(
                                                            item.value, (int, float)
                                                        ):
                                                            is_coalesce_with_numeric = (
                                                                True
                                                            )
                                                            break
                                                elif isinstance(
                                                    columns[i].value, MockLiteral
                                                ):
                                                    if isinstance(
                                                        columns[i].value.value,
                                                        (int, float),
                                                    ):
                                                        is_coalesce_with_numeric = True
                                    # Fallback: check by column name (heuristic for coalesce operations)
                                    if (
                                        not is_coalesce_with_numeric
                                        and "coalesce" in column.name.lower()
                                    ):
                                        # Check if any column matches this name and has coalesce with numeric
                                        for col in columns:
                                            if (
                                                hasattr(col, "name")
                                                ...
                                                if (
                                                    hasattr(col, "operation")
                                                    and col.operation == "coalesce"
                                                ):
                                                    if hasattr(col, "value"):
                                                        if isinstance(
                                                            col.value, (list, tuple)
                                                        ):
                                                            for item in col.value:
                                                                if isinstance(
                                                                    item, MockLiteral
                                    ...
                                    if value is None:
                                        # For coalesce operations, None should never occur if we have a default value
                                        # Convert to 0.0 for Float columns from coalesce operations
                                        if (
                                            is_coalesce_with_numeric
                                            or "coalesce" in column.name.lower()
                                        ):
                                            # Coalesce with numeric literal should never return None
                                            # Convert to 0.0
                                            value = 0.0
                                        # Otherwise, keep None as None for nullable Float columns
                                    elif value == 0 or value == 0.0:
                                        # Ensure 0 becomes 0.0 for Float columns
                                        value = 0.0
                                    else:
                                        # Ensure other numeric values are floats
                                        try:
                                            value = float(value)
                    ...
                    # Convert None to 0.0 for coalesce operations with numeric literals
                    # This is a safety net to ensure coalesce never returns None when a default is provided
                    # This conversion applies to all result types (dict or tuple/list)
                    for col_name, col_value in result_dict.items():
                        if "coalesce" in col_name.lower() and col_value is None:
    ...
    def _apply_join(
        ...
        # Use the same approach as _infer_join_schema: left fields first, then right fields
        ...
        # Add ALL fields including duplicates (PySpark behavior)
