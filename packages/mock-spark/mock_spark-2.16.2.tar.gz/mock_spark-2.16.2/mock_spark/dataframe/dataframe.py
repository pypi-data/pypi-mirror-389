...
class MockDataFrame:
    ...
    def _queue_op(self, op_name: str, payload: Any) -> "MockDataFrame":
        ...
        # Use LazyEvaluationEngine.queue_operation to ensure schema is updated
        lazy_engine = self._get_lazy_engine()
        return lazy_engine.queue_operation(self, op_name, payload)
    ...
    def collect(self) -> List[MockRow]:
        ...
        if self._operations_queue:
            ...
            # Update self to be the materialized version so that columns/schema reflect the final state
            # Use object.__setattr__ to bypass property setters if needed
            object.__setattr__(self, "_data", materialized.data)
            object.__setattr__(self, "_schema", materialized.schema)
            object.__setattr__(self, "_operations_queue", [])
    ...
    def _project_schema_with_operations(self) -> MockStructType:
        ...
        Delegates to SchemaManager for consistency.

        For joins, we need to use the original base schema (before the join was applied),
        not the current _schema which may already include the join.
        """
        from ..dataframe.schema.schema_manager import SchemaManager

        # Check if we have a join operation and if the schema already reflects it
        # If queue_operation already updated the schema, we need to use the original base
        has_join = any(op_name == "join" for op_name, _ in self._operations_queue)

        if has_join:
            # Find the original base schema by working backwards
            # Start from the current schema and remove the last operation's effect
            # For now, we'll recompute from scratch using the first operation's base
            # Actually, the simplest fix: use the schema from before the join was queued
            # Since queue_operation sets new_schema, we need to get the schema before the join
            # But we don't store that... so we need to recompute correctly

            # Alternative: if _schema already has the join applied, we shouldn't reproject
            # Let's check if the schema matches what we'd get from the operations
            # If it does, just return it
            # Note: We don't need to use the inferred schema here since we're checking
            # if the current schema already matches what we'd get from operations

            # If the inferred schema has more fields than current, it means we're duplicating
            # In that case, we need to find the base schema before the join
            # For now, let's check if current schema field count matches what we'd expect
            # Actually, the simplest: use the schema from the first DataFrame in the chain
            # But we don't have that...

            # Better approach: when queue_operation sets new_schema for a join,
            # it already includes the join. So when we project, we should use
            # the schema BEFORE the join was applied. Since we don't store that,
            # we need to extract it from the current schema.

            # Actually, the real fix: don't use _schema as base if it already includes
            # operations from the queue. We need to track the original base schema.
            # For now, let's use a simpler heuristic: if _schema already has the join
            # fields, don't reproject from it.

            # Check if current schema already has duplicate fields (indicating join was applied)
            field_names = [f.name for f in self._schema.fields]
            has_duplicates = len(field_names) != len(set(field_names))

            if has_duplicates:
                # Schema already has join applied - just return it
                return self._schema

        # Normal case: project from current schema
    ...
    def drop(self, *cols: str) -> "MockDataFrame":
        """Drop columns."""
        # Use lazy evaluation to queue the operation
        # This allows CTE optimization to handle drop operations in the query chain
        return self._queue_op("drop", cols)
