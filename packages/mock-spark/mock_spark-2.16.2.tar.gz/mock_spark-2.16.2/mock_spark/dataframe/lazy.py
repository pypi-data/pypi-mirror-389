...
class LazyEvaluationEngine:
    ...
    def queue_operation(
        ...
        if op_name == "select":
            ...
            new_schema = LazyEvaluationEngine._infer_drop_schema(df, payload)
    ...
    def materialize(df: "MockDataFrame") -> "MockDataFrame":
        ...
        try:
            ...
            try:
                ...
                # Use the schema that was already inferred during queue_operation
                # (The schema has already been updated by _infer_*_schema methods)
                from ..dataframe import MockDataFrame

                # The schema has already been inferred and updated during queue_operation
                # Use the current schema which should already reflect all operations
                return MockDataFrame(
                    materialized_data,
                    df.schema,  # Use already-inferred schema, not reproject
                    df.storage,
                )
            finally:
                materializer.close()
        ...
        for row in data:
            for field in schema.fields:
                if field.name in row:
                    ...
                    # Skip coalesce operations - 0.0 is a valid result, not a default value
                    if "coalesce" in field.name.lower():
                        # For coalesce, only None indicates a problem (shouldn't happen with default)
                        if value is None:
                            return True
                        continue  # Skip default value checks for coalesce
    ...
    def _convert_materialized_rows(
        ...
        for row in rows:
            ...
            # Convert None to 0.0 for coalesce operations with numeric literals FIRST
            # This ensures coalesce never returns None when a default is provided
            for col_name, col_value in list(row_dict.items()):
            ...
            for field in schema.fields:
                ...
                # Double-check coalesce conversion (in case it was reset during type conversion)
                if "coalesce" in field.name.lower() and value is None:
                    row_dict[field.name] = 0.0
                    value = 0.0
    ...
    def _infer_withcolumn_schema(
        ...
        if (
            ...
            # Addition operation - could be numeric or string concatenation
            # Check if it's string concatenation first
            from ..functions.core.literals import MockLiteral
            ...
            if hasattr(col.column, "name"):
                for field in df.schema.fields:
                    if field.name == col.column.name:
                        ...
                        break
            elif isinstance(col.column, MockLiteral):
                # Check if left is a string literal
                if isinstance(col.column.value, str):
                    is_string_operation = True
                elif hasattr(col.column, "data_type"):
                    if isinstance(col.column.data_type, StringType):
                        is_string_operation = True

            # Get right operand type
            if hasattr(col, "value") and col.value is not None:
                if isinstance(col.value, MockLiteral):
                    # Check if right is a string literal
                    if isinstance(col.value.value, str):
                        is_string_operation = True
                    elif hasattr(col.value, "data_type"):
                        if isinstance(col.value.data_type, StringType):
                            is_string_operation = True
            ...
            # Check if either operand is StringType
            if isinstance(left_type, StringType) or isinstance(right_type, StringType):
                is_string_operation = True

            if is_string_operation:
                # String concatenation - return StringType
                new_fields.append(MockStructField(col_name, StringType()))
            else:
                # Numeric addition - infer type from operands
            ...
            # Other arithmetic operations - always numeric
            left_type = None
            right_type = None
            ...
            isinstance(col, MockColumnOperation)
            and hasattr(col, "operation")
            and col.operation == "concat"
        ):
            # String concatenation always returns StringType
            new_fields.append(MockStructField(col_name, StringType()))
        else:
    ...
    @staticmethod
    def _infer_drop_schema(df: "MockDataFrame", drop_params: Any) -> "MockStructType":
        """Infer schema after drop operation.

        Args:
            df: Source DataFrame
            drop_params: Columns to drop (can be str, list, tuple, or Column objects)

        Returns:
            Inferred schema with dropped columns removed
        """
        from ..spark_types import MockStructType

        # Convert drop_params to list of column names
        # drop_params can be a tuple from drop(*cols) or a single value
        if isinstance(drop_params, (list, tuple)):
            cols_to_drop = list(drop_params)
        else:
            cols_to_drop = [drop_params]
        ...
        # Create new schema with dropped columns removed
        remaining_fields = [
            field for field in df.schema.fields if field.name not in col_names_to_drop
        ]

        return MockStructType(remaining_fields)
