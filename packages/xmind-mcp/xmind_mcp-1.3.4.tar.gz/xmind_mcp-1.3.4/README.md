...
│   ├── __init__.py
│   └── __main__.py
└── xmind_mcp_config.json         # Server config (configure default absolute output dir)
```
...
- For MCP output tools, configure a default absolute output directory via `xmind_mcp_config.json` (`default_output_dir`).
