...
RandomizableCT = TypeVar("RandomizableCT", bound="RandomizableCiphertext[Any, Any]")


class RandomizableCiphertext(Generic[CV, RR], Ciphertext[CV], ABC):
    """
    Ciphertext that can be rerandomized. Subclass of Ciphertext.
    """

    scheme: RandomizedEncryptionScheme[Any, Any, Any, Self, RR]  # type: ignore[assignment]

    def __init__(
        self,
        raw_value: CV,
        scheme: RandomizedEncryptionScheme[Any, Any, Any, Self, RR],
        *,
        fresh: bool = False,
    ) -> None:
        """
...
RandomizedES = TypeVar(
    "RandomizedES",
    bound="RandomizedEncryptionScheme[        Any,        Any,        Any,        RandomizableCiphertext[Any, Any],        Any,    ]",
)


class RandomizedEncryptionScheme(
    Generic[KM, PT, RP, RandomizableCT, RR],
    EncryptionScheme[KM, PT, RP, RandomizableCT],
    ABC,
):
    """
    ...
    def _encrypt_raw(self, plaintext: EncodedPlaintext[RP]) -> RandomizableCT:
    ...
    @abstractmethod
    def _unsafe_encrypt_raw(self, plaintext: EncodedPlaintext[RP]) -> RandomizableCT:
        """
