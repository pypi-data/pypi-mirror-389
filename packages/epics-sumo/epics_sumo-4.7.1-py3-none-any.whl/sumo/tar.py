...
class Repo():
    ...
    def checkout(spec, destdir, _, verbose, dry_run):
        ...
        sumo.fileurl.get(url, ap_file, verbose, dry_run)

        ap_tempdir= destdir+".tmp"
        sumo.system.os_makedirs(ap_tempdir, verbose, dry_run)
        cwd= sumo.system.changedir(ap_tempdir, verbose, dry_run)
        try:
            sumo.system.system("tar %s %s" % (tar_args, ap_file),
        ...
        if not ap_subdir:
            ...
            sumo.system.os_rename(ap_tempdir, ap_destdir, verbose, dry_run)
            return
        ...
        sumo.system.os_rename(ap_subdir, ap_renamed_subdir,
                                 verbose, dry_run)
        sumo.system.shutil_move(ap_renamed_subdir, ap_destdir,
                                   verbose, dry_run)
        sumo.system.os_rmdir(ap_tempdir, verbose, dry_run)
        sumo.system.os_remove(ap_file, verbose, dry_run)
