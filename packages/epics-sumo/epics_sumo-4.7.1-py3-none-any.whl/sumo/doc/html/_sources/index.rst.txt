...
- There is sample dependency database with entries for some
  `EPICS <http://www.aps.anl.gov/epics>`_ device supports and
  `EPICS <http://www.aps.anl.gov/epics>`_ base
  at the 
...
See :doc:`ioc` for a script that builds a simple IOC with sumo on your host
(only for Linux/Unix Systems).

See :doc:`demo` for scripts that show how to import an existing EPICS project
into sumo (only for Linux/Unix Systems).
...
`repository at Sourceforge <https://sourceforge.net/p/epics-sumo/mercurial/ci/default/tree/>`_.
