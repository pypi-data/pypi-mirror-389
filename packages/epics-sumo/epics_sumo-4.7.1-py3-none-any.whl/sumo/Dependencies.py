...
class OldDB(sumo.JSON.Container):
...
class DB(sumo.JSON.Container):
    ...
    def merge(self, other):
        ...
        returns:
          - messages: A list of strings. For each merge conflict, a line is
                      added. When there were no merge conflicts, the list is
                      empty.
        """
        # pylint: disable=too-many-statements too-many-branches
        def add_msg(lst, modulename, versionname, text, exception_obj):
            """utility fpr messages."""
            if exception_obj:
                lst.append("module %s version %s %s: %s" % \
                           (modulename, versionname, text,
                            str(exception_obj)))
            else:
                lst.append("module %s version %s %s" % \
                           (modulename, versionname, text))
        messages= []
                ...
                for dictname, dictval in vdict2.items():
                    if dictname=="aliases":
                        try:
                            ...
                            add_msg(messages, modulename, versionname,
                                    "aliases", e)
                        continue
                    if dictname=="dependencies":
                        ...
                        if set(vdict[dictname])!=set(vdict2[dictname]):
                            add_msg(messages, modulename, versionname,
                                    "dependencies: contradiction %s %s" % \
                                    (repr(vdict[dictname]), repr(vdict2[dictname])),
                                    None)
                        continue
                    if dictname=="extra":
                        if dictname not in vdict:
                        ...
                        sumo.utils.lines_unique_update(vdict[dictname],
                                                          vdict2[dictname])
                        continue
    ...
    def set_source_spec(self, module_name, versionname, sourcespec):
        ...
        if old_source is None:
            ...
            return True
        old_spec= sumo.repos.SourceSpec.from_deps_dict(old_source)
    ...
    def set_source_spec_by_tag(self, module_name, versionname, tag):
        ...
        try:
            ...
            # if ret is False, the tag was already set.
        except ValueError as e:
    ...
    def module_source_object(self, modulename, versionname):
        ...
        return sumo.repos.SourceSpec.from_deps_dict(src)
    ...
    def sets_dict(self, modulespecs):
        ...
        return new
    def ver_dict(self, modulespecs):
        """create a dict of versions according to modulespecs.

        modulespecs must be a sumo.ModuleSpec.Specs object.

        convert modulespecs to a versions dict::

          { modulename1 : version,
            modulename2 : version,
          }

        This always selects the newest version available
        """
        if not isinstance(modulespecs, sumo.ModuleSpec.Specs):
            raise TypeError("wrong type: '%s'" % repr(modulespecs))
        new= {}
        for modulespec in modulespecs:
            ...
            if not match_versions:
                raise ValueError("Error, no data found in dependency "
                                 "database for module specification '%s'" % \
                                 modulespec.to_string())
            new[modulename]= sumo.utils.newest_rev(match_versions)
        return new
    ...
    def complete_ver_dict(self, ver_dict):
        """makes a ver_dict complete with respect to dependencies.

        A ver dict has this form:
        ...
        For each dependency that is missing, this program creates a new entry
        in the version dict which contains the newest version for the missing
        module.

        Returns a set of modulenames of added dependencies.
        """
        modules_added= set()
        modlist= list(ver_dict.keys())
        while modlist:
            new_modlist= []
            for modulename in modlist:
                versionname= ver_dict[modulename]
                for dep_name in self.iter_dependencies(modulename,
                                                       versionname):
                    if dep_name not in ver_dict:
                        modules_added.add(dep_name)
                        ver_dict[dep_name]= \
                            sumo.utils.newest_rev(self.iter_versions(dep_name))
                        new_modlist.append(dep_name)
            modlist= new_modlist
        return modules_added
