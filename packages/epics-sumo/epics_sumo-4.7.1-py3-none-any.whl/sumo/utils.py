...
def ask_arbitrary_confirmed(question):
    """ask an arbitrary question with confirmation."""
    question+= " "
    while True:
        try:
            if _trace_input:
        ...
        if ask_yes_no("Is %s okay (Y/N) ?" % repr(inp), force_yes= None):
            return inp
...
def ask_abort(question, force_yes= None):
    ...
    if not ask_yes_no(question + "Enter 'y' to continue or "
        ...
        raise SystemExit("program aborted by user request")
...
def stdin_read():
    """read a list of lines from stdin.

    Removes leading and trailing whitespaces of linefeeds.
    """
    lst= []
...
def pipe_support(lst_):
    """if lst_ contains '-', read from stdin."""
    if not lst_:
        return lst_
    if "-" not in lst_:
        return lst_
...
def string_interpolate(st, variables):
    ...
    'ab cd'
    >>> string_interpolate(r"ab $ab \$ab cd", {"ab":"AB","cd":"CD"})
    'ab AB \\$ab cd'
    >>> print(string_interpolate(r"ab $ab \$ab cd", {"ab":"AB","cd":"CD"}))
    ab AB \$ab cd
    """
    result= None
    if '$' not in st:
...
def definition_list_to_dict(deflist):
    ...
    while True:
        ...
        while True:
            ...
            pos= m.end()
            if val is None:
                val= sumo.JSON.anytxt2scalar(m.group(1))
            else:
                if not isinstance(val, list):
                    val= [val]
                val.append(sumo.JSON.anytxt2scalar(m.group(1)))
...
class TextFile:
    ...
    def writelines(self, lines, sep= ""):
        ...
        if self.stdout:
            ...
            self.fh.write(sep.join(lines)) # type: ignore
...
def backup_file(filename, backup_no, verbose, dry_run):
    """rename or copy "filename" to "filename.bak[N]" to make a backup.

    This maintains a number of <backup_no> backup files.

    If file "filename" doesn't exist, do nothing.

    Parameters:
      - filename: the name of the file
      - backup_no: number of backup files held
      - verbose: show what the function does
      - dry_run: do not actually change files
    Returns:
      Nothing
    """
    def bak(n):
        ...
        example:
          DEPS.DB         # original file
          DEPS.DB.bak     # most recent backup
          DEPS.DB.bak1    # older backup
          DEPS.DB.bak2    # even older backup
        """
        if n==0:
            return "%s.bak" % filename
        return "%s.bak%d" % (filename, n)
    if not os.path.exists(filename):
        return
    if backup_no < 1:
        raise AssertionError("backup_no must be greater than 0")
    # move backups:
    for i in range(backup_no-2, -1, -1):
        b_newer= bak(i)
        b_older= bak(i+1)
        if not os.path.exists(b_newer):
            continue
        if verbose or dry_run:
            print("mv %s %s" % (b_newer, b_older))
        if not dry_run:
            os.replace(bak(i), bak(i+1))
    b_first= bak(0)
    if verbose or dry_run:
        print("cp %s %s" % (filename, b_first))
    if not dry_run:
        # copy with permissions and timestamps:
        shutil.copy2(filename, b_first)
...
def edit_file(filename, editor, verbose, dry_run):
    ...
    for ed in ed_lst:
        try:
            sumo.system.system("%s %s" % (ed, filename),
...
def newest_rev(versions):
    """return the newest version of a list of versions."""
    return sorted(versions, key= rev2key)[-1]
