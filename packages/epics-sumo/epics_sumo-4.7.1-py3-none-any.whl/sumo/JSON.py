...
def dump_file(filename, var):
    ...
    This function first copies the file, then writes it, then renames the
    new version thereby overwriting the original file.
    This should assume that the contents of the file are in all circumstance valid.
    """
    new_file= "%s.tmp" % filename
    # copy with all permissions/timestamps:
    if os.path.exists(filename):
        shutil.copy2(filename, new_file)

    with open(new_file, "wt", encoding="utf-8") as fh:
...
class Container():
    ...
    def dirname(self):
        ...
        return os.path.dirname(self._filename) # type: ignore
    def lock_file(self):
        ...
        if self.lock:
            # already locked
            return
        lk= sumo.lock.MyLock(self._filename, self.timeout)
    ...
    def from_json_file(cls, filename,
        ...
        if not use_lock:
            ...
            # use locking
            l= sumo.lock.MyLock(filename, timeout)
            # may raise lock.LockedError, lock.AccessError or OSError:
            try:
                l.lock()
    ...
    def json_save(self, filename, verbose, dry_run):
        ...
        try:
            if not dry_run:
                ...
                self.lock_file()
            sumo.utils.backup_file(self._filename,
                                   backup_no= 2, # keep 2 backup files
                                   verbose= verbose,
                                   dry_run= dry_run)
            if dry_run:
            ...
            # *Any* exception here may cause the file to be corrupted.
            # Notify the user that he probably has to restore the file
            # from one of it's backups by hand:
            _errmsg= ("Error while writing file %s, you may need "
                      "to restore this from one of it's backup files") % \
                      self._filename
            raise sumo.utils.annotate(_errmsg, e) from e
        finally:
            # unlocks only of self._use_lock==True:
            if not dry_run:
                self.unlock_file()
