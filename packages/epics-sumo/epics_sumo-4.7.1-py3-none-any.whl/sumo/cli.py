"""Command line interface with completion."""
...
def process_opts(args, optionspecs, catch_exceptions):
    """parse options and handle option completion.

    expects args[0] to be the program's name.

    parameters:
      - args :  The program's arguments, usually sys.argv
      - optionspecs: An OptionSpecs object
      - catch_exceptions:
                A bool, if False raise exceptions on errors, if True exit the
                program on errors.

    returns: 
      - options  : an Options object
      - rest_args: a list of arguments
    ...
    def po_pr_complete(lst):
        """print completion list."""
        print("\n".join(sorted(lst)))

    def po_complete(spec, st, options):
        """print completion.
    ...
    while True:
        ...
        if a=="--":
            ...
            if not completion_mode:
                if consumer and not help_mode:
                    _my_exit(("error, argument '%s' missing for "
        ...
        if consumer:
            ...
            if i<len(new_args)-1:
                ...
                if not completion_mode:
                    ...
                    if not consumer.check(a):
                        _my_exit(("invalid option value '%s' for "
            ...
            if completion_mode=="new":
                ...
                try:
                    # may raise any exception:
                    po_complete(consumer, a, options)
        ...
        if a=="-":
            ...
            if not completion_mode:
                ...
                return (options, rest_args)
            if options_off:
                # cannot po_complete "-" if options are off:
                _my_exit()
                return (None, None) # never reached
            # pylint: disable=no-else-return
            if completion_mode=="new":
        ...
        if i>=len(new_args)-1:
            ...
            if not optionspec.arg_name:
                ...
                if completion_mode=="new":
                    try:
                        # may raise any exception:
                        po_complete(optionspec, "", options)
                    except sumo.JSON.ParseError as e:
                        if not catch_exceptions:
                            raise
                        _my_exit(str(e))
...
def process_cmd(cli_args, cmd_list, completion_mode,
    ...
    parameters:
      - cli_args:
                A list of strings, all arguments that are not options.
      - cmd_list:
                A list of set of known commands.
      - completion_mode:
                A bool, if True, we need command completion.
      - item_name:
                Name of expected next item, used for completion messages.

    returns:
      - cmd: A string, the first parsed command.
      - args: A list of strings, the rest of the arguments.
    """
    # pylint: disable=R0912
    #                          Too many branches
    # pylint: disable= too-many-return-statements
    def pc_complete(st, lst):
        """print completion."""
        l= sorted(lst)
        if st:
    ...
    if not cli_args:
        # no args given
        if completion_mode:
            pc_complete("", cmd_list)
            _my_exit()
            return ("",[])
        # may produce "command missing" message:
        _my_exit("%s missing" % item_name)
    if completion_mode:
        if len(cli_args)<=1:
            # pylint: disable=no-else-return
            ...
            if completion_mode=="new":
                ...
                # completion_mode=="word":
                pc_complete(cli_args[0], cmd_list)
                _my_exit()
                return ("",[])
        else:
            ...
            # here:
            return(cli_args[0], cli_args[1:])
    # no completion_mode from here:
    if cli_args[0] not in cmd_list:
        # may produce "unknown command" message:
        _my_exit("unknown %s: %s" % (item_name, cli_args[0]))
        return ("",[])
    return(cli_args[0], cli_args[1:])

def process_single_arg(cli_args, arg_name, completion_mode):
    ...
    parameters:
      - cli_args:
                A list of strings, all arguments that are not options.
      - arg_name:
                Name of expected argument that can be printed on the console
                as a hint.
      - completion_mode:
                A bool, if True, we need command completion.
    returns:
      - cmd: A string, the argument
      - args: A list of strings, the rest of the arguments.
    """
    # pylint: disable= too-many-branches
    # pylint: disable= too-many-return-statements
    def psa_complete(_, name):
        """call completion func if it exists and exit."""
        _display_help(name)
    if not cli_args:
        # no args given
        if completion_mode:
            psa_complete("", arg_name)
            _my_exit()
            return ("",[])
        # may produce "command missing" message:
        _my_exit("%s missing" % arg_name)
    if completion_mode:
        if len(cli_args)<=1:
            # pylint: disable=no-else-return
            # only a single command
            if completion_mode=="new":
                # accept argument, continue:
                return(cli_args[0], cli_args[1:])
            else:
                # completion_mode=="word":
                # accept argument, continue:
...
def process_args(cli_args, argspec, completion_mode,
    ...
    parameters:
      - cli_args:
                A list of strings, all arguments that are not options.
      - argspec :
                A CmdSpecs object, the arguments the command can get
      - completion_mode:
                A bool, if True, we need command completion.
      - catch_exceptions:
                A bool, if False raise exceptions on errors, if True exit the
    ...
    returns:
      result: An Arguments object or None
    ...
    def pa_rcpy(l):
        ...
        n.reverse()
        return n
    def pa_complete(name, spec, st, result):
        """call completion func if it exists and exit.
        ...
        if comp_func:
            ...
            return
        # If no completion function defined, display help:
        _display_help(name)
    if not argspec:
        ...
        if not cli_args:
            return Arguments()
        _my_exit("error, unexpected arguments: %s" % (" ".join(cli_args)))
        return None
    if not isinstance(argspec, CmdSpecs):
        raise TypeError("argspec '%s' has wrong type" % repr(argspec))
    # start iteration on items in argspec:
    argspec.start_get()
    result= Arguments()
    # Create a reversed itemwise copy of cli_args:
    rargs= pa_rcpy(cli_args)
    spec= CmdSpec(name="",array=False)
    arg= ""
    name= ""
    while True:
        if not rargs:
            # when no more arguments, leave loop:
            break
        if not argspec.more() and not spec.array:
            # when no more arguments expected (and no array argument), leave
            # loop:
            break
        # get next argument, remove it from rargs:
        arg= rargs.pop()
        if not spec.array:
            # If the current argument spec is not of "array" type, take next
            # argument spec:
        ...
        if completion_mode=="word" and not rargs:
            try:
                # Call completion func if it exists and exit.
                # may raise any exception:
                pa_complete(name, spec, arg, result)
            except sumo.JSON.ParseError as e:
                if not catch_exceptions:
                    raise
                _my_exit(str(e))
    ...
    if not argspec.more():
        ...
        if rargs:
            # extra arguments
            _my_exit("error, unexpected arguments: %s" % \
                 (" ".join(pa_rcpy(rargs))))
            return None
    else:
        if completion_mode:
        ...
        while argspec.more():
            ...
            if not spec.optional:
                _my_exit("error, mandatory argument %s missing" % name)
                return None
