...
def _cvs_managed_subdirs(path):
    ...
    """
    dirs=[]
    for(dirpath, dirnames, filenames) in sumo.utils.dirwalk(path):
...
def _cvs_dir_status(path, verbose, dry_run):
    ...
    (reply,_)= sumo.system.system(cmd, True, True, None, verbose, dry_run)
    data= {}
    filename= None
...
def _cvs_status(path, verbose, dry_run):
    ...
    """
    cwd= sumo.system.changedir(path, verbose, dry_run=False)
    data= {}
    ...
    try:
        ...
        sumo.system.changedir(cwd, verbose, dry_run=False)
    return data
...
class Repo():
    ...
    def _find_remote(self, patcher):
        ...
        (_,_,rc)= sumo.system.system_rc(cmd, True, True, None,
    ...
    def _local_changes(self, matcher):
        ...
        if is_ssh:
            _cvs_set_ssh()
        cmd= "cvs -n -q -d %s update %s" % (root, self.directory)
        (reply,stderr,rc)= sumo.system.system_rc(cmd, True, True, None,
                                                    self.verbose,
                                                    self.dry_run)
        if is_ssh:
            _cvs_unset_ssh()
    ...
    def checkout(spec, destdir, lock_timeout, verbose, dry_run):
        ...
        try:
            if destdir_head:
                cwd= sumo.system.changedir(destdir_head, verbose, dry_run)
            if is_ssh:
                _cvs_set_ssh()
            ...
            cmd_l.append(repo)
            cmd= " ".join(cmd_l)
            mylock= sumo.lock.MyLock(repo, lock_timeout)
            mylock.lock()
            # we catch stdout and throw it away in order to keep cvs
            # quiet:
            (_, stderr, rc)= sumo.system.system_rc(cmd, True, True, None,
                                                      verbose, dry_run)
            if rc:
                sys.stdout.flush()
                sys.stderr.write(stderr)
            ...
            if is_ssh:
                _cvs_unset_ssh()
            sumo.system.os_rename(repo, destdir_tail, verbose, dry_run)
        finally:
            mylock.unlock()
            if destdir_head:
    ...
    def commit(self, logmessage):
        ...
        try:
            cwd= sumo.system.changedir(self.directory,
                                          self.verbose, self.dry_run)
            (_,stderr,rc)= sumo.system.system_rc(cmd,
                                                    catch_stdout, True, None,
                                                    self.verbose,
                                                    self.dry_run)
        finally:
            if cwd is not None:
                sumo.system.changedir(cwd, self.verbose, self.dry_run)
        if rc:
            sys.stdout.flush()
            sys.stderr.write(stderr)
            sys.stderr.flush()
    ...
    def update(self):
        ...
        if is_ssh:
            _cvs_set_ssh()
        (stdout,stderr,rc)= sumo.system.system_rc(cmd, \
                                           True, True, None, \
                                           self.verbose, self.dry_run)
        if is_ssh:
            _cvs_unset_ssh()
