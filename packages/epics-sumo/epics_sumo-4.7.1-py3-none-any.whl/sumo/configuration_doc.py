...
continues with all configuration files specified by the '-c' or '--
config' option.
...
no-err-build-exists
  When with 'build new' a build matching the modulespecs already
  exists, just print a warning and terminate without an error. For
  details see sumo options.
...
url-patch
  A list of url patch expressions for the 'db convert' and 'db
  modconvert' commands. For details see for details see sumo options.
...
dbrepomode
  Mode "'push'" means that before each read operation on the
  dependency database, sumo performs a "pull" and "merge" command and
  for all write operations it commits all changes and pushes them to
  the central repository.
#opt-preload
  This defines that sumo tries to load "'configure/MODULES.HOST'" and
  "'configure/MODULES'" first, if these files exist. In our
  application our definition of used EPICS
  <http://www.aps.anl.gov/epics> support modules is placed in these
  two files. If we run "'sumo build use'" in the top directory of our
  application, sumo uses module definitions from these two files.

Example of MODULES files
++++++++++++++++++++++++

Module definitions are configuration files where only the keys
"'alias'" and "'module'" are defined. These are specific for each
EPICS <http://www.aps.anl.gov/epics> application. Here are examples of
