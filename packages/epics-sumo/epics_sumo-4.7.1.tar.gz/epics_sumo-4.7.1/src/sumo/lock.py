...
import sumo.system # pylint: disable=wrong-import-position
...
# order to lock a file when against concurrent calls of sumo.
...
class MyLock():
    ...
    def _mkfile(self):
        ...
        if self.method=="link": # type: ignore
        ...
        while True:
            try:
                if self.method=="link": # type: ignore
                    ...
                    open(os.path.join(self.lockname,self.info),'w',
    ...
    def unlock(self, force= False):
        ...
        if self.method=="link":
            ...
            sumo.system.os_rmdir(self.lockname,
...
def edit_with_lock(filename, verbose, dry_run):
    ...
    try:
        ...
        for editor in ed_lst:
            try:
                sumo.system.system("%s %s" % (editor, filename),
