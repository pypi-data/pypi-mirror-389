...
# remove 'x' below to disable mypy warnings:
# xmypy:  ignore_errors=True
...
import sumo.repos
import sumo.git
from sumo.sumo_doc import commands as doc_commands, \
                          completion as doc_completion, \
                          pager as doc_pager, \
                          options as doc_options # type: ignore
from sumo.configuration_doc import text as doc_configuration

# We must disable the mypy index check here since all the variables imported
# from sumo.sumo_doc make problems otherwise.
...
if _pyver < (3,9):
    sys.exit("error: python 3.9 or newer is required for this application")
...
assert __version__==sumo.cli.__version__
assert __version__==sumo.complete.__version__
assert __version__==sumo.Config.__version__
assert __version__==sumo.ModuleSpec.__version__
assert __version__==sumo.Dependencies.__version__
assert __version__==sumo.Builds.__version__
assert __version__==sumo.repos.__version__
...
KNOWN_CONFIG_NEW_TEMPLATE_FLAGS= set(("ADD_PRELOAD", "AT_LIBDIR", "GIT_REPO", "INITIAL"))
KNOWN_CONFIG_NEW_CLONE_FLAGS= set(("ADD_PRELOAD", "AT_LIBDIR", "REPO_WRITE", "INITIAL"))

CONFIG_NEW_SOURCES= \
    { "empty" : {
                  "type": "template",
                  "known_flags": KNOWN_CONFIG_NEW_TEMPLATE_FLAGS,
                },
      "github": {
                  "type": "template",
                  "known_flags": KNOWN_CONFIG_NEW_TEMPLATE_FLAGS,
      ...
      "HZB-internal-RO" : \
                {
                  "type": "clone",
                  "repotype": "darcs",
                  "url": "http://repo.acc.bessy.de/controls/"
                         "darcs/epics/support/sumo-deps-db",
                  "known_flags": KNOWN_CONFIG_NEW_CLONE_FLAGS,
                },
      "HZB-internal-RW" : \
                {
                  "type": "clone",
                  "repotype": "darcs",
                  "url": "repo.acc.bessy.de:/opt/repositories/"
                         "controls/darcs/epics/support/sumo-deps-db",
                  "known_flags": KNOWN_CONFIG_NEW_CLONE_FLAGS,
                  "flags": set(("REPO_WRITE",))
                },
      "sumo-free-database" : \
                {
                  "type": "clone",
                  "repotype": "git",
                  "url": "http://git.code.sf.net/p/sumo-free-database/code",
                  "known_flags": KNOWN_CONFIG_NEW_CLONE_FLAGS,
                },
      "git"   : {
                  "type": "clone",
...
KNOWN_CONFIG_OPTIONS= sumo.utils.set_union(KNOWN_CONFIG_BOOL_OPTIONS,
                                              KNOWN_CONFIG_STR_OPTIONS,
                                              KNOWN_CONFIG_LIST_OPTIONS)

KNOWN_DB_MERGE_FLAGS= set(("WARN", "ASK", "ERROR"))

# only ONE of the following flags can be given:
MERGE_FLAGS_UNIQUE= KNOWN_DB_MERGE_FLAGS
...
KNOWN_REPO_MODES= sumo.repos.known_repo_modes
...
DB_REPO_INITIAL_COMMIT_MESSAGE="Initial commit."
DB_REPO_IGNORE= ["*.bak\n", "*.tmp\n", "*.lock\n"]
...
if env_help:
    ...
    if "nocache" in helpoptions:
        sumo.complete.CACHING_ENABLED= False
...
annotate= sumo.utils.annotate

# internal exception class

class SumoError(Exception):
    """Currently raised from apprelease().
    """
...
class SumoBuildNotFoundError(SumoError):
    """Currently raised from apprelease().
    """
    def __init__(self, msg):
        """constructor."""
...
def load_config_files(options, disable_loading):
    ...
    except (TypeError, KeyError, IOError, sumo.JSON.ParseError) as e:
        # pylint: disable=raising-format-tuple
        raise sumo.utils.annotate("Error while loading config file(s): %s",
                                     e) from e
...
def repo_manager(dbdir, sourcespec_string, mode,
    ...
    May raise:
        ...
        sumo.lock.AccessError, sumo.lock.LockedError,
        sumo.lock.NoSuchFileError from ManagedRepo
    """
    if not sourcespec_string:
        # may raise AssertionError, OSError, TypeError, ValueError,
        #           sumo.lock.AccessError, sumo.lock.LockedError,
        #           sumo.lock.NoSuchFileError:
        return sumo.repos.ManagedRepo(None,None,None,None,verbose,dry_run)
    if not mode:
        mode= 'get'
    # Backwards compatibility: If there is no "=" in sourcespec_string we
    # assume the old sourcespec format:

    source_spec_obj= sumo.repos.SourceSpec.from_string_sourcespec(sourcespec_string)
    try:
        # may raise AssertionError, OSError, TypeError, ValueError,
        #           sumo.lock.AccessError, sumo.lock.LockedError,
        #           sumo.lock.NoSuchFileError:
        mngr= sumo.repos.ManagedRepo(source_spec_obj,
                                        mode,
                                        dbdir, LOCK_TIMEOUT, verbose, dry_run)
    except (AssertionError, OSError, TypeError, ValueError,
            sumo.lock.AccessError,
            sumo.lock.LockedError,
            sumo.lock.NoSuchFileError) as e:
        # pylint: disable=raising-format-tuple
...
def db_from_json_file(filename, use_lock= True, keep_locked= False):
    ...
    May raise:
        ValueError, IOError, sumo.JSON.ParseError:
            from trying to load the file
    """
    try:
        # may raise IOError, ValueError:
        db= sumo.Dependencies.DB.from_json_file(filename,
                                                   use_lock= use_lock,
...
def db_from_repo(options, keep_locked= False, changes_check= True,
    ...
    May raise:
        ...
        IOError, ValueError, sumo.JSON.ParseError from db_from_json_file
    ...
    if mngr.local_changes():
        ...
        if not changes_check:
            ...
            if options.yes:
                ...
                # may raise EOFError:
                try:
                    reply= sumo.utils.ask_from_options(\
                            ("%s Commit changes ?\n"
    ...
    # may raise IOError, ValueError, sumo.JSON.ParseError:
...
def db_to_module_cache(options):
    ...
    May raise:
        EOFError, IOError, ValueError, sumo.JSON.ParseError
            from db_from_repo
    """
    # may raise EOFError, IOError, ValueError, sumo.JSON.ParseError:
...
def builddb_from_json_file(path, path_local, keep_locked= False,
    ...
    if not os.path.exists(file_):
        ...
        builddb= sumo.Builds.DB_overlay(use_lock= True,
        ...
        try:
            # may raise IOError, ValueError:
            builddb= sumo.Builds.DB_overlay.from_json_file(\
                                    file_,
                                    use_lock= True,
                                    keep_lock= keep_locked,
                                    timeout= LOCK_TIMEOUT)
...
def init_buildcache(scandb_name, builddb, db):
    ...
    """
    if not scandb_name:
        buildcache= sumo.Builds.BuildCache(lock_timeout= LOCK_TIMEOUT)
    else:
        try:
            # may raise IOError, ValueError:
            buildcache= sumo.Builds.BuildCache.from_json_file(\
                                scandb_name,
                                use_lock=False,
                                keep_lock= False,
                                timeout= LOCK_TIMEOUT)
...
def gen_RELEASE(db, builddb, buildtag,
    ...
    f= sumo.utils.TextFile(filename, verbose, dry_run)
...
def create_makefile(dist_dict, db, builddb,
    ...
    f= sumo.utils.TextFile(filename, False, dry_run)
    ...
    for modulename, versionname in sorted(special_make_recipes.keys()):
        ...
        for target in sorted(recipes.keys()):
            ...
            if lines:
                lines= ["\t"+sumo.utils.string_interpolate(s, \
                                                              special_vars) \
                        for s in lines]
...
def create_source(db, modulename, versionname,
    ...
    try:
        # may raise IOError:
        sumo.repos.checkout(sourcespec,
                               destdir,
                               LOCK_TIMEOUT,
                               verbose, dry_run)
    except IOError as e:
...
def create_app_data(deps, repoinfo, groups):
    ...
    for module_name, groupdata in groups.items():
        ...
        sourcespec_obj= sumo.repos.SourceSpec(r_dict)
...
def create_database(deps, repoinfo, groups, dir_patches, url_patches):
    ...
    # pylint: disable=R0913
    #                          Too many arguments
    dir_patcher= sumo.utils.RegexpPatcher()
    url_patcher= sumo.utils.RegexpPatcher()
    if url_patches:
    ...
    _path2namevname= {}
    _namevname2path= {}
    db= sumo.Dependencies.DB(lock_timeout= LOCK_TIMEOUT)
    ...
    for module_name, groupdata in groups.items():
        ...
        for root_path, subdirs in groupdata.items():
            for subdir in sorted(subdirs):
                ...
                src_sourcespec= sumo.repos.SourceSpec(r_dict)
                if src_sourcespec.sourcetype()=="path":
                    ...
                    if subdir.startswith("PATH-"):
                        ...
                        versionname= sumo.utils.split_treetag(subdir)[0]
                    else:
                        versionname= "PATH-%s" % subdir
...
def apprelease(build_tag, modulespec_complete, modulespecs,
    ...
    for modulespec in modulespecs:
        ...
        if build_tag is None:
            ...
            if not modulespec.is_exact_spec():
                raise SumoError(f"modulespec '{modulespec.to_string()}' is "
                                f"not an exactly specified version")

    if build_tag is None:
        # must look for a matching build:
        new_builddb= builddb.filter_by_modulespecs(modulespecs)
        if new_builddb.is_empty():
            raise SumoBuildNotFoundError("no build found that matches "
                                         "modulespecs")
        # take only builds that are "testing" or "stable":
        tags= [b for b in new_builddb.iter_builds() \
                 if builddb.is_testing_or_stable(b)]
        if not tags:
            raise SumoBuildNotFoundError("no build with state 'stable' or "
                                         "'testing' found that matches "
                                         "modulespecs")
        if len(tags)>1:
            # more than one build match:
            if not modulespec_complete:
                tag_str= " ".join(sorted(tags))
                raise SumoError(f"Your module specification is incomplete "
                                f"and more than one build matches your "
                                f"specification. Select one of the "
                                f"following builds and specify it with "
                                f"option '-t': \n{tag_str}")
    ...
    for modulespec in modulespecs:
        ...
        if v is None:
            raise SumoError(f"error: module {modulename} not found in "
                            f"build {build_tag}")
        if not modulespec.test(v):
            raise SumoError(f"error: no module matching "
                            f"{modulespec.to_string()} found in build "
                            f"{build_tag}")
...
def subcmd_config_list(arguments, options):
    ...
    """
    sumo.cli.process_args(arguments, None, options.list, catch_exceptions)
...
def subcmd_config_make_show(cmd, arguments, options):
    ...
    """
    # pylint: disable= too-many-branches
    argspec= sumo.cli.CmdSpecs()
    if cmd=="make":
        argspec.add("CONFIGFILE", completion= sumo.cli.complete_file)

    argspec.add("OPTIONNAMES", array= True, optional= True,
                completion= lambda o, r: \
                   sumo.cli.complete_list(KNOWN_CONFIG_OPTIONS, o, r))

    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
    ...
    if options.getmodules:
        ...
        sumo.cli.assert_options(catch_exceptions,  options, "builddir")
        sumo.cli.assert_options(catch_exceptions,  options, "dbdir")

    modulespecs_obj= None
    if options.module:
        # may raise ValueError:
        # mspecs_from_build may raise AssertionError, ValueError:
        modulespecs_obj= sumo.ModuleSpec.Specs.from_strings(\
                             options.module,
...
def subcmd_config_standalone(arguments, options):
    ...
    """
    argspec= sumo.cli.CmdSpecs()
    argspec.add("SUMODIR", completion= sumo.cli.complete_dir)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
...
def subcmd_config_new(arguments, options):
    ...
    MODE_CLONE=2
    mode= MODE_UNDEF
    (sumodir, arguments)= sumo.cli.process_single_arg(arguments,"SUMODIR",
                                                      options.list)
    (source_tag, arguments)= sumo.cli.process_cmd(arguments,
                                 sorted(CONFIG_NEW_SOURCES.keys()),
                                 options.list)
    source_spec= CONFIG_NEW_SOURCES.get(source_tag)
    if source_spec is None:
        l=" ".join(sorted(CONFIG_NEW_SOURCES.keys()))
        raise ValueError("only %s are allowed for SOURCE got %s" % \
                         (l,repr(source_tag)))
    argspec= sumo.cli.CmdSpecs()
    source_type= source_spec["type"]
    source_flags= source_spec["known_flags"]
    if source_type == "template":
        mode= MODE_TEMPLATE
    elif source_type == "clone":
        mode= MODE_CLONE
    else:
    ...
    if mode == MODE_TEMPLATE:
        argspec.add("FLAGS", array= True, optional= True,
                    completion= lambda o, r: \
                       sumo.cli.complete_list(source_flags, o, r))
    elif mode == MODE_CLONE:
        repotype= source_spec.get("repotype")
        if repotype is None:
            # source_tag must be repotype
            repotype= source_tag
        url= source_spec.get("url")
        if url is None:
            # expect URL on the command line:
            argspec.add("URL")
        argspec.add("FLAGS", array= True, optional= True,
                    completion= lambda o, r: \
                       sumo.cli.complete_list(source_flags, o, r))
    ...
    flags= source_spec.get("flags", set())

    if args.FLAGS:           # pylint: disable=no-member
        if mode == MODE_TEMPLATE:
            for f in args.FLAGS: # pylint: disable=no-member
                if f not in source_flags:
                    raise ValueError("Error, flag %s is unknown" % repr(f))
                flags.add(f)
            if "INITIAL" in flags:
                flags.add("ADD_PRELOAD")
                flags.add("AT_LIBDIR")
                flags.add("GIT_REPO")
        elif mode == MODE_CLONE:
            for f in args.FLAGS: # pylint: disable=no-member
    ...
    if "ADD_PRELOAD" in flags:
        config.merge({"#opt-preload":["configure/MODULES"]}, True)

    sumodir= os.path.abspath(sumodir)
    if os.path.exists(sumodir):
        if not os.path.isdir(sumodir):
            sys.exit("error: '%s' is a file" % sumodir)
        else:
            sys.exit("error: '%s' already exists" % sumodir)
    #builddb= sumo.Builds.DB(use_lock= False,
    #                           lock_timeout= LOCK_TIMEOUT)

    # pylint: disable=no-member
    if mode == MODE_TEMPLATE:
        if source_tag == "empty":
            db= sumo.Dependencies.DB(use_lock= False,
                                        lock_timeout= LOCK_TIMEOUT)
        else:
            # Note: KNOWN_CONFIG_NEW_TEMPLATES includes 'empty', but this has been handled
            # before, so we know that args.TEMPLATE never has the value 'empty'
            # here:
            t_dir= os.path.join(sumo.utils.sumolib_dir(),
                                TEMPLATES_SUBDIR, source_tag)
            db= db_from_json_file(db_f(t_dir), use_lock= False)
    elif mode == MODE_CLONE:
        if url is None:
            # URL was given on command line:
            url= args.URL
        config.set("dbrepo", " ".join((repotype, url)))
        if "REPO_WRITE" not in flags:
            config.set("dbrepomode", "pull")
        else:
            config.set("dbrepomode", "push")
    ...
    if mode == MODE_TEMPLATE:
        ...
        if "GIT_REPO" in flags:
            # create local git repository
            sumo.git.new_repo(dbdir, [DB], DB_REPO_INITIAL_COMMIT_MESSAGE,
                              DB_REPO_IGNORE,
                              options.verbose, options.dry_run)
            config.set("dbrepo", "git")
            config.set("dbrepomode", "local")
    elif mode == MODE_CLONE:
        ensure_dir(sumodir, options.dry_run)
        # repo_manager() creates a repos.ManagedRepo object which at
        # initialization checks out the repository:
        repo_manager(dbdir, config.get("dbrepo"), config.get("dbrepomode"),
                     options.verbose, options.dry_run)
    ...
    if "AT_LIBDIR" in flags:
        config_filename= os.path.join(sumo.utils.sumolib_dir(), CONFIG_NAME)

    config.save(config_filename, None, options.verbose, options.dry_run)
...
def subcmd_db_edit(arguments, options):
    ...
    try:
        ...
        while True:
            sumo.utils.edit_file(db_tmp, options.editor,
                                    options.verbose, options.dry_run)
            try:
                # may raise ValueError, IOError
                # or sumo.JSON.ParseError:
            ...
            except sumo.JSON.ParseError as e:
                print("\nError: file has parse errors:")
                print(str(e))
                # may raise EOFError:
                try:
                    reply= sumo.utils.ask_yes_no("Re-edit the file (y) or "
                                                    "abort (n) ? ")
...
def subcmd_db_convert(arguments, options):
    ...
    """
    argspec= sumo.cli.CmdSpecs()
    argspec.add("SCANFILE", completion= sumo.cli.complete_file)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)

    sumo.cli.assert_options(catch_exceptions,  options, "dbdir", "scandb")
    ...
    if os.path.exists(options.scandb):
        sys.exit("error, scandb file '%s' already exists" % \
                 options.scandb)
    scandata= sumo.JSON.loadfile(args.SCANFILE) # pylint: disable=no-member
...
def subcmd_db_format(arguments, options):
    ...
    May raise:
        ValueError, IOError, sumo.JSON.ParseError from db_from_repo
...
def subcmd_db_weight(arguments, options):
    ...
    May raise:
        ...
        IOError, ValueError, sumo.JSON.ParseError from db_from_repo
        IOError from db_to_repo
    ...
    argspec.add("MODULES", array= True,
                completion= sumo.complete.moduleversion)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
...
def subcmd_db_merge(arguments, options):
    ...
    """
    argspec= sumo.cli.CmdSpecs()
    argspec.add("DB", completion= sumo.cli.complete_file)
    argspec.add("FLAGS", array= True, optional= True,
                completion= lambda o, r: \
                    sumo.cli.complete_list(KNOWN_DB_MERGE_FLAGS, o, r))
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
    ...
    if args.FLAGS:           # pylint: disable=no-member
        for f in args.FLAGS: # pylint: disable=no-member
            if f not in KNOWN_DB_MERGE_FLAGS:
                raise ValueError("Error, flag %s is unknown" % repr(f))
            flags.add(f)
    if len(MERGE_FLAGS_UNIQUE.intersection(flags)) > 1:
        raise ValueError("Error, contradicting FLAGS")
    ...
    if messages:
        ...
        if "WARN" in flags:
            ...
            sumo.utils.ask_abort("Merging of database had conflicts.")
        else:
            # "ERROR" mode is not explicitly tested here, this is the default:
            raise ValueError("Merge of database failed")
...
def subcmd_db_alias_add(arguments, options):
    ...
    argspec= sumo.cli.CmdSpecs()
    argspec.add("MODULE", completion= sumo.complete.moduleversion)
    argspec.add("DEPENDENCY",
                completion= lambda d,r: \
                        sumo.complete.dependency("MODULE", d, r))
    argspec.add("ALIAS")
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)

    # pylint: disable=no-member
    module_spec= sumo.ModuleSpec.Spec.from_string(args.MODULE)
    # may raise ValueError:
...
def subcmd_db_dependency_add(arguments, options):
    ...
    argspec= sumo.cli.CmdSpecs()
    argspec.add("MODULE", completion= sumo.complete.moduleversion)
    argspec.add("DEPENDENCY", completion= sumo.complete.module)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
...
def subcmd_db_make_recipes(arguments, options):
    ...
    argspec.add("TARGET", optional= True,
                completion= lambda o, r: \
                   sumo.cli.complete_list(KNOWN_MAKERULE_TARGETS, o, r))
    argspec.add("LINES", array= True, optional= True)
...
def subcmd_db_clone_replace_version(command, arguments, options):
    ...
    May raise:
        EOFError, ValueError, IOError, sumo.JSON.ParseError
            from db_from_repo
    ...
    argspec= sumo.cli.CmdSpecs()
    argspec.add("MODULE", completion= sumo.complete.module)
    argspec.add("OLD-VERSION",
                completion= lambda v, r: \
                        sumo.complete.version("MODULE", v, r))
    ...
    if not sourcespec:
        ...
        # pylint: disable=no-member
        changed= db.set_source_spec_by_tag(module_name= modulename,
                                           versionname= args.NEW_VERSION,
                                           tag= args.NEW_VERSION)
    else:
        # may raise ValueError:
        source_spec_obj= \
            sumo.repos.SourceSpec.from_string_sourcespec(sourcespec)
        # pylint: disable=no-member
    ...
    # may raise EOFError:
    try:
        sumo.utils.ask_abort("Proceed ? ", options.yes)
    except EOFError as e:
        # pylint: disable=raising-format-tuple
        raise annotate("%s. "
                       "You may want to use option '--yes' "
...
def subcmd_db_clonemodule(arguments, options):
    ...
    argspec= sumo.cli.CmdSpecs()
    argspec.add("OLD-MODULE", completion= sumo.complete.module)
    argspec.add("NEW-MODULE")
    argspec.add("VERSIONS",
                completion= lambda v,r: \
                      sumo.complete.dependency("NEW-MODULE", v, r),
...
def subcmd_db_list(arguments, options):
    ...
    argspec= sumo.cli.CmdSpecs()
    argspec.add("MODULES", completion= sumo.complete.module,
                array= True, optional= True)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
    ...
    if len(modules)==1 and modules[0]==".":
        ...
        # may raise ValueError:
        modulespecs_obj= \
                sumo.ModuleSpec.Specs.from_strings(modules, None)
        if options.dump_modules:
            dump_modules(modulespecs_obj)
            return
    ...
    if options.json:
        ...
        specs= sumo.ModuleSpec.Specs.from_dict(result)
        print("\n".join(specs.to_stringlist()))
...
def subcmd_db_find(arguments, options):
    ...
    if not options.json:
        specs= sumo.ModuleSpec.Specs()
        for (module,version) in results:
            specs.add(sumo.ModuleSpec.Spec.from_name_version(module, version))
        print("\n".join(specs.to_stringlist()))
        return
...
def subcmd_build_show(arguments, options):
    ...
    sumo.cli.assert_options(catch_exceptions,  options, "builddir")
    argspec= sumo.cli.CmdSpecs()
    argspec.add("BUILDTAG", completion= sumo.complete.builds)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
    ...
    new_builddb= sumo.Builds.DB(use_lock= True, lock_timeout= LOCK_TIMEOUT)
...
def build_getmodules(db, builddb, modulespecs_obj, buildtag):
    ...
    build_module_specs= sumo.ModuleSpec.Specs()
    ...
    for module in sorted(modules.keys()):
        ...
        build_module_specs.add(sumo.ModuleSpec.Spec(module,
                                                       modules[module],
                                                       'eq'))
    ...
    if modulespecs_obj:
        ...
        for added_module in db.complete_sets_dict(sets_dict):
            build_module_specs.add(\
                    sumo.ModuleSpec.Spec(added_module,
...
def subcmd_build_getmodules(arguments, options):
    ...
    argspec.add("BUILDTAG", optional= False,
                completion= sumo.complete.builds)
    argspec.add("MODULES", array= True, optional= True,
                completion= sumo.complete.moduleversion)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
    ...
    if not new_state:
        ...
        if new_state!="disabled":
            ...
            if builds:
                ...
                # may raise EOFError:
                try:
                    sumo.utils.ask_abort("Disabling %s would also disable "
                                            "these.\nProceed ? " % buildtag,
...
def subcmd_build_delete(arguments, options):
    ...
    sumo.cli.assert_options(catch_exceptions,  options, "builddir")
    argspec= sumo.cli.CmdSpecs()
    argspec.add("BUILDTAGS", array= True, completion= sumo.complete.builds)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)
...
def subcmd_build_try(arguments, options):
    ...
    # may raise re.error:
    try:
        exclude_matcher= sumo.utils.RegexpMatcher(options.exclude_states)
    except re.error as e:
        # pylint: disable=raising-format-tuple
        raise sumo.utils.annotate("Error in --exclude-states option: %s", e)
    ...
    if options.detail:
        ...
        if (detail==1) and wanted:
            ...
            sumo.JSON.dump(short_report)
        elif detail>=3:
            print("Details on all modules:\n")
            sumo.JSON.dump(report)
...
def subcmd_build_complete(arguments, options):
    """implement "subcmd_build_complete".
    ...
    ver_dict= db.ver_dict(modulespecs_obj)
    db.complete_ver_dict(ver_dict)
    specs= sumo.ModuleSpec.Specs()
    for m, v in ver_dict.items():
        specs.add(sumo.ModuleSpec.Spec(m, v, "eq"))
    module_strings= specs.to_stringlist()
    if options.lines:
        print(" ".join(module_strings))
    else:
        print("\n".join(module_strings))
...
def subcmd_build_new(arguments, options):
    ...
    if not options.no_make and not options.no_checkout:
        ...
        if options.testmode:
            # set build state to "testing" even if checkout or make were not
            # done:
            builddb.change_state(buildtag, "testing")
            builddb.json_save(None, options.verbose, options.dry_run)
...
def subcmd_build_use(arguments, options):
    ...
    May raise:
        ...
        ValueError from ModuleSpec.Specs.from_strings
        ValueError from modulespecs_obj.to_dist_dict
        SumoError, SumoBuildNotFoundError from apprelease
    """
    # pylint: disable=too-many-branches
    # pylint: disable=too-many-statements
    sumo.cli.assert_options(catch_exceptions,  options, "dbdir")
    ...
    # may raise EOFError, ValueError, IOError, sumo.JSON.ParseError:
    (_, db)= db_from_repo(options)
...
def subcmd_build_utilize(arguments, options):
    """implement "subcmd_build_utilize".
    ...
    May raise:
        ...
        ValueError from ModuleSpec.Specs.from_strings
        ValueError from modulespecs_obj.to_dist_dict
        SumoError from subcmd_build_use
    """
    try:
        subcmd_build_use(arguments, options)
        return
    except SumoBuildNotFoundError:
        # in this case, try "sumo build new"
        pass
    errmsg("must create a build first")
    subcmd_build_new(arguments, options)
    subcmd_build_use(arguments, options)
...
def maincmd_config(commands, options):
    ...
    # given:
    (cmd, c_args)= sumo.cli.process_cmd(commands, KNOWN_CONFIG_COMMANDS,
                                           options.list)
...
def maincmd_lock(arguments, options):
    ...
    May raise:
        sumo.lock.LockedError, sumo.lock.AccessError
    """
    if options.readonly:
        sys.exit("--readonly forbids editing a database file")

    argspec= sumo.cli.CmdSpecs()
    argspec.add("FILE", completion= sumo.cli.complete_file)
    args= sumo.cli.process_args(arguments, argspec, options.list,
                                   catch_exceptions)

    l= sumo.lock.MyLock(args.FILE) # pylint: disable=no-member
    # may raise sumo.lock.LockedError, sumo.lock.AccessError:
    l.lock()
...
def maincmd_unlock(arguments, options):
    ...
    try:
        l.lock()
    except sumo.lock.LockedError as _:
        lock_found= True
    except sumo.lock.AccessError as _:
...
def maincmd_db(commands, options):
    ...
    # note: the following function may exit the program when options.list is
    # given:
    (cmd, c_args)= sumo.cli.process_cmd(commands, KNOWN_DB_COMMANDS,
                                           options.list)
    ...
    if cmd=="list":
        # may raise IOError, KeyError, ValueError, sumo.JSON.ParseError:
...
def maincmd_build(commands, options):
    ...
    # note: the following function may exit the program when options.list is
    # given:
    (cmd, c_args)= sumo.cli.process_cmd(commands, KNOWN_BUILD_COMMANDS,
                                           options.list)
    ...
    if cmd=="try":
        ...
        #           sumo.JSON.ParseError, ValueError:
    ...
    if cmd=="complete":
        # may raise EOFError, IOError, KeyError, re.error
        #           sumo.JSON.ParseError, ValueError:
        subcmd_build_complete(c_args, options)
        return
    ...
    if cmd=="use":
        # may raise IOError, KeyError, ValueError,
        # SumoError, SumoBuildNotFoundError
    ...
    if cmd=="utilize":
        # may raise IOError, KeyError, ValueError
        # SumoError:
        subcmd_build_utilize(c_args, options)
        return
...
_sumo()
{
    ...
    if [ $(basename $SHELL) = "bash" ]; then
        ...
        if declare -F "__reassemble_comp_words_by_ref" > /dev/null; then
            # Use __reassemble_comp_words_by_ref if it is defined, this
            # function is no longer present in newer versions of
            # bash-completion:
            __reassemble_comp_words_by_ref : words cnt
        else
            _comp__reassemble_words : words cnt;
        fi
    else
...
def process(options, commands):
    ...
    try:
        (cmd, c_args)= sumo.cli.process_cmd(commands, KNOWN_MAIN_COMMANDS,
                                               options.list)
        ...
        return
            ...
            EOFError, re.error,
            SumoError,
            sumo.JSON.ParseError, sumo.JSON.ParseError,
            sumo.lock.AccessError, sumo.lock.LockedError) as e:
        if not catch_exceptions:
            raise
        # does sys.exit() :
        sumo.utils.exception_exit(e)
...
def main():
    ...
    specs.add("--append -A",
              lambda s,o: \
                  sumo.cli.complete_list(KNOWN_CONFIG_LIST_OPTIONS, s, o),
              arg_name= "optionname", array= True)
    specs.add("--#preload", sumo.cli.complete_file, "FILES", array= True)
    specs.add("--#opt-preload", sumo.cli.complete_file, "FILES",
              array= True)
    specs.add("--#postload", sumo.cli.complete_file, "FILES", array= True)
    specs.add("--#opt-postload", sumo.cli.complete_file, "FILES",
              array= True)
    specs.add("--dbdir", sumo.cli.complete_dir, arg_name= "DBDIR")
    specs.add("--dbrepomode", arg_name= "MODE",
              value_list= KNOWN_REPO_MODES)
    specs.add("--dbrepo", arg_name= "REPOSITORY")
    specs.add("--scandb", sumo.cli.complete_file, "SCANDB")
    specs.add("--dumpdb")
    ...
    specs.add("--builddir", sumo.cli.complete_dir, arg_name= "BUILDDIR")
    specs.add("--localbuilddir", sumo.cli.complete_dir,
              arg_name= "BUILDDIR")
    ...
    # we must parse '--exceptions' in the command line options early in order
    # to use it for sumo.cli.process_opts():
    local_catch_exceptions= catch_exceptions
    if "--exceptions" in sys.argv:
        local_catch_exceptions= False

    try:
        (options, args)= sumo.cli.process_opts(sys.argv, specs,
                                               local_catch_exceptions)
    except ValueError as e:
        if not local_catch_exceptions:
            raise
        # does sys.exit() :
        sumo.utils.exception_exit(e)
    ...
    options.alias      = sumo.utils.opt_join(options.alias, do_sort= True)
    options.module     = sumo.utils.opt_join(options.module)
    options.makeflags   = sumo.utils.opt_join(options.makeflags)

    # handle '-' in modules, this means to read modules
    # from standard input, one module per line:
    options.module     = sumo.utils.pipe_support(options.module)
    ...
    if not options.append:
        ...
        options.append= sumo.utils.opt_join(options.append)
