...
def assert_darcs():
    ...
    sumo.system.test_program("darcs")
...
class Repo():
    ...
    def _find_remote(self, patcher):
        ...
        try:
            ...
            sumo.system.system(cmd, True, True,
                                  _env,
                                  self.verbose, self.dry_run)
        except IOError as _:
            # probably no darcs repo found
    ...
    def _local_changes(self, matcher):
        ...
        (reply,_,rc)= sumo.system.system_rc(cmd, True, False,
                                               _env,
                                               self.verbose, self.dry_run)
    ...
    def _local_patches(self):
        ...
        (reply,_)= sumo.system.system(cmd, True, False,
                                         _env,
                                         self.verbose, self.dry_run)
    ...
    def checkout(spec, destdir, _, verbose, dry_run):
        ...
        sumo.system.system(cmd, True, False, _env, verbose, dry_run)
    ...
    def push(self):
        ...
        cmd="darcs push --repodir %s -a %s" % (self.directory,
                                               self.remote_url)
        sumo.system.system(cmd, True, False,
                              _env,
                              self.verbose, self.dry_run)
    def pull_merge(self):
        ...
        cmd="darcs pull --repodir %s -q -a %s" % (self.directory,
                                                  self.remote_url)
        (stdout,stderr)= sumo.system.system(cmd, True, True,
                                               _env,
                                               self.verbose, self.dry_run)
        output_lines= []
        if stderr:
