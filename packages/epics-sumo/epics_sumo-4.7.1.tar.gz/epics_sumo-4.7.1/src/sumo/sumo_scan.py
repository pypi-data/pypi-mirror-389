...
assert __version__==sumo.makefile_scan.__version__
...
def support_configure_data(support_trees,
    ...
    def slice_buildtag(l, buildtag):
        ...
        i= None
        for e in l:
            if sumo.utils.split_treetag(e)[1]==buildtag:
                i= index
                break
            index+=1
        if i is None:
    ...
    cnt= 0
    if progress:
        sumo.utils.show_progress(cnt, cnt_max,
    ...
    for support_tree in support_trees:
        for (dirpath, dirnames, filenames) in \
                                 sumo.utils.dirwalk(support_tree):
            if trace:
                # pylint: disable= consider-using-f-string
                sys.stderr.write("%s\n" % dirpath)
            if progress:
                cnt= sumo.utils.show_progress(cnt, cnt_max)
...
def name2path_from_deps(deps):
    ...
    for dependends in deps.values():
        for name, dep_path in dependends.items():
            sumo.utils.dict_of_sets_add(name2path, name, dep_path)
    return sumo.utils.dict_sets_to_lists(name2path)
...
def path2name_from_deps(deps):
    ...
    for dependends in deps.values():
        for name, dep_path in dependends.items():
            sumo.utils.dict_of_sets_add(path2name, dep_path, name)
    return sumo.utils.dict_sets_to_lists(path2name)
...
def repo_info(deps, progress,
    ...
    if ignore_changes_matcher:
        repo_hints["ignore changes"]= \
                sumo.utils.RegexpMatcher(ignore_changes_matcher)
    if dir_patches:
        patcher= sumo.utils.RegexpPatcher()
    ...
    for path in path_set:
        ...
        if flagdict.get("path"):
            repo_obj= sumo.repos.src_from_dir(path, repo_hints_path,
                                                 verbose, dry_run)
        else:
            ...
            repo_obj= sumo.repos.repo_from_dir(path, hints_param,
                                                  verbose, dry_run)
            if (not repo_obj) or repo_obj.local_changes:
                # no valid repository found  - or -
                # local changes
                repo_obj= sumo.repos.src_from_dir(path, hints_param,
                                                     verbose, dry_run)
...
def get_configuration(options):
    ...
    exclude_matcher= sumo.utils.RegexpMatcher(options.exclude_path)
...
def process(options, commands):
    ...
    options.dir            = sumo.utils.opt_join(options.dir)
    options.exclude_path   = sumo.utils.opt_join(options.exclude_path)
    options.group_basedir  = sumo.utils.opt_join(options.group_basedir)
    options.ignore_changes = sumo.utils.opt_join(options.ignore_changes)
    options.ignore_name    = sumo.utils.opt_join(options.ignore_name)
    ...
    hints= sumo.utils.Hints(options.hint)
    ...
    if options.dir:
        ...
        deps= sumo.JSON.loadfile(options.info_file)
    else:
