...
utilize
  find or create and use a build that matches your module specification
...
This command provides an easy way to set up your sumo configuration with a
single command. After this step you are ready to use sumo.

The command creates a sumo directory that contains a build directory and a
database directory. A dependency database is created in the database directory
that, except for the "empty" SOURCETAG, contains definitions for a number of
EPICS supports and the EPICS Base.

This command accepts all settings and command line or definitions of already
existing configuration files but sets 'dbdir' to DIRECTORY/database and 'builddir'
to DIRECTORY/build. If you clone the dependency database from a repository, it
also sets 'dbrepo' and 'dbrepomode'.

SOURCETAG determines how the dependency database file is created. These are
possible values for SOURCETAG:
...
sumo-free-database
  Create a dependency database from the public
  `sumo-free-database <https://sourceforge.net/projects/sumo-free-database>`_.
  repository.
  This dependency database contains some common EPICS device supports published
  by various institutions.

HZB-internal-RO
  Create a dependency database from the internal repository at the
  `Helmholtz-Zentrum Berlin <https://www.helmholtz-berlin.de>`_ with read-only
  access. This will only work when you are connected to the internal network of
  the
  `Helmholtz-Zentrum Berlin <https://www.helmholtz-berlin.de>`_.

HZB-internal-RW
  Create a dependency database from the internal repository at the
  `Helmholtz-Zentrum Berlin <https://www.helmholtz-berlin.de>`_ with write
  access. This will only work when you are connected to the internal network of
  the
  `Helmholtz-Zentrum Berlin <https://www.helmholtz-berlin.de>`_ and you have
  write access to the dependency database repository.

git
  Create a dependency database by cloning a git repository. The next argument
  must the be URL of the git upstream repository.

hg
  Create a dependency database by cloning a mercurial repository. The next argument
  must the be URL of the mercurial upstream repository.

darcs
  Create a dependency database by cloning a darcs repository. The next argument
  must the be URL of the darcs upstream repository.

FLAGS can modify what the command does. Known FLAGS are:

ADD_PRELOAD
  Add an '#opt-preload' statement to the created configuration file to always
  try to load file "configure/MODULES". When you install a sumo configuration
  file from scratch, it is recommended that you use this flag. EPICS
  applications that use sumo usually have a "configure/MODULES" file.

AT_LIBDIR
  Create the configuration file in the library directory of sumo. This is a
  portable way to put the configuration file at a location that can always be
  found by sumo without the need to set any environment variables.

GIT_REPO
  Create a local stand-alone git repository to manage the
  :term:`dependency database` :term:`DEPS.DB`. All changes to the database file
  that are done with sumo will automatically be tracked. This flag can only be
  used if you *do not* clone from an existing repository.

REPO_WRITE
  Unless this flag is given, the default
  :ref:`dbrepomode <reference-sumo-Options-dbrepomode>`. is 'pull'. When this flag
  is given, the
  :ref:`dbrepomode <reference-sumo-Options-dbrepomode>`. is set to 'push'.
  See also :ref:`dbrepomode <reference-sumo-Options-dbrepomode>`.
  This flag can only be used if you *did clone* from an existing repository.

INITIAL
  This is a combination of flags, It always includes ADD_PRELOAD and AT_LIBDIR.
  If you didn't clone an existing repository, it also includes GIT_REPO.

When the flags 'AT_LIBDIR' or 'INITIAL' are not given, the file "sumo.config"
is created in the current working directory. If such a file already exists, it
is renamed to "sumo.config.bak" before "sumo.config" is created.
...
The result is printed on the console, one module and version per line or as a
complete dependency database structure in JSON format when option ``--json`` is
given.
...
:term:`modules`. If called with argument MODULES, a list of
:term:`modulespecs` MODULE:{+-}VERSION or MODULE:/VERSION/ that specifies
:term:`modules` and :term:`versions`, list all the matching :term:`versions`
of all specified :term:`modules`.

The result is printed on the console, one module and version per line or as a
JSON structure when option ``--json`` is given.
...
ERROR
  Merge conflicts are treated as an error and abort the program. This is the
  default when no flags are given.

ASK
  In case of merge conflicts, the user is aked if he wants to continue or
  abort. If he chooses to continue, the conflicting parts of the other
  :term:`dependency database` are ignored.

WARN
  In case of merge conflicts, warnings are shown and the conflicting parts of
  the other :term:`dependency database` are ignored.

In any case, If a module has an :ref:`"extra" <reference-sumo-extra>` section,
only *new* lines are appended to the existing
:ref:`"extra" <reference-sumo-extra>` section. This avoids having the same line
several times in :ref:`"extra" <reference-sumo-extra>` after a merge operation.
...
form MODULE or MODULE:{+-}VERSION or MODULE:/VERSION/ that specifies just a
module name, a module and some versions or a single version. For details on
:term:`modulespecs` see :doc:`Module Specifications <modulespecs>`.
...
With option ``--lines`` for each build the output is a single line instead of a
number of indented lines. With ``--brief``, the build name is not printed. If you
use ``--lines`` and ``--brief``, the output is compatible with the ``-m`` option of
...
MODULE:{+-}VERSION or MODULE:/VERSION/ that specifies just a module name, a
module and some versions or a single version. For details on
:term:`modulespecs` see :doc:`Module Specifications <modulespecs>`. You can
specify an incomplete list of :term:`modules`.
...
This command is intended to help you create :term:`module` specifications for
the "build new" command. The difference to ``build try MODULES`` is that this
command always takes the newest version of each module that is needed as a
dependency or that is given in MODULES but whose version is not exactly
specified.
...
When option ``--lines`` is used, the completed module list in printed in a
single line on the console, otherwise the program prints a single line for each
module.

Examples:

Show completed module list from an incomplete list with an EPICS Base and the
MCAN device support::

  sumo  build complete BASE:R3-15-9 MCAN

Show completed module list from an incomplete list of device supports in file
"MODULES.incomplete"::

  sumo -c configure/MODULES.incomplete build complete

Create "configure/MODULES" from a file with an incomplete list of device
supports in file "configure/MODULES.incomplete". We assume that this file
contains some alias definitions that must be preserved, for this reason the
second sumo call has to read this file again::

  sumo -c configure/MODULES.incomplete build complete | \
  sumo -c configure/MODULES.incomplete -m - config make configure/MODULES alias module
...
This command is for the common workflow where we want to use a build, and if it
odesn't exist already, create it first.

The command first runs ``build use``. If no matching build is found, it runs
``build new`` and then again ``build use``. 

See also `build new MODULES`_ and `build use MODULES`_.
...
+++++++++++++++++++++
    ...
    four possible values: 'local', 'get', 'pull' and 'push'. Mode 'get' is the
    default. The meaning depends on the used version control system (VCS), if
    it is distributed (git,mercurial,darcs) or centralized (subversion,cvs).
...
++++++++++
    ...
    Show information for automatic command completion. You usually don't use
    this option. It is needed by the shell command completion function.
...
++++++++++++++++++++++++++++++
    ...
    values in a single string separated by spaces. If the arguments to this
    option contain a single dash '-', the program reads the module
    specifications at this position in the module list from lines on standard
    input. A default for this option can be put in a configuration file.
...
+++++++++++++++++++++++++++
    ...
    configuration file "sumo.config" from several standard locations (see
    :ref:`Confiuration Files <configuration-files-paths>`).
...
``--testmode``
++++++++++++++
    Run program in test mode, this is needed for some program tests and should
