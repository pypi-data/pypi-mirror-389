...
class Repo():
    ...
    def _local_changes(self, matcher):
        ...
        (reply,_,rc)= sumo.system.system_rc(cmd, True, False, None,
                                               self.verbose, self.dry_run)
    ...
    def _local_patches(self):
        ...
        (_,_,rc)= sumo.system.system_rc(cmd, True, False, None,
                                           self.verbose, self.dry_run)
        if rc not in (0,1):
            raise IOError(rc, "cmd \"%s\" failed" % cmd)
        return rc==0
    ...
    def pull_merge(self):
        ...
        (stdout,_,rc)= sumo.system.system_rc(cmd,
                                                True, False, None,
                                                self.verbose, self.dry_run)
        if rc!=0:
