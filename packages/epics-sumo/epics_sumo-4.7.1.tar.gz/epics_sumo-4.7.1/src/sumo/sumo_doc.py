...
database directory and the build directory with '--dbdir' and '--
builddir' or a configuration file.
...
use
  find and use a build that matches your module specification

utilize
  find or create and use a build that matches your module
  specification
...
'_order': ['config list', 'config local DIRECTORY', 'config make FILENAME [OPTIONNAMES]', 'config new DIRECTORY SOURCETAG [URL] [FLAGS]', 'config show [OPTIONNAMES]', 'config standalone DIRECTORY'],
"": r"""
subcommands for maincommand "config"
++++++++++++++++++++++++++++++++++++
...
module specifications are updated from the specified build. This works
like the command "build getmodules.
...
""",
'config new DIRECTORY SOURCETAG [URL] [FLAGS]': r"""
config new DIRECTORY SOURCETAG [URL] [FLAGS]
::::::::::::::::::::::::::::::::::::::::::::

This command provides an easy way to set up your sumo configuration
with a single command. After this step you are ready to use sumo.

The command creates a sumo directory that contains a build directory
and a database directory. A dependency database is created in the
database directory that, except for the "empty" SOURCETAG, contains
definitions for a number of EPICS supports and the EPICS Base.

This command accepts all settings and command line or definitions of
already existing configuration files but sets 'dbdir' to
DIRECTORY/database and 'builddir' to DIRECTORY/build. If you clone the
dependency database from a repository, it also sets 'dbrepo' and
'dbrepomode'.

SOURCETAG determines how the dependency database file is created.
These are possible values for SOURCETAG:
...
sumo-free-database
  Create a dependency database from the public "sumo-free-database
  <https://sourceforge.net/projects/sumo-free-database>". repository.
  This dependency database contains some common EPICS device supports
  published by various institutions.

HZB-internal-RO
  Create a dependency database from the internal repository at the
  "Helmholtz-Zentrum Berlin <https://www.helmholtz-berlin.de>" with
  read-only access. This will only work when you are connected to the
  internal network of the "Helmholtz-Zentrum Berlin
  <https://www.helmholtz-berlin.de>".

HZB-internal-RW
  Create a dependency database from the internal repository at the
  "Helmholtz-Zentrum Berlin <https://www.helmholtz-berlin.de>" with
  write access. This will only work when you are connected to the
  internal network of the "Helmholtz-Zentrum Berlin
  <https://www.helmholtz-berlin.de>" and you have write access to the
  dependency database repository.

git
  Create a dependency database by cloning a git repository. The next
  argument must the be URL of the git upstream repository.

hg
  Create a dependency database by cloning a mercurial repository. The
  next argument must the be URL of the mercurial upstream repository.

darcs
  Create a dependency database by cloning a darcs repository. The next
  argument must the be URL of the darcs upstream repository.

FLAGS can modify what the command does. Known FLAGS are:

ADD_PRELOAD
  Add an '#opt-preload' statement to the created configuration file to
  always try to load file "configure/MODULES". When you install a sumo
  configuration file from scratch, it is recommended that you use this
  flag. EPICS applications that use sumo usually have a
  "configure/MODULES" file.

AT_LIBDIR
  Create the configuration file in the library directory of sumo. This
  is a portable way to put the configuration file at a location that
  can always be found by sumo without the need to set any environment
  variables.

GIT_REPO
  Create a local stand-alone git repository to manage the dependency
  database DEPS.DB. All changes to the database file that are done
  with sumo will automatically be tracked. This flag can only be used
  if you *do not* clone from an existing repository.

REPO_WRITE
  Unless this flag is given, the default dbrepomode. is 'pull'. When
  this flag is given, the dbrepomode. is set to 'push'. See also
  dbrepomode. This flag can only be used if you *did clone* from an
  existing repository.

INITIAL
  This is a combination of flags, It always includes ADD_PRELOAD and
  AT_LIBDIR. If you didn't clone an existing repository, it also
  includes GIT_REPO.

When the flags 'AT_LIBDIR' or 'INITIAL' are not given, the file
"sumo.config" is created in the current working directory. If such a
file already exists, it is renamed to "sumo.config.bak" before
"sumo.config" is created.
...
'_order': ['db alias-add MODULE DEPENDENCY ALIAS', 'db appconvert SCANFILE', 'db check', 'db clonemodule OLD-MODULE NEW-MODULE [VERSIONS]', 'db cloneversion MODULE OLD-VERSION NEW-VERSION [SOURCESPEC]', 'db commands MODULE LINES', 'db convert SCANFILE', 'db dependency-add MODULE DEPENDENCY', 'db dependency-delete MODULE DEPENDENCY', 'db edit', 'db extra MODULE [LINES]', 'db find REGEXP', 'db format', 'db list MODULES', 'db make-recipes MODULE [TARGET] [LINES]', 'db merge DB [FLAGS]', 'db modconvert SCANFILE MODULES', 'db releasefilename MODULE RELEASEFILENAME', 'db replaceversion MODULE OLD-VERSION NEW-VERSION', 'db show MODULES', 'db weight WEIGHT MODULES'],
"": r"""
subcommands for maincommand "db"
++++++++++++++++++++++++++++++++
""",
...
created dependency database file is stored, with '--scandb' you
specify the name of the scan database file. The scan database file
...
sumo commits the changes if option '--dbrepo' was specified.  If
option '--logmsg' was given, this is used as commit log message,
otherwise an editor is started where you can enter a log message.
Finally the file lock is released. If you want to edit the dependency
...
The result is printed on the console, one module and version per line
or as a complete dependency database structure in JSON format when
option '--json' is given.
""",
'db format': r"""
db format
:::::::::
...
are restored. If you specified a repository with '--dbrepo,' the
command will commit the changes. If you want a log message different
from "db format" use option '--logmsg'
...
or MODULE:/VERSION/ that specifies modules and versions, list all the
matching versions of all specified modules.

The result is printed on the console, one module and version per line
or as a JSON structure when option '--json' is given.
...
the original dependency database are added.

FLAGS can modify what the command does. Known FLAGS are:

ERROR
  Merge conflicts are treated as an error and abort the program. This
  is the default when no flags are given.

ASK
  In case of merge conflicts, the user is aked if he wants to continue
  or abort. If he chooses to continue, the conflicting parts of the
  other dependency database are ignored.

WARN
  In case of merge conflicts, warnings are shown and the conflicting
  parts of the other dependency database are ignored.

In any case, If a module has an "extra" section, only *new* lines are
appended to the existing "extra" section. This avoids having the same
line several times in "extra" after a merge operation.
""",
...
'_order': ['build delete BUILDTAGS', 'build find MODULES', 'build getmodules BUILDTAG [MODULES]', 'build list', 'build new MODULES', 'build remake BUILDTAG', 'build show BUILDTAG', 'build showmodules [BUILDTAG]', 'build showdependencies [BUILDTAG]', 'build showdependents [BUILDTAG]', 'build state BUILDTAG [NEW-STATE]', 'build try MODULES', 'build complete MODULES', 'build use MODULES', 'build utilize MODULES'],
"": r"""
subcommands for maincommand "build"
+++++++++++++++++++++++++++++++++++
""",
...
MODULE or MODULE:{+-}VERSION or MODULE:/VERSION/ that specifies just a
module name, a module and some versions or a single version. For
details on modulespecs see Module Specifications .
...
If option '--all-builds' is given, builds whose state is not 'stable'
or 'testing' are also shown.

Options '--sort-build-dependencies-first' and '--sort-build-
dependencies-last' can be used to change the order reported builds.
Without these options, builds are sorted by *match rank*. Builds that
have more modules with matching versions are placed first, followed by
the ones with more wrong versions followed by the ones with more
missing versions.

Option '--detail' determines, what is shown. This must be an integer
between 0 and 2. With '0', the default, only builds with matching
module versions are shown. With '1', also builds with wrong versions
are shown and with '2' even builds with missing modules are shown.
...
build is reported anyway since we used '--detail 1'. `MCAN` matches
our specification since we didn't define a version.

Finally, if the command 'sumo build use' doesn't find a build for your
module specifications, you may want to run 'sumo build find --detail
1' or ``sumo build find --detail 2``. This may help you finding errors
in your module specification in file `configure/MODULES`.
...
With option '--lines' for each build the output is a single line
instead of a number of indented lines. In this case, the output is
compatible with the '-m' option of sumo.

Here are some applications for this, please look also at :ref:`"config
make <reference-config-make>` with option '--getmodules' which does
the same:
...
dependencies-first' and '--sort-build-dependencies-last' can be used
to change the order of the builds shown. The command shows only builds
...
warning if option '--no-err-build-exists' is given.  If the buildtag
is not given as an option, the program generates a buildtag in the
form "AUTO-nnn". A new build is created according to the modulespecs.
Your module specifications must be *complete* and *exact* meaning that
all dependencies are included and all modules are specified with
...
With option '--lines' for each build the output is a single line
instead of a number of indented lines. With '--brief', the build name
is not printed. If you use '--lines' and '--brief', the output is
compatible with the '-m' option of sumo. Here are some applications
for this:
...
Options '--sort-build-dependencies-first' and '--sort-build-
dependencies-last' can be used to change the order reported
dependencies.
...
MODULE:{+-}VERSION or MODULE:/VERSION/ that specifies just a module
name, a module and some versions or a single version. For details on
modulespecs see Module Specifications . You can specify an incomplete
list of modules.

The detail of the output is determined by option '--detail' which is
an integer between 0 and 3. 0, the default, gives the shortest, 3
gives the longest report. The program then shows which modules you
have to
...
""",
'build complete MODULES': r"""
build complete MODULES
::::::::::::::::::::::

This command is intended to help you create module specifications for
the "build new" command. The difference to 'build try MODULES' is that
this command always takes the newest version of each module that is
needed as a dependency or that is given in MODULES but whose version
...
When option '--lines' is used, the completed module list in printed in
a single line on the console, otherwise the program prints a single
line for each module.

Examples:

Show completed module list from an incomplete list with an EPICS Base
and the MCAN device support:

  sumo  build complete BASE:R3-15-9 MCAN

Show completed module list from an incomplete list of device supports
in file "MODULES.incomplete":

  sumo -c configure/MODULES.incomplete build complete

Create "configure/MODULES" from a file with an incomplete list of
device supports in file "configure/MODULES.incomplete". We assume that
this file contains some alias definitions that must be preserved, for
this reason the second sumo call has to read this file again:

  sumo -c configure/MODULES.incomplete build complete | \
  sumo -c configure/MODULES.incomplete -m - config make
  configure/MODULES alias module
...
- Create a build matching your module specification with:
  ...
  - 'sumo build new'
""",
'build utilize MODULES': r"""
build utilize MODULES
:::::::::::::::::::::

This command is for the common workflow where we want to use a build,
and if it odesn't exist already, create it first.

The command first runs 'build use'. If no matching build is found, it
runs 'build new' and then again 'build use'.

See also "build new MODULES" and "build use MODULES".
...
to your $HOME/.bashrc or $HOME/.zshrc file.
...
+++++++++++++++++++
    ...
    There are four possible values: 'local', 'get', 'pull' and 'push'.
    Mode 'get' is the default. The meaning depends on the used version
    control system (VCS), if it is distributed (git,mercurial,darcs)
    or centralized (subversion,cvs).

    Note that mode 'local' can only be used for a distributed VCS.

    There are three possible operations on the dependency database:
    ...
    +------+----------+--------------------+--------------------------
    -----+
    |mode  |operation |action centr. VCS   | action distr. VCS
    |
    +======+==========+====================+==========================
    =====+
    |local |init      | (invalid)          | none
    |
    |      |          |                    | (local repository must
    exist) |
    |
    +----------+----------------------------------------------------+
    |      |read      | (invalid)          | none
    ...
    |
    +----------+----------------------------------------------------+
    |      |write     | update, commit     | pull, commit, push
    |
    +------+----------+-----------------------------------------------
    -----+
""",
...
+++++++++++++++++++++++++++++++++++++++
    ...
    It specifies a directory patchexpression. Such an expression
    ...
    expression is applied to every source path generated. You can
...
++++++++
    ...
    Show results of 'db list' and 'db find' in JSON format.
""",
'``--list``': r"""
'--list'
++++++++

    Show information for automatic command completion. You usually
    don't use this option. It is needed by the shell command
    completion function.
""",
...
++++++++++++++++++++++++++++
    ...
    joining values in a single string separated by spaces. If the
    arguments to this option contain a single dash '-', the program
    reads the module specifications at this position in the module
    list from lines on standard input. A default for this option can
    be put in a configuration file.
""",
'``--no-checkout``': r"""
'--no-checkout'
...
+++++++++++++++++++++++++
    ...
    If this option is not given, the program tries to load the default
    configuration file "sumo.config" from several standard locations
    (see Confiuration Files).
""",
...
'--testmode'
++++++++++++
    Run program in test mode, this is needed for some program tests
