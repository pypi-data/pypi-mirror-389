...
class SerReader(object):
    ...
    def getAll(self) :
        all = []
        z = datetime.datetime(1,1,2).timestamp()-25*3600
        with open(self.fname,"rb") as f:
            f.seek(178)
            for i in range(self.header.frameCount) :
                frame = np.frombuffer(f.read(self.imgSizeBytes),dtype=self.dtype)
                frame = np.reshape(frame,(self.header.imageHeight,self.header.imageWidth,self.header.numPlanes))
                hdu = fits.PrimaryHDU(frame)
                date = Time(datetime.datetime.fromtimestamp(self.times[i]/1.e7+z),scale='utc')
                hdu.header['DATE-OBS'] = date.iso
                hdu.header['OBSERVER'] = self.header.observer
                hdu.header['INSTRUME'] = self.header.instrument
                hdu.header['TELESCOP'] = self.header.telescope
                all.append(hdu)
        return all

    def getTimes(self,imgNum=None):
        with open(self.fname,"rb") as f:
            f.seek(int(178+(self.header.frameCount)*(self.imgSizeBytes)))
            frame = np.frombuffer(f.read(self.header.frameCount*8),dtype=np.uint64)
