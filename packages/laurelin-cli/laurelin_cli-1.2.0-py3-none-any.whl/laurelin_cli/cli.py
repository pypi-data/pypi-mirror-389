...
def _monitor_simulation(api_client: APIClient, monitor_info: dict) -> None:
    """Monitor a simulation and display progress."""
    import time

    simulation_id = monitor_info.get('simulation_id')
    poll_interval = monitor_info.get('poll_interval', 5000) / 1000  # Convert ms to seconds

    click.echo(f"\n{Colors.YELLOW}📊 Monitoring simulation: {simulation_id[:40]}...{Colors.RESET}")
    click.echo(f"{Colors.YELLOW}Press Ctrl+C to stop monitoring{Colors.RESET}\n")

    # Determine simulation type from ID prefix
    if simulation_id.startswith('nimrod_'):
        status_func = api_client.get_nimrod_status
    elif simulation_id.startswith('warpx_'):
        from .api_client import APIClient  # Ensure we have the right method
        # WarpX uses different endpoint - we'll handle generically
        status_func = lambda sid: api_client._make_request('GET', f'/simulations/{sid}/status')
    elif simulation_id.startswith('gs2_'):
        status_func = lambda sid: api_client._make_request('GET', f'/simulations/{sid}/status')
    else:
        warning(f"Unknown simulation type for {simulation_id}")
        return
    ...
    try:
        ...
        while True:
            try:
                result = status_func(simulation_id)
                ...
                if result.get('success'):
                    ...
                    if status != last_status or progress != last_progress:
                        ...
                        # Display status line
                        status_line = f"{status_icon} {Colors.BOLD}{status.upper()}{Colors.RESET} │ {bar} {progress}%"
                    ...
                    if status in ['completed', 'failed', 'cancelled']:
                        ...
                        if status == 'completed':
                            ...
                            warning(f"Simulation cancelled")
                        break
                else:
                    error(f"Failed to get status: {result.get('message', 'Unknown error')}")
                    break

            except KeyboardInterrupt:
                click.echo()  # New line
                info("Stopped monitoring (simulation continues running)")
                break
...
@click.group()
@click.version_option(version='1.2.0', prog_name='laurelin')
def cli():
    """
...
@click.option('--once', is_flag=True, help='Send a single message and exit (default is interactive)')
def chat(message: Optional[str], session: Optional[str], once: bool):
    """
    Start an interactive chat session with Laurelin AI.

    By default, starts in interactive mode. Press Ctrl+C or type 'exit' to quit.

    Examples:
      laurelin chat                           # Interactive mode (default)
      laurelin chat "What is the plasma density?"  # Single message
      laurelin chat --once "Quick question"   # Explicit single message mode
      laurelin chat -s abc123                 # Continue previous session
    """
    auth_manager = AuthManager()
    ...
    # Determine mode: interactive if no message provided or --once not specified
    interactive = not message or not once

    # Interactive mode
    if interactive and not message:
        click.echo(f"\n{Colors.BOLD}Welcome to Laurelin - Your AI assistant for nuclear fusion research.{Colors.RESET}")
        click.echo("=" * 70)
        click.echo(f"{Colors.CYAN}Type your questions or commands below. Use 'exit' or 'quit' to end.{Colors.RESET}\n")

        current_session_id = session

        while True:
            try:
                user_message = click.prompt(f"{Colors.GREEN}laurelin@fusion:~${Colors.RESET}", prompt_suffix=" ")
                ...
                try:
                    ...
                    if response.get('success'):
                        ...
                        # Display response without prefix for cleaner output
                        click.echo(f" {ai_response}")

                        # Update session ID for continuation
                        if not current_session_id:
                            current_session_id = response.get('session', {}).get('session_id')

                        # Check if simulation monitoring is needed
                        if 'simulation_monitor' in response:
                            _monitor_simulation(api_client, response['simulation_monitor'])
                    else:
    ...
    # Single message mode (when message is provided)
    if message:
        try:
            click.echo(f"\n{Colors.GREEN}laurelin@fusion:~${Colors.RESET} {message}\n")
...
@click.option('--type', '-t', 'dataset_type', help='Dataset type (e.g., input_data, initial_conditions, mesh)')
@click.option('--description', '-d', help='Description of the dataset')
def storage_upload(file_path, dataset_type, description):
    """Upload a dataset file for analysis and simulation use.

    Example:
        laurelin storage upload plasma_profile.h5 --type input_data --description "Plasma density profile"
    """
    auth_manager = AuthManager()
    ...
    try:
        ...
        click.echo(f"\n{Colors.CYAN}Uploading:{Colors.RESET} {filename} ({file_size_mb:.2f} MB)")
        ...
        with open(file_path, 'rb') as f:
            ...
            import requests
            auth_header = api_client._get_headers()
            response = requests.post(
                f"{api_client.base_url}/storage/upload",
                files=files,
                data=data,
                headers=auth_header
            )
            ...
            if response.status_code == 201:
                ...
                click.echo(f"\n{Colors.CYAN}Dataset ID:{Colors.RESET} {dataset.get('dataset_id')}")
                click.echo(f"{Colors.CYAN}Filename:{Colors.RESET} {dataset.get('filename')}")
                click.echo(f"{Colors.CYAN}Size:{Colors.RESET} {dataset.get('size_mb')} MB")
                click.echo(f"{Colors.CYAN}Type:{Colors.RESET} {dataset.get('dataset_type')}")
                click.echo(f"{Colors.CYAN}Uploaded:{Colors.RESET} {dataset.get('uploaded_at')}")
                click.echo()

                info("You can now reference this dataset in your simulations")
...
def storage_datasets():
    ...
    try:
        result = api_client.get(f"/api/storage/datasets")
        ...
        if result.get('success'):
            ...
            click.echo(f"\n{Colors.BOLD}Uploaded Datasets ({count}){Colors.RESET}")
            click.echo("=" * 90)
            ...
            for dataset in datasets:
                ...
                dtype = dataset.get('dataset_type', 'general')
                uploaded = dataset.get('uploaded_at', '')[:10]

                click.echo(f"\n{Colors.CYAN}[{dataset_id}]{Colors.RESET}")
                click.echo(f"  Filename: {filename}")
                click.echo(f"  Size: {size_mb} MB")
            ...
            error(f"Failed to list datasets: {result.get('error', 'Unknown error')}")
            sys.exit(1)
...
@storage.command(name='delete-dataset')
@click.argument('dataset_id')
def storage_delete_dataset(dataset_id):
    ...
    try:
        result = api_client.delete(f"/api/storage/datasets/{dataset_id}")

        if result.get('success'):
            success(f"Deleted dataset {dataset_id}")
        else:
            error(f"Failed to delete dataset: {result.get('error', 'Unknown error')}")
            sys.exit(1)
