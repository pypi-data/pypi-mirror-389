...
try:
    ...
    print("✅ Imported ExecutionNodeConnector successfully")
except ImportError as e:
    print(f"❌ Failed to import ExecutionNodeConnector: {e}")
...
async def send_message(request: Request, req: SendMessageRequest):
    ...
    print(f"💬 User {user.username} sent message in conversation: {conversation_id} (cloud storage fallback)")
    
    # Return error message about execution node not available
    ...
    return {
        ...
        "conversation_id": conversation_id,
        "dt_status": {"active": False, "error": "node_not_available"},
        "source": "cloud_storage"
    }
