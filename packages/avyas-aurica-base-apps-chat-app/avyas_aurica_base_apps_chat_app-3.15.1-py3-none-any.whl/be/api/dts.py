...
def discover_dts() -> List[dict]:
    ...
    # Track app names to avoid duplicates
    app_names = {dt["metadata"].get("data_path", "").split("/")[-1] for dt in app_dts if dt["metadata"].get("data_path")}
    ...
    for item in DATA_DIR.iterdir():
        ...
        # Skip if this is an app's data directory (already handled as app DT)
        if item.name in app_names:
            continue
