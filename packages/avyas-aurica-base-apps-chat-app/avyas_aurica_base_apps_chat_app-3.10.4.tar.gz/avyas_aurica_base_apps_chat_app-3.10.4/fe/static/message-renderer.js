...
class MessageRenderer {
    ...
    render(message) {
        ...
        if (message.render_blocks && message.render_blocks.length > 0) {
            ...
            message.render_blocks.forEach((block, index) => {
                ...
                // Auto-detect HTML content in markdown blocks
                if (block.type === 'markdown' && this.isHtmlContent(block.data.content)) {
                    console.log('  🔒 Detected HTML content, rendering in sandbox');
                    block.type = 'html';
                }
            ...
            let blockType = 'markdown';
            
            if (this.isHtmlContent(content)) {
                console.log('  🔒 Detected HTML content in fallback, rendering in sandbox');
                blockType = 'html';
            }
    ...
    /**
     * Detect if content contains HTML that should be sandboxed
     */
    isHtmlContent(content) {
        ...
        return htmlMarkers.some(marker => lowerContent.includes(marker.toLowerCase()));
    }
     ...
     * Note: Escapes HTML by default to prevent injection attacks
     */
    renderMarkdown(data, metadata) {
        ...
        // Basic markdown parsing on escaped content
        // Bold
        ...
        // Code blocks (preserve newlines within them)
     ...
     * This prevents HTML from affecting the parent page
     */
    renderHtml(data, metadata) {
        const container = document.createElement('div');
        container.className = 'render-html-sandbox';
        
        const iframe = document.createElement('iframe');
        iframe.sandbox = 'allow-scripts allow-same-origin'; // Sandboxed for security
        ...
        // Write HTML content to iframe after it's added to DOM
        setTimeout(() => {
