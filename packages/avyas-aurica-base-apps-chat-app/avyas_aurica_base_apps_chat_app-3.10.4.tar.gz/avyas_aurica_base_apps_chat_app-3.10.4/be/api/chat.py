...
# Import Aurica Auth SDK (simplified - no fallback needed)
from src.aurica_auth import protected, get_current_user, public

# ExecutionNodeConnector is deprecated with transparent proxy architecture
# Setting to None to indicate it's not available
...
class SendMessageRequest(BaseModel):
    ...
    content: str
    sender: str = "user"
    dt_id: Optional[str] = None  # Optional DT to associate with new conversations
...
@protected
async def list_conversations(request: Request, dt_id: Optional[str] = None):
    """List all conversations for the authenticated user, optionally filtered by DT."""
    user = get_current_user(request)
    storage = get_storage()
    
    filter_msg = f" (filtered by DT: {dt_id})" if dt_id else ""
    print(f"💬 User {user.username} listing conversations{filter_msg}")
    ...
    user_conversations = storage.get_conversations(user_id=user.user_id, dt_id=dt_id)
...
async def send_message(request: Request, req: SendMessageRequest):
    ...
    if not conversation_id:
        ...
        # Associate with DT if provided
        if req.dt_id:
            conversation["dt_id"] = req.dt_id
            print(f"🤖 Associating conversation with DT: {req.dt_id}")
