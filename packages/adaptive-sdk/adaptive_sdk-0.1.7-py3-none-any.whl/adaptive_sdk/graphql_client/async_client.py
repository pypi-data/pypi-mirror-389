...
class AsyncGQLClient(AsyncBaseClientOpenTelemetry):
    ...
    async def attach_model_to_use_case(self, input: AttachModel, **kwargs: Any) -> AttachModelToUseCase:
        ...
        variables: Dict[str, object] = {'input': input}
        response = await self.execute(query=query, operation_name='AttachModelToUseCase', variables=variables, **kwargs)
        data = self.get_data(response)
    ...
    async def deploy_model(self, id_or_key: Any, wait: bool, placement: Union[Optional[ModelPlacementInput], UnsetType]=UNSET, **kwargs: Any) -> DeployModel:
