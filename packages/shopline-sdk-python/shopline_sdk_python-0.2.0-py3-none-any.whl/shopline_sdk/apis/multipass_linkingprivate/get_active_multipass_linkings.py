from typing import Any, Dict, List, Optional, Union
import aiohttp
from pydantic import BaseModel, ValidationError, Field
from typing_extensions import Literal

# 导入异常类
from shopline_sdk.exceptions import ShoplineAPIError

# 导入需要的模型
from shopline_sdk.models.multipass_linkings import MultipassLinkings
from shopline_sdk.models.unprocessable_entity_error import UnprocessableEntityError

class Params(BaseModel):
    """查询参数模型"""
    customer_ids: Optional[List[str]] = None
    """Which customers to find"""
    cursor: Optional[str] = None
    """starting ID of returned linkings"""
    limit: Optional[float] = None
    """how many item to return at the same time, default 10, max 100"""

async def call(
    session: aiohttp.ClientSession, params: Optional[Params] = None
) -> MultipassLinkings:
    """
    Get active multipass linkings
    
    Given customer_ids, return active linking records of customers
    
    Path: GET /multipass/linkings
    """
    # 构建请求 URL
    url = "multipass/linkings"

    # 构建查询参数
    query_params = {}
    if params:
        params_dict = params.model_dump(exclude_none=True)
        for key, value in params_dict.items():
            if value is not None:
                query_params[key] = value

    # 构建请求头
    headers = {"Content-Type": "application/json"}

    # 发起 HTTP 请求
    async with session.get(
        url, params=query_params, headers=headers
    ) as response:
        if response.status >= 400:
            error_data = await response.json()
            if response.status == 422:
                error = UnprocessableEntityError(**error_data)
                raise ShoplineAPIError(
                    status_code=422,
                    error=error
                )
            # 默认错误处理
            raise ShoplineAPIError(
                status_code=response.status,
                **error_data
            )
        response_data = await response.json()

        # 验证并返回响应数据
        return MultipassLinkings(**response_data)