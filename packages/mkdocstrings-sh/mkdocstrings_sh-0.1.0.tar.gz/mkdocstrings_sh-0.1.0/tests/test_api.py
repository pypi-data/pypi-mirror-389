...
from mkdocstrings_handlers import sh
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("mkdocstrings_handlers.sh")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.sh._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.sh"]
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("mkdocstrings_handlers.sh")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and _module_or_child("mkdocstrings_handlers.sh", item.name)
        ):
