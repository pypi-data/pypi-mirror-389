...
if TYPE_CHECKING:
    ...
    from mkdocstrings_handlers.sh.handler import ShHandler
...
def fixture_mkdocs_conf(request: pytest.FixtureRequest, tmp_path: Path) -> Iterator[config.Config]:
    ...
    conf_dict = {
        ...
        "plugins": [{"mkdocstrings": {"default_handler": "sh"}}],
...
def fixture_handler(plugin: MkdocstringsPlugin, ext_markdown: Markdown) -> ShHandler:
    ...
    Returns:
        A handler instance.
    """
    handler = plugin.handlers.get_handler("sh")
