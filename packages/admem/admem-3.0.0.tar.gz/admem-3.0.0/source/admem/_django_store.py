...
class DjangoStore:
    ...
    def create_dj_path(
        self, model: type[models.Model], dc_field: ModelFieldInfo
    ...
    def django_to_dataclass(  # pylint: disable=too-many-locals,too-many-branches
        ...
        if is_pydantic_model(dataclass):
            return self._django_to_pydantic(dataclass, dj_obj, defer=defer, only=only)
        ...
        for field in dc.fields(dataclass):
            ...
            if issubclass(field_type, Path):  # type: ignore[unused-ignore,arg-type]
                create_dj_path = self.create_dj_path(dj_obj.__class__, field)  # type: ignore
    ...
    def _django_to_pydantic(
        ...
        pyd = require_pydantic("to load BaseModel subclasses")
        assert issubclass(model_cls, pyd.BaseModel)

        obj_kwgs: dict[str, tp.Any] = {}
        true_defer, sub_defer = self._true_key_and_subkey(defer)
        true_only, sub_only = self._true_key_and_subkey(only, is_only=True)
        for name in model_cls.model_fields:  # type: ignore
            if name in true_defer or (true_only and name not in true_only):
                obj_kwgs[name] = None
                continue
            val = getattr(dj_obj, name)
            if type(val) in BACKEND_LINKER.backend_to_dc:
                val = self.django_to_dataclass(
                    val,
                    defer=sub_defer.get(name, set()),
                    only=sub_only.get(name, set()),
                )
            obj_kwgs[name] = val
        return model_cls(**obj_kwgs)
