...
class SinglePassProcessor:
    def __init__(self, model_args: Any, water_lab: Tuple[int, int, int]):
        ...
        # ADDED: Store list of thresholds
    ...
    def reconstruct_full_mask(
        ...
        # For single pass, "post-merge" blur is the only relevant blur.
        # This now returns a confidence map, not a binary mask
        final_water_confidence_map = self.model.reconstruct_full_mask_gpu(
            results,
            slice_info,
            return_gpu_tensor=True, # Always get the tensor back
        ...
        # If only calculating coverage, process thresholds and return dict
        if coverage_only:
            coverage_results = {}
        ...
        # Otherwise, create a dictionary of binary masks
        mask_results = {}
        for thresh in self.kelp_confidence_thresholds:
...
class HierarchicalProcessor:
    ...
    def _merge_masks_gpu(
        ...
        """
        Merges fine and coarse confidence maps directly in the 'water confidence' space.
        The function's output is always a final, merged water confidence map.
        """
        if self.merge_strategy == 'kelp-based':
            # Optimistic for kelp -> Pessimistic for water.
            # Takes the MINIMUM water confidence from either pass.
            merged_water_confidence = torch.min(fine_pass_confidence_map, coarse_pass_confidence_map)
        
        elif self.merge_strategy == 'average':
            # A simple average of the two water confidence maps.
            merged_water_confidence = (fine_pass_confidence_map + coarse_pass_confidence_map) / 2.0
        
        elif self.merge_strategy == 'water-based':
            # Pessimistic for kelp -> Optimistic for water.
            # Takes the MAXIMUM water confidence from either pass.
            merged_water_confidence = torch.max(fine_pass_confidence_map, coarse_pass_confidence_map)
        
        else:
            raise ValueError(f"Unknown merge strategy: {self.merge_strategy}")

        # The optional color validation logic is applied to the merged water map.
        if self.use_color_validation:
            image_lab_tensor = image_lab_tensor_cpu.to(self.device)
            
            # Identify pixels that are confidently kelp based on the new merged map
            # We use the first threshold as a representative value for this check
            ...
            if kelp_pixels_lab.numel() > 0:
                ...
                # Check for invalid kelp pixels (too bright or too close to water color)
                ...
                # Penalize invalid pixels by setting their confidence to 'definitely water' (1.0)
        ...
        # The function now directly returns the final merged water confidence map.
