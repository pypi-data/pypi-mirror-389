...
class SAHISAM:
    ...
    def _apply_gaussian_blur_gpu(self, tensor: torch.Tensor, kernel_size: int, sigma: float) -> torch.Tensor:
        """Applies Gaussian blur to a tensor on the GPU."""
        # Add batch and channel dimensions for torchvision
    ...
    def reconstruct_full_mask_gpu(
        ...
        # This part is now handled by the processor to allow multiple thresholds
        if coverage_only:
            # This path is less used now but kept for simple cases
