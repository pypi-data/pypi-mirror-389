...
# For c parameters: [0, 1) range -> 16-bit integers (sufficient precision)
# For z0 parameters: larger range -> 32-bit floats (preserve precision)
C_SCALE = 2**16  # 65536 for 16-bit unsigned integers (c is in [0, 1))
C_RANGE = 1.0


def quantize_c_float(value: float) -> int:
    """
    Quantize a c parameter float to a 16-bit unsigned integer.
    ...
    Returns:
        Quantized integer value (0-65535)
    """
    # Clamp to [0, 1) range and scale
    clamped = max(0.0, min(0.999999, value))  # Avoid 1.0
    return int(clamped * C_SCALE)


def dequantize_c_float(value: int) -> float:
    """
    Dequantize a 16-bit integer back to c float.
    
    Args:
        value: Quantized integer value (0-65535)
...
def optimize_fractal_params(fractal_params: list) -> Dict[str, Any]:
    ...
    """
    optimized = {
        "c_real": [],  # 16-bit unsigned ints
        "c_imag": [],  # 16-bit unsigned ints
    ...
    for param in fractal_params:
        ...
        optimized["c_real"].append(quantize_c_float(param["c"][0]))
        optimized["c_imag"].append(quantize_c_float(param["c"][1]))
        # Keep z0 as 32-bit floats (they can be any float64 value)
        optimized["z0_real"].append(float(param["z0"][0]))
        optimized["z0_imag"].append(float(param["z0"][1]))
...
def serialize_encrypted_data(encrypted_dict: Dict, compress: bool = True) -> bytes:
    ...
    # Apply compression if requested (use level 9 for maximum compression)
    if compress:
        packed = zlib.compress(packed, level=9)
