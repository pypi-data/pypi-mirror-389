...
def _encode_bytes_batch_numba(
    ...
    for i in prange(num_bytes):
        ...
        c_r = ((byte_val / 256.0) + key_offset) % 1.0
        c_i = key_offset_imag
        
        # Get z0 from key
        z0_r = key_val_z0_real
        z0_i = key_val_z0_imag
...
def _encode_bytes_batch_vectorized(
    ...
    if NUMBA_AVAILABLE:
        ...
        c_real = ((bytes_float / 256.0) + key_offsets) % 1.0
        c_imag = key_offsets_imag.copy()
        z0_real = key_vals_z0_real.copy()
        z0_imag = key_vals_z0_imag.copy()
...
class FractalEncoder:
    ...
    def _encode_parallel(
        ...
        # Note: _encode_bytes_batch must be a top-level function for pickling
