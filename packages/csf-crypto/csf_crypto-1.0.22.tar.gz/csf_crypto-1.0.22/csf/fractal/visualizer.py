...
class FractalSignature:
    ...
    def __init__(self, width: int = 8, height: int = 8):
        ...
        if USE_NOVERRAZ:
            ...
            self.julia = JuliaClass(iterations=5)
            self.noverraz = None
    ...
    def generate_signature(
        ...
        # OPTIMIZATION: 2 passes instead of 4 (still 2x faster than original)
        # Reduced iterations: 256 → 50 per pass (still 5x faster than 256)
        # CRITICAL: Signatures need multiple passes for uniqueness and security
        from csf.core.fractal_hash import FractalHash
        
        # Pass 1: Hash the input
        hasher1 = FractalHash(output_length=16, iterations=50)
        hash1 = hasher1.hash(combined_input_bytes)
        
        # Pass 2: Hash with reversed input + pass 1 result for better mixing
        hasher2 = FractalHash(output_length=16, iterations=50)
        ...
        # Final pass: Hash the combined result for final 32 bytes
        hasher_final = FractalHash(output_length=32, iterations=50)
