...
DEFAULT_LOG_DIR = Path(r"C:\nginx\logs") if is_windows() else Path("/var/log/nginx")
...
DEFAULT_CACHE_BASE = Path(r"C:\nginx\cache") if is_windows() else Path("/var/cache/nginx/sites")
...
NginxManager(sites_available, sites_enabled, cache_base, templates_dir, dry_run)
...
validate_upstream(upstream, fallback, timeout, use_http) -> str
    Validate upstream reachability with fallback support.
reload_dns() -> None
...
def _find_server_blocks(self, text: str) -> List[Tuple[int, int, str]]:
...
def _server_has_port(self, block: str, port: int) -> bool:
...
class NginxManager:
    ...
    def setup_site_logs(self, site: Site) -> Tuple[Path, Path]:
        """Setup access/error logs for site."""
        ...
        try:
            set_owner(str(access), "www-data" if is_linux() else None)
            set_owner(str(error), "www-data" if is_linux() else None)
        except Exception as e:
            log.debug(f"Failed to set log owner: {e}")
        return access, error

    def ensure_cache_dir(self, site: Site) -> Path:
        """Ensure per-site cache directory exists."""
        cache_dir = self.cache_base / site.name
        ensure_dir(str(cache_dir))
        if not self.dry_run:
            set_mode(str(cache_dir), 0o755)
            try:
                set_owner(str(cache_dir), "www-data" if is_linux() else None)
            except Exception as e:
                log.debug(f"Failed to set cache dir owner: {e}")
        return cache_dir
    ...
    def create_site(
        ...
        default_tpl: Union[str, Path] = "default.conf",
    ) -> Path:
        ...
        if file_exists(str(conf_path)) and not site.force:
            ...
            return conf_path
        upstream = self.validate_upstream(site.upstream or "")
        tpl = serve_tpl if site.is_serve else proxy_tpl
        context = {
            "SITE": site.name,
            "IP_ADDRESS": upstream,
            "ROOT_PATH": upstream if site.is_serve else "",
            "CLIENT_MAX_BODY_SIZE": "10M",  # default, upload flag will override
        }
        try:
            rendered = self.render_template(default_tpl, context) + "\n" + self.render_template(tpl, context)
        except NginxOpsError as e:
            raise NginxOpsError(f"Failed to render base templates for {site.name}: {e}")
        self.write_site_config(site, rendered)
        self.setup_site_logs(site)
        # Sync flags (includes locations and upload override)
...
def _apply_snippet(self, conf_path: Path, snippet: str, placement: str = "server_443_out", 
    ...
    blocks = self._find_server_blocks(txt)
    placed = False
    ...
    for start, end, block in blocks:
        if not self._server_has_port(block, port):
            continue
...
def _insert_snippet_into_block(self, block: str, snippet: str, placement: str) -> str:
    ...
    def _set_client_max_body_size(self, conf_path: Path, val: str) -> None:
        ...
        for start, endpos, block in blocks:
            if not regex_search(block, r"listen\s+[^;]*\b443\b"):
                continue
...
def _get_flag_info(self, flag: str, site: Site) -> Dict[str, Any]:
    """Get flag configuration including template meta."""
    flag_info = {
        "name": flag,
        "placement": "server_443_out",
        "begin_marker": f"# ---- BEGIN {flag.upper()} {site.name} ----",
        "end_marker": f"# ---- END {flag.upper()} {site.name} ----"
    }
    
    # Find template
    template_path = self.flags_dir / f"{flag}.conf"
    if template_path.exists():
        meta = self.parse_template_meta(template_path)
        flag_info.update(meta)
        flag_info["template_content"] = self._strip_meta_lines(template_path.read_text(encoding="utf-8"))
    
    return flag_info

def apply_flag_to_site(self, site: Site, flag: str, value: Optional[str] = None) -> None:
    """Apply flag with template meta support."""
    flag_info = self._get_flag_info(flag, site)
    
    # Special handling for different flag types
    if flag == "cache":
        self._apply_cache_flag(site, flag_info, value)
    elif flag == "upload":
        self._apply_upload_flag(site, value)
    elif flag == "dns":
        self._apply_dns_flag(site, flag_info)
    elif flag.startswith("/"):
        self._apply_location_flag(site, flag, value, flag_info)
    else:
        self._apply_generic_flag(site, flag, flag_info, value)

def _apply_cache_flag(self, site: Site, flag_info: Dict[str, Any], value: Optional[str]) -> None:
    """Apply cache flag with combined file support."""
    cache_dir = self.ensure_cache_dir(site)
    
    # Render main cache snippet
    context = {"SITE": site.name, "CACHE_DIR": str(cache_dir)}
    snippet = render_jinja(flag_info["template_content"], context)
    ...
    self._apply_snippet(
        conf_path, 
        snippet, 
        placement=flag_info.get("placement", "server_443_out_after_location"),
        begin_marker=flag_info["begin_marker"],
        end_marker=flag_info["end_marker"]
    )
    
    # Handle cache-path combined file
    cache_path_tpl = self.flags_dir / "cache-path.conf"
    if cache_path_tpl.exists():
        cache_path_meta = self.parse_template_meta(cache_path_tpl)
        cache_path_content = self._strip_meta_lines(cache_path_tpl.read_text(encoding="utf-8"))
        entry = render_jinja(cache_path_content, context)
        
        begin_marker = cache_path_meta.get("begin", "# ---- CACHE-PATH-BEGIN ----")
        end_marker = cache_path_meta.get("end", "# ---- CACHE-PATH-END ----")
        
        self._append_to_combined_file(self.cache_combined, entry, begin_marker, end_marker)
...
def sync_flags_for_site(self, site: Site) -> None:
    ...
    for flag in desired_flags:
        ...
        self.apply_flag_to_site(site, flag, value)
    
    # Remove unwanted known flags
    for flag in known_flags:
        if flag not in desired_flags:
            self.remove_flag_from_site(site.name, flag)
    ...
    if conf_path.exists():
        ...
        for loc in current_locations:
            if loc != "/" and loc not in desired_locations:
                self.remove_flag_from_site(site.name, loc)
