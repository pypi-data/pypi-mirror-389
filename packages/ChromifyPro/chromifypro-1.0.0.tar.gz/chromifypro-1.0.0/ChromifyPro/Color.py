...
from .utils import TEMPLATE_COLORS
#import termcolor
...
class Color:
    ...
    def get_palette(cls, name: str):
        """Return list of Color objects from a palette name."""
        hex_list = TEMPLATE_COLORS.get(name, [])
        return [cls(hex_code) for hex_code in hex_list]

    @classmethod
    def list_palettes(cls):
        return list(TEMPLATE_COLORS.keys())

    # --- Static methods for template loading ---
    @staticmethod
    def load_from_zip(zip_path: str) -> List["Color"]:
        """
        Load templates from a user-specified ZIP file and return a flat list of Color objects.
        This updates the global TEMPLATE_COLORS.
        """
        utils_load_from_zip(zip_path)
        utils_load_from_template()
        all_colors: List[Color] = []
        for hex_list in TEMPLATE_COLORS.values():
            all_colors.extend(Color(hex_code) for hex_code in hex_list)
        return all_colors

    @staticmethod
    def load_from_template() -> List["Color"]:
        """
        Auto-load templates from the default 'chromify_template' folder.
        Returns all Color objects as a flat list.
        """
        utils_load_from_template()
        all_colors: List[Color] = []
        for hex_list in TEMPLATE_COLORS.values():
