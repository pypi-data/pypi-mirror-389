...
class ASRClient:
    ...
    def __init__(
        ...
        self.ws_use_ssl = ws_use_ssl if ws_use_ssl is not None else cfg.use_https
        ...
        self.ws_url = ws_url or self._compose_ws_url(self.base_url, self.ws_endpoint, self.ws_use_ssl)
        self._client = httpx.Client(timeout=self.timeout, verify=self.verify_ssl, headers=self._default_headers())
        self._aclient = httpx.AsyncClient(timeout=self.timeout, verify=self.verify_ssl, headers=self._default_headers())
    ...
    def _compose_ws_url(self, base_url: str, endpoint: str, force_ssl: bool) -> str:
        parsed = urlparse(base_url)
        scheme = "wss" if (force_ssl or parsed.scheme == "https") else "ws"
    ...
    def _build_ws_client(
        ...
        *,
        ...
        target_use_ssl = self.ws_use_ssl if use_ssl is None else use_ssl
        if url is not None:
            target_url = url
        elif use_ssl is not None:
            target_url = self._compose_ws_url(self.base_url, self.ws_endpoint, target_use_ssl)
        else:
    ...
    def stream_transcribe_file(
        ...
        *,
        ...
        ws_client = self._build_ws_client(url=ws_url, token=token, use_ssl=use_ssl)
