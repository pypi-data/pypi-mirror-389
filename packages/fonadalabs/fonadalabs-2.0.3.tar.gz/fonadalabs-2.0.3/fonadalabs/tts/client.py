...
class TTSClient:
    ...
    def __init__(self, api_key: Optional[str] = None, timeout: int = 300, verify_ssl: bool = True):
        """
        ...
        Args:
            ...
            verify_ssl: Verify SSL certificates (default: True). Set to False for self-signed certificates (not recommended for production)
        ...
        # Users cannot override it for security reasons
        import os
        self.base_url = os.getenv("FONADALABS_API_URL", "https://kb.fonada.ai").rstrip("/")
    ...
    async def _get_http_client(self) -> httpx.AsyncClient:
        ...
        if self.http_client is None:
            self.http_client = httpx.AsyncClient(timeout=self.timeout, verify=self.verify_ssl)
    ...
    async def generate_audio_ws_async(
        ...
        1. Client sends: {"api_key": "...", "text": "...", "voice_id": "..."}
        ...
        try:
            ...
            async with websockets.connect(ws_endpoint, max_size=16 * 1024 * 1024, ping_interval=20, ssl=ssl_context) as websocket:                
                # Send TTS request (current API format)
                request_data = {
                    "api_key": self.api_key,
                    "text": text,
                    "voice_id": voice  # Note: voice_id not voice
                }
