...
class FileFlowHandleV2:

    def __init__(self, not_allow_file_type_list=None, postgres_uri=None, database_name=None):
        self.postgres_uri = postgres_uri or os.environ['PG_URI_AIRFLOW12_USER_NEWSFEEDSITE']
        ...
        self.postgres_engine = create_engine(f"{self.postgres_uri}/{self.databases}", connect_args={"sslmode": "require"})
    ...
    def _validate_params(self, params: Dict[str, Any]) -> Tuple[bool, str]:
        required_fields = ['start_stage', 'target_stage', 'tags']
    ...
    def _update_low_priority_processing_data(self, meta_id_set, params):
        meta_id_list = list(meta_id_set)
        if not meta_id_list:
            return
        with self.session_scope() as session:
            stmt_update_op_meta = update(self.table_op_meta).where(
                self.table_op_meta.c.id.in_(meta_id_list)
            ).values(
                tags=params['tags'],
                priority=params['priority'],
                updated_at=datetime.datetime.now(datetime.timezone.utc)
            )
            session.execute(stmt_update_op_meta)
            logger.info(f"Low-priority historical data updated: {len(meta_id_list)}")
    ...
    def _file_flow_entry_point_internal(
        ...
        for record in records:
            ...
            if len(meta_batch) >= batch_size:
                self._update_low_priority_processing_data(exist_data_ids, params)
        ...
        return True, [], f"Data successfully queued. inserted or updated count: {count}"
    ...
    def exit_point(
        ...
        :param tags: 查询过滤条件 ['test_tag_name', 'xxxx']
        ...
        conditions = [op_meta.c.status.in_(['success', 'failed'])]
        ...
        if report_id:
            conditions.append(op_meta.c.id == report_id)
        ...
        with self.session_scope() as session:
            ...
            for row in result:
                ...
                if meta_id not in meta_map:
                    ...
                    meta_fields['x_attachments_pdf'] = []
                    meta_map[meta_id] = meta_fields

                if attachment_fields['category'] == 'x_attachments_pdf':
                    meta_map[meta_id]['x_attachments_pdf'].append(attachment_fields)
                elif attachment_fields['category'] == 'x_attachments':
                    meta_map[meta_id]['x_attachments'].append(attachment_fields)
                else:
                    raise Exception("attachment_fields['category'] must be 'x_attachments_pdf' or 'x_attachments'")
            ...
            for item in result_data:
                ...
                try:
                    status = item['status']
                    meta_id = item['id']
                    if status == 'success' and item['current_stage'] == item['target_stage']:
