...
class CacheManager:
    ...
    def _cache_write(self, table_name: str, con: Connection, data: bytes) -> None:
        ...
        # MySQL emits a warning, which the mysqlconnector raises as an exception.
        # However the data is inserted correctly.
        # (The pymysql connector inserts binary data without issue.)
        try:
            con.execute(stmt)
        except sqlalchemy.exc.DatabaseError as err:
            # When mysql.connector emits a DatabaseError (e.g., mysql.connector.errors.DatabaseError(errno=1300, msg='Invalid utf8mb4 character string')),
            # SQLAlchemy wraps it and raises it as a sqlalchemy.exc.DatabaseError.
            # The original MySQLConnector exception is in the .orig attribute:
