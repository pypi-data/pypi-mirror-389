...
Fixes:
- Namespaced tokens like [[id:t:72ca28_zh]] or [[id:s:t:72ca28_zh]]
  now canonicalize to the last colon segment ("72ca28_zh") for lookup.
- Preserves original line breaks (LF/CRLF). No more merging lines after annotation.
"""
...
# New config keys (added dynamically to your plugin's config_scheme)
...
# NOTE: we no longer let the "tail" consume the line break; per-line EOL is handled separately.
_VW_TOKEN_RE = re.compile(
    r"(\[\[\s*id:([a-zA-Z0-9_:-]{1,})\s*\]\])"
    r"(?P<tail>[^\S\r\n]*(?:<!--\s*vwidref-title:.*?-->)?)"
)

_FENCE_RE = re.compile(r"^\s*```")                        # fenced code start/end
_COMMENT_PREFIX = "vwidref-title:"

_EOL_RE = re.compile(r"(?:\r\n|\r|\n)$")                  # capture exact line ending
...
def _canonicalize_id(raw: str) -> str:
    """
    Convert a possibly namespaced token id (e.g., 't:72ca28_zh' or 's:t:72ca28_zh')
    into the canonical page id by taking the **LAST** colon-segment.
    """
    if not raw:
        return raw
...
def _build_id_title_map(docs_dir: pathlib.Path,
    ...
    """
    Build { canonical_id(lower) -> title or None } from page front-matter.
    The stored key is canonicalized to the last colon segment.
    """
...
def _split_eol(s: str) -> Tuple[str, str]:
    """
    Split a single line into (content_without_eol, eol).
    Preserves exact EOL so we can restore it after substitutions.
    """
    m = _EOL_RE.search(s)
    if not m:
        return s, ""
...
def _annotate_line_content(line_content: str, id2title: Dict[str, Optional[str]]) -> str:
    """
    Add/update our managed comment after each [[id:...]] inside *content without EOL*.
    Avoid duplicate *vwidref* comments; refresh when title changes.
    """
    def repl(m):
        token = m.group(1)             # the [[id:...]] text
        raw_id = (m.group(2) or "")    # may include namespaces
        tail = m.group("tail") or ""   # only horizontal whitespace + optional our comment

        canon_id = _canonicalize_id(raw_id)
        ...
        if _COMMENT_PREFIX in tail:
            # Update our managed comment in place if stale
        ...
        # Different (non-vwidref) comment or just spaces present -> append ours too
...
def _annotate_file(md_path: pathlib.Path, id2title: Dict[str, Optional[str]]) -> bool:
    ...
    for ln in lines:
        # Preserve exact EOL before any regex work
        content, eol = _split_eol(ln)

        # Toggle fence based on the *content* (EOL removed)
...
def _extend_config_scheme(original_scheme):
    """
    Append new config keys to the existing plugin's config_scheme without breaking it.
    """
    try:
        from mkdocs.config import config_options
...
def _find_and_wrap_target_class():
    ...
    globals()[name] = wrapped  # export wrapped subclass

# Activate wrapper at import
