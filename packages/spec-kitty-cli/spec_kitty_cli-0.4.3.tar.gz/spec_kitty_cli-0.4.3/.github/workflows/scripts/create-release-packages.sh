...
rewrite_paths() {
  sed -E \
    -e 's@(/?)memory/@.kittify/memory/@g' \
    -e 's@(/?)scripts/@.kittify/scripts/@g' \
    -e 's@(/?)templates/@.kittify/templates/@g'
}
...
generate_commands() {
  ...
  for template in templates/commands/*.md; do
    ...
    case $ext in
      toml)
        { echo "description = \"$description\""; echo; echo "prompt = \"\"\""; echo "$body"; echo "\"\"\""; } > "$output_dir/spec-kitty.$name.$ext" ;;
      md)
        echo "$body" > "$output_dir/spec-kitty.$name.$ext" ;;
      prompt.md)
        echo "$body" > "$output_dir/spec-kitty.$name.$ext" ;;
    esac
  done
...
build_variant() {
  ...
  SPEC_DIR="$base_dir/.kittify"
  mkdir -p "$SPEC_DIR"
  
  [[ -d memory ]] && { cp -r memory "$SPEC_DIR/"; echo "Copied memory -> .kittify"; }
  ...
  if [[ -d scripts ]]; then
    ...
    case $script in
      sh)
        [[ -d scripts/bash ]] && { cp -r scripts/bash "$SPEC_DIR/scripts/"; echo "Copied scripts/bash -> .kittify/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
      ps)
        [[ -d scripts/powershell ]] && { cp -r scripts/powershell "$SPEC_DIR/scripts/"; echo "Copied scripts/powershell -> .kittify/scripts"; }
        # Copy any script files that aren't in variant-specific directories
        find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
        ;;
    esac
