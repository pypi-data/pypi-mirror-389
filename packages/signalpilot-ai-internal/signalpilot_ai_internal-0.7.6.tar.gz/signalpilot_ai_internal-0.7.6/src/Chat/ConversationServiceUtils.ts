...
export class ConversationServiceUtils {
  ...
  static async processSingleToolCall(
    ...
    if (toolName === 'terminal-execute_command') {
      const shouldContinue = await ConversationServiceUtils.handleTerminalCommandApproval(
        context,
        content
      );
      if (!shouldContinue) return false;
  ...
  static async handleTerminalCommandApproval(
    ...
    const shouldRun = await context.codeConfirmationDialog.showConfirmation(
      command
    );
