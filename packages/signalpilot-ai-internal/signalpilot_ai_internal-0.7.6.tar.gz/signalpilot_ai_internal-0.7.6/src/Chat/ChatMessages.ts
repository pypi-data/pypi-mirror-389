...
export class ChatMessages {
  ...
  private renderUserMessage(message: string, checkpoint?: ICheckpoint): void {
    ...
    const shouldHideMessage = isLauncherActive && isWelcomeTrigger;
    ...
    if (shouldHideMessage) {
      ...
      console.log('[ChatMessages] Hiding welcome trigger message in launcher mode (from history)');
    }
  ...
  addUserMessage(message: string): void {
    ...
    if (shouldHideMessage) {
      messageElement.style.display = 'none';
      console.log('[ChatMessages] Hiding welcome trigger message in launcher mode');
    }
  ...
  private addTerminalOutputDisplay(container: HTMLElement, result: any): void {
    try {
      ...
      headerDiv.style.cssText = 'display: flex; align-items: center; gap: 10px; padding: 0 8px;';
      ...
      content.style.cssText = 'display: none; margin: 4px 8px 0 8px; padding: 8px; background: var(--jp-layout-color1); border-radius: 3px; overflow-x: auto; max-height: 300px; overflow-y: auto; font-family: var(--jp-code-font-family); font-size: 11px; line-height: 1.4; border-left: 2px solid var(--jp-border-color2);';
  ...
  addThinkingIndicator(): HTMLDivElement {
    ...
    const contentElement = document.createElement('div');
    contentElement.className = 'sage-ai-message-content sage-ai-thinking-content';
