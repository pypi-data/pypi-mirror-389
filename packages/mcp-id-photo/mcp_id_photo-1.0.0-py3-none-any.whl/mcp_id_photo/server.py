...
def make_id_photo(
    ...
    2) 将生成结果按目标尺寸进行裁剪——默认二寸-标准（413×531），若传入 `spec` 或 `size` 则按参数；
    ...
    - `size`（可选）：自定义像素尺寸，如 `295*413` 或 `413x579`。优先级：`size` > `spec`。
    ...
    try:
        if size:
            target_size = _parse_size_str(size)
        ...
        return {"success": False, "error": f"目标尺寸解析失败: {e}"}
    ...
    prompt = (
        f"主体一致性保持，将背景替换为{bg_prompt}，生成标准证件照，穿着规范、居中构图、肩部完整、自然光照、无阴影"
    )
    ...
    try:
        ...
        return {"success": False, "error": f"调用生成接口失败: {e}"}
    ...
    if not images:
        return {"success": False, "error": "未在响应中解析到图片结果"}
    ...
    try:
        ...
        return {"success": False, "error": f"下载或解析图片失败: {e}"}
    ...
    try:
        ...
        return {"success": False, "error": f"保存文件失败: {e}", "suggestion": f"请检查输出路径是否可写: {file_path}"}
    ...
    result = {
        ...
        "bg": bg,
        "spec": spec if spec else ("二寸-标准" if not size else None),
