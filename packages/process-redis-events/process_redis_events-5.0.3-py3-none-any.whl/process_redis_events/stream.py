...
from process_redis_events.types import Json
...
class Stream(Generic[T]):
    ...
    def __init__(
        ...
        self.dlq_stream_key = f"stream:dlq:{name}"
    ...
    async def _process_item(
        ...
        try:
            ...
            if not should_retry(new_attempts, item.data):
                # Move to dead letter queue and acknowledge
                try:
                    await self.redis.xadd(
                        name=self.dlq_stream_key,
                        fields={
                            ...
                            "consumerGroup": consumer_group,
                            "consumerId": heartbeat_manager.consumer_id,
                            "timestamp": str(
                                int(asyncio.get_event_loop().time() * 1000)
                            ),
                            "data": json.dumps(item.data),
                    ...
                    print(
                        f"Failed to add message to dead letter queue {self.dlq_stream_key}: {dlq_error}"
                    )
            # If should_retry returns True, leave message unacknowledged for retry
