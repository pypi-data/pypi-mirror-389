...
class D4RK_BotManager:
    def __init__(self, 
        ...
        if self.logger_bot_token:
            ...
            self.logger_bot_util.set_token(self.logger_bot_token,self.web_app_url)
    ...
    def get_least_busy_client(self):
        """Get the client with the least work load"""
        if not self.work_load:
            return None
            
        # Find the client with minimum tasks
        min_tasks = float('inf')
        least_busy_client = None
        
        for work_item in self.work_load:
            for client, tasks in work_item.items():
                if tasks < min_tasks:
                    min_tasks = tasks
                    least_busy_client = client
                    
        return least_busy_client

    @property
    def clients(self):
        """Property to access the client list"""
        return self.client_list

    def get_all_clients(self):
        """Get all clients including logger bot client if available"""
        all_clients = self.client_list.copy()
        logger_bot = self.logger_bot_client
        if logger_bot:
            # Only add if it's a Pyrogram client, not a python-telegram-bot Bot
            # The logger bot is a different type of client (python-telegram-bot)
            # We only want to include Pyrogram clients in the client list
            pass
        return all_clients

    @property
    def workload(self):
        """Property to access the work load list"""
        return self.work_load

    @property
    def logger_bot_client(self):
        """Property to access the logger bot client if available"""
        return self.get_logger_bot_client()

    def get_client(self, index):
        """Get a client by index"""
        if 0 <= index < len(self.client_list):
    ...
    def get_client_by_id(self, client_id):
        ...
        for client in self.client_list:
            if hasattr(client, 'me') and client.me and client.me.id == client_id:
                return client
        return None
