...
class D4RK_BotManager:
    ...
    async def _start_bot_instance(self, token: str):
        ...
        try:
            ...
            # Assuming bot_instance has a 'client' attribute that is the Pyrogram client
            if hasattr(bot_instance, 'client'):
                self.client_list.append(bot_instance.client)
                # Initialize work load for this client with 0 tasks
                self.work_load.append({bot_instance.client: 0})
