...
class D4RK_BotManager:
    def __init__(self, 
        ...
        if self.web_app_url:
            self.web_app_btn = WebAppInfo(url=f"https://{self.web_app_url}" if not 'https' in self.web_app_url else self.web_app_url)
        self.logger_bot_util = None
        if self.logger_bot_token:
            self.logger_bot_util = LoggerBotUtil()
            self.logger_bot_util.set_token(self.logger_bot_token,self.web_app_btn)
