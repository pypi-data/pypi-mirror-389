...
class SharedNumpy(np.ndarray):
    ...
    def _auto_grow(self, min_add_rows):
        """
        ...
        try:
            ...
            # Reinitialize index - handles all index file sizing
    ...
    def upsert(self, new_records, acquire=True):        
        ...
        try:
            ...
            if (self.count + nrec) >= threshold:
                ...
                # CRITICAL: Use the returned updated records object
                updated_records = self._auto_grow(needed)
                # Update table's records reference AND use it for the rest of this method
                self.table.records = updated_records
                self = updated_records
                # Get the FULL array (not just up to count) from the updated records
                # Must use super().__getitem__ to bypass the custom __getitem__ that slices to count
            ...
            if self.table.is_schemaless:
                loc = self.get_loc(new_records,acquire=False)  # ensure index is created
                vidx = loc!=-1
                if vidx.any():
                ...
                if not vidx.all():
                    # New records being inserted - set bson_ptr/size to 0
            ...
            # for schemaless table, store bson data
            if self.table.is_schemaless:                
                if dict_list:
    ...
    def store_dictionaries_batch(self, records, upsertpos, dict_list):
        ...
        No space reclamation - use defragment_bson() periodically to compact unused space.
        ...
        if table.bson_mmap is None:
            table._init_bson_mmap()
    ...
    def get_dict_list(self, records, acquire=True):
        """
        ...
        """
        if not self.table.is_schemaless or not records.size:
