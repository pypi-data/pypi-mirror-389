...
def list_metrics(
    ...
    if not metrics_dir.exists():
        ...
        click.echo("     dtk init-project my-project-name")
        return
...
def init_project(
    ...
    Examples:
        ...
        # Interactive mode
        dtk init-project my-project --interactive
        ...
        while batch_start < end_time:
            ...
            try:
                ...
                if datapoints:
                    ...
                    # Run detection on batch if not skipping
                    if not skip_detection and detectors_info:
                        ...
                        for detector_info in detectors_info:
                            try:
                                ...
                                # Run batch detection (much faster than per-point)
                                detection_results = detector.detect_batch(
                                    metric_name=config.name,
                                    datapoints=datapoints,
                                )

                                # Save all detections if enabled
                                if config.storage.params.get("save_detections", True):
                                    for detection in detection_results:
                                        try:
                                            ...
                                            logger.debug(f"Failed to save detection for {detection.timestamp}: {e}")

                            except Exception as e:
                                # Log but don't fail the entire load
                                logger.warning(f"Batch detection failed for detector {detector_config.id}: {e}")
