...
class Affine(Layer):
    ...
    def __init__(self,
        ...
        if b is None:
            ...
            if b.data.shape != (fan_out,):
                raise ValueError(f"Incompatible b shape {b.data.shape}; expected {(fan_out,)}")
            self.b = b

        _logger.debug(
            "Affine initialized: seed=%s, fan_in=%d, fan_out=%d, method=%s, W.shape=%s, b.shape=%s",
            self.seed, fan_in, fan_out, self.method, getattr(self.W, "shape", None), getattr(self.b, "shape", None)
        )
    ...
    def apply_state(self, *, tunable=(), buffers=None) -> None:
        ...
        Args:
            tunable: Iterable with two arrays ``(W, b)``. ``W`` may be shaped
        ...
        if tunable:
            if len(tunable) != 2:
                raise ValueError(f"Affine.apply_state expected 2 arrays (W, b); got {len(tunable)}")
