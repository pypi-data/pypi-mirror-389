import copy
import numpy as np
from extendedstim import MajoranaCode, PauliCode
...
def Code2Circuit(code,p_noise,p_measure,noise_model,cycle_number):
    ...
    if noise_model=='phenomenological':
        ...
        elif isinstance(code,PauliOperator):
            raise NotImplementedError
        else:
            raise NotImplementedError
...
def MajoranaCSSCode2CircuitLevelCircuit(code,p_noise,p_measure,cycle_number:int):
    ...
    ##  生成初始态
    majorana_state=stabilizers_x.tolist()+stabilizers_z.tolist()+logical_occupy+[MajoranaOperator([], [], 1) for _ in range(pauli_number)]
    pauli_state=[PauliOperator([],[],1) for _ in range(majorana_number)]+[PauliOperator([],[temp],1) for temp in range(pauli_number)]

    ##  ---生成线路---
    ##  强制初始化
    circuit = Circuit()
    circuit.append({"name":"FORCE",
    ...
    for i,logical_operator in enumerate(logical_occupy):
        ...
        observable_include.append(len(circuit.measurements)-1)
    for i,stabilizer in enumerate(stabilizers_x):
    ...
    for i,logical_operator in enumerate(logical_occupy):
        circuit.append({"name":"MPP","target":logical_operator})
        circuit.append({"name":"OBSERVABLE_INCLUDE","target":[len(circuit.measurements) - 1, observable_include[i]]})
...
def PauliCSSCode2CircuitLevelCircuit(code,p_noise,p_measure,cycle_number:int):
    ...
    ##  添加稳定子态
    pauli_state = []
    for i in range(len(stabilizers_x)):
        pauli_state.append(stabilizers_x[i])
    for i in range(len(stabilizers_z)):
        pauli_state.append(stabilizers_z[i])
    pauli_state_x=copy.deepcopy(pauli_state)
    pauli_state_z=copy.deepcopy(pauli_state)

    ##  添加逻辑态
    for i in range(len(logical_x)):
        pauli_state_x.append(logical_x[i])
    for i in range(len(logical_z)):
        pauli_state_z.append(logical_z[i])

    ##  添加辅助qubit态
    for i in range(data_number,stabilizer_number+data_number):
        pauli_state_x.append(PauliOperator([],[i],1))
        pauli_state_z.append(PauliOperator([],[i],1))

    ##  ---生成线路---
    ##  强制初始化
    circuit_x = Circuit()
    circuit_x.append({"name": "FORCE",
                      "majorana_state": [MajoranaOperator([],[],1)]*len(pauli_state_x),
                      'pauli_state': pauli_state_x,
                      'pauli_number': pauli_number,
                      'majorana_number': 0})
    circuit_z = Circuit()
    circuit_z.append({"name": "FORCE",
                      "majorana_state": [MajoranaOperator([],[],1)]*len(pauli_state_z),
                      'pauli_state': pauli_state_z,
                      'pauli_number': pauli_number,
                      'majorana_number': 0})
    ...
    for i in range(len(logical_x)):
        ...
        observable_include.append(len(circuit_x.measurements) - 1)
    for i, stabilizer in enumerate(stabilizers_x):
    ...
    for i in range(len(logical_x)):
        ...
        circuit_x.append({"name": "MPP", "target": logical_x[i]})
        circuit_z.append({"name": "OBSERVABLE_INCLUDE", "target": [len(circuit_z.measurements) - 1, observable_include[i]]})
        circuit_x.append({"name": "OBSERVABLE_INCLUDE", "target": [len(circuit_x.measurements) - 1, observable_include[i]]})
...
def syndrome_pauli_css_measurement_circuit(stabilizer, qubit_index, type, p_noise, p_measure):
    ...
    for j in range(len(occupy)):
        if type == 'X' or type == 'x':
            ...
            sequence.append({'name': 'H', 'target': occupy[j]})
            sequence.append({'name': 'DEPOLARIZE1', 'target': occupy[j], 'p': p_noise})
            sequence.append({'name': 'CX', 'target': [occupy[j], qubit_index]})
            sequence.append({'name': 'DEPOLARIZE1', 'target': [occupy[j],qubit_index], 'p': p_noise})
            sequence.append({'name': 'H', 'target': occupy[j]})
            sequence.append({'name': 'DEPOLARIZE1', 'target': occupy[j], 'p': p_noise})
