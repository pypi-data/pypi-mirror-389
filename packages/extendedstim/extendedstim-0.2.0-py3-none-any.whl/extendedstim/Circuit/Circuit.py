...
from extendedstim.tools import isinteger, islist, isoperator
...
class Circuit:
    ...
    def append(self, params):
        ...
        example={
            ...
            'target':[1,2],  # 操作作用的对象
            'p':0.1,  # 操作对应的概率，不一定有
            'index':2,  # 测量对应平台上的stabilizers的序号
            'majorana_state':[MajoranaOperator([],[],1)],  # 强制初始化的majorana state
            'pauli_state':[PauliOperator([],[],1)],  # 强制初始化的pauli state
            'pauli_number':10,  # 强制初始化的qubits数目
            'majorana_number':10,  # 强制初始化的fermionic sites数目
        }
        ...
        ##  添加single-fermionic-site上的噪声
        elif name == 'U_ERROR' or name == 'V_ERROR' or name == 'N_ERROR':
            self._sequence.append(params.copy())
            self.sequence.append(self._sequence[-1])

        ##  添加受控非门
        elif name == 'CX' or name == 'CNX' or name=='CNN':
            self._sequence.append(params.copy())
            self.sequence.append(self._sequence[-1])

        ##  添加交错门
        elif name=='B' or name=='braid':
            x,y=params['target']
        ...
        ##  添加single qubit and single fermionic site上的测量
            ...
            if islist(params['target']):
                for temp in params['target']:
                    dict_temp = copy.deepcopy(params)
                    dict_temp['target']=temp
                    self.append(dict_temp)
            elif isinteger(params['target']):
                dict_temp = copy.deepcopy(params)
                dict_temp['name'] = 'MPP'
                if name == 'MZ':
                    dict_temp['target']=PauliOperator([], [dict_temp["target"]], 1)
                elif name == 'MN':
                    dict_temp['target']=MajoranaOperator([dict_temp["target"]], [dict_temp["target"]], 1j)
                self.append(dict_temp)
        ...
        ##  强制初始化
            ...
            stabilizers_majorana = copy.deepcopy(params["majorana_state"])
            stabilizers_pauli = copy.deepcopy(params["pauli_state"])
        ...
        for i, gate in enumerate(self._sequence):
            ...
            if name in ['X','Y','Z','H','S','U','V','N']:
                ...
                if isinstance(target, int):
                    eval('platform.'+name+'(target)')
                elif isinstance(target,Iterable):
                    for qubit in target:
                        eval('platform.'+name+f'({qubit})')
            ...
            ##  执行重置
                ...
                if isinstance(target, int):
                    ...
                    for qubit in target:
                        platform.fermionic_reset(qubit)
            ...
            ##  执行误差门
                ...
                if isinstance(target, int):
                    ...
                    for qubit in target:
                        eval('platform.'+name.lower()+f'({qubit},{p})')
            elif name == 'M_ERROR':
                assert p is not None
                if np.random.rand() < p:
            ...
            ##  执行测量
                ...
                if isinstance(target, Iterable):
                    for op in target:
                        if 'index' in gate:
                            measurement_sample[flag_measurement] = platform.measure(op, gate['index'])
                        else:
                            measurement_sample[flag_measurement] = platform.measure(op)
                        flag_measurement += 1
                elif isinstance(target,PauliOperator) or isinstance(target,MajoranaOperator):
                    if 'index' in gate:
                    ...
                    raise ValueError("M_ERROR gate target must be consecutive")

            ##  执行强制初始化
            elif name=='FORCE':
                assert gate['majorana_state'] is not None
                assert gate['pauli_state'] is not None
                platform.initialize(self.majorana_number, self.pauli_number,gate['majorana_state'],gate['pauli_state'])
