...
def test_lexer_with_strings():
    ...
    assert len(tokens) == 10  # @, block_name, (, key, =, ^, value, ^, ), EOF
    assert tokens[0].type == TokenType.AT
    assert tokens[1].type == TokenType.IDENTIFIER
    assert tokens[1].value == "block_name"
    assert tokens[2].type == TokenType.LPAREN
    ...
    assert tokens[6].value == "value"
    assert tokens[7].type == TokenType.CARET
...
def test_lexer_with_multiline_strings():
    ...
    assert tokens[3].value == "key"
    assert tokens[4].type == TokenType.EQUALS
    assert tokens[5].type == TokenType.DOUBLE_CARET
    assert tokens[6].type == TokenType.STRING
    assert tokens[6].value == "multiline\nstring"
    assert tokens[7].type == TokenType.DOUBLE_CARET
