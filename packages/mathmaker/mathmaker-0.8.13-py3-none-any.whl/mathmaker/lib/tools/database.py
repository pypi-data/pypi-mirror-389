...
class IntspansProduct(object):
    ...
    def _random_draw_attempt(spans, failed_attempts, return_all=False,
        ...
        for i, span in enumerate(spans):
            ...
            if kwargs.get('rectangle', False) and i == 0:
                kwargs['nb2_neq'] = drawn_nb
    ...
    def _kw_conditions(self, wrap_in_AND=True, **kwargs):
        ...
        for kw in kwargs:
            if kw == "raw":
                ...
                if 'nb1' in self.allcols and 'nb2' in self.allcols:
...
def preprocess_int_pairs_tag(tag, qkw=None):
    ...
    # 'table_11' is a shortcut for a special range
        ...
        nb2list = ['11', '12', '13', '14', '15', '16', '17', '18', '21', '22',
                   '23', '24', '25', '26', '27', '31', '32', '33', '34', '35',
                   '36', '41', '42', '43', '44', '45', '51', '52', '53', '54',
                   '61', '62', '63', '71', '72', '81']
        if qkw.get('except_11×11', 'false') == 'true':
            # remove 11 in case we don't want to possibly draw 11×11
            nb2list.pop(0)
        d = {'nb2_in': nb2list,
             'nb1': '11'}
    elif tag == 'table_11_tens_hard':
        d = {'nb2_in': ['19', '28', '29', '37', '38', '39', '46', '47', '48',
...
def random_decimal_for_multi_10_100_1000(width: int) -> Decimal:
    """
    Return random Decimal matching expected 'width'.
    width must be in {1,2,3,4}.
    The result has its lowest possible rank at 10^-4 and highest possible
    rank at 10^2 depending on the random shift r in 1..4.
    """
    if width not in (1, 2, 3, 4):
        raise ValueError("width must be 1, 2, 3 or 4")

    n = width  # number of significant positions (span length)
    # d0 .. d_{n-1} digits, with d0 != 0 and d_{n-1} != 0
    digits = []
    digits.append(random.randint(1, 9))    # most significant digit (non-zero)
    if n >= 3:
        for _ in range(n - 2):
            digits.append(random.randint(0, 9))  # interior digits can be zero
    if n >= 2:
        # least significant digit (non-zero)
        digits.append(random.randint(1, 9))

    # Build integer value from digits
    val_int = 0
    for d in digits:
        val_int = val_int * 10 + d

    # choose random decimal shift r in 1..4 (places after decimal point)
    r = random.randint(1, 4)

    # construct Decimal safely
    dec = Decimal(val_int) / (Decimal(10) ** r)
    return dec
...
def generate_values(source_id):
    ...
    # decimal_and_10_100_1000_for_multi
    # or decimal_and_10_100_1000_for_divi
    elif source_id.startswith('decimal_and_10_100_1000_for'):
        box_10_100_1000 = [10, 100, 1000]
        result = set()
        for n in range(20):
            if not box_10_100_1000:
                box_10_100_1000 = [10, 100, 1000]
                random.shuffle(box_10_100_1000)
            chosen_10_100_1000 = box_10_100_1000.pop()
            width = random.randint(1, 4)
            deci = random_decimal_for_multi_10_100_1000(width)
            if source_id.endswith('divi'):
                deci = deci * chosen_10_100_1000
...
def generate_random_big_nn(position, max_digits=None):
    """
    Creates a natural number between position and 9,876,543,210, all digits are
    different;
    position gives the minimum value that the generated number must have.
    position can be 1, 10, 100, 1000, etc. up to 1,000,000,000
    """
    # Input validation
    if position not in [10**i for i in range(10)]:
        raise ValueError(
            "Position must be a power of 10 between 1 and 1,000,000,000")

    # Determine minimum number of digits needed
    min_digits = len(str(position))
    if max_digits is None or max_digits > 10:
        max_digits = 10  # Maximum possible with all different digits

    if min_digits > max_digits:
        max_digits = min_digits
        warnings.warn(f"Provided value for max_digits is too small for "
                      f"provided position value ({position}), "
                      f"autofix this using max_digits={min_digits} instead.",
                      UserWarning)

    # Randomly choose number of digits (at least min_digits)
    ...
    if available_digits[0] == 0:
        for i in range(1, len(available_digits)):
            if available_digits[i] != 0:
                available_digits[0], available_digits[i] \
                    = available_digits[i], available_digits[0]
                break

    # Take the first num_digits digits
    selected_digits = available_digits[:num_digits]
...
class mc_source(object):
    ...
    def next(self, source_id, q_id='', qkw=None, **kwargs):
        ...
        if tag_classification == 'natural_nb_tuples':
            log = settings.dbg_logger.getChild('db')
            nb_of_elts = \
                {'singletons': 1, 'pairs': 2, 'triples': 3, 'quadruples': 4,
                 'quintuples': 5,
            ...
            db_source = {1: shared.nnsingletons_source,
                         2: shared.nnpairs_source,
            ...
            return shared.signed_numbers_comparisons_source.next(**kwargs)
        elif tag_classification == 'coordinates_xy':
            return shared.coordinates_xy_source.next(**kwargs)
