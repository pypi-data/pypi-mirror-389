...
def request_handler(environ, start_response):
    ...
    try:
        # PATH has been defined at daemon initialization
        command = ['mathmaker', '--pdf', *optional_args, sheet_name]
        res = subprocess.run(command, stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE, check=True)
    except subprocess.CalledProcessError:
        app_logger.error(f'{log_header} 500 — mathmaker failed', exc_info=True)
        start_response('500 Internal Server Error',
                       [('Content-Type', 'text/html')])
        return [b'Error 500: something failed']
    except OSError:
        app_logger.error(f'{log_header} 500 — could not execute mathmaker',
                         exc_info=True)
        start_response('500 Internal Server Error',
                       [('Content-Type', 'text/html')])
        return [b'Error 500: failed to execute mathmaker']
    else:
