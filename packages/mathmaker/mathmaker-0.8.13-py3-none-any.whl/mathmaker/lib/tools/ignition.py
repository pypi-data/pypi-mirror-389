...
def check_dependency(name: str, goal: str, path_to: str,
    ...
    if name in ['lualatex']:
        ...
        try:
            ...
            logger.info(f'Check dependency for {name} failed!')
            warnings.warn(warning_msg(name=name, path_to=path_to,
                                      c_out=the_call_out, c_err=the_call_err,
...
def check_dependencies(xmllint='xmllint',
                       lualatex='lualatex',
...
def check_settings_consistency(language=None, od=None):
    ...
    #     'in the list of available fonts for lualatex. Will try to update '
    # the '\
    #     'list.'
    # if not check_font():
    #     log.warning(err_msg)
    #     err_msg = 'Unable to find the chosen font (' + str(font) + ')
    # after '\
    #         'the update of luatex available fonts. Check if it is
    # installed '\
