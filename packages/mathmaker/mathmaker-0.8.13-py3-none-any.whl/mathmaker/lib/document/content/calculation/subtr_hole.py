...
class sub_object(component.structure):
    ...
    def __init__(self, build_data, hidden=None,
        ...
        elif swap_complement in ['false', 'False', 'no']:
            swap_complement = False
        super().setup("minimal", **options)
        super().setup("numbers", nb=build_data, shuffle_nbs=False,
                      **options)
        super().setup("nb_variants", nb=build_data, enforce_decimal=False,
                      **options)
        ...
        if (self.nb_source.startswith('complements_to_')
            and self.nb_variant.startswith('decimal')
            and int(self.nb_source[len('complements_to_'):]) == 10 * self.nb1):
            self.nb1 //= 10
        if (self.nb_source.startswith('complements_to_')
            and swap_complement):
