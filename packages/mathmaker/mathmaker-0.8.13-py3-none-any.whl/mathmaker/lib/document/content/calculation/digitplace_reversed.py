...
from mathmaker.lib.tools.database import generate_random_big_nn
...
class sub_object(component.structure):
    ...
    def __init__(self, **options):
        ...
        if options.get('nb_variant', '') == 'big_nn':
            self.chosen_nb = generate_random_big_nn(pos, max_digits=8)
        else:
            self.chosen_nb = \
                generate_random_decimal_nb(position=pos, **options)[0]
        self.chosen_figure = (self.chosen_nb
                              % (pos * Decimal('10'))) // pos
        self.chosen_nb_str = Number((self.chosen_nb)).printed
    ...
    def q(self, **options):
        ...
        if self.preset == 'mental calculation':
            result = _(r'Digit of {position} in \num{{{number}}}?')
        else:
            result = _(r'Which figure matches the {position} in the number \
    \num{{{number}}}?')

        return result.format(number=self.chosen_nb_str,
                             position=_(str(
