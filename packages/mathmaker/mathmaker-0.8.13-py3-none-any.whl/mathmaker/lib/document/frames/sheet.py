...
from mathmakerlib.LaTeX import KNOWN_AMSMATH_SYMBOLS, KNOWN_MNSYMBOLS
...
class Sheet(object):
    ...
    def __str__(self):
        ...
        if any([s in result for s in KNOWN_TEXTCOMP_SYMBOLS]):
            pkg.append('textcomp')
        if any([s in result for s in KNOWN_MNSYMBOLS]):
            pkg.append('mnsymbol')
        macros = []
        if 'handhighlight' in result:
            macros.append('handhighlight')
...
\TextField[name=mark,width=3cm,height=0.2cm,value=Score :,
    ...
    def answers_title_to_str(self, variant='default'):
        ...
        if variant == 'slideshow' and self.answers_title != '':
            ...
            result += shared.machine.write_set_font_size_to('Large')
            emph = 'bold'
            if 'textbf' in self.answers_title:
                emph = ''
