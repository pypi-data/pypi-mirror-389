...
Purpose
-------
Provide the ``python -m security_dependency_pinning`` path mandated by the
project's packaging guidelines. The wrapper delegates to
:func:`security_dependency_pinning.cli.main` so that module execution mirrors the
...
def _module_main() -> int:
    ...
    Why
        Implements ``python -m security_dependency_pinning`` by delegating to the
