...
def test_get_project_metadata_fields():
    ...
    assert meta.name == "security_dependency_pinning"
    assert meta.slug == "security-dependency-pinning"
    assert meta.import_package == "security_dependency_pinning"
    assert meta.coverage_source == "src/security_dependency_pinning"
    assert meta.github_tarball_url("1.2.3").endswith("/bitranox/security_dependency_pinning/archive/refs/tags/v1.2.3.tar.gz")
    assert meta.version
    assert meta.summary
    assert meta.author_name
    assert meta.metadata_module.as_posix().endswith("src/security_dependency_pinning/__init__conf__.py")
