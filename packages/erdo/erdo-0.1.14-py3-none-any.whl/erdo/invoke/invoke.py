...
class InvokeResult:
    ...
    result: Optional[Any] = None  # The actual bot result (from messages)
...
class Invoke:
    ...
    def _extract_result_data(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Iterate through events in reverse to find the final result
        for event in reversed(events):
            ...
            # Look for the final result (status=success/error with output)
            if (
        ...
        # Now iterate forward to collect messages and steps
        for event in events:
            if not event:
                continue
            ...
            # Extract messages from message content
            if (
                isinstance(payload, str)
                and metadata.get("user_visibility") == "visible"
            ):
                message_content_id = metadata.get("message_content_id")
                if message_content_id:
                    # Try to construct message object
                    role = metadata.get("role", "assistant")

                    # If we're starting a new message or continuing the current one
                    if (
                        current_message is None
                        or current_message.get("id") != message_content_id
                    ):
                        # Start new message
                        current_message = {
                        ...
                        current_message["content"] += payload

        # Clean up messages (remove id field used for tracking)
        cleaned_messages = [
