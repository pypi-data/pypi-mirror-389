...
def create_plots_for_vars(
    ...
    for i, colname in enumerate(colnames.flat):
        ...
        if not grid.sample_grid_bounds is None and not grid.sample_area is None:
            ...
            # row_min, row_max = min([row for row,col in grid.ids_rndm_sample]), max([row for row,col in grid.ids_rndm_sample])
            # col_min, col_max = min([col for row,col in grid.ids_rndm_sample]), max([col for row,col in grid.ids_rndm_sample])
            X = _np_array([[not (ids_rndm_sample if type(ids_rndm_sample)==bool else (row,col) in ids_rndm_sample ) for col in  range(col_min,col_max+1)] for row in range(row_min,row_max+1)[::-1]])
