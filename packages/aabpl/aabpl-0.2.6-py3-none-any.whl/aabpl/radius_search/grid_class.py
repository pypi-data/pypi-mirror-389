...
class Grid(object):
    ...
    def __init__(
        ...
        if set_fixed_spacing:
            ...
            # create function to find optimal spacing and nest_depth:
            ...
            if n_pts_tgt is None:
                n_pts_tgt = n_pts_src
            print("TODO find optimal spacing for",r, n_pts_src, n_pts_tgt, (xmax-xmin, ymax-ymin))
            spacing = r/3
        ...
        self.ids = tuple(flatten_list([[(int(row_id), int(col_id)) for col_id in col_ids] for row_id in row_ids]))
        self.n_cells = len(self.ids)

        self.sample_area = None
        self.sample_grid_bounds = None
        self.ids_rndm_sample = set()

        x_centers = tuple([float((l+u)/2) for l,u in zip(x_steps[:-1],x_steps[1:])])
        y_centers = tuple([float((l+u)/2) for l,u in zip(y_steps[:-1],y_steps[1:])])
        def get_centroid_xy(row:int, col:int, x_centers:tuple=x_centers, y_centers:tuple=y_centers):
            return (x_centers[col], y_centers[row])
        def get_centroid_x(col:int, x_centers:tuple=x_centers):
            return x_centers[col]
        def get_centroid_y(row:int, y_centers:tuple=y_centers):
            return y_centers[row]
        ...
        self.row_col_to_centroid2 = {g_row_col:centroid for (g_row_col,centroid) in flatten_list([
                [((int(row_id),int(col_id)),(float(x_steps[col_id:col_id+2].mean()), float(y_steps[row_id:row_id+2].mean()))) for col_id in col_ids] 
                for row_id in row_ids]
                )}
        xmin = total_bounds.xmin
        ymin = total_bounds.ymin
        xmin_c = xmin + 0.5 * spacing
        ymin_c = ymin + 0.5 * spacing
        self.get_cell_centroid = lambda row, col: (float(xmin_c+col*spacing),float(ymin_c+row*spacing))
        # self.get_cell_centroid = lambda row, col, x_steps=x_steps, y_steps=y_steps: (float(x_steps[col:col+2].mean()), float(y_steps[row:row+2].mean()))
        self.get_cell_centroid2 = lambda row, col, x_steps=x_steps, y_steps=y_steps: (x_steps[col:col+2].mean(), y_steps[row:row+2].mean())
        self.get_cell_centroid3 = lambda row, col, x_steps=self.x_steps, y_steps=self.y_steps: (x_steps[col:col+2].mean(), y_steps[row:row+2].mean())
        # TODO replace row_col_to_centroid with function fetching it from 1d arrays to no clog up memory for fine grids
        ...
        self.row_col_to_bounds2 = {(row_id,col_id): bounds for ((row_id,col_id),bounds) in flatten_list([
                [((int(row_id),int(col_id)),((float(x_steps[col_id]), float(y_steps[row_id])), (float(x_steps[col_id+1]), float(y_steps[row_id+1])))) for col_id in col_ids] 
                for row_id in row_ids]
                )}
        self.get_cell_bounds = lambda row, col: ((float(xmin+col*spacing), float(ymin+row*spacing)), (float(xmin+(col+1)*spacing), float(ymin+(row+1)*spacing)))
        # self.get_cell_bounds = lambda row, col, x_steps=x_steps, y_steps=y_steps: ((float(x_steps[col]), float(y_steps[row])), (float(x_steps[col+1]), float(y_steps[row+1])))
        # self.get_cell_bounds2 = lambda row, col, x_steps=x_steps, y_steps=y_steps: ((x_steps[col], y_steps[row]), (x_steps[col+1], y_steps[row+1]))
        # self.get_cell_bounds3 = lambda row, col, x_steps=self.x_steps, y_steps=self.y_steps: ((x_steps[col], y_steps[row]), (x_steps[col+1], y_steps[row+1]))
        if not silent:
