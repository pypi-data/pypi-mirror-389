...
def install_dependencies_to_path_prod(
    ...
    # CRITICAL: Determine platform tag AFTER pip-compile, not before
    # pip-compile resolves transitive dependencies that may require platform-specific wheels
    # (e.g., pydantic -> pydantic_core, which needs manylinux wheels)
    # Without this step, we'd only analyze top-level packages and might incorrectly
    # use py3-none-any for packages that actually need platform-specific binaries
    requirements_for_platform = (
        str(compiled_requirements) if use_compiled else requirements_file
    )
    platform = determine_best_platform_tag(requirements_for_platform)
    print(f"Determined platform tag: {platform}")
