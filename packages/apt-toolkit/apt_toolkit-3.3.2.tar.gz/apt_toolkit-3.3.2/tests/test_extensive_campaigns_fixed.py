...
class TestExtensiveCampaignExecution(unittest.TestCase):
    ...
    def test_campaign_tools_availability(self):
        """Test that campaigns with execution scripts have access to tools"""
        campaigns = self.executor.list_available_campaigns()
        
        self.assertGreater(len(campaigns), 0, "Should find at least one campaign")
        
        # Check that campaigns with run scripts have tools directory
        for campaign in campaigns[:5]:  # Test first 5 campaigns
            ...
            # Only check campaigns that have tools directory
            if tools_path.exists():
                # Check for common tools
                common_tools = ['apt_persistence.py']  # This tool should be available
                for tool in common_tools:
                    tool_path = tools_path / tool
                    # Check if tool exists or is a symlink
                    if tool_path.exists() or tool_path.is_symlink():
                        self.assertTrue(True, f"Campaign {campaign} has tool {tool}")
                    else:
                        # It's OK if some tools are missing, as long as the framework works
                        pass
    ...
    def test_campaign_tool_integration(self):
        ...
        # Check for any campaign-specific tools
        campaign_specific_tools = list(campaign_tools.glob('*.py'))
        self.assertGreater(len(campaign_specific_tools), 0, 
                          "Defense contractor campaign should have some tools")
...
class TestCampaignToolIntegration(unittest.TestCase):
    ...
    def test_campaign_specific_tools(self):
        """Test that campaigns have appropriate tools"""
        campaigns_to_test = [
            'defense_contractor_campaign',
            'financial_institution_campaign',
            'healthcare_campaign',
            'government_agency_campaign'
        ]
        ...
        for campaign in campaigns_to_test:
            ...
            self.assertTrue(tools_path.exists(), 
                           f"Campaign {campaign} should have tools directory")
            
            # Check for any Python tools
            python_tools = list(tools_path.glob('*.py'))
            self.assertGreater(len(python_tools), 0, 
                              f"Campaign {campaign} should have Python tools")
    ...
    def test_campaign_execution_scripts(self):
        """Test that campaigns with execution scripts work properly"""
        campaigns_dir = ROOT_DIR / 'campaigns'
        
        # Only test campaigns that have run_campaign.py
            ...
            if campaign_path.is_dir():
                ...
                if run_script.exists():
                    ...
                    try:
                        ...
                        compile(content, str(run_script), 'exec')
                        self.assertTrue(True, f"Campaign {campaign_path.name} has valid run script")
                    except SyntaxError as e:
                        self.fail(f"Campaign {campaign_path.name} has invalid Python syntax: {e}")
