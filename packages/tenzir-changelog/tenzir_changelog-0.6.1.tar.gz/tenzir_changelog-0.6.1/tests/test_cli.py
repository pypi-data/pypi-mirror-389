...
def test_release_publish_uses_gh(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def fake_run(
        ...
        *,
        ...
        if len(args) >= 3 and args[1:3] == ["auth", "token"]:
    ...
    assert recorded_command["env"] is not None
    env = recorded_command["env"]
    assert isinstance(env, dict)
    assert env["GH_TOKEN"] == "test-token"
    # Ensure token lookup and existence check ran first.
    assert commands[0][:3] == ["/usr/bin/gh", "auth", "token"]
    assert commands[1][:3] == ["/usr/bin/gh", "release", "view"]
...
def test_release_publish_updates_existing_release(
    ...
    def fake_run(
        ...
        *,
        ...
        if len(args) >= 3 and args[1:3] == ["auth", "token"]:
            assert capture_output is True
            assert text is True
            return subprocess.CompletedProcess(args, 0, stdout="token-xyz\n")
        if len(args) >= 3 and args[1:3] == ["release", "view"]:
            return subprocess.CompletedProcess(args, 0)
        if len(args) >= 3 and args[1:3] == ["release", "edit"]:
            assert env is not None
            assert env.get("GH_TOKEN") == "token-xyz"
            return subprocess.CompletedProcess(args, 0)
    ...
    assert publish_result.exit_code == 0, publish_result.output
    assert calls[0][0][:3] == ["/usr/bin/gh", "auth", "token"]
    assert calls[1][0][:3] == ["/usr/bin/gh", "release", "view"]
    assert calls[2][0][:3] == ["/usr/bin/gh", "release", "edit"]
...
def test_release_publish_handles_abort(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def fake_run(
        ...
        *,
        ...
        if len(args) >= 3 and args[1:3] == ["auth", "token"]:
            return subprocess.CompletedProcess(args, 0, stdout="abort-token\n")
        if len(args) >= 3 and args[1:3] == ["release", "view"]:
            return subprocess.CompletedProcess(args, 0)
