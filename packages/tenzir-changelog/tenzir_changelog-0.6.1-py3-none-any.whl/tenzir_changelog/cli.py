...
def release_publish_cmd(
    ...
    env_overrides = github_token_environment(gh_path)
    gh_env = os.environ.copy()
    if env_overrides:
        gh_env.update(env_overrides)
    ...
    release_exists = _github_release_exists(config.repository, manifest.version, gh_path, gh_env)
    if release_exists:
        command = [
            gh_path,
            "release",
    ...
    try:
        subprocess.run(command, check=True, env=gh_env)
