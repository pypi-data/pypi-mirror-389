...
def setup_parser(args: list[str], subparsers: Any) -> Any:
    ...
    sub = cli_shared.add_command_subparser(subparsers, "token", "issue-fungible", "Issue a new fungible DCDT token.")
    ...
    sub = cli_shared.add_command_subparser(
        ...
        "token",
        "issue-semi-fungible",
        "Issue a new semi-fungible DCDT token.",
    )
    ...
    sub = cli_shared.add_command_subparser(
        subparsers, "token", "issue-non-fungible", "Issue a new non-fungible DCDT token (NFT)."
    )
    add_issuing_tokens_args(sub)
    add_common_args(args, sub)
    sub.set_defaults(func=issue_non_fungible)

    sub = cli_shared.add_command_subparser(subparsers, "token", "register-meta-dcdt", "Register a MetaDCDT token.")
    add_issuing_tokens_args(sub, with_num_decimals=True)
    add_common_args(args, sub)
    sub.set_defaults(func=register_meta_dcdt)
    ...
    sub = cli_shared.add_command_subparser(
        subparsers,
        "token",
        "set-special-role-meta-dcdt",
        "Set special roles on a meta-dcdt token for a user.",
    )
    _add_token_identifier_arg(sub)
    _add_user_arg(sub)
    _add_special_roles_args_for_meta_dcdt(sub)
    add_common_args(args, sub)
    sub.set_defaults(func=set_special_role_on_meta_dcdt)

    sub = cli_shared.add_command_subparser(
        subparsers,
        "token",
        "unset-special-role-meta-dcdt",
        "Unset special roles on a meta-dcdt token for a user.",
    )
    _add_token_identifier_arg(sub)
    _add_user_arg(sub)
    _add_special_roles_args_for_meta_dcdt(sub, with_nft_create=False)
    add_common_args(args, sub)
    sub.set_defaults(func=unset_special_role_on_meta_dcdt)
    ...
    sub.add_argument(
        "--denominator", type=int, default=None, help="The number of decimals, only needed when token type is META DCDT"
    )
    add_common_args(args, sub)
    ...
    sub.add_argument("--decimals", type=int, required=True, help="The number of decimals the meta dcdt will have")
    add_common_args(args, sub)
    sub.set_defaults(func=change_sft_to_meta_dcdt)
...
def _add_special_roles_args_for_fungible(sub: Any):
    ...
    sub.add_argument(
        ...
        action="store_true",
        default=False,
        help="role for dcdt transfer",
    )


def _add_special_roles_args_for_meta_dcdt(sub: Any, with_nft_create: bool = True):
    if with_nft_create:
        sub.add_argument(
            "--nft-create",
            action="store_true",
...
def register_meta_dcdt(args: Any):
    _ensure_args(args)
    ...
    controller = _initialize_controller(args)
    transaction = controller.create_transaction_for_registering_meta_dcdt(
        sender=sender,
        nonce=sender.nonce,
        token_name=args.token_name,
        token_ticker=args.token_ticker,
...
def set_special_role_on_fungible(args: Any):
    ...
    transaction = controller.create_transaction_for_setting_special_role_on_fungible_token(
        ...
        add_role_dcdt_transfer_role=args.dcdt_transfer_role,
        guardian=guardian_and_relayer_data.guardian_address,
        relayer=guardian_and_relayer_data.relayer_address,
        gas_limit=args.gas_limit,
        gas_price=args.gas_price,
...
def unset_special_role_on_fungible(args: Any):
    ...
    transaction = controller.create_transaction_for_unsetting_special_role_on_fungible_token(
        ...
        remove_role_dcdt_transfer_role=args.dcdt_transfer_role,
        guardian=guardian_and_relayer_data.guardian_address,
        relayer=guardian_and_relayer_data.relayer_address,
        gas_limit=args.gas_limit,
        gas_price=args.gas_price,
...
def set_special_role_on_semi_fungible(args: Any):
    ...
    transaction = controller.create_transaction_for_setting_special_role_on_semi_fungible_token(
        ...
        add_role_dcdt_modify_royalties=args.dcdt_modify_royalties,
        add_role_dcdt_set_new_uri=args.dcdt_set_new_uri,
        add_role_dcdt_modify_creator=args.dcdt_modify_creator,
...
def unset_special_role_on_semi_fungible(args: Any):
    ...
    transaction = controller.create_transaction_for_unsetting_special_role_on_semi_fungible_token(
        ...
        remove_role_dcdt_modify_royalties=args.dcdt_modify_royalties,
        remove_role_dcdt_set_new_uri=args.dcdt_set_new_uri,
        remove_role_dcdt_modify_creator=args.dcdt_modify_creator,
...
def set_special_role_on_meta_dcdt(args: Any):
    _ensure_args(args)
    ...
    controller = _initialize_controller(args)
    transaction = controller.create_transaction_for_setting_special_role_on_meta_dcdt(
        sender=sender,
        nonce=sender.nonce,
        token_identifier=args.token_identifier,
        user=Address.new_from_bech32(args.user),
...
def unset_special_role_on_meta_dcdt(args: Any):
    _ensure_args(args)
    ...
    controller = _initialize_controller(args)
    transaction = controller.create_transaction_for_unsetting_special_role_on_meta_dcdt(
        sender=sender,
        nonce=sender.nonce,
        token_identifier=args.token_identifier,
        user=Address.new_from_bech32(args.user),
...
def change_sft_to_meta_dcdt(args: Any):
    _ensure_args(args)
    ...
    controller = _initialize_controller(args)
    transaction = controller.create_transaction_for_changing_sft_to_meta_dcdt(
        sender=sender,
        nonce=sender.nonce,
