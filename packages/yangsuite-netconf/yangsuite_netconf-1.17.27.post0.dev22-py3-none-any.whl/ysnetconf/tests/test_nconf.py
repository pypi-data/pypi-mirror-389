...
"""
    ...
    def test_gen_rpc_api_confirm_commit_basic(self):
        """Confirm-commit options reflected in basic gentype output."""
        result = nconf.gen_rpc_api({
            'gentype': 'basic',
            'prefix_namespaces': 'always',
            'commit': 'add',
            'commit_options': {'confirmed': True, 'confirm_timeout': '45'},
            'cfgd': self.EDIT_CONFIG_CFGD,
        })

        self.assertRegex(result['rpc'], r'<(/?)(nc:)?confirmed')
        self.assertRegex(result['rpc'], r'<(nc:)?confirm-timeout>45</(nc:)?confirm-timeout>')

        commit_kwargs = result['ncrpcs'][1][1]
        self.assertEqual({'confirmed': True, 'timeout': 45}, commit_kwargs)

    def test_gen_rpc_api_confirm_commit_raw(self):
        """Confirm-commit options reflected in raw gentype output."""
        result = nconf.gen_rpc_api({
            'gentype': 'raw',
            'prefix_namespaces': 'always',
            'commit': 'add',
            'commit_options': {'confirmed': True, 'confirm_timeout': 30},
            'cfgd': self.EDIT_CONFIG_CFGD,
        })

        self.assertRegex(result['rpc'], r'<(nc:)?confirmed/>')
        self.assertRegex(result['rpc'], r'<(nc:)?confirm-timeout>30</(nc:)?confirm-timeout>')

    def test_gen_rpc_api_confirm_commit_script(self):
        """Confirm-commit options reflected in generated script output."""
        payload = json.dumps([{
            'commit': 'add',
            'commit_options': {'confirmed': True, 'confirm_timeout': 15},
            'cfgd': self.EDIT_CONFIG_CFGD,
        }])
        ...
        self.assertRegex(result['rpc'], r'<(nc:)?confirm-timeout>15</(nc:)?confirm-timeout>')

    def test_gen_rpc_api_confirm_commit_invalid_timeout(self):
        """Invalid confirm-timeout values raise RpcInputError."""
        with self.assertRaises(nconf.RpcInputError):
            nconf.gen_rpc_api({
                'gentype': 'basic',
                'prefix_namespaces': 'always',
                'commit': 'add',
                'commit_options': {'confirm_timeout': 'abc'},
                'cfgd': self.EDIT_CONFIG_CFGD,
            })

    def test_normalize_commit_options_converts_timeout(self):
        """String confirm-timeout values normalized to integers."""
        normalized = nconf._normalize_commit_options({
            'confirmed': True,
            'confirm_timeout': '10',
        })
        self.assertEqual({'confirmed': True, 'confirm_timeout': 10}, normalized)

    def test_build_commit_kwargs_maps_timeout(self):
        """Ncclient kwargs reflect confirmed commit options."""
        kwargs = nconf._build_commit_kwargs({
            'confirmed': True,
            'confirm_timeout': 20,
        })
        self.assertEqual({'confirmed': True, 'timeout': 20}, kwargs)

    def test_commit_rpc_raw_with_confirm_options(self):
        """Raw commit RPC includes confirmed commit elements."""
        rpc = nconf._commit_rpc('raw', 'always', {
            'confirmed': True,
            'confirm_timeout': 10,
        })
...
<nc:rpc xmlns:nc="urn:ietf:params:xml:ns:netconf:base:1.0" nc:message-id="102">
  <nc:commit>
    <nc:confirmed/>
    <nc:confirm-timeout>10</nc:confirm-timeout>
  </nc:commit>
</nc:rpc>
""", rpc)

    def test_commit_rpc_basic_with_confirm_options(self):
        """Basic commit RPC includes confirmed commit elements."""
        rpc = nconf._commit_rpc('basic', commit_options={
            'confirmed': True,
            'confirm_timeout': 5,
        })
        self.assertMultiLineEqual("""\
...
<nc:unlock xmlns:nc="urn:ietf:params:xml:ns:netconf:base:1.0">
  <nc:target>
    ...
    def test_gen_rpc_api_duplicate_xpaths(self):
        """Check if gen_rpc only returns RPCs for unique XPaths only"""
        dict_data = {
            'prefix_namespaces': 'always',
            'gentype': 'basic',
            'cfgd': {
                ...
                'modules': {
                    'openconfig-interfaces': {
                        ...
                        'configs': [{
                            'xpath': '/ocif:interfaces/ocif:interface/ocif:name',  # noqa: E501
                            'value': 'GigabitEthernet1'
                        }, {
                            'xpath': '/ocif:interfaces/ocif:interface/ocif:name',  # noqa: E501
                            'value': 'GigabitEthernet2'
                        }, {
                            'xpath': '/ocif:interfaces/ocif:interface/ocif:details',  # noqa: E501
                            'value': 'none'
                        }, {
                            'xpath': '/ocif:interfaces/ocif:interface/ocif:name',  # noqa: E501
        ...
        result = nconf.gen_rpc_api(dict_data)
        self.assertEqual(
            result['rpc'].replace(' ', '').replace('\n', ''),
            '''<nc:filter xmlns:nc="urn:ietf:params:xml:ns:netconf:base:1.0">
            <ocif:interfacesxmlns:ocif=
            "http://openconfig.net/yang/interfaces">
            <ocif:interface><ocif:name>GigabitEthernet1</ocif:name>
            <ocif:details>none</ocif:details>
            </ocif:interface></ocif:interfaces></nc:filter>'''
            .replace(' ', '')
            .replace('\n', '')
        )
