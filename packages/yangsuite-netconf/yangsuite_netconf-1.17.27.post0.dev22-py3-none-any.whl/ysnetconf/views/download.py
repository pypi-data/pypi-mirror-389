...
def download_schemas_to_repo(key, schemas, targetrepo):
    ...
    getsc.append(ident)
    # Specify the data modeling language the schema is written in
    schema_format = et.SubElement(getsc, 'format')
    schema_format.text = 'yang'
    ...
    try:
        ...
        with ncs:
            ...
            for sc in schemas:
                ...
                # Specify the version of the schema that is indended to be
                # downloaded
                vers = getsc.find('version')
                if revision and any(i.isdigit() for i in revision):
                    if vers is None:
                        vers = et.SubElement(getsc, 'version')
                    vers.text = revision
                else:
                    if vers is not None:
                        getsc.remove(vers)
