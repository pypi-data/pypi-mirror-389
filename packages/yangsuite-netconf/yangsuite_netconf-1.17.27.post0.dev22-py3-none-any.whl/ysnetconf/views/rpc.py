...
@login_required
@json_request
def get_ansible(request, jsondata):
    ...
    result = {'reply': "No RPC reply"}
    status_val = 500
    res = nconf.gen_ansible_api(jsondata)
    if res:
        result['reply'] = res['rpc']
        status_val = res['status']
    return JsonResponse(result, status=status_val)
...
def get_yang(request, yangset=None, modulenames=None):
    ...
    return render(request, 'ysnetconf/index.html', {
        'devices': devices,
        'yangset': yangset or '',
...
def list_datastores(request, key, **kwargs):
    """Get a dict of datastores applicable to a given device
    for each operation.
