        ...
        for nc_op, kwargs in rpc:
            ...
            try:
                ...
                if nc_op == 'edit-config':
                    ...
                    if commit_ok:
                        commit_kwargs = kwargs.copy()
                        if 'timeout' in commit_kwargs and commit_kwargs['timeout'] is not None:
                            commit_kwargs['timeout'] = str(commit_kwargs['timeout'])
                        ret = session_manager.commit(**commit_kwargs)
...
def _normalize_commit_options(commit_options):
    """Validate and normalize commit options payload."""
    if not commit_options:
        return {}

    normalized = {}

    if commit_options.get('confirmed'):
        normalized['confirmed'] = True

    timeout = commit_options.get('confirm_timeout')
    if timeout not in [None, '', False]:
        try:
            timeout_int = int(timeout)
        except (TypeError, ValueError):
            raise RpcInputError('commit_options.confirm_timeout',
                                timeout,
        ...
        normalized['confirm_timeout'] = timeout_int

    return normalized


def _add_commit_option_elements(commit_elem, commit_options):
    """Append confirmed commit options as child elements."""
    if not commit_options:
        return

    ns = QName(commit_elem).namespace

    def _subelement(tag):
        if ns:
            return et.SubElement(commit_elem, QName(ns, tag))
        return et.SubElement(commit_elem, tag)

    if commit_options.get('confirmed'):
        _subelement('confirmed')

    if 'confirm_timeout' in commit_options:
        timeout_elem = _subelement('confirm-timeout')
        timeout_elem.text = str(commit_options['confirm_timeout'])


def _build_commit_kwargs(commit_options):
    """Translate commit options into ncclient commit keyword arguments."""
    if not commit_options:
        return {}
    ...
    if 'confirm_timeout' in commit_options:
        kwargs['timeout'] = commit_options['confirm_timeout']
    return kwargs


def _commit_rpc(gentype, prefix_namespaces='always', commit_options=None):
    ...
    Returns:
      str: commit RPC
    """
    commit_options = commit_options or {}

    if gentype == 'raw':
        rpcbuilder = YSNetconfRPCBuilder(prefix_namespaces=prefix_namespaces)
        rpc = rpcbuilder.netconf_element('rpc')
        rpcbuilder.add_netconf_attr(rpc, 'message-id', str(102))
        commit_elem = rpcbuilder.netconf_element('commit')
        _add_commit_option_elements(commit_elem, commit_options)
        rpc.append(commit_elem)
    elif gentype == 'script':
        rpcbuilder = YSNetconfRPCBuilder(prefix_namespaces=prefix_namespaces)
        rpc = rpcbuilder.netconf_element('commit')
        _add_commit_option_elements(rpc, commit_options)
...
def gen_rpc_api(request):
    ...
    commit_options = _normalize_commit_options(request.get('commit_options'))
    prefix_namespaces = request.get('prefix_namespaces', 'minimal')
    ...
    def rem_dup_xpaths(modules):
        ...
        for module_name in result:
            ...
            for index in range(len(module['configs'])):
                ...
                elif config.get('nodetype', '') == 'leaf-list':
                    # leaf-list nodes can have multiple entries.
                    for entry in tmp_configs:
        ...
        if ('string_rpc' in request) and (request.get('string_rpc')):
            ...
            for rpc in cfgd:
                ...
                if (rpc.get('commit', False) and
                    ...
                    rpc_commit_options = _normalize_commit_options(
                        rpc.get('commit_options')
                    )
                    rpcs.append(
                        _commit_rpc(gentype,
                                    prefix_namespaces=prefix_namespaces,
                                    commit_options=rpc_commit_options))
            ...
            try:
                ...
                log.error(
                    "Problem forming return string for RPC\n{0}".format(
                        traceback.format_exc()))
        if commit and cfgd['dsstore'] == 'candidate':
            commit_kwargs = _build_commit_kwargs(commit_options)
            ncrpcs += [['commit', commit_kwargs]]
            rpctext += "\n" + _commit_rpc(gentype,
                                          prefix_namespaces=prefix_namespaces,
                                          commit_options=commit_options)
