...
def get_client():
    ...
    if _NEURALK_CLIENT is None:
        user_name = os.environ.get("NEURALK_USERNAME", None)
        if not user_name:
            user_name = input(
                "env variable 'NEURALK_USERNAME' is not defined.\nPlease enter user name: "
            ).strip()
        password = os.environ.get("NEURALK_PASSWORD", None)
        if not password:
            password = getpass.getpass(
                "env variable 'NEURALK_PASSWORD' is not defined.\nPlease enter password: "
            ).strip()
        _NEURALK_CLIENT = Neuralk(user_name, password)
