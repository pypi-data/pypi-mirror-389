...
def n_action(event: Event, database: DbProtocol) -> tuple[int, pydicom.Dataset | None]:  # noqa: PLR0911
    ...
    match ActionType(event.action_type):  # type: ignore
        ...
        case ActionType.SUBSCRIBE:
            ...
            server = next((server for server in get_servers().values() if server.ae == ae), None)
            if server is None:
                return 0x0110, pydicom.Dataset()
