...
def main():
    ...
    parser.add_argument('-s', '--short', action='store_true', help='Use short format with common parent')
    ...
    if len(args.files) == 1 and args.files[0] == "-" and not sys.stdin.isatty():
        args.files = [line.strip() for line in sys.stdin if line.strip()]
    elif len(args.files) == 1 and args.files[0] == "-":
        parser.error("No files specified")
    elif len(args.files) == 0:
        parser.error("No files specified. Use '-' to read from stdin.")
