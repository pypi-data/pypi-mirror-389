...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2846(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2848(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2849(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2853(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2856(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2859(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2846, DataContext__get_Explication(dc)), try_include("unit", _arrow2848, DataContext__get_Unit(dc)), try_include("objectType", _arrow2849, DataContext__get_ObjectType(dc)), try_include("description", _arrow2853, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2856, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2859, DataContext__get_Label(dc))]))
...
def _arrow2876(get: IGetters) -> DataContext:
    data: Data
    ...
    def _arrow2870(__unit: None=None) -> OntologyAnnotation | None:
    ...
    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2866(), _arrow2867(), _arrow2870(), _arrow2871(), _arrow2872(), _arrow2873(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2876)
