...
def array_from_map(otm: Any) -> Array[OntologyAnnotation]:
    ...
    return to_array(map(mapping, sort_by(projection, Dictionary_items(otm), ObjectExpr2783())))
...
def decode_oa(ot: Array[OntologyAnnotation]) -> Decoder_1[OntologyAnnotation]:
    def _arrow2798(get: IGetters, ot: Any=ot) -> OntologyAnnotation:
        i: int = get.Required.Raw(int_1) or 0
        return ot[i]

    return object(_arrow2798)
