...
def encoder(id_map: Any | None, value: MaterialAttribute) -> IEncodable:
    def f(value_1: MaterialAttribute, id_map: Any=id_map, value: Any=value) -> IEncodable:
        ...
        def _arrow2514(value_2: str, value_1: Any=value_1) -> IEncodable:
        ...
        def _arrow2515(oa: OntologyAnnotation, value_1: Any=value_1) -> IEncodable:
            return OntologyAnnotation_ISAJson_encoder(id_map, oa)

        values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([try_include("@id", _arrow2514, gen_id(value_1)), try_include("characteristicType", _arrow2515, value_1.CharacteristicType)]))
        class ObjectExpr2516(IEncodable):
        ...
        return ObjectExpr2516()

    if id_map is not None:
        def _arrow2517(m_1: MaterialAttribute, id_map: Any=id_map, value: Any=value) -> str:
            return gen_id(m_1)

        return encode(_arrow2517, f, value, id_map)
...
def _arrow2519(get: IGetters) -> MaterialAttribute:
    def _arrow2518(__unit: None=None) -> OntologyAnnotation | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("characteristicType", OntologyAnnotation_ISAJson_decoder)

    return MaterialAttribute(None, _arrow2518())


decoder: Decoder_1[MaterialAttribute] = object(_arrow2519)
