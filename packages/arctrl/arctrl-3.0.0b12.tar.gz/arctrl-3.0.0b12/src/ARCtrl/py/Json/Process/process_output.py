...
def _arrow2720(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2722(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2723(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2720, ROCrate_decoder_1), map(_arrow2722, ROCrate_decoder_2), map(_arrow2723, ROCrate_decoder_3)]))
...
def _arrow2725(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2726(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2727(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2725, ISAJson_decoder_1), map(_arrow2726, ISAJson_decoder_2), map(_arrow2727, ISAJson_decoder_3)]))
