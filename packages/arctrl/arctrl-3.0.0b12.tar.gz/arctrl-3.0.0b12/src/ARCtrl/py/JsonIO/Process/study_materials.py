...
def encoder(id_map: Any | None, ps: FSharpList[Process]) -> IEncodable:
    ...
    def _arrow3773(oa: Source, id_map: Any=id_map, ps: Any=ps) -> IEncodable:
        return ISAJson_encoder(id_map, oa)

    def _arrow3774(oa_1: Sample, id_map: Any=id_map, ps: Any=ps) -> IEncodable:
        return ISAJson_encoder_1(id_map, oa_1)

    def _arrow3775(c: Material, id_map: Any=id_map, ps: Any=ps) -> IEncodable:
        return ISAJson_encoder_2(id_map, c)

    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([try_include_list("sources", _arrow3773, source), try_include_list("samples", _arrow3774, samples), try_include_list("otherMaterials", _arrow3775, materials)]))
