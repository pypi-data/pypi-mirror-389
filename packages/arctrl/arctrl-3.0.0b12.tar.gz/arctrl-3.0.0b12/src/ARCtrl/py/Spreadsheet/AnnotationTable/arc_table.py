...
def compose_columns(string_cell_columns: Array[Array[str]]) -> Array[CompositeColumn]:
    def _arrow1365(columns: Array[Array[str]], string_cell_columns: Any=string_cell_columns) -> CompositeColumn:
        return from_string_cell_columns(columns)

    return map_1(_arrow1365, group_columns_by_header(string_cell_columns), None)
...
def try_from_fs_worksheet(sheet: FsWorksheet) -> ArcTable | None:
    try: 
        ...
        if match_value is None:
            ...
            def _arrow1369(__unit: None=None) -> IEnumerable_1[Array[str]]:
                def _arrow1368(c: int) -> Array[str]:
                    def _arrow1367(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow1366(r: int) -> IEnumerable_1[str]:
                        ...
                        return collect(_arrow1366, range_big_int(1, 1, t.RangeAddress.LastAddress.RowNumber))

                    return to_array_1(delay(_arrow1367))

                return map_2(_arrow1368, range_big_int(1, 1, t.RangeAddress.LastAddress.ColumnNumber))

            pattern_input: tuple[Array[CompositeHeader], ArcTableValues] = compose_arc_table_values(map_1(fix_deprecated_ioheader, to_array_1(delay(_arrow1369)), None))
...
def to_fs_worksheet(index: int | None, table: ArcTable) -> FsWorksheet:
    ...
    if table.ColumnCount == 0:
        ...
        def _arrow1370(column_1: CompositeColumn, index: Any=index, table: Any=table) -> FSharpList[FSharpList[str]]:
            return to_string_cell_columns(column_1)

        def _arrow1371(column: CompositeColumn, index: Any=index, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
            return classify_column_order(column)

        class ObjectExpr1372:
            @property
        ...
        columns: FSharpList[FSharpList[str]] = collect_1(_arrow1370, sort_by(_arrow1371, of_seq(table.Columns), ObjectExpr1372()))
        table_row_count: int
        def folder(acc: int, c: FSharpList[str], index: Any=index, table: Any=table) -> int:
            def _arrow1373(x_1: int, y_1: int, acc: Any=acc, c: Any=c) -> int:
                return compare_primitives(x_1, y_1)

            return max(_arrow1373, acc, length(c))
