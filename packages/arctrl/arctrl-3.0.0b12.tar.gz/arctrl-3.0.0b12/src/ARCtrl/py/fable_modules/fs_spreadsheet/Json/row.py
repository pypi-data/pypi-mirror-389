...
def encode(row: FsRow) -> IEncodable:
    def _arrow307(__unit: None=None, row: Any=row) -> IEncodable:
    ...
    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("number", _arrow307()), ("cells", seq(map(mapping, row.Cells)))])
    class ObjectExpr309(IEncodable):
