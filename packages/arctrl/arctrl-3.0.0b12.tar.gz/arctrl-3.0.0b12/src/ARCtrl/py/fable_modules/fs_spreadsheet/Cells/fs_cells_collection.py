...
class FsCellsCollection:
    ...
    def RemoveCellAt(self, row: int, column: int) -> None:
        ...
        if (row == this._maxRowUsed) if row_removed else False:
            ...
            this._maxRowUsed = (max(this._rowsUsed.keys(), ObjectExpr169()) if (not is_empty(this._rowsUsed.keys())) else 0) or 0
        ...
        if (column == this._maxColumnUsed) if column_removed else False:
            ...
            this._maxColumnUsed = (max(this._columnsUsed.keys(), ObjectExpr170()) if (not is_empty(this._columnsUsed.keys())) else 0) or 0
        ...
        if match_value is None:
            ...
            def _arrow171(__unit: None=None) -> bool:
