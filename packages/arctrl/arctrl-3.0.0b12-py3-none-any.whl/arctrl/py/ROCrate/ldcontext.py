...
class LDContext:
    ...
    def PropertyNamesMatch(self, p1: str, p2: str) -> bool:
        ...
        if p1 == p2:
            ...
            return (_arrow1464() if (p2def is not None) else False) if (p1def is None) else (_arrow1465() if (p2def is None) else _arrow1466())
    ...
    def try_combine_optional(base_context: LDContext | None=None, specific_context: LDContext | None=None) -> LDContext | None:
        def _arrow1467(__unit: None=None) -> LDContext | None:
            s_1: LDContext = specific_context
            return s_1

        def _arrow1468(__unit: None=None) -> LDContext | None:
            f_1: LDContext = base_context
            return f_1

        def _arrow1469(__unit: None=None) -> LDContext | None:
            f: LDContext = base_context
            s: LDContext = specific_context
            return LDContext.combine(f, s)

        return (_arrow1467() if (specific_context is not None) else None) if (base_context is None) else (_arrow1468() if (specific_context is None) else _arrow1469())
    ...
    def __hash__(self, __unit: None=None) -> int:
        ...
        class ObjectExpr1470:
            @property
            def Compare(self) -> Callable[[str, str], int]:
                return compare_primitives

        mappings_hash: int = box_hash_seq(map(mapping, sort(this.Mappings.keys(), ObjectExpr1470())))
        name_hash: int
    ...
    def add_reverse_mapping(self, key: str, value: str) -> None:
        ...
        if active_pattern_result is not None:
            ...
            def _arrow1472(v: str) -> None:
                nonlocal out_arg
                out_arg = v

            pattern_input = (try_get_value(this.mappings_004045, prefix, FSharpRef(_arrow1471, _arrow1472)), out_arg)
            ...
            def _arrow1473(__unit: None=None) -> tuple[str, str]:
                return out_arg_1

            def _arrow1474(v_2: tuple[str, str]) -> None:
                nonlocal out_arg_1
                out_arg_1 = v_2

            pattern_input_1 = (try_get_value(this.compact_reverse_mappings, key, FSharpRef(_arrow1473, _arrow1474)), out_arg_1)
    ...
    def try_find_term(self, term: str) -> str | None:
        ...
        pattern_input: tuple[bool, str]
        out_arg: str = None
        def _arrow1475(__unit: None=None) -> str:
            return out_arg

        def _arrow1476(v: str) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this.mappings_004045, term, FSharpRef(_arrow1475, _arrow1476)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        ...
        if definition is None:
            ...
            return _arrow1477() if (active_pattern_result is not None) else _arrow1478()
    ...
    def try_find_iri(self, iri: str) -> str | None:
        ...
        pattern_input: tuple[bool, str]
        out_arg: str = None
        def _arrow1479(__unit: None=None) -> str:
            return out_arg

        def _arrow1480(v: str) -> None:
            nonlocal out_arg
            out_arg = v

        pattern_input = (try_get_value(this.reverse_mappings, iri, FSharpRef(_arrow1479, _arrow1480)), out_arg)
        match_value = pattern_input[1] if pattern_input[0] else None
        def chooser(ctx: LDContext) -> str | None:
...
LDContext_reflection = _expr1481
