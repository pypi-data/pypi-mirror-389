...
def _arrow345(builder: IGetters) -> FsWorksheet:
    row_index: int = 0
    n: str
    ...
    def _arrow344(__unit: None=None) -> IEnumerable_1[tuple[int | None, IEnumerable_1[FsCell]]] | None:
    ...
    rs: IEnumerable_1[tuple[int | None, IEnumerable_1[FsCell]]] = default_arg(_arrow344(), empty())
...
decode_rows: Decoder_1[FsWorksheet] = object(_arrow345)

def encode_columns(no_numbering: bool, sheet: FsWorksheet) -> IEncodable:
    sheet.RescanRows()
    def _arrow349(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[IEncodable]:
        def _arrow348(c: int) -> IEncodable:
            def _arrow347(__unit: None=None) -> IEnumerable_1[FsCell]:
                def _arrow346(r: int) -> IEnumerable_1[FsCell]:
                    match_value: FsCell | None = sheet.CellCollection.TryGetCell(r, c)
                    return singleton(FsCell("")) if (match_value is None) else singleton(match_value)

                return collect(_arrow346, range_big_int(1, 1, sheet.MaxRowIndex))

            return encode_no_numbers_1(to_list(delay(_arrow347)))

        return map(_arrow348, range_big_int(1, 1, sheet.MaxColumnIndex))

    def mapping(col_1: FsColumn, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEncodable:
        return encode_2(col_1)

    j_columns: IEncodable = seq(to_list(delay(_arrow349))) if no_numbering else seq(map(mapping, sheet.Columns))
    def _arrow354(__unit: None=None, no_numbering: Any=no_numbering, sheet: Any=sheet) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow351(__unit: None=None) -> IEncodable:
            value: str = sheet.Name
        ...
        def _arrow353(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow352(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                return singleton(("columns", j_columns))

            return append(singleton(("tables", seq(map(encode_1, sheet.Tables)))) if (not is_empty(sheet.Tables)) else empty(), delay(_arrow352))

        return append(singleton(("name", _arrow351())), delay(_arrow353))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow354))
...
def _arrow357(builder: IGetters) -> FsWorksheet:
    col_index: int = 0
    n: str
    object_arg: IRequiredGetter = builder.Required
    n = object_arg.Field("name", string)
...
decode_columns: Decoder_1[FsWorksheet] = object(_arrow357)
