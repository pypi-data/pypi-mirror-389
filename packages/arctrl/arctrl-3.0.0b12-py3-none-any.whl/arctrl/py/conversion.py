...
from .Core.Table.arc_table_aux import (Unchecked_tryGetCellAt, get_empty_cell_for_header, Unchecked_alignByHeaders, ArcTableValues)
...
from .fable_modules.fable_library.list import (FSharpList, choose, length, empty, map as map_3, sort_by, singleton as singleton_1, initialize, collect, of_seq)
...
from .fable_modules.fable_library.seq import (indexed, to_list, filter, try_pick, choose as choose_1, map as map_2, delay, append, empty as empty_1, singleton, fold, zip, is_empty)
from .fable_modules.fable_library.seq2 import (List_groupBy, List_distinct)
from .fable_modules.fable_library.string_ import (to_text, printf, to_fail, join)
...
from .fable_modules.fable_library.types import (FSharpRef, Array, to_string as to_string_1)
from .fable_modules.fable_library.util import (int32_to_string, IEnumerable_1, string_hash, compare_primitives, get_enumerator, dispose, ignore, equals, safe_hash)

def DateTime_tryFromString(s: str) -> Any | None:
    try: 
        def _arrow3900(__unit: None=None) -> Any:
...
def ColumnIndex_tryInt(str_1: str) -> int | None:
    ...
    out_arg: int = 0
    def _arrow3901(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow3902(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow3901, _arrow3902)), out_arg)
    if match_value[0]:
...
BaseTypes_reflection = _expr3903
...
ProcessConversion_reflection = _expr3904
...
def ProcessConversion_tryComponentGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[ArcTable, int], LDNode] | None:
    if value_header.tag == 0:
        def Value(table: ArcTable, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3906(i: int, table: Any=table) -> LDNode:
                def _arrow3905(__unit: None=None) -> CompositeCell:
                ...
                c: LDNode = BaseTypes_composeComponent(value_header, _arrow3905())
...
def ProcessConversion_tryParameterGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[ArcTable, int], LDNode] | None:
    if value_header.tag == 3:
        def Value(table: ArcTable, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3908(i: int, table: Any=table) -> LDNode:
                def _arrow3907(__unit: None=None) -> CompositeCell:
                    match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                    return get_empty_cell_for_header(value_header, None) if (match_value is None) else match_value

                p: LDNode = BaseTypes_composeParameterValue(value_header, _arrow3907())
...
def ProcessConversion_tryFactorGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[ArcTable, int], LDNode] | None:
    if value_header.tag == 2:
        def Value(table: ArcTable, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3910(i: int, table: Any=table) -> LDNode:
                def _arrow3909(__unit: None=None) -> CompositeCell:
                    match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                    return get_empty_cell_for_header(value_header, None) if (match_value is None) else match_value

                f: LDNode = BaseTypes_composeFactorValue(value_header, _arrow3909())
...
def ProcessConversion_tryCharacteristicGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[ArcTable, int], LDNode] | None:
    if value_header.tag == 1:
        def Value(table: ArcTable, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3912(i: int, table: Any=table) -> LDNode:
                def _arrow3911(__unit: None=None) -> CompositeCell:
                    match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                    return get_empty_cell_for_header(value_header, None) if (match_value is None) else match_value

                c: LDNode = BaseTypes_composeCharacteristicValue(value_header, _arrow3911())
                ARCtrl_ROCrate_LDNode__LDNode_SetColumnIndex_Z524259A4(c, value_i)
                return c
...
def ProcessConversion_tryGetProtocolTypeGetter(general_i: int, header: CompositeHeader) -> Callable[[ArcTable, int], LDNode] | None:
    if header.tag == 4:
        def Value(table: ArcTable, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3914(i: int, table: Any=table) -> LDNode:
                def _arrow3913(__unit: None=None) -> OntologyAnnotation:
                    match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                    if match_value is None:
                ...
                return BaseTypes_composeDefinedTerm_ZDED3A0F(_arrow3913())
...
def ProcessConversion_tryGetProtocolVersionGetter(general_i: int, header: CompositeHeader) -> Callable[[ArcTable, int], str] | None:
    if header.tag == 7:
        def Value(table: ArcTable, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3918(i: int, table: Any=table) -> str:
                match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                if match_value is None:
                    return ""
...
def ProcessConversion_tryGetInputGetter(general_i: int, header: CompositeHeader, fs: FileSystem | None=None) -> Callable[[ArcTable, int], LDNode] | None:
    if header.tag == 11:
        def Value(table: ArcTable, general_i: Any=general_i, header: Any=header, fs: Any=fs) -> Callable[[int], LDNode]:
            def _arrow3920(i: int, table: Any=table) -> LDNode:
                def _arrow3919(__unit: None=None) -> CompositeCell:
                    match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                    return get_empty_cell_for_header(header, None) if (match_value is None) else match_value

                return BaseTypes_composeProcessInput(header, _arrow3919(), fs)
...
def ProcessConversion_tryGetOutputGetter(general_i: int, header: CompositeHeader, fs: FileSystem | None=None) -> Callable[[ArcTable, int], LDNode] | None:
    if header.tag == 12:
        def Value(table: ArcTable, general_i: Any=general_i, header: Any=header, fs: Any=fs) -> Callable[[int], LDNode]:
            def _arrow3922(i: int, table: Any=table) -> LDNode:
                def _arrow3921(__unit: None=None) -> CompositeCell:
                    match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                    return get_empty_cell_for_header(header, None) if (match_value is None) else match_value

                return BaseTypes_composeProcessOutput(header, _arrow3921(), fs)
...
def ProcessConversion_tryGetCommentGetter(general_i: int, header: CompositeHeader) -> Callable[[ArcTable, int], str] | None:
    if header.tag == 14:
        ...
        def Value(table: ArcTable, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3924(i: int, table: Any=table) -> str:
                def _arrow3923(__unit: None=None) -> Comment:
                    match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                    if match_value is None:
                        return Comment(c)
...
def ProcessConversion_tryGetPerformerGetter(general_i: int, header: CompositeHeader) -> Callable[[ArcTable, int], LDNode] | None:
    if header.tag == 9:
        def Value(table: ArcTable, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3925(i: int, table: Any=table) -> LDNode:
                performer: str
                match_value: CompositeCell | None = Unchecked_tryGetCellAt(general_i, i, table.Values)
                if match_value is None:
                    performer = ""
...
def ProcessConversion_getProcessGetter(assay_name: str | None, study_name: str | None, process_name_root: str, headers: IEnumerable_1[CompositeHeader], fs: FileSystem | None=None) -> Callable[[ArcTable, int], LDNode]:
    ...
    if match_value is None:
        def _arrow3927(table_1: ArcTable, assay_name: Any=assay_name, study_name: Any=study_name, process_name_root: Any=process_name_root, headers: Any=headers, fs: Any=fs) -> Callable[[int], Array[LDNode]]:
            def _arrow3926(i_1: int) -> Array[LDNode]:
        ...
        def _arrow3929(table_2: ArcTable, assay_name: Any=assay_name, study_name: Any=study_name, process_name_root: Any=process_name_root, headers: Any=headers, fs: Any=fs) -> Callable[[int], Array[LDNode]]:
            def _arrow3928(i_2: int) -> Array[LDNode]:
                return []

            return _arrow3928

        input_getter_1 = _arrow3927 if (length(char_getters) != 0) else _arrow3929

    else: 
        input_getter: Callable[[ArcTable, int], LDNode] = match_value
        def _arrow3931(table: ArcTable, assay_name: Any=assay_name, study_name: Any=study_name, process_name_root: Any=process_name_root, headers: Any=headers, fs: Any=fs) -> Callable[[int], Array[LDNode]]:
            def _arrow3930(i: int) -> Array[LDNode]:
    ...
    if match_value_1 is None:
        def _arrow3933(table_4: ArcTable, assay_name: Any=assay_name, study_name: Any=study_name, process_name_root: Any=process_name_root, headers: Any=headers, fs: Any=fs) -> Callable[[int], Array[LDNode]]:
            def _arrow3932(i_4: int) -> Array[LDNode]:
        ...
        def _arrow3935(table_5: ArcTable, assay_name: Any=assay_name, study_name: Any=study_name, process_name_root: Any=process_name_root, headers: Any=headers, fs: Any=fs) -> Callable[[int], Array[LDNode]]:
            def _arrow3934(i_5: int) -> Array[LDNode]:
                return []

            return _arrow3934

        output_getter_1 = _arrow3933 if (length(factor_value_getters) != 0) else _arrow3935

    else: 
        output_getter: Callable[[ArcTable, int], LDNode] = match_value_1
        def _arrow3937(table_3: ArcTable, assay_name: Any=assay_name, study_name: Any=study_name, process_name_root: Any=process_name_root, headers: Any=headers, fs: Any=fs) -> Callable[[int], Array[LDNode]]:
            def _arrow3936(i_3: int) -> Array[LDNode]:
        ...
        output_getter_1 = _arrow3937

    def _arrow3939(table_6: ArcTable, assay_name: Any=assay_name, study_name: Any=study_name, process_name_root: Any=process_name_root, headers: Any=headers, fs: Any=fs) -> Callable[[int], LDNode]:
        def _arrow3938(i_6: int) -> LDNode:
...
def ProcessConversion_groupProcesses_Z27F0B586(processes: FSharpList[LDNode], graph: LDGraph | None=None, context: LDContext | None=None) -> FSharpList[tuple[str, FSharpList[LDNode]]]:
    def projection(p: LDNode, processes: Any=processes, graph: Any=graph, context: Any=context) -> str:
        ...
        if match_value is not None:
            ...
            def _arrow3941(__unit: None=None, p: Any=p) -> bool:
    ...
    class ObjectExpr3943:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3942(x: str, y: str) -> bool:
                return x == y
    ...
    return List_groupBy(projection, processes, ObjectExpr3943())
...
def ProcessConversion_processToRows_Z6839B9E8(p: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> Array[FSharpList[tuple[CompositeHeader, CompositeCell]]]:
    ...
    if match_value_1 is None:
        ...
        def _arrow3952(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3951(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                ...
                def _arrow3950(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    ...
                    def _arrow3949(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(_arrow3947(), delay(_arrow3948))

                    return append(_arrow3946(), delay(_arrow3949))

                return append(_arrow3945(), delay(_arrow3950))

            return append(_arrow3944(), delay(_arrow3951))

        prot_vals = to_list(delay(_arrow3952))
    ...
    def _arrow3953(Value: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value

    def _arrow3954(Value_1: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_1

    def _arrow3955(Value_2: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_2

    def _arrow3956(Value_3: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_3

    pattern_input: tuple[Array[LDNode | None], Array[LDNode | None]] = ((ResizeArray_create(len(outputs), None), ResizeArray_map(_arrow3953, outputs))) if ((len(outputs) != 0) if (len(inputs) == 0) else False) else (((ResizeArray_map(_arrow3954, inputs), ResizeArray_create(len(inputs), None))) if ((len(outputs) == 0) if (len(inputs) != 0) else False) else ((ResizeArray_map(_arrow3955, inputs), ResizeArray_map(_arrow3956, outputs))))
    ...
    if (len(outputs_1) == 0) if (len(inputs_1) == 0) else False:
        ...
        def _arrow3958(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
            def _arrow3957(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                return pvs

            return append(components, delay(_arrow3957))

        class ObjectExpr3959:
            @property
            def Compare(self) -> Callable[[int, int], int]:
                return compare_primitives

        vals: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_3(mapping, sort_by(projection, to_list(delay(_arrow3958)), ObjectExpr3959()))
        def _arrow3962(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow3961(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3960(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    return comments

                return append(vals, delay(_arrow3960))

            return append(prot_vals, delay(_arrow3961))

        return ResizeArray_singleton(to_list(delay(_arrow3962)))
        ...
        def f_5(tupled_arg: tuple[LDNode | None, LDNode | None], p: Any=p, graph: Any=graph, context: Any=context) -> FSharpList[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3966(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                def _arrow3965(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                    def _arrow3964(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                        ...
                        return append(pvs, delay(_arrow3963))

                    return append(components, delay(_arrow3964))

                return append(chars, delay(_arrow3965))

            class ObjectExpr3967:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            vals_1: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_3(mapping_1, sort_by(projection_1, to_list(delay(_arrow3966)), ObjectExpr3967()))
            def _arrow3972(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3971(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow3970(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        def _arrow3969(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                            ...
                            return append(comments, delay(_arrow3968))

                        return append(vals_1, delay(_arrow3969))

                    return append(prot_vals, delay(_arrow3970))

                return append(singleton(BaseTypes_decomposeProcessInput_Z2F770004(value_4(i), context)) if (i is not None) else empty_1(), delay(_arrow3971))

            return to_list(delay(_arrow3972))
...
def ARCtrl_ArcTable__ArcTable_GetProtocols(this: ArcTable) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        def _arrow3973(__unit: None=None, this: Any=this) -> LDNode:
        ...
        return singleton_1(_arrow3973())

    else: 
        def _arrow3974(i: int, this: Any=this) -> LDNode:
        ...
        return List_distinct(initialize(this.RowCount, _arrow3974), ObjectExpr3975())
...
def ARCtrl_ArcTable__ArcTable_GetProcesses_5E660E5C(this: ArcTable, assay_name: str | None=None, study_name: str | None=None, fs: FileSystem | None=None) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        ...
        def _arrow3977(__unit: None=None, this: Any=this, assay_name: Any=assay_name, study_name: Any=study_name, fs: Any=fs) -> IEnumerable_1[LDNode]:
            def _arrow3976(i: int) -> LDNode:
                return getter(this)(i)

            return map_2(_arrow3976, range_big_int(0, 1, this.RowCount - 1))

        return to_list(delay(_arrow3977))
...
DatamapConversion_reflection = _expr3978
...
PersonConversion_reflection = _expr3979
...
def PersonConversion_composeAddress_Z721C83C5(address: str) -> Any:
    try: 
        def _arrow3980(__unit: None=None) -> LDNode:
...
ScholarlyArticleConversion_reflection = _expr3981
...
def ScholarlyArticleConversion_composeScholarlyArticle_D324A6D(publication: Publication) -> LDNode:
    ...
    def _arrow3983(__unit: None=None, publication: Any=publication) -> IEnumerable_1[LDNode]:
        def _arrow3982(__unit: None=None) -> IEnumerable_1[LDNode]:
        ...
        return append(singleton(LDPropertyValue.create_doi(value_4(publication.DOI))) if ((value_4(publication.DOI) != "") if (publication.DOI is not None) else False) else empty_1(), delay(_arrow3982))

    identifiers: Array[LDNode] = list(to_list(delay(_arrow3983)))
...
AssayConversion_reflection = _expr3984
...
def AssayConversion_composeAssay_Z5C53FD5C(assay: ArcAssay, fs: FileSystem | None=None) -> LDNode:
    ...
    def _arrow3987(__unit: None=None, assay: Any=assay, fs: Any=fs) -> Array[LDNode] | None:
        fds: Array[LDNode] = fragment_descriptors
        vm: LDNode = variable_measured
        return ResizeArray_appendSingleton(vm, fds)

    variable_measureds: Array[LDNode] | None = (None if (fragment_descriptors is None) else _arrow3985()) if (variable_measured is None) else (_arrow3986() if (fragment_descriptors is None) else _arrow3987())
...
StudyConversion_reflection = _expr3988

def StudyConversion_composeStudy_ZFE0E38E(study: ArcStudy, fs: FileSystem | None=None) -> LDNode:
    def _arrow3989(s: str, study: Any=study, fs: Any=fs) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3989, study.SubmissionDate)
    def _arrow3990(s_1: str, study: Any=study, fs: Any=fs) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any | None = bind(_arrow3990, study.PublicReleaseDate)
...
def StudyConversion_decomposeStudy_Z6839B9E8(study: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcStudy:
    def _arrow3991(d: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3991, LDDataset.try_get_date_created_as_date_time(study, context))
    def _arrow3992(d_1: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3992, LDDataset.try_get_date_published_as_date_time(study, context))
...
InvestigationConversion_reflection = _expr3993
...
def InvestigationConversion_composeInvestigation_5AEC717D(investigation: ArcInvestigation, fs: FileSystem | None=None) -> LDNode:
    ...
    def _arrow3994(s: str, investigation: Any=investigation, fs: Any=fs) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3994, investigation.SubmissionDate)
    def _arrow3995(s_1: str, investigation: Any=investigation, fs: Any=fs) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any = default_arg(bind(_arrow3995, investigation.PublicReleaseDate), now())
    ...
    def _arrow3996(__unit: None=None, investigation: Any=investigation, fs: Any=fs) -> Array[LDNode]:
    ...
    has_parts: Array[LDNode] | None = Option_fromSeq(_arrow3996())
...
def InvestigationConversion_decomposeInvestigation_Z6839B9E8(investigation: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcInvestigation:
    ...
    def _arrow3997(d: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3997, LDDataset.try_get_date_created_as_date_time(investigation, context))
    def _arrow3998(d_1: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3998, LDDataset.try_get_date_published_as_date_time(investigation, context))
...
TypeExtensions_Conversion_reflection = _expr3999
