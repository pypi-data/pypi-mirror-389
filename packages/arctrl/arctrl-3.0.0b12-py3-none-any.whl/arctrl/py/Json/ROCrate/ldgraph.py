...
def encoder(obj: LDGraph) -> IEncodable:
    ...
    def _arrow2012(__unit: None=None, obj: Any=obj) -> IEnumerable_1[tuple[str, IEncodable | None]]:
        def _arrow2007(value: str) -> IEncodable:
        ...
        def _arrow2011(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
            def _arrow2010(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                def _arrow2008(kv: Any) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                    l: str = kv[0].lower()
                ...
                def _arrow2009(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                    return singleton(("@graph", seq(map_1(encoder_2, obj.Nodes))))

                return append(collect(_arrow2008, obj.GetProperties(True)), delay(_arrow2009))

            return append(singleton(try_include("@context", encoder_1, obj.TryGetContext())), delay(_arrow2010))

        return append(singleton(try_include("@id", _arrow2007, obj.Id)), delay(_arrow2011))

    values_1: FSharpList[tuple[str, IEncodable]] = choose(chooser, to_list(delay(_arrow2012)))
...
class ObjectExpr2015(Decoder_1[LDGraph]):
...
decoder: Decoder_1[LDGraph] = ObjectExpr2015()
