...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow3571(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3570(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3569(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3568(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3567(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow3566(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow3566))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3567))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3568))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3569))

            return append(singleton("{"), delay(_arrow3570))

        return join("\n", to_list(delay(_arrow3571)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3576(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3575(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3575, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3576))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3586
