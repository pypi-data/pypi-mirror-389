...
def directory_exists_async(path: str) -> Async[bool]:
    def _arrow3813(__unit: None=None, path: Any=path) -> Async[bool]:
...
def create_directory_async(path: str) -> Async[None]:
    def _arrow3814(__unit: None=None, path: Any=path) -> Async[None]:
        Path(path).mkdir(parents=True, exist_ok=True)
        return singleton.Zero()

    return singleton.Delay(_arrow3814)


def ensure_directory_async(path: str) -> Async[None]:
    def _arrow3816(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3815(_arg: bool) -> Async[None]:
            return singleton.ReturnFrom(create_directory_async(path)) if (not _arg) else singleton.Zero()

        return singleton.Bind(directory_exists_async(path), _arrow3815)

    return singleton.Delay(_arrow3816)


def ensure_directory_of_file_async(file_path: str) -> Async[None]:
    def _arrow3817(__unit: None=None, file_path: Any=file_path) -> Async[None]:
...
def file_exists_async(path: str) -> Async[bool]:
    def _arrow3818(__unit: None=None, path: Any=path) -> Async[bool]:
...
def read_file_text_async(path: str) -> Async[str]:
    def _arrow3820(__unit: None=None, path: Any=path) -> Async[str]:
        def _arrow3819(__unit: None=None) -> str:
            with open(path, 'r', encoding='utf-8') as f: return f.read()

        return singleton.Return(_arrow3819())

    return singleton.Delay(_arrow3820)


def read_file_binary_async(path: str) -> Async[bytearray]:
    def _arrow3822(__unit: None=None, path: Any=path) -> Async[bytearray]:
        def _arrow3821(__unit: None=None) -> bytearray:
            with open(path, 'rb') as f: return f.read()

        return singleton.Return(_arrow3821())

    return singleton.Delay(_arrow3822)


def read_file_xlsx_async(path: str) -> Async[FsWorkbook]:
    def _arrow3823(__unit: None=None, path: Any=path) -> Async[FsWorkbook]:
...
def move_file_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3824(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
...
def delete_directory_async(path: str) -> Async[None]:
    def _arrow3826(__unit: None=None, path: Any=path) -> Async[None]:
...
def write_file_text_async(path: str, text: str) -> Async[None]:
    def _arrow3827(__unit: None=None, path: Any=path, text: Any=text) -> Async[None]:
...
def write_file_binary_async(path: str, bytes: bytearray) -> Async[None]:
    def _arrow3828(__unit: None=None, path: Any=path, bytes: Any=bytes) -> Async[None]:
...
def write_file_xlsx_async(path: str, wb: FsWorkbook) -> Async[None]:
    def _arrow3829(__unit: None=None, path: Any=path, wb: Any=wb) -> Async[None]:
...
def get_sub_files_async(path: str) -> Async[Array[str]]:
    def _arrow3831(__unit: None=None, path: Any=path) -> Async[Array[str]]:
        paths: Array[str] = [str(entry) for entry in Path(path).iterdir() if entry.is_file()]
        return singleton.Return(map(standardize_slashes, paths, None))

    return singleton.Delay(_arrow3831)


def get_all_file_paths_async(directory_path: str) -> Async[Array[str]]:
    directory_path_1: str = standardize_slashes(directory_path)
    def _arrow3837(__unit: None=None, directory_path: Any=directory_path) -> Async[Array[str]]:
        def all_files(dirs: IEnumerable_1[str]) -> Async[IEnumerable_1[str]]:
            def _arrow3835(__unit: None=None, dirs: Any=dirs) -> Async[IEnumerable_1[str]]:
                def _arrow3834(_arg: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                    sub_files_1: IEnumerable_1[str] = concat(_arg)
                    def _arrow3833(_arg_1: Array[Array[str]]) -> Async[IEnumerable_1[str]]:
                        def _arrow3832(_arg_2: Array[IEnumerable_1[str]]) -> Async[IEnumerable_1[str]]:
                            sub_dir_contents_1: IEnumerable_1[str] = concat(_arg_2)
                            return singleton.Return(append(sub_dir_contents_1, sub_files_1))

                        return singleton.Bind(all(map_1(all_files, _arg_1)), _arrow3832)

                    return singleton.Bind(all(map_1(get_sub_directories_async, dirs)), _arrow3833)

                return singleton.Return(empty()) if is_empty(dirs) else singleton.Bind(all(map_1(get_sub_files_async, dirs)), _arrow3834)

            return singleton.Delay(_arrow3835)

        def _arrow3836(_arg_3: IEnumerable_1[str]) -> Async[Array[str]]:
        ...
        return singleton.Bind(all_files(to_enumerable([directory_path_1])), _arrow3836)

    return singleton.Delay(_arrow3837)


def rename_file_or_directory_async(old_path: str, new_path: str) -> Async[None]:
    def _arrow3840(__unit: None=None, old_path: Any=old_path, new_path: Any=new_path) -> Async[None]:
        def _arrow3839(_arg: bool) -> Async[None]:
            def _arrow3838(_arg_1: bool) -> Async[None]:
                if _arg:
            ...
            return singleton.Bind(directory_exists_async(old_path), _arrow3838)

        return singleton.Bind(file_exists_async(old_path), _arrow3839)

    return singleton.Delay(_arrow3840)


def delete_file_or_directory_async(path: str) -> Async[None]:
    def _arrow3843(__unit: None=None, path: Any=path) -> Async[None]:
        def _arrow3842(_arg: bool) -> Async[None]:
            def _arrow3841(_arg_1: bool) -> Async[None]:
                if _arg:
            ...
            return singleton.Bind(directory_exists_async(path), _arrow3841)

        return singleton.Bind(file_exists_async(path), _arrow3842)
