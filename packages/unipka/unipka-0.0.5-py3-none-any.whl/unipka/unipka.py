...
class KpuuPrediction(NamedTuple):
    """Prediction for Kp,uu prediction"""
    prob: float
    logD: float
    state_penalty: float
    G_solv: float
...
class UnipKa(object):
    ...
    def _predict_macro_pKa(self, mol: Chem.Mol | str, /, *, mode: Literal["a2b", "b2a"]) -> float:
        ...
        if len(macrostate_A)==0 or len(macrostate_B)==0:
            logger.warning(f"Could not find any microstates in direction {mode} for {smi}")
            return np.nan
    ...
    def predict_brain_penetrance(self, mol: Chem.Mol) -> KpuuPrediction:
        ...
        pred = KpuuPrediction(prob=prob, logD=logD, state_penalty=sp, G_solv=G_solv)
        return pred
...
if __name__=="__main__":
    ...
    smi = "c1ncccc1"
    calc = UnipKa()
    print(calc.get_basic_macro_pka(smi))
    print(calc.get_acidic_macro_pka(smi))
