...
class AspenClient:
    ...
    def _parse_snapshot_sql_response(
        self, response: list[dict], tag_names: list[str]
    ) -> tuple[pd.DataFrame, dict[str, str]]:
        """Parse SQL snapshot response into DataFrame with descriptions."""
        try:
            if not response or not isinstance(response, list):
            ...
            df.index = pd.Index(["snapshot"], name="snapshot")
