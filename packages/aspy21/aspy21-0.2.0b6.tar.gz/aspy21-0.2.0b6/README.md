...
| `start` | str | required | Start timestamp (ISO format) |
| `end` | str | required | End timestamp (ISO format) |
| `interval` | int\|None | None | Interval in seconds for aggregated data |
| `read_type` | ReaderType | RAW | Data retrieval mode |
| `include_status` | bool | True | Include status column in output |
