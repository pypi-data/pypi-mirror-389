...
def test_snapshot_read_uses_sql(mock_api):
    ...
    assert (
        "Select name, name->ip_description, name->ip_input_value from all_records "
        "where name in ('GXV1255.PV', 'GP901.PV')"
    ) in request_body
    assert "snapshot" in df.index.tolist()
    assert df.loc["snapshot", "GXV1255.PV"] == 12.5
    assert df.loc["snapshot", "GP901.PV"] == 74.0
    c.close()


def test_read_without_range_defaults_to_snapshot(mock_api):
    ...
    assert isinstance(df, pd.DataFrame)
    df = cast(pd.DataFrame, df)
    assert df.loc["snapshot", "TAG1"] == 11.0
    c.close()
...
def test_sql_multi_tag_read_dataframe(mock_api):
    ...
    status_cols = df.filter(regex="status")
    assert status_cols.shape == (1, 2)
    assert sorted(status_cols.loc[ts].tolist()) == [0, 8]
    c.close()
