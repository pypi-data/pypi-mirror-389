...
def CLSPSummary(
    ...
    if summary.get("zhat")    is not None:
    ...
    if summary.get("z_lower") is not None:
    ...
    def _format(v, width=10):
        ...
        dp       = max(3, width // 2 - 1)
        ep       = max(2, dp - 1)
        return fmt.format(f"{v:.{dp}f}" if ((abs(v) >= 10**-(ep - 1)     and
                                             abs(v) <  10**(width - ep)) or
                                            len(f"{v:.{dp}f}") <= width) else
                          f"{v:.{ep}e}")
    print("Estimator Configuration:")
    _print(summary, "inverse",  "Generalized inverse")
    _print(summary, "r",        "Iterations (r)")
    _print(summary, "tolerance")
    _print(summary, "final",    "Final correction")
    _print(summary, "alpha")
    print("\nNumerical Stability:")
