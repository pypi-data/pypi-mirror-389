...
class RepoImpl:
    ...
    async def promote_ephemeral_branch(
        ...
        *,
        ...
        raw_base = base_branch if base_branch is not None else baseBranch
        if raw_base is None:
            raise ValueError("promote_ephemeral_branch base_branch is required")

        base_clean = raw_base.strip()
        if not base_clean:
            raise ValueError("promote_ephemeral_branch base_branch is required")

        raw_target = target_branch if target_branch is not None else targetBranch
        target_clean = raw_target.strip() if raw_target is not None else base_clean
        if not target_clean:
