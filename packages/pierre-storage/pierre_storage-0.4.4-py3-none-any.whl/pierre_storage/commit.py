...
def _normalize_commit_options(options: CreateCommitOptions) -> Dict[str, Any]:
    ...
    target_branch = (options.get("target_branch") or "").strip()  # type: ignore[arg-type]
    ...
    commit_message = (options.get("commit_message") or "").strip()  # type: ignore[arg-type]
...
async def _to_async_iterator(source: FileSource) -> AsyncIterator[bytes]:
    ...
    if isinstance(source, str):
        ...
        async for chunk in source:  # type: ignore[assignment]
            if isinstance(chunk, str):
                yield chunk.encode("utf-8")
            else:
                yield bytes(chunk)
    elif hasattr(source, "__iter__"):
        for chunk in source:  # type: ignore[assignment]
            if isinstance(chunk, str):
                yield chunk.encode("utf-8")
            else:
                yield bytes(chunk)
