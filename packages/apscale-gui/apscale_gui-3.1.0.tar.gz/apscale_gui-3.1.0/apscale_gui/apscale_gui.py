...
def run_apscale_blast(project_folder, available_fasta_files, available_databases):
    ...
    filter_mode = 1
    b_filter(output_folder, database, thresholds, str(st.session_state['n_cores']), filter_mode)
    ...
    if not path_to_projects.exists() and not path_to_projects.is_dir():
        ...
        if n_demultiplexing_data == 0:
            ...
            if not settings_xlsx.exists():
                ...
                with st.expander("🧩 TaxonTableTools Table Conversion", expanded=False):
                    st.write(
                        "Combine your read and taxonomy tables into a single, merged table. "
                        "This format is fully compatible with TaxonTableTools for downstream analyses "
                        "and provides a more convenient structure for working with your data."
                    )
                    available_read_tables = {Path(i).name: Path(i) for i in glob.glob(str(project_folder / '11_read_table' / 'data' / '*_read_table_*.xlsx'))}
                    available_taxonomy_tables = {Path(i).name: Path(i) for i in glob.glob(str(project_folder / '11_read_table' / 'data' / 'blastn_*' / 'blastn_*.xlsx'))}
                    st.selectbox(label='Read table', key='TTT_read_table', options=list(available_read_tables.keys()))
                    st.selectbox(label='Taxonomy table', key='TTT_taxonomy_table', options=list(available_taxonomy_tables.keys()))

                    if st.button('Merge tables'):
                        merge_tables(available_read_tables[st.session_state['TTT_read_table']], available_taxonomy_tables[st.session_state['TTT_taxonomy_table']])
