...
class ChatseeTracker:
    ...
    _API_ENDPOINT = "https://qa-api.chatsee.ai:8000/v1/api/process_interaction"
    ...
    def __init__(self, 
        ...
        Args:
            ...
            api_key: Your Chatsee API key. If None, SDK will try to read
                     from the 'CHATSEE_API_KEY' environment variable.
        ...
        _api_key_to_use = api_key or "this_is_a_demo_key_please_replace"
        if not _api_key_to_use:
            raise ValueError("api_key must be provided either as an argument or "
                             "as a 'CHATSEE_API_KEY' environment variable.")
        ...
        self.session.headers.update({
            "Authorization": f"Bearer {_api_key_to_use}",
    ...
    def _send_to_api(self, entry: ConversationEntry):
        ...
        try:
            ...
            logger.error(f"API Error: Failed to send turn data. "
                         ...
                         f"Response: {e.response.text}", exc_info=True)
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            'session_id': self.session_id,
            'agent_id': self.agent_id,
            'user_id': self.user_id,  # <-- HERE IS THE FIX
            'tenant_id': self.tenant_id,
            'tenant_name': self.tenant_name,
