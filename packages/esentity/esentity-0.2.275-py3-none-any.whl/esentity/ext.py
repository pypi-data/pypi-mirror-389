#coding: utf-8
from flask import current_app, g, request, abort, url_for
...
class CasinosExtension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        if has_append and found < count and tags:
            kwargs = {
                'is_searchable': True,
                'is_redirect': False,
                'country': current_user.country_full,
...
class UIExtension(Extension):
    ...
    def _render(self, args, caller):
        logger.info(f'UI extension Args IN: {args}')

        default = ['features', False, '|', ':']
        _ca = len(default)
        args = args[:_ca] + default[len(args):] + args[_ca:]

        logger.info('UI extension Args OUT: {0}'.format(args))
        ...
        # else:
        #     logger.info(f'RV: {rv}')
        #     vars = rv.split(dlmtr_items)
        #     if vars and ('ui__payload' == vars[0] or vars[0].startswith('http')):

        #         url = vars[0]
        #         if 'ui__payload' == vars[0]:
        #             url = url_for(vars[0], _external=True)
        ...
        #         logger.debug(f'UI extension request url: {url}, payload: {payload}')
        #         r = requests.post(url, json=payload)
        #         if r:
        #             items = r.json()
        #         else:
        #             logger.warning(f'UI extension request error: {r.status_code}')
...
class SitemapExtension(Extension):
    ...
    def _render(self, args, caller=None):
        ...
        pages, total = Page.get(
            ...
            is_redirect=False, 
            locale=g.language, 
            _source=['category', 'path', 'title', 'alias', 'publishedon', 'tags', 'locale_available'],
            _all=True,
            _sort=[
