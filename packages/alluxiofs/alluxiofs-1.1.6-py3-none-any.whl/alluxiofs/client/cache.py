...
class LocalCacheManager:
    ...
    def _atomic_write(self, file_path_hashed, worker_host, worker_http_port, file_path, start, end):
        ...
        try:
            ...
            self.current_cache_size.add(end - start)
            self.logger.debug(
            ...
            with self.lock:
                self.current_cache_size.sub(end-start)
            self.logger.debug(
    ...
    def _evict_if_needed(self, length):
        ...
        if (
            self.current_cache_size.get() + length
            <= self.max_cache_size * self.evcit_rate
        ):
            return
        cached_files = self._get_files_sorted_by_atime_scandir()
        while (
            self.current_cache_size.get() + length > self.max_cache_size / 2
            and len(cached_files)
        ):
            old_path = cached_files.pop(0)["name"]
            try:
