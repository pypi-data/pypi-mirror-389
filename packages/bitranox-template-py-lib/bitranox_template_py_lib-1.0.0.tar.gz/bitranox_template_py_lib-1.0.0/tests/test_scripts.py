...
def test_get_project_metadata_fields():
    ...
    assert meta.name == "bitranox_template_py_lib"
    assert meta.slug == "bitranox-template-py-lib"
    assert meta.import_package == "bitranox_template_py_lib"
    assert meta.coverage_source == "src/bitranox_template_py_lib"
    assert meta.github_tarball_url("1.2.3").endswith("/bitranox/bitranox_template_py_lib/archive/refs/tags/v1.2.3.tar.gz")
    assert meta.version
    assert meta.summary
    assert meta.author_name
    assert meta.metadata_module.as_posix().endswith("src/bitranox_template_py_lib/__init__conf__.py")
