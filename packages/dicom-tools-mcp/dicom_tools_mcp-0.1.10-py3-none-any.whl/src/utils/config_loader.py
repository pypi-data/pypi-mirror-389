...
def load_config(config_path: str = 'config.json') -> Dict:
    ...
    raise FileNotFoundError(
        ...
        f"  1. Set environment variables:\n"
        f"     - BASE_URL (analysis server URL)\n"
        f"     - ORTHANC_BASE_URL (Orthanc server URL)\n"
        f"     - ORTHANC_COOKIE (authentication cookie)\n"
        f"  2. Create {config_path} file in:\n"
        f"     - Current directory: {os.getcwd()}\n\n"
        f"Example environment variables:\n"
        f"  BASE_URL=http://192.168.4.220:26666\n"
        f"  ORTHANC_BASE_URL=http://192.168.4.220:18997\n"
