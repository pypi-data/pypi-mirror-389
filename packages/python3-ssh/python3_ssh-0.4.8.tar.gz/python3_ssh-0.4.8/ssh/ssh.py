...
class SSHClient(object):
    ...
    def _scp_to_remote(self, local_path, remote_path):
        ...
        if local_path.is_file():
            log.info(f" {self.__ip}:{self.__port} | Copy file: {local_path} -> {remote_path}")
            self.__sftp.put(str(local_path), str(remote_path))
            # 验证文件是否存在
            rs = self.exec(f"ls {remote_path}")
            if rs.exit_status_code != 0:
                log.warning(f" {self.__ip}:{self.__port} | File copy failed: {remote_path} not found")
                return False
            return True

        # 2. 处理本地文件夹：递归创建远端目录并拷贝所有内容
        if local_path.is_dir():
            # 创建远端目录（-p 确保父目录存在，忽略已存在错误）
            mkdir_cmd = f"mkdir -p {remote_path}"
            rs = self.exec(mkdir_cmd)
            if rs.exit_status_code != 0:
                log.error(f" {self.__ip}:{self.__port} | Failed to create remote dir: {remote_path}, cmd: {mkdir_cmd}")
                return False
            ...
            for root, dirs, files in os.walk(local_path):
                ...
                remote_root = remote_path / relative_root

                # 创建远端子目录
                if dirs:
                    remote_dir_cmd = f"mkdir -p {remote_root}"
                    self.exec(remote_dir_cmd)
                ...
                for file in files:
                    ...
                    try:
                        ...
                        log.debug(f" {self.__ip}:{self.__port} | Copied: {local_file} -> {remote_file}")
                    except Exception as e:
                        log.error(
                            f" {self.__ip}:{self.__port} | Copy failed: {local_file} -> {remote_file}, error: {str(e)}")
                        return False

            log.info(f" {self.__ip}:{self.__port} | Directory copy completed: {local_path} -> {remote_path}")
            return True

        # 3. 本地路径不存在或不是文件/文件夹
        log.error(f" {self.__ip}:{self.__port} | Local path invalid: {local_path} (not file/dir)")
        return False
    ...
    def scp_to_remote(self, local_path, remote_path, timeout=120):
        try:
            ...
            # 转换为 Path 类型，统一路径处理（兼容字符串输入）
            local_path = Path(local_path)
            remote_path = Path(remote_path)

            # 优化：如果本地是文件夹，远端路径自动指向目标文件夹（而非文件夹下的文件）
            if local_path.is_dir() and self.exec(f"test -f {remote_path}").exit_status_code == 0:
                log.warning(
                    f" {self.__ip}:{self.__port} | Remote path exists as file: {remote_path}, will overwrite to dir")
                self.exec(f"rm -f {remote_path}")

            return self._scp_to_remote(local_path, remote_path)
        except Exception:
            log.warning(f" {self.__ip}:{self.__port} | SCP exception: local {local_path} -> remote {remote_path}",
                        exc_info=True)
            return False
