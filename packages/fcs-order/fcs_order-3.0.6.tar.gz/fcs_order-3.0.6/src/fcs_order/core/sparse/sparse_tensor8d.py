"""
Sparse 8D tensor implementation using scipy sparse matrices.
"""
...
class SparseTensor8D:
    """
    A sparse 8D tensor implementation using dictionary storage with scipy support.
    ...
    def __init__(
        ...
        """
        Initialize sparse 8D tensor.

        Args:
            shape: Shape of the 8D tensor (dim1, dim2, dim3, dim4, dim5, dim6, dim7, dim8)
        ...
        self._data = {}  # {(i1,i2,i3,i4,i5,i6,i7,i8): value}
    ...
    def _flatten_index(
        self, indices: Tuple[int, int, int, int, int, int, int, int]
    ) -> int:
        """Convert 8D indices to flattened 1D index."""
        i1, i2, i3, i4, i5, i6, i7, i8 = indices
        return (
            (
                (
                    (
                        ((i1 * self.shape[1] + i2) * self.shape[2] + i3) * self.shape[3]
                        + i4
                    )
                    * self.shape[4]
                    + i5
    ...
    def _unflatten_index(
        ...
        """Convert flattened 1D index back to 8D indices."""
        i8 = flat_index % self.shape[7]
        flat_index //= self.shape[7]
        i7 = flat_index % self.shape[6]
        flat_index //= self.shape[6]
    ...
    def __getitem__(self, indices):
        ...
        if isinstance(indices, tuple) and len(indices) == 8:
            ...
            raise IndexError("SparseTensor8D requires 8 indices")
    ...
    def nonzero(
        ...
        i1, i2, i3, i4, i5, i6, i7, i8 = zip(*indices)
        return (
            ...
            np.array(i7, dtype=int),
            np.array(i8, dtype=int),
        )
    ...
    def __repr__(self):
        return f"SparseTensor8D(shape={self.shape}, non_zero={len(self._data)}, sparsity={self.get_sparsity():.2%})"
