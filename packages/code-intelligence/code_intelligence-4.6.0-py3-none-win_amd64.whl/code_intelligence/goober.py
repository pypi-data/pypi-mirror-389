...
class Definition_Link:
    ...
    @classmethod
    def from_c(cls, link: Definition_Link_C):
        return cls(
...
def find_symbols_in_text(text: str, callback=None, data=None, ignore_word_like: bool = False, sync: bool = False, first_look_at_these_files: list[Path|str] = []) -> bool:
    if callback is None:
        return True
    ...
    for i, it in enumerate(first_look_at_these_files):
        first_look_at_these_files_c[i] = to_str_c(os.fsencode(it))
