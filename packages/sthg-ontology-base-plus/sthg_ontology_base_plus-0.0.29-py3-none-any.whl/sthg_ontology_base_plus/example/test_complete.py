...
def test_direct_imports():
    ...
    try:
        ...
        from sthg_ontology_base_plus.function.aggregation_types import Range, TwoDimensionalAggregation, \
            ThreeDimensionalAggregation
        from sthg_ontology_base_plus.function.ontology_types import Object, OntologyEdit
...
def test_type_properties():
    ...
    try:
        ...
        print(f"-----------> att.filename: {att.filename}")
...
if __name__ == "__main__":
    ...
    from sthg_ontology_base_plus.function.api import Attachment


    class myClass():
        # @Function
        # def testgetmodel(self) -> list[str]:
        # #"这是另一段描述"
        #     api_name = "agg_huancun"
        #     client = FoundryClient(api_name)
        #     agg = client.get_model(api_name)
        #     ret = agg.where(agg.ad_id==1).all()
        #     return ret

        def testwjz111(self, att: Attachment):
            filename = att.filename
            return filename


    test_class = myClass()
    att_obj = Attachment("test.txt")
    res1 = test_class.testwjz111(att_obj)
    print(res1)

    # a(Attachment(filename="test.txt", content=b"Hello World"))
