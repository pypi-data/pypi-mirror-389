...
@ExportLibCpp.register("Reciprocal",
    aidge_core.ImplSpec(aidge_core.IOSpec(aidge_core.dtype.float32)),
    aidge_core.ProdConso.in_place_model)
class ReciprocalCPP(ExportNodeCpp):
    def __init__(self, node, mem_info):
        super().__init__(node, mem_info)
        self.config_template = str(ROOT / "templates" / "configuration" / "reciprocal_config.jinja")
        self.forward_template = str(ROOT / "templates" / "kernel_forward" / "reciprocal_forward.jinja")
        self.include_list = []
        self.add_kernel_to_copy(ROOT / "kernels" / "reciprocal.hpp", "include/kernels/cpp")
