...
#ifdef _OPENMP
        // Wrapping cannot occur in the middle of a line, except if there is only one line (1D)!
#pragma omp parallel for collapse(2) if (!(OUTPUT_MEM_WRAP_SIZE > 0 && KERNEL_WIDTH > 1 && OUTPUTS_HEIGHT == 1))
#endif
            ...
            for (size_t outputPack = 0; outputPack < NB_OUTPUTS_PACKED; ++outputPack) {
                ...
                for (size_t packElt = 0; packElt < n_pack<Output_T>(); ++packElt) {
                    ...
                    for (size_t sy = 0; sy < KERNEL_HEIGHT; ++sy) {
                        ...
                        // sxMin must be included here, otherwise iPos could point
                        // to the previous line and wrapping not added correctly!
                        const size_t iPos = (ix + sxMin) + CHANNELS_WIDTH * (iy + sy*DILATION_Y);
