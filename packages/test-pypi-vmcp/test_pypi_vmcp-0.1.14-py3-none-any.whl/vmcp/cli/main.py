...
def run(
    ...
    if host is not None: settings.host = host
    if port is not None: settings.port = port
...
def vmcp_list():
    ...
    """
    try:
        from vmcp.vmcps.config_manager import VMCPConfigManager
...
def docs_start(
    ...
    if open_browser:
        def open_browser_when_ready():
            ...
            for attempt in range(max_attempts):
                try:
                    response = httpx.get(f"{settings.base_url}", timeout=1.0)
                    if response.status_code == 200:
