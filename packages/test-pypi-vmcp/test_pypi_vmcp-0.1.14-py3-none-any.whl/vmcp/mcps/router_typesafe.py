...
from vmcp.mcps.models_new import (
    # Request models
    MCPInstallRequest,
    MCPUpdateRequest,
    RenameServerRequest,
...
from vmcp.shared.models import BaseResponse, ErrorResponse
...
async def list_mcp_servers(
    ...
    try:
        ...
        connected_count = len([s for s in server_info if s.status == "connected"])
        disconnected_count = len([s for s in server_info if s.status == "disconnected"])
        auth_required_count = len([s for s in server_info if s.status == "auth_required"])
        error_count = len([s for s in server_info if s.status == "error"])
...
async def list_global_mcp_servers(
    ...
    try:
        ...
        logger.error(f"❌ Exception type: {type(e).__name__}")
        logger.error(f"❌ Full traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Failed to list global MCP servers: {str(e)}")
    finally:
        if 'db' in locals():
            db.close()
