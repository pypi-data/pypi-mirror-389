...
def _parse_python_function_schema(custom_tool: dict) -> dict:
    ...
    This helper function is used by tools_list to build JSON schema for Python-based custom tools.

    Args:
        custom_tool: Dictionary containing tool configuration with 'variables' key
        ...
        for tool in server_tools:
            ...
            if "widget_id" in tool_override and tool_override["widget_id"]:
                ...
                try:
                    ...
                    blob = ""
                    # Note: Built files logic commented out in original
                    ...
                    logger.info(f"Loaded {len(widgets)} widgets for vMCP: {vmcp_id}")
