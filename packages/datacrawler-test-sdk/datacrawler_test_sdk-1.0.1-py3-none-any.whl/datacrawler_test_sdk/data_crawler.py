...
from datacrawler_test_sdk.utils import (FuzzyText, ResultItem, get_non_rec_text,
                               normalize, text_match, unique_hashable,
                               unique_stack_list)


class DataCrawler(object):
    """
    DataCrawler : A Smart, Automatic, Fast and Lightweight Web Scraper for Python.
    DataCrawler automatically learns a set of rules required to extract the needed content
        from a web page. So the programmer doesn't need to explicitly construct the rules.

    Attributes
    ----------
    stack_list: list
        List of rules learned by DataCrawler
    ...
    def build(
        ...
        wanted_list: list of strings or compiled regular expressions, optional
            A list of needed contents to be scraped.
                DataCrawler learns a set of rules to scrape these targets. If specified,
        ...
        wanted_dict: dict, optional
            A dict of needed contents to be scraped. Keys are aliases and values are list of target texts
                ...
                DataCrawler learns a set of rules to scrape these targets and sets its aliases.
