    ...
    def __init__(self, root: str):
        ...
        while retries < max_retries:
            try:
                ...
                self._logger.info("Zenoh session opened successfully.")
                break  # exit the retry loop
            except Exception as e:
                retries += 1
                self._logger.warning(f"Attempt {retries}/{max_retries} failed to open Zenoh session: {e}")
                if retries < max_retries:
                    self._logger.info(f"Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                else:
                    self._logger.error(f"Could not open Zenoh session after {max_retries} attempts.")
                    import traceback
                    traceback.print_exc()
                    os._exit(1)  # Force container exit if all retries fail
