...
class AutomationToolkit:
    ...
    def detect_qr_code(self, region: Tuple[int, int, int, int] = None,
                       debug: bool = False) -> Dict[str, Any]:
        """
        使用OpenCV检测屏幕上的二维码并解析内容
        ...
        try:
            ...
            if region:
                ...
                if x2 <= x1 or y2 <= y1:
                    return {"success": False, "error": "无效的区域设置"}

                detection_image = screenshot[y1:y2, x1:x2]
                region_offset = (x1, y1)
            else:
            ...
            # 使用OpenCV检测二维码
            qr_detect_start = time.time()
            data, points, straight_qrcode = self.qr_detector.detectAndDecode(detection_image)
            qr_detect_time = time.time() - qr_detect_start
            ...
            if points is not None and len(points) > 0 and data:
                ...
                # 转换为全屏坐标
                top_left_full = (
                    top_left[0] + region_offset[0],
                    top_left[1] + region_offset[1]
                )
                bottom_right_full = (
                    bottom_right[0] + region_offset[0],
                    bottom_right[1] + region_offset[1]
                )

                # 计算中心点
                center_x = (top_left_full[0] + bottom_right_full[0]) // 2
                center_y = (top_left_full[1] + bottom_right_full[1]) // 2

                qr_info = {
                    "data": data,
                    "type": "QRCODE",
                    "position": {
                        "top_left": top_left_full,
                        "bottom_right": bottom_right_full,
                        "center": (center_x, center_y),
                        "polygon": points[0].tolist()
                    },
                    "bounding_box": (top_left_full[0], top_left_full[1],
                                     bottom_right_full[0], bottom_right_full[1])
                }

                qr_codes.append(qr_info)

                logger.info(
                    f"{self.task_id}--检测到二维码: {data[:50]}{'...' if len(data) > 50 else ''}, "
                    f"位置: {top_left_full} -> {bottom_right_full}"
                )

            # 保存调试图像
            if debug:
                debug_start = time.time()
                self._save_qr_debug_image(
                    detection_image, qr_codes, region_offset
                )
                debug_time = time.time() - debug_start
                logger.debug(f"{self.task_id}--二维码调试图像保存耗时: {debug_time:.3f}s")
            ...
            result = {
                ...
                "qr_detect_time": qr_detect_time
            }

            logger.debug(
                f"{self.task_id}--二维码检测完成: 找到{len(qr_codes)}个二维码, "
                f"总耗时: {total_time:.3f}s "
                f"(截图: {screenshot_time:.3f}s, 区域: {region_time:.3f}s, "
                f"识别: {qr_detect_time:.3f}s)"
            )
            ...
            total_time = time.time() - start_time
            logger.error(f"{self.task_id}--二维码检测失败: {e}, 耗时: {total_time:.3f}s")
            return {
                "success": False,
                "error": str(e),
                "processing_time": total_time
    ...
    # def wait_for_qr_code(self, timeout: int = 30, check_interval: float = 2,
    ...
    #
    #     logger.info(f"{self.task_id}--开始等待二维码出现，超时: {timeout}秒")
    #
    #     while time.time() < end_time:
    #         check_start = time.time()
    #         result = self.detect_qr_code(region=region, debug=debug)
    #         check_time = time.time() - check_start
    #
    #         if result["success"] and result["qr_codes_found"] > 0:
    #             total_time = time.time() - start_time
    #             logger.info(
    #                 f"{self.task_id}--成功检测到二维码，等待时间: {total_time:.1f}秒, "
    #                 f"内容: {result['qr_codes'][0]['data'][:50]}..."
    #             )
    #             result["wait_time"] = total_time
    #             return result
    #
    #         # 计算剩余时间
    ...
    #             logger.debug(
    #                 f"{self.task_id}--未检测到二维码，{sleep_time:.1f}秒后重试，"
    #                 f"剩余时间: {remaining:.1f}秒，本次检查耗时: {check_time:.3f}s"
    #             )
    #             sleep(sleep_time)
    #
    #     total_time = time.time() - start_time
    #     logger.warning(f"{self.task_id}--等待二维码超时，总耗时: {total_time:.1f}秒")
    #     return {
    #         "success": False,
    #         "error": "等待二维码超时",
    ...
    # def click_qr_code(self, qr_index: int = 0, offset_x: int = 0, offset_y: int = 0,
    #                   region: Tuple[int, int, int, int] = None) -> bool:
    #     """
    #     点击检测到的二维码
    #
    #     Args:
    #         qr_index: 二维码索引（当有多个二维码时）
    #         offset_x: X轴偏移
    #         offset_y: Y轴偏移
    #         region: 检测区域
    #
    ...
    #     result = self.detect_qr_code(region=region)
    #     detect_time = time.time() - detect_start
    #
    #     if not result["success"] or result["qr_codes_found"] == 0:
    #         total_time = time.time() - start_time
    #         logger.warning(f"{self.task_id}--未检测到二维码，无法点击，耗时: {total_time:.3f}s")
    #         return False
    #
    #     if qr_index >= result["qr_codes_found"]:
    #         total_time = time.time() - start_time
    #         logger.warning(
    #             f"{self.task_id}--二维码索引超出范围: {qr_index}，"
    #             f"实际检测到{result['qr_codes_found']}个二维码，耗时: {total_time:.3f}s"
    #         )
    #         return False
    #
    #     try:
    #         # 获取二维码中心坐标
    #         qr_code = result["qr_codes"][qr_index]
    #         center_x, center_y = qr_code["position"]["center"]
    #
    #         # 应用偏移
    ...
    #         total_time = time.time() - start_time
    #         logger.info(
    #             f"{self.task_id}--点击二维码成功: 位置({target_x}, {target_y}), "
    #             f"内容: {qr_code['data'][:30]}..., "
    #             f"总耗时: {total_time:.3f}s (检测: {detect_time:.3f}s, 点击: {click_time:.3f}s)"
    #         )
    #
    #         time.sleep(1.5)
    #         return True
    ...
    #     except Exception as e:
    #         total_time = time.time() - start_time
    #         logger.error(f"{self.task_id}--点击二维码失败: {e}, 耗时: {total_time:.3f}s")
    #         return False

    # def monitor_qr_code_changes(self, check_interval: float = 5,
    #                             max_checks: int = 10,
    ...
    #     changes_detected = 0
    #
    #     logger.info(f"{self.task_id}--开始监控二维码变化，最多检查{max_checks}次")
    #
    ...
    #         current_data = None
    #         if result["success"] and result["qr_codes_found"] > 0:
    #             current_data = result["qr_codes"][0]["data"]
    #
    #         # 检查变化
    #         if current_data != previous_data:
    #             changes_detected += 1
    #
    #             if previous_data is None and current_data:
    #                 logger.info(f"{self.task_id}--检测到二维码出现: {current_data[:50]}...")
    #             elif current_data is None and previous_data:
    #                 logger.info(f"{self.task_id}--检测到二维码消失")
    #             else:
    #                 logger.info(
    #                     f"{self.task_id}--检测到二维码变化: "
    #                     f"'{previous_data[:30]}...' -> '{current_data[:30]}...'"
    #                 )
    #
    ...
    #             logger.debug(
    #                 f"{self.task_id}--第{check_count + 1}次检查完成，"
    #                 f"{check_interval}秒后继续，本次检查耗时: {check_time:.3f}s"
    #             )
    ...
    #
    #     logger.info(
    #         f"{self.task_id}--二维码监控完成: 检查{max_checks}次，"
    #         f"检测到{changes_detected}次变化，总耗时: {total_time:.1f}秒"
    #     )
    #
    #     return monitoring_result

    def get_qr_code_content(self, region: Tuple[int, int, int, int] = None) -> Optional[str]:
        """
        快速获取二维码内容（简化版）
        ...
        Returns:
            str: 二维码内容，如果没有检测到则返回None
        """
        try:
            result = self.detect_qr_code(region=region)

            if result["success"] and result["qr_codes_found"] > 0:
                return result["qr_codes"][0]["data"]
            else:
                return None

        except Exception as e:
            logger.error(f"{self.task_id}--获取二维码内容失败: {e}")
            return None
    ...
    def batch_ocr_check_all(self, text_checks: List[dict]) -> Dict[str, Any]:
        """
        批量OCR检查，必须所有文本都存在才返回真
        text_checks示例:
        [
            {"text": "进入游戏", "region": None},
            {"text": "密码错误", "region": (729, 464, 1534, 570)},
        ...
        try:
            ...
            screenshot = self.d.screenshot(format='opencv')
            if screenshot is None:
                return {"found": False, "result": None, "all_results": {}}
            ...
            if all_found:
                ...
                # 找出未找到的文本
                missing_texts = [text for text, found in results.items() if not found]
                return {
                    "found": False,
                    "all_results": results,
                    "processing_time": total_time,
                    "missing_texts": missing_texts,
                    "message": f"以下文本未找到: {missing_texts}"
                }
