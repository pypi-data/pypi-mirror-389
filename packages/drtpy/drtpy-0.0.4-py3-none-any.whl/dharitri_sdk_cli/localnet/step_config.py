...
def copy_config_to_nodes(config: ConfigRoot):
    config_prototype = config.software.drt_go_chain.get_node_config_folder()
...
def copy_config_to_seednode(config: ConfigRoot):
    config_source = config.software.drt_go_chain.get_seednode_config_folder()
...
def copy_config_to_proxy(config: ConfigRoot):
    config_prototype = config.software.drt_go_chain_proxy.get_proxy_config_folder()
    ...
    # https://github.com/TerraDharitri/drt-go-chain-proxy/pull/454
...
def copy_binaries_into_localnet_workspace(config: ConfigRoot):
    cmd_node = config.software.drt_go_chain.get_cmd_node_folder()
    cmd_seednode = config.software.drt_go_chain.get_cmd_seednode_folder()
    cmd_proxy = config.software.drt_go_chain_proxy.get_cmd_proxy_folder()
