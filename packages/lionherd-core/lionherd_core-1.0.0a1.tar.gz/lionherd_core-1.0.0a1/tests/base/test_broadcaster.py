...
Singleton Invariant:
    ∀ calls: Broadcaster() → same instance

Subscription:
    subscribe(cb) ⟹ _subscribers.append(cb) if cb ∉ _subscribers
    unsubscribe(cb) ⟹ _subscribers.remove(cb) if cb ∈ _subscribers
    broadcast(evt) ⟹ ∀cb ∈ _subscribers: cb(evt) // Fire-and-forget
