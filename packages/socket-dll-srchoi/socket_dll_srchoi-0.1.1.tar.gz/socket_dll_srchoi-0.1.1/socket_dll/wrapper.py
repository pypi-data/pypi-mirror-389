...
class TCPServer:
    ...
    def accept(self):
        ...
        return TCPClient(_socket_fd=client_fd, _socket_dll=self.socket_dll)
...
class TCPClient:
    ...
    def __init__(self, host=None, port=None, dll_path=None, _socket_fd=None, _socket_dll=None):
        """TCP 클라이언트 생성"""
        if _socket_dll:
            self.socket_dll = _socket_dll
        else:
            self.socket_dll = SocketDLL(dll_path)

        self.socket_fd = _socket_fd if _socket_fd is not None else -1
