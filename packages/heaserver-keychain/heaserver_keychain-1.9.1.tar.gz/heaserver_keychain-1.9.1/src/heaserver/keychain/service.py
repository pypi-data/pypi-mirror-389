...
from heaobject.root import Share, ShareImpl, Permission, ViewerPermissionContext, PermissionContext, desktop_object_type_for_name
...
from heaserver.service.runner import init_cmd_line, routes, start, web, scheduled_cleanup_ctx_factory
...
async def get_credentials_view(request: web.Request) -> web.Response:
    ...
    async with mongo.MongoContext(request) as mongo_:
        ...
        if credentials_dict is not None:
            view, perms, attr_perms = await _new_credentials_view(context, credentials_dict)
            return await response.get(request, view.to_dict(), perms, attr_perms)
        else:
            return response.status_not_found()
...
async def _create_or_get_managed_user(iam_client: IAMClient, username: str,
    ...
    for policy in policies['AttachedPolicies']:
        await asyncio.to_thread(iam_client.attach_user_policy, UserName=username, PolicyArn=policy['PolicyArn'])
    return (await asyncio.to_thread(iam_client.create_access_key, UserName=username))['AccessKey']
