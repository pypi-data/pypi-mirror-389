...
def get_language_for_tree(language: str) -> Language:
    try:
        ...
        so_library_path: str = os.path.join(
            TREE_SITTER_PARSERS_PATH or "", f"{language}.so"
        )
...
def query_function_by_name(
    ...
    result = QueryCursor(tree_sitter.Query(language_parser, query)).captures(
        tree.root_node
    )
    return [
        node
...
def find_function_expression_by_query(
    ...
    language_parser = get_language_for_tree(language)
    target_nodes: tuple[str, ...] = TREE_SITTER_FUNCTION_DECLARATION_MAP.get(
        language, ()
    )
...
def _calculate_positions(
    lines: list[bytes], desired_line: int
...
def extract_imports(source_code: str, language: str) -> tuple[Node, ...]:
    ...
    result = query_nodes_by_language(
        language, tree, TREE_SITTER_IMPORTS_DECLARATIONS_MAP
    )
...
def indent_function(
    file_content_bytes: bytes, new_function: str, function_location: int
) -> str:
    indentation = (
        file_content_bytes[
            function_location
    ...
    return "\n".join(
        [indentation + line if line else line for line in new_function.splitlines()]
    )
