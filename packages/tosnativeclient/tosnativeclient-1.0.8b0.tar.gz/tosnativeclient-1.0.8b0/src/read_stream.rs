...
pub struct ReadStream {
    ...
    runtime: Arc<Runtime>,
    output: Option<Result<GetObjectOutput, TosError>>,
    #[pyo3(get)]
    bucket: String,
    #[pyo3(get)]
    key: String,
...
impl ReadStream {
    ...
    pub fn read(
        ...
        if let Some(output) = slf.output.as_mut() {
            return match output {
                Err(ex) => Err(map_tos_error(ex.clone())),
                Ok(o) => match o.read_all() {
                    ...
                    Ok(data) => {
                        if data.len() == 0 {
                            ...
                            Ok(Some(PyBytes::new(slf.py(), data.as_ref())))
                        }
                    }
                },
            };
