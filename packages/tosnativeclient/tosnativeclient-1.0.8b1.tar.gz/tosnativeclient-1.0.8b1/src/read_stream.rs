use crate::tos_client::{InnerTosClient, SharedPrefetchContext, SyncInnerTosClient};
...
use pyo3::{pyclass, pymethods, Bound, IntoPyObject, PyErr, PyRef, PyRefMut, PyResult};
...
use ve_tos_rust_sdk::object::{GetObjectInput, GetObjectOutput, ObjectAPI as OAPI, ObjectContent};
...
pub struct ReadStream {
    ...
    runtime: Arc<Runtime>,
    output: Option<Arc<std::sync::Mutex<Result<GetObjectOutput, TosError>>>>,
    #[pyo3(get)]
    bucket: String,
    #[pyo3(get)]
    key: String,
...
impl ReadStream {
    ...
    pub fn read(
        ...
        if slf.output.is_some() {
            let output = slf.output.clone().unwrap();
            return match slf.py().allow_threads(move || {
                let mut output = output.lock().unwrap();
                match output.as_mut() {
                    Err(ex) => Err(ex.clone()),
                    Ok(o) => match o.read_all() {
                        Err(ex) => Err(ex),
                        Ok(data) => {
                            if data.len() == 0 {
                                Ok(None)
...
impl ReadStream {
    pub(crate) fn sync_new(
        sync_client: Arc<SyncInnerTosClient>,
        ...
        let mut input = GetObjectInput::new(bucket.as_str(), key.as_str());
        input.set_if_match(etag.as_str());
        let output = sync_client.get_object(&input);
        Self {
            ...
            runtime,
            output: Some(Arc::new(std::sync::Mutex::new(output))),
            bucket,
            key,
            etag,
            size: 0,
