...
try:
    ...
    MPSBackend = None
    MPS_IMPORT_ERROR = str(exc)
else:
    MPS_IMPORT_ERROR = None
...
def build_low_entanglement_circuit(qubits: int, depth: int) -> QuantumCircuit:
    circuit = QuantumCircuit(qubits, qubits)
    for layer in range(depth):
        for idx in range(qubits):
            circuit.ry(0.1 * (layer + 1), idx)
            circuit.rz(0.2 * (layer + 1), idx)
        for idx in range(0, qubits - 1, 2):
            circuit.cx(idx, idx + 1)
        for idx in range(1, qubits - 1, 2):
            circuit.cx(idx, idx + 1)
    circuit.measure_all()
    return circuit


def build_entangling_chain(qubits: int) -> QuantumCircuit:
    circuit = QuantumCircuit(qubits, qubits)
    circuit.h(0)
    for idx in range(qubits - 1):
        circuit.cx(idx, idx + 1)
    for idx in range(qubits):
        circuit.ry(0.15 * (idx + 1), idx)
        circuit.rz(0.27 * (idx + 1), idx)
    circuit.measure_all()
    return circuit
...
def case_definitions() -> list[BenchmarkCase]:
    return [
        BenchmarkCase("low_entanglement_10", build_low_entanglement_circuit(10, depth=4)),
        BenchmarkCase("low_entanglement_16", build_low_entanglement_circuit(16, depth=3)),
        BenchmarkCase("entangling_chain_12", build_entangling_chain(12)),
    ]
...
def run_mps_backend(circuit: QuantumCircuit, shots: int, backend: MPSBackend) -> None:  # type: ignore[name-defined]
...
def benchmark_case(
    ...
    repetitions: int,
    include_qiskit: bool,
    mps_backend: MPSBackend | None,  # type: ignore[name-defined]
    ...
    if mps_backend is None:
        success = False
        error = MPS_IMPORT_ERROR or "MPS backend unavailable"
    else:
        for _ in range(repetitions):
            try:
                timings.append(time_call(lambda: run_mps_backend(case.circuit, shots, mps_backend)))
            except Exception as exc:  # noqa: BLE001
                success = False
                error = str(exc)
                break
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark MPS backend against Qiskit CPU")
    ...
    backend_instance: MPSBackend | None
    if MPSBackend is None:
        backend_instance = None
    else:
        backend_instance = MPSBackend()
