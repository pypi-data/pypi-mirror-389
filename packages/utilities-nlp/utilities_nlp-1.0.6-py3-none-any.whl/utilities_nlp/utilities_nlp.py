...
class UtilitiesNLP:
    ...
    def listGitHubFiles(self, url_path=''):
        try:
            ...
            from urllib.request import urlopen
            from json import loads
            list_path = url_path.split('https://github.com/')[1:][0].split('/')
            user_name, repository_name, folder_name = list_path[0], list_path[1], list_path[-1]
            api_url = f'https://api.github.com/repos/{user_name}/{repository_name}/contents/{folder_name}?ref=main'
            data = loads(urlopen(api_url).read().decode())
            files = []
            for item in data:
                if item['type'] == 'file': files.append(item['download_url'])
                elif item['type'] == 'dir': files.extend(listGitHubFiles(user_name, repository_name, item['path'], 'main'))
            return files
        except Exception as error:
            if self.__show_errors:
