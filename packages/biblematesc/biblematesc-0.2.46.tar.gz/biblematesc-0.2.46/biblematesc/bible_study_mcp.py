...
1. Call tool 'retrieve_chinese_bible_verses' for Bible text, 
...
# Additional tools for Chinese version
...
def retrieve_english_bible_verses(request:str) -> str:
    ...
    if not refs:
        return "Please provide a valid Bible reference to complete your request."
    return run_uba_api(f"BIBLE:::NET:::{refs}")
...
def retrieve_chinese_bible_verses(request:str) -> str:
    ...
    if not refs:
        return "Please provide a valid Bible reference to complete your request."
    return run_uba_api(f"BIBLE:::CUVs:::{refs}")

@mcp.tool
def retrieve_english_bible_chapter(request:str) -> str:
    """retrieve a whole Bible chapter in English; bible chapter reference must be given, e.g. John 3"""
    global BibleVerseParser, run_uba_api, re
    refs = re.sub("[Cc]hapter ([0-9])", r"\1", request)
    ...
    output = []
    for ref in refs.split("; "):
        output.append(run_uba_api(f"CHAPTER:::NET:::{ref}"))
    return "\n\n".join(output)
...
def retrieve_chinese_bible_chapter(request:str) -> str:
    ...
    output = []
    for ref in refs.split("; "):
        output.append(run_uba_api(f"CHAPTER:::CUVs:::{ref}"))
    return "\n\n".join(output)
