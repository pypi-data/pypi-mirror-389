...
from .transport import CombinedTransport, HTTPTransport
...
class AISAgent:
    ...
    def __init__(
        ...
        # Transport layer
        self.transport: Union[CombinedTransport, HTTPTransport]
    ...
    async def _handle_message(self, message: AISMessage) -> AISMessage:
        ...
        try:
            ...
            elif message.message_type == MessageType.SESSION_END:
    ...
    async def _handle_request(self, message: AISMessage) -> AISMessage:
        ...
        if not capability_name or not isinstance(capability_name, str):
            return AISMessage.create_error(
                ...
                error_message="Missing or invalid 'capability' field in request",
                session_id=message.session_id,
        ...
        try:
            ...
            # request_id should always be present in valid requests, but provide fallback
            final_request_id = request_id if request_id else "unknown"
    ...
    async def start(
        ...
        # Start transport
        if isinstance(self.transport, CombinedTransport):
            # Combined transport has start() method
            await self.transport.start(host=host, port=port)
        else:
            # HTTP transport has start_server() method
    ...
    async def stop(self) -> None:
        ...
        # Stop transport
        if isinstance(self.transport, CombinedTransport):
            # Combined transport has stop() method
            await self.transport.stop()
        else:
            # HTTP transport has stop_server() method
