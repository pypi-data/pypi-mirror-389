...
class AISClient:
    ...
    def __init__(
        ...
        # Streaming
        self._stream_handlers: Dict[str, Callable[..., Any]] = {}
    ...
    async def connect(
        ...
        handshake = AISMessage.create_handshake(
            ...
            capabilities=list(self.capabilities),  # Convert to list to satisfy Sequence type
            auth=auth_data,
        ...
        if response.message_type != MessageType.CAPABILITY_DECLARATION:
    ...
    async def disconnect(self) -> None:
        ...
        try:
            # Send session end message (only if we have valid session info)
            if self._server_agent_id and self._session_id:
    ...
    async def call(
        ...
        # Ensure we have valid session info
        if not self._server_agent_id or not self._session_id:
            raise SessionError("Invalid session state. Please reconnect.")
    ...
    async def call_with_retry(
        ...
        if last_exception:
            ...
            raise AISException("Call failed with no recorded exception")
...
class ConnectionPool:
    ...
    def __init__(
        ...
        self._available: Dict[str, asyncio.Queue[AISClient]] = {}
    ...
    async def call(
        ...
        **kwargs: Any
        ...
        raise TransportError(
            f"All servers failed for capability {capability}: {last_exception}",
            transport_type="pool"
        )
