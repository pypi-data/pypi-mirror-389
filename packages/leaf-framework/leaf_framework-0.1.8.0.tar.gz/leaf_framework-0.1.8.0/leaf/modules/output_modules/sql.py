...
class SQL(OutputModule):
    ...
    def retrieve(self, topic: str) -> list:
        """
        Retrieve all data associated with a specific topic.
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'SELECT data FROM messages WHERE topic = ? ORDER BY id',
                (topic,)
            )
            ...
            # Deserialize JSON data from each row
            results = []
            for row in rows:
                try:
