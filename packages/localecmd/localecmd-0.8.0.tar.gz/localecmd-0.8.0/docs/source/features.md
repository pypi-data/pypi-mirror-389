...
Sometimes it is useful to let functions share attributes in a class.
The instance can be converted to a module like a python module.
When creating the module, the instance must be given as prepending argument
since *self* (the instance) is the obligatory first parameter to a python method:
...
::::
:::{attention}
The inserted arguments not injected when calling the function directly.
