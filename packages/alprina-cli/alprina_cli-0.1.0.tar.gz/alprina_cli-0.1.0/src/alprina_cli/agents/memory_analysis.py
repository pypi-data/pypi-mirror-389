...
# Import actual CAI Memory Analysis Agent
try:
    from cai.agents import get_agent_by_name
    CAI_AVAILABLE = True
    logger.info("CAI Memory Analysis Agent available")
except ImportError as e:
    CAI_AVAILABLE = False
    logger.warning(f"CAI agents not available: {e}")


class MemoryAnalysisWrapper:
    """
    Wrapper for CAI Memory Analysis Agent.

    Provides synchronous interface to the async CAI agent.
    """

    def __init__(self):
        self.name = "Memory Analysis Agent"
        self.agent_type = "memory-forensics"
        self.description = "Memory forensics and analysis"
    ...
    def _get_cai_agent(self):
        ...
        if self._cai_agent is None:
            try:
                ...
                self._cai_agent = get_agent_by_name("memory_analysis_agent")
                logger.info("CAI Memory Analysis Agent initialized")
            except Exception as e:
                logger.error(f"Failed to initialize CAI Memory Analysis Agent: {e}")
                return None
    ...
    async def _scan_async(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = f"""Perform memory-forensics analysis on: {target}
...
Provide detailed findings with severity levels."""
            ...
            logger.error(f"CAI Memory Analysis Agent error: {e}")
            # Fallback to mock
            return self._mock_scan(target, safe_only)
    ...
    def _parse_cai_response(self, response: str, target: str) -> List[Dict[str, Any]]:
        ...
        if not findings and len(response) > 50:
            findings.append({
                "type": "Memory Artifact",
                "severity": "INFO",
                "title": "Memory Analysis Agent Analysis Complete",
    ...
    def scan(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with scan results
        """
        logger.info(f"Memory Analysis Agent scanning: {target} (safe_only={safe_only}, CAI={CAI_AVAILABLE})")
...
memory_analysis_agent = MemoryAnalysisWrapper()


def run_memory_analysis_scan(target: str, safe_only: bool = True) -> Dict[str, Any]:
    """
    Run security scan.
    ...
    Returns:
        Scan results
    """
    return memory_analysis_agent.scan(target, safe_only)
