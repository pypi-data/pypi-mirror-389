...
# Import actual CAI DFIR Agent
try:
    from cai.agents import get_agent_by_name
    CAI_AVAILABLE = True
    logger.info("CAI DFIR Agent available")
...
class DFIRWrapper:
    """
    Wrapper for CAI DFIR Agent.

    Provides synchronous interface to the async CAI agent.
    """

    def __init__(self):
        self.name = "DFIR Agent"
        self.agent_type = "forensics"
        self.description = "Digital forensics and incident response"
    ...
    def _get_cai_agent(self):
        ...
        if self._cai_agent is None:
            try:
                ...
                self._cai_agent = get_agent_by_name("dfir")
                logger.info("CAI DFIR Agent initialized")
            except Exception as e:
                logger.error(f"Failed to initialize CAI DFIR Agent: {e}")
                return None
    ...
    async def _scan_async(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = f"""Perform forensics analysis on: {target}
...
Provide detailed findings with severity levels."""
            ...
            logger.error(f"CAI DFIR Agent error: {e}")
            # Fallback to mock
            return self._mock_scan(target, safe_only)
    ...
    def _parse_cai_response(self, response: str, target: str) -> List[Dict[str, Any]]:
        ...
        if not findings and len(response) > 50:
            findings.append({
                "type": "Forensic Finding",
                "severity": "INFO",
                "title": "DFIR Agent Analysis Complete",
    ...
    def scan(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with scan results
        """
        logger.info(f"DFIR Agent scanning: {target} (safe_only={safe_only}, CAI={CAI_AVAILABLE})")
...
dfir_agent = DFIRWrapper()


def run_dfir_scan(target: str, safe_only: bool = True) -> Dict[str, Any]:
    """
    Run security scan.
    ...
    Returns:
        Scan results
    """
    return dfir_agent.scan(target, safe_only)
