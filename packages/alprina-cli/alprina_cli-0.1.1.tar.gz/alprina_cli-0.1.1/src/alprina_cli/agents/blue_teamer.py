...
# Import actual CAI Blue Team Agent
try:
    from cai.agents import get_agent_by_name
    CAI_AVAILABLE = True
    logger.info("CAI Blue Team Agent available")
except ImportError as e:
    CAI_AVAILABLE = False
    logger.warning(f"CAI agents not available: {e}")


class BlueTeamerWrapper:
    """
    Wrapper for CAI Blue Team Agent.

    Provides synchronous interface to the async CAI agent.
    """

    def __init__(self):
        self.name = "Blue Team Agent"
        self.agent_type = "defensive-security"
        self.description = "Defensive security posture assessment"
    ...
    def _get_cai_agent(self):
        ...
        if self._cai_agent is None:
            try:
                ...
                self._cai_agent = get_agent_by_name("blueteam_agent")
                logger.info("CAI Blue Team Agent initialized")
            except Exception as e:
                logger.error(f"Failed to initialize CAI Blue Team Agent: {e}")
                return None
    ...
    async def _scan_async(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = f"""Perform defensive-security analysis on: {target}
...
Provide detailed findings with severity levels."""
            ...
            logger.error(f"CAI Blue Team Agent error: {e}")
            # Fallback to mock
            return self._mock_scan(target, safe_only)
    ...
    def _parse_cai_response(self, response: str, target: str) -> List[Dict[str, Any]]:
        ...
        if not findings and len(response) > 50:
            findings.append({
                "type": "Defense Gap",
                "severity": "INFO",
                "title": "Blue Team Agent Analysis Complete",
    ...
    def scan(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with scan results
        """
        logger.info(f"Blue Team Agent scanning: {target} (safe_only={safe_only}, CAI={CAI_AVAILABLE})")
...
blue_teamer_agent = BlueTeamerWrapper()


def run_blue_team_scan(target: str, safe_only: bool = True) -> Dict[str, Any]:
    """
    Run security scan.
    ...
    Returns:
        Scan results
    """
    return blue_teamer_agent.scan(target, safe_only)
