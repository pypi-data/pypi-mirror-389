"""
Alprina Guardrails Agent

Safety validation before executing scans
...
# Import actual CAI Guardrails Agent
try:
    from cai.agents import get_agent_by_name
    CAI_AVAILABLE = True
    logger.info("CAI Guardrails Agent available")
...
class GuardrailsWrapper:
    """
    Wrapper for CAI Guardrails Agent.

    Provides synchronous interface to the async CAI agent.
    """

    def __init__(self):
        self.name = "Guardrails Agent"
        self.agent_type = "safety-check"
        self.description = "Safety validation before executing scans"
    ...
    def _get_cai_agent(self):
        ...
        if self._cai_agent is None:
            try:
                ...
                self._cai_agent = get_agent_by_name("guardrails")
                logger.info("CAI Guardrails Agent initialized")
            except Exception as e:
                logger.error(f"Failed to initialize CAI Guardrails Agent: {e}")
                return None
    ...
    async def _scan_async(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt = f"""Perform safety-check analysis on: {target}
...
Provide detailed findings with severity levels."""
            ...
            logger.error(f"CAI Guardrails Agent error: {e}")
            # Fallback to mock
            return self._mock_scan(target, safe_only)
    ...
    def _parse_cai_response(self, response: str, target: str) -> List[Dict[str, Any]]:
        ...
        if not findings and len(response) > 50:
            findings.append({
                "type": "Safety Check",
                "severity": "INFO",
                "title": "Guardrails Agent Analysis Complete",
    ...
    def scan(self, target: str, safe_only: bool = True) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with scan results
        """
        logger.info(f"Guardrails Agent scanning: {target} (safe_only={safe_only}, CAI={CAI_AVAILABLE})")
...
guardrails_agent = GuardrailsWrapper()


def run_guardrails_scan(target: str, safe_only: bool = True) -> Dict[str, Any]:
    """
    Run security scan.
    ...
    Returns:
        Scan results
    """
    return guardrails_agent.scan(target, safe_only)
