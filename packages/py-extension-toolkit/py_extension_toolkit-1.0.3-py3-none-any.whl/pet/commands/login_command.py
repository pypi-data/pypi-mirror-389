...
def login(server, username, password):
    ...
    if not server:
        server = click.prompt('🌐 Server URL', default='https://id.aqaryint.com')
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            expires_in = result['result']['expires_in']  # seconds
            user_info = result['result']['user']
            role_info = result['result']['role']
...
def logout():
    ...
    if session_file.exists():
        ...
        if session:
            ...
            session_file.unlink()
            click.echo(f"✅ Goodbye, {username}! You have been logged out.")
