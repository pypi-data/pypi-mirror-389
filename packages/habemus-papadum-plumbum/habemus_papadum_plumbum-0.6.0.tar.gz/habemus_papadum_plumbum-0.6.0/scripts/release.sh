...
"""Release automation script for plumbum.
...
INIT_PY = REPO_ROOT / "src" / "pdum" / "plumbum" / "__init__.py"
...
STEPS: list[Step] = [
    ...
    Step(
        ...
        description="Read version from pyproject.toml, __init__.py",
        category=StepCategory.VALIDATION,
    ...
    Step(
        ...
        description="Update version to release version in pyproject.toml, __init__.py",
...
def main() -> None:
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description="Automate the release process for plumbum")
    parser.add_argument(
        "bump_level",
    ...
    # Show title
    title = Panel.fit(
        "[bold cyan]plumbum Release Script[/bold cyan]\n\n"
