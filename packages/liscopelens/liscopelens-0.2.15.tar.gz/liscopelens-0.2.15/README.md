...
```shell
usage: liscopelens project_path clang [-h] --gn-file GN_FILE [--ignore-test] [--pass-sda] (--scancode-file SCANCODE_FILE | --scancode-dir SCANCODE_DIR) [--file-shadow FILE_SHADOW]
                                      [--rm-ref-lang] [--ignore-unk] [--license-shadow LICENSE_SHADOW] [--save-kg] [--output OUTPUT] [--echo]
...
options:
  ...
  --license-shadow LICENSE_SHADOW
                        The file path which storage license incompatibility shadow rules (JSON format: {license: [ignored_licenses]})
