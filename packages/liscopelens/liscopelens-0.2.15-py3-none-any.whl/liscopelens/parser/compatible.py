...
class BaseCompatiblityParser(BaseParser):
    ...
    arg_table = {
        ...
        "--license-shadow": {
            "type": str,
            "help": "The file path which storage license incompatibility shadow rules (JSON format: {license: [ignored_licenses]})",
            "default": None,
        },
    ...
    def __init__(self, args: argparse.Namespace, config: Config):
        ...
        # Load license shadow rules from file if provided
        self.license_shadow: dict[str, list[str]] = {}
        if license_shadow_path := getattr(args, "license_shadow", None):
            if os.path.exists(license_shadow_path):
                with open(license_shadow_path, "r", encoding="utf-8") as f:
                    self.license_shadow = json.load(f)
                print(f"Loaded license shadow rules from {license_shadow_path}")
            else:
                warnings.warn(f"License shadow file not found: {license_shadow_path}")
