...
class HpoaTableRow:
    def __init__(
            ...
            publication: str, 
            biocurator: str,  
            freq_num: typing.Optional[int]=None, 
            freq_denom: typing.Optional[int]=None):
    ...
    def get_dict(self) -> typing.Dict[str, str]:
...
class HpoaTableCreator:
    ...
    def __init__(
            self,
            phenopacket_list: typing.List[PPKt.Phenopacket],
            onset_term_d: typing.Dict[str, typing.List[CountedHpoTerm]],
            moi_d: typing.Dict[str, typing.List[HpTerm]],
            created_by: str,
    ) -> None:
        """Constructor
        ...
        :param onset_term_d: typing.Dict[str, HpTerm], key is a PMID string
        :type: Dict[str, HpTerm]
        :param moi_d: typing.Dict[str, str], key is PMID and value is Mode of inheritance
        """
    ...
    def _get_all_hpos(self) -> typing.Dict[str,HpTerm]:
    ...
    def _get_biocurator_d(self) -> typing.Dict[str, str]:
    ...
    def _count_hpos(self) -> typing.Dict[str, HpoaPmidCounter]:
        """
        We count the frequencies of HPO terms separately for each PMID using an HpoaPmidCounter object
        """
    ...
    def _add_age_of_onset_terms(self, onset_term_d) -> typing.List[HpoaTableRow]:
        """
        ...
        for pmid, oterm_list in onset_term_d.items():
            ...
            if biocurator is None:
                raise ValueError(f"Cannot calculate onset rows without non-None biocurator")
    ...
    def _add_moi_rows(self, moi_d) -> typing.List[HpoaTableRow]:
    ...
    def get_dataframe(self) -> pd.DataFrame:
        ...
        for pmid, counter in self._hpo_counter_d.items():
            ...
            for hpo_id in measured_d:
                ...
                if hpo_term is None:
                    raise ValueError(f"Cannot calculate onset rows with missing hpo_term for id {hpo_id}")
    ...
    def write_data_frame(self) -> None:
...
class HpoaTableBuilder:
    ...
    HpoaTableBuilder
    ----------
    _onset_term_d : DefaultDict[str, List[str]]
        Maps onset terms (the HPO ids) to lists of CountedHpoTerms. Initialized with defaultdict(list).
    _moi_d: typing.Dict[str, typing.List[HpTerm]]
        Maps PMIDs to lists of HpTerms representing a mode of inheritance
    """
    _onset_term_d: typing.Dict[str, typing.List[CountedHpoTerm]]
    _moi_d: typing.Dict[str, typing.List[HpTerm]]
    ...
    def filter_diseases(
        ...
        """
        Filter the phenopackets to get just those that correspend to disease_id 
        This is useful because here we ingest phenopackets to make HPOA, but
        the phenopackets folder may have different diseases, and we have the small files be one disease only.
        """
        target_list = list()
    ...
    def build(self):
        if self._created_by is None:
            raise ValueError("Cannot build HPOA with null created_by string")
    ...
    @staticmethod
    def from_individuals(individual_list: typing.List[Individual], created_by:str):
