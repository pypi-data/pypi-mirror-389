...
from bigconsole_sdk.client.base_client import AuthTokens, BaseGraphQLClient, BigconsoleSDKConfig
...
def test_headers_generation():
    ...
    config = BigconsoleSDKConfig(endpoint="https://api.test.com/graphql", api_key="test-key")
    client = BaseGraphQLClient(config)
...
async def test_graphql_error_response():
    ...
    # Mock error response
    mock_response = AsyncMock()
    mock_response.json = AsyncMock(return_value={"errors": [{"message": "User not found"}]})
    mock_response.raise_for_status = AsyncMock()
