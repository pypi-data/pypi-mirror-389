...
def query_enterpriseDirectory_parse(query_subparsers):
    query_enterpriseDirectory_parser = query_subparsers.add_parser('enterpriseDirectory', 
            help='enterpriseDirectory() query operation', 
            usage=get_help("query_enterpriseDirectory"), formatter_class=CustomSubparserHelpFormatter)

    query_enterpriseDirectory_parser.add_argument('json', nargs='?', default='{}', help='Variables in JSON format (defaults to empty object if not provided).')
    query_enterpriseDirectory_parser.add_argument('-accountID', help='The cato account ID to use for this operation. Overrides the account_id value in the profile setting.  This is use for reseller and MSP accounts to run queries against cato sub accounts from the parent account.')
