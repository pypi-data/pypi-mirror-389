...
def query_accountMetrics_parse(query_subparsers):
    query_accountMetrics_parser = query_subparsers.add_parser('accountMetrics', 
            ...
            usage=get_help("query_accountMetrics"), formatter_class=CustomSubparserHelpFormatter)
    ...
    query_accountMetrics_parser.add_argument('-accountID', help='The cato account ID to use for this operation. Overrides the account_id value in the profile setting.  This is use for reseller and MSP accounts to run queries against cato sub accounts from the parent account.')
    ...
    query_accountMetrics_parser.add_argument('-n', '--stream-events', dest='stream_events', help='Send events over network to host:port TCP')
