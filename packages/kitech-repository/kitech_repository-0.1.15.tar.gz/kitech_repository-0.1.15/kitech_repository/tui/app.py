...
class KitechTUI(App):
    ...
    async def _handle_download_single(self, operation) -> None:
        ...
        logger.info(
            f"Calling client.download_file with repo_id={self.current_repository_id}, path={operation.remote_path}"
        )
    ...
    async def _handle_download_folder(self, operation) -> None:
        ...
        logger.info(
            f"Getting download info from API... (repo_id={self.current_repository_id}, path={operation.remote_path!r})"
        )
        try:
        ...
        if isinstance(download_info, dict) and "files" in download_info:
            ...
            files = [f for f in all_items if not f.get("isDir") and not f.get("isDirectory")]
            ...
            async def download_single_file(file_index: int, file_info: dict) -> tuple[int, str, bool, str | None]:
                ...
                async with semaphore:
                    try:
                        ...
                        progress_msg = FileOperationProgress(
                            operation_id=operation.operation_id, progress_percent=progress
                        )
                        self.post_message(progress_msg)
                        self._relay_to_progress_panel_progress(progress_msg)
            ...
            if failed:
                ...
                completed_msg = FileOperationCompleted(
                    operation_id=operation.operation_id, success=False, error=error_msg
                )
            else:
    ...
    async def _handle_upload_single(self, operation) -> None:
        ...
        logger.info(
            f"Calling client.upload_file with repo_id={self.current_repository_id}, file_path={operation.local_path}"
        )
