...
class ZipSecurityValidator:
    ...
    def _check_zip_bomb(self, zip_ref: zipfile.ZipFile) -> None:
        ...
        if total_uncompressed > self.MAX_UNCOMPRESSED_SIZE:
            raise DownloadError(f"ZIP file too large: {total_uncompressed / 1024**3:.2f}GB")

        if total_compressed > 0 and (total_uncompressed / total_compressed) > self.MAX_EXTRACTION_RATIO:
            raise DownloadError(
...
class ZipExtractor:
    ...
    def extract(self, zip_path: Path, output_dir: Path, requested_path: str | None = None) -> tuple[Path, list[str]]:
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            ...
            actual_files = (
                [f for f in zip_contents if not f.endswith(".sha256")] if self.filter_checksums else zip_contents
            )
            ...
            result = self._determine_result_path(actual_files, output_dir, requested_path)
    ...
    def _validate_paths(self, files: list[str], output_dir: Path) -> None:
        ...
        for file in files:
            ...
            try:
                ...
                raise DownloadError(f"Path traversal attempt detected: {file}") from None

    def _determine_result_path(self, extracted_files: list[str], output_dir: Path, requested_path: str | None) -> Path:
