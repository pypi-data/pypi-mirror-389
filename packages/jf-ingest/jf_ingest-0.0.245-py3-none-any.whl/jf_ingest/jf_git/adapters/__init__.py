            ...
            if use_async:
                with tqdm_to_logger(
                    ...
                    def batch_process_commits(
                        ...
                        commit_results = [commit for commit in commit_function()]
                        result = len(commit_results)
                        dict_results = self._transform_data_objects_before_saving(commit_results)
                        _write_to_s3_or_local(
                            object_name=GitObject.GitCommits.value,
                            json_data=dict_results,
            ...
            if use_async:
                with tqdm_to_logger(
                    ...
                    def batch_process_prs(
                        ...
                        pr_results = [pr for pr in pr_function()]
                        result = len(pr_results)
                        dict_results = self._transform_data_objects_before_saving(pr_results)
                        _write_to_s3_or_local(
