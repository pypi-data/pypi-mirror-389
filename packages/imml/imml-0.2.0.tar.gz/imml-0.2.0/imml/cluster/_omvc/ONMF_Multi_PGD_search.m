function [U_total, V, U_star_total, Loss] = ONMF_Multi_PGD_search(X, option, n_samples, incomplete_index, block_size)

    for i = 1:numel(X)
        W{i} = diag(sparse(ones(n_samples,1)));
        if exist('incomplete_index')
            counter = 0;
