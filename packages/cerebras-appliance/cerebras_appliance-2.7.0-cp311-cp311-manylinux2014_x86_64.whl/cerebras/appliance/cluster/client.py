...
class ClusterManagementClient:
    ...
    def get_job_handle(
        ...
        service_url_to_check = ingress_response.service_url

        coordinator_ip_port = None
        # Extract coordinator pod IP for in-cluster connections
        if self._is_in_cluster:
            coordinator_ip_port = self.get_coordinator_ip_port_from_run_meta(
                job_id
            )
            if coordinator_ip_port:
                response["coordinator_ip_port"] = coordinator_ip_port
                logger.debug(
                    f"Added coordinator IP:port for in-cluster connection: {coordinator_ip_port}"
                )
                service_url_to_check = coordinator_ip_port
        ...
        connectivity_ok = (
            ingress_response.service_url
            and self.validate_connectivity(service_url_to_check, False)
        )

        if connectivity_ok:
            response["service_url"] = str(ingress_response.service_url)
            if self._is_in_cluster:
                service_url = coordinator_ip_port
                response["service_url"] = service_url
        elif self._is_in_cluster:
            # Raise exception if in-cluster and connectivity failed
            raise RuntimeError(
                f"Failed to connect to {service_url_to_check} from in-cluster environment."
            )
    ...
    def dump_job_meta(self, job_id):
        ...
        # Store coordinator IP:port in run meta for in-cluster connections
        if self._is_in_cluster:
    ...
    def list_workload_manager(self, request):
        ...
        logger.debug(
            f"Found {len(response.workload_managers)} workload managers"
        )
        return response
    ...
    def _detect_in_cluster(self) -> bool:
        ...
        """
        token_exists = Path(K8S_SERVICE_ACCOUNT_TOKEN_PATH).exists()
        env_set = "KUBERNETES_SERVICE_HOST" in os.environ
        in_cluster = token_exists and env_set
        logger.info(
            f"Detecting if running inside a Kubernetes cluster... sa_token_exists={token_exists}, env_set={env_set} => in_cluster={in_cluster}"
        )
        return in_cluster
