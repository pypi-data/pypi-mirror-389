...
class Fringes:
    ...
    _choices = {
        ...
        "dtype": {
            ...
            "uint16",
            "uint32",
            # "uint64",  # todo: integer overflow in pyqtgraph -> replace line 685 of ImageItem.py with: bins = self._xp.arange(mn, mx + 1.01 * step, step, dtype="uint64")
    ...
    def __init__(
        ...
        *args,  # bundles all args, what follows are only kwargs
        ...
        v: float | Sequence[float] | str = ((13.0, 7.0), (13.0, 7.0)),
        f: float | Sequence[float] = ((1.0, 1.0), (1.0, 1.0)),
        ...
        # set default values to private attributes used by properties
    ...
    def _demodulate(
        ...
        if self.uwr == "FTM":
            ...
            try:
                ...
                if threads:
                    threads = min(max(1 - max_threads, threads), max_threads)
                    if threads < 0:
                        threads += max_threads
    ...
    def decode(
        ...
        # return namedtuple
        if verbose:
            return _Decoded_verbose(a, b, x, p, k, r, u)
        else:
            return _Decoded(a, b, x)
    ...
    def uncertainty(
        ...
        # todo: :attr:`~fringes.fringes.Fringes.u` denotes the minimum possible uncertainty of the measurement in pixels.
        ...
        ux = up / _2PI * l  # position uncertainties
        u = np.sqrt(1 / np.sum(1 / ux**2, axis=1))  # position uncertainty
    ...
    @grid.setter
    def grid(self, grid: Literal["image"]):  # todo: , "Cartesian", "polar", "log-polar"]):
        _grid = str(grid)
    ...
    @f.setter
    def f(self, f: float | Sequence[float]):
    ...
    def uq(self):
        ...
        uq = 1 / np.sqrt(12)  # quantization noise
        up = np.sqrt(2) / np.sqrt(self.M) / np.sqrt(self._N) * uq / self.B  # phase uncertainties  # todo: M
        ux = up / _2PI * self._l  # position uncertainties
        u = np.sqrt(1 / np.sum(1 / ux**2, axis=1))  # position uncertainty
        return u
