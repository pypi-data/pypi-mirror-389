...
def validate_path(path_str: str, allow_write: bool = False) -> Path:
    ...
    Args:
        path_str: Relative path from apps directory
    ...
    try:
        ...
        # Resolve relative to apps directory
        target_path = (APPS_DIR / path_str).resolve()
        ...
        # For write operations, restrict to data directories
        if allow_write:
            # Check if path is in a data directory
            parts = target_path.relative_to(APPS_DIR).parts
            if len(parts) < 2 or parts[1] != 'data':
                raise HTTPException(
                    status_code=403,
                    detail=f"Write access denied: Can only write to app data directories (apps/{{app-name}}/data/...)"
                )
    ...
    try:
        for item in sorted(target_path.iterdir()):
            ...
            items.append({
                ...
                "path": str(item.relative_to(APPS_DIR))
            })
