...
def _localize_horizontal_bounds(
    ...
    def thresholded_from_center(deriv: np.ndarray, center: int, threshold: float = 0.4) -> tuple[int, int]:
        an_l, an_r = -1, -1
    ...
    an_left, an_right = thresholded_from_center(g1, center_idx, threshold)
...
def detect_bbox(work_color: np.ndarray, work_gray: np.ndarray, threshold: float) -> BoundingBox:
    top, bottom = _localize_vertical_bounds(work_gray)
    left, right = _localize_horizontal_bounds(work_gray, top, bottom, threshold)
...
def process_image(
    ...
    bbox = detect_bbox(rect_color, rect_gray, threshold)
